...
namespace GJLocal.HISFC.Components.OpGuide.LisBillPrint
{
            ...
            orderList.GroupBy(o => o.Combo.ID).ToList().ForEach(g =>
            {
                ...
                g.OrderBy(s => s.SubCombNO)
                    .ToList().ForEach(order =>
                    {
                        ...
                        labelitems +=eName  + (order.IsEmergency ? "【急】" : "") + (string.IsNullOrEmpty(order.Memo) ? "" : "（备注:" + order.Memo + "）") + "  数量:" + order.Qty.ToString() + order.Unit + "  " + order.Qty * order.Item.Price + "元" + "  " + order.Sample.Name + "\n";
