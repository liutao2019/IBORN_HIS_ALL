...
namespace GJLocal.HISFC.Components.OpGuide.InDrugStore.InPatient
{
    ...
    public partial class ucNormalDrugBill : UserControl, FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.IInpatientBill
    {
        ...
        private void ShowDetailData(ArrayList alData, FS.HISFC.Models.Pharmacy.DrugBillClass drugBillClass, FS.FrameWork.Models.NeuObject stockDept)
        {
            ...
            this.nlbDocName.Text = "医生：" + FS.SOC.HISFC.BizProcess.Cache.Common.GetEmployeeName((alData[0] as FS.HISFC.Models.Pharmacy.ApplyOut).RecipeInfo.ID);

            this.nlbOrderCount.Text = "医嘱数量  " + alData.Count.ToString();

            this.nlbDrugDate.Text = "配药时间： " + (alData[0] as FS.HISFC.Models.Pharmacy.ApplyOut).Operation.ExamOper.OperTime.ToString();
            ...
            foreach (FS.HISFC.Models.Pharmacy.ApplyOut info in alData)
            {
                ...
                if (patient == null || string.IsNullOrEmpty(patient.ID))
                {
                    ...
                    //病区
                    this.nlblPatientDept.Text = "申请科室：" + FS.SOC.HISFC.BizProcess.Cache.Common.GetDeptName(drugBillClass.ApplyDept.ID);
                ...
                //药品名称
                this.neuSpread1.SetCellValue(0, iRow, "名     称", info.Item.Name);
                ...
                this.neuSpread1.SetCellValue(0, iRow, "数量", applyQty.ToString("F2").TrimEnd('0').TrimEnd('.') + unit);
                this.neuSpread1.SetCellValue(0, iRow, "货位号", FS.SOC.HISFC.BizProcess.Cache.Pharmacy.GetItemPlaceNo(info.StockDept.ID, info.Item.ID));
                this.neuSpread1.SetCellValue(0, iRow, "使用方法", FS.SOC.HISFC.BizProcess.Cache.Common.GetUsageName(info.Usage.ID));

                //服药次数
                this.neuSpread1.SetCellValue(0, iRow, "服药次数", Common.Function.GetFrequenceName(info.Frequency));

                //每次用量
                this.neuSpread1.SetCellValue(0, iRow, "每次用量", info.DoseOnce.ToString("F2").TrimEnd('0').TrimEnd('.') + info.Item.DoseUnit);
                //金额
                ...
                this.neuSpread1.SetCellValue(0, iRow, "金额", drugTotalPrice.ToString("F2"));
                //开单医生
                this.neuSpread1.SetCellValue(0, iRow, "开单医生", FS.SOC.HISFC.BizProcess.Cache.Common.GetEmployeeName(info.RecipeInfo.ID));
                //用药说明
            ...
            this.nlbTotCost.Text = drugListTotalPrice.ToString("F2");
          
            this.neuPanel12.Dock = DockStyle.None;
            this.neuPanel13.Dock = DockStyle.None;
            this.neuSpread1.Dock = DockStyle.None;
            this.neuPanel12.Location = new Point(0, this.neuPanel1.Height);
            this.neuSpread1.Height = (int)(this.neuSpread1_Sheet1.ColumnHeader.Rows.Default.Height + alData.Count * this.neuSpread1_Sheet1.Rows.Default.Height);
            this.neuPanel12.Height = this.neuSpread1.Height + 10;
            this.neuPanel13.Location = new Point(0, this.neuPanel1.Height + this.neuPanel12.Height);
            this.neuPanel13.Height = this.Height - this.neuPanel12.Height - this.neuPanel1.Height;
            this.neuPanel13.Dock = DockStyle.Bottom;
            this.neuPanel12.Dock = DockStyle.Fill;
