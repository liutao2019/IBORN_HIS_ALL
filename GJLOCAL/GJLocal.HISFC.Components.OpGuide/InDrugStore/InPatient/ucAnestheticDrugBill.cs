...
namespace GJLocal.HISFC.Components.OpGuide.InDrugStore.InPatient
{
    ...
    public partial class ucAnestheticDrugBill : UserControl, FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.IInpatientBill
    {
        ...
        {
            this.Clear();
            allDataTmp = new ArrayList();
            foreach (FS.HISFC.Models.Pharmacy.ApplyOut info in alData)
            {
                allDataTmp.Add(info.Clone() as FS.HISFC.Models.Pharmacy.ApplyOut);
            }
        ...
        private void ShowDetailData(ArrayList alData, FS.HISFC.Models.Pharmacy.DrugBillClass drugBillClass, FS.FrameWork.Models.NeuObject stockDept)
        {
            ...
            foreach (FS.HISFC.Models.Pharmacy.ApplyOut info in alData)
            {
                ...
                if (samePatient != info.PatientNO)
                {
                    ...
                    patientInfo = string.Format("{0}  {1}住院流水号：{2}   年龄：{3}", bedNO, FS.SOC.Public.String.PadRight(privPatientName, 8, ' '), info.PatientNO, age);
                    //如果不是在本页的第一行，需要患者信息
                ...
                this.neuSpread1.SetCellValue(0, iRow, "编码", FS.SOC.HISFC.BizProcess.Cache.Pharmacy.GetItemCustomNO(info.Item.ID));
                ...
                {
                    this.neuSpread1.SetCellValue(0, iRow, "用法", FS.SOC.HISFC.BizProcess.Cache.Common.GetUsageName(info.Usage.ID));
                }
                else
                {
                ...
                this.neuSpread1_Sheet1.Cells[iRow, 8].Text = info.DoseOnce.ToString("F2").TrimEnd('0').TrimEnd('.') + info.Item.DoseUnit;
                ...
                this.neuSpread1.SetCellValue(0, iRow, "总量", applyQty.ToString("F2").TrimEnd('0').TrimEnd('.') + unit);
                ...
                this.neuSpread1.SetCellValue(0, iRow, "单价", price.ToString("F2").TrimEnd('0').TrimEnd('.'));
                this.neuSpread1.SetCellValue(0, iRow, "金额", (info.Operation.ApplyQty / info.Item.PackQty * info.Item.PriceCollection.RetailPrice).ToString("F2").TrimEnd('0').TrimEnd('.'));
        ...
        public void PrintData(ArrayList alData, FS.HISFC.Models.Pharmacy.DrugBillClass drugBillClass, FS.FrameWork.Models.NeuObject stockDept)
        {
            ...
            ucAnestheticByPatientDrugBill ucAnestheticByPatientDrugBill = new ucAnestheticByPatientDrugBill();
            ucAnestheticByPatientDrugBill.PrintData(alData, drugBillClass, stockDept);
        }
        ...
        {
            ucAnestheticByPatientDrugBill ucAnestheticByPatientDrugBill = new ucAnestheticByPatientDrugBill();
            ucAnestheticByPatientDrugBill.PrintData(this.allDataTmp, drugBillClassTmp, stockDeptTmp);
        }
