...
namespace GJLocal.HISFC.Components.OpGuide.InDrugStore.InPatient.IBORN
{
    ...
    public partial class ucTotalAnestheticDrugBill : UserControl, FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.IInpatientBill
    {
        public ucTotalAnestheticDrugBill()
        {
            InitializeComponent();
            ...
            this.nlbRowCount.Visible = false;
        }
        ...
        public void ShowDetailData(ArrayList alData, FS.HISFC.Models.Pharmacy.DrugBillClass drugBillClass, FS.FrameWork.Models.NeuObject stockDept)
        {
            ...
            {
                if (drugBillClass.ID == "QS") //{AA5A1F7C-9D5E-4cf9-80AB-E86CC29B9CA6}
                    this.lblTitleName.Text = "全身麻醉剂领药-发药单";
                else if (drugBillClass.ID == "ZDP2")
                    this.lblTitleName.Text = "精二药品重点品种领药-发药单";
                else
                    this.lblTitleName.Text = "麻醉药品、第一类精神药品领药-发药单";// drugBillClass.Name + "(汇总)";//+ "(" + sendType + ")";
            ...
            if (drugBillClass.ID == "ZDP2")//重点精二摆药单-发药单-领药科室：申请科室 {AA5A1F7C-9D5E-4cf9-80AB-E86CC29B9CA6}
            {
                this.nlbNurseCell.Text = string.Format(" 领药科室：{0}", applyDeptName);
            }
            else
                this.nlbNurseCell.Text = " 领药科室：麻醉手术中心";
            //this.nlbNurseCell.Text = " 领药科室：麻醉手术中心";

            if (FS.FrameWork.Function.NConvert.ToInt32(drugBillClass.ApplyState) != 0)
            {
                this.nlbReprint.Visible = false;// true;
            }
            else
            {
                this.nlbReprint.Visible = false;
            ...
            var applylistinfo = (from r in applyList group r by new { r.Item.Name, r.Item.Specs, r.Item.PackUnit, r.Item.MinUnit } into rr select new { name = rr.Key.Name, ordernocount = rr.Sum(r => r.Operation.ApplyQty), specs = rr.Key.Specs, unit = rr.Key.PackUnit,minunit=rr.Key.MinUnit });


            //string b = "";
            //foreach (var a in applylistinfo)
            //{
            //    b += a.name + " " + a.ordernocount;
            //    MessageBox.Show(b);
            //}

            int a=0;

            foreach (var info in applylistinfo)
            {

                a=a+1;
                int cou = (from r in applyList where r.Item.Name == info.name select r.OrderNO).Count();
                ...
                this.neuSpread1.SetCellValue(0, iRow, "药名", info.name);

                //{0CF0D6CC-172A-405d-96B8-C54DAFCA9EDD}

                this.neuSpread1.SetCellValue(0, iRow, "规格", info.specs);
                this.neuSpread1.SetCellValue(0, iRow, "单位", info.minunit);
                this.neuSpread1.SetCellValue(0, iRow, "请领数量", info.ordernocount);
                this.neuSpread1.SetCellValue(0, iRow, "交回处方数量（张）", cou);
                this.neuSpread1.SetCellValue(0, iRow, "处方总用量", info.ordernocount);
                this.neuSpread1.SetCellValue(0, iRow, "交回空安瓿数量", info.ordernocount);
                ...
                this.neuSpread1_Sheet1.Cells[iRow, 13].Border = noBottomBorder;
                iRow++;
                id++;
            }
            ...
            this.lblOrderDate.Text = "领药时间：" + DateTime.Now.ToShortDateString();// startTime.ToShortDateString() + " 至 " + enTime.ToShortDateString();
                ...
                if (this.neuSpread1_Sheet1.Rows.Count > 0)
                {
                    ...
                    for (int page = totPageNO; page > 0; page--)
                    {
                        if (page == totPageNO)
                        {
                            ...
                            this.neuSpread1_Sheet1.Cells[index, 0].Text = "       请领人：                             审核人（部门负责人）：     ";
