    ...
    public partial class ucReicpeInfoLabelIBORN : UserControl
    {
        ...
        public int PrintDrugLabel(System.Collections.ArrayList alData, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal, bool isPrintRegularName, string hospitalName, bool isPrintMemo, string diagnose, DateTime printTime)
        {
            ...
            foreach (FS.HISFC.Models.Pharmacy.ApplyOut applyInfo in alData)
            {
                ...
                this.lbPatientName1.Text = drugRecipe.PatientName;
                int strLength = this.LengthString(drugRecipe.PatientName);
                if (strLength > 18)
                {
                ...
                //appleinfo中的频次名称取的是pha_com_applyout中的频次名称，与字典中的对不上，此处置空，让其重新取名称
                //具体为何对不上，需要跟踪收费函数，暂时未查
                applyInfo.Frequency.Name = string.Empty;
                this.nlbUsageName.Text = applyInfo.Usage.Name + "，" + Common.Function.GetFrequenceName(applyInfo.Frequency) + "，" + "每次" + Common.Function.GetOnceDose(applyInfo);

               
                this.nlbPageNO.Text = index + "-" + alData.Count;
                this.lblPrintTime.Text = applyInfo.Operation.ApplyOper.OperTime.ToString("yyyy-MM-dd HH:mm"); //DateTime.Now.ToString("yyyy-MM-dd HH:mm");
                this.nlbName.Text = "门诊取药";
                ...
                //this.npbCardNo.Image = this.CreateBarCode(drugRecipe.CardNO);
                //{A7E432B5-97C4-4cca-A9A6-726301F4B38F}
