...
namespace GJLocal.HISFC.Components.OpGuide.DrugStore.Common
{
    /// <summary>
    /// [功能描述: 本地化公用函数]<br></br>
    /// [创 建 者: cube]<br></br>
    ...
    public class Function
    {
        ...
        public static string GetOnceDose(FS.HISFC.Models.Pharmacy.ApplyOut applyOut)
        {
            ...
            //{F7CBC9BE-7E88-4a3d-BAB4-04A2F9E34450}
            if (applyOut.Item.DoseUnit == item.DoseUnit && applyOut.DoseOnce / item.BaseDose == ((int)(applyOut.DoseOnce / item.BaseDose)))
            {
                return ((int)(applyOut.DoseOnce / item.BaseDose)).ToString("F2").TrimEnd('0').TrimEnd('.') + item.MinUnit;
            }
            else if (applyOut.Item.DoseUnit == item.MinUnit && applyOut.DoseOnce != (int)applyOut.DoseOnce)
            {
                return (applyOut.DoseOnce * item.BaseDose).ToString("F2").TrimEnd('0').TrimEnd('.') + item.DoseUnit + " ";
            }
            else
            {
                return applyOut.DoseOnce.ToString("F2").TrimEnd('0').TrimEnd('.') + applyOut.Item.DoseUnit;
            ...
            else
            {
                    ...
                    || (item.SecondBaseDose == 1m && applyOut.Item.DoseUnit == item.SecondDoseUnit))
                {
                    ...
                    {
                        labelPrintConvertAways = FS.SOC.Public.XML.SettingFile.ReadSetting(fileName, "Label", "OnceDoseConvertAways", "False");
                    }
                    else
                    {
                        FS.SOC.Public.XML.SettingFile.SaveSetting(fileName, "Label", "OnceDoseConvertAways", "False");
                    }
            ...
            //如果剂量单位是中文
            if (!FS.SOC.Public.Char.IsLetterOrNumber(applyOut.Item.DoseUnit[0]))
            {
                return ConvertDoseOnce(applyOut.DoseOnce, applyOut.Item.DoseUnit) + applyOut.Item.DoseUnit;
            }           
        ...
        private static string ConvertDoseOnce(decimal value,string doseUnit)
        {
            ...
            //英文数字单位不转换
            if (FS.SOC.Public.Char.IsLetterOrNumber(doseUnit[0]))
            {
                return value.ToString("F2").TrimEnd('0').TrimEnd('.');
            }
        ...
        /// <summary>
        /// 获取频次名称{5194480D-3CBA-4866-877A-8BC7464502F1}
        /// </summary>
        /// <param name="frequencyID"></param>
        /// <returns></returns>
        public static string GetFrequenceName(string frequencyID)
        {
            ...
            FS.FrameWork.Management.DataBaseManger dbMgr = new FS.FrameWork.Management.DataBaseManger();
            string fName = dbMgr.ExecSqlReturnOne("SELECT FREQUENCY_NAME FROM MET_COM_DEPTFREQUENCY WHERE FREQUENCY_CODE ='" + frequencyID + "'");
            if (fName != "-1")
            {
                return fName;
            }
        ...
        /// <param name="frequency"></param>
        /// <returns></returns>
        public static string GetFrequenceTimesPerDay(FS.HISFC.Models.Order.Frequency frequency)
        {

            FS.FrameWork.Management.DataBaseManger dbMgr = new FS.FrameWork.Management.DataBaseManger();
            string fName = dbMgr.ExecSqlReturnOne("SELECT ENGLISH_AB FROM MET_COM_DEPTFREQUENCY WHERE FREQUENCY_CODE ='" + frequency.ID + "'");
            if (fName != "-1")
            {
                return fName;
            }
        ...
        /// <summary>
        /// 获取频次每天执行次数/// 根据频次编号获取名称{A46B15F7-CA11-4E0C-93E1-341AFD679963} ccz添加
        /// </summary>
        /// <param name="frequency"></param>
        /// <returns></returns>
        public static string GetFrequenceTimesPerDay(string code)
        {

            FS.FrameWork.Management.DataBaseManger dbMgr = new FS.FrameWork.Management.DataBaseManger();
            string fName = dbMgr.ExecSqlReturnOne("SELECT ENGLISH_AB FROM MET_COM_DEPTFREQUENCY WHERE FREQUENCY_CODE ='" + code+ "'");
            if (fName != "-1")
            {
                return fName;
            }
        ...
        /// <param name="frequency"></param>
        /// <returns></returns>
        public static string GetDosageFormName(string drug_code)
        {

            FS.FrameWork.Management.DataBaseManger dbMgr = new FS.FrameWork.Management.DataBaseManger();
            string fName = dbMgr.ExecSqlReturnOne("select c.name from pha_com_baseinfo b ,com_dictionary c where b.dose_model_code=c.code and c.type ='DOSAGEFORM' and b.drug_code ='" + drug_code + "'");
            if (fName != "-1")
            {
                return fName;
            }
        ...
        {
            return FS.FrameWork.Function.NConvert.ToBoolean(FS.SOC.Public.XML.SettingFile.ReadSetting(FS.FrameWork.WinForms.Classes.Function.CurrentPath + "\\Profile\\InpatientDrugStorePrintSetting.xml", "BillNO" + drugBillClassNO, "PrintInpatientDrugBag", "0"));
        }
        /// <summary>
        /// 获取摆药单是否打印标签
        /// </summary>
        ...
        {
            return FS.FrameWork.Function.NConvert.ToBoolean(FS.SOC.Public.XML.SettingFile.ReadSetting(FS.FrameWork.WinForms.Classes.Function.CurrentPath + "\\Profile\\InpatientDrugStorePrintSetting.xml", "BillNO" + drugBillClassNO, "PrintInpatientDrugLabel", "0"));
        }
        /// <summary>
        /// 获取明细打印为主的摆药单是否打印汇总单
        /// </summary>
        ...
        {
            return FS.FrameWork.Function.NConvert.ToBoolean(FS.SOC.Public.XML.SettingFile.ReadSetting(FS.FrameWork.WinForms.Classes.Function.CurrentPath + "\\Profile\\InpatientDrugStorePrintSetting.xml", "BillNO" + drugBillClassNO, "PrintInpatientDrugTot", "0"));
        }
