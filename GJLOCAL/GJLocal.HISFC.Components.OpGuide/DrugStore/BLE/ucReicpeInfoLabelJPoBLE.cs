...
{
    public partial class ucReicpeInfoLabelJPoBLE : UserControl
    {
        public ucReicpeInfoLabelJPoBLE()
        {
            InitializeComponent();
        }
        ...
        public int PrintDrugLabel(System.Collections.ArrayList alData, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal, bool isPrintRegularName, string hospitalName, bool isPrintMemo, string diagnose, DateTime printTime)
        {
            ...
            foreach (FS.HISFC.Models.Pharmacy.ApplyOut applyInfo in alData)
            {
                ...
                this.lbAge.Text = (new FS.FrameWork.Management.DataBaseManger()).GetAge(drugRecipe.Age);
                if (FS.FrameWork.Function.NConvert.ToInt32(this.lbAge.Text.Trim()) < 15)
                {
                    this.lbAge.Visible = true;
                }
                else
                {
                ...
                //}
                string strTemp = "({0})日分　　一日({1})回　（一回に飲む量）";
                string time = Common.Function.GetFrequenceTimesPerDay(applyInfo.Frequency);
                string days = applyInfo.Days.ToString();
                strTemp = string.Format(strTemp, days, time);
                this.lbTimeAndDay1.Text = strTemp;
                this.lbTimeAndDay.Text = Common.Function.GetFrequenceName(applyInfo.Frequency); //strTemp;
