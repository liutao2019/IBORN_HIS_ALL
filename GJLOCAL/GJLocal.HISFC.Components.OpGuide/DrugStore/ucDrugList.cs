...
namespace GJLocal.HISFC.Components.OpGuide.DrugStore
{
    public partial class ucDrugList : UserControl
    {
        ...
        private void SetLableValue(FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe,string diagnose,string hospitalName)
        {
            ...
            this.lbDoctor.Text = FS.SOC.HISFC.BizProcess.Cache.Common.GetEmployeeName(drugRecipe.Doct.ID);
            //处方号
            this.lbRecipe.Text = "处方号：" + drugRecipe.RecipeNO;
            //科室名称
            lbDeptName.Text = "科室名称：" + FS.SOC.HISFC.BizProcess.Cache.Common.GetDeptName(drugRecipe.DoctDept.ID);
            //诊断
        ...
        {
            //自定义码
            this.neuSpread1_Sheet1.SetText(iRow, 0, FS.SOC.HISFC.BizProcess.Cache.Pharmacy.GetItemCustomNO(info.Item.ID));
            ...
            try
            {
                this.neuSpread1_Sheet1.SetText(iRow, 7, FS.SOC.HISFC.BizProcess.Cache.Common.GetUsage(info.Usage.ID).Name);
            }
            catch { }
            //每次用量
            if (info.DoseOnce == 0)
            ...
            try
            {
                this.neuSpread1_Sheet1.Cells[iRow,11].Text = FS.SOC.Local.DrugStore.ZhuHai.Common.Function.GetOrder(info.OrderNO).HerbalQty.ToString();
            }
            catch { }
