...
{
    public partial class ucFinIpbInpatientFeeList :FSDataWindow.Controls.ucQueryBaseForDataWindow 
    {
        ...
        //FS.HISFC.BizLogic.RADT.InPatient managerRADTInPatient = new FS.HISFC.BizLogic.RADT.InPatient();
        FS.HISFC.BizLogic.RADT.InPatient managerRADTInPatient = new FS.HISFC.BizLogic.RADT.InPatient(); 
        /// <summary>
        ...
        private void getInpatientNO()
        {
            ...
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("您输入的住院号错误,请重新输入!"));
                this.inpatientNo = "";
                return;
            }

            FS.HISFC.Models.RADT.PatientInfo patientInfo = this.managerRADTInPatient.QueryPatientInfoByInpatientNO(this.ucQueryInpatientNo1.InpatientNo);
            if (patientInfo == null || patientInfo.ID == null || patientInfo.ID == string.Empty)
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("获取患者基本信息出错!") + this.managerRADTInPatient.Err);
                this.inpatientNo = "";
                return;
            }
