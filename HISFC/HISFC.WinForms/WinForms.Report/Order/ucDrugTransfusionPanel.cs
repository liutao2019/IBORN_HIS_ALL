...
namespace FS.WinForms.Report.Order
{
    ...
    public partial class ucDrugTransfusionPanel : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.HISFC.BizProcess.Interface.IPrintTransFusion
    {
        ...
        private ArrayList GetValidControl()
        {
            ...
            {
                if (c.GetType().ToString() == "FS.WinForms.Report.Order.ucInfusionLabel")
                {
                    FS.WinForms.Report.Order.ucInfusionLabel cTemp = c as FS.WinForms.Report.Order.ucInfusionLabel;
        ...
        public void Print()
        {
            try
            {
                ...
                ucLabel.SetPrint();
                FS.FrameWork.WinForms.Classes.Function.AddControlToPanel(alTemp, ucLabel, this.pnCard, new Size(800, 1100), 1);
        ...
        public void Query(List<FS.HISFC.Models.RADT.PatientInfo> patients, string usageCode, DateTime dtBegin, DateTime dtEnd, bool isPrinted)
        {
            ...
            for (int i = 0; i < patients.Count; i++)
            {
                ...
                string strDiff = "";//分组用
                ArrayList alModelss = null;//分组用
                int iComboNum = 0;
                int MaxComboNum = 7;        //每页最多5个药
                int iPage = 0;//分页用
                ...
                for (int j = 0; j < alOrder.Count; j++)//医嘱信息
                {
                        ...
                        && ((FS.HISFC.Models.Order.ExecOrder)alOrder[j]).Order.Combo.ID != "")
                    {
                        ...
                        if (strDiff != execOrderID)
                        {
                            ...
                            if (alModelss != null)
                            {
                                al.Add(alModelss);//不同的
                            }
                            alModelss = new ArrayList();

                            strDiff = execOrderID;
                            alModelss.Add(alOrder[j]);
        ...
        protected void SetValues(ArrayList alValues)
        {
            ...
            if (alValues != null)
            {
                //this.tabControl1.TabPages[this.tabControl1.SelectedIndex].Title = ((FS.neuFC.Models.neuModels)this.tabControl1.SelectedTab.Tag).Name + "【" + value.Count.ToString() + "条】";
