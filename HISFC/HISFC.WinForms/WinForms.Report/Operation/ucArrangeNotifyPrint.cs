...
namespace FS.WinForms.Report.Operation
{
    ...
    ///  />
    /// </summary>
    public partial class ucArrangeNotifyPrint : UserControl, FS.HISFC.BizProcess.Interface.Operation.IArrangeNotifyFormPrint
    {
        /// <summary>
        /// [功能描述: 手术安排打印]<br></br>
        /// [创 建 者: 王铁全]<br></br>
...
#region 方法
        ...
        public FS.HISFC.Models.Operation.OperationAppllication OperationApplicationForm
        {
            set 
            {
                ...
                string BedNO = "";
                this.lbSpecial.Text = thisOpsApp.SpecialItem;//特殊感染
                ...
                //}
                this.lbDept.Text = thisOpsApp.PatientInfo.PVisit.PatientLocation.Dept.Name + thisOpsApp.PatientInfo.PVisit.PatientLocation.Bed.Name + "床";//科室
                ...
                //手术项目（手术名称）
                //麻醉方式
                //>>{87493E86-0154-42bb-A4ED-E8DD807A54AC}麻醉方式常数字典与申请时一致，麻醉方式为空时打印麻醉类别
                NeuObject obj = new NeuObject();
                if (thisOpsApp.AnesType.ID != null && thisOpsApp.AnesType.ID != "")
                {
                    //obj = this.constManager.GetConstant(FS.HISFC.Models.Base.EnumConstant.ANESTYPE,thisOpsApp.AnesType.ID);
                    obj = this.integrateManager.GetConstant("CASEANESTYPE", thisOpsApp.AnesType.ID);
                    if (obj != null)
                    {
                        this.lbAnaeType.Text = obj.Name;
                    }
                }
                else if(!string.IsNullOrEmpty (thisOpsApp .AnesWay ))
                {
                    obj = this.constManager.GetConstant(FS.HISFC.Models.Base.EnumConstant.ANESWAY, thisOpsApp.AnesWay);
                    if (obj != null)
                    {
                        this.lbAnaeType.Text = obj.Name;
                    }
