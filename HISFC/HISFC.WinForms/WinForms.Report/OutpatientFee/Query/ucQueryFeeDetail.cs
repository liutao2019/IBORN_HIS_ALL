...
{
    public partial class ucQueryFeeDetail : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        private bool SetFeeInfo(string invoiceNO)
        {
            ...
            foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList f in alFee)
            {
                ...
                if (f.PayType == PayTypes.Balanced && f.CancelType == CancelTypes.Valid)
                {
                    ...
                    {
                        FS.HISFC.Models.Pharmacy.Item item = phaManager.GetItem(f.Item.ID);
                        if (item == null)
                        {
                        ...
                        {
                            qty = FS.FrameWork.Public.String.FormatNumber(f.Item.Qty / f.Item.PackQty,2);
                            priceUnit = item.PackUnit;
                        }
                    }
                    ...
                    this.neuSpread1_Sheet1.Cells[count, 3].Text = FS.FrameWork.Public.String.FormatNumber(price,2).ToString();
                    this.neuSpread1_Sheet1.Cells[count, 4].Text = priceUnit;
                    this.neuSpread1_Sheet1.Cells[count, 5].Text = FS.FrameWork.Public.String.FormatNumber(qty,2).ToString();
