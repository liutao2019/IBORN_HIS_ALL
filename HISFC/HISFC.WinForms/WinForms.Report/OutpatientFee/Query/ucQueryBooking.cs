...
namespace FS.WinForms.Report.OutpatientFee.Query
{
    ...
    /// 预约情况查询
    /// </summary>
    public partial class ucQueryBooking : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        private int getCompoundWhere(ref string rtn)
        {
            ...
            if (this.checkBox1.Checked)
            {
                ...
                //chenxin 2012-10-22 结束时间为何要+1？this.dateEnd.Value.AddDays(1).ToString()
                where = "OPER_DATE >=to_date('" + this.dateBegin.Value.ToString() + "','yyyy-mm-dd HH24:mi:ss')" +
                    " AND OPER_DATE <=to_date('" + this.dateEnd.Value.ToString() + "','yyyy-mm-dd HH24:mi:ss')";
            }
        ...
        {
            foreach (FS.HISFC.Models.Registration.Booking booking in al)
            {
                ...
                this.fpSpread1_Sheet1.SetValue(row, 3, booking.Age, false);
                this.fpSpread1_Sheet1.SetValue(row, 4, booking.DoctorInfo.Templet.Dept.Name, false);
                this.fpSpread1_Sheet1.SetValue(row, 5, booking.DoctorInfo.Templet.Doct.Name, false);
                this.fpSpread1_Sheet1.SetValue(row, 6, booking.DoctorInfo.SeeDate.ToString("yyyy-MM-dd"), false);
                this.fpSpread1_Sheet1.SetValue(row, 7, booking.DoctorInfo.Templet.Begin.ToString("HH:mm"), false);
                this.fpSpread1_Sheet1.SetValue(row, 8, booking.DoctorInfo.Templet.End.ToString("HH:mm"), false);
                this.fpSpread1_Sheet1.SetValue(row, 9, booking.IDCard, false);
                this.fpSpread1_Sheet1.SetValue(row, 10, booking.PhoneHome, false);
                this.fpSpread1_Sheet1.SetValue(row, 11, booking.AddressHome, false);
                ...
                if (booking.DoctorInfo.Templet.IsAppend)
                    this.fpSpread1_Sheet1.SetValue(row, 13, "是", false);
                else
                {
                    //this.fpSpread1_Sheet1.Rows[row].BackColor = Color.MistyRose;
                    this.fpSpread1_Sheet1.SetValue(row, 13, "否", false);
                }
