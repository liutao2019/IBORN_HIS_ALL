...
namespace Report.Logistics.Pharmacy
{
    ...
    public partial class ucCustomQuery : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        private void SetReportControlPerporty()
        {
            ...
            this.myReport.FilerType = FS.HISFC.Components.Common.Controls.ucPrivePowerReport.EnumFilterType.汇总过滤;
        }
        ...
        private int GetFilterAndSumCol(ref string filters, ref string sumCols)
        {
            ...
            try
            {
                ...
                {
                    //跳过没有选中的字段
                    if (!FS.FrameWork.Function.NConvert.ToBoolean(neuFpEnter1_Sheet1.Cells[rowIndex, (int)ColSet.选中].Value))
                    {
                        continue;
                    }
                    ...
                    //合计
                    if (FS.FrameWork.Function.NConvert.ToBoolean(neuFpEnter1_Sheet1.Cells[rowIndex, (int)ColSet.添加合计].Value))
                    {
                        sumCols += colIndex.ToString() + ",";
                    }

                    //过滤
                    if (FS.FrameWork.Function.NConvert.ToBoolean(neuFpEnter1_Sheet1.Cells[rowIndex, (int)ColSet.过滤].Value))
                    {
        ...
        {
            FS.FrameWork.WinForms.Classes.Function.ShowWaitForm("SQL验证中...");
                ...
                while (ds == null || ds.Tables.Count == 0 || ds.Tables[0].Rows.Count == 0)
                {
                    ...
                    if (times > 1)
                    {
                        ...
                        dr = System.Windows.Forms.MessageBox.Show(
                            FS.FrameWork.Management.Language.Msg("定义验证后才能生效，但是根据您的定义最近 " + (DaysSpan + DaysSpan * times).ToString() + " 天内没有可以查询的数据，是否扩大时间范围？"),
                            FS.FrameWork.Management.Language.Msg("请确认"),
        ...
        private FS.FrameWork.Models.NeuObject GetSQL()
        {
            ...
            for (int rowIndex = 0; rowIndex < this.neuFpEnter1_Sheet1.RowCount; rowIndex++)
            {
                ...
                if (FS.FrameWork.Function.NConvert.ToBoolean(neuFpEnter1_Sheet1.Cells[rowIndex, (int)ColSet.平均数].Value))
                {
                    isNeedGroup = true;
                ...
                {
                    if (!FS.FrameWork.Function.NConvert.ToBoolean(neuFpEnter1_Sheet1.Cells[rowIndex, (int)ColSet.分组求和].Value))
                    {
                        //自定义了分组字段，则用自定义分组字段
                ...
                //排序
                if (FS.FrameWork.Function.NConvert.ToBoolean(neuFpEnter1_Sheet1.Cells[rowIndex, (int)ColSet.排序].Value))
                {
            ...
            {
                this.myReport.FilerType = FS.HISFC.Components.Common.Controls.ucPrivePowerReport.EnumFilterType.不过滤;
            }
            else
            {
                this.myReport.FilerType = FS.HISFC.Components.Common.Controls.ucPrivePowerReport.EnumFilterType.汇总过滤;
        ...
        private int Query()
        {
            ...
            {
                MessageBox.Show( "请选择左侧查询类型", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information );
                return -1;
            }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            this.toolBarService.SetToolButtonEnabled( "查询", true );
        ...
        {
            if (this.neuTabControl1.SelectedTab == this.tpSet)      //设置
            {
                this.toolBarService.SetToolButtonEnabled( "保存", true );
                this.toolBarService.SetToolButtonEnabled( "查询", false );
            }
            else if (this.neuTabControl1.SelectedTab == this.tpQuery)   //查询
            {                
                this.toolBarService.SetToolButtonEnabled( "保存", false );
                this.toolBarService.SetToolButtonEnabled( "查询", true );
            }
