...
namespace FS.Report.Logistics.DrugStore
{
    ...
    public partial class ucMzOutRecipeQuery : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        {
            FS.FrameWork.WinForms.Classes.Function.ShowWaitForm(Language.Msg("正在加载基础查询数据,请稍候...."));
        ...
        private void SetFP()
        {
            ...
            else
            {
                ...
                this.neuSpread1_Sheet1.Columns[4].Width = 60F;		//合同单位
                this.neuSpread1_Sheet1.Columns[5].Width = 70F;		//病历号
                this.neuSpread1_Sheet1.Columns[5].Visible = true;
                this.neuSpread1_Sheet1.Columns[6].Width = 80F;		//发票号
                this.neuSpread1_Sheet1.Columns[6].Visible = true;
                this.neuSpread1_Sheet1.Columns[7].Width = 70F;		//配药台
                this.neuSpread1_Sheet1.Columns[8].Width = 50F;		//配药人
                this.neuSpread1_Sheet1.Columns[9].Width = 120F;		//配药时间
                this.neuSpread1_Sheet1.Columns[10].Width = 70F;		//发药台
                this.neuSpread1_Sheet1.Columns[11].Width = 50F;		//发药人
                this.neuSpread1_Sheet1.Columns[12].Width = 120F;	//发药时间
                this.neuSpread1_Sheet1.Columns[13].Width = 60F;		//开方医生
                this.neuSpread1_Sheet1.Columns[14].Width = 100F;		//诊断
            }
        ...
        private int GetDataSet(ref DataSet dsData)
        {
            ...
            if (dsHead != null && dsHead.Tables.Count > 0 && dsHead.Tables[0].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dtHead.Rows.Count; i++)
                {
                    ...
                    dtHead.Rows[i]["年龄"] = this.drugStoreManager.GetAge(FS.FrameWork.Function.NConvert.ToDateTime(dtHead.Rows[i]["年龄"].ToString()));
                    if (this.personHelper != null)
                    {
                    ...
                    if (FS.FrameWork.Function.NConvert.ToDateTime(dtHead.Rows[i]["配药时间"]) == System.DateTime.MinValue)
                    {
                        dtHead.Rows[i]["配药时间"] = "";
                    }
                    if (FS.FrameWork.Function.NConvert.ToDateTime(dtHead.Rows[i]["发药时间"]) == System.DateTime.MinValue)
                    {
                        dtHead.Rows[i]["发药时间"] = "";
                    }
        ...
        private int GetDrugDataSet(ref DataSet dsData, object queryData)
        {
            ...
            if (dsHead != null && dsHead.Tables.Count > 0 && dsHead.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    dtDetail.Rows[i]["年龄"] = this.itemManager.GetAge(FS.FrameWork.Function.NConvert.ToDateTime(dtDetail.Rows[i]["年龄"].ToString()));

                    #region 配/发药日期显示

                    if (FS.FrameWork.Function.NConvert.ToDateTime(dtDetail.Rows[i]["配药时间"]) == System.DateTime.MinValue)
                    {
                        dtDetail.Rows[i]["配药时间"] = "";
                    }
                    if (FS.FrameWork.Function.NConvert.ToDateTime(dtDetail.Rows[i]["发药时间"]) == System.DateTime.MinValue)
                    {
        ...
        private void fpSpread1_ChildViewCreated(object sender, FarPoint.Win.Spread.ChildViewCreatedEventArgs e)
        {
            ...
            if (this.cmbQueryType.SelectedValue != null && this.cmbQueryType.SelectedValue.ToString() == "D")
            {
                ...
                e.SheetView.Columns[4].Width = 60F;		//合同单位
                e.SheetView.Columns[5].Width = 70F;		//病历号
                e.SheetView.Columns[6].Width = 80F;		//发票号
                e.SheetView.Columns[7].Width = 60F;		//配药数量
                e.SheetView.Columns[8].Width = 50F;		//配药人
                e.SheetView.Columns[9].Width = 120F;	//配药时间
                e.SheetView.Columns[10].Width = 60F;		//发药数量
                e.SheetView.Columns[11].Width = 50F;	//发药人
                e.SheetView.Columns[12].Width = 120F;	//发药时间
                e.SheetView.Columns[13].Width = 60F;	//开方医生
                e.SheetView.Columns[14].Width = 100F;	//诊断
        ...
        private void txtQueryData_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Space && this.cmbQueryType.SelectedValue != null && this.cmbQueryType.SelectedValue.ToString() == "D" && this.drugCollectioon != null)
            {
                ...
                if (FS.FrameWork.WinForms.Classes.Function.ChooseItem(this.drugCollectioon, new string[] { "编码", "商品名称", "规格" }, new bool[] { false, true, true, false, false, false, false, false, false }, new int[] { 100, 160, 80 }, ref drugInfo) == 0)
                {
                    return;
                }
                else
