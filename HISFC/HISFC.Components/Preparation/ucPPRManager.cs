    ...
    public partial class ucPPRManager : FS.FrameWork.WinForms.Controls.ucBaseControl , FS.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        ...
        public FS.HISFC.Models.Preparation.EnumState SaveState
        {
            ...
            set
            {
                ...
                switch ( value )
                {
                    case FS.HISFC.Models.Preparation.EnumState.Plan:       //计划
                    case FS.HISFC.Models.Preparation.EnumState.Input:      //入库
        ...
        {
            this.toolBarService.AddToolButton ( "增加" , "新增制剂计划明细" , FS.FrameWork.WinForms.Classes.EnumImageList.T添加 , true , false , null );
            this.toolBarService.AddToolButton ( "新建" , "新建制剂计划单" , FS.FrameWork.WinForms.Classes.EnumImageList.X新建 , true , false , null );
            this.toolBarService.AddToolButton ( "工艺流程" , "工艺流程录入" , FS.FrameWork.WinForms.Classes.EnumImageList.H化验 , true , false , null );
            this.toolBarService.AddToolButton ( "删除" , "删除未开始配置的计划单明细" , FS.FrameWork.WinForms.Classes.EnumImageList.S删除 , true , false , null );
            this.toolBarService.AddToolButton ( "成本调整" , "调整选择项目的成本价" , FS.FrameWork.WinForms.Classes.EnumImageList.S设置 , true , false , null );
            ...
            else
            {
                switch ( this.saveState )
                {
                    ...
                    using ( Process.ucConfectProcess uc = new FS.HISFC.Components.Preparation.Process.ucConfectProcess ( ) )
                    {
                        ...
                        FS.FrameWork.WinForms.Classes.Function.PopForm.Text = "生产工艺流程信息录入";
        ...
        protected void DelPreparationPlanDetail ( )
        {
            ...
            DialogResult rs = MessageBox.Show ( FS.FrameWork.Management.Language.Msg ( "是否确认删除当前选择的计划信息" ) , "" , MessageBoxButtons.YesNo , MessageBoxIcon.Question );
            if ( rs == DialogResult.No )
            {
                return;
            }
            ...
            if ( info != null )
            {
                ...
                {
                    MessageBox.Show ( FS.FrameWork.Management.Language.Msg ( "删除制剂计划明细信息失败" ) + this.preparationManager.Err );
                    return;
                }
            }
