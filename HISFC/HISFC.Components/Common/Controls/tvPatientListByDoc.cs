...
namespace FS.HISFC.Components.Common.Controls
{
    ...
    /// <summary>
    /// [功能描述: 患者列表树]<br></br>tvPatientListByDoc{9C8C5614-96E4-49d6-A3BF-74996194A63F}
    /// [创 建 者: wolf]<br></br>
    ...
    ///  />
    /// </summary>
    public class tvPatientListByDoc : FS.FrameWork.WinForms.Controls.NeuTreeView
    {

        /// <summary>
        /// 无参构造函数
        /// </summary>
        public tvPatientListByDoc()
        {
        ...
        protected virtual void RefreshList()
        {
            this.Nodes.Clear();
            int Branch = 0;
            int attendingDoctorIndex = 0;//主治医师节点索引
            int responsibleDoctorIndex = 0;//责任医师节点索引
           ...
           // System.Collections.Generic.Dictionary<string, TreeNode> areaIndexDic = new System.Collections.Generic.Dictionary<string, TreeNode>();//护士站对应节点
            ...
            System.Collections.Generic.Dictionary<int, string> benchDic_ResponsibleDoctor = new System.Collections.Generic.Dictionary<int, string>();//存责任医生名称
            System.Collections.Generic.Dictionary<int, System.Collections.Generic.Dictionary<string, TreeNode>> benchDic_AttendingDoctorList = new System.Collections.Generic.Dictionary<int, System.Collections.Generic.Dictionary<string, TreeNode>>();//存主治医师节点
            System.Collections.Generic.Dictionary<int, System.Collections.Generic.Dictionary<string, TreeNode>> benchDic_ResponsibleDoctorList = new System.Collections.Generic.Dictionary<int, System.Collections.Generic.Dictionary<string, TreeNode>>();//存责任医师名称——主治医师Id 节点
            bool isInChargePatient = false;//判断是否属于分管患者节点
            ArrayList listArea = new ArrayList();
            ArrayList listAreaNodes = new ArrayList();
            int inChargeCount = 0;//分管患者总数
            if (myPatients.Count == 0) this.AddRootNode();
            for (int i = 0; i < myPatients.Count; i++)
            {
                System.Windows.Forms.TreeNode newNode = new System.Windows.Forms.TreeNode();
                System.Windows.Forms.TreeNode newNode3 = new System.Windows.Forms.TreeNode();//责任医师节点
                FS.FrameWork.Models.NeuObject obj = new FS.FrameWork.Models.NeuObject();
                //类型为叶
                if (myPatients[i].GetType().ToString() == "FS.HISFC.Models.RADT.PatientInfo")
                {
                    try
                    {
                        if (isInChargePatient)//分管患者节点
                        {
                            inChargeCount++;//分管患者总数
                        }
                        FS.HISFC.Models.RADT.PatientInfo PatientInfo = (FS.HISFC.Models.RADT.PatientInfo)myPatients[i];
                        obj.ID = PatientInfo.PID.PatientNO;
                        obj.Name = PatientInfo.Name;
                        ...
                        string areaId = "";//主治医生工号
                        string areaId_index = "";//主治医生工号
                        string areatext = "";//界面主治医生节点显示的内容
                        
                        if (isInChargePatient)//分管患者
                        {
                            areaId = PatientInfo.PVisit.AttendingDoctor.ID;//主治医生工号
                            area = PatientInfo.PVisit.AttendingDoctor.ID;
                            areatext = "主治医生【" + PatientInfo.PVisit.AttendingDoctor.Name + "】";
                        ...
                        {                            
                            area = PatientInfo.PVisit.PatientLocation.NurseCell.Name;//科室名称
                            //areatext = PatientInfo.PVisit.PatientLocation.NurseCell.Name;
                        }
                        ...
                        if (benchDic[Branch].ContainsKey(area) == false)
                        {
                            ...
                            if (isInChargePatient)//分管患者节点
                            {
                                newNode.Text = areatext;
                                newNode.Name = areatext + areaId;//用于查找当前主治医师下的责任医师节点是哪个
                            }
                            else
                            {
                                newNode.Text = area;
                            ...
                            if (isInChargePatient)//分管患者节点
                            {
                                benchDic_AttendingDoctorList[Branch].Add(areaId, newNode);//储存主治医师
                                attendingDoctorIndex++;
                                System.Collections.Generic.Dictionary<string, TreeNode> tree = benchDic[Branch];
                                TreeNode tr = tree[area];//获取主治医师节点

                                string area2 = "";
                                string area2Id = "";//责任医师工号
                                string area2Text = "";//

                                if (PatientInfo.IsBaby)
                                {
                                    area2Id = PatientInfo.PVisit.ResponsibleDoctor.ID;
                                    area2 = "责任医师【" + PatientInfo.PVisit.ResponsibleDoctor.Name + "】";
                                    area2Text = area2 + areaId;
                                }
                                else
                                {
                                    area2Id = PatientInfo.PVisit.AdmittingDoctor.ID;
                                    area2 = "责任医师【" + PatientInfo.PVisit.AdmittingDoctor.Name + "】";
                                    area2Text = area2 + areaId;
                                }
                                if (string.IsNullOrEmpty(area2Id))
                                {
                                    area2Id = "123";
                                }   
                                //三级节点（责任医师节点）
                                if (benchDic_ResponsibleDoctor.ContainsValue(area2Text) == false)//判断当前责任医师节点是否已经存在
                                {                                   
                                    newNode3.Text = area2;
                                    newNode3.Name = area2Text;
                                    newNode3.Tag = "";
                                    newNode3.ImageIndex = this.BranchImageIndex;
                                    newNode3.SelectedImageIndex = this.BranchSelectedImageIndex;

                                    tr.Nodes.Add(newNode3);

                                    benchDic_ResponsibleDoctor.Add(attendingDoctorIndex, area2Text);

                                    benchDic_ResponsibleDoctorList[Branch].Add(area2Text, newNode3);//存责任医师名称——主治医师Id-- 节点
                                    responsibleDoctorIndex++;

                                                                      
                                    //四级节点（患者节点）
                                    System.Windows.Forms.TreeNode node = new System.Windows.Forms.TreeNode();
                                    this.CreateNodeInfo(obj, PatientInfo, ref node);
                                    newNode3.Nodes.Add(node);
                                }
                                else
                                {
                                    System.Collections.Generic.Dictionary<string, TreeNode> tree1 = benchDic_AttendingDoctorList[Branch];
                                    TreeNode tr1 = tree1[area];//获取主治医师节点

                                    TreeNode tr2 = FindResponsibleDoctorNode(area2Text, tr1);//查找该主治医师节点下的当前责任医师节点

                                    string trID = tr2.Name as string;
                                    System.Collections.Generic.Dictionary<string, TreeNode> tree2 = benchDic_ResponsibleDoctorList[Branch];
                                    TreeNode tr3 = tree2[trID];//从字典里面获取责任医师节点

                                    System.Windows.Forms.TreeNode node = new System.Windows.Forms.TreeNode();//患者节点
                                    this.CreateNodeInfo(obj, PatientInfo, ref node);

                                    tr3.Nodes.Add(node);//在对应的责任医师节点下添加患者节点
                                }
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            {
                                System.Collections.Generic.Dictionary<string, TreeNode> tree = benchDic[Branch];
                                TreeNode tr = tree[areaId];//获取对应的主治医师节点
                                ...
                                if (benchDic_ResponsibleDoctor.ContainsValue(area2Text) == false)//判断当前责任医师节点是否已经存在
                                {
                                    ...
                                    tr.Nodes.Add(newNode3);//在对应的主治医师节点下添加责任节点

                                    benchDic_ResponsibleDoctor.Add(attendingDoctorIndex, area2Text);

                                    System.Windows.Forms.TreeNode node = new System.Windows.Forms.TreeNode();
                                    this.CreateNodeInfo(obj, PatientInfo, ref node);
                                    newNode3.Nodes.Add(node);//在责任医师节点下添加患者节点

                                    benchDic_ResponsibleDoctorList[Branch].Add(area2Text, newNode3);//存主治医师ID-责任医师
                                    responsibleDoctorIndex++;
                                    attendingDoctorIndex++;
                                }
                                else
                                {
                                    ...
                                    string trID = tr2.Name as string;
                                    System.Collections.Generic.Dictionary<string, TreeNode> tree2 = benchDic_ResponsibleDoctorList[Branch];
                                    TreeNode tr3 = tree2[trID];//获取责任医师节点
                ...
                {//为干
                    ...
                    newNode.Text = s[0];
                    if (newNode.Text == "分管患者")
                    {
                        isInChargePatient = true;
                        benchDic_AttendingDoctorList.Add(attendingDoctorIndex, new System.Collections.Generic.Dictionary<string, TreeNode>());
                        benchDic_ResponsibleDoctorList.Add(responsibleDoctorIndex, new System.Collections.Generic.Dictionary<string, TreeNode>());
                    }
                    else
                        isInChargePatient = false;
            ...
            if (this.bIsShowCount)
            {
                foreach (System.Windows.Forms.TreeNode node in this.Nodes)
                {
                    ...
                    {//结点
                        ...
                        if (node.Text == "分管患者")
                        {
                            node.Text = node.Text + "(" + inChargeCount + ")";
                        }
                        else
                        {
                            node.Text = node.Text + "(" + count.ToString() + ")";
        ...
        public void setPatientInfolist()
        {
        ...
        /// <returns>树节点</returns>
        private TreeNode FindResponsibleDoctorNode(string area2Text, TreeNode parentNode)
        {
            foreach (TreeNode node in parentNode.Nodes)
            {
                string nodeName = node.Name as string;
                if (nodeName == area2Text)
                {
                    return node;
                }
            }
