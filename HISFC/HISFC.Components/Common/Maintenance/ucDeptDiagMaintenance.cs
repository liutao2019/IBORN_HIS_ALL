        ...
        private int DeleteOneData(int rowIndex,string deptID)
        {
            if (rowIndex >= 0)
            {
                ...
                if (iReturn < 0)
                {
                    ...
                    MessageBox.Show(FS.FrameWork.Management.Language.Msg("删除失败！"));
                    return -1;
                }
        ...
        protected override int OnSave(object sender, object neuObject)
        {
            ...
            for (int i = 0; i < this.neuFpEnter1_Sheet1.RowCount; i++)
            {
                if (this.neuFpEnter1_Sheet1.Cells[i, 6].Text == "NEW")
                {
                    ...
                    {
                        FS.FrameWork.Management.PublicTrans.RollBack();
                        MessageBox.Show(FS.FrameWork.Management.Language.Msg("保存失败！" + this.deptDiagManager.Err));
                        return -1;
                    }
            ...
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("没有需要保存的数据！"));
            }
            ...
            try
            {
                ...
                if (icd != null)
                {
                    for (int i = 0; i < this.neuFpEnter1_Sheet1.RowCount; i++)
                    {
                        ...
                        {
                            MessageBox.Show(FS.FrameWork.Management.Language.Msg("列表中已存在" + icd.Name + "！"));
                            return -1;
                        }
                    }
