...
namespace FS.HISFC.Components.Privilege
{
    public partial class ucItemQueryManager : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        FS.FrameWork.Models.NeuObject  queryType = new FS.FrameWork.Models.NeuObject();
        public FS.FrameWork.Models.NeuObject QueryType
        {
            get
            {
        ...
        {        
            ...
            ///加载已维护的项目查询类型
            dsItem.Clear();
            if (this.queryManager.QueryItemQueryMend("ALL", this.queryType.ID, ref dsItem) == -1)
            {
        ...
        private void InitTree()
        {
            ...
            FS.HISFC.Models.Base.Const curr = new FS.HISFC.Models.Base.Const();
            curr.ID = this.queryType.ID;
            curr.Name = this.queryType.Name;

            queryTypeArray.Add(curr);
