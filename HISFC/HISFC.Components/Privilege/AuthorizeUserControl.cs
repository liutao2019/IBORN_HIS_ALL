...
namespace FS.HISFC.Components.Privilege
{
    public partial class AuthorizeUserControl : UserControl
    {
        ...
        public void ModifyUser()
        {
            ...
            {
                MessageBox.Show( FS.FrameWork.Management.Language.Msg( "请选择要修改的数据" ) );
                return;
            }
        ...
        public void DeleteUser()
        {
            ...
            if (MessageBox.Show(FS.FrameWork.Management.Language.Msg("确认需要删除当前选择的数据吗"), "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.No) return;
        ...
        private TreeListViewItem NewItem(User user)
        {
            ...
            item.SubItems.AddRange( new string[] {user.Name,user.Account,
                                         (user.IsLock? FS.FrameWork.Management.Language.Msg("是"):FS.FrameWork.Management.Language.Msg("否")),user.Description } );
            item.Tag = user;
            item.ImageIndex = 0;
            return item;
        }
