    ...
    public partial class ucDepartmentExt : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        private void GetDepartmentExt()
        {
            ...
            for (int i = 0; i < this.alDepartExt.Count; i++)
            {
                ...
                row["操作时间"] = FS.FrameWork.Function.NConvert.ToDateTime( departmentExt.OperEnvironment.Memo);
            ...
            else
            {
                for (int i = 0; i < al.Count; i++)
                {
                    if (!htDepartment.ContainsKey(((FS.HISFC.Models.Base.DepartmentStat)al[i]).DeptCode))
                    {
                        ...
                        DataRow row = this.dtDepartmentExt.NewRow();
                        row["科室编码"] = ((FS.HISFC.Models.Base.DepartmentStat)al[i]).DeptCode;
                        row["科室名称"] = ((FS.HISFC.Models.Base.DepartmentStat)al[i]).DeptName;
                        row["属性编码"] = "CASE_BED_STAND";
                        row["属性名称"] = "床位数";
                        row["床位数"] = 0;
                        row["操作人"] = this.myDeptManager.Operator.ID;
