...
{
    internal partial class ucEmployeeRecord : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        #region 变量

        //人员实体
        private FS.HISFC.Models.Base.Employee myPerson = new FS.HISFC.Models.Base.Employee();
        ...
        //定义人员属性变动管理类
        private FS.HISFC.BizLogic.Manager.EmployeeRecord recordManager = new FS.HISFC.BizLogic.Manager.EmployeeRecord();

        //单条人员属性变动记录
        private FS.HISFC.Models.Base.EmployeeRecord myEmployeeRecord = new FS.HISFC.Models.Base.EmployeeRecord();
        ...
        public FS.HISFC.Models.Base.EmployeeRecord EmployeeRecord
        {
            get
            {
        ...
        public void Delete()
        {
            ...
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("请选择你要取消的专科申请记录"));
                return;
            }
            //验证数据是否允许删除，只有未核准的数据允许删除
            if (this.myEmployeeRecord.State == "1")
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("该条数据已生效不能删除"));
                return;
            }

            if (MessageBox.Show(FS.FrameWork.Management.Language.Msg("确定要删除此记录吗？"), FS.FrameWork.Management.Language.Msg("确认删除"), MessageBoxButtons.YesNo) == DialogResult.No) return;
            ...
            if (parm == -1)
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg(recordManager.Err));
                return;
            }
        ...
        public void Save()
        {
            ...
            //取控件中的数据
            FS.HISFC.Models.Base.EmployeeRecord record = this.EmployeeRecord;
            //验证数据是否有效
            //人员编码
            if (record.Employee.ID == "")
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("人员编码不能为空"));
                return;
            }
            //变动类型
            if (record.ShiftType.ID == "")
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("变动类型不能为空"));
                return;
            }
            //变动前编码
            if (record.OldData.ID == "")
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("变动前编码不能为空"));
                return;
            }
            //变动后编码
            if (record.NewData.ID == "")
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("变动后编码不能为空"));
                return;
            }
            //变动后名称
            if (record.NewData.Name == "")
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("变动后名称不能为空"));
                return;
            }

            if (record.OldData.ID == record.NewData.ID || record.OldData.Name == record.NewData.Name)
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("变动前后的数据不能相同"));
                return;
            }
            ...
            recordManager.SetTrans(FS.FrameWork.Management.PublicTrans.Trans);
            ...
            if (this.myIsApply)//申请
            {
                ...
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("转科申请成功"));
            }
            else
            {
                ...
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();;
                    MessageBox.Show(FS.FrameWork.Management.Language.Msg(personManager.Err));
                    return;
                }

                FS.FrameWork.Management.PublicTrans.Commit();;
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("确认转科成功"));
            }
        ...
        public void ShowListView(ArrayList alRecord)
        {
            ...
            foreach (FS.HISFC.Models.Base.EmployeeRecord info in alRecord)
            {
                //设置插入的节点信息
                ...
                lvi.Tag = info;
                //设置listView的子节点
                lvi.SubItems.Add(info.State == "1" ? FS.FrameWork.Management.Language.Msg("已生效") : FS.FrameWork.Management.Language.Msg("未生效"));       //状态
                ...
                lvi.SubItems.Add(FS.FrameWork.Management.Language.Msg("科室变动")); //临时使用
        ...
        private void lvEmployeeRecord_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (this.lvEmployeeRecord.SelectedItems.Count == 0)
                this.EmployeeRecord = new FS.HISFC.Models.Base.EmployeeRecord();
            else
                this.EmployeeRecord = this.lvEmployeeRecord.SelectedItems[0].Tag as FS.HISFC.Models.Base.EmployeeRecord;
        }
