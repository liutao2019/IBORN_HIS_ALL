...
using FS.HISFC.Components.Manager.Forms;

namespace FS.HISFC.Components.Manager.Controls
{
    public partial class ucMaintenaceInterface : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        private List<FS.FrameWork.Models.NeuObject> resourceTypes = null;
        private List<ReportPrint> allInterfaceInfo = null;
        private Dictionary<string, string> resourceTypesMapping = null;
        private List<FS.FrameWork.WinForms.Classes.ControlParam> allControlParamInfo = null;
        private List<FS.FrameWork.WinForms.Classes.ControlParam> typeControlParamInfo = null;

        public ucMaintenaceInterface()
        {
            InitializeComponent();
            this.BackColor = FS.FrameWork.WinForms.Classes.Function.GetSysColor(FS.FrameWork.WinForms.Classes.EnumSysColor.Green);
            FS.FrameWork.WinForms.Classes.Function.SetFarPointStyle(neuSpread1);
            this.neuSpread1_Sheet1.RowCount = 0;
            FS.FrameWork.WinForms.Classes.Function.SetTabControlStyle(tbControl);
            FS.FrameWork.WinForms.Classes.Function.SetListViewStyle(tvInterface);
            this.splitter1.BackColor = FS.FrameWork.WinForms.Classes.Function.GetSysColor(FS.FrameWork.WinForms.Classes.EnumSysColor.Green);
            //接口控件设置
            InittvContainType();
            InitInterfaceInfo();
        ...
        protected override FS.FrameWork.WinForms.Forms.ToolBarService OnInit(object sender, object neuObject, object param)
        {
            ...
            toolbar.AddToolButton("添加", "添加新接口及实现！", FS.FrameWork.WinForms.Classes.EnumImageList.T添加, true, false, null);
            toolbar.AddToolButton("修改", "修改新接口及实现！", FS.FrameWork.WinForms.Classes.EnumImageList.X修改, true, false, null);
            toolbar.AddToolButton("删除", "删除新接口及实现！", FS.FrameWork.WinForms.Classes.EnumImageList.S删除, true, false, null);
            toolbar.AddToolButton("添加分类", "删除新接口及实现！", FS.FrameWork.WinForms.Classes.EnumImageList.F分类添加, true, false, null);
            toolbar.AddToolButton("删除分类", "删除新接口及实现！", FS.FrameWork.WinForms.Classes.EnumImageList.F分类删除, true, false, null);
            return toolbar;
        }
        ...
        public void UpdateParam()
        {
            ...
            frmEditParam frmParm = new frmEditParam(resourceTypes, neuSpread1_Sheet1.ActiveRow.Tag as FS.FrameWork.WinForms.Classes.ControlParam, resourceTypesMapping);
            if (frmParm.ShowDialog() == DialogResult.OK)
            {
                InitNeuSpreed();
            }
        ...
        {
            if (neuSpread1_Sheet1.ActiveRow == null || neuSpread1_Sheet1.ActiveRow.Index == -1)
            {
            ...
            int ret = new FS.FrameWork.WinForms.Classes.ControlParamManager().Delete((neuSpread1_Sheet1.ActiveRow.Tag as FS.FrameWork.WinForms.Classes.ControlParam).ID);
            if (ret != -1)
            {
        ...
        {
            FS.HISFC.Components.Manager.Forms.frmMaintenacePop newAddForm = new FS.HISFC.Components.Manager.Forms.frmMaintenacePop(resourceTypes);
        ...
        {
            FS.HISFC.Components.Manager.Forms.frmMaintenacePop newAddForm = new FS.HISFC.Components.Manager.Forms.frmMaintenacePop(reportPrint, resourceTypes, resourceTypesMapping);
            if (newAddForm.ShowDialog() == DialogResult.OK)
            {
                IntiTreeListView();
            }
        ...
        public void DeleteType()
        {
            ...
            if (new FS.FrameWork.WinForms.Classes.ReportPrintManager().JudgeType(tvContainType.SelectedNode.Tag as FS.FrameWork.Models.NeuObject) == -1)
            {
            ...
            FS.FrameWork.Management.PublicTrans.BeginTransaction();
            int ret = new FS.FrameWork.WinForms.Classes.ReportPrintManager().DeleteType(tvContainType.SelectedNode.Tag as FS.FrameWork.Models.NeuObject);
            if (ret == -1)
            {
        ...
        private List<FS.FrameWork.Models.NeuObject> GetResourceTypes()
        {
            return new FS.FrameWork.WinForms.Classes.ReportPrintManager().QueryType();
        }
        ...
        private void InittvContainType()
        {
            ...
            rootNode.ExpandAll();
            //只设置一级节点，不考虑多级节点。
            foreach (FS.FrameWork.Models.NeuObject res in resourceTypes)
            {
                TreeNode newNode = new TreeNode();
                newNode.Tag = res;
        ...
        {
            allControlParamInfo = new FS.FrameWork.WinForms.Classes.ControlParamManager().Query();
        }
        ...
        private void InitNeuSpreed()
        {
            ...
            for (int i = 0; i < allControlParamInfo.Count; i++)
            {
                ...
                neuSpread1_Sheet1.Cells[i, 3].Text = FS.FrameWork.Function.NConvert.ToBoolean(allControlParamInfo[i].ParamState).ToString();
                ...
                if (allControlParamInfo[i].ParamControlKind == frmEditParam.ControlTypeValue.颜色.ToString())
                {
                    ...
                    neuSpread1_Sheet1.Cells[i, 2].BackColor = Color.FromArgb(FS.FrameWork.Function.NConvert.ToInt32(allControlParamInfo[i].ParamValue));
                }
                else if (allControlParamInfo[i].ParamControlKind == frmEditParam.ControlTypeValue.选择框.ToString())
                {
                    ...
                    neuSpread1_Sheet1.Cells[i, 2].Value = FS.FrameWork.Function.NConvert.ToBoolean(allControlParamInfo[i].ParamValue).ToString();
                }
                else
                {
                    neuSpread1_Sheet1.Cells[i, 2].CellType = new FarPoint.Win.Spread.CellType.TextCellType();
        ...
        private void InitNeuSpreed(List<FS.FrameWork.WinForms.Classes.ControlParam> typeControlParamInfo)
        {
            ...
            for (int i = 0; i < typeControlParamInfo.Count; i++)
            {
                ...
                neuSpread1_Sheet1.Cells[i, 3].Text = FS.FrameWork.Function.NConvert.ToBoolean(typeControlParamInfo[i].ParamState).ToString();
                ...
                if (typeControlParamInfo[i].ParamControlKind == frmEditParam.ControlTypeValue.颜色.ToString())
                {
                    neuSpread1_Sheet1.Cells[i, 2].CellType = new FarPoint.Win.Spread.CellType.TextCellType();
                    neuSpread1_Sheet1.Cells[i, 2].Text = string.Empty;
                    neuSpread1_Sheet1.Cells[i, 2].BackColor = Color.FromArgb(FS.FrameWork.Function.NConvert.ToInt32(typeControlParamInfo[i].ParamValue));
                }
                else if (typeControlParamInfo[i].ParamControlKind == frmEditParam.ControlTypeValue.选择框.ToString())
                {
                    neuSpread1_Sheet1.Cells[i, 2].CellType =new  FarPoint.Win.Spread.CellType.CheckBoxCellType();
                    neuSpread1_Sheet1.Cells[i, 2].BackColor = Color.White;
                    neuSpread1_Sheet1.Cells[i, 2].Value = FS.FrameWork.Function.NConvert.ToBoolean(typeControlParamInfo[i].ParamValue).ToString();
                }
                else
                {
                    neuSpread1_Sheet1.Cells[i, 2].CellType = new FarPoint.Win.Spread.CellType.TextCellType();
        ...
        private void InterfaceInfoByType()
        {
            ...
            if (tvContainType.SelectedNode != null)
            {
                ...
                else
                {
                    foreach (ReportPrint pri in allInterfaceInfo)
                    {
                        if (pri.ContainerType == (tvContainType.SelectedNode.Tag as FS.FrameWork.Models.NeuObject).ID)
                        {
                            TreeListViewItem newItem = new TreeListViewItem();
                            newItem.Tag = pri;
                            newItem.Text = pri.ContainerDllName;
        ...
        private void ParamInfoByType()
        {
            ...
            if (tvContainType.SelectedNode != null)
            {
                ...
                else
                {
                    ...
                    foreach (FS.FrameWork.WinForms.Classes.ControlParam contorlParam in allControlParamInfo)
                    {
                        if (contorlParam.ParamKind == (tvContainType.SelectedNode.Tag as FS.FrameWork.Models.NeuObject).ID)
                        {
        ...
        private int SaveType(FS.FrameWork.Models.NeuObject obj)
        {
            FS.FrameWork.Management.PublicTrans.BeginTransaction();
            int ret = new FS.FrameWork.WinForms.Classes.ReportPrintManager().SaveReportPrintType(obj);
            if (ret == -1)
            {
        ...
        private void tvContainType_AfterLabelEdit(object sender, NodeLabelEditEventArgs e)
        {
            ...
            FS.FrameWork.Models.NeuObject obj = tvContainType.SelectedNode.Tag as FS.FrameWork.Models.NeuObject;
            obj.Name = e.Label;
            this.SaveType(obj);
        }
