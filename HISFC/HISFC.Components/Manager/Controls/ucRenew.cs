...
{
    public partial class ucRenew : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.HISFC.BizProcess.Interface.Common.IControlParamMaint
    {
        //接口使用
        ...
        public int Init()
        {
            try
            {
                ...
                {
                    MessageBox.Show(FS.FrameWork.Management.Language.Msg(obj.Err));
                }
                SetfpSpread1();
            }
            catch (Exception ee)
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg(ee.Message));
            }
            return 1;
        }
        ...
        private void AlterSql()
        {
            ...
            if (neuSpread1_Sheet1.Rows.Count > 0)
            {
                ...
                {
                    //执行失败
                    MessageBox.Show(FS.FrameWork.Management.Language.Msg("编译失败"));
                }
                else
                {
                    //执行成功

                    MessageBox.Show(FS.FrameWork.Management.Language.Msg("编译成功"));
                }
            }
        }
        /// <summary>
        ...
        public void ExportInfo()
        {
            ...
            try
            {
                ...
                if (saveFileDialog1.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (ret)
                    {
                        MessageBox.Show(FS.FrameWork.Management.Language.Msg("导出成功"));
                    }
                }
            }
            catch (Exception ee)
