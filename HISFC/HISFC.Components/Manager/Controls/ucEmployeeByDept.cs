...
{
    public partial class ucEmployeeByDept : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        #region 变量
        ...
        private void ApplyTransfer()
        {
            ...
            FS.HISFC.Models.Base.EmployeeRecord record = new FS.HISFC.Models.Base.EmployeeRecord();
            //取人员未核准的科室变动记录
            ...
            if (al.Count > 0)
            {
                ...
                record = al[0] as FS.HISFC.Models.Base.EmployeeRecord;
            }
            else
            {
            ...
            //取窗口返回参数
            DialogResult dlg = FS.FrameWork.WinForms.Classes.Function.PopShowControl(temp);
        ...
        {
            FS.HISFC.Models.Base.EmployeeRecord record = new FS.HISFC.Models.Base.EmployeeRecord();
            FS.HISFC.Models.Base.EmployeeRecord newRecord = new FS.HISFC.Models.Base.EmployeeRecord();
            
            record.ShiftType.ID = "DEPT";
            //record.NewData.ID = (this.personManager.Operator as FS.HISFC.Models.Base.Employee).Dept.ID;
            //record.NewData.Name = (this.personManager.Operator as FS.HISFC.Models.Base.Employee).Dept.Name;
            ArrayList al = recordManager.GetEmployeeRecordListByEmpl(this.myPerson.ID, "0");
            if (al == null)
            {
                MessageBox.Show(recordManager.Err, "错误提示");
            ...
            if(al.Count > 0)
            {
                newRecord = al[0] as FS.HISFC.Models.Base.EmployeeRecord;
            ...
            temp.EmployeeRecord = new FS.HISFC.Models.Base.EmployeeRecord();
            temp.EmployeeRecord = record;
            temp.IsApply = false;
        ...
        private void ShowEmployee()
        {
            ...
            root.Tag = new FS.HISFC.Models.Base.Employee();
            this.tvEmployee.Nodes.Add(root);

            //添加人员节点			
            TreeNode node = null;
            foreach (FS.HISFC.Models.Base.Employee person in alEmployee)
            {
                node = new TreeNode();
        ...
        private void Init()
        {
            ...
            //取人员类型
            myEmplType = ((FS.HISFC.Models.Base.Employee)this.personManager.Operator).EmployeeType.ID.ToString();

            //根据人员类型，取科室或者病区
            if (this.myEmplType == "N")
                this.myDept = ((FS.HISFC.Models.Base.Employee)this.personManager.Operator).Nurse;
            else
                this.myDept = ((FS.HISFC.Models.Base.Employee)this.personManager.Operator).Dept;
            //显示人员列表
            this.ShowEmployee();
        }
        ...
        {
            toolBarService.AddToolButton("申请", "", FS.FrameWork.WinForms.Classes.EnumImageList.X新建, true, false, null);
            toolBarService.AddToolButton("确认", "", FS.FrameWork.WinForms.Classes.EnumImageList.Z执行, true, false, null);
            toolBarService.AddToolButton("取消", "", FS.FrameWork.WinForms.Classes.EnumImageList.Q取消, true, false, null);
        ...
        private void tvEmployee_AfterSelect(object sender, TreeViewEventArgs e)
        {
            if (this.tvEmployee.SelectedNode != null)
                this.myPerson = e.Node.Tag as FS.HISFC.Models.Base.Employee;
            else
                this.myPerson = new FS.HISFC.Models.Base.Employee();
            //刷新人员属性变动数据
            this.RefreshData();
