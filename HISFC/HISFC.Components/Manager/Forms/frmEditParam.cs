...
namespace FS.HISFC.Components.Manager.Forms
{
    ...
    {
        private List<FS.FrameWork.Models.NeuObject> resoucesTypeList = null;
        private FS.FrameWork.WinForms.Classes.ControlParam currentControlParam = null;
        ...
        public frmEditParam(List<FS.FrameWork.Models.NeuObject> resoucesTypes)
        {
        ...
        public frmEditParam(List<FS.FrameWork.Models.NeuObject> resoucesTypes, FS.FrameWork.WinForms.Classes.ControlParam controlParam, Dictionary<string, string> resourceTypesMapping1)
        {
        ...
        private void Add()
        {
            ...
            int ret = new FS.FrameWork.WinForms.Classes.ControlParamManager().Insert(GetInfo());
            if (ret == -1)
            {
        ...
        {
            FS.FrameWork.Management.PublicTrans.BeginTransaction();
            int ret = new FS.FrameWork.WinForms.Classes.ControlParamManager().update(GetInfo());
            if (ret == -1)
            {
                FS.FrameWork.Management.PublicTrans.RollBack();
                MessageBox.Show("保存失败！");
        ...
        /// </summary>
        /// <returns></returns>
        private FS.FrameWork.WinForms.Classes.ControlParam GetInfo()
        {

            FS.FrameWork.WinForms.Classes.ControlParam newParam = new FS.FrameWork.WinForms.Classes.ControlParam();
            newParam.ID = txtParamID.Text.Trim();
            newParam.Name = txtParamName.Text.Trim();
            newParam.ParamKind = (cmbParamType.SelectedValue as FS.FrameWork.Models.NeuObject).ID;
            newParam.ParamState = FS.FrameWork.Function.NConvert.ToInt32(chbState.Checked).ToString();
            newParam.ParamControlKind = cmbControlType.Text.Trim();
            newParam.Oper = FS.FrameWork.Management.Connection.Operator.Name;
            newParam.OperDate = FS.FrameWork.Function.NConvert.ToDateTime(new FS.FrameWork.Management.DataBaseManger().GetSysDateTime());
            ...
            {
                newParam.ParamValue = FS.FrameWork.Function.NConvert.ToInt32(ckbvalue.Checked).ToString();
            }
        ...
        private void InitUpdateInfo()
        {
            ...
            ckbvalue.Checked = FS.FrameWork.Function.NConvert.ToBoolean(currentControlParam.ParamState);
            ...
            {
                ckbvalue.Checked = FS.FrameWork.Function.NConvert.ToBoolean(currentControlParam.ParamValue);
            }
            else if (currentControlParam.ParamControlKind == ControlTypeValue.颜色.ToString())
            {
                colorResult.BackColor = Color.FromArgb(FS.FrameWork.Function.NConvert.ToInt32(currentControlParam.ParamValue));
            }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (judgeOperation == 0)
            {
                if (new FS.FrameWork.WinForms.Classes.ControlParamManager().JudgeOnly(txtParamID.Text.Trim()) != 0)
                {
