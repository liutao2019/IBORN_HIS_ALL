...
namespace FS.HISFC.Components.OutpatientFee.SelfFee
{
    public partial class ucDisplay : UserControl, FS.HISFC.BizProcess.Integrate.FeeInterface.IOutpatientItemInputAndDisplay,FS.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        ...
        /// <param name="saleprice">{40DFDC91-0EC1-4cd4-81BC-0EAE4DE1D3AB}零售价--物资收费增加</param>
        ...
        private bool IsStoreEnough(FeeItemList feeItem,int row)
        {
            ...
            if (storeSum <= 0 || storeSum - storeSumTemp < 0)
            {
                ...
                {
                    MessageBox.Show("当前库存数:" + (storeSum / feeItem.Item.PackQty).ToString() + "|输入库存数:" + this.fpSpread1_Sheet1.Cells[row, 4].Value.ToString() + "   库存不足!");
                }
                else
                {
                    MessageBox.Show("当前库存数:" + storeSum.ToString() + "|输入库存数:" + this.fpSpread1_Sheet1.Cells[row, 4].Value.ToString() + "   库存不足!");
                }
                    ...
                    if (keyData == Keys.Enter)
                    {
                        ...
                        if (currColumn == (int)Columns.Amount)
                        {
                            ...
                            if (price == 0)//项目没有价格，直接跳转到输入价格的位置
                            {
                                ...
                                bool bReturn = InputDataIsValid(currRow, (int)Columns.Amount, "数量", 99999, 0, ref qty);
                                if (!bReturn)
                                {
                                    this.isDealCellChange = true;
