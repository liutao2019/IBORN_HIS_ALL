    ...
    public partial class ucDisplay : UserControl, FS.HISFC.BizProcess.Integrate.FeeInterface.IOutpatientItemInputAndDisplay,FS.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        ...
        private void SetChargeInfo()
        {
            ...
            foreach (FeeItemList f in alChargeInfo)
            {
                ...
                if (f.Item.ID != "999")
                {
                    // 零差价处理, 代用childprice属性保存零售价
                    // 2011-10-27
                    decimal decTemp = 0;
                    decimal.TryParse(rowFind["unit_price"].ToString(), out decTemp);
                    if (decTemp > 0)
                    {
                        f.Item.ChildPrice = decTemp;
                        f.OrgPrice = decTemp;
                    }
                }
                ...
                try
                {
                    ...
                    this.interfaceManager.GetCompareSingleItem(this.rInfo.Pact.ID, f.Item.ID, ref f.Compare);
                }
                catch { }
                ...
                if (this.rInfo.Pact.PayKind.ID == "02")
                {
                    if (f.ItemRateFlag != "3")
                    {
                        ...
                        {
                            if (!string.IsNullOrEmpty(f.Compare.CenterItem.ItemGrade))
                            {
                                this.fpSpread1_Sheet1.Cells[currRow, (int)Columns.Self].Text = FS.HISFC.BizLogic.Fee.Interface.ShowItemGradeByCode(f.Compare.CenterItem.ItemGrade);
                            }
                            else
                            {
           ...
           // {B9303CFE-755D-4585-B5EE-8C1901F79450}
            ...
            if (findRow != null)
            {
                ...
                try
                {
                    ...
                    // {9F4187BB-C1FA-4c2d-826D-619DC9AC6695}
                    if (this.rInfo.Pact.PayKind.ID == "03")
                    {
                        //做个参数维护广州医保的合同单位代码
            ...
            for (int currRow = 0; currRow < this.fpSpread1_Sheet1.RowCount; currRow++)
            {
                if (this.fpSpread1_Sheet1.Rows[currRow].Tag != null && this.fpSpread1_Sheet1.Rows[currRow].Tag.GetType() == typeof(FeeItemList))
                {
                    ...
                    if (this.rInfo.Pact.PayKind.ID == "01")//自费
                    {
                        ...
                        ((FeeItemList)this.fpSpread1_Sheet1.Rows[currRow].Tag).FT.RebateCost = 0;
        ...
        public void PreCountInvos()
        {
            ...
            FS.HISFC.Components.OutpatientFee.Forms.frmChoosePatientInfo frmChoosePatientinfo = new FS.HISFC.Components.OutpatientFee.Forms.frmChoosePatientInfo();
