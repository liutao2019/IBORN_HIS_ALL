...
namespace FS.HISFC.Components.RADT.Controls
{
    ...
    ///  />
    /// </summary>
    public partial class ucModifyPatientInfo : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        {
            toolBarService.AddToolButton("确定", "确定", FS.FrameWork.WinForms.Classes.EnumImageList.B保存, true, false, null);
            ...
            toolBarService.AddToolButton("退出", "退出", FS.FrameWork.WinForms.Classes.EnumImageList.T退出, true, true, null);
            toolBarService.AddToolButton("补打", "补打", FS.FrameWork.WinForms.Classes.EnumImageList.D打印, true, true, null);
        ...
        protected virtual int Init()
        {
            ...
            try
            {
                ...
                this.cmbMark.AddItems(managerIntegrate.GetConstantList(FS.HISFC.Models.Base.EnumConstant.REMARK));
        ...
        protected void SetPatientInfo(FS.HISFC.Models.RADT.PatientInfo patientInfo)
        {
            ...
            FS.HISFC.Models.RADT.Patient Patient = patientInfo;
            ...
            if (patientInfo.PVisit.InState.ID.ToString() == FS.HISFC.Models.Base.EnumInState.R.ToString())
            {
        ...
        private void ucQueryInpatientNo1_myEvent()
        {
            ...
            {
                FS.FrameWork.WinForms.Classes.Function.Msg("不存在该患者，或者该患者已经出院！", 111);
                this.Clear();
                return;
            }
            //获取住院号赋值给实体
            ...
            //如果是已经出院，则不允许修改住院信息
            FS.HISFC.Models.Base.EnumInState instate = (FS.HISFC.Models.Base.EnumInState)Enum.Parse(typeof(FS.HISFC.Models.Base.EnumInState), this.patientInfo.PVisit.InState.ID.ToString());
        ...
        private bool GetPatientInfomation(PatientInfo patientInfo)
        {
            if (patientInfo == null)
            {
                FS.FrameWork.WinForms.Classes.Function.Msg("患者信息实体为空!", 111);
            ...
            patientInfo.FT.FixFeeInterval = FS.FrameWork.Function.NConvert.ToInt32(this.txtBedInterval.Text);//床费间隔
            ...
            {
                this.patientInfo.NormalName = FS.FrameWork.WinForms.Classes.Function.Encrypt3DES(this.patientInfo.Name);
        ...
        protected virtual int Confirm()
        {
            ...
            //判断患者病床信息，是否已接诊，防止并发
            FS.HISFC.Models.RADT.PatientInfo patientInfoReGet = new PatientInfo();
            ...
            if (string.IsNullOrEmpty(this.patientInfo.PVisit.PatientLocation.Bed.ID) && !string.IsNullOrEmpty(patientInfoReGet.PVisit.PatientLocation.Bed.ID))
            {
                ...
                FS.FrameWork.WinForms.Classes.Function.Msg("保存失败！" + "患者信息已发生变动，请刷新后再试一次！", 211);
                return -1;
            }
            ...
            {
                FS.FrameWork.Management.PublicTrans.RollBack();
                FS.FrameWork.WinForms.Classes.Function.Msg("保存失败！" + this.inpatientManager.Err, 211);
            ...
            if ( this.IsAllowModifyPatientNO && this.txtPatientNO.Text.Trim().Length > 0)
            {
                ...
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                    FS.FrameWork.WinForms.Classes.Function.Msg("输入的住院号目前"+radtIntegrate.Err, 211);
                    return -1;
                }
                if (patient.PatientNOType == EnumPatientNOType.Second)
                {
                    FS.HISFC.Models.Base.EnumInState inState = (FS.HISFC.Models.Base.EnumInState)Enum.Parse(typeof(FS.HISFC.Models.Base.EnumInState), patient.PVisit.InState.ID.ToString());
                        ...
                        inState == EnumInState.P)
                    {
                        ...
                        FS.FrameWork.Management.PublicTrans.RollBack();
                        FS.FrameWork.WinForms.Classes.Function.Msg("输入的住院号目前处于在院状态(" + patient.PVisit.InState.Name + ")！", 211);
                        return -1;
                    }
                    if (!patient.Name.Equals(this.txtName.Text))
                    {
                        FS.FrameWork.Management.PublicTrans.RollBack();
                        FS.FrameWork.WinForms.Classes.Function.Msg("输入的住院号对应的姓名与当前姓名不相同！", 211);
                        return -1;
                    }
                    //姓名不允许修改
                ...
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                    FS.FrameWork.WinForms.Classes.Function.Msg("修改住院号错误" + radtManager.Err, 211);
                    return -1;
                }
                ...
                if (patientInfo.InTimes <= 1)
                {
                    ...
                    {
                        FS.FrameWork.Management.PublicTrans.RollBack();
                        FS.FrameWork.WinForms.Classes.Function.Msg("插入住院号回收表失败！" + radtManager.Err, 211);
                        return -1;
                    }
                }
            ...
            {
                this.patientInfo.InTimes = FS.FrameWork.Function.NConvert.ToInt32(this.neuTxtInTimes.Text);
                if (radtManager.UpdatePatientInTimes(this.patientInfo.ID, this.patientInfo.InTimes) == -1)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                    FS.FrameWork.WinForms.Classes.Function.Msg("更新住院次数失败！" + radtManager.Err, 211);
                    return -1;
                }
            }

            //插入变更信息
            if (this.radtIntegrate.InsertShiftData(this.patientInfo.ID, FS.HISFC.Models.Base.EnumShiftType.F, "患者基本信息修改", patientInfoOld, patientInfo) == -1)
            {
                FS.FrameWork.Management.PublicTrans.RollBack();
                FS.FrameWork.WinForms.Classes.Function.Msg("插入变更信息出错!" + this.inpatientManager.Err, 211);
            ...
            FS.FrameWork.WinForms.Classes.Function.Msg("修改成功!", 111);
