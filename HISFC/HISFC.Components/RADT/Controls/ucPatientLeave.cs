...
namespace FS.HISFC.Components.RADT.Controls
{
    ...
    ///  />
    /// </summary>
    public partial class ucPatientLeave : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        FS.HISFC.Models.RADT.Leave myLeave = null;
        FS.FrameWork.Models.NeuObject dept = null;
		...
		private  void InitControl() {
			...
			try {
				//初始化医生列表
				this.cmbDoc.AddItems(manager.QueryEmployee(FS.HISFC.Models.Base.EnumEmployeeType.D,dept.ID ));
		...
		private void InitTree() {
			...
			//添加患者请假信息
			foreach(FS.HISFC.Models.RADT.Leave leave in al) {	
		...
		private void AddTreeNode(FS.HISFC.Models.RADT.Leave leave) {
			System.Windows.Forms.TreeNode Node=new System.Windows.Forms.TreeNode();
			try {
		...
		/// </summary>
		/// <param name="leave"></param>
		private void ShowLeaveInfo(FS.HISFC.Models.RADT.Leave leave) {
			//如果没有患者信息,则清屏
		...
		private bool GetLeaveInfo() {
			...
			if(this.myLeave == null)
			{
				this.myLeave = new FS.HISFC.Models.RADT.Leave();
				isNew = true;
			}
			else
			{
			...
			//请假天数
			this.myLeave.LeaveDays = FS.FrameWork.Function.NConvert.ToInt32(this.txtDays.Text);
			//给假医生
		...
		public int Save() {
			...
			//天数不能为0
			if (FS.FrameWork.Function.NConvert.ToInt32(this.txtDays.Text) == 0) {
            ...
            //FS.FrameWork.Management.Transaction SQLCA=new FS.FrameWork.Management.Transaction(this.inpatient.Connection);
		...
		private void treeView1_AfterSelect(object sender, System.Windows.Forms.TreeViewEventArgs e) {
			//获取当前选中请假信息
			this.myLeave = e.Node.Tag as FS.HISFC.Models.RADT.Leave ;
