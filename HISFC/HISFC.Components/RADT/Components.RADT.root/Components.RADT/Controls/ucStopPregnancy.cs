...
namespace Neusoft.HISFC.Components.RADT.Controls
{
    ...
    ///  />
    /// </summary>
    public partial class ucStopPregnancy : Neusoft.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        public void Init()
        {
            ...
            this.ucQueryInpatientNo.myEvent -= new Neusoft.HISFC.Components.Common.Controls.myEventDelegate(ucQueryInpatientNo_myEvent);
            this.ucQueryInpatientNo.myEvent += new Neusoft.HISFC.Components.Common.Controls.myEventDelegate(ucQueryInpatientNo_myEvent);
        }
        ...
        public int Save()
        {
            ...
            Neusoft.HISFC.Models.RADT.PatientInfo patient = inpatientManager.QueryPatientInfoByInpatientNO(this.patientInfo.ID);
            if (patient == null)
            {
            ...
            if (Neusoft.FrameWork.Public.String.IsNumeric(this.txtStopPregnancyWeeks.Text) == false || Int32.TryParse(this.txtStopPregnancyWeeks.Text, out stopPregnancyWeeks) == false || stopPregnancyWeeks <= 0)
            {
                MessageBox.Show("终止妊娠周数必须填写正整数", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return -1;
            }

            Neusoft.HISFC.BizLogic.RADT.StopPregnancy stopPregnancyMgr = new Neusoft.HISFC.BizLogic.RADT.StopPregnancy();

            //保存终止妊娠登记记录
            Neusoft.FrameWork.Management.PublicTrans.BeginTransaction();
            stopPregnancyMgr.SetTrans(Neusoft.FrameWork.Management.PublicTrans.Trans);
        ...
        public void SetPatientInfo(Neusoft.HISFC.Models.RADT.PatientInfo patient)
        {
            if (patient!=null&&patient.ID != null && patient.ID != "")
            {
                ...
                //查找终止妊娠登记记录
                Neusoft.HISFC.BizLogic.RADT.StopPregnancy stopPregnancyMgr = new Neusoft.HISFC.BizLogic.RADT.StopPregnancy();
                Neusoft.HISFC.Models.RADT.StopPregnancy stopPregnancy = stopPregnancyMgr.Get(patient.ID);
