...
namespace FS.HISFC.Components.Nurse.Print
{
    public partial class ucPrintTreatment : FS.FrameWork.WinForms.Controls.ucBaseControl,FS.HISFC.BizProcess.Interface.Nurse.ITreatmentPrint
    {
        ...
        private void SetBill(ArrayList alFeeItem)
        {
            ...
            FS.HISFC.Models.Fee.Outpatient.FeeItemList feeItem = alFeeItem[0] as FS.HISFC.Models.Fee.Outpatient.FeeItemList;
            FS.HISFC.BizLogic.Registration.Register registerMgr = new FS.HISFC.BizLogic.Registration.Register();
            if (string.IsNullOrEmpty(feeItem.Patient.Name))
            {
                FS.HISFC.Models.Registration.Register r = registerMgr.GetByClinic(feeItem.Patient.ID);
                if (r != null)
                {
            ...
            {
                this.nlbPatientName.Text = feeItem.Patient.Name;
                this.nlbSex.Text = "性别：" + feeItem.Patient.Sex.Name;
                this.nlbAge.Text = "年龄：" + registerMgr.GetAge(feeItem.Patient.Birthday);
            }
            this.nlbTime.Text = "时间：" + registerMgr.GetDateTimeFromSysDateTime().ToString("yyyy-MM-dd HH:mm");
            //取医院名称
            this.neuLabel1.Text = FS.HISFC.BizProcess.Integrate.Function.GetHosName() + "治疗单";
            ...
            foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList f in alFeeItem)
            {
                ...
                this.neuSpread1_Sheet1.Cells[rowIndex, (int)ColPrint.ExecOper].Text = " "; 
                rowIndex++;
            }
