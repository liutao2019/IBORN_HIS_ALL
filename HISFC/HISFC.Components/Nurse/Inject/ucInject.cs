...
namespace FS.HISFC.Components.Nurse.Inject
{
    ...
    public partial class ucInject : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        {
            //HIS 5.0 重构移植 该功能屏蔽
            //toolBarService.AddToolButton("床位划价", "床位划价", (int)FS.FrameWork.WinForms.Classes.EnumImageList.H划价保存, true, false, null);
        ...
        private void BedFee()
        {
            ...
            //FS.HISFC.Components.OutpatientFee.Controls.ucCharge charge = new FS.HISFC.Components.OutpatientFee.Controls.ucCharge();

            //FS.FrameWork.WinForms.Classes.Function.PopShowControl(charge);
        }
        ...
        {
            List<FS.HISFC.Models.Nurse.InjectInfo> itemList = this.GetAlreadyPrecontractInfos();
        ...
        private void ucPatientInfo1_PatientEvent(FS.HISFC.Models.Registration.Register register)
        {
            if (register == null)
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("Register参数错误"));
                return;
            }

            this.patient = register;

            List<FS.HISFC.Models.Nurse.InjectInfo> itemList = this.decompound.QueryItemListByCardNoAndClinicCode(register.PID.CardNO, register.ID);
            if (itemList == null)
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("获取患者注射信息失败"));
                return;
            }
        ...
        private void FillFarPoint1(List<FS.HISFC.Models.Nurse.InjectInfo> injectInfos)
        {
            foreach (FS.HISFC.Models.Nurse.InjectInfo injectInfo in injectInfos)
            {
        ...
        private void DrowCombo()
        {
            ...
            FS.HISFC.Components.Common.Classes.Function.DrawCombo(this.neuSpread1_Sheet1, 3, 6);
        }
        ...
        private List<FS.HISFC.Models.Nurse.InjectInfo> GetPrecontractInjectInfos()
        {
            if (this.neuSpread1_Sheet1.Rows.Count <= 0)
            {
                return null;
            ...
            for (int i = 0; i < this.neuSpread1_Sheet1.Rows.Count; i++)
            {
                ...
                {
                    itemList.Add(this.neuSpread1_Sheet1.Rows[i].Tag as FS.HISFC.Models.Nurse.InjectInfo);
                }
            }
            return itemList;
        }

        private int GeneratePrecontractInjectInfos(List<FS.HISFC.Models.Nurse.InjectInfo> itemList)
        {
            if (itemList == null || itemList.Count == 0)
            {
                return -1;
        ...
        private void FillNeuspread2(FS.HISFC.Models.Nurse.InjectInfo inject, int number, int rowspan)
        {
            if (inject == null)
            {
                return;
        ...
        private void btnDetach_Click(object sender, EventArgs e)
        {
            ...
            {
                FS.HISFC.Components.Common.Classes.Function.DrawCombo(this.neuSpread2_Sheet1, 4, 8);
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        private List<FS.HISFC.Models.Nurse.InjectInfo> GetAlreadyPrecontractInfos()
        {
            List<FS.HISFC.Models.Nurse.InjectInfo> itemList = new List<FS.HISFC.Models.Nurse.InjectInfo>();
            ...
            FS.HISFC.Models.Nurse.InjectInfo inject;

            for (int i = 0; i < this.neuSpread2_Sheet1.Rows.Count; i++)
            {
                //inject = new FS.HISFC.Models.Nurse.InjectInfo();
                inject = this.neuSpread2_Sheet1.Rows[i].Tag as FS.HISFC.Models.Nurse.InjectInfo;
                //inject = this.neuSpread2_Sheet1.GetTag(i, 0) as FS.HISFC.Models.Nurse.InjectInfo;
                /*
                 * //暂时写死了，合并第二列,预约时间
                   //合并第五列，注射类型
                   //合并第六列，编号
                ...
                {
                    inject.PrecontractDate = FS.FrameWork.Function.NConvert.ToDateTime(strDatetime);
                }
                else
                {
                    inject.PrecontractDate = FS.FrameWork.Function.NConvert.ToDateTime(this.neuSpread2_Sheet1.GetText(i, 2));
                    strDatetime = FS.FrameWork.Function.NConvert.ToDateTime(this.neuSpread2_Sheet1.GetText(i, 2)).ToString();
                }
                //inject.PrecontractDate = FS.FrameWork.Function.NConvert.ToDateTime(this.neuSpread2_Sheet1.GetText(i, 2));
                ...
                inject.OperEnv.ID = ((FS.HISFC.Models.Base.Employee)(FS.FrameWork.Management.Connection.Operator)).ID;
                inject.OperEnv.Name = ((FS.HISFC.Models.Base.Employee)(FS.FrameWork.Management.Connection.Operator)).Name;
        ...
        {
            //(this.neuSpread2_Sheet1.GetTag(e.Row,e.Column) as FS.HISFC.Models.Nurse.InjectInfo).PrecontractDate =             
        }
    }
}