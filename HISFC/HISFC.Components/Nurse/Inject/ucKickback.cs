...
{
    public partial class ucKickback : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        {
            this.toolBarService.AddToolButton("添加", "添加新不良反应", (int)FS.FrameWork.WinForms.Classes.EnumImageList.T添加, true, false, null);
        ...
        private void InitTable()
        {
            ...
            List<FS.HISFC.Models.Nurse.Kickback> kbList = this.kbManager.QueryKickback();
            if (kbList == null || kbList.Count == 0)
            {
                return;
            }
            foreach (FS.HISFC.Models.Nurse.Kickback kb in kbList)
            {
        ...
        private void FillTable(DataRow dr, FS.HISFC.Models.Nurse.Kickback kb)
        {
            dr["ID"] = kb.ID;
            dr["名称"] = kb.Name;
            dr["拼音码"] = kb.SpellCode;
