...
namespace FS.HISFC.Components.Nurse.Inject
{
    ...
    ///		/>
    /// </summary>
    public partial class ucDosageAlter : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        #region 构造函数
        ...
        /// 注射业务层
        /// </summary>
        private HISFC.BizLogic.Nurse.Inject injectManager = new FS.HISFC.BizLogic.Nurse.Inject();
        ...
        private void AddDataToTable(ArrayList al)
        {
            ...
            foreach (FS.HISFC.Models.Nurse.Inject info in al)
            {
                ...
                {
                    FS.HISFC.Models.Nurse.Inject lastInject = this.injectManager.QueryLastByPatient(info.Patient.PID.CardNO);
                    if (lastInject == null)
                    {
            ...
            {
                FS.HISFC.Models.Nurse.Inject info = ht[key] as FS.HISFC.Models.Nurse.Inject;
                //增加一行
                DataRow dr = this.dt.NewRow();
                this.dt.Rows.Add(dr);
                dr["患者姓名"] = info.Patient.Name;
                dr["开单医生"] = info.Item.Order.ReciptDoctor.Name;
