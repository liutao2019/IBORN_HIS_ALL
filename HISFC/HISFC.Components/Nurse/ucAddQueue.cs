...
namespace FS.HISFC.Components.Nurse
{
    ...
    internal partial class ucAddQueue : UserControl
    {
        ...
        private void Initcbo()
        {
            ...
            bool isNurseDept = controlIntegrate.GetControlParam<bool>("900010", true, false);//是否存在科室病区对应关系{F044FCF3-6736-4aaa-AA04-4088BB194C20}
            if (isNurseDept)//{F044FCF3-6736-4aaa-AA04-4088BB194C20}
            {
                al = this.depMgr.QueryDepartmentForArray(this.myResult.Nurse.ID);
            }
            else
            {
                al = this.depMgr.GetDeptmentByType("C");
            }
        ...
        private bool ValidData()
        {
            ...
            if (this.isNeedDoct)
            {
                ...
                {
                    MessageBox.Show("医生不能为空", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    this.cmbDoct.Focus();
                    return false;
                }
            }
        ...
        private int SaveData()
        {
            ...
            if (this.myResult.strTab.ToUpper() == "EDIT")
            {
                ...
                //停止队列更新排版信息 schemaBizLogic
                //if (!this.Queue.IsValid)
                //{
                //    if (this.schemaBizLogic.UpdateDoctSchemaValid(FS.FrameWork.Function.NConvert.ToInt32(this.Queue.IsValid).ToString(), this.Queue.QueueDate.ToShortDateString(), this.Queue.Noon.ID, this.Queue.AssignDept.ID, this.Queue.Doctor.ID, "14", "护士停止队列", this.myQueue.Operator.ID, this.seatMgr.GetDateTimeFromSysDateTime()) == -1)
                //    {
                //        FS.FrameWork.Management.PublicTrans.RollBack();
                //        MessageBox.Show("更新医生排版信息出错。" + this.myQueue.Err);
                //        return -1;
                //    }
                //}
            }
