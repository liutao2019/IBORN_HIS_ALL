...
namespace FS.HISFC.Components.HealthRecord.Visit
{
    ...
    public partial class ucVisitInfo : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        {
            toolBarService.AddToolButton("作废", "作废", (int)FS.FrameWork.WinForms.Classes.EnumImageList.S删除, true, false, null);
            toolBarService.AddToolButton("随访列表", "随访列表", (int)FS.FrameWork.WinForms.Classes.EnumImageList.S刷新, true, false, null);
        ...
        private int DeleteRecord()
        {
                ...
                == DialogResult.Yes)
            {
                ...
                for (int i = 0; i < fpVisitRecord_Sheet1.Rows.Count; i++)
                {
                    ...
                    as FS.HISFC.Models.HealthRecord.Visit.VisitRecord;//Tag中的对象
                        if (visitRecord != null)
                        {
