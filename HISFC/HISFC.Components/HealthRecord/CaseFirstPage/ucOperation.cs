...
namespace FS.HISFC.Components.HealthRecord.CaseFirstPage
{
    ...
    public partial class ucOperation : UserControl
    {
        ...
        private void LockFpEnter()
        {
            ...
            //this.fpSpread1_Sheet1.Columns[0].Locked = true;//手术日期
            this.fpSpread1_Sheet1.Columns[1].Width = 85; //ICD-9-CM-3编号
            //this.fpSpread1_Sheet1.Columns[1].Locked = true; //ICD-9-CM-3编号
            //this.fpSpread1_Sheet1.Columns[1].Visible = false;

            this.fpSpread1_Sheet1.Columns[2].Width = 200;//手术名称
            //this.fpSpread1_Sheet1.Columns[1].Locked = true;//手术名称
            this.fpSpread1_Sheet1.Columns[3].Width = 40;//术者 A
            //this.fpSpread1_Sheet1.Columns[2].Locked = true;//术者 A
            this.fpSpread1_Sheet1.Columns[4].Width = 40; //术者 B
            //this.fpSpread1_Sheet1.Columns[3].Locked = true;//术者 B
            this.fpSpread1_Sheet1.Columns[4].Visible = false;
            this.fpSpread1_Sheet1.Columns[5].Width = 40; //I 助
            //this.fpSpread1_Sheet1.Columns[4].Locked = true;//I 助
            this.fpSpread1_Sheet1.Columns[6].Width = 45; //II 助
            //this.fpSpread1_Sheet1.Columns[5].Locked = true;//II 助
            this.fpSpread1_Sheet1.Columns[7].Width = 40; //麻醉方式
            this.fpSpread1_Sheet1.Columns[7].Locked = true;//麻醉方式
            this.fpSpread1_Sheet1.Columns[7].Visible = false;
            this.fpSpread1_Sheet1.Columns[8].Width = 60; //切口愈合等级
            this.fpSpread1_Sheet1.Columns[8].Locked = true;//切口愈合等级
            this.fpSpread1_Sheet1.Columns[8].Visible = false;
            this.fpSpread1_Sheet1.Columns[9].Width = 55; //麻醉医师
            //this.fpSpread1_Sheet1.Columns[8].Locked = true;//麻醉医师
            this.fpSpread1_Sheet1.Columns[10].Width = 50; //切口
            //this.fpSpread1_Sheet1.Columns[10].Locked = true;//切口
            ...
            this.fpSpread1_Sheet1.Columns[19].Visible = false; //手术序列号
            this.fpSpread1_Sheet1.Columns[20].Width = 60;//麻醉方式1
            this.fpSpread1_Sheet1.Columns[21].Width = 60;//麻醉方式2
        }
        /// <summary>
        /// 清空原有的数据
        /// </summary>
        ...
        private bool GetChange(DataTable table, ArrayList list)
        {
            try
            {
                ...
                foreach (DataRow row in table.Rows)
                {
                    ...
                    bb.OperationDate = FS.FrameWork.Function.NConvert.ToDateTime(row["手术日期"]);
        ...
        private bool InitDataTable()
        {
            try
            {
                ...
                dtOperation.Columns.AddRange(new DataColumn[]{
                                                                 ...
                                                                 new DataColumn("手术序列号", strType),//19
                                                                 new DataColumn("麻醉方式1",strType),//20
                                                                 new DataColumn("麻醉方式2",strType)});//21
        ...
        {
            if (info.OperationDate.Date <= FS.FrameWork.Function.NConvert.ToDateTime("2011-10-09 23:00:00"))//更改了常数
            {
        ...
        public bool GetOperationList(ArrayList list)
        {
            try
            {
                ...
                for (int row = 0; row < this.fpSpread1_Sheet1.RowCount; row++)
                {
                    ...
                    bb.OperationInfo.ID = this.fpSpread1_Sheet1.Cells[row, 1].Text;//ICD-9-CM-3编号
                    if (bb.OperationInfo.ID == "")
                    {
                        bb.OperationInfo.ID = "MS999";
                    }

                    bb.OperationInfo.Name = this.fpSpread1_Sheet1.Cells[row, 2].Text;//手术名称
                    bb.FirDoctInfo.Name = this.fpSpread1_Sheet1.Cells[row, 3].Text; //术者 A
                    //bb.FourDoctInfo.Name = row["术者 B"].ToString();
                    ...
                    if (this.fpSpread1_Sheet1.Cells[row, 20].Text != "")//麻醉方式1
                    {
                        bb.MarcKind = NarcTypeHelper.GetID(this.fpSpread1_Sheet1.Cells[row, 20].Text);
                    }
                    if (this.fpSpread1_Sheet1.Cells[row, 21].Text != "")//麻醉方式2
                    {
                        bb.OpbOpa = NarcTypeHelper.GetID(this.fpSpread1_Sheet1.Cells[row, 21].Text);
                    }

                    bb.NarcDoctInfo.Name = this.fpSpread1_Sheet1.Cells[row, 9].Text;//麻醉医师

                    if (this.fpSpread1_Sheet1.Cells[row, 10].Text != "")//切口
                    {
                        bb.NickKind = NickTypeHelper.GetID(this.fpSpread1_Sheet1.Cells[row, 10].Text);
                    }
                    if (this.fpSpread1_Sheet1.Cells[row, 11].Text != "")//愈合
                    {
                    ...
                    bb.FirDoctInfo.ID = DoctorHelper.GetID(bb.FirDoctInfo.Name);//this.fpSpread1_Sheet1.Cells[row, 13].Text;// row["医师代码1"].ToString();
                    bb.FourDoctInfo.ID = "";// row["医师代码2"].ToString();
                    bb.SecDoctInfo.ID = DoctorHelper.GetID(bb.SecDoctInfo.Name);//this.fpSpread1_Sheet1.Cells[row, 15].Text;// row["助手编码1"].ToString();
                    bb.ThrDoctInfo.ID = DoctorHelper.GetID(bb.ThrDoctInfo.Name);//this.fpSpread1_Sheet1.Cells[row, 16].Text;// row["助手编码2"].ToString();
                    bb.NarcDoctInfo.ID = DoctorHelper.GetID(bb.NarcDoctInfo.Name);//this.fpSpread1_Sheet1.Cells[row, 17].Text;// row["麻醉医师编码"].ToString();
