...
namespace FS.HISFC.Components.Operation.ElectronicDisplay
{
    ...
    /// </summary>
    public partial class ucArrangeDisplay2 : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        public ucArrangeDisplay2()
        {
            InitializeComponent();
        ...
        ArrangeQuery2 arrQuery;

        #endregion

        #region 事件

        private void ucArrangeDisplay2_Load(object sender, EventArgs e)
        {
        ...
        protected override int OnSave(object sender, object neuObject)
        {
            ...
            try
            {
                dw.LoadHtmlTemp("ucArrangeDisplay2");
            }
            catch (Exception ee)
            {
        ...
        private int SaveSet(string[] SetInfo)
        {
            try
            {
                ...
                StreamWriter sr = new StreamWriter(path + "\\ArrangeDisplay\\DisplaySet2.xml", false, System.Text.Encoding.Default);
        ...
        {
            if (File.Exists(Application.StartupPath + "\\ArrangeDisplay\\DisplaySet2.xml"))
            {
                XmlDocument doc = new XmlDocument();
                try
                {
                    StreamReader sr = new StreamReader(Application.StartupPath + "\\ArrangeDisplay\\DisplaySet2.xml", System.Text.Encoding.Default);
        ...
        private void WriteHtmlFiles()
        {
            StreamWriter SW;
            string path = Application.StartupPath;
            path += "\\ArrangeDisplay\\ucArrangeDisplay2.html";
            ...
            SW.WriteLine("<link type=\"text/css\" rel=\"stylesheet\" href=\"ucArrangeDisplay2.css\" />");
            SW.WriteLine("</head>");
        ...
        private void ucArrangeDisplay2_ExitChanged(object sender, EventArgs e)
        {
            this.timer1.Enabled = false;
            this.dw.Close();
        }
    ...
    /// </summary>
    public class ArrangeQuery2
    {
        public ArrangeQuery2() { }

        /// <summary>
        /// 手术安排查询
        /// </summary>
        /// <param name="dt">默认查询的日期</param>
        public ArrangeQuery2(DateTime dt)
        {
            this.dtBegin = dt;
            this.dtEnd = dt;
        }
