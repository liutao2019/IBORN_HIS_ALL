...
{
    public partial class ucOpsTableAllot : FS.FrameWork.WinForms.Forms.BaseForm
    {
        ...
        //业务控制
        private FS.HISFC.BizLogic.Operation.OpsTableAlloc OpsMana = new FS.HISFC.BizLogic.Operation.OpsTableAlloc();
        //业务实体
        public FS.HISFC.Models.Operation.OpsTableAllot OpsObj = new FS.HISFC.Models.Operation.OpsTableAllot();
        //手术安排信息数组，元素为FS.HISFC.Object.Operator.OpsTableAllot类型
        ...
        {
            this.imageList1.Images.Add(FS.FrameWork.WinForms.Classes.Function.GetImage(FS.FrameWork.WinForms.Classes.EnumImageList.L楼房));
            this.imageList1.Images.Add(FS.FrameWork.WinForms.Classes.Function.GetImage(FS.FrameWork.WinForms.Classes.EnumImageList.L楼层));
            this.imageList1.Images.Add(FS.FrameWork.WinForms.Classes.Function.GetImage(FS.FrameWork.WinForms.Classes.EnumImageList.F房间));
            ...
            try
            {
                ...
                foreach (FS.HISFC.Models.Base.Department ThisDept in OpsDept)
                {
                    if (ThisDept.Name.IndexOf("护士站") <= 0)
                    {
        ...
        private void lvDept_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            foreach (FS.HISFC.Models.Operation.OpsTableAllot ThisAllot in this.OpsTableAllotAl)
            {
        ...
        private void btAdd_Click(object sender, EventArgs e)
        {
            ...
            #region 
            if (FS.FrameWork.Function.NConvert.ToInt32(nnudMon.Text) > 100)
            {
                MessageBox.Show("星期一分配数量过大");
                nnudMon.Focus();
                return;
            }
            if (FS.FrameWork.Function.NConvert.ToInt32(nnudTues.Text) > 100)
            {
                MessageBox.Show("星期二分配数量过大");
                nnudTues.Focus();
                return;
            }
            if (FS.FrameWork.Function.NConvert.ToInt32(nnudWed.Text) > 100)
            {
                MessageBox.Show("星期三分配数量过大");
                nnudWed.Focus();
                return;
            }
            if (FS.FrameWork.Function.NConvert.ToInt32(nnudThur.Text) > 100)
            {
                MessageBox.Show("星期四分配数量过大");
                nnudThur.Focus();
                return;
            }
            if (FS.FrameWork.Function.NConvert.ToInt32(nnudFri.Text) > 100)
            {
                MessageBox.Show("星期五分配数量过大");
                nnudFri.Focus();
                return;
            }
            if (FS.FrameWork.Function.NConvert.ToInt32(nnudSat.Text) > 100)
            {
                MessageBox.Show("星期六分配数量过大");
                nnudSat.Focus();
                return;
            }
            if (FS.FrameWork.Function.NConvert.ToInt32(nnudSun.Text) > 100)
            {
            ...
            //FS.FrameWork.Management.Transaction trans = new FS.FrameWork.Management.Transaction(this.OpsMana.Connection);
            //trans.BeginTransaction();

            this.OpsMana.SetTrans(FS.FrameWork.Management.PublicTrans.Trans);
        ...
        private void btDel_Click(object sender, EventArgs e)
        {
            ...
            if (this.tvShow.SelectedNode != null && this.tvShow.Nodes.Count != 0 && this.tvShow.Parent != null)
            {
                ...
                if (this.ncbDept.Tag.ToString() == sOldNode)
                {
                    ...
                    {
                        case "1":
                            this.nnudMon.Text = Convert.ToString(FS.FrameWork.Function.NConvert.ToInt32(this.nnudMon.Text) - thisAllot.Qty);
                            this.iOpsNum[0] = Convert.ToInt16( this.nnudMon.Text );
                            break;
                        case "2":
                            this.nnudTues.Text =   Convert.ToString(FS.FrameWork.Function.NConvert.ToInt32(this.nnudTues.Text) - thisAllot.Qty);
                            this.iOpsNum[1] = Convert.ToInt16(this.nnudTues.Text );
                            break;
                        case "3":
                            this.nnudWed.Text = Convert.ToString(FS.FrameWork.Function.NConvert.ToInt32(this.nnudWed.Text) - thisAllot.Qty);
                            this.iOpsNum[2] = Convert.ToInt16(this.nnudWed.Text );
                            break;
                        case "4":
                            this.nnudThur.Text =   Convert.ToString(FS.FrameWork.Function.NConvert.ToInt32(this.nnudThur.Text) - thisAllot.Qty);
                            this.iOpsNum[3] = Convert.ToInt16(this.nnudThur.Text );
                            break;
                        case "5":
                            this.nnudFri.Text =   Convert.ToString(FS.FrameWork.Function.NConvert.ToInt32(this.nnudFri.Text) - thisAllot.Qty);
                            this.iOpsNum[4] = Convert.ToInt16(this.nnudFri.Text );
                            break;
                        case "6":
                            this.nnudSat.Text =   Convert.ToString(FS.FrameWork.Function.NConvert.ToInt32(this.nnudSat.Text) - thisAllot.Qty);
                            this.iOpsNum[5] = Convert.ToInt16(this.nnudSat.Text );
                            break;
                        case "7":
                            this.nnudSun.Text =   Convert.ToString(FS.FrameWork.Function.NConvert.ToInt32(this.nnudSun.Text) - thisAllot.Qty);
                            this.iOpsNum[6] = Convert.ToInt16(this.nnudSun.Text );
                            break;
                    }
                    this.SumQtyToShow();
