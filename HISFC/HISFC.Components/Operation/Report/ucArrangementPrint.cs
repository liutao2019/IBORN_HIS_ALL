...
namespace FS.HISFC.Components.Operation.Report
{
    ...
    /// 手术一览表打印
    /// </summary>
    public partial class ucArrangementPrint : UserControl,FS.HISFC.BizProcess.Interface.Operation.IArrangePrint
    {
        /// <summary>
        /// 手术一览表打印
        /// </summary>
        public ucArrangementPrint()
        {
            InitializeComponent();
        ...
        public void AddAppliction(FS.HISFC.Models.Operation.OperationAppllication appliction)
        {
            ...
            this.fpSpread1_Sheet1.Cells[row, 10].Text = forediagnose;
            ...
            //总巡
            string MainIntinerantNurse = string.Empty;

            //接班人员
            string ShiftNurse = string.Empty;
            ...
            foreach (FS.HISFC.Models.Operation.ArrangeRole role in appliction.RoleAl)
            {
                ...
                else if (role.RoleType.ID.ToString() == FS.HISFC.Models.Operation.EnumOperationRole.OpsShiftNurse.ToString())
                {
                    ShiftNurse = role.Name;
                }
           ...
           // this.fpSpread1_Sheet1.Cells[row, 9].Text = operatorDoc;//手术医生
            this.fpSpread1_Sheet1.Cells[row, 14].Text = appliction.OperationDoctor.Name;  
            this.fpSpread1_Sheet1.Cells[row, 15].Text = operatorDoc1 + operatorDoc2 + operatorDoc3;//助手
            //this.fpSpread1_Sheet1.Cells[row, 10].Text =appliction.HelperAl.
            this.fpSpread1_Sheet1.Cells[row, 16].Text = WashingNurse1 + WashingNurse2 + WashingNurse3;//洗手护士
            this.fpSpread1_Sheet1.Cells[row, 17].Text = itinerantNurse1 + itinerantNurse2 + itinerantNurse3;//巡回护士
            this.fpSpread1_Sheet1.Cells[row, 19].Text = MainIntinerantNurse;
            //this.fpSpread1_Sheet1.Cells[row, 17].Text = AnaesthetistDoc1 + AnaesthetistDoc2;//麻醉医生
            this.fpSpread1_Sheet1.Cells[row, 20].Text = ShiftNurse; //接班人员
        }
