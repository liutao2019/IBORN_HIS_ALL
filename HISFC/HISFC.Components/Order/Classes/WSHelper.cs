...
namespace FS.HISFC.Components.Order.Classes
{
    ...
    public class WSHelper
    {
        ...
        {
            string @namespace = "FS.HISFC.Components.Order.Classes";
        ...
        private static string GetUrl(string hospitalID,URLTYPE urlType)
        {
            ...
            if (hospitalID == "IBORNGZ" || hospitalID == "IBORNCLINIC")
            {
                ...
                //url = @"http://localhost:8080/IbornMobileService.asmx";
            }
            ...
            try
            {
                ...
                string req = @"<?xml version='1.0' encoding='UTF-8'?>
                            <req>　                              
                              ...
                              <deptName>{3}</deptName>               
                              <doctName>{4}</doctName>              
                              <visitingTime>{5}</visitingTime>       
                              <operCode>{6}</operCode>              
                              <operName>{7}</operName>               
                              <operTime>{8}</operTime>               
                              <hospitalID>{9}</hospitalID>           
                              <hospitalName>{10}</hospitalName>      
                             </req>";
                ...
                req = string.Format(req, clinicNO, patientID, patientName, dept.Name, empl.Name, timeNow, empl.ID, empl.Name, timeNow, hospitalCode, hospitalName);
        ...
        /// <param name="ErrorMsg"></param>
        /// <returns></returns>
        public static int CallIn(string clinicNO, out string ResultCode, out string ErrorMsg)
        {
            ResultCode = "1";
            ErrorMsg = "";
            ...
            try
            {
                ...
                string url = WSHelper.GetUrl(hospitalCode, URLTYPE.HIS);
                req = string.Format(req, clinicNO, "2",empl.ID, empl.Name, timeNow, hospitalCode, hospitalName);
                string resultXml = WSHelper.InvokeWebService(url, "ChangeAssignState", new string[] { req }) as string;
        ...
        /// <param name="ErrorMsg"></param>
        /// <returns></returns>
        public static int CallOut(string clinicNO, out string ResultCode, out string ErrorMsg)
        {
            ResultCode = "1";
            ErrorMsg = "";
            ...
            try
            {
                ...
                string url = WSHelper.GetUrl(hospitalCode, URLTYPE.HIS);
                req = string.Format(req, clinicNO, "1", empl.ID, empl.Name, timeNow, hospitalCode, hospitalName);
                string resultXml = WSHelper.InvokeWebService(url, "ChangeAssignState", new string[] { req }) as string;
