...
namespace FS.HISFC.Components.Account.Controls
{
    ...
    /// 就诊卡发放
    /// </summary>
    public partial class ucCardManagerGyzl : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        public ucCardManagerGyzl()
        {
            InitializeComponent();
        }
        ...
        /// 是否自动匹配卡类型
        /// </summary>
        private bool isAutoMatchCardType = true;
        ...
        private string myCardNo = string.Empty;
        /// <summary>
        /// 修改时，传入卡号
        /// </summary>
        public string CardNO
        {
            ...
            {
                this.myCardNo = value;
                this.ucRegPatientInfo1.CardNO = value;
            }
            get
            {
        ...
        [Category("控件设置"), Description("是否自动匹配卡类型")]
        public bool IsAutoMatchCardType
        {
            get
            {
                return isAutoMatchCardType;
            }
            set
            {
                isAutoMatchCardType = value;
            }
        }
        ...
        private bool Valid()
        {
            ...
            FS.HISFC.Models.RADT.PatientInfo patient1 = this.ucRegPatientInfo1.GetPatientInfomation();
            if (string.IsNullOrEmpty(patient1.IDCard))
            {
                MessageBox.Show("请输入身份证后回车确认！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return false;
            }
            if (isJudgeByIDCard)
            {
        ...
        protected virtual void Save()
        {
            ...
            {
                accountCard.MarkNO = this.txtMarkNo.Text.Trim().Substring(0, 16);
                if (this.txtMarkNo.Text.Trim().Length >= 24)
                {
                    accountCard.SecurityCode = this.txtMarkNo.Text.Trim().Substring(16, 8);
                }
            }
            ...
            if (accountCard.MarkType.Name.Contains("健康"))
            {
                ...
                if (lstStopBackCard != null && lstStopBackCard.Count > 0)
                {
                    for (int idx = 0; idx < lstStopBackCard.Count; idx++)
                    {
                        lstChangeCard.Add(lstStopBackCard[idx]);
                    }
                }
                ...
                if (questPersonResponse.status.Equals("0"))
                {
                    ...
                    {
                        //发新卡
                        if (lstChangeCard.Count == 0)
                        {
                            generalResponse = cardService.newCard(newCardRequest);
                        }
                        else
                        {
                            List<RHINCardServiceImplService.CardType> cardTypes = new List<RHINCardServiceImplService.CardType>();

                            RHINCardServiceImplService.CardType replacedCardType;
                            foreach (AccountCard card in lstChangeCard)
                            {
                                if (card.MarkType.Name.Contains("健康"))
                                {
                                    replacedCardType = new RHINCardServiceImplService.CardType();
                                    replacedCardType.type = "0";
                                    replacedCardType.number = card.MarkNO;
                                    replacedCardType.verfyNumber = card.SecurityCode;
                                    cardTypes.Add(replacedCardType);
                                }
                                else
                                {
                                    replacedCardType = new RHINCardServiceImplService.CardType();
                                    replacedCardType.type = "7455350760";
                                    replacedCardType.number = card.Patient.PID.CardNO;
                                    cardTypes.Add(replacedCardType);
                                }
                            }
                            personType.cards = cardTypes.ToArray();

                            RHINCardServiceImplService.ChangeCardRequestType changeCardRequest = new RHINCardServiceImplService.ChangeCardRequestType();

                            RHINCardServiceImplService.SimplePersonType applyPerson = new RHINCardServiceImplService.SimplePersonType();
                            applyPerson.id = this.getIDCardType(accountCard);
                            applyPerson.name = accountCard.Patient.Name;

                            changeCardRequest.authObject = this.getAuthObject();
                            changeCardRequest.newCard = cardType;
                            changeCardRequest.person = personType;
                            changeCardRequest.secrecyLevel = "0";
                            changeCardRequest.onlineEnquiry = "1";
                            changeCardRequest.accessToEHR = "1";
                            changeCardRequest.applyPerson = applyPerson;

                            generalResponse = cardService.changeCard(changeCardRequest);
                        }
                    }
                    else
                    {
        ...
        protected virtual int QueryPatientInfo(string patientName, string sexId, string pactId, string CassNo, string idCardType, string idCard, string SSN)
        {
            ...
            List<AccountCard> list = accountManager.GetAccountCard(patientName, sexId, pactId, CassNo, idCardType, idCard, SSN);
        ...
        private void ucCardManager_Load(object sender, EventArgs e)
        {
            ...
            if (isAutoMatchCardType)
            {
                if (!string.IsNullOrEmpty(this.txtMarkNo.Text))
                {
                    string curMarkType = string.Empty;
                    ArrayList cardTypeList = constManager.GetList("MarkType");
                    bool isChecked = false;
                    foreach (FS.HISFC.Models.Base.Const markType in cardTypeList)
                    {
                        Regex regex = new Regex(@markType.UserCode, RegexOptions.Multiline);
                        if (regex.IsMatch(txtMarkNo.Text))
                        {
                            curMarkType = markType.Name;
                            break;
                        }
                    }
                    for (int i = 0; i < this.cmbMarkType.Items.Count; i++)
                    {
                        if ((this.cmbMarkType.Items[i]).Equals(curMarkType))
                        {
                            this.cmbMarkType.SelectedIndex = i;
                            isChecked = true;
                            break;
                        }
                    }
                    if (!isChecked)
                    {
                        MessageBox.Show("没有匹配的卡规则,请重新确认卡号");
                        this.txtMarkNo.Text = "";
                    }
                }
            }
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            {
                #region 健康卡
                if (this.ActiveControl.Name == "txtMarkNo")//"txtMarkNo"
                {
                    //外接屏显示{67C90AAC-CFAD-4089-96F4-9F9FC82D8754}
                    if (Screen.AllScreens.Length > 1)
                    {
                ...
                if (this.ActiveControl.Name == "txtMedicalCardNo")//"txtMarkNo"
                {

                    FS.HISFC.Models.Account.AccountCard accountCard = new FS.HISFC.Models.Account.AccountCard();
                    int returnValue = 0;
                    returnValue = accountManager.GetCardByRule(this.txtMedicalCardNo.Text, ref accountCard);
                    ...
                    // this.txtMarkNo.Text = accountCard.MarkNO + accountCard.SecurityCode;
                    if (returnValue <= 0)
                    {
                        if (accountCard.MarkType.Name.Contains("健康"))
                        {
                            ...
                            RHINCardServiceImplService.CardType cardType = new RHINCardServiceImplService.CardType();
                            cardType.type = "0";
                            cardType.number = accountCard.MarkNO.Substring(0, 16);
                            ...
                            if (questPersonResponse.status.Equals("0"))
                            {
                                if (questPersonResponse.size > 0)
                                {
                                    RHINCardServiceImplService.PersonType person = questPersonResponse.persons[0];
                                    this.ucRegPatientInfo1.SetPatient(this.getPatientInfo(person));
                                    this.txtMarkNo.Text = this.txtMedicalCardNo.Text;
                                    this.txtMedicalCardNo.Text = "";
                                    this.txtMarkNo.Focus();
                                }
                        ...
                        {
                            MessageBox.Show("无病人记录！请重新发卡");
                            return false;
                        }
                    }
                    else if (returnValue == 1)
                    {
                        this.txtMedicalCardNo.Text = accountCard.Patient.PID.CardNO;
                        this.ucRegPatientInfo1.CardNO = accountCard.Patient.PID.CardNO;
                        this.txtMarkNo.Focus();
        ...
        private string getHISSexCode(string sexCode)
        {
        ...
        private string getHISMaritalStatus(string maritalStatus)
        {
            string result = string.Empty;
            if (maritalStatus == "10")
            {
            ...
            else if (maritalStatus == "20")
            {
                result = "M";
            }
            else if (maritalStatus == "30")
            {
                result = "W";
            }
            else if (maritalStatus == "40")
            {
                result = "D";
            }
            else if (maritalStatus == "90")
            {
                result = "R";
            }
            return result;
        ...
        private FS.HISFC.Models.RADT.PatientInfo getPatientInfo(RHINCardServiceImplService.PersonType personType)
        {
            FS.HISFC.Models.RADT.PatientInfo patInfo = new FS.HISFC.Models.RADT.PatientInfo();

            patInfo.Name = personType.name;
            patInfo.Sex.ID = this.getHISSexCode(personType.gender);
            patInfo.Birthday = (DateTime)personType.birthDay;
            patInfo.MaritalStatus.ID = this.getHISMaritalStatus(personType.maritalStatus);
            if (personType.ids != null)
            {
                if (personType.ids.Length > 0)
                {
                    for (int i = 0; i < personType.ids.Length; i++)
                    {
                        if (personType.ids[i].type == "01")
                        {
                            patInfo.IDCardType.ID = personType.ids[i].type;
                            patInfo.IDCard = personType.ids[i].number;
                        }
                    }

                    //patInfo.IDCardType.ID = personType.ids[0].type;
                    // patInfo.IDCard = personType.ids[0].number;
                }
            }


            if (personType.asEmployer != null)
            {
                patInfo.CompanyName = personType.asEmployer.name;
                patInfo.PhoneBusiness = personType.asEmployer.telephoneNumber;
            }

            if (personType.addressOfResidence != null)
            {
                patInfo.AddressHome = personType.addressOfResidence.houseNumber;
            }

            patInfo.PhoneHome = personType.telephoneNumber;

            if (personType.contactPerson != null)
            {
                patInfo.Kin.Name = personType.contactPerson.Name;
                patInfo.Kin.RelationPhone = personType.contactPerson.telephoneNumber;
                if (personType.contactPerson.Address != null)
                {
                    patInfo.Kin.RelationAddress = personType.contactPerson.Address.houseNumber;
                }
            }
