...
namespace FS.HISFC.Components.Registration
{
    public partial class ucUnCancel : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        private int save()
        {
            ...
            {
                //t.RollBack() ;
                MessageBox.Show( FS.FrameWork.Management.Language.Msg( "该挂号信息不是作废号,不能取消作废" ), "提示" );
                return -1;
            }

            if (reg.BalanceOperStat.IsCheck)
            {
                MessageBox.Show( FS.FrameWork.Management.Language.Msg( "该挂号信息已经日结,不能取消作废" ), "提示" );
                return -1;
            }
            ...
            MessageBox.Show( FS.FrameWork.Management.Language.Msg( "取消作废成功" ), "提示" );
