...
namespace FS.HISFC.Components.Registration.NewRegister
{
    ...
    public partial class ucCancelRegister : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Forms.IInterfaceContainer, FS.FrameWork.WinForms.Classes.IPreArrange
    {
        ...
        /// 挂号支付方式管理类
        /// </summary>
        private FS.HISFC.BizLogic.Registration.RegPayMode regPayModeMgr = new FS.HISFC.BizLogic.Registration.RegPayMode();
        /// <summary>
        /// 挂号费用明细管理类
        /// </summary>
        private FS.HISFC.BizLogic.Registration.RegDetail regDetailMgr = new FS.HISFC.BizLogic.Registration.RegDetail();
        ...
        /// <summary>
        /// 退号是否提示填写退费原因// {5A6C578A-D565-42c8-B3FA-B4A52D1FABFB}
        /// </summary>
        private bool isShowWriteReason = true;

        [Category("控件设置"), Description("退号是否提示填写退费原因"), DefaultValue(true)]
        public bool IsShowWriteReason
        {
            get
            {
                return isShowWriteReason;
            }
            set
            {
                isShowWriteReason = value;
            }
        }
        /// <summary>
        /// //{DA67A335-E85E-46e1-A672-4DB409BCC11B}
        ...
        private void QueryRegisterByIDCard(string IDCard)
        {
            ...
            if (arlRegInfo != null && arlRegInfo.Count > 0)
            {
                ...
                else
                {
                    ...
                    fpShow.StartPosition = FormStartPosition.CenterParent;
                    fpShow.Width = 600;
                    fpShow.Height = 300;
        ...
        private void QueryRegisterByInvoiceNO(string invoiceNo)
        {
            ...
            List<RegisterFeeDetail> detailList = this.regDetailMgr.QueryDetailByInvoiceNO(invoiceNo);
            ...
            string clinicCode = detailList[0].ClinicNO;
        ...
        private int save()
        {
            ...
            List<RegisterFeeDetail> detailRetFee = new List<RegisterFeeDetail>(); //退费信息
            List<RegisterFeeDetail> detailUnRetFee = new List<RegisterFeeDetail>(); //不退费

            #region 获取退费数据
            RegisterFeeDetail cardFee = null;
            bool blnCheck = false;
            if (this.neuTabControl1.SelectedIndex == 0)
            {
                if (this.fpSpread1_Sheet1.Rows.Count >= 2)
                ...
                {
                    cardFee = this.neuSpread1_Sheet1.Rows[idx].Tag as RegisterFeeDetail;
                    blnCheck = FS.FrameWork.Function.NConvert.ToBoolean(this.neuSpread1_Sheet1.Cells[idx, 0].Value);
                    if (blnCheck)
                    {
                        detailRetFee.Add(cardFee);
                    }
                    else
                    {
                        detailUnRetFee.Add(cardFee);
                    }
                }

                if (blnRegReturn && IsMustAllReturnFee)
                {
                    if (detailUnRetFee.Count > 0)
                    {
            ...
            else
            {
                ...
                {
                    cardFee = this.neuSpread2_Sheet1.Rows[idx].Tag as RegisterFeeDetail;
            ...
            if (blnReturnRegFee)
            {
                //{DDC4DC99-38CB-4f01-9A1C-3CC97F272053}
                if ((detailRetFee == null || detailRetFee.Count <= 0 ) && !string.IsNullOrEmpty(regSelect.InvoiceNO))
                {
            ...
            {
                if (detailRetFee == null || detailRetFee.Count != 1)
                {
            ...
            if (MessageBox.Show("请选择退费方式，只针对非账户支付的费用，账户支付的费用将退回原账户！", "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                frmPayModeChoose frmPayModeChoose = new frmPayModeChoose();
                frmPayModeChoose.ShowDialog();
                payModeCode = frmPayModeChoose.PayModeCode;
                if (string.IsNullOrEmpty(payModeCode))
                {
                    return -1;
                }
            }
            ...
            {
                if (this.isUseLogout && detailRetFee.Count > 0 && detailRetFee[0].Oper.ID.Equals(regMgr.Operator.ID) && detailRetFee[0].IsBalance == false)
                {
                    flag = FS.HISFC.BizLogic.Registration.EnumUpdateStatus.Bad;
                    registerStatus = FS.HISFC.Models.Base.EnumRegisterStatus.Bad;
                }
            ...
            try
            {
                ...
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                    MessageBox.Show(FS.FrameWork.Management.Language.Msg("请选择退号记录！"), "提示");
                    return -1;
                }
                ...
                if (detailUnRetFee != null && detailUnRetFee.Count > 0)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                    MessageBox.Show(FS.FrameWork.Management.Language.Msg("费用必须全退"), "提示");
                    return -1;
                }

                if (isShowWriteReason)// {5A6C578A-D565-42c8-B3FA-B4A52D1FABFB}
                {
                    frmReturnReason frmReturnReason = new frmReturnReason();
                    frmReturnReason.ShowDialog();
                    returbReason = frmReturnReason.Reason;
                ...
                {
                    oldInvoiceNo = detailRetFee[0].InvoiceNo;
                }

                couponInvoiceNO = oldInvoiceNo;
                ...
                if (string.IsNullOrEmpty(oldInvoiceNo) == false)
                {
                    ...
                    if (!this.IsEnumEqualDataBase&&registerStatus==FS.HISFC.Models.Base.EnumRegisterStatus.Back)
                    {
                        ...
                        List<RegisterFeeDetail> details = this.regDetailMgr.QueryDetailByInvoiceNO(oldInvoiceNo);
                        if (details == null)
                        {
                            FS.FrameWork.Management.PublicTrans.RollBack();
                            MessageBox.Show(FS.FrameWork.Management.Language.Msg("操作失败！" + this.regDetailMgr.Err), "提示");
                            return -1;
                        }

                        foreach (RegisterFeeDetail detail in details)
                        {
                            //更新旧记录
                            detail.CancelFlag = 2;
                            detail.CancelOper.ID = FS.FrameWork.Management.Connection.Operator.ID;
                            detail.CancelOper.OperTime = this.regDetailMgr.GetDateTimeFromSysDateTime();
                            if (this.regDetailMgr.Update(detail) < 0)
                            {
                                FS.FrameWork.Management.PublicTrans.RollBack();
                                MessageBox.Show(FS.FrameWork.Management.Language.Msg("操作失败！" + this.regDetailMgr.Err), "提示");
                                return -1;
                            ...
                            //插入新纪录
                            detail.TransType = TransTypes.Negative;
                            detail.CancelFlag = 2;
                            ...
                            detail.CancelOper.ID = FS.FrameWork.Management.Connection.Operator.ID;
                            detail.CancelOper.OperTime = this.regDetailMgr.GetDateTimeFromSysDateTime();
                            detail.BalanceOper = new OperEnvironment();
                            detail.BalanceNo = "";
                            if (this.regDetailMgr.Insert(detail) < 0)
                            {
                                FS.FrameWork.Management.PublicTrans.RollBack();
                                MessageBox.Show(FS.FrameWork.Management.Language.Msg("操作失败！" + this.regDetailMgr.Err), "提示");
                                return -1;
                        ...
                        List<RegisterPayMode> paymodes = this.regPayModeMgr.QueryDetailByInvoiceNO(oldInvoiceNo);
                        if (paymodes == null)
                        {
                            FS.FrameWork.Management.PublicTrans.RollBack();
                            MessageBox.Show(FS.FrameWork.Management.Language.Msg("操作失败！" + this.regPayModeMgr.Err), "提示");
                            return -1;
                        }
                        ...
                        foreach (RegisterPayMode paymode in paymodes)
                        {
                            //{9F2C17EC-0AE6-4df8-B959-F4D99314E227}
                            //判断该支付方式是否计算积分
                            if (obj.Name.Contains(paymode.Mode_Code.ToString()))
                            {
                                ...
                                if (accountPay.UpdateCoupon(regSelect.PID.CardNO, -paymode.Tot_cost, paymode.InvoiceNo) <= 0)
                                {
                                    FS.FrameWork.Management.PublicTrans.RollBack();
                                    MessageBox.Show(FS.FrameWork.Management.Language.Msg("计算积分出错!" + accountPay.Err), "提示");
                                    return -1;
                                }
                            }
                            ...
                            {
                                operateCoupon -= paymode.Tot_cost;
                                //cashCouponAmount -= paymode.Tot_cost;
                            }

                            if ("CO" == paymode.Mode_Code)
                            {
                                costCoupon -= paymode.Tot_cost;
                            }

                            if (paymode.Mode_Code == "YS" || paymode.Mode_Code == "DC")
                            {
                                FS.HISFC.Models.RADT.PatientInfo PatientInfo = accountMgr.GetPatientInfoByCardNO(regSelect.PID.CardNO);
                                if (accountPay.OutpatientPay(PatientInfo,
                                                   paymode.AccountID,
                                                   paymode.AccountType,
                                                   paymode.Mode_Code == "YS"?paymode.Tot_cost:0,
                                                   paymode.Mode_Code == "YS" ? 0:paymode.Tot_cost,
                                                   paymode.InvoiceNo,
                                                   PatientInfo,
                                ...
                                {
                                    FS.FrameWork.Management.PublicTrans.RollBack();
                                    MessageBox.Show(FS.FrameWork.Management.Language.Msg("操作失败！" + this.accountPay.Err), "提示");
                                    return -1;
                                }
                            }
                            ...
                            paymode.CancelOper.OperTime = this.regDetailMgr.GetDateTimeFromSysDateTime();
                            if (this.regPayModeMgr.Update(paymode) < 0)
                            {
                                FS.FrameWork.Management.PublicTrans.RollBack();
                                MessageBox.Show(FS.FrameWork.Management.Language.Msg("操作失败！" + this.regPayModeMgr.Err), "提示");
                                return -1;
                            ...
                            //插入新纪录
                            paymode.TransType = TransTypes.Negative;
                            paymode.CancelFlag = 2;
                            paymode.Tot_cost = paymode.Real_cost = -paymode.Real_cost;
                            //{6F583C8D-0FB9-48a8-8842-58C5EFAAF369}退号支付方式 操作人修复
                            paymode.Oper.ID = FS.FrameWork.Management.Connection.Operator.ID;
                            paymode.Oper.ID = FS.FrameWork.Management.Connection.Operator.ID;
                            paymode.Oper.OperTime = this.regDetailMgr.GetDateTimeFromSysDateTime();
                            paymode.CancelOper.ID = FS.FrameWork.Management.Connection.Operator.ID;
                            paymode.CancelOper.OperTime = this.regDetailMgr.GetDateTimeFromSysDateTime();
                            paymode.BalanceOper = new OperEnvironment();
                            paymode.BalanceNo = "";
                            if (paymode.Mode_Code != "YS" && paymode.Mode_Code != "DC" && paymode.Mode_Code != "RC" && paymode.Mode_Code != "SW")
                            {
                                //{152399FC-8086-488b-8A53-9BC6C26D8321}
                                if (!string.IsNullOrEmpty(payModeCode) && !payModeCode.Equals("ADYYSFH"))
                                {
                                    paymode.Mode_Code = payModeCode;
                                }
                            }
                            if (this.regPayModeMgr.Insert(paymode) < 0)
                            {
                                FS.FrameWork.Management.PublicTrans.RollBack();
                                MessageBox.Show(FS.FrameWork.Management.Language.Msg("操作失败！" + this.regPayModeMgr.Err), "提示");
                                return -1;
                        ...
                        //    if (cashCouponPrc.CashCouponSave("GHTF", regSelect.PID.CardNO, regSelect.InvoiceNO, cashCouponAmount, ref errText) <= 0)
                        //    {
                        //        FS.FrameWork.Management.PublicTrans.RollBack();
                    ...
                    else
                    {
                        ...
                        {
                            //更新旧记录
                            detail.CancelFlag = (int)registerStatus;
                            detail.CancelOper.ID = FS.FrameWork.Management.Connection.Operator.ID;
                            detail.CancelOper.OperTime = this.regDetailMgr.GetDateTimeFromSysDateTime();
                            if (this.regDetailMgr.Update(detail) < 0)
                            {
                            ...
                            detail.Etc_cost = -detail.Etc_cost;
                            detail.CancelFlag = (int)registerStatus;
                            //{1AB8083B-5CA3-4bf9-8771-A4551C92A701} 更改取消记录操作人
                            detail.Oper.ID = FS.FrameWork.Management.Connection.Operator.ID;
                            detail.Oper.OperTime = this.regDetailMgr.GetDateTimeFromSysDateTime();
                            detail.Oper.ID = FS.FrameWork.Management.Connection.Operator.ID;
                            detail.CancelOper.ID = FS.FrameWork.Management.Connection.Operator.ID;
                            detail.CancelOper.OperTime = this.regDetailMgr.GetDateTimeFromSysDateTime();
                            detail.BalanceOper = new OperEnvironment();
                ...
                if (blnReturnRegFee)
                {
                    ...
                    objReturn.Mark1 = returbReason;//退费原因
            ...
            if (IsCouponModuleInUse)
            {
                ...
                if (costCoupon != 0)
                {
                    reqRtn = FS.HISFC.BizProcess.Integrate.WSHelper.CostCoupon(regSelect.PID.CardNO, regSelect.Name, regSelect.PID.CardNO, regSelect.Name, "GHTF", couponInvoiceNO, costCoupon, 0.0m, out resultCode, out errorMsg);
                    if (reqRtn < 0)
                    {
                        FS.FrameWork.Management.PublicTrans.RollBack();
                ...
                if (operateCoupon != 0)
                {
                    reqRtn = FS.HISFC.BizProcess.Integrate.WSHelper.OperateCoupon(regSelect.PID.CardNO, regSelect.Name, "GHTF", couponInvoiceNO, operateCoupon, 0.0m, out resultCode, out errorMsg);
                    if (reqRtn < 0)
                    {
                        FS.FrameWork.Management.PublicTrans.RollBack();
                        MessageBox.Show("处理会员积分出错:" + errorMsg);
        ...
        private void fpSpread1_SelectionChanged(object sender, FarPoint.Win.Spread.SelectionChangedEventArgs e)
        {
            ...
            {
                List<RegisterFeeDetail> detailList = this.regDetailMgr.QueryDetailByInvoiceNO(obj.InvoiceNO);
                ...
                if (detailList != null && detailList.Count > 0)
                {
                    AddCardFeeByRegister(detailList, FS.FrameWork.Function.NConvert.ToBoolean(this.fpSpread1_Sheet1.Cells[this.fpSpread1_Sheet1.ActiveRowIndex, 0].Value));
                }
            }
        ...
        private void AddCardFeeByRegister(List<RegisterFeeDetail> lstDetailFee, bool isSelect)
        {
            decimal decTotCost = 0;
            this.neuSpread1_Sheet1.RowCount = 0;
            if (lstDetailFee == null || lstDetailFee.Count <= 0)
                return;

            int idx = 0;
            string strTypeName = "";
            foreach (RegisterFeeDetail detailFee in lstDetailFee)
            {
                if (detailFee.CancelFlag != 1)
                    continue;
                ...
                this.neuSpread1_Sheet1.SetValue(idx, 1, detailFee.FeeItem.Name, false);
                this.neuSpread1_Sheet1.SetValue(idx, 2, detailFee.Real_cost.ToString(), false);
                this.neuSpread1_Sheet1.SetValue(idx, 3, detailFee.Gift_cost.ToString(), false);
                this.neuSpread1_Sheet1.SetValue(idx, 4, detailFee.Etc_cost.ToString(), false);
                this.neuSpread1_Sheet1.SetValue(idx, 5, detailFee.Oper.ID, false);
                this.neuSpread1_Sheet1.SetValue(idx, 6, detailFee.Oper.OperTime.ToString("yyyy-MM-dd HH:mm:ss"), false);
                this.neuSpread1_Sheet1.Rows[idx].Tag = detailFee;
                decTotCost += detailFee.Real_cost + detailFee.Gift_cost + detailFee.Etc_cost;
            }
