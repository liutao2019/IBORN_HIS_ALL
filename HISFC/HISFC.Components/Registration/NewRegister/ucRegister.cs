...
namespace FS.HISFC.Components.Registration.NewRegister
{
    public partial class ucRegister : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        ...
        /// 是否启用挂号数据节点模块
        /// </summary>
        private bool IsRegisterStatisticsPointInUse = false;
        ...
        /// <summary>
        /// 是否打印腕带// {AD405AA0-3101-46c0-A0B6-846DC3AAB10A}
        /// </summary>
        [Category("控件设置"), Description("是否打印腕带"), DefaultValue(false)]
        public bool IsPrintBracelet
        {
            get { return isPrintBracelet; }
            set { isPrintBracelet = value; }
        }
        ...
        private bool isDeseaseMustChoose = false;
        /// <summary>
        /// 挂号医生是否随着科室变化
        /// </summary>
        [Category("控件设置"), Description("是否必须填写一级病种二级病种"), DefaultValue(false)]
        public bool IsDeseaseMustChoose
        {
            get { return isDeseaseMustChoose; }
            set { isDeseaseMustChoose = value; }
        }

        //{508278C5-1966-40e1-9A05-A28976447CAA}
        private bool isDeseaseDefault = true;
        /// <summary>
        /// 挂号医生是否随着科室变化
        /// </summary>
        [Category("控件设置"), Description("是否默认填充一级病种二级病种"), DefaultValue(false)]
        public bool IsDeseaseDefault
        {
            get { return isDeseaseDefault; }
            set { isDeseaseDefault = value; }
        }
        ...
        //{D3268012-7646-4c69-943C-CF8487AB7997}
        private bool isShowPackageCheckBox = false;
        /// <summary>
        /// 是否判断套餐内是否有挂号费
        /// </summary>
        [Category("控件设置"), Description("是否显示套餐挂号勾选框"), DefaultValue(false)]
        public bool IsShowPackageCheckBox
        {
            get { return isShowPackageCheckBox; }
            set { isShowPackageCheckBox = value; }
        }
        /// <summary>
        /// 医保接口代理
        /// </summary>
        ...
        /// 积分支付
        /// </summary>
        private ArrayList couponPayList = new ArrayList();
        ...
        private void InitControls()
        {
            ...
            this.InitDoct();
            this.InitPact();
            //{4913F006-9D91-43c7-826C-B7BA5A7F793E}
            this.InitNoon();
            this.InitChooseUC();
            this.InitOtherCtls();
            this.SetRegList();
            ...
            //imgeallery.Image = imageList1.Images[4];
        ...
        private void AddEvents()
        {
            ...
            this.btnCoupon.Click += new EventHandler(btnCoupon_Click);
            ...
            //{36D75F21-3F54-4dba-840E-0195A0717771}
            this.chkPackage.CheckStateChanged += new EventHandler(chkPackage_CheckStateChanged);
        }
        ...
        private void cmbClass1Desease_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            ArrayList class2desease = this.conMgr.QueryConstantList(queryCode);
            ...
            //{508278C5-1966-40e1-9A05-A28976447CAA}
            if (this.cmbClass2Desease.Items != null && this.cmbClass2Desease.Items.Count > 0 && this.IsDeseaseDefault)
            {
                this.cmbClass2Desease.SelectedIndex = 0;
            }
        }

        private void chkPackage_CheckStateChanged(object sender, EventArgs e)
        {
            this.IsDocterChargeFee =  this.chkPackage.Checked;
            this.getCost();
        }
        ...
        private void GetParameter()
        {
            ...
            //是否启用挂号数据节点模块
            this.IsRegisterStatisticsPointInUse = (this.ctlMgr.QueryControlerInfo("CPP001") == "1");

            //{5D855B3C-5A4F-42c9-931D-333D0A01D809}
            this.Class1Desease = this.conMgr.QueryConstantList(CLASS1DESEASE);
        }
        ...
        private void SetStyles()
        {
            ...
            this.fpRegList.ColumnHeader.Cells[0, 9].Text = "原总金额";
            this.fpRegList.Columns[9].Width = 80;
            this.fpRegList.Columns[9].AllowAutoSort = true;
            this.fpRegList.ColumnHeader.Cells[0, 10].Text = "实际缴费";// {F53BD032-1D92-4447-8E20-6C38033AA607}
            this.fpRegList.Columns[10].Width = 80;
            this.fpRegList.Columns[10].AllowAutoSort = true;
            this.fpRegList.ColumnHeader.Cells[0, 11].Text = "支付方式";
            this.fpRegList.Columns[11].Width = 80;
            this.fpRegList.Columns[11].AllowAutoSort = true;
            this.fpRegList.ColumnHeader.Cells[0, 12].Text = "记帐诊金金额";
            this.fpRegList.Columns[12].Width = 80;
            this.fpRegList.Columns[12].AllowAutoSort = true;
            this.fpRegList.ColumnHeader.Cells[0, 13].Text = "票据电脑号";
            this.fpRegList.Columns[13].Width = 100;
            this.fpRegList.Columns[13].AllowAutoSort = true;
            this.fpRegList.Columns.Count = 14;
        ...
        private int InitDoct()
        {
            ...
            DataSet ds = this.SchemaMgr.QueryDoct(this.dtSeeDate.Value, this.regMgr.GetDateTimeFromSysDateTime());
        ...
        private int InitRealDoctor()
        {
           // FS.FrameWork.Management.Connection.Operator.
            ...
            if (hospitalID == "BELLAIRE")
            {
                ArrayList realDoctorList = this.conMgr.QueryConstantList("RealDoct");
                if (realDoctorList == null)
                {
                    MessageBox.Show("获取实际看诊医生出错!" + this.conMgr.Err, "提示");
                    return -1;
                }
                this.cmbRealDoctor.AddItems(realDoctorList);
            }
            else
            {
                neuLabel13.Visible = false;
                cmbRealDoctor.Visible = false;
                plRegInfo.Height = 71;
            }
            return 0;
        }
        ...
        {
            //{477BED8D-9473-4e5e-86E9-C39B185D4540}
            if (this.IFeeDiagReg == 0 && this.IsDocterChargeFee)
            {
                this.tbInvoiceNO.Visible = false;
                this.tbRealInvoiceNO.Visible = false;
            ...
            {
                //{C7D96888-A677-45d8-8BB7-CCDBC1E5B5CD}
                this.tbInvoiceNO.Visible = true;
                this.tbRealInvoiceNO.Visible = true;
        ...
        private void addRegister(FS.HISFC.Models.Registration.Register obj)
        {
            ...
            this.fpRegList.SetValue(cnt, 10, obj.EcoCost, false);// {F53BD032-1D92-4447-8E20-6C38033AA607}
            this.fpRegList.SetValue(cnt, 11, payMode(obj.InvoiceNO), false);
            this.fpRegList.SetValue(cnt, 12, obj.PubCost, false);
            this.fpRegList.SetValue(cnt, 13, obj.InvoiceNO, false);
        ...
        {
            this.chkPackage.Visible = this.IsShowPackageCheckBox;
            if (this.IsShowPackageCheckBox)
            {
                this.IsDocterChargeFee = false;
            }
            ...
            if (e.KeyCode == Keys.Enter)
            {
                try
                {
                    ...
                    if (rev > 0)
                    {
                        ...
                        if (accountCard != null)
                        {
                            FS.HISFC.Models.Account.Account account = accountMgr.GetAccountByCardNoEX(accountCard.Patient.PID.CardNO);
                            if (account != null)
                            {
                            ...
                            {
                                // MessageBox.Show("该患者没有账户信息！");{48AA1A53-C32D-4a05-BA64-A5AB9AF50564}
                        ...
                        //{51C02BB2-BBC3-45c2-B985-5E14ECEB0943}
                        if (patient.Memo == "作废患者")
                        {
                            throw new Exception("该患者信息已经作废，请尝试通过姓名或电话进行检索！");
                        }
        ...
        private void cmbDept_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                //{5D855B3C-5A4F-42c9-931D-333D0A01D809}
                this.clearDeseaseInfo();
                this.setDeseaseInfo(this.cmbDept.Tag.ToString());
            }
            catch (Exception ex)
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg(ex.Message), FS.FrameWork.Management.Language.Msg("提示"));
        ...
        private void cmbDoctor_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                //普通挂号级别无需选择医生
                FS.HISFC.Models.Registration.RegLevel regLevel = (FS.HISFC.Models.Registration.RegLevel)this.cmbRegLevel.SelectedItem;
                if (regLevel != null && !regLevel.IsExpert && !regLevel.IsFaculty && !regLevel.IsSpecial)
                {
                    this.cmbDoctor.Tag = "";
                    throw new Exception("[" + regLevel.Name + "]此挂号级别无需选择医生！");

                }

                #region 修改{DF90A779-384E-4334-81FD-D1F5D5BCF47C}
                ...
                //选了科室的话查该科室下医生的排版，没选科室的话查所有排班
                //{4913F006-9D91-43c7-826C-B7BA5A7F793E}
                if (this.cmbDept.Tag == null || string.IsNullOrEmpty(this.cmbDept.Tag.ToString()))
                {
                    this.ucChooseDate.QueryDoctBooking(this.dtSeeDate.Value, this.cmbDoctor.Tag.ToString(), Registration.RegTypeNUM.Expert);
                }
                else
                {
                    this.ucChooseDate.QueryDoctBooking(this.dtSeeDate.Value, this.cmbDoctor.Tag.ToString(), this.cmbDept.Tag.ToString(), Registration.RegTypeNUM.Expert);
                }

                //默认显示第一条符合条件（时间未过期、限额未满）的排班信息
                FS.HISFC.Models.Registration.Schema schema = this.ucChooseDate.GetValidBooking(Registration.RegTypeNUM.Expert);

                if (schema == null)//没有符合条件的
                {
                    this.cmbDoctor.Tag = "";
                    this.SetDefaultSeeDateTime(this.dtSeeDate.Value.Date);
                    MessageBox.Show("未找到当前科室有当前医生的排班！");
                }
                else
                {
                        ...
                        if (schema.Templet.Begin.Date == currentTime.Date)
                        {
                            ...
                            if (schema.Templet.Noon.ID.Equals(CommonController.Instance.GetNoonID(currentTime)) == false)
                            {
                                if (MessageBox.Show(this, "您选择的排班午别是：" + CommonController.Instance.GetNoonName(schema.Templet.Noon.ID) + "，是否继续？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.No)
                                {
                                    ...
                                    this.dtSeeDate.Tag = null;
                                    return;
                                    //  throw new Exception("没有该时段的号源！");97046BB8-4C04-499c-91D0-AC5FAFFB98F5//更改了因为午别提示选择否抛出异常
                                }
                            }
                        }
            ...
            else
            {
                ...
                if (level.IsExpert || level.IsSpecial)
                {
                    ...
                    if (doctID == null || doctID == "")
                    {
                        ...
                        if (string.IsNullOrEmpty(this.cmbDept.Tag.ToString()))
                        {
                            this.ucChooseDate.QueryDeptBooking(seeDate, "ALL", regType, level);
                        }
                        else
                        {
                            this.ucChooseDate.QueryDeptBooking(seeDate, this.cmbDept.Tag.ToString(), regType, level);
                        }
                    ...
                    else //{DF90A779-384E-4334-81FD-D1F5D5BCF47C}
                    {
        ...
        private void Clear()
        {
            ...
            try
            {
                ...
                this.tbCoupon.Text = "0.0";
                this.tbRealCost.Text = "0.0";
                this.tbEtcCost.Text = "0.0";
                this.tbTotCost.Text = "0.0";
                //  //{6AE6BDBB-FB7C-4c50-BC04-6667F3FB81E4}添加人员备注信息
                this.ntxtMemo.Text = "";
                    
                //{5D855B3C-5A4F-42c9-931D-333D0A01D809}
                this.cmbClass1Desease.Items.Clear();
                this.cmbClass2Desease.Items.Clear();

                //{36D75F21-3F54-4dba-840E-0195A0717771}
                if (this.chkPackage.Visible)
                {
                    this.IsDocterChargeFee = false;
        ...
        private void setDeseaseInfo(string deptCode)
        { 
            if(this.Class1Desease == null || string.IsNullOrEmpty(deptCode))
            {
                return;
            }
            ...
            {
                if (obj.Memo.Contains(deptCode) || obj.Memo.Contains("ALL"))
                {
                    deptDesease.Add(obj);
                }
            }
            ...
            //{508278C5-1966-40e1-9A05-A28976447CAA}
            if (this.cmbClass1Desease.Items != null && this.cmbClass1Desease.Items.Count > 0 && this.IsDeseaseDefault)
            {
                this.cmbClass1Desease.SelectedIndex = 0;
                this.cmbClass1Desease_SelectedIndexChanged(new object(), new EventArgs());
            }
        }
        ...
        private void setRegInfo(FS.HISFC.Models.Registration.Register regInfo, FS.HISFC.Models.Account.AccountCard accountCard)
        {
            try
            {
                ...
                //{6AE6BDBB-FB7C-4c50-BC04-6667F3FB81E4}添加人员备注信息
                //if (accountCard != null)
                //    this.ntxtMemo.Text = accountCard.Patient.Memo;
                //{58B76445-C6F0-4492-921E-6407AAE9901A}增加备注信息更改
                ntxtMemo.Text = regInfo.Memo;

                //{74958B4A-AD55-4775-BE30-E030DDC47A64}获取客户标签
                ucCustomerTag1.PatientInfo = regInfo;
                //if (regInfo.VipFlag)
                //{
                  
                //    imgVip.Image = imageList1.Images[1]; ;
                //}
               
                //FS.HISFC.BizLogic.MedicalPackage.Fee.Package pckMgr = new FS.HISFC.BizLogic.MedicalPackage.Fee.Package();
                // model = pckMgr.QueryPakcage(regInfo.PID.CardNO);
                // if (model != null && model.PackageNames != null && model.PackageNames.Length > 0)
                // {
                //     IsPackage = true;
                //     imgPackage.Image = imageList1.Images[3];
                // }
        ...
        private void setAge(DateTime birthday)
        {
            ...
            //{36BC9E15-C333-4028-AC5D-C5EE24A49FCF}
            //this.regMgr.GetAge(birthday, current, ref year, ref month, ref day);
            this.regMgr.GetAge(birthday, current, ref year, ref month, ref day, ref hour);
            if (year > 1)
            {
            ...
            try
            {
                ...
                if (this.helpPayMode.ArrayObject != null)
                {
                    ...
                    foreach (FS.HISFC.Models.Base.Const paymode in this.helpPayMode.ArrayObject)
                    {
                        ...
                        //账户支付
                        if (paymode.ID == "DC" || paymode.ID == "YS" || paymode.ID == "CO")
                        {
                        ...
                        {
                            this.fpPayMode_Sheet1.Cells[this.fpPayMode_Sheet1.RowCount - 1, (int)PayModeCols.TotCost].Locked = true;
                            //{B4E84BBE-B9C8-4710-A65B-D18C7C780116}
                            this.fpPayMode_Sheet1.Cells[this.fpPayMode_Sheet1.RowCount - 1, (int)PayModeCols.Memo].Locked = false;
                        }
                    }
                }
        ...
        public int InitInvoiceInfo()
        {
            ...
            if (iRes == -1)
            {
                MessageBox.Show("获取挂号发票出错!\r\n" + strErrText, "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return -1;
            }
        ...
        private decimal setCostInfoAfterEdit()
        {
            ...
            //{333D2AD8-DC4A-4c30-A14E-D6815AC858F9}
            JugdeCost = (decimal)this.hsPayCost["REAL"] - (decimal)this.hsPayCost["ACTU"] - (decimal)this.hsPayCost["GIFT"] - (decimal)this.hsPayCost["COU"];
        ...
        private int SetDeptZone(string deptID, DateTime bookingDate, FS.HISFC.Models.Registration.RegLevel level)
        {
            ...
            //{DF90A779-384E-4334-81FD-D1F5D5BCF47C}
            //如果选择了挂号级别则按照挂号级别查找排班，如果没有则查所有的排班
            if (this.cmbRegLevel.Tag == null || string.IsNullOrEmpty(this.cmbRegLevel.Tag.ToString()))
            {
            ...
            if (schema == null)//没有符合条件的
            {
                ...
                this.cmbDept.Tag = "";
                MessageBox.Show("未找到当前科室排班！");
            }
            else
            {
                //如果当前时间，选择了排班,否则会造成重复提示
            ...
            foreach (Row row in this.fpPayMode_Sheet1.Rows)
            {
                    ...
                    Double.Parse(this.fpPayMode_Sheet1.Cells[row.Index, (int)PayModeCols.TotCost].Value.ToString()) > 0)
                {
                    ...
                    if (cst.ID != "YS" && cst.ID != "DC")
                    {
                        ...
                        if (cst.ID == "YE")//{B7E5CA30-EC53-4fea-BB60-55B8D8DE3CAB} 判断应收其他是否填写了备注信息
                        {
        ...
        /// </summary>
        /// <param name="regObj"></param>
        private void BraceletPrint(FS.HISFC.Models.Registration.Register regObj)
        {
            FS.HISFC.BizProcess.Interface.Registration.IBraceletPrint braceletPrint = null;
            braceletPrint = FS.FrameWork.WinForms.Classes.UtilInterface.CreateObject(this.GetType(), typeof(FS.HISFC.BizProcess.Interface.Registration.IBraceletPrint)) as FS.HISFC.BizProcess.Interface.Registration.IBraceletPrint;
            if (braceletPrint == null)
            {
                MessageBox.Show(FS.FrameWork.WinForms.Classes.UtilInterface.Err, "警告", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            regObj.InputOper.ID = regMgr.Operator.ID;
            braceletPrint.SetPrintValue(regObj);
            braceletPrint.Print();
        }
        /// <summary>
        /// 套餐折扣
        ...
        private void Reprint()
        {
            ...
            if (IsPrintBracelet)// {AD405AA0-3101-46c0-A0B6-846DC3AAB10A}
            {
                if (MessageBox.Show("是否需要打印腕带?", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                {
                    this.BraceletPrint(obj);
                }
            }
        ...
        {
            FS.HISFC.Models.Registration.Register obj = new FS.HISFC.Models.Registration.Register();
            int index = fpRegList.ActiveRowIndex;
            obj = this.fpRegList.Rows.Get(index).Tag as FS.HISFC.Models.Registration.Register;
            if (obj == null)// {6BF1F99D-7307-4d05-B747-274D24174895}
            {
            ...
            if (MessageBox.Show("是否需要打印发票?", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
            {
                this.Print(obj, this.regMgr);
            }
        ...
        private void PackageQuery()
        {
            ...
            //{81CF5800-BC28-4974-9E44-CA05DC37AC5E}
        ...
        private int valid()
        {
            ...
            if ((level.IsExpert || level.IsFaculty || level.IsSpecial ) && this.dtSeeDate.Tag == null)
            {
            ...
            //{5D855B3C-5A4F-42c9-931D-333D0A01D809}
            if (IsDeseaseMustChoose)
            {
                if (string.IsNullOrEmpty(this.cmbClass1Desease.Tag.ToString()))
                {
                    MessageBox.Show("一级病种不能为空！");
                    return -1;
                }

                if (string.IsNullOrEmpty(this.cmbClass2Desease.Tag.ToString()))
                {
                    MessageBox.Show("二级病种不能为空！");
                    return -1;
                }
            }
        ...
        private int GetValue()
        {
            ...
            FS.HISFC.Models.RADT.PatientInfo patientInfoByCardNO = account.GetPatientInfoByCardNO(this.tbCardNO.Text.Trim());
            this.tbName.Text = (this.regObj.Name = patientInfoByCardNO.Name);
            this.cmbGender.Tag =this.regObj.Sex.ID = patientInfoByCardNO.Sex.ID;
            this.dtpBirthday.Value = (this.regObj.Birthday = patientInfoByCardNO.Birthday);
            ...
            //{5D855B3C-5A4F-42c9-931D-333D0A01D809}
            this.regObj.Class1Desease = this.cmbClass1Desease.Tag.ToString();
            this.regObj.Class2Desease = this.cmbClass2Desease.Tag.ToString();
            ...
            //{1AD141F7-CF31-4975-BAC4-304AF45E60C3}
            //从原来的初诊历史为前面一年有就诊即为初诊，改为前面99年有就诊过即为初诊，当然不会到99年，所以相当于来过就诊之后就一定是复诊了
            int isHospitalFirst = this.regMgr.IsFirstRegister("1", this.regObj.PID.CardNO, this.regObj.DoctorInfo.Templet.Dept.ID, this.regObj.DoctorInfo.Templet.Doct.ID, this.regMgr.GetDateTimeFromSysDateTime().AddYears(-99).Date);
            int isRootDeptFirst = this.regMgr.IsFirstRegister("2", this.regObj.PID.CardNO, this.regObj.DoctorInfo.Templet.Dept.ID, this.regObj.DoctorInfo.Templet.Doct.ID, this.regMgr.GetDateTimeFromSysDateTime().AddYears(-99).Date);
            int isDeptFirst = this.regMgr.IsFirstRegister("3", this.regObj.PID.CardNO, this.regObj.DoctorInfo.Templet.Dept.ID, this.regObj.DoctorInfo.Templet.Doct.ID, this.regMgr.GetDateTimeFromSysDateTime().AddYears(-99).Date);
            int isDoctFirst = this.regMgr.IsFirstRegister("4", this.regObj.PID.CardNO, this.regObj.DoctorInfo.Templet.Dept.ID, this.regObj.DoctorInfo.Templet.Doct.ID, this.regMgr.GetDateTimeFromSysDateTime().AddYears(-99).Date);

            this.regObj.HospitalFirstVisit = isHospitalFirst > 0 ? "0" : "1";
            this.regObj.RootDeptFirstVisit = isRootDeptFirst > 0 ? "0" : "1";
            this.regObj.IsFirst = isDeptFirst > 0 ? false:true;
            this.regObj.DoctFirstVist = isDoctFirst > 0 ? "0" : "1";
            ...
            else
            {
               ...
               // regObj.Pact.Memo = ntxtMemo.Text;
            }
            ...
            //{58B76445-C6F0-4492-921E-6407AAE9901A}增加备注信息更改
            this.regObj.Memo = ntxtMemo.Text;
            regObj.Birthday = this.dtpBirthday.Value;//{94F7BB9C-A3AC-43c8-A5B0-272575A6DF75}
            this.regObj.IsFee = true;
            //{AE399953-4F87-4199-8060-EFDC16AFAAF3} 综合门诊添加实际医生
            this.regObj.RealDoctorID = this.cmbRealDoctor.Tag.ToString() ;
            if (this.cmbRealDoctor.SelectedItem != null)
                this.regObj.RealDoctorName = this.cmbRealDoctor.SelectedItem.Name;
            return 0;
        }
        ...
        private int Save()
        {
            ...
            //{D8F6425B-1CFD-4b3f-921E-03B1ECA0F95E}
            if (regMgr.QueryRegByNoonDoctor(regObj.PID.CardNO, regObj.DoctorInfo.Templet.Noon.ID, regObj.DoctorInfo.Templet.Doct.ID, regObj.DoctorInfo.Templet.Dept.ID, regObj.DoctorInfo.SeeDate))
            {
                DialogResult dr = MessageBox.Show("该患者在同一午别同医生已经挂过号，是否继续？", "是否继续", MessageBoxButtons.OKCancel);
            ...
            {
                //{C7D96888-A677-45d8-8BB7-CCDBC1E5B5CD}
                if (this.IFeeDiagReg == 0 && this.IsDocterChargeFee && !this.IsPrintIfZero)
                {
                    //如果是不收费流程,且收费为0的时候不打印发票，则不进行发票打印
                }
                else
                {
                    if (this.GetRecipeType == 1)
            ...
            if (RegFeeList.Count > 0)
            {
                ...
                decimal giftCost = (decimal)this.hsPayCost["GIFT"] + (decimal)this.hsPayCost["COU"];
            ...
            //积分相关的操作
            decimal costCoupon = 0;     //计算支付方式中的积分消费
            decimal operateCoupon = 0;  //计算当前交易产生多少积分
            FS.FrameWork.Models.NeuObject cashCouponPayMode = this.conMgr.GetConstant("XJLZFFS", "1");
            ...
            foreach (FS.HISFC.Models.Registration.RegisterPayMode pay in payMode)
            {
                ...
                if (pay.Mode_Code == "CO")
                {
                    costCoupon += pay.Tot_cost;
                }

                //统计能产生多少积分
                if (cashCouponPayMode.Name.Contains(pay.Mode_Code.ToString()))
                {
                    operateCoupon += pay.Tot_cost;
                }
            ...
            if (this.IsCouponModuleInUse)
            {
                ...
                if (costCoupon != 0)
                {
                    reqRtn = FS.HISFC.BizProcess.Integrate.WSHelper.CostCoupon(this.regObj.PID.CardNO, this.regObj.Name, this.regObj.PID.CardNO, this.regObj.Name, "GHSF", strInvioceNO, costCoupon, 0.0m, out resultCode, out errorMsg);
                    if (reqRtn < 0)
                    {
                        FS.FrameWork.Management.PublicTrans.RollBack();
                ...
                if (operateCoupon != 0)
                {
                    reqRtn = FS.HISFC.BizProcess.Integrate.WSHelper.OperateCoupon(regObj.PID.CardNO, regObj.Name, "GHSF", regObj.InvoiceNO, operateCoupon, 0.0m, out resultCode, out errorMsg);
                    if (reqRtn < 0)
                    {
                        MessageBox.Show("处理会员积分出错:" + errorMsg);
                        FS.FrameWork.Management.PublicTrans.RollBack();
                        ...
                        if (costCoupon != 0)
                        {
                            reqRtn = FS.HISFC.BizProcess.Integrate.WSHelper.CostCoupon(this.regObj.PID.CardNO, this.regObj.Name, this.regObj.PID.CardNO, this.regObj.Name, "GHTF", strInvioceNO, -costCoupon, 0.0m, out resultCode, out errorMsg);
            ...
            if (this.RegFeeList.Count > 0)
            {
                ...
                //挂号数据统计节点是否启用-CPP002
                if (this.IsRegisterStatisticsPointInUse)
                {
                    FS.HISFC.BizProcess.Interface.StatisticsPoint.IStatisticsPoint iStatistics = new FS.HISFC.BizProcess.Integrate.StatisticsPoint.RegistStatisticsPoint();
            ...
            if (IsPrintBracelet)// {AD405AA0-3101-46c0-A0B6-846DC3AAB10A}
            {
                ...
                //if (MessageBox.Show("是否需要打印腕带?", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                //{
                    this.BraceletPrint(this.regObj);
                //}
            }
        ...
        private string payMode(string invoiceNo)
        {
            string str = "";
            string sql = @"select wm_concat(y.name || ':'|| e.tot_cost) from exp_register_paymode e,com_dictionary y  
                        where e.invoice_no = '{0}'
                        and e.trans_type = '1'
                        and e.mode_code = y.code
                        and y.type = 'PAYMODES'";
            sql = string.Format(sql, invoiceNo);
            str = this.regMgr.ExecSqlReturnOne(sql);
            return str;
        }
        ...
        private void CouponPayShow()
        {
            ...
            try
            {
                ...
                //获取当前账户支付的金额
                ArrayList tmp = new ArrayList();
                tmp.AddRange(this.couponPayList);
                ...
                couponCost.OriginalCardNO = this.regObj.PID.CardNO;
                couponCost.DeliverableCost = CashCost + Decimal.Parse(this.tbCoupon.Text);
        ...
        private int couponCost_SetPayModeRes(Hashtable hsTable, decimal totCost)
        {
            try
            {
                ...
                {
                    this.couponPayList = hsTable["CO"] as ArrayList;
                }

                if (this.couponPayList == null)
                {
                    foreach (Row row in this.fpPayMode_Sheet1.Rows)
                    {
                        FS.HISFC.Models.Base.Const pay = row.Tag as FS.HISFC.Models.Base.Const;
                    ...
                    this.couponPayList = new ArrayList();
                ...
                foreach (Row row in this.fpPayMode_Sheet1.Rows)
                {
                    ...
                    if (pay.ID == "CO")
                    {
                        foreach (FS.HISFC.Models.Registration.RegisterPayMode payMode in this.couponPayList)
                        {
                            couponPayCount += payMode.Tot_cost;
                        }

                        this.fpPayMode_Sheet1.Cells[row.Index, (int)PayModeCols.TotCost].Value = couponPayCount;
                        totCost -= couponPayCount;
                        this.fpPayMode_Sheet1.Cells[CashRow, (int)PayModeCols.TotCost].Value = totCost;
                    }
                }

                this.tbCoupon.Text = (couponPayCount).ToString("F2");
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        /// <summary>
        /// 标记点击事件{74958B4A-AD55-4775-BE30-E030DDC47A64}
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        ...
        //private void imgVip_MouseHover(object sender, EventArgs e)
        //{
        ...
        //        //fmtag = new FrmTag();
        //        //fmtag.PackageNames = model.PackageNames;

        //        //fmtag.StartPosition = FormStartPosition.Manual;
        //        //fmtag.Location = new Point(PointToScreen(this.lblTag.Location).X, PointToScreen(this.lblTag.Location).Y + this.lblTag.Height * 3);
        //        //fmtag.FormBorderStyle = FormBorderStyle.None;
        //        //// fmtag.ShowInTaskbar = true;
        //        //fmtag.Show();
