...
namespace FS.HISFC.Components.Terminal.Confirm
{
	...
	///  />
	/// </summary>
	public partial class ucConfirmRecord : FS.FrameWork.WinForms.Controls.ucBaseControl
	{
		...
		/// 业务明细
		/// </summary>
		public FS.HISFC.Models.Terminal.TerminalConfirmDetail confirmDetail = new TerminalConfirmDetail();
        FS.HISFC.BizProcess.Integrate.Terminal.Booking confirmMgr = new FS.HISFC.BizProcess.Integrate.Terminal.Booking();
		...
		/// </summary>
		public FS.HISFC.Models.Terminal.TerminalApply terminalApply = new TerminalApply();
        ...
        {
            //toolBarService.AddToolButton("查询", "查询", FS.FrameWork.WinForms.Classes.EnumImageList.A查找, true, false, null);
            toolBarService.AddToolButton("取消", "取消", (int)FS.FrameWork.WinForms.Classes.EnumImageList.Q取消, true, false, null);  
		...
		private void DisplayApply(ArrayList alApply)
		{
			...
			FS.HISFC.BizProcess.Integrate.Terminal.Confirm confirmIntegrate = new FS.HISFC.BizProcess.Integrate.Terminal.Confirm();
			// 员工实体
			FS.HISFC.Models.Base.Employee employee = new Employee();
			// 科室实体
			FS.HISFC.Models.Base.Department department = new FS.HISFC.Models.Base.Department();

			this.neuSpreadItem_SheetItem.RowCount = 0;
			
			foreach (FS.HISFC.Models.Terminal.TerminalApply tempTerminalApply in alApply)
			{
				// 增加一行
				...
				confirmIntegrate.GetEmployee(ref employee, tempTerminalApply.Item.Order.ReciptDoctor.ID);
            ...
            if (alApply.Count > 0)
            {
                FS.HISFC.Models.Terminal.TerminalApply tempTerminalApply = (FS.HISFC.Models.Terminal.TerminalApply)alApply[0];
		...
		private void DisplayDetail(ArrayList terminalConfirmDetailList)
		{
			...
			foreach (FS.HISFC.Models.Terminal.TerminalConfirmDetail tempConfirmDetail in terminalConfirmDetailList)
			{
				// 增加一行新记录
		...
		protected void TerminalQuery()
		{
			...
			// 根据病历号获取申请单明细
			//
			FS.FrameWork.WinForms.Classes.Function.ShowWaitForm("正在获取项目信息...");

            Application.DoEvents();
			// 获取申请单明细

			result = confirmIntegrate.QueryApplyListByCardNO(this.neuTextBoxCardNO.Text, ref alApply);
			if (FS.HISFC.BizProcess.Integrate.Terminal.Result.Failure == result)
			{
		...
		private void neuSpreadItem_CellDoubleClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
		{
			...
			this.terminalApply = (FS.HISFC.Models.Terminal.TerminalApply)this.neuSpreadItem_SheetItem.Rows[this.neuSpreadItem_SheetItem.ActiveRowIndex].Tag;
			//
			// 根据申请单号获取业务明细
			//
        ...
        private void CancelConfirm()
        {
            ...
            FS.HISFC.BizLogic.Terminal.TerminalConfirm terminalConfim = new FS.HISFC.BizLogic.Terminal.TerminalConfirm();
            FS.HISFC.BizProcess.Integrate.PhysicalExamination.ExamiManager examiMgr = new FS.HISFC.BizProcess.Integrate.PhysicalExamination.ExamiManager();
            FS.HISFC.BizProcess.Integrate.Fee feeMgr = new FS.HISFC.BizProcess.Integrate.Fee();
            FS.HISFC.BizProcess.Integrate.RADT serviceInpatient = new FS.HISFC.BizProcess.Integrate.RADT();

            //FS.FrameWork.Management.Transaction t = new FS.FrameWork.Management.Transaction(terminalConfim.Connection);            
            //t.BeginTransaction();
            ...
            FS.HISFC.Models.Terminal.TerminalConfirmDetail detail = (FS.HISFC.Models.Terminal.TerminalConfirmDetail)this.neuSpreadItemDetail_SheetItemDetail.Rows[this.neuSpreadItemDetail_SheetItemDetail.ActiveRowIndex].Tag;
            FS.HISFC.Models.Terminal.TerminalApply apply = terminalConfim.GetItemListBySequence(detail.Apply.Order.ID,detail.Apply.ID);

            FS.HISFC.Models.Base.Employee employee = (FS.HISFC.Models.Base.Employee)terminalConfim.Operator;
            ...
            int BackQty = FS.FrameWork.Function.NConvert.ToInt32(detail.Apply.Item.ConfirmedQty);
            ...
            {
                //体检中心的可退数量 
                int NoBackQty = FS.FrameWork.Function.NConvert.ToInt32(apply.Item.Item.Qty - apply.Item.ConfirmedQty + BackQty);
            ...
            if (apply.PatientType == "2" && detail.Apply.SpecalFlag == "2") //回退住院费用
            {
                ...
                if (terminalApply.Item.Order.ReciptDoctor.ID == null || terminalApply.Item.Order.ReciptDoctor.ID == "")
                {
            ...
            {
                FS.HISFC.Models.Terminal.TerminalApply obj = (FS.HISFC.Models.Terminal.TerminalApply)this.neuSpreadItem_SheetItem.Rows[i].Tag;
                if (obj.Order.ID == detail.Apply.Order.ID)
                {
                    int freeQty = FS.FrameWork.Function.NConvert.ToInt32(detail.Apply.Item.ConfirmedQty) + FS.FrameWork.Function.NConvert.ToInt32(neuSpreadItem_SheetItem.Cells[i, (int)ApplyField.FreeQty].Text);
