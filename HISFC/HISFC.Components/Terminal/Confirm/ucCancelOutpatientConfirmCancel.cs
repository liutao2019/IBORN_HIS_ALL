...
namespace FS.HISFC.Components.Terminal.Confirm
{
    ...
    ///  />
    /// </summary>
    public partial class ucCancelOutpatientConfirmCancel : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        public ucCancelOutpatientConfirmCancel()
        {
            InitializeComponent();
        }
        ...
        private void QueryConfirmInfo(string cardNo)
        {
            ...
            result = confirmIntegrate.QueryConfirmInfoByClinicNoCancel(cardNo, deptID, ref apply);
        ...
        /// </summary>
            //FS.HISFC.BizLogic.Terminal.TerminalConfirm confirmIntegrate = new TerminalConfirm();
            // 业务层
            ...
            foreach (FarPoint.Win.Spread.Row r in this.neuSpread2_Sheet1.Rows)
            {
                ...
                result = confirmIntegrate.UpdateApplyConfirmQtyCancel(apply.User02, cancelQty);//confirm_num=confirm_num-cancelQty
                ...
                //更新fin_opb_feedetail中对应医嘱的可退数量UpdateApplyConfirmQtyCancelUpdateNobackNumCancel
                result = confirmIntegrate.UpdateNobackNumCancel(apply.Order.ID, apply.Item.ID, cancelQty);//noback_num=noback_num+cancelQty;confirm_num=confirm_num-cancelQty
                if (result != Result.Success)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                    MessageBox.Show("更新可退数量失败。召回数量大于收费数量" + confirmIntegrate.Err);

                    return;
                }
                //直接更新met_tec_ta_detail中的已确认数量，将之前的已确认数量存入extend_field3
                result = confirmIntegrate.UpdateConfirmedQtyCancel(apply.ID, apply.Item.ConfirmedQty, oldConfirmedQty.ToString());
                if (result != Result.Success)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                ...
                //部分确认暂不考虑-即不允许部分确认
                //直接更新met_tec_ta_detail中的已确认数量，将之前的已确认数量存入extend_field3
                result = confirmIntegrate.UpdateConfirmedFlagCancel(apply.ID, FS.FrameWork.Management.Connection.Operator.ID, "");
                if (result != Result.Success)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
