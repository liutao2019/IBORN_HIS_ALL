...
namespace FS.HISFC.Components.Terminal.Booking
{
	...
	///  />
	/// </summary>
	public partial class ucBookingTemplet : FS.FrameWork.WinForms.Controls.ucBaseControl
	{
		...
		/// 午别列表
		/// </summary>
        //FS.FrameWork.WinForms.Controls.PopUpListBox popUpListBox = new PopUpListBox();
		...
		{
			FS.FrameWork.WinForms.Classes.MarkCellType.NumCellType numbType = new FS.FrameWork.WinForms.Classes.MarkCellType.NumCellType();
        ...
        //{
        //    foreach (FS.FrameWork.Models.NeuObject noon in this.popUpListBox.Items)
        //    {
        //        if (noon.ID == noonID)
        //        {
        //            return noon.Name;
        ...
        //                return -1;
        //            }
        //            if (FS.FrameWork.Function.NConvert.ToInt32(dataRow["BookLmt"]) == 0 && FS.FrameWork.Function.NConvert.ToInt32(dataRow["SpecialBookLmt"]) == 0)
        //            {
        //                MessageBox.Show("预约限额和特诊限额不允许同时为0", "提示");
        //                return -1;
        //            } 
        //            if (FS.FrameWork.Public.String.ValidMaxLengh(dataRow["Remark"].ToString(), 100) == false)
        //            {
			...
			if (dataTable != null)
			{
				try
				{
					foreach (DataRow dataRow in dataTable.Rows)
					{
						...
						medTechItemTemp.BookLmt = FS.FrameWork.Function.NConvert.ToDecimal(dataRow["BookLmt"]);
						medTechItemTemp.SpecialBookLmt = FS.FrameWork.Function.NConvert.ToDecimal(dataRow["SpecialBookLmt"]);
		...
		private int SaveAdd(ref string error)
		{
			try
			{
				foreach (FS.HISFC.Models.Terminal.MedTechItemTemp templet in tempList)
				{
		...
		private int SaveModify(ArrayList modifyTempList, DataTable dataTableModify, ref string error)
		{
			try
			{
				...
				foreach (FS.HISFC.Models.Terminal.MedTechItemTemp templet in modifyTempList)
				{
					if (this.bookingIntegrate.InsertMedTechItemTemp(templet) == -1)
					{
						error = bookingIntegrate.Err;
		...
		private int ProcessNoon()
		{
			...
			// 午别控件
			FS.FrameWork.WinForms.Controls.PopUpListBox listBox = this.neuSpread1.getCurrentList(this.neuSpread1_Sheet1, 5);
			//选中的午别
		...
		private int GetItem(ref FS.HISFC.Models.Terminal.MedTechItemTemp itemTemp, int row)
		{
			bool isValid = false;
			...
			itemTemp.BookLmt = FS.FrameWork.Function.NConvert.ToDecimal(this.neuSpread1_Sheet1.Cells[row, (int)cols.BookLmt].Text);
			itemTemp.SpecialBookLmt = FS.FrameWork.Function.NConvert.ToDecimal(this.neuSpread1_Sheet1.Cells[row, (int)cols.SpecialBookLmt].Text);
			...
			if (FS.FrameWork.Public.String.ValidMaxLengh(itemTemp.MedTechItem.Item.Notice, 100) == false)
			{
		...
		public void Query(string deptCode, string currentWeek)
		{
			...
			try
			{
				foreach (FS.HISFC.Models.Terminal.MedTechItemTemp medTechItemTemplet in tempList)
				{
					// 有效性
					string strValid = "无效";
					// 设置有效性
		...
		///  增加项目
		/// </summary>
        public void Add(FS.HISFC.Models.Terminal.MedTechItem medTechItem)
		{
			try
			{
				// 增加一行
		...
		public void Delete()
		{
            ...
            //FS.FrameWork.Management.Transaction transaction = new Transaction();			
            //transaction.BeginTransaction(FS.FrameWork.Management.Connection.Instance);
		...
		public int Save()
		{
			...
			for (int i = 0;i < this.neuSpread1_Sheet1.RowCount;i++)
			{
				FS.HISFC.Models.Terminal.MedTechItemTemp tempItem = new MedTechItemTemp();
