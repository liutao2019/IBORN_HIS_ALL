...
namespace FS.HISFC.Components.Terminal.Booking
{
	...
	///  />
	/// </summary>
	public partial class ucMedTechBookingArrange : FS.FrameWork.WinForms.Controls.ucBaseControl
	{
        ...
        FS.HISFC.BizLogic.Terminal.Terminal applyMgr = new FS.HISFC.BizLogic.Terminal.Terminal();
        /// <summary>
        /// 
        /// </summary>
        private FS.FrameWork.Public.ObjectHelper noonListHelper = new FS.FrameWork.Public.ObjectHelper();
        private FS.FrameWork.Public.ObjectHelper machineObjHelper = new FS.FrameWork.Public.ObjectHelper();
		/// <summary>
		/// 显示星期
		/// </summary>
		...
		/// 操作环境
		/// </summary>
		public FS.HISFC.Models.Base.OperEnvironment operEnvironment = new OperEnvironment();
		
		/// <summary>
		/// 午别列表
		/// </summary>
        //private FS.FrameWork.WinForms.Controls.PopUpListBox noonListBox = new PopUpListBox();
        ...
        //{
        //    foreach (FS.FrameWork.Models.NeuObject noon in this.noonListBox.Items)
        //    {
        //        if (noon.ID == noonID)
        //        {
        //            return noon.Name;
		...
		{
			FS.FrameWork.WinForms.Classes.MarkCellType.NumCellType numnberCellType = new FS.FrameWork.WinForms.Classes.MarkCellType.NumCellType();
			FS.FrameWork.WinForms.Classes.MarkCellType.NumCellType readOnlyNumnberCellType = new FS.FrameWork.WinForms.Classes.MarkCellType.NumCellType();
		...
		private int Valid(DataTable tempDataTable)
		{
			if (tempDataTable != null)
			{
				foreach (DataRow row in tempDataTable.Rows)
				{
                    ...
                    if (FS.FrameWork.Function.NConvert.ToInt32(row["BookLmt"]) == 0 && FS.FrameWork.Function.NConvert.ToInt32(row["SpecialBookLmt"]) == 0)
                    {
			...
			if (tempDataTable != null)
			{
				try
				{
					foreach (DataRow row in tempDataTable.Rows)
					{
						...
						tempMedTechItemTemp.BookLmt = FS.FrameWork.Function.NConvert.ToDecimal(row["BookLmt"]);
						tempMedTechItemTemp.SpecialBookLmt = FS.FrameWork.Function.NConvert.ToDecimal(row["SpecialBookLmt"]);
						tempMedTechItemTemp.MedTechItem.Item.ChildPrice = FS.FrameWork.Function.NConvert.ToDecimal(row["BookNum"]);
						tempMedTechItemTemp.MedTechItem.Item.SpecialPrice = FS.FrameWork.Function.NConvert.ToDecimal(row["SpecialBookNum"]);
		...
		private int SaveAdd(ArrayList addArrayList, ref string errorMessage)
		{
			try
			{
				foreach (FS.HISFC.Models.Terminal.MedTechItemTemp templet in addArrayList)
				{
		...
		private int SaveModify(ArrayList modifyArrayList, DataTable modifyDataTable, ref string error)
		{
			try
			{
				...
				foreach (FS.HISFC.Models.Terminal.MedTechItemTemp templet in modifyArrayList)
				{
					if (this.bookingIntegrate.InsertMedTechItemArrange(templet) == -1)
					{
						error = this.bookingIntegrate.Err;
		...
		private int ProcessNoon()
		{
			...
			FS.FrameWork.WinForms.Controls.PopUpListBox listBox = this.fpSpread1.getCurrentList(this.fpSpread1_Sheet1, 2);
			//获取选中的信息
			FS.FrameWork.Models.NeuObject noon = null;
		...
		/// <returns>1－成功；－1－失败</returns>
		private int GetTempItem(ref FS.HISFC.Models.Terminal.MedTechItemTemp tempMedTechItemTemp, int row)
        {
            //{5A111831-190D-4187-8076-83E220BC97B2}
			...
			tempMedTechItemTemp.BookLmt = FS.FrameWork.Function.NConvert.ToDecimal(this.fpSpread1_Sheet1.Cells[row, (int)cols.BookLmt].Text);
			tempMedTechItemTemp.SpecialBookLmt = FS.FrameWork.Function.NConvert.ToDecimal(this.fpSpread1_Sheet1.Cells[row, (int)cols.SpecialBookLmt].Text);
			tempMedTechItemTemp.MedTechItem.Item.ChildPrice = FS.FrameWork.Function.NConvert.ToDecimal(this.fpSpread1_Sheet1.Cells[row, (int)cols.BookNum].Text);
			tempMedTechItemTemp.MedTechItem.Item.SpecialPrice = FS.FrameWork.Function.NConvert.ToDecimal(this.fpSpread1_Sheet1.Cells[row, (int)cols.SpecialBookNum].Text);
			tempMedTechItemTemp.MedTechItem.Item.Oper.ID = this.operEnvironment.ID;
        ...
        private void InitCarrier()
        {
            ...
            deptEquipmentList = bookingIntegrate.QueryMedTechEquipment(((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept.ID);
            ...
            foreach (FS.HISFC.Models.Terminal.TerminalCarrier car in deptEquipmentList)
            {
        ...
        private int ProcessMachine()
        {
            ...
            FS.FrameWork.WinForms.Controls.PopUpListBox listBox1 = this.fpSpread1.getCurrentList(this.fpSpread1_Sheet1, (int)cols.Machine);
            //获取选中的信息

            FS.FrameWork.Models.NeuObject machine = null;
		...
		public void Query(string deptCode, string currentDay)
		{
			...
			try
			{
				foreach (FS.HISFC.Models.Terminal.MedTechItemTemp tempMedTechItemTemp in this.arrayList)
				{
        ...
        public void Add(FS.HISFC.Models.Terminal.MedTechItem tempMedTechItem)
		{
			try
			{
                //{5A111831-190D-4187-8076-83E220BC97B2}
        ...
        ///  增加设备
        /// </summary>
        public void Add ( FS.HISFC.Models.Terminal.TerminalCarrier terminalCarrier )
        {
            try
            {
                //增加一行
		...
		public void AddTemp(FS.HISFC.Models.Terminal.MedTechItemTemp tempMedTechItemTemp)
		{
			try
			{
				this.fpSpread1_Sheet1.Rows.Add(this.fpSpread1_Sheet1.RowCount, 1);
        ...
        public void Delete()
		{
			...
			// 实体
			FS.HISFC.Models.Terminal.MedTechItemTemp tempItemTemp = new MedTechItemTemp();
            ...
            applyMgr.SetTrans(FS.FrameWork.Management.PublicTrans.Trans);
		...
		/// <returns></returns>
		int fpSpread1_SetItem(FS.FrameWork.Models.NeuObject obj)
		{
			this.ProcessNoon();
            this.ProcessMachine();
			return 0;
