...
namespace FS.HISFC.Components.Terminal.Booking
{
	...
	///  />
	/// </summary>
	public partial class ucBookingArrange : FS.FrameWork.WinForms.Controls.ucBaseControl
	{
		public ucBookingArrange()
		{
			...
			this.operEnvironment.ID = FS.FrameWork.Management.Connection.Operator.ID;
			this.operEnvironment.Name = FS.FrameWork.Management.Connection.Operator.Name;

			this.operEnvironment.Dept = ((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept;
		}
		...
		/// 医技预约项目
		/// </summary>
        private FS.HISFC.Models.Terminal.MedTechItem medTechItem = new MedTechItem();
        /// <summary>
        /// 设备
        /// </summary>
        private FS.HISFC.Models.Terminal.TerminalCarrier terminalCarrier = new TerminalCarrier ( );
		/// <summary>
		/// 排班控件数组,方便操作
		/// </summary>
		...
		/// 工具栏服务
		/// </summary>
		protected FS.FrameWork.WinForms.Forms.ToolBarService toolbarService = new ToolBarService();
		...
		private void InitItem()
		{
            ...
            foreach (FS.HISFC.Models.Terminal.MedTechItem obj in itemList)
			{
                foreach (FS.HISFC.Models.Base.Const con in conList)
                {
        ...
        private void InitEquipment ( )
        {
            ...
            foreach ( FS.HISFC.Models.Terminal.TerminalCarrier obj in deptEquipmentList )
            {
                TreeNode node = new TreeNode ( );
		...
		{
			FS.FrameWork.Models.NeuObject tempDept = new NeuObject();
		...
		private void LoadTemplet()
		{
			...
			if (formSelectWeek.ShowDialog() == DialogResult.OK)
			{
                ...
                foreach (FS.HISFC.Models.Terminal.MedTechItemTemp tempMedTechItemTemp in templetList)
                {
		...
		private void Add()
		{
            ...
            if ( this.neuTreeView1.SelectedNode.Tag.GetType ( ).FullName.Equals ( "FS.HISFC.Models.Terminal.MedTechItem" ) )
            {
                this.medTechItem = ( FS.HISFC.Models.Terminal.MedTechItem ) this.neuTreeView1.SelectedNode.Tag;
            ...
            else if ( this.neuTreeView1.SelectedNode.Tag.GetType ( ).FullName.Equals ( "FS.HISFC.Models.Terminal.TerminalCarrier" ) )
            {
                this.terminalCarrier = ( FS.HISFC.Models.Terminal.TerminalCarrier ) this.neuTreeView1.SelectedNode.Tag;
		...
		protected override FS.FrameWork.WinForms.Forms.ToolBarService OnInit(object sender, object neuObject, object param)
		{
			...
			this.toolbarService.AddToolButton("上周", "移动到上周", (int)FS.FrameWork.WinForms.Classes.EnumImageList.S上一个, true, false, this.PriorWeekEvent);
			this.toolbarService.AddToolButton("下周", "移动到上周", (int)FS.FrameWork.WinForms.Classes.EnumImageList.X下一个, true, false, this.NextWeekEvent);
			this.toolbarService.AddToolButton("调模板", "调用其它模板", (int)FS.FrameWork.WinForms.Classes.EnumImageList.Z自动录入, true, false, this.LoadTempEvent);
			this.toolbarService.AddToolButton("删除", "删除当前记录", (int)FS.FrameWork.WinForms.Classes.EnumImageList.S删除, true, false, this.DeleteEvent);
            this.toolbarService.AddToolButton ( "项目" , "项目" , ( int ) FS.FrameWork.WinForms.Classes.EnumImageList.K科室 , true , false , this.DeptEvent );
            this.toolbarService.AddToolButton ( "设备" , "设备" , ( int ) FS.FrameWork.WinForms.Classes.EnumImageList.S设备 , true , false , this.EquipmentEvent );
			this.InitArray();
			this.InitComb();
			this.InitTab();
			this.InitItem();
		...
		private void neuButtonQuery_Click(object sender, EventArgs e)
		{
			...
			else
			{
				...
				{
                    FS.HISFC.Models.Terminal.MedTechItem tempItem  = (FS.HISFC.Models.Terminal.MedTechItem)node.Tag;

					// 查询码实体
					FS.HISFC.Models.Base.Spell spell = new Spell();
