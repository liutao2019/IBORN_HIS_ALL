...
using MedTechItemTemp=FS.HISFC.Models.Terminal.MedTechItemTemp;
...
namespace FS.HISFC.Components.Terminal.Booking
{
	...
	///  />
	/// </summary>
    public partial class ucMedTechBooking : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Forms.IInterfaceContainer
	{
		public ucMedTechBooking()
		{
			...
			this.operEnviroment.ID = FS.FrameWork.Management.Connection.Operator.ID;
			this.operEnviroment.Name = FS.FrameWork.Management.Connection.Operator.Name;
			this.operEnviroment.Dept = ((FS.HISFC.Models.Base.Employee) FS.FrameWork.Management.Connection.Operator).Dept;
		...
		/// 
		/// </summary>
		private FS.FrameWork.Public.ObjectHelper noonListHelper = new ObjectHelper();
        /// <summary>
        /// 当前几天
        /// </summary>
		...
		/// 拖动对象
		/// </summary>
		private FS.HISFC.Models.Terminal.MedTechBookApply dragMedTechBookApply = new MedTechBookApply();
		...
		/// 基础数据库类
		/// </summary>
		FS.FrameWork.Management.DataBaseManger databaseManagerFunction = new DataBaseManger();
        FS.HISFC.Components.Common.Controls.baseTreeView baseTreeView = new FS.HISFC.Components.Common.Controls.baseTreeView();
		...
		/// 操作环境
		/// </summary>
		private FS.HISFC.Models.Base.OperEnvironment operEnviroment = new OperEnvironment();
		
		/// <summary>
		/// 业务
		/// </summary>
		private FS.HISFC.BizProcess.Integrate.Terminal.Booking bookingIntegrate = new FS.HISFC.BizProcess.Integrate.Terminal.Booking ();
        ...
        {
            toolBarService.AddToolButton("预约", "预约", (int)FS.FrameWork.WinForms.Classes.EnumImageList.B保存, true, false, null);
            toolBarService.AddToolButton("取消", "取消", (int)FS.FrameWork.WinForms.Classes.EnumImageList.S删除, true, false, null); 
            ...
            FS.FrameWork.WinForms.Classes.Function.ShowWaitForm("正在查询请等待 ...");
        ...
        { 
            try
            {
                ...
                for (int i = 0; i < this.neuSpreadBooking_Sheet1.RowCount; i++)
                {
                    ...
                    {
                        FS.HISFC.Models.Terminal.MedTechBookApply obj = (FS.HISFC.Models.Terminal.MedTechBookApply)this.neuSpreadBooking_Sheet1.Rows[i].Tag;
                        list.Add(obj);
                    }
                }
                if (list.Count == 0)
                ...
                if (list.Count > 0)
                {
                    ...
                    {
                        foreach (FS.HISFC.Models.Terminal.MedTechBookApply obj in list)
                        {
                            this.bookingPrint = FS.FrameWork.WinForms.Classes.UtilInterface.CreateObject(this.GetType(), typeof(IBookingPrint)) as IBookingPrint;
                            if (this.bookingPrint == null)
                            {
        ...
        private int Arrange(int row)
        {
            ...
            try
            {
                ...
                // 医技预约实体
                FS.HISFC.Models.Terminal.MedTechBookApply tempObj = (FS.HISFC.Models.Terminal.MedTechBookApply)this.ucChooseList1.tvList.SelectedNode.Tag;
                // 预约信息
                ...
                FS.HISFC.Models.Terminal.MedTechBookApply tempMedTechBoolApply = (FS.HISFC.Models.Terminal.MedTechBookApply)tempList[0];
                ...
                {
                    this.dragMedTechBookApply = (FS.HISFC.Models.Terminal.MedTechBookApply)tempList[0];
                }
                // 项目代码
                ...
                FS.HISFC.Models.Base.Const conItem = (FS.HISFC.Models.Base.Const)managerMgr.GetConstant("MEDTECHITEM", dragMedTechBookApply.ItemComparison.ID);
                if (!conItem.IsValid)
                {
                ...
                // 预约时间
                this.dragMedTechBookApply.MedTechBookInfo.BookTime = FS.FrameWork.Function.NConvert.ToDateTime(this.neuSpreadArrange_1.Cells[row, (int)ExCols.bookDate].Value);
                // 午别
                ...
                FS.HISFC.Models.Base.Const con = (FS.HISFC.Models.Base.Const)managerMgr.GetConstant("NOON", dragMedTechBookApply.Noon.ID);
                if (!con.IsValid)
                {
                ...
                this.dragMedTechBookApply.ReportTime = FS.FrameWork.Function.NConvert.ToDateTime(this.neuSpreadArrange_1.Cells[row, (int)ExCols.reportDate].Text);
                // 有创无创
        ...
        private int ValidBeforeSave(int row)
        {
            ...
            if (FS.FrameWork.Function.NConvert.ToDateTime(this.neuSpreadArrange_1.Cells[row, (int)ExCols.bookDate].Text).Date < System.DateTime.Now.Date)
            {
        ...
        /// <param name="row">行号</param>
        /// <returns></returns>
        private int Save(FS.HISFC.Models.Terminal.MedTechBookApply tempMedTechBoolApply, int row)
        {
            // 数据库事务
            ...
            // 医技预约排班信息
            FS.HISFC.Models.Terminal.MedTechItemTemp tempMedTechItemTemp = new MedTechItemTemp();
            // 返回值
            ...
            this.bookingIntegrate.SetTrans(FS.FrameWork.Management.PublicTrans.Trans);
            //
            // 查询信息的注意事项
            //
            ...
            // 医技预约排班信息
            tempMedTechItemTemp = (FS.HISFC.Models.Terminal.MedTechItemTemp)itemArrangeList[0];
        ...
        private bool IsSpecalPatient(FS.HISFC.Models.Terminal.MedTechBookApply tempMedTechBookApply, ref int i)
        {
            if (this.isClinic)
            {
                // 门诊挂号实体
                FS.HISFC.Models.Registration.Register tempRegister = this.bookingIntegrate.GetByClinic(tempMedTechBookApply.ItemList.Patient.ID);
                ...
                if (tempRegister.RegType == FS.HISFC.Models.Base.EnumRegType.Spe)
                {
                    return true;
                }
            }
            ...
            {
                FS.HISFC.Models.RADT.PatientInfo tempPatient = this.bookingIntegrate.QueryPatientInfoByInpatientNO(tempMedTechBookApply.ItemList.ID); //住院流水号
                if (tempPatient == null)
                {
        ...
        private void InitTree(string ExecDept, System.DateTime beginDate, System.DateTime endDate, string clinicNO, string codeType)
        {
            ...
            foreach (FS.HISFC.Models.Terminal.MedTechBookApply tempMedTechBookApply in applyList)
            {
                //未安排 
                ...
                {
                    FS.HISFC.Models.Terminal.MedTechBookApply temp = applyList[0] as FS.HISFC.Models.Terminal.MedTechBookApply;
                    FS.HISFC.Models.Registration.Register tempRegister = this.bookingIntegrate.GetByClinic(temp.ItemList.Patient.ID);
            ...
            {
                FS.HISFC.Models.Terminal.MedTechBookApply temp = Arrangelist[0] as FS.HISFC.Models.Terminal.MedTechBookApply;
                FS.HISFC.Models.Registration.Register regInfo = this.bookingIntegrate.GetByClinic(temp.ItemList.ID);
        ...
        private void AddFp(ArrayList list)
        {
            ...
            foreach (FS.HISFC.Models.Terminal.MedTechBookApply temp in list)
            {
        ...
        { 
            ...
            foreach (FS.HISFC.Models.Terminal.MedTechBookApply info in list)
            {
        ...
        private void AddFeeItemList()
        {
            ...
            {//门诊患者
                ...
                foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList tempItemList in list)
                {
        ...
        void Delete()
        {
            ...
            for (int i = 0; i < this.neuSpreadBooking_Sheet1.RowCount; i++)
            {
                ...
                {
                    FS.HISFC.Models.Terminal.MedTechBookApply tempApply = (FS.HISFC.Models.Terminal.MedTechBookApply)this.neuSpreadBooking_Sheet1.Rows[i].Tag;
            ...
            FS.HISFC.Models.Terminal.MedTechBookApply tempBookApply = (FS.HISFC.Models.Terminal.MedTechBookApply)selectList[0];
            if (FS.FrameWork.Function.NConvert.ToDateTime(tempBookApply.MedTechBookInfo.BookTime.ToShortDateString()) < FS.FrameWork.Function.NConvert.ToDateTime(System.DateTime.Now.ToShortDateString()))
            {
            ...
            FS.HISFC.Models.Base.Const con = (FS.HISFC.Models.Base.Const)managerMgr.GetConstant("NOON", tempBookApply.Noon.ID);
            if (!con.IsValid)
            {
                MessageBox.Show("该午别已经作废或删除,不能再取消预约");
                return ;
            }
            FS.HISFC.Models.Base.Const conItem = (FS.HISFC.Models.Base.Const)managerMgr.GetConstant("MEDTECHITEM", tempBookApply.ItemComparison.ID);
            if (!conItem.IsValid)
            {
            ...
            FS.HISFC.Models.Terminal.MedTechBookApply tempInfo = (FS.HISFC.Models.Terminal.MedTechBookApply)tempList[0];
            ...
            try
            {
                ...
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                    MessageBox.Show("取消安排失败" + FS.FrameWork.Management.PublicTrans.Err);
                    return;
                }
                else if (Result == 0)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                    MessageBox.Show("数据已经改变请刷新数据" + FS.FrameWork.Management.PublicTrans.Err);
                    return;
                }
                ...
                FS.HISFC.Models.Terminal.MedTechItemTemp info = (FS.HISFC.Models.Terminal.MedTechItemTemp)list[0];
		...
		void tvList_NodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
		{
			...
			try
			{
                this.dragMedTechBookApply   = (FS.HISFC.Models.Terminal.MedTechBookApply)e.Node.Tag;
				System.Data.DataSet ds = new System.Data.DataSet();
				//查询指定时间内的预约扩展信息
                ...
                FS.HISFC.Models.Fee.Item.Undrug undrug = feeMgr.GetItem(dragMedTechBookApply.ItemList.Item.ID);
		...
		void tvList_ItemDrag(object sender, ItemDragEventArgs e)
		{
			...
			if (e.Button == MouseButtons.Left)
			{
				...
				this.dragMedTechBookApply = (FS.HISFC.Models.Terminal.MedTechBookApply)(e.Item as TreeNode).Tag;
		...
		//        FS.HISFC.Models.Base.Const constEntity = (FS.HISFC.Models.Base.Const)this.bookingIntegrate.GetConstant("MEDTECHITEM", this.neuSpreadArrange_1.Cells[e.Row, (int)ExCols.itemCode].Text);
		...
		private void neuSpreadArrange_CellDoubleClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
		{
			if (this.neuSpreadArrange.ActiveSheet == this.neuSpreadArrange_1)
			{
				...
				// 预约时间
				System.DateTime d1 = FS.FrameWork.Function.NConvert.ToDateTime(this.neuSpreadArrange_1.Cells[this.neuSpreadArrange_1.ActiveRowIndex, (int)ExCols.bookDate].Text);
