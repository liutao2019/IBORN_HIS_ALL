...
namespace FS.HISFC.Components.DrugStore.OutBase
{
    ...
    public partial class ucDrugTerminal : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        private void AddTerminal( )
        {
            ...
            {
                info.Name = "新建发药窗口";
                info.TerminalType = FS.HISFC.Models.Pharmacy.EnumTerminalType.发药窗口;
                info.User01 = "New";
            }
            else
            {
                info.Name = "新建配药台";
                info.TerminalType = FS.HISFC.Models.Pharmacy.EnumTerminalType.配药台;
                info.User01 = "New";
            }
            //添加节点
            this.SetItem( info );
        ...
        private void DeleteTerminal( )
        {
            ...
            //确认删除
            DialogResult result = MessageBox.Show( FS.FrameWork.Management.Language.Msg( "删除后将不能恢复，是否继续？" ) , FS.FrameWork.Management.Language.Msg( "提示" ) , MessageBoxButtons.YesNo , MessageBoxIcon.Question , MessageBoxDefaultButton.Button2 );
            ...
            if( !( drugTerminal.User01 == "New" ) )
            {
                ...
                {
                    //提示用户是否确认删除
                    DialogResult res = MessageBox.Show( FS.FrameWork.Management.Language.Msg( "该条记录为其他终端的替代终端、确认进行删除吗？" ) , FS.FrameWork.Management.Language.Msg( "提示" ) , MessageBoxButtons.YesNo , MessageBoxIcon.Question , MessageBoxDefaultButton.Button2 , MessageBoxOptions.RightAlign );
        ...
        private int IsValid( )
        {
            ...
            for( int i = 0 ; i < currListView.Items.Count ; i++ )
            {
                ...
                //不允许关闭所有普通配药台
                if( info.TerminalProperty == FS.HISFC.Models.Pharmacy.EnumTerminalProperty.普通 && !info.IsClose )
                {
                    closeFlag = false;
                }
                //配药台已存在发药窗口时才进行此项判断
                if( info.TerminalType == FS.HISFC.Models.Pharmacy.EnumTerminalType.配药台 )
                {
        ...
        private bool IsValid(FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal)
        {
            ...
            if (drugTerminal.TerminalType == FS.HISFC.Models.Pharmacy.EnumTerminalType.发药窗口)
            {
                if (drugTerminal.IsClose)
                {
        ...
        protected override int OnSave( object sender , object NeuObject )
        {
            if (this.SaveTerminal() == 1)
            {
                ...
                {
                    FS.FrameWork.WinForms.Classes.Function.ShowWaitForm(Language.Msg("正在刷新界面显示"));
