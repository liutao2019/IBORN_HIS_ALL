...
namespace FS.HISFC.Components.DrugStore.Outpatient
{
    ...
    public partial class ucClinicDrug : DrugStore.Outpatient.ucClinicBase,FS.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        ...
        {
            FS.FrameWork.WinForms.Classes.Function.ShowWaitForm(FS.FrameWork.Management.Language.Msg("正在加载单据打印基础数据..."));
        ...
        public virtual void ShowData(FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe)
        {
            ...
            if (al == null)
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("根据调剂信息获取申请明细信息发生错误") + itemManager.Err);
                return;
            }
        ...
        internal List<FS.HISFC.Models.Pharmacy.ApplyOut> GetData()
        {
            ...
            for (int i = 0; i < this.neuSpread1_Sheet1.Rows.Count - 1; i++)
            {
                ...
                {
                    //验证是否存在封帐状态的盘点单 by Sunjh 2010-11-2 {60232B8F-906A-490a-A9F7-4C86679CF7D2}
                    ...
                    {
                        if (MessageBox.Show("药品目" + appTemp.Item.Name + "前在本库房处于封帐状态，是否继续保存？", "封帐提示", MessageBoxButtons.YesNo) == DialogResult.No)
                        {
                            return null;
                        }
                    }
        ...
        public virtual int Save()
        {
            ...
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("请设置操作员"));
                return -1;
            }
            if (this.OperDept == null || this.OperDept.ID == "")
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("请设置操作科室"));
                return -1;
            }
            ...
            switch (this.funModle)
            {
                ...
                case OutpatientFun.Send:                //保存扣库
                    ...
                    if (alData != null && alData.Count > 0)
                    {
                        ...
                        if (applyOut.State == "2")                        
                        {
                            MessageBox.Show(FS.FrameWork.Management.Language.Msg("该药品已发药 不需保存"));
                            return -1;
                        }
                    }
                    try
