...
{
    public class ucQueryLocked:UserControl,FS.FrameWork.WinForms.Forms.IMaintenanceControlable
    {
        ...
        //FS.HISFC.Management.EPR.EMR manager = new FS.HISFC.Management.EPR.EMR();
        DataView dv = null;
        ...
        private void Unlock()
        {
            ...
            if (MessageBox.Show(string.Format("是否给患者【{0}】解锁", name), "警告", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                FS.HISFC.Models.File.DataFileInfo dfi = new FS.HISFC.Models.File.DataFileInfo();
                dfi.ID = fileid;
                dfi.Name = fileName;
                if (FS.HISFC.BizProcess.Factory.Function.IntegrateEPR.SetEMRLocked(dfi,patient, FS.FrameWork.Management.Connection.Operator, false) == -1)
                {
        ...
        private void textBox1_TextChanged(object sender, System.EventArgs e)
        {
         ...
         #region IMaintenanceControlable 成员
            ...
            public int Query()
            {
                DataSet ds = FS.HISFC.BizProcess.Factory.Function.IntegrateEPR.QueryEMRLocked();
                if (ds == null)
                {
                    MessageBox.Show(FS.HISFC.BizProcess.Factory.Function.IntegrateEPR.Err);
                    return -1;
