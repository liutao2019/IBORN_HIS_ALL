			...
			foreach(FS.HISFC.Models.RADT.PatientInfo obj in alPatients)
			{
				...
				foreach(FS.HISFC.Models.EPR.QCConditions condition  in alConditions)
				{
					...
					if(b)
					{
						...
						try
						{
							this.fpSpread1_Sheet1.Cells[0,iColumns].Text = "Ｘ";//+((FS.FrameWork.Models.NeuObject)condition.AlConditions[0]).Memo+((FS.FrameWork.Models.NeuObject)condition.AlConditions[1]).Memo;
						}
						catch{}
