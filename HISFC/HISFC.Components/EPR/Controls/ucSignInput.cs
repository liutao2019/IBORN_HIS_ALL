...
namespace FS.HISFC.Components.EPR
{
	...
	public class ucSignInput : System.Windows.Forms.UserControl
	{
		...
		{
			curSign = obj as FS.FrameWork.Models.NeuObject;
...
//				stream.Close();
				string fileName = FS.FrameWork.WinForms.Classes.Function.GetTempFileName();
		...
		private FS.FrameWork.Models.NeuObject curSign = null;//当前权限
		...
		{
			curSign = sender.Clone(); // as FS.HISFC.Models.RADT.Person;
			this.txtID.Text = curSign.ID;
			this.txtName.Text = curSign.Name;
		}
		...
		{
			this.curSign = null;
			curSign = new FS.FrameWork.Models.NeuObject();
			...
			curSign.User02 = FS.FrameWork.Management.Connection.Operator.ID;
			...
			if(FS.HISFC.BizProcess.Factory.Function.IntegrateEPR.SetSign(this.curSign, byteimg)==-1)
			{
