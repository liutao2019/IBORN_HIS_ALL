...
namespace FS.HISFC.Components.EPR.Controls
{
    ...
    public partial class ucEMRInput : System.Windows.Forms.UserControl, FS.FrameWork.EPRControl.IGroup, FS.FrameWork.EPRControl.IUserControlable
    {
        ...
        [TypeConverter(typeof(FS.FrameWork.EPRControl.emrGroup)), CategoryAttribute("设计"), DefaultValueAttribute(""), DescriptionAttribute("选择控件所在组")]
        public string 组
        {
            get
            {
                 ...
                 if (iEnd > 2 )
                 {
                     ...
                     if (ss.Length >= 2)
                     {
                         ...
                         if (al.Count > 0)
                         {
                             ...
                             FS.HISFC.BizProcess.Factory.Function.IntegrateEPR.SaveNodeToDataStore("DataStore_EMR", dt, this.名称+"\\"+al[0].ToString(),al[0].ToString(), ss[0]);
                         }
                     }
                 }
                 else
