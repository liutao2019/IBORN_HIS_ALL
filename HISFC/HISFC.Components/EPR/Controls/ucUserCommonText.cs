...
namespace FS.HISFC.Components.EPR.Controls
{
    ...
    public partial class ucUserCommonText : UserControl
    {
        ...
        public void RefreshList()
        {
            ...
            #region 获得常用符号列表
            ArrayList al = FS.HISFC.BizProcess.Factory.Function.IntegrateManager.GetUserTextList("SIGN", 2);//获得常用符号列表
            if (al == null)
            {
            ...
            #region 获得常用单词
            al = FS.HISFC.BizProcess.Factory.Function.IntegrateManager.GetUserTextList("WORD", 2);//获得常用单词列表
            if (al == null)
            {
                MessageBox.Show(FS.HISFC.BizProcess.Factory.Function.IntegrateManager.Err);
                return;
            ...
            {
                al = FS.HISFC.BizProcess.Factory.Function.IntegrateManager.GetUserTextList("RELATION", 2);//获得常用相关信息列表
                if (al == null)
                {
                    MessageBox.Show(FS.HISFC.BizProcess.Factory.Function.IntegrateManager.Err);
                    return;
        ...
        public void SetPatient(string inpatientNo, string table, FS.FrameWork.Management.Interface ISql)
        {
           // FS.HISFC.Management.File.DataFile datafileManager = new FS.HISFC.Management.File.DataFile();
            ...
            ArrayList al = FS.HISFC.BizProcess.Factory.Function.IntegrateManager.GetUserTextList("RELATION", 2);//获得常用相关信息列表
            if (al == null)
            {
                MessageBox.Show(FS.HISFC.BizProcess.Factory.Function.IntegrateManager.Err);
                return;
