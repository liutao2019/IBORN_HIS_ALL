...
namespace FS.HISFC.Components.Pharmacy.Base
{
    ...
    /// [创建时间: 2006-11]<br></br>
    /// <修改记录>
    ///    1.实现药品字典列表的自定义过滤与排序 by Sunjh 2010-9-26 {D1523F8E-E81A-4f94-BF39-CCFD16886BDE}
    ///    2.实现药品字典列表的打印功能 by Sunjh 2010-9-26 {037BD776-60DA-4b8f-8E92-49846FDE1ABD}
    /// </修改记录>
    /// </summary>
    public partial class ucPharmacyQuery : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        {
            //FS.FrameWork.WinForms.Classes.Function.GetImage(FS.FrameWork.WinForms.Classes.EnumImageList.T添加);
            toolBarService.AddToolButton("增加", "新增药品", (int)FS.FrameWork.WinForms.Classes.EnumImageList.T添加, true, false, null);
            toolBarService.AddToolButton("修改", "修改当前药品信息", (int)FS.FrameWork.WinForms.Classes.EnumImageList.X修改, true, false, null);
            toolBarService.AddToolButton("删除", "删除当前药品信息", (int)FS.FrameWork.WinForms.Classes.EnumImageList.S删除, true, false, null);
            toolBarService.AddToolButton("复制", "复制当前已有药品信息 生成新药", (int)FS.FrameWork.WinForms.Classes.EnumImageList.F复制, true, false, null);
            toolBarService.AddToolButton("弹出维护", "采用弹出列表方式维护药品信息", (int)FS.FrameWork.WinForms.Classes.EnumImageList.X信息, true, false, null);
            toolBarService.AddToolButton("直接维护", "设置需要直接维护的列", (int)FS.FrameWork.WinForms.Classes.EnumImageList.Z注销, true, false, null);
        ...
        public override void ToolStrip_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            switch (e.ClickedItem.Text)
            {
                ...
                case "复制":
                    ...
                    break;
                case "弹出维护":
                    FS.FrameWork.WinForms.Classes.Function.ShowWaitForm("正在应用维护方式变更 更改为弹出维护方式 请稍候..");
        ...
        protected virtual void SetRow(DataRow row, FS.HISFC.Models.Pharmacy.Item myItem)
        {
            ...
            //by cube 2011-03-28 住院长期医嘱拆分属性设置
            ...
            row["别名五笔码"] = myItem.NameCollection.OtherSpell.WBCode;

            row["学名自定义码"] = myItem.NameCollection.FormalSpell.UserCode;
            row["别名自定义码"] = myItem.NameCollection.OtherSpell.UserCode;
        }
        ...
        private void ShowMaintenanceForm(string inputType,FS.HISFC.Models.Pharmacy.Item item,bool isShow)
        {
            ...
            this.MainteranceUC.DrugAutoSpell = this.drugAutoSpell;
             ...
             //{4EED03A7-7E22-4a93-9ADC-5FF007D51A92} 药品名称和商品名合并
        ...
        public void Print()
        {
            //实现药品字典列表的打印功能 by Sunjh 2010-9-26 {037BD776-60DA-4b8f-8E92-49846FDE1ABD}
        ...
        private void DirectSaveColSet()
        {
            ...
            if (this.isPopSetType == true)         //非直接维护状态
            {
                ...
                this.ucSetColumn.SetDataTable( this.neuSpread1_Sheet1, this.GetIndexByColName( "药品类型" ), this.GetIndexByColName( "系统类别" ), this.GetIndexByColName( "规格" ),
                   this.GetIndexByColName("药品性质"), this.GetIndexByColName("剂型"), this.GetIndexByColName("临嘱拆分"), this.GetIndexByColName("长嘱拆分"), this.GetIndexByColName("使用方法"),
            ...
            FS.FrameWork.WinForms.Classes.Function.PopForm.Text = "直接维护列设置";
            FS.FrameWork.WinForms.Classes.Function.PopShowControl(this.ucSetColumn);
            ...
            foreach (string str in checkCol)
            {
                ...
                switch (str)
                {
                    ...
                    case "临嘱拆分":
                        ...
                        cmbDivCell.Items = new string[] { "最小单位总量取整", "包装单位总量取整", "最小单位每次取整", "包装单位每次取整" };
                    ...
                    case "长嘱拆分":
                        ...
                        cmbDivCell2.Items = new string[] { "最小单位总量取整", "包装单位总量取整", "最小单位每次取整", "包装单位每次取整,最小单位可拆分" };
        ...
        private int SaveModify(DataRow dr)
        {
            ...
            if (drugNO != "")
            {
                ...
                if (itemTemp != null)
                {
                    ...
                    //by cube 2011-03-28 住院长期医嘱拆分属性
                    //itemTemp.SplitType = dr["门诊拆分"].ToString() == "门诊不可拆分包装单位" ? "1" : "0";
                    string splitType = dr["临嘱拆分"].ToString();
                    if (splitType == "最小单位总量取整")
                    {
                        itemTemp.SplitType = "0";
                    }
                    else if (splitType == "包装单位总量取整")
                    {
                        itemTemp.SplitType = "1";
                    }
                    else if (splitType == "最小单位每次取整")
                    {
                        itemTemp.SplitType = "2";
                    }
                    else if (splitType == "包装单位每次取整")
                    {
                        itemTemp.SplitType = "3";
                    }

                    splitType = dr["长嘱拆分"].ToString();
                    if (splitType == "最小单位总量取整")
                    {
        ...
        private void ucSetColumn_DisplayEvent(object sender, EventArgs e)
        {
            if (this.isPopSetType)
            {
                ...
                FS.FrameWork.WinForms.Classes.Function.ShowWaitForm("正在应用列设置...请稍候");
                Application.DoEvents();
        ...
        #region 实现药品字典列表的自定义过滤与排序 by Sunjh 2010-9-26 {D1523F8E-E81A-4f94-BF39-CCFD16886BDE}
