    ...
    public partial class ucQueryBase : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        protected virtual int InitItemData(int itemIndex,CustomItemTypeEnum customItemType,string customTitle,ArrayList alCustomData)
        {
            ...
            if (customItemType == CustomItemTypeEnum.Drug)
            {
                ...
                if (drugList == null)
                {
                    MessageBox.Show(FS.FrameWork.Management.Language.Msg("加载药品信息发生错误" + itemManager.Err));
                    return -1;
                }
            ...
            else if (customItemType == CustomItemTypeEnum.Dept)
            {
                ...
                if (alData == null)
                {
                    MessageBox.Show(FS.FrameWork.Management.Language.Msg("加载科室信息发生错误" + deptManager.Err));
                    return -1;
                }
                itemTitle = "查询科室";
            }
            else if (customItemType == CustomItemTypeEnum.Employee)
            {
                ...
                if (alData == null)
                {
                    MessageBox.Show(FS.FrameWork.Management.Language.Msg("加载人员信息发生错误" + personManager.Err));
                    return -1;
                }
                itemTitle = "查询人员";
            }
        ...
        protected virtual DataSet QueryDataSet(string sql)
        {
            ...
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("执行Sql获取查询数据发生错误"));
                return null;
            }
