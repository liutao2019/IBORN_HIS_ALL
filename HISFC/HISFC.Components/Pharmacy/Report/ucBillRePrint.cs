...
namespace FS.HISFC.Components.Pharmacy.Report
{
    ...
    ///    2、入出库单据补打的多字段过滤条件修改 by Sunjh 2010-9-25 {EEB6DD4C-B37C-4c6b-B453-68AC4CF4370F}
    /// </修改记录>
    /// </summary>
    public partial class ucBillRePrint : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
            ...
            set
            {
                if (value != this.billtype)
                {
                    ...
                    if (value != "I")
                    {
                        ...
                        #endregion
                        Function.IPrint = this.outputInstance as FS.HISFC.BizProcess.Interface.Pharmacy.IBillPrint;
                    }
                    else//入库单显示状态
                    {
                        #region {61FB8119-4622-4bdc-AC03-12527535AB76}
                        ...
                        Function.IPrint = this.inputInstance as FS.HISFC.BizProcess.Interface.Pharmacy.IBillPrint;
                    }
                }
            }
        }
        ...
        {
            toolBarService.AddToolButton("选择", "对当前数据反向选择", FS.FrameWork.WinForms.Classes.EnumImageList.S刷新, true, false, null);
            #region {61FB8119-4622-4bdc-AC03-12527535AB76}
            toolBarService.SetToolButtonEnabled("选择", false); 
            #endregion
        ...
        public void Print()
        {
            ...
            if (this.BillType == "I")
            {
                Function.IPrint = inputInstance as FS.HISFC.BizProcess.Interface.Pharmacy.IBillPrint;
                this.QueryInputDetail(this.neuSpread1_Sheet1.ActiveRowIndex);
            }
            //出库
            if (this.BillType == "O" || this.BillType == "D")
            {
                Function.IPrint =  outputInstance as FS.HISFC.BizProcess.Interface.Pharmacy.IBillPrint;
        ...
        private void QueryInputDetail(int activeRowIndex)
        {
            ...
            {
                //接口打印处理
                FS.FrameWork.WinForms.Classes.Function.ShowWaitForm("正在打印,请稍候...");
        ...
        public void SelectAll()
        {
            ...
            if (this.BillType == "I")
            {
                if (this.neuSpread1_Sheet1.RowCount > 0)
                {
                    ...
                    for (int i = 0; i < this.neuSpread1_Sheet1.Rows.Count; i++)
                    {
                        this.neuSpread1_Sheet1.Cells[i, (int)ColIndex.PrintIndex].Value = !FS.FrameWork.Function.NConvert.ToBoolean(this.neuSpread1_Sheet1.Cells[i, (int)ColIndex.PrintIndex].Value);
                    }
                }
            }
            else
        ...
        protected override int OnSetValue(object neuObject, TreeNode e)
        {
            ...
            {
                this.BillType = (e.Parent.Tag as FS.FrameWork.Models.NeuObject).ID;

                this.privDept = neuObject as FS.FrameWork.Models.NeuObject;
        ...
        {
            //入出库单据补打的多字段过滤条件修改 by Sunjh 2010-9-25 {EEB6DD4C-B37C-4c6b-B453-68AC4CF4370F}
                ...
                if (this.BillType == "I")
                {
                    if (this.dvInput != null)
                    {
                        this.dvInput.RowFilter = "(单据号 LIKE '%" + tempfilter + "%')"
                            ...
                            + "OR (药品来源 LIKE '%" + tempfilter + "%')";  //入出库单据补打的多字段过滤条件修改 by Sunjh 2010-9-25 {EEB6DD4C-B37C-4c6b-B453-68AC4CF4370F}
        ...
        {
            FS.HISFC.Components.Pharmacy.In.ucPhaIn ucInInstance = new FS.HISFC.Components.Pharmacy.In.ucPhaIn();

            inputInstance = ucInInstance.IInPrint;

            FS.HISFC.Components.Pharmacy.Out.ucPhaOut ucOutInstance = new FS.HISFC.Components.Pharmacy.Out.ucPhaOut();
            ...
            //    string billValue1 = ctrlIntegrate.GetControlParam<string>(FS.HISFC.BizProcess.Integrate.PharmacyConstant.Pha_Output_Bill, true, "Report.Pharmacy.ucPhaOutputBill");
