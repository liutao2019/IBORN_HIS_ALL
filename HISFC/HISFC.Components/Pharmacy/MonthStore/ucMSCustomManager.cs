...
namespace FS.HISFC.Components.Pharmacy.MonthStore
{
    ...
    public partial class ucMSCustomManager : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        {
            toolBarService.AddToolButton("增加", "新建计划单", FS.FrameWork.WinForms.Classes.EnumImageList.X新建, true, false, null);
           
            toolBarService.AddToolButton("删除", "删除当前选择的计划药品", FS.FrameWork.WinForms.Classes.EnumImageList.S删除, true, false, null);
        ...
        protected int AddDataToFp(List<FS.HISFC.Models.Pharmacy.MSCustom> msCustomLis)
        {
            ...
            foreach (FS.HISFC.Models.Pharmacy.MSCustom info in msCustomLis)
            {
                ...
                this.neuSpread1_Sheet1.Cells[0, 6].Text = info.Trans == FS.HISFC.Models.Base.TransTypes.Positive ? "收入" : "支出";
        ...
        protected FS.HISFC.Models.Pharmacy.MSCustom GetDataFormFp(int iRowIndex)
        {
            ...
            msCustom.Trans = this.neuSpread1_Sheet1.Cells[iRowIndex, 6].Text == "支出" ? FS.HISFC.Models.Base.TransTypes.Negative : FS.HISFC.Models.Base.TransTypes.Positive;
        ...
        protected string GetDescriptionFromTypeItem(EnumMSCustomType customType,string typeItem,TransTypes trans)
        {
            switch (customType)
            {
                case FS.HISFC.Models.Base.EnumMSCustomType.入库:       //根据入库类型检索                         
                case FS.HISFC.Models.Base.EnumMSCustomType.出库:       //根据出库类型检索
                    FS.HISFC.BizLogic.Manager.PowerLevelManager powerManager = new FS.HISFC.BizLogic.Manager.PowerLevelManager();
                ...
                case FS.HISFC.Models.Base.EnumMSCustomType.科室:       //科室 根据入出库科室选择
                    FS.HISFC.BizLogic.Manager.PrivInOutDept privInOutManager = new FS.HISFC.BizLogic.Manager.PrivInOutDept();

                    return "科室 － " + this.deptHelper.GetName(typeItem);   
                case FS.HISFC.Models.Base.EnumMSCustomType.调价:
                    if (typeItem == "00")
                    {
                ...
                case FS.HISFC.Models.Base.EnumMSCustomType.盘点:       //根据盘点盈亏
                    if (typeItem == "00")
                    {
