...
namespace FS.HISFC.Components.Pharmacy.MonthStore
{
    ...
    /// [创建时间: 2010-06]<br></br>
    /// <修改记录>
    ///    1.修改金额四舍五入至2位小数 by Sunjh 2010-9-13 {48E64E69-A0D8-4240-8F49-9D12C94E4989}
    ///    2.月结界面增加月结报表直接打印 by Sunjh 2010-9-26 {2198371D-2492-46ce-8023-3E7D745097E7}
    ///    3.校验是否存在未录入发票的入库单 by Sunjh 2010-9-26 {413E7A2F-21B4-476c-AC74-76FFD11CA24B}
    ///    4.增加月结作废功能 by Sunjh 2010-11-5 {F8DE60C3-00AF-4b5f-BC50-9D044523926E}
    /// </修改记录>
    /// </summary>
    public partial class ucStoreStat : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        /// 是否验证入库发票号
        /// </summary>
        private bool isCheckInputInvoice = false;
        ...
        /// 是否验证入库发票号
        /// </summary>
        [Category("设置"), Description("是否验证入库发票号"), DefaultValue(false)]
        public bool IsCheckInputInvoice
        {
            get 
            { 
                return isCheckInputInvoice; 
            }
            set 
            { 
                isCheckInputInvoice = value; 
            }
        }
        ...
        {
            //增加月结作废功能 by Sunjh 2010-11-5 {F8DE60C3-00AF-4b5f-BC50-9D044523926E}
            toolBarService.AddToolButton("作废", "作废月结记录", FS.FrameWork.WinForms.Classes.EnumImageList.Z注销, true, false, null);            
        ...
        {
            //修改金额四舍五入至2位小数 by Sunjh 2010-9-13 {48E64E69-A0D8-4240-8F49-9D12C94E4989}
            string sql = @" SELECT DRUG_DEPT_CODE,DRUG_CODE,round(SUM( STORE_SUM / PACK_QTY * (RETAIL_PRICE - PRE_RETAIL_PRICE)),2) COST
            ...
            if (this.isStatTogether == true)            //全院月结 
            {
                sql = @" SELECT DRUG_DEPT_CODE,DRUG_CODE,round(SUM( STORE_SUM / PACK_QTY * (RETAIL_PRICE - PRE_RETAIL_PRICE)),2) COST
                            FROM   PHA_COM_ADJUSTPRICEDETAIL
        ...
        public DateTime GetMonthStoreLastDateTime(string deptCode)
        {
            ...
            {
                lastTime = FS.FrameWork.Function.NConvert.ToDateTime( strLastTime );
            }
            return lastTime;
        }
        ...
        private int SetMonthStoreStatRange(string deptCode)
        {
            ...
            {
                this.dtpBeginStat.Value = FS.FrameWork.Function.NConvert.ToDateTime( this.dataManager.Reader[0] );         //上期月结截止时间、本期月结开始时间
                this.dtpEndStat.Value = FS.FrameWork.Function.NConvert.ToDateTime( this.dataManager.Reader[1] );           //计划月结截止时间
        ...
        private int InitData()
        {
            ...
            foreach (FS.HISFC.Models.Pharmacy.DeptConstant info in alCons)
            {
        ...
        private DataTable CreatMonthStoreDept(string operCode, string deptCode, DateTime lastStatDate,DateTime dtBegin, DateTime dtEnd)
        {
            ...
            {
                lastMonthCost = FS.FrameWork.Function.NConvert.ToDecimal( tempDr["CURRENT_MONTH_COST"].ToString() );
                lastMonthPurchaseCost = FS.FrameWork.Function.NConvert.ToDecimal(tempDr["CURRENT_MONTH_PURCHASE_COST"].ToString());
        ...
        public int ExeMonthStore(string operCode, string deptCode, DateTime dtBegin, DateTime dtEnd)
        {
            ...
            foreach (DataRow dr in dtItem.Rows)
            {
                ...
                dr["LAST_MONTH_NUM"] = FS.FrameWork.Function.NConvert.ToDecimal( drLast["CURRENT_MONTH_NUM"].ToString() ).ToString();
                dr["LAST_MONTH_COST"] = FS.FrameWork.Function.NConvert.ToDecimal( drLast["CURRENT_MONTH_COST"].ToString() ).ToString();
                dr["LAST_MONTH_PURCHASE_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(drLast["CURRENT_MONTH_PURCHASE_COST"].ToString()).ToString();//{A1F19F7E-69AE-407b-B033-D5FCA2BF2A0B}
                ...
                if (drInput != null)
                {
                    dr["IN_NUM"] = FS.FrameWork.Function.NConvert.ToDecimal( drInput["IN_NUM"] );
                    dr["IN_COST"] = FS.FrameWork.Function.NConvert.ToDecimal( drInput["RETAIL_COST"] );
                    dr["IN_PURCHASE_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(drInput["PURCHASE_COST"]);//{A1F19F7E-69AE-407b-B033-D5FCA2BF2A0B}
                }
                else
                {
                    dr["IN_NUM"] = 0;
                ...
                if (drSpeInput != null)
                {
                    dr["SPECIAL_IN_NUM"] = FS.FrameWork.Function.NConvert.ToDecimal(drSpeInput["IN_NUM"]);
                    dr["SPECIAL_IN_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(drSpeInput["RETAIL_COST"]);
                    dr["SPECIAL_IN_PURCHASE_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(drSpeInput["PURCHASE_COST"]);//{A1F19F7E-69AE-407b-B033-D5FCA2BF2A0B}
                }
                else
                {
                    dr["SPECIAL_IN_NUM"] = 0;
                ...
                if (drOutput != null)
                {
                    dr["OUT_NUM"] = FS.FrameWork.Function.NConvert.ToDecimal( drOutput["OUT_NUM"] );
                    dr["OUT_COST"] = FS.FrameWork.Function.NConvert.ToDecimal( drOutput["SALE_COST"] );
                    dr["OUT_PURCHASE_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(drOutput["PURCHASE_COST"]);//{A1F19F7E-69AE-407b-B033-D5FCA2BF2A0B}
                }
                else
                {
                    dr["OUT_NUM"] = 0;
                ...
                if (drSpeOutput != null)
                {
                    dr["SPECIAL_OUT_NUM"] = FS.FrameWork.Function.NConvert.ToDecimal( drSpeOutput["OUT_NUM"] );
                    dr["SPECIAL_OUT_COST"] = FS.FrameWork.Function.NConvert.ToDecimal( drSpeOutput["SALE_COST"] );
                    dr["SPECIAL_OUT_PURCHASE_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(drSpeOutput["PURCHASE_COST"]);//{A1F19F7E-69AE-407b-B033-D5FCA2BF2A0B}
                }
                else
                {
                ...
                {
                    dr["EXPEND_NUM"] = FS.FrameWork.Function.NConvert.ToDecimal( drDispensingOutput["OUT_NUM"].ToString() );
                    dr["EXPEND_COST"] = FS.FrameWork.Function.NConvert.ToDecimal( drDispensingOutput["SALE_COST"].ToString() );
                }
                else
                {
                    dr["EXPEND_NUM"] = 0;
                ...
                if (drCheck != null)
                {
                    decimal checkQty = FS.FrameWork.Function.NConvert.ToDecimal( drCheck["QTY"] );
                    if (checkQty >= 0)
                    {
                        dr["CHECK_PROFIT_NUM"] = checkQty;
                        dr["CHECK_PROFIT_COST"] = FS.FrameWork.Function.NConvert.ToDecimal( drCheck["COST"] );
                        dr["CHECK_PROFIT_PURCHASE_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(drCheck["PURCHASE_COST"]);//{A1F19F7E-69AE-407b-B033-D5FCA2BF2A0B}
                    }
                    else
                    {
                        dr["CHECK_LOSS_NUM"] = checkQty;
                        dr["CHECK_LOSS_COST"] = FS.FrameWork.Function.NConvert.ToDecimal( drCheck["COST"] );
                        dr["CHECK_LOSS_PURCHASE_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(drCheck["PURCHASE_COST"]);//{A1F19F7E-69AE-407b-B033-D5FCA2BF2A0B}
                    }
                }
                else
                {
                ...
                if (drAdjust != null)
                {
                    decimal adjustCost = FS.FrameWork.Function.NConvert.ToDecimal( drAdjust["COST"] );
                    if (adjustCost >= 0)
                    {
            ...
            foreach (string deptKey in statDrugDept.Keys)
            {
                ...
                drDept["CURRENT_MONTH_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(dtItem.Compute("SUM(STORE_COST)", filter));  //本期结存金额
                drDept["CURRENT_STORE_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(drDept["CURRENT_MONTH_COST"]);        
                drDept["IN_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(dtItem.Compute("SUM(IN_COST)", filter));               //入库金额
                drDept["OUT_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(dtItem.Compute("SUM(OUT_COST)", filter));         //出库金额
                //{A1F19F7E-69AE-407b-B033-D5FCA2BF2A0B}
                drDept["CURRENT_MONTH_PURCHASE_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(dtItem.Compute("SUM(CURRENT_MONTH_PURCHASE_COST)", filter)); //本期结存购入金额
                drDept["IN_PURCHASE_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(dtItem.Compute("SUM(IN_PURCHASE_COST)", filter));                     //入库购入金额
                drDept["OUT_PURCHASE_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(dtItem.Compute("SUM(OUT_PURCHASE_COST)", filter));               //出库购入金额
                drDept["SPECIAL_IN_PURCHASE_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(dtItem.Compute("SUM(SPECIAL_IN_PURCHASE_COST)", filter));   //特殊入库购入金额
                drDept["SPECIAL_OUT_PURCHASE_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(dtItem.Compute("SUM(SPECIAL_OUT_PURCHASE_COST)", filter));     //特殊出库购入金额
                drDept["SPECIAL_OUT_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(dtItem.Compute("SUM(SPECIAL_OUT_COST)", filter));           //特殊出库金额
                drDept["SPECIAL_IN_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(dtItem.Compute("SUM(SPECIAL_IN_COST)", filter));                 //特殊入库金额
                drDept["CHECK_PROFIT_PURCHASE_COST"]=FS.FrameWork.Function.NConvert.ToDecimal(dtItem.Compute("SUM(CHECK_PROFIT_PURCHASE_COST)", filter));//盘盈购入金额
                drDept["CHECK_LOSS_PURCHASE_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(dtItem.Compute("SUM(CHECK_LOSS_PURCHASE_COST)", filter));//盘亏购入金额
                drDept["CHECK_PROFIT_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(dtItem.Compute("SUM(CHECK_PROFIT_COST)", filter));       //盘盈金额
                drDept["CHECK_LOSS_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(dtItem.Compute("SUM(CHECK_LOSS_COST)", filter));             //盘亏金额
                drDept["ADJUST_PROFIT_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(dtItem.Compute("SUM(ADJUST_PROFIT_COST)", filter));    //调盈金额
                drDept["ADJUST_LOSS_COST"] = FS.FrameWork.Function.NConvert.ToDecimal(dtItem.Compute("SUM(ADJUST_LOSS_COST)", filter));          //调亏金额

                //decimal cost = FS.FrameWork.Function.NConvert.ToDecimal(dtCheck.Compute("SUM(COST)", filter));
                //if (cost > 0)
                //{
                ...
                //cost = FS.FrameWork.Function.NConvert.ToDecimal(dtAdjust.Compute("SUM(COST)", filter));
                //if (cost > 0)
                //{
            ...
            this.dataManager.SetTrans( FS.FrameWork.Management.PublicTrans.Trans );
        ...
        {
            FS.FrameWork.Models.NeuObject tempOutList = info as FS.FrameWork.Models.NeuObject;
        ...
        {
            FS.FrameWork.Models.NeuObject tempCheck = info as FS.FrameWork.Models.NeuObject;
            ...
            if (alCheckDetil == null)
            {
             ...
             foreach (FS.HISFC.Models.Pharmacy.Check checkInstance in alCheckDetil)
             {
                 this.ucVeriftDataDisplay.DetailSheet.Rows.Add( 0, 1 );
        ...
        private bool Verify()
        {
            ...
            {
                FS.FrameWork.Models.NeuObject tempCheck = new FS.FrameWork.Models.NeuObject();
            ...
            if (alCheckList.Count > 0)
            {
                ...
                if (rs == DialogResult.OK)
                {
                    ...
                    FS.FrameWork.WinForms.Classes.Function.PopForm.Text = "待结存盘点单";
                    FS.FrameWork.WinForms.Classes.Function.PopShowControl( ucVeriftDataDisplay );
                }
                return false;
            }
            ...
            while (this.dataManager.Reader.Read())
            {
                FS.FrameWork.Models.NeuObject tempOutList = new FS.FrameWork.Models.NeuObject();
            ...
            if (alOutList.Count > 0)
            {
                ...
                if (rs == DialogResult.OK)
                {
                    ...
                    FS.FrameWork.WinForms.Classes.Function.PopForm.Text = "出库待核准信息";
                    FS.FrameWork.WinForms.Classes.Function.PopShowControl( ucVeriftDataDisplay );
                }
                return false;
            }
            ...
            #region 校验是否存在未录入发票的入库单 by Sunjh 2010-9-26 {413E7A2F-21B4-476c-AC74-76FFD11CA24B}

            if (this.isCheckInputInvoice)
            {
                verifySql = @"select count(*) from pha_com_input t where t.drug_dept_code='{0}' and t.class3_meaning_code='11' and (t.invoice_no is null or t.invoice_no='') ";
                verifySql = string.Format(verifySql, this.drugDeptCode);

                if (this.dataManager.ExecQuery(verifySql) == -1)
                {
                    MessageBox.Show("校验未录入发票的入库单数据发生错误" + this.dataManager.Err, "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return false;
                }
                ...
                while (this.dataManager.Reader.Read())
                {
                    iCount = Convert.ToInt32(this.dataManager.Reader[0].ToString());
                }

                if (iCount > 0)
                {
                    MessageBox.Show("存在未录入发票的入库单，请录入后再进行月结。");
                    return false;
                }
            }            
        ...
        protected override int OnSave(object sender, object neuObject)
        {
            ...
            //在点击月结保存时 将月结截止时间更改为系统当前时间 {1BE6E869-F631-4886-A633-27FFBD9023D4}
            this.dtpEndStat.Value = this.dataManager.GetDateTimeFromSysDateTime();

            DateTime dtBegin = FS.FrameWork.Function.NConvert.ToDateTime( this.dtpBeginStat.Text );
            DateTime dtEnd = FS.FrameWork.Function.NConvert.ToDateTime( this.dtpEndStat.Text );
        ...
        {
            //月结界面增加月结报表直接打印 by Sunjh 2010-9-26 {2198371D-2492-46ce-8023-3E7D745097E7}
            FS.HISFC.Components.Pharmacy.Report.ucMonthBalanceQuery ucMonthQuery = new FS.HISFC.Components.Pharmacy.Report.ucMonthBalanceQuery();
            ucMonthQuery.InitDept();
            ucMonthQuery.cmbDept.Tag = this.drugDeptCode;
            ucMonthQuery.dtStart.Value = Convert.ToDateTime(this.fpHeadSheet.Cells[this.fpHeadSheet.ActiveRowIndex, 1].Text).AddSeconds(-1);
            ucMonthQuery.dtEnd.Value = Convert.ToDateTime(this.fpHeadSheet.Cells[this.fpHeadSheet.ActiveRowIndex, 1].Text).AddSeconds(1);
            try
            {
                ucMonthQuery.Query();
                ucMonthQuery.cmbMonth.SelectedIndex = 0;
                ucMonthQuery.Print(sender, neuObject);
            }
            catch(Exception ex)
            {
                MessageBox.Show("打印月结单出错!" + ex.Message);
        ...
        {
            DateTime dtBeginTime = FS.FrameWork.Function.NConvert.ToDateTime(this.fpHeadSheet.Cells[this.fpHeadSheet.ActiveRowIndex, 0].Text);
            DateTime dtEndTime = FS.FrameWork.Function.NConvert.ToDateTime(this.fpHeadSheet.Cells[this.fpHeadSheet.ActiveRowIndex, 1].Text);
        ...
        protected int DelMonthStore()
        {
            ...
            FS.FrameWork.Management.PublicTrans.Commit();
            MessageBox.Show(Language.Msg("作废成功"));
