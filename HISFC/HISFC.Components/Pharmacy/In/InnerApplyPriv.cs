    ...
    public class InnerApplyPriv : IPhaInManager
    {
        ...
        private void SetPhaManagerProperty(FS.HISFC.Components.Pharmacy.In.ucPhaIn ucPhaManager)
        {
            ...
            if (this.phaInManager != null)
            {
                ...
                if (!isBack)
                {
                    ...
                    //{37D3D84C-702A-4090-8CB0-B9993279C735}   入库申请暂存
                    this.phaInManager.AddToolBarButton("警戒线", "根据库存上下限自动生成申请", FS.FrameWork.WinForms.Classes.EnumImageList.B报警, 2, false, eFun);

                    System.EventHandler eOutFun = new EventHandler(OutAlterHandler);

                    this.phaInManager.AddToolBarButton("日消耗", "根据日消耗情况自动生成申请", FS.FrameWork.WinForms.Classes.EnumImageList.F分解, 3, false, eOutFun);

                    System.EventHandler stencilFun = new EventHandler(StencilHandler);

                    this.phaInManager.AddToolBarButton("模版", "根据申请模版形成申请计划", FS.FrameWork.WinForms.Classes.EnumImageList.F复制, 4, false, stencilFun);

                    //{37D3D84C-702A-4090-8CB0-B9993279C735}   入库申请暂存
                    System.EventHandler temporaryListFun = new EventHandler(TemporaryListHandler);
                    this.phaInManager.AddToolBarButton("暂存单", "已录入的暂存申请单列表", FS.FrameWork.WinForms.Classes.EnumImageList.X信息, 5, true, temporaryListFun);

                    //{37D3D84C-702A-4090-8CB0-B9993279C735}   入库申请暂存
                    System.EventHandler temporarySaveFun = new EventHandler(TemporarySaveHandler);
                    this.phaInManager.AddToolBarButton("暂存", "对当前已录入的申请单数据进行暂存", FS.FrameWork.WinForms.Classes.EnumImageList.Z暂存, 7, false, temporarySaveFun);

                    //{37D3D84C-702A-4090-8CB0-B9993279C735}   入库申请暂存
                    System.EventHandler clearFun = new EventHandler(ClearHandler);
                    this.phaInManager.AddToolBarButton("清空", "数据初始化 清空已录入信息", FS.FrameWork.WinForms.Classes.EnumImageList.Q清空,9, true, clearFun);
                }
        ...
        protected virtual int AddDataToTable(FS.HISFC.Models.Pharmacy.ApplyOut applyOut,string dataSource)
        {
            if (!Function.JudgePriceConsinstency(this.phaInManager.TargetDept.ID, applyOut.Item))
            {
                ...
                using (uc)
                {
                    FS.FrameWork.WinForms.Classes.Function.PopForm.Text = applyOut.Item.Name + " 全院库存分布";
            ...
            try
            {
                ...
                //{2418802A-9F8E-4390-AEBA-700C984820A0}当库存数量这一列不显示时,没有取出药品库存,入库退库申请时报库存不足
        ...
        public void AddStencilData()
        {
            ...
            {
                FS.FrameWork.WinForms.Classes.Function.ShowWaitForm(Language.Msg("正在根据所选模版生成申请信息..."));
        ...
        public void Save()
        {
            ...
            foreach (DataRow dr in this.dt.Rows)
            {
                ...
                applyOut.Operation.ApplyQty = FS.FrameWork.Function.NConvert.ToDecimal( dr["申请数量"] ) * applyOut.Item.PackQty;
