...
namespace InterfaceInstanceDefault.IOutpatientItemInputAndDisplay
{
    public partial class ucDisplay : UserControl, Neusoft.HISFC.BizProcess.Integrate.FeeInterface.IOutpatientItemInputAndDisplay,Neusoft.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        ...
        /// 补诊查费费用编码
        /// </summary>
        protected string docFeeItemCode = string.Empty;
        ...
        protected int InitControlParams() 
        {
            ...
            //临时挂号费费用编码
            this.docFeeItemCode = this.controlParamIntegrate.GetControlParam<string>("MZ9924", true, string.Empty);
            //docFeeItemCode
            //自费诊查费项目编码
                        ...
                        if (f.ItemRateFlag != "3")
                        {
                            if (pactRate.Rate.PayRate != this.rInfo.Pact.Rate.PayRate)
                            {
                                if (pactRate.Rate.PayRate == 1)//自费
                                {
                                    ...
                                    this.fpSpread1_Sheet1.Cells[currRow, (int)Columns.Self].Text = "自费";
                                    //this.fpSpread1_Sheet1.Cells[currRow, (int)Columns.Self].ForeColor = Color.Red;
                                    this.fpSpread1_Sheet1.Rows[currRow].BackColor = Color.Red;
                                }
                                else
                                {
                                    f.ItemRateFlag = "3";
        ...
        private void SetItem(string itemCode, string drugFlag, string exeDeptCode, int row, decimal amount, decimal saleprice, string unitFlag)
        {
            ...
            if (findRow != null)
            {
                ...
                //{CA82280B-51B6-4462-B63E-43F4ECF456A3}
                if (drugFlag == "0" && exeDeptCode != null)//非药品
                {
                    exeDept = exeDeptCode;
                }
                        ...
                        if (feeItemList.ItemRateFlag != "3")
                        {
                            if (pactRate.Rate.PayRate != this.rInfo.Pact.Rate.PayRate)
                            {
                                if (pactRate.Rate.PayRate == 1)//自费
                                {
                                    ...
                                    this.fpSpread1_Sheet1.Cells[row, (int)Columns.Self].Text = "自费";
                                    //this.fpSpread1_Sheet1.Cells[row, (int)Columns.Self].ForeColor = Color.Red;
                                    this.fpSpread1_Sheet1.Rows[row].BackColor = Color.Red;
                                }
                                else
                                {
        ...
        private ArrayList ConvertGroupToDetail(FeeItemList f)
        {
            ...
            foreach (Neusoft.HISFC.Models.Fee.Item.UndrugComb undrugCombo in undrugCombList)
            {
                ...
                PactItemRate myRate = Neusoft.HISFC.Components.OutpatientFee.Class.Function.PactRate(this.rInfo, feeDetail, ref errMsg);
                if (myRate == null)
                {
                    this.errText = errMsg;
                    return null;
                ...
                if (this.fpSpread1_Sheet1.Rows[i].Tag != null)
                {
                    if (this.fpSpread1_Sheet1.Rows[i].Tag is FeeItemList)
                    {
                        ...
                        if (NConvert.ToDecimal(priceObj.User01) != 0)
                        {
                            ...
                            PactItemRate pRate = Neusoft.HISFC.Components.OutpatientFee.Class.Function.PactRate(this.rInfo, fTemp, ref msgErr);
                            if (pRate == null)
                            {
        ...
        public ArrayList GetFeeItemList()
        {
            ...
            //{F023F620-85F1-4de7-A819-B43FD66D467F}
            //增加 赋值医生所在科室 
            foreach (FeeItemList f in feeItemLists)
            {
                if (!string.IsNullOrEmpty(f.RecipeOper.ID))
                {
                    ...
                    if (e != null)
                    {
                        f.RecipeOper.Dept.User01 = e.Dept.ID;
                    }
                }
        ...
        /// 职级
        /// </summary>
        private ArrayList alDocLevelDig = null;
        /// <summary>
        /// 帮助类
        /// </summary>
        private Neusoft.FrameWork.Public.ObjectHelper alDocLevelDigHelper = new Neusoft.FrameWork.Public.ObjectHelper();
        /// <summary>
        /// 业务层
        /// </summary>
        private Neusoft.HISFC.BizLogic.Manager.Constant consManager = new Neusoft.HISFC.BizLogic.Manager.Constant();
  
        private const string KEY_LOCAL_DOCLEVEL_DIG = "LOCAL_DOCLEVEL_DIG";
        /// <summary>
        /// 添加挂号费
        /// </summary>
        public void AddDocFee()
        {
            if (this.rInfo == null || string.IsNullOrEmpty(this.rInfo.DoctorInfo.Templet.Dept.Name) == true||
                string.IsNullOrEmpty(this.rInfo.DoctorInfo.Templet.Doct.ID)==true)
            {
                return;
            }
            if (this.isCanAddItem==false)
            {
                return;
            }
            if (rInfo.Pact.ID == "15")
            {
                for(int i=0;i<this.fpSpread1_Sheet1.Rows.Count;i++)
                {
                    if (this.fpSpread1_Sheet1.Rows[i].Tag != null)
                    {
                        FeeItemList fitem = this.fpSpread1_Sheet1.Rows[0].Tag as FeeItemList;
                        if (fitem != null && fitem.Item.ID == this.docFeeItemCode)
                        {
            ...
            //如果挂号费用项目没有维护,那么终止
            if (this.docFeeItemCode == string.Empty)
            {
                return;
            }
                ...
                for (int i = 0; i < this.fpSpread1_Sheet1.RowCount; i++)
                {
                    if (this.fpSpread1_Sheet1.Rows[i].Tag != null)
                    {
                        if (this.fpSpread1_Sheet1.Rows[i].Tag is FeeItemList)
                        {
                            ...
                            if (fSame.Item.ID == this.docFeeItemCode)
                            {
            ...
            #region 补诊察费的算法
            #region 诊察费是否为0
            if (this.rInfo.RegLvlFee.OwnDigFee != 0)
            {
                return;
            }
            #endregion
            #region 诊察费是否收过了
            ArrayList fls =
                     this.outpatientManager.QueryFeeItemListsByClinicNO(this.rInfo.ID);
           
            for (int i = 0; i < fls.Count; i++)
            {
                FeeItemList fSame = fls[i] as FeeItemList;
                if (fSame.Item.ID == this.docFeeItemCode)
                {
                    return;
                }
            ...
            #region 真的开始补了
            decimal digFee = 0m;
            this.alDocLevelDig = consManager.GetList(KEY_LOCAL_DOCLEVEL_DIG);
            if (alDocLevelDig == null)
            {
                //MessageBox.Show(Neusoft.FrameWork.Management.Language.Msg("加载列表发生错误"));
                return ;
            }
            this.alDocLevelDigHelper = new Neusoft.FrameWork.Public.ObjectHelper(this.alDocLevelDig);
            NeuObject noDigFee = new NeuObject();
            noDigFee = alDocLevelDigHelper.GetObjectFromID(
                managerIntegrate.GetEmployeeInfo(this.rInfo.DoctorInfo.Templet.Doct.ID).Level.ID
                );
            digFee = Neusoft.FrameWork.Function.NConvert.ToDecimal(noDigFee
               .Name);
            if (digFee==0)
            {
                return;
            ...
            DataRow[] rowFinds = this.dsItem.Tables[0].Select("ITEM_CODE = " + "'" + this.docFeeItemCode + "'");
            ...
            this.alAddRows.Clear();
            this.SetItem(this.docFeeItemCode, "0", this.rInfo.DoctorInfo.Templet.Dept.ID, 0, digFee/
            (NConvert.ToDecimal(rowFinds[0]["UNIT_PRICE"].ToString())), 0, "0");
            ...
            this.fpSpread1_Sheet1.Cells[0, (int)Columns.Price].Locked = true ;
            this.fpSpread1_Sheet1.Cells[0, (int)Columns.ExeDept].Locked = true ;
            this.fpSpread1_Sheet1.Cells[0, (int)Columns.Amount].Locked = true ;
            this.fpSpread1_Sheet1.Cells[0, (int)Columns.Select].Locked = true;

            this.fpSpread1_Sheet1.Cells[0, (int)Columns.CombNo].Locked = true ;
           // this.fpSpread1_Sheet1.Cells[0, (int)Columns.Price].Text = digFee.ToString();
            this.fpSpread1_Sheet1.Cells[0, (int)Columns.Select].Value = true;
            this.fpSpread1_Sheet1.Cells[0, (int)Columns.Select].Locked = true;
            FarPoint.Win.Spread.CellType.TextCellType textCellType = new FarPoint.Win.Spread.CellType.TextCellType();
            textCellType.ReadOnly = true;
            this.fpSpread1_Sheet1.Cells[0, (int)Columns.InputCode].CellType = textCellType;

            FeeItemList fTemp = this.fpSpread1_Sheet1.Rows[0].Tag as FeeItemList;

           // fTemp.Item.Price = digFee;
            ...
            this.fpSpread1_Sheet1.SetActiveCell(1, (int)Columns.InputCode, false);
        }
        /// <summary>
        /// 添加挂号费
        /// </summary>
        ...
        #region IOutpatientItemInputAndDisplay 成员

        DataSet feeItem = null;
