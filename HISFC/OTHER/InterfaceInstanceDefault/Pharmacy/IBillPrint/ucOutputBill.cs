...
namespace InterfaceInstanceDefault.Pharmacy.IBillPrint
{
    ...
    /// [创建时间: 2010-01]<br></br>
    /// </summary>
    public partial class ucOutputBill : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.HISFC.BizProcess.Interface.Pharmacy.IBillPrint
    {
        public ucOutputBill()
        {
            InitializeComponent();
        ...
        /// 设置单据列标题显示
        /// </summary>
        private void SetTitle(FS.HISFC.Models.Pharmacy.Output info)
        {
        ...
        private void Print(ArrayList al, int inow, int icount,bool isLastPage)
        {
            ...
            FS.HISFC.Models.Base.Department stockDept = this.managerIntegrate.GetDepartment( info.StockDept.ID );
            if (stockDept != null)
            {
            ...
            for (int i = 0; i < al.Count; i++)
            {
                ...
                FS.HISFC.Models.Pharmacy.Item tempItem = this.itemManager.GetItem( output.Item.ID );
                if (tempItem != null)
                {
