    ...
    public class ReadMarkNO : FS.HISFC.BizLogic.Fee.IReadMarkNO
    {
        ...
        private int GetCardType(string markNO, ref FS.FrameWork.Models.NeuObject cardTypeObj, ref string errText, ref string realCardNo,string memo)
        {
            try
            {
                ...
                //FS.FrameWork.Models.NeuObject healthCard = new FS.FrameWork.Models.NeuObject();
                //FS.FrameWork.Models.NeuObject medicalCard = new FS.FrameWork.Models.NeuObject();
                ...
                foreach (FS.HISFC.Models.Base.Const conObj in cardTypeList)
                {
                    if (conObj.Memo.Contains(keyWord))
                    {
                        ...
                        //对于刷卡为[:0000000001?]的特殊处理
                        //if (realCardNo.Length > 10)
                        //{
                        //    realCardNo = realCardNo.Substring(0, 10);
                        //}
                ...
                if (memo != "2")
                {
                    cardList = this.accountManager.GetAccountCardByNormalNo(markNO);
                    if (cardList == null)
                    {
                    ...
                    foreach (FS.HISFC.Models.Account.AccountCard accountObj in cardList)
                    {
                        if (cardTypeHelper == null)
                        {
                            cardTypeHelper = new FS.FrameWork.Public.ObjectHelper(cardTypeList);
                        }

                        if (cardTypeHelper.GetObjectFromID(accountObj.MarkType.ID) != null &&
                            !string.IsNullOrEmpty(cardTypeHelper.GetObjectFromID(accountObj.MarkType.ID).Memo))
                        {
                            continue;
                        }
                        ...
                        obj.ID = accountObj.Patient.PID.CardNO;
                        obj.Name = accountObj.Patient.Name;
                        obj.Memo = accountObj.Patient.Sex.Name;
                        obj.SpellCode = accountObj.Patient.Birthday.ToString();
                        obj.WBCode = cardTypeHelper.GetName(accountObj.MarkType.ID);
                        obj.UserCode = accountObj.MarkNO;

                        alTemp.Add(obj);
                    }

                    FS.FrameWork.Models.NeuObject selectObj = null;

                    if (alTemp.Count == 1)
                    {
                        realCardNo = ((FS.FrameWork.Models.NeuObject)alTemp[0]).ID;
                    }
                    else if (alTemp.Count > 1)
                    {
                        if (FS.FrameWork.WinForms.Classes.Function.ChooseItem(alTemp, new string[] { "病历号", "姓名", "性别", "出生日期", "卡类型", "物理卡号" }, new bool[] { true, true, true, false, true, true }, new int[] { 70, 70, 45, 90, 100, 100 }, ref selectObj) != 1)
                        {
                            return 1;
                        }
                    }
