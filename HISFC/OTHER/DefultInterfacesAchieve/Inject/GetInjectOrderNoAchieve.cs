...
using System.Linq;
using System.Text;
using FS.HISFC.BizProcess.Interface.Nurse;
using System.Collections;

namespace FS.DefultInterfacesAchieve.Inject
{
    /// <summary>
    ...
    public class GetInjectOrderNoAchieve : IGetInjectOrderNo
    {
        ...
        public string GetOrderNo(FS.HISFC.Models.Registration.Register reg)
        {
        ...
        public void SetPrintNo(ArrayList injectList)
        {
            ...
            foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList feeItem in injectList)
            {
                string key = feeItem.Order.Combo.ID + feeItem.User03;
                if (htCombo.ContainsKey(key))
                {
                    List<FS.HISFC.Models.Fee.Outpatient.FeeItemList> list = htCombo[key] as List<FS.HISFC.Models.Fee.Outpatient.FeeItemList>;
                    list.Add(feeItem);
                }
                else
                {
                    List<FS.HISFC.Models.Fee.Outpatient.FeeItemList> list = new List<FS.HISFC.Models.Fee.Outpatient.FeeItemList>();
                    list.Add(feeItem);
            ...
            foreach (string key in htCombo.Keys)
            {
                ...
                int count = 5;
                string printNo = "";
                foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList feeItem in list)
                {
                    if (count >= 5)
                    {
