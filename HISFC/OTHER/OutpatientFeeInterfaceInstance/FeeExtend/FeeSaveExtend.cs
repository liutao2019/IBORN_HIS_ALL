...
{
    public class FeeSaveExtend : FS.HISFC.BizProcess.Interface.FeeInterface.IFeeExtendOutpatient
    {
        #region 变量
        ...
        public bool IsValid(FS.HISFC.Models.Registration.Register r, System.Collections.ArrayList Invoices, System.Collections.ArrayList feeItemLists, params object[] otherObjects)
        {
        ...
        protected virtual bool IsAllergyValid(ArrayList feeItemLists) 
        {
            ...
            //需要验证过敏结果的药品集合
            List<FS.HISFC.Models.Fee.Outpatient.FeeItemList> pharmarcys = new List<FS.HISFC.Models.Fee.Outpatient.FeeItemList>();
            //药品业务层
            ...
            foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList f in feeItemLists)
            {
                ...
                {
                    FS.HISFC.Models.Pharmacy.Item itemTemp = pharmacyIntegrate.GetItem(f.Item.ID);
                    if (itemTemp == null) 
                    {
            ...
            foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList f in pharmarcys) 
            {
                ...
                // {BF4D14DA-84F3-4a8d-8CEB-6E5A7BF425F8}
                // 接口更新
                FS.HISFC.Models.Order.OutPatient.Order orderTemp = orderIntegrate.GetOneOrder(f.Patient.ID, f.Order.ID);
                //FS.HISFC.Models.Order.OutPatient.Order orderTemp = orderIntegrate.GetOneOrder(f.Order.ID);
                ...
                //需要皮试,但是还没有皮试结果
                if (orderTemp.HypoTest == FS.HISFC.Models.Order.EnumHypoTest.NeedHypoTest) 
                {
                    this.errText += "[ " + f.Item.Name  + " ]" + "应该皮试,没有皮试结果!" + "\n";

                    unValidList.Add(f);
                }
                if (orderTemp.HypoTest == FS.HISFC.Models.Order.EnumHypoTest.Negative) 
                {
