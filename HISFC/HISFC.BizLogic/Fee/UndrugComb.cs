...
namespace FS.HISFC.BizLogic.Fee
{
    ...
    ///  />
    /// </summary>
    [Obsolete("作废,复合项目于非药品合成一个,此业务层已完全无效",true)]
    public class UndrugComb : FS.FrameWork.Management.Database
    {
        ...
        private ArrayList QueryUndrugCombDetailsBySql(string sql, params string[] args) 
        {
            ...
            FS.HISFC.Models.Fee.Item.UndrugComb undrugComb = new FS.HISFC.Models.Fee.Item.UndrugComb();//组合项目实体

            try
            {
                while (this.Reader.Read())
                {
                    undrugComb = new FS.HISFC.Models.Fee.Item.UndrugComb();
        ...
        private string[] GetUndrugCombParams(FS.HISFC.Models.Fee.Item.UndrugComb undrugComb)
        {
            string[] args = 
            {
        ...
        private ArrayList QueryUndrugCombs(string whereIndex, params string[] args)
        {
            ...
            if (this.Sql.GetCommonSql("Fee.undrugzt.GetUndrugzt.Valid", ref sql) == -1)
            {
        ...
        public ArrayList QueryUndrugCombsAll()
        {
            ...
            if (this.Sql.GetCommonSql("Fee.undrugzt.GetUndrugzt", ref sql) == -1)
            {
        ...
        public FS.HISFC.Models.Fee.Item.UndrugComb GetUndrugCombValidByCode(string undrugCombCode)
        {
            ...
            return undrugCombs[0] as FS.HISFC.Models.Fee.Item.UndrugComb;
        }
        ...
        public FS.HISFC.Models.Fee.Item.UndrugComb GetUndrugCombByCode(string undrugCombCode)
        {
            ...
            //if (this.Sql.GetCommonSql("Fee.undrugzt.GetUndrugzt.1", ref sql) == -1)
            //{
        ...
        public ArrayList QueryUndrugCombDetailsByCombCode(string combCode)
        {
            ...
            if (this.Sql.GetCommonSql("Fee.undrugzt.GetUndrugztinfo", ref sql) == -1)
            {
        ...
        public FS.HISFC.Models.Fee.Item.UndrugComb GetSingleUndrugzt(string ztID)
		{
			string sql="";
			if(this.Sql.GetCommonSql("Fee.undrugzt.GetSingleUndrugzt.1",ref sql)==-1)return null;
			FS.HISFC.Models.Fee.Item.UndrugComb info= new FS.HISFC.Models.Fee.Item.UndrugComb();
			try
			{			
				sql=string.Format(sql,ztID);
				...
				while(this.Reader.Read())
				{
					...
					info.IsNeedConfirm = FS.FrameWork.Function.NConvert.ToBoolean(Reader[8].ToString());
        ...
        public FS.HISFC.Models.Fee.Item.UndrugComb GetValidUndrugzt(string ztID)
        {
            string sql = "";
            string strSql = "";
            if (this.Sql.GetCommonSql("Fee.undrugzt.GetUndrugzt.Valid", ref sql) == -1) return null;
            if (this.Sql.GetCommonSql("Fee.undrugzt.GetUndrugzt.1", ref strSql) == -1) return null;
            sql = sql + strSql;
            ...
            try
            {
                ...
                while (this.Reader.Read())
                {
                    ...
                    if (Reader[9].ToString() == "0")
                    {
                        ...
                        info.IsNeedConfirm = FS.FrameWork.Function.NConvert.ToBoolean(this.Reader[8].ToString());
                        info.ValidState = Reader[9].ToString();
                        info.User01 = Reader[10].ToString();
                        info.User02 = Reader[11].ToString();
                        info.Memo = Reader[12].ToString(); //备注
        ...
        public ArrayList GetValidUndrugzt()
        {
            ...
            ArrayList List = null;
            string strSql = "";
            if (this.Sql.GetCommonSql("Fee.undrugzt.GetUndrugzt.Valid", ref strSql) == -1) return null;
            try
            {
