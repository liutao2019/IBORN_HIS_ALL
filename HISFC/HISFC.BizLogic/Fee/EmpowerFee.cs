...
namespace FS.HISFC.BizLogic.Fee
{
    ...
    /// </summary>
    public class EmpowerFee : FS.FrameWork.Management.Database
    {
        #region 查询
        /// <summary>
        /// 查询收费员是否被收费
        ...
        public bool QueryIsEmpower(string operCode)
        {
            ...
            if (this.Sql.GetCommonSql("Fee.Inpatient.IsEmpowerFee", ref sql) == -1)
            {
                this.Err = "查询索引为Fee.Inpatient.IsEmpowerFee的SQL语句失败！";
                return false;
            }
            sql = string.Format(sql, operCode);
            int returnValue = FS.FrameWork.Function.NConvert.ToInt32(this.ExecSqlReturnOne(sql));
            if (returnValue <= 0)
            {
                return false;
            }
        ...
        {
            string sql = string.Empty;
            if (this.Sql.GetCommonSql("Fee.Inpatient.IsDayBalance", ref sql) == -1)
            {
                this.Err = "查询索引为Fee.Inpatient.IsEmpowerFee的SQL语句失败！";
                return false;
            }
        ...
        {
            string sql = string.Empty;
            if (this.Sql.GetCommonSql("Fee.Inpatient.QueryAllDayBalance", ref sql) == -1)
            {
        ...
        {
            string sql = string.Empty;
            if (this.Sql.GetCommonSql("Fee.Inpatient.QueryAllEmpowered", ref sql) == -1)
            {
        ...
        {
            string sql = string.Empty;
            if (this.Sql.GetCommonSql("Fee.Inpatient.DeleteEmpowerByPk", ref sql) == -1)
            {
            ...
            int returnValue = FS.FrameWork.Function.NConvert.ToInt32(this.ExecNoQuery(sql));
            if (returnValue <= 0)
            {
                return false;
            }
        ...
        public bool InsertEmpowerOper(FS.HISFC.Models.Fee.EmpowerFeeOper empowerFeeOper)
        {
            string sql = string.Empty;
            if (this.Sql.GetCommonSql("Fee.Inpatient.InsertEmpowerOper", ref sql) == -1)
            {
