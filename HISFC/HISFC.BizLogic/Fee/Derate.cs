        ...
        public ArrayList GetDerateByClinicAndBalance(string Clinic, int BalanceNo)
        {
            ...
            try
            {
                ...
                while (this.Reader.Read())
                {
                    ...
                    info.confirmName = Reader[10].ToString(); // 批准人编码
                    info.deptCode = Reader[11].ToString(); //  科室
                    info.BalanceNo = FS.FrameWork.Function.NConvert.ToInt32(Reader[12].ToString());//结算号
                    info.balanceState = Reader[13].ToString(); // 结算状态
                    info.opercode = Reader[14].ToString(); // 操作员
                    info.operdate = Convert.ToDateTime(Reader[15]);// 操作时间
                    List.Add(info);
                }
...
/// <returns></returns>
        ...
        private ArrayList QueryDerateInfoByIndex(string sqlIndex, params string[] args)
        {
            ...
            while (this.Reader.Read())
            {
                ...
                {
                    info.SequenceNO = FS.FrameWork.Function.NConvert.ToInt32((Reader[4].ToString())); //处方内项目流水号
                }
