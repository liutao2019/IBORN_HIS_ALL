...
namespace FS.HISFC.BizLogic.Fee 
{
	...
	public class TemporaryFee :FS.FrameWork.Management.Database
	{
		...
		public int Insert(FS.HISFC.Models.Fee.Inpatient.FeeItemList item,string InpatientNO,string DeptCode,string ApplyNO,string OrderID)
		{
			...
			try
			{  
				string[] strParm = new string[9];  //取参数列表
		...
		public ArrayList  Query(string InpatientNO,string DeptCode,string ApplyNO)
		{
			...
			{  
				...
				while(this.Reader.Read())
				{
					...
					obj.Item.Qty = FS.FrameWork.Function.NConvert.ToDecimal(Reader[2].ToString());//数量
					obj.FTRate.ItemRate = FS.FrameWork.Function.NConvert.ToDecimal(Reader[3].ToString()); //比率
					obj.Item.Price =FS.FrameWork.Function.NConvert.ToDecimal(Reader[8].ToString());//价格
					list.Add(obj);
				}
				this.Reader.Close();
				return list;
