    ...
    public class DataFileInfo : FS.FrameWork.Management.Database, FS.HISFC.Models.Base.IManagement
    {
        ...
        private ArrayList myGetFiles(FS.HISFC.Models.File.DataFileParam param, string strSql)
        {
            ...
            while (this.Reader.Read())
            {
                ...
                try
                {
                    DataFileInfo.valid = FS.FrameWork.Function.NConvert.ToInt32(this.Reader[13].ToString());//有效标志
                }
                catch
                { }
                try
                {
                    DataFileInfo.UseType = FS.FrameWork.Function.NConvert.ToInt32(this.Reader[14].ToString());//用户类型
                    DataFileInfo.Count = FS.FrameWork.Function.NConvert.ToInt32(this.Reader[15].ToString());//使用次数
                }
                catch
                { }
                al.Add(DataFileInfo);
        ...
        public FS.FrameWork.Models.NeuObject Get(object id, int iType)
        {
            ...
            if (this.Reader.Read())
            {
                ...
                DataFileInfo.Param.IsInDB = FS.FrameWork.Function.NConvert.ToBoolean(this.Reader[13]);//是否在数据库
                try
                {
                    DataFileInfo.valid = FS.FrameWork.Function.NConvert.ToInt32(this.Reader[14]);//有效标致
                }
                catch { }
                try
                {
                    DataFileInfo.UseType = FS.FrameWork.Function.NConvert.ToInt32(this.Reader[15].ToString());//用户类型
                    DataFileInfo.Count = FS.FrameWork.Function.NConvert.ToInt32(this.Reader[16].ToString());//使用次数                
                }
                catch { }
            }
            this.Reader.Close();
