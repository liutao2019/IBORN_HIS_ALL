...
namespace FS.HISFC.BizLogic.Nurse.InjectManager
{
    ...
    ///  />
    /// </summary> 
    public class KickbackMgr : FS.FrameWork.Management.Database
    {
        public KickbackMgr()
        {
        }
        ...
        public List<FS.HISFC.Models.Nurse.Kickback> QueryKickback()
        {
            string strsql = "";
            if (this.Sql.GetCommonSql("INJECT.KICKBACK.SELECTALL", ref strsql) == -1)
            {
            ...
            List<FS.HISFC.Models.Nurse.Kickback> kbList = new List<FS.HISFC.Models.Nurse.Kickback>();
        ...
        public int InsertKBItem(FS.HISFC.Models.Nurse.Kickback kb)
        {
            if (kb == null)
            {
                return -1;
            ...
            string strsql = "";
            if (this.Sql.GetCommonSql("INJECT.KICKBACK.INSERTITEM", ref strsql) == -1)
            {
        ...
        /// <param name="kb">实体</param>
        /// <returns>-1失败，1成功</returns>
        public int UpdateKBItem(FS.HISFC.Models.Nurse.Kickback kb)
        {
            string strsql = "";
            if (this.Sql.GetCommonSql("INJECT.KICKBACK.UPDATEITEM_ID", ref strsql) == -1)
            {
                return -1;
            }
            try
