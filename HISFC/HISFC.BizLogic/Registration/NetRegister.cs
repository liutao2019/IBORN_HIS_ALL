...
{
    public class NetRegister : FS.FrameWork.Management.Database
    {
        #region 私有方法
        /// <summary>
        /// 获取集成平台挂号信息SQL语句
        ...
        private string GetNetRegisterSql()
        {
            ...
            if (this.Sql.GetCommonSql("Net_GetRegister.Sql", ref sql) ==-1)
            {
        ...
        public DataSet  GetNetRegister(DateTime dtBegin,DateTime dtEnd)
        {
            ...
            if (this.Sql.GetCommonSql("Net_getRegister.Where1", ref sqlWhere) == -1)
            {
        ...
        {
            string sql = string.Empty ;
            if (this.Sql.GetCommonSql("Net_UpdateRegisterConfirm", ref sql) == -1)
            {
