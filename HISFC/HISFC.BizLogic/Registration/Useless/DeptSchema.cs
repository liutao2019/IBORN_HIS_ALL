...
namespace FS.HISFC.BizLogic.Registration
{
	...
	/// 科室排班管理类
	/// </summary>
	public class DeptSchema:FS.FrameWork.Management.Database
	{
		/// <summary>
		/// 
		/// </summary>
		...
		private FS.HISFC.Models.Registration.DeptSchema schema=null;
		/// <summary>
		/// 插入科室出诊表记录
		/// </summary>
		/// <param name="schema"></param>
		/// <returns></returns>
		public int Insert(FS.HISFC.Models.Registration.DeptSchema schema)
		{
			string sql="";
		...
		/// <param name="schema"></param>
		/// <returns></returns>
		public int Delete(FS.HISFC.Models.Registration.DeptSchema schema)
		{
			string sql="";
		...
		/// <param name="schema"></param>
		/// <returns></returns>
		public int Update(FS.HISFC.Models.Registration.DeptSchema schema)
		{
			string sql="";
		...
		private ArrayList QuerySchema(string sql)
		{
			...
			try
			{
				...
				{
					schema=new FS.HISFC.Models.Registration.DeptSchema();
					schema.ID=this.Reader[2].ToString();
					schema.SeeDate=FS.FrameWork.Function.NConvert.ToDateTime(this.Reader[3].ToString());//看诊日期
					...
					if(this.Reader.IsDBNull(9)==false)
						schema.RegLimit=FS.FrameWork.Function.NConvert.ToInt32(this.Reader[9].ToString());//挂号限额
					if(this.Reader.IsDBNull(10)==false)
						schema.PreRegLimit=FS.FrameWork.Function.NConvert.ToInt32(this.Reader[10].ToString());//预约挂号限额
					schema.HasReg=FS.FrameWork.Function.NConvert.ToInt32(this.Reader[12].ToString());//已挂
					schema.HasPreReg=FS.FrameWork.Function.NConvert.ToInt32(this.Reader[13].ToString());//预约已挂
					schema.IsValid=FS.FrameWork.Function.NConvert.ToBoolean(this.Reader[16].ToString());
					...
					if(this.Reader.IsDBNull(20)==false)
						schema.StopDate=FS.FrameWork.Function.NConvert.ToDateTime(this.Reader[20].ToString());
					schema.Memo=this.Reader[21].ToString();//备注
					schema.OperDate=FS.FrameWork.Function.NConvert.ToDateTime(this.Reader[22].ToString());
