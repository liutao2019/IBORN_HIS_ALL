   ...
   AND dept_code='{1}'*/
		...
		private int QueryTabular(string sql)
		{
			...
			while(Reader.Read())
			{
				...
				tabulation.Workdate=FS.FrameWork.Function.NConvert.ToDateTime(Reader[4].ToString());//出勤日期
				...
				if(Reader.IsDBNull(8)==false)tabulation.Kind.BeginTime=FS.FrameWork.Function.NConvert.ToDateTime(Reader[8].ToString());
				if(Reader.IsDBNull(9)==false)tabulation.Kind.EndTime=FS.FrameWork.Function.NConvert.ToDateTime(Reader[9].ToString());
				if(Reader.IsDBNull(10)==false)tabulation.Kind.MinusDays=FS.FrameWork.Function.NConvert.ToDecimal(Reader[10].ToString());
				if(Reader.IsDBNull(11)==false)tabulation.Kind.PositiveDays=FS.FrameWork.Function.NConvert.ToDecimal(Reader[11].ToString());
				tabulation.arrangeID=Reader[12].ToString();//排班序号
				tabulation.OperID=Reader[13].ToString();//操作员
				if(Reader.IsDBNull(14)==false)tabulation.OperDate=FS.FrameWork.Function.NConvert.ToDateTime(Reader[14].ToString());//操作时间
				tabulation.Memo=Reader[16].ToString();//备注
				tabulation.SortID = FS.FrameWork.Function.NConvert.ToInt32(Reader[17].ToString());
