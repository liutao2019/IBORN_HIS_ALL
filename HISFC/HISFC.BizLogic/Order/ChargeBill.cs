		...
		private ArrayList myQueryChargeBill( string sql )
		{
			...
			try
			{
				...
				while(Reader.Read())
				{
                    ...
                    bill.Price = FS.FrameWork.Function.NConvert.ToDecimal(Reader[13].ToString());//价格
                    bill.Qty = FS.FrameWork.Function.NConvert.ToDecimal(Reader[14].ToString());//数量
					...
					bill.IsPrint=FS.FrameWork.Function.NConvert.ToBoolean(Reader[22].ToString());//是否打印
		...
		private ArrayList myQueryChargeBillByNurseStation( string sql )
		{
			...
			try
			{
				...
				while(Reader.Read())
				{
					...
					bill.ExecOrderID = Reader[20].ToString();//执行流水号
					bill.BillNO = Reader[21].ToString();//单据号
					bill.IsPrint = FS.FrameWork.Function.NConvert.ToBoolean(Reader[22].ToString());//打印标记
					bill.Oper.ID =Reader[23].ToString();//操作人
					bill.Oper.OperTime = FS.FrameWork.Function.NConvert.ToDateTime(Reader[24].ToString());//操作时间
					bill.PrintOper.ID =Reader[25].ToString();//打印人
					bill.PrintOper.OperTime = FS.FrameWork.Function.NConvert.ToDateTime(Reader[26].ToString());//打印时间
					bill.IsCharge=FS.FrameWork.Function.NConvert.ToBoolean(Reader[27].ToString());//是否收费标记
					bill.Oper.ID =Reader[28].ToString();//收费人
					bill.Oper.OperTime = FS.FrameWork.Function.NConvert.ToDateTime(Reader[29].ToString());//收费时间
					bill.TotCost = FS.FrameWork.Function.NConvert.ToDecimal(Reader[30].ToString());//总额
					bill.Combo.ID = Reader[31].ToString();//组合号
