...
namespace FS.HISFC.BizLogic.Manager
{
	...
	public class Report:DataBase 
	{
		...
		public FS.HISFC.Models.Base.Report GetReport(string ParentGroupID,string GroupID,string ReportID)
		{
			...
			try
			{
				while(this.Reader.Read())
				{
					...
					Report.IsValid = FS.FrameWork.Function.NConvert.ToBoolean(Reader[7].ToString());//是否有效
					Report.SortID = FS.FrameWork.Function.NConvert.ToInt32(Reader[10].ToString()); //序号
				}
			}
			catch(Exception ex)
			{
		...
		/// <summary>
		/// 根据组获得报表-FS.HISFC.Models.Admin.SysModelFunction 
		/// </summary>
		/// <param name="GroupID">组ID</param>
		/// <returns>模块列表FS.HISFC.Models.Admin.SysModelFunction </returns>
