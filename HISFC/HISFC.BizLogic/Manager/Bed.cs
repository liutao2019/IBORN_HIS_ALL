    ...
    public class Bed : DataBase
    {
        ...
        private string[] myGetParm(FS.HISFC.Models.Base.Bed objBed)
        {
            string[] strParm ={   objBed.ID,						//0 床号
								 ...
								 FS.FrameWork.Function.NConvert.ToInt32(objBed.IsValid).ToString(),	//5 是否有效
								 ...
								 FS.FrameWork.Function.NConvert.ToInt32(objBed.IsPrepay).ToString(),	//13是否预约
        ...
        /// 传入病区ID得到病区所有床位信息
        /// </summary>
        /// <param name="NurseStationId">ALL 查询所有</param>
        /// <returns></returns>
            ...
            if (this.GetSQL("Manager.Bed.GetFeeBedList.1", ref strSql) == 0)
            {
                ...
                while (this.Reader.Read())
                {
                    ...
                    try
                    {
                        ...
                        obj.PrepayOutdate = FS.FrameWork.Function.NConvert.ToDateTime(this.Reader[10].ToString());		//预约出院日期
                        //是否有效:0有效,1无效
                        ...
                        obj.IsPrepay = FS.FrameWork.Function.NConvert.ToBoolean(this.Reader[12].ToString());			//是否预约
                        obj.SortID = FS.FrameWork.Function.NConvert.ToInt32(this.Reader[13].ToString());	//排序
        ...
        private ArrayList myGetBedList()
        {
            ...
            while (this.Reader.Read())
            {
                ...
                try
                {
                    ...
                    obj.InpatientNO = this.Reader[9].ToString();			//患者住院流水号
                    obj.PrepayOutdate = FS.FrameWork.Function.NConvert.ToDateTime(this.Reader[10].ToString());		//预约出院日期			
                    obj.IsValid = FS.FrameWork.Function.NConvert.ToBoolean(this.Reader[11].ToString());			//是否有效:0有效,1无效
                    obj.IsPrepay = FS.FrameWork.Function.NConvert.ToBoolean(this.Reader[12].ToString());			//是否预约
                    obj.SortID = FS.FrameWork.Function.NConvert.ToInt32(this.Reader[13].ToString());	//排序
                    obj.User01 = this.Reader[14].ToString();			//操作人编码
                    obj.User02 = this.Reader[15].ToString();				//操作日期
                    obj.BedGrade.Name = this.Reader[16].ToString();		//床位等级名称
                    obj.User03 = this.Reader["sumfee"].ToString();		//床位费
                    obj.Memo = this.Reader["specialFee"].ToString();	//特诊费
                    obj.AdmittingDoctor.ID = this.Reader[18].ToString();//住院医生
                    obj.AttendingDoctor.ID = this.Reader[19].ToString();//主治医生
                    obj.ConsultingDoctor.ID = this.Reader[20].ToString();//主任医生
                    obj.AdmittingNurse.ID = this.Reader[21].ToString();	//责任护士
