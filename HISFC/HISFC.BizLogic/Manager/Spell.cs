...
namespace FS.HISFC.BizLogic.Manager
{
	...
	public class Spell:DataBase
	{
		...
		public FS.HISFC.Models.Base.ISpell Get(string Words)
		{
			...
			if(this.GetSQL("Manager.Spell.GetSpellCode",ref strSql)==-1) return null;
			strExeSql =  strSql;
		...
		/// <summary>
		/// 根据拼音码获取同音的字  返回 FS.HISFC.Models.Base.SpellCode类型实体，Name存储汉字
		/// </summary>
		...
		public ArrayList  GetWord(string SpellCode)
		{
			try
			{
				...
				string strSql = "";
				if(this.GetSQL("Manager.Spell.GetWord",ref strSql)==-1) return null;
		...
		public string GetSpellCode(string word)
		{
            ...
            //    string SpellCode = "";
            //    string strSql = "";
            //    if(this.GetSQL("Manager.Spell.GetSpellCode.2",ref strSql)==-1) return null;
            ...
            {
                string temp = tochinesespell(word[i].ToString());
                if (temp.Length >= 1)
                {
                    spell += temp[0];
                }
        ...
        /// <returns></returns>
        private static string tochinesespell(string strtext)
        {
        ...
        #region 汉字拼音首字母列表 本列表包含了20902个汉字,用于配合 tochinesespell 函数使用,本表收录的字符的unicode编码范围为19968至40869
