...
namespace FS.HISFC.BizLogic.Manager
{
	...
	/// 调价
	/// </summary>
	public class AdjustPrice  : DataBase
	{
		public AdjustPrice()
		{
			//
		...
		public string  GetAdjustPriceSequence()
		{
			...
			try
			{
				...
				if (this.GetSQL("Fee.CAdjustPrice.GetAdjustPriceSequence",ref strSql)==-1)return null;
		...
		public int  InsertAdjustPrice(FS.HISFC.Models.Fee.Item.AdjustPrice info)
		{
			string strSql = "";
			if(info.IsValid)
			{
				//调价即时生效
				if (this.GetSQL("Fee.CAdjustPrice.InsertCAdjustPrice1",ref strSql)==-1)return -1;
				try
				{
			...
			{
				if (this.GetSQL("Fee.CAdjustPrice.InsertCAdjustPrice2",ref strSql)==-1)return -1;
				try
				{
					string OperId = this.Operator.ID;
		...
		/// <param name="info"></param>
		/// <returns></returns>
		public int InsertAdjustPriceDetail(FS.HISFC.Models.Fee.Item.AdjustPrice info)
		{
			string strSql = "";
			if (this.GetSQL("Fee.CAdjustPrice.InsertCAdjustPriceDetail",ref strSql)==-1)return -1;
			try
			{
				string OperId = this.Operator.ID;
				string ValueState ="";
		...
		public ArrayList SelectPriceAdjustHead(System.DateTime  BeginTime,System.DateTime EndTime )
		{
			ArrayList List = null;
			string strSql = "";
			if (this.GetSQL("Fee.CAdjustPrice.SelectPriceAdjustHead",ref strSql)==-1) return null;
			try
			{
				strSql = string.Format(strSql,BeginTime,EndTime);
				this.ExecQuery(strSql);
				FS.HISFC.Models.Fee.Item.AdjustPrice info = null;
				List = new ArrayList();
				while(this.Reader.Read())
				{
					info = new FS.HISFC.Models.Fee.Item.AdjustPrice();
		...
		public ArrayList SelectPriceAdjustTail(string AdjustPriceNo)
		{
			ArrayList List = null;
			string strSql = "";
			if (this.GetSQL("Fee.CAdjustPrice.SelectPriceAdjustTail",ref strSql)==-1) return null;
			try
			{
		...
		/// <returns></returns>
		public int UpdateAdjustPriceDetail(string ItemCode)
		{
			string strSql ="";
			try
			{
				if (this.GetSQL("Manager.CAdjustPrice.UpdateCAdjustPriceDetail",ref strSql)==-1) return -1;
