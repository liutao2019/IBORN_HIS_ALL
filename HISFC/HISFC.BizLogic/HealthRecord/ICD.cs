...
namespace FS.HISFC.BizLogic.HealthRecord
{
    ...
    /// <summary>
    /// (ICD10, ICD9, 手术ICD, ICD9, ICD10共用一个业务层 Creator: zhangjunyi@FS.com  2005/05/30
    /// </summary>
    public class ICD : FS.FrameWork.Management.Database
    {
        ...
        /// <summary>
        /// 函数说明: 根据ICD类别插入ICD表 Creator: zhangjunyi@FS.com  2005/06/01
        /// </summary>
            ...
            try
            {
                ...
                switch (ICDType)
                {
                    case ICDTypes.ICD10:
                        ...
                        {
                            this.Err = "获取SQL语句失败,索引:Case.ICDDML.Query.ICD10.Base.Ds.Case";
                            return -1;
                        }
                        break;
                    case ICDTypes.ICD9:
        ...
        private ArrayList ICDReaderInfo()
        {
            ...
            try
            {
                ...
                while (this.Reader.Read())
                {
                    ...
                    icd.StandardDays = FS.FrameWork.Function.NConvert.ToInt32(Reader[11].ToString());//标准住院日
                    ...
                    icd.IsValid = FS.FrameWork.Function.NConvert.ToBoolean(Reader[16].ToString());//是否有效
                    ...
                    //佛山南庄等五家医院models不能更新，必须屏蔽下面24才行 2012-11-14
        ...
        public List<FS.HISFC.Models.HealthRecord.ICD> QueryTempICD(ICDTypes ICDType)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        icd = new FS.HISFC.Models.HealthRecord.ICD();
                        icd.ID = Reader[0].ToString();//ICD编码
                        icd.Name = Reader[1].ToString(); //疾病名称
                        icd.DiseaseCode = Reader[2].ToString(); //疾病分类代码
                        icd.WBCode=this.Reader[3].ToString();
                        icd.SpellCode=this.Reader[4].ToString();
                        arryList.Add(icd); //填充数据
                        icd = null;
                    }
                    this.Reader.Close(); //关闭reade
                }
        ...
        #endregion
        #region 诊断查询比较慢，这里处理一下，暂时减少检索的字段
        /// <summary>
        /// 删除DeleteICDDrgs表
        /// </summary>
        /// <returns></returns>
        ...
        private ArrayList ICDReaderInfoDrgs()
        {
            ...
            try
            {
                ...
                while (this.Reader.Read())
                {
                    ...
                    icd.KeyCode = Reader[0].ToString(); //ICD主键
                    icd.ID = Reader[1].ToString();//ICD编码
                    icd.Name = Reader[2].ToString(); //ICD名称
                    icd.SpellCode = Reader[3].ToString();    //拼音码
                    icd.WBCode = Reader[4].ToString();//五笔码
                    list.Add(icd); //填充数据
                    icd = null;
                }
                this.Reader.Close(); //关闭reade
