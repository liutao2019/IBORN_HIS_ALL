...
namespace FS.HISFC.BizLogic.HealthRecord.Case
{
    ...
    ///  />
    /// </summary>
    public class CaseBill : FS.FrameWork.Management.Database
    {
        #region 变量
        ...
        private int ConvertEntityToArray( HISFC.Models.HealthRecord.Case.CaseBill caseBill )
        {
            try
            {
                ...
                {
                    caseBill.ID = this.GetSequence( "FS.HISFC.Management.HealthRecord.Case.CaseBill.GetSequence" );
                }
                field [0] = caseBill.ID;
                // 出库单号码，一个单据可以包括多个病历
        ...
        private int ConvertReaderToObject( ref HISFC.Models.HealthRecord.Case.CaseBill caseBill )
        {
            try
            {
                ...
                // 入库申请时间
                caseBill.InRequestOper.OperTime = FS.FrameWork.Function.NConvert.ToDateTime(this.Reader [8].ToString());
                // 入库申请分区编码
                ...
                // 出库审核时间
                caseBill.OutAuditingOper.OperTime = FS.FrameWork.Function.NConvert.ToDateTime(this.Reader[17].ToString());
                // 入库确认人工号

                caseBill.InConfirmOper.ID = this.Reader[18].ToString();
                caseBill.InConfirmOper.Name = this.Reader [19].ToString();
                // 入库确认时间
                caseBill.InConfirmOper.OperTime = FS.FrameWork.Function.NConvert.ToDateTime(this.Reader[20].ToString());
                // 单据类型、出库类型，对应常数表的CASE01
                ...
                caseBill.SendOper.OperTime = FS.FrameWork.Function.NConvert.ToDateTime(this.Reader[25].ToString());
                // 接收人工号

                caseBill.ReceiveOper.ID = this.Reader[26].ToString();
                caseBill.ReceiveOper.Name = this.Reader [27].ToString();
                // 接收时间
                caseBill.ReceiveOper.OperTime = FS.FrameWork.Function.NConvert.ToDateTime(this.Reader[28].ToString());
                // 是否已经接收：1－是、0－否
                ...
                caseBill.CaseInfo.Patient.Birthday = FS.FrameWork.Function.NConvert.ToDateTime(this.Reader [34].ToString());
                // 单据状态：0－入库申请、1－出库审核、2、入库确认

                switch ( this.Reader [35].ToString() )
                {
                    case "0":
                        caseBill.CaseBillState = FS.HISFC.Models.HealthRecord.Case.EnumCaseBillState.InRequest;
                        break;
                    case "1":
                ...
                caseBill.CaseInfo.InType = FS.FrameWork.Function.NConvert.ToInt32(this.Reader[43].ToString());
        ...
        private int ConvertReaderToArrayList( ref List<HISFC.Models.HealthRecord.Case.CaseBill> caseBillList )
        {
            try
            {
                ...
                {
                    HISFC.Models.HealthRecord.Case.CaseBill caseBill = new FS.HISFC.Models.HealthRecord.Case.CaseBill();
        ...
        private int Delete( string deleteCondition )
        {
            ...
            intReturn = this.Sql.GetSql( "FS.HISFC.Management.HealthRecord.Case.CaseBill.Delete", ref deleteSql );
            if ( this.intReturn < 0 )
            {
        ...
        private int Update( HISFC.Models.HealthRecord.Case.CaseBill caseBill, string updateCondition )
        {
            ...
            intReturn = this.Sql.GetSql( "FS.HISFC.Management.HealthRecord.Case.CaseBill.Update", ref updateSql );
            if ( this.intReturn < 0 )
            {
        ...
        private int Select( List<HISFC.Models.HealthRecord.Case.CaseBill> caseBillList, string selectCondition )
        {
            ...
            intReturn = this.Sql.GetSql( "FS.HISFC.Management.HealthRecord.Case.CaseBill.Select", ref selectSql );
            if ( this.intReturn < 0 )
            {
        ...
        public int Insert( HISFC.Models.HealthRecord.Case.CaseBill caseBill )
        {
            ...
            intReturn = this.Sql.GetSql( "FS.HISFC.Management.HealthRecord.Case.CaseBill.Insert", ref insertSql );
            if ( this.intReturn < 0 )
            {
        ...
        public int DeleteByID( string id )
        {
            ...
            intReturn = this.Sql.GetSql( "FS.HISFC.Management.HealthRecord.Case.CaseBill.ID", ref deleteSql );
            if ( this.intReturn < 0 )
            {
                this.Err = "删除病历出库单失败" + this.Err;
                return -1;
        ...
        public int UpdateByID( HISFC.Models.HealthRecord.Case.CaseBill caseBill)
        {
            ...
            intReturn = this.Sql.GetSql( "FS.HISFC.Management.HealthRecord.Case.CaseBill.ID", ref updateSql );
            if ( this.intReturn < 0 )
            {
                this.Err = "更新病历出库单失败" + this.Err;
                return -1;
        ...
        public int QueryByBillCode( ref List<HISFC.Models.HealthRecord.Case.CaseBill> caseBillList, string billCode )
        {
            ...
            intReturn = this.Sql.GetSql( "FS.HISFC.Management.HealthRecord.Case.CaseBill.SelectByBillCode", ref whereSql );
            if ( this.intReturn < 0 )
            {
            ...
            HISFC.Models.HealthRecord.Case.EnumCaseBillState caseBillState )
        {
            ...
            intReturn = this.Sql.GetSql( "FS.HISFC.Management.HealthRecord.Case.CaseBill.QueryAlreadyRequest", ref whereSql );
            if ( this.intReturn < 0 )
            {
            ...
            HISFC.Models.HealthRecord.Case.EnumCaseBillState caseBillState )
        {
            ...
            intReturn = this.Sql.GetSql( "FS.HISFC.Management.HealthRecord.Case.CaseBill.QueryWaitAuditing", ref whereSql );
            if ( this.intReturn < 0 )
            {
            ...
            HISFC.Models.HealthRecord.Case.EnumCaseBillState caseBillState )
        {
            ...
            intReturn = this.Sql.GetSql( "FS.HISFC.Management.HealthRecord.Case.CaseBill.QueryWaitConfirm", ref whereSql );
            if ( this.intReturn < 0 )
            {
        ...
        /// <param name="caseBillState">病历出库单状态</param>
        /// <returns>－1－失败，1－成功</returns>
        public int QueryAlreadyRequestTreeNode( ref List<FS.FrameWork.Models.NeuObject> caseBillList, string deptCode, DateTime fromDate,
            HISFC.Models.HealthRecord.Case.EnumCaseBillState caseBillState )
        {
            string whereSql = string.Empty;

            intReturn = this.Sql.GetSql( "FS.HISFC.Management.HealthRecord.Case.CaseBill.QueryAlreadyRequestTreeNode", ref whereSql );
            if ( this.intReturn < 0 )
            {
                this.Err = "查询已经申请入库的病历出库单失败" + this.Err;
                return -1;
            ...
            try
            {
                ...
                while ( this.Reader.Read() )
                {
                    FS.FrameWork.Models.NeuObject node = new FS.FrameWork.Models.NeuObject();
        ...
        /// <param name="caseBillState">病历出库单状态</param>
        /// <returns>－1－失败，1－成功</returns>
        public int QueryWaitAuditingTreeNode( ref List<FS.FrameWork.Models.NeuObject> caseBillList, string deptCode, DateTime fromDate,
            HISFC.Models.HealthRecord.Case.EnumCaseBillState caseBillState )
        {
            string whereSql = string.Empty;

            intReturn = this.Sql.GetSql( "FS.HISFC.Management.HealthRecord.Case.CaseBill.QueryWaitAuditingTreeNode", ref whereSql );
            if ( this.intReturn < 0 )
            {
        ...
        /// <param name="caseBillState">病历出库单状态</param>
        /// <returns>－1－失败，1－成功</returns>
        public int QueryWaitConfirmTreeNode( ref List<FS.FrameWork.Models.NeuObject> caseBillList, string deptCode, DateTime fromDate,
            HISFC.Models.HealthRecord.Case.EnumCaseBillState caseBillState )
        {
            string whereSql = string.Empty;

            intReturn = this.Sql.GetSql( "FS.HISFC.Management.HealthRecord.Case.CaseBill.QueryWaitConfirmTreeNode", ref whereSql );
            if ( this.intReturn < 0 )
            {
                this.Err = "查询等待入库确认的病历出库单失败" + this.Err;
                return -1;
        ...
        {
            return this.GetSequence( "FS.HISFC.Management.HealthRecord.Case.CaseBill.GetBillCode" );
        }
