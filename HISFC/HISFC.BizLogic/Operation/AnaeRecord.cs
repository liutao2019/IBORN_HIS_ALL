...
namespace FS.HISFC.BizLogic.Operation
{
	...
	public abstract class AnaeRecord : FS.FrameWork.Management.Database
	{
		...
		public FS.HISFC.Models.Operation.AnaeRecord GetAnaeRecord( string operatorNo )
		{
			...
			try
			{
				while(this.Reader.Read())
				{
					...
					anaeRecord.AnaeDate = FS.FrameWork.Function.NConvert.ToDateTime(Reader[7].ToString());//麻醉时间
					...
					try
					{
						anaeRecord.IsPACU = FS.FrameWork.Function.NConvert.ToBoolean(Reader[11].ToString());//是否入PACU,1是 0否 
					}
					catch{}
					anaeRecord.InPacuDate = FS.FrameWork.Function.NConvert.ToDateTime(Reader[12].ToString());//入(PACU)室时间
					anaeRecord.InPacuStatus.ID = Reader[13].ToString();//入(PACU)室状态
					anaeRecord.OutPacuDate = FS.FrameWork.Function.NConvert.ToDateTime(Reader[14].ToString());//出(PACU)室时间
					anaeRecord.OutPacuStatus.ID = Reader[15].ToString();//入(PACU)室状态
					anaeRecord.Memo = Reader[16].ToString();//备注
					anaeRecord.IsDemulcent = FS.FrameWork.Function.NConvert.ToBoolean(Reader[17].ToString());//术后镇痛，1是0否
					anaeRecord.DemulcentType.ID = Reader[18].ToString();//镇痛方式
					anaeRecord.DemulcentModel.ID = Reader[19].ToString();//泵型
					anaeRecord.DemulcentDays = FS.FrameWork.Function.NConvert.ToInt32(Reader[20].ToString());//镇痛天数
					anaeRecord.PullOutDate = FS.FrameWork.Function.NConvert.ToDateTime(Reader[21].ToString());//拔管时间
					anaeRecord.PullOutOperator.ID = Reader[22].ToString();//拔管人
					anaeRecord.DemulcentEffect.ID = Reader[23].ToString();//镇痛效果
					anaeRecord.IsCharged = FS.FrameWork.Function.NConvert.ToBoolean(Reader[24].ToString());//0未记帐/1已记帐
		...
		/// <returns>麻醉登记记录列表（元素为FS.HISFC.Models.Operation.AnaeRecord类型）</returns>
		public ArrayList GetAnaeRecords(string ExeDeptID,DateTime BeginDate,DateTime EndDate)
		{
			...
			try
			{
				while(this.Reader.Read())
				{
					...
					thisAnaeRec.AnaeDate = FS.FrameWork.Function.NConvert.ToDateTime(Reader[7].ToString());//麻醉时间
					//麻醉医师、麻醉助手的信息已经存在于thisAnaeRec.m_objOpsApp.RoleAl中
					//Reader[8] 麻醉医师
					//Reader[9] 麻醉助手
					...
					try
					{
						thisAnaeRec.IsPACU = FS.FrameWork.Function.NConvert.ToBoolean(Reader[11].ToString());//是否入PACU,1是 0否 
					}
					catch{}
					thisAnaeRec.InPacuDate = FS.FrameWork.Function.NConvert.ToDateTime(Reader[12].ToString());//入(PACU)室时间
					thisAnaeRec.InPacuStatus.ID = Reader[13].ToString();//入(PACU)室状态
					thisAnaeRec.OutPacuDate = FS.FrameWork.Function.NConvert.ToDateTime(Reader[14].ToString());//出(PACU)室时间
					thisAnaeRec.OutPacuStatus.ID = Reader[15].ToString();//入(PACU)室状态
					thisAnaeRec.Memo = Reader[16].ToString();//备注
					thisAnaeRec.IsDemulcent = FS.FrameWork.Function.NConvert.ToBoolean(Reader[17].ToString());//术后镇痛，1是0否
					thisAnaeRec.DemulcentType.ID = Reader[18].ToString();//镇痛方式
					thisAnaeRec.DemulcentModel.ID = Reader[19].ToString();//泵型
					thisAnaeRec.DemulcentDays = FS.FrameWork.Function.NConvert.ToInt32(Reader[20].ToString());//镇痛天数
					thisAnaeRec.PullOutDate = FS.FrameWork.Function.NConvert.ToDateTime(Reader[21].ToString());//拔管时间
					thisAnaeRec.PullOutOperator.ID = Reader[22].ToString();//拔管人
					thisAnaeRec.DemulcentEffect.ID = Reader[23].ToString();//镇痛效果
					thisAnaeRec.IsCharged = FS.FrameWork.Function.NConvert.ToBoolean(Reader[24].ToString());//0未记帐/1已记帐
