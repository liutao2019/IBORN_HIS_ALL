...
namespace FS.HISFC.BizLogic.PhysicalExamination.Table
{
	...
	public class Query : Base.BaseFunction,Base.TableInterface
	{
		...
		private void ReaderToObject(ref FS.HISFC.Models.PhysicalExamination.Management.Query query)
		{
		}
		...
		{
			// 转换成检索码实体
			FS.HISFC.Models.PhysicalExamination.Management.Query query;
			query = (FS.HISFC.Models.PhysicalExamination.Management.Query) record;
			...
			// 获取SQL语句
			if (this.Sql.GetSql("FS.HISFC.BizLogic.PhysicalExamination.Table.Query.Insert", ref this.SQL) == -1)
			{
				return -1;
			}
		...
		public int Update(NeuObject record)
		{
			...
			// 获取SQL语句
			if (this.Sql.GetSql("FS.HISFC.BizLogic.PhysicalExamination.Table.Query.Update", ref this.SQL) == -1)
			{
				return -1;
			}
		...
		public void FillFields(NeuObject record)
		{
			...
			// 填充数组
			this.fields[(int)EnumQuery.Hospital] = this.GetSequence("FS.HISFC.BizLogic.PhysicalExamination.Table.GetHospital");
			...
			if (query.Validity.Equals(FS.HISFC.Models.PhysicalExamination.Enum.EnumValidity.Valid))
			{
