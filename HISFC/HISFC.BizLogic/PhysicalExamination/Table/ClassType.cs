...
namespace FS.HISFC.BizLogic.PhysicalExamination.Table
{
	...
	///  />
	/// </summary>
	public class ClassType : FS.HISFC.BizLogic.PhysicalExamination.Base.BaseFunction, FS.HISFC.BizLogic.PhysicalExamination.Base.TableInterface
	{
		#region 私有变量
		...
		private void ReaderToObject(ref FS.HISFC.Models.PhysicalExamination.Management.ClassType classType)
		{
			...
			classType.CreateEnvironment.OperTime = FS.FrameWork.Function.NConvert.ToDateTime(this.Reader[7].ToString());
			classType.InvalidEnvironment.ID = this.Reader[8].ToString();
			classType.InvalidEnvironment.Name = this.Reader[9].ToString();
			classType.InvalidEnvironment.OperTime = FS.FrameWork.Function.NConvert.ToDateTime(this.Reader[10].ToString());
			if (this.Reader[11].ToString().Equals("1"))
			{
				classType.Validity = FS.HISFC.Models.PhysicalExamination.Enum.EnumValidity.Valid;
			}
			else
			{
				classType.Validity = FS.HISFC.Models.PhysicalExamination.Enum.EnumValidity.Invalid;
			}
		...
		{
			// 转换成类别类
			FS.HISFC.Models.PhysicalExamination.Management.ClassType classType = (FS.HISFC.Models.PhysicalExamination.Management.ClassType)record;
			...
			// 获取SQL语句
			if (this.Sql.GetSql("FS.HISFC.BizLogic.PhysicalExamination.Table.ClassType.Insert", ref this.SQL) == -1)
			{
				return -1;
			}
		...
		public int Update(NeuObject record)
		{
			...
			// 获取SQL语句
			if (this.Sql.GetSql("FS.HISFC.BizLogic.PhysicalExamination.Table.ClassType.Update", ref this.SQL) == -1)
			{
				return -1;
			}
		...
		public int Select(ref ArrayList recordList, string whereCondition)
		{
			...
			// 获取SQL语句
			if (this.Sql.GetSql("FS.HISFC.BizLogic.PhysicalExamination.Table.ClassType.Select", ref this.SQL) == -1)
			{
				return -1;
			}
		...
		public void FillFields(NeuObject record)
		{
			...
			this.fields[(int)Table.Enum.EnumClassType.Hospital] = this.GetSequence("FS.HISFC.BizLogic.PhysicalExamination.Table.GetHospital");
			;
			...
			if (classType.Validity.Equals(FS.HISFC.Models.PhysicalExamination.Enum.EnumValidity.Valid))
			{
		...
		{
			// 类别类
			FS.HISFC.Models.PhysicalExamination.Management.ClassType classType;

			// 循环添加数组
			while (this.Reader.Read())
			{
				classType = new FS.HISFC.Models.PhysicalExamination.Management.ClassType();
