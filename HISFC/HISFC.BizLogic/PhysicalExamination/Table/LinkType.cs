...
using FS.HISFC.Models.PhysicalExamination.Collective;
...
namespace FS.HISFC.BizLogic.PhysicalExamination.Table
{
	...
	public class LinkType : Base.BaseFunction, Base.TableInterface
	{
		...
		private void ReaderToObject(ref FS.HISFC.Models.PhysicalExamination.Collective.Relation relation)
		{
			...
			{
				relation.PEType = FS.HISFC.Models.PhysicalExamination.Enum.EnumPEType.Collective;
			}
			else
			{
				relation.PEType = FS.HISFC.Models.PhysicalExamination.Enum.EnumPEType.Personal;
			}
			// 联系方式的类别
			...
			relation.InvalidEnvironment.OperTime = FS.FrameWork.Function.NConvert.ToDateTime(this.Reader[12].ToString());
		...
		{
			// 转换成健康档案类
			FS.HISFC.Models.PhysicalExamination.Collective.Relation relation = new Relation();
			relation = (FS.HISFC.Models.PhysicalExamination.Collective.Relation)record;
			...
			// 获取SQL语句
			if (this.Sql.GetSql("FS.HISFC.BizLogic.PhysicalExamination.Table.LinkType.Insert", ref this.SQL) == -1)
			{
				return -1;
			}
		...
		public int Update(NeuObject record)
		{
			...
			// 获取SQL语句
			if (this.Sql.GetSql("FS.HISFC.BizLogic.PhysicalExamination.Table.LinkType.Update", ref this.SQL) == -1)
			{
				return -1;
			}
		...
		public int Select(ref ArrayList recordList, string whereCondition)
		{
			...
			// 获取SQL语句
			if (this.Sql.GetSql("FS.HISFC.BizLogic.PhysicalExamination.Table.LinkType.Select", ref this.SQL) == -1)
			{
				return -1;
			}
		...
		public void FillFields(NeuObject record)
		{
			...
			this.fields[(int) EnumRelation.Hospital] = this.GetSequence("FS.HISFC.BizLogic.PhysicalExamination.Table.GetHospital");
			this.fields[(int) EnumRelation.RelationID] = relation.ID;
			this.fields[(int) EnumRelation.ObjectID] = relation.Code;
			if (relation.PEType.Equals(FS.HISFC.Models.PhysicalExamination.Enum.EnumPEType.Collective))
			{
		...
		{
			// 健康档案类
			FS.HISFC.Models.PhysicalExamination.Collective.Relation relation;
