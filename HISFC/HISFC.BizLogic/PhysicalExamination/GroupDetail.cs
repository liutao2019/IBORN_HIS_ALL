        ...
        private ArrayList QueryGroupDetail(string strSql)
        {
            ...
            try
            {
                ...
                while (this.Reader.Read())
                {
                    ...
                    info.SortNum = FS.FrameWork.Function.NConvert.ToInt32(Reader[1]);// 序号 
                    info.itemCode = Reader[2].ToString(); //编码  
                    info.Spacs = Reader[3].ToString(); //规格 
                    info.qty = FS.FrameWork.Function.NConvert.ToDecimal(Reader[4]); //开立数量
                    ...
                    info.ChkTime = FS.FrameWork.Function.NConvert.ToInt32(Reader[9]);//检测次数
                    info.operCode = Reader[10].ToString();//操作员
                    info.OperDate = FS.FrameWork.Function.NConvert.ToDateTime(Reader[11]);//操作日期
