...
namespace FoShanDiseasePay
{
    ...
    /// 佛山市【医用耗材】阳光采购平台接口
    /// </summary>
    public partial class ucUploadMaterialSun : Neusoft.FrameWork.WinForms.Controls.ucBaseControl
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        public ucUploadMaterialSun()
        {
            InitializeComponent();
        }
        ...
        /// 耗材信息上传
        /// </summary>
        private MaterialJob matJob = new MaterialJob();
        ...
        /// 医用耗材业务管理类
        /// </summary>
        private BizLogic.MaterialManager matMgr = new FoShanDiseasePay.BizLogic.MaterialManager();
        ...
        {
            //当前时间
            DateTime dtNow = this.matMgr.GetDateTimeFromSysDateTime();
        ...
        private void QueryMatInfo()
        {
            ...
            ArrayList alMat = this.matMgr.QueryMaterialBaseInfoByALL(queryCode, upType);
            if (alMat != null && alMat.Count > 0)
            {
                this.AddNeedUpload(alMat);
            }
        ...
        /// 刷新耗材信息
        /// </summary>
        private void QueryNeedUploadMat()
        {
            //清屏
            this.fpNeedUpload_Sheet1.Rows.Count = 0;
            this.cbChooseAll.Checked = true;
            string queryCode = "";// {385E49F2-947B-43e3-931D-BE89481BA68C}
            queryCode = this.txtQueryCode.Text.Trim();
            ...
            //待上传的医用耗材// {385E49F2-947B-43e3-931D-BE89481BA68C}
            ArrayList alMat = this.matMgr.QueryMaterialBaseInfoByQuery(queryCode, dtBeginTime.Value.Date, this.dtEndTime.Value.AddDays(1).Date);
            if (alMat != null && alMat.Count > 0)
            {
                this.AddNeedUpload(alMat);
            }
        ...
        private void AddNeedUpload(ArrayList alMat)
        {
            if (alMat != null && alMat.Count > 0)
            {
                int rowIndex = this.fpNeedUpload_Sheet1.Rows.Count;
                foreach (Neusoft.HISFC.BizLogic.Material.Object.MatBase matBase in alMat)
                {
                    ...
                    this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColDrug.ColMatId].Value = matBase.ID;
                    this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColDrug.ColMatName].Value = matBase.Name;
                    this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColDrug.ColUserCode].Value = matBase.UserCode;
                    this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColDrug.ColGBCode].Value = matBase.GBCode;
                    this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColDrug.ColSpecs].Value = matBase.Specs;
                    this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColDrug.ColPrice].Value = matBase.Price;
                    this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColDrug.ColRegNo].Value = matBase.RegisterNo;
                    this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColDrug.ColCompany].Value = matBase.Factory.Name;
                    this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColDrug.ColGHS].Value = matBase.Company.Name;
                    this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColDrug.ColTotSum].Value = matBase.Memo;
                    this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColDrug.ColInvoice].Value = matBase.User01;//发票号
                    this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColDrug.ColPlatCode].Value = matBase.PlatMatCode;
                    this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColDrug.ColSunCode].Value = matBase.SunMatCode;
                    if (matBase.User02 == "9999")
                    {
                    ...
                    else if (matBase.User02 == "0")
                    {
                        type = "未上传";
                    }
                    this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColDrug.ColUpType].Value = type;//上传标志
                    
                    //实体
                    this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColDrug.ColMatId].Tag = matBase;
        ...
        /// 一键上传患者所有医用耗材的信息
        /// </summary>
        private void AllUploadMatDetail()
        {
            if (this.fpNeedUpload_Sheet1.Rows.Count <= 0)
            {
        ...
        /// 已上传的药品编码--避免一次上传多条数据
        /// </summary>
        private Hashtable uploadMatBaseInfoHS = new Hashtable();
        /// <summary>
        /// 是否有数据上传
        /// </summary>
        ...
        private void UploadMatIfo(string upload)
        {
            if (this.fpNeedUpload_Sheet1.Rows.Count <= 0)
            {
                MessageBox.Show("无需要上传信息!");
            ...
            uploadMatBaseInfoHS = new Hashtable();
            for (int k = 0; k < this.fpNeedUpload_Sheet1.Rows.Count; k++)
            {
                Neusoft.FrameWork.WinForms.Classes.Function.ShowWaitForm(k, this.fpNeedUpload_Sheet1.Rows.Count);
                ...
                if (isChoose)
                {
                    ...
                    matBase = this.fpNeedUpload_Sheet1.Cells[k, (int)ColDrug.ColMatId].Tag as Neusoft.HISFC.BizLogic.Material.Object.MatBase;

                    if (matBase != null && !string.IsNullOrEmpty(matBase.ID))
                    {
                        if (!uploadMatBaseInfoHS.Contains(matBase.ID))//已经上传的药品则不重复上传
                        {
                            uploadMatBaseInfoHS.Add(matBase.ID, matBase);
                        }
                        else
                        {
                            continue;
                        ...
                        switch (upload)
                        {
                            case "上传耗材基本信息":
                                ...
                                this.UploadMatBaseInfo(matBase);
                                break;

                            case "上传耗材库存信息":
                                this.UploadMatStoreInfo(matBase);
                                break;

                            case "上传耗材使用详情":
                                this.UploadMatUseInfo(matBase);
                                break;

                            case "上传耗材采购订单":
                                this.UploadMatPurchaseOrder(matBase);
                                break;

                            case "上传耗材退货信息":
                                this.UploadMatReturnInfo(matBase);
                                break;

                            case "上传耗材采购订单发票":
                                this.UploadMatInvoiceInfo(matBase);
                                break;

                            case "上传耗材采购订单发票附件":
                                this.UploadMatInvoiceInfoFile(matBase);
                                break;

                            case "作废基本信息":
                                this.CancelBaseInfo(matBase);
                                break;
                        }
                    }
                }
        ...
        private int CancelBaseInfo(Neusoft.HISFC.BizLogic.Material.Object.MatBase matBase)
        {
            if (matBase == null || string.IsNullOrEmpty(matBase.ID))
            {
                MessageBox.Show("请选作废的耗材!");
                return -1;
            }

            matBase = this.matMgr.QueryMaterialBaseInfo(matBase.ID);
            if (matBase == null || string.IsNullOrEmpty(matBase.ID))
            {
                MessageBox.Show("查询耗材【" + matBase.ID + "-" + matBase.Name + "】基本信息出错!");
                return -1;
            }
            
            string error = string.Empty;
            int result = this.matJob.DeleteBaseInfo(matBase, ref error);
            if (result < 0)
            {
                MessageBox.Show("作废耗材信息出错【" + matBase.Name + "】\r\n" + error);
                return -1;
            }
            else
            {
                this.matMgr.UpdateMatUploadFlat(matBase.ID, "0");
                this.matMgr.UpdatBaseInfoFlat(matBase.ID, "0");
                return 1;
            }
        }

        #region 佛山市【医用耗材】阳光采购平台接口
        ...
        /// <param name="matBase"></param>
        /// <returns></returns>
        private int UploadMatBaseInfo(Neusoft.HISFC.BizLogic.Material.Object.MatBase matBase)
        {
            if (matBase == null || string.IsNullOrEmpty(matBase.ID))
            {
                MessageBox.Show("请选择需要上传的耗材!");
                return -1;
            }
            ...
            string error = string.Empty;
            int result = this.matJob.UploadMatBaseInfo(matBase, ref error);
            if (error.Contains("重复提交") || error.Contains("已经匹配通过"))
            {
                result = 0;
            }
            if (result < 0)
            {
                MessageBox.Show("上报医疗机构耗材信息出错【" + matBase.Name + "】\r\n" + error);
                return -1;
            }
            else
            {
                //MessageBox.Show("上报医疗机构耗材信息成功【" + matBase.Name + "】");
                isHaveUpload = true;
                this.matMgr.UpdateMatUploadFlat(matBase.ID, "1");
                this.matMgr.UpdatBaseInfoFlat(matBase.ID, "1");
                 return 1;
            }
        }
        ...
        /// <param name="matBase"></param>
        /// <returns></returns>
        private int UploadMatStoreInfo(Neusoft.HISFC.BizLogic.Material.Object.MatBase matBase)
        {
            if (matBase == null || string.IsNullOrEmpty(matBase.ID))
            {
                MessageBox.Show("请选择需要上传的耗材!");
            ...
            string error = string.Empty;
            int result = this.matJob.UploadMatStoreInfo(alMat, ref error);
            ...
            if (result < 0)
            {
                MessageBox.Show("上报医疗机构耗材库存信息出错【" + matBase.Name + "】\r\n" + error);
                return -1;
            }
            else
            {
                //MessageBox.Show("上报医疗机构耗材库存信息成功【" + matBase.Name + "】");
                isHaveUpload = true;
                return 1;
            }
        }
        ...
        /// <param name="matBase"></param>
        /// <returns></returns>
        private int UploadMatUseInfo(Neusoft.HISFC.BizLogic.Material.Object.MatBase matBase)
        {
            if (matBase == null || string.IsNullOrEmpty(matBase.ID))
            {
                MessageBox.Show("请选择需要上传的耗材!");
            ...
            ArrayList alOutPut = this.matMgr.QueryMaterialUseDetail(matBase.ID,dtBeginTime.Value.Date, this.dtEndTime.Value.AddDays(1).Date);
            ...
            string error = string.Empty;
            int result = 0;
            foreach (Neusoft.HISFC.BizLogic.Material.Object.Output output in alOutPut)
            {
                result = this.matJob.UploadMatUseInfo(output, ref error);
                ...
                if (result < 0)
                {
                    MessageBox.Show("上报医疗机构耗材使用详情出错【" + matBase.Name + "】\r\n" + error);
                    return -1;
                }
            }

            //MessageBox.Show("上报医疗机构耗材使用详情成功【" + matBase.Name + "】");
            isHaveUpload = true;
            return 1;
        }
        ...
        /// <param name="matBase"></param>
        /// <returns></returns>
        private int UploadMatPurchaseOrder(Neusoft.HISFC.BizLogic.Material.Object.MatBase matBase)
        {
            if (matBase == null || string.IsNullOrEmpty(matBase.ID))
            {
                MessageBox.Show("请选择需要上传的耗材!");
            ...
            DateTime dtNow = this.matMgr.GetDateTimeFromSysDateTime();
            ArrayList alInput = this.matMgr.QueryMaterialInputList(dtBeginTime.Value.Date, this.dtEndTime.Value.AddDays(1).Date, matBase.ID);
            ...
            string error = string.Empty;
            int result = this.matJob.UploadMatOrderInfo(alInput, ref error);
            ...
            if (result < 0)
            {
                MessageBox.Show("上报医疗机构耗材采购订单出错【" + matBase.Name + "】\r\n" + error);
                return -1;
            }
            else
            {
                isHaveUpload = true;
                //MessageBox.Show("上报医疗机构耗材采购订单成功【" + matBase.Name + "】");
                return 1;
            }
        }
        ...
        /// <param name="matBase"></param>
        /// <returns></returns>
        private int UploadMatReturnInfo(Neusoft.HISFC.BizLogic.Material.Object.MatBase matBase)
        {
            if (matBase == null || string.IsNullOrEmpty(matBase.ID))
            {
                MessageBox.Show("请选择需要上传的耗材!");
            ...
            DateTime dtNow = this.matMgr.GetDateTimeFromSysDateTime();
            ArrayList alReturnList = this.matMgr.QueryMaterialReturnList(dtBeginTime.Value.Date, this.dtEndTime.Value.AddDays(1).Date, matBase.ID);
            ...
            string error = string.Empty;
            int result = this.matJob.UploadMatReturnInfo(alReturnList, ref error);
            ...
            if (result < 0)
            {
                MessageBox.Show("上报医疗机构耗材退货信息出错【" + matBase.Name + "】\r\n" + error);
                return -1;
            ...
            {
                isHaveUpload = true;
                //MessageBox.Show("上报医疗机构耗材退货信息成功【" + matBase.Name + "】");
                return 1;
            }
        }

        /// <summary>
        /// 上报医疗机构耗材采购订单发票(HH006)
        /// </summary>
        /// <param name="matBase"></param>
        /// <returns></returns>
        private int UploadMatInvoiceInfo(Neusoft.HISFC.BizLogic.Material.Object.MatBase matBase)
        {
            if (matBase == null || string.IsNullOrEmpty(matBase.ID))
            {
                MessageBox.Show("请选择需要上传的耗材!");
            ...
            DateTime dtNow = this.matMgr.GetDateTimeFromSysDateTime();
            ArrayList alInvoiceList = this.matMgr.QueryMaterialInvoiceInfo(dtBeginTime.Value.Date, this.dtEndTime.Value.AddDays(1).Date, matBase.ID);
            ...
            string error = string.Empty;
            int result = this.matJob.UploadMatInvoiceInfo(alInvoiceList, ref error);
            ...
            if (result < 0)
            {
                MessageBox.Show("上报医疗机构耗材采购订单发票出错【" + matBase.Name + "】\r\n" + error);
                return -1;
            }
            else
            {
                isHaveUpload = true;
                //MessageBox.Show("上报医疗机构耗材采购订单发票成功【" + matBase.Name + "】");
                return 1;
            }
        }
        ...
        /// <returns></returns>
        private int UploadMatInvoiceInfoFile(Neusoft.HISFC.BizLogic.Material.Object.MatBase matBase)
        {
            if (matBase == null || string.IsNullOrEmpty(matBase.ID))
            {
                MessageBox.Show("请选择需要上传的耗材!");
            ...
            Neusoft.HISFC.BizLogic.Material.Object.Input input = new Neusoft.HISFC.BizLogic.Material.Object.Input();
            ...
            input.User01 = fileName;
            input.Memo = imageBase64String;

            string error = string.Empty;
            int result = this.matJob.UploadMatInvoiceFileInfo(input, ref error);
        ...
        protected override Neusoft.FrameWork.WinForms.Forms.ToolBarService OnInit(object sender, object neuObject, object param)
        {
            ...
            this.toolBarService.AddToolButton("耗材基本信息查询", "耗材基本信息查询", (int)Neusoft.FrameWork.WinForms.Classes.EnumImageList.C查询, true, false, null);
            this.toolBarService.AddToolButton("耗材入库记录查询", "耗材入库记录查询", (int)Neusoft.FrameWork.WinForms.Classes.EnumImageList.C查询, true, false, null);
            
            this.toolBarService.AddToolButton("一键上传", "一键上传医用耗材所有信息", (int)Neusoft.FrameWork.WinForms.Classes.EnumImageList.X新建, true, false, null);


            this.toolBarService.AddToolButton("上传耗材基本信息", "上传耗材基本信息", (int)Neusoft.FrameWork.WinForms.Classes.EnumImageList.S手动录入, true, false, null);

            this.toolBarService.AddToolButton("上传耗材库存信息", "上传耗材库存信息", (int)Neusoft.FrameWork.WinForms.Classes.EnumImageList.J接诊, true, false, null);

            this.toolBarService.AddToolButton("上传耗材使用详情", "上传耗材使用详情", (int)Neusoft.FrameWork.WinForms.Classes.EnumImageList.J借出, true, false, null);

            this.toolBarService.AddToolButton("上传耗材采购订单", "上传耗材采购订单", (int)Neusoft.FrameWork.WinForms.Classes.EnumImageList.C出库单, true, false, null);

            this.toolBarService.AddToolButton("上传耗材退货信息", "上传耗材退货信息", (int)Neusoft.FrameWork.WinForms.Classes.EnumImageList.T退费, true, false, null);

            this.toolBarService.AddToolButton("上传耗材采购订单发票", "上传耗材采购订单发票", (int)Neusoft.FrameWork.WinForms.Classes.EnumImageList.C采购单 , true, false, null);

            this.toolBarService.AddToolButton("上传耗材采购订单发票附件", "上传耗材采购订单发票附件", (int)Neusoft.FrameWork.WinForms.Classes.EnumImageList.D导入, true, false, null);
        ...
        public override void ToolStrip_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            switch (e.ClickedItem.Text)
            {
                case "耗材基本信息查询":
                    ...
                    break;
                case "耗材入库记录查询":
                    this.QueryNeedUploadMat();
                    break;

                case "一键上传":
                    this.AllUploadMatDetail();
                    break;

                case "上传耗材基本信息":
                    this.UploadMatIfo("上传耗材基本信息");
                    break;

                case "上传耗材库存信息":
                    this.UploadMatIfo("上传耗材库存信息");
                    break;

                case "上传耗材使用详情":
                    this.UploadMatIfo("上传耗材使用详情");
                    break;

                case "上传耗材采购订单":
                    this.UploadMatIfo("上传耗材采购订单");
                    break;

                case "上传耗材退货信息":
                    this.UploadMatIfo("上传耗材退货信息");
                    break;

                case "上传耗材采购订单发票":
                    this.UploadMatIfo("上传耗材采购订单发票");
                    break;

                case "上传耗材采购订单发票附件":
                    this.UploadMatIfo("上传耗材采购订单发票附件");
                    break;
