...
namespace FoShanDiseasePay.Jobs
{
    ...
    public class BaseJob
    {
        ...
        {
            foreach (FS.FrameWork.Models.NeuObject obj in Manager.setObj.ContentList)
            {
        ...
        public int UploadInfo(string funCode, string cdata, ref string error)
        {
            try
            {
                ...
                //Object obj = WebServiceHelper.InvokeWebService("http://200.10.20.34:8000/fshrssHisApi/openApi/sendReveForHospital", "SendReveForHospital", args, ref error);
        ...
        /// <param name="isReturn">是否需要回参</param>
        /// <param name="error"></param>
        /// <returns></returns>
        public int UploadInfoNew(string funCode, StringBuilder cdata, bool isReturn, ref string error)
        {
            try
            {
                StringBuilder sb = new StringBuilder();
                ...
                sb.Append("\"hApiCode\":\"").Append(funCode).Append("\",");
                sb.Append("\"hPlatCode\":\"hApi\",");
                sb.Append("\"hUserCode\":\"").Append(Manager.setObj.NewUserID).Append("\",");
                sb.Append("\"hUserOrgCode\":\"").Append(Manager.setObj.NewOrgCode).Append("\",");
                sb.Append("\"hUserIP\":\"").Append(Manager.setObj.IPAddress).Append("\",");
                if (funCode == "HD002" || funCode == "HH002")
                {
                    sb.Append("\"hData\":").Append(cdata.ToString().Replace("'", "\"")).Append(",");
                }
                else
                {
                    sb.Append("\"hData\":").Append("{").Append(cdata.ToString().Replace("'", "\"")).Append("},");
                }
                sb.Append("\"hSignData\":\"").Append(Manager.setObj.NewSign).Append("\"}");
                ...
                //2、调用医院前置机服务
                string[] args = { json };
                Object obj = WebServiceHelper.InvokeWebService(Manager.setObj.NewWebServiceAddress, "SendDRGS", args, ref error);
