...
namespace FoShanDiseasePay
{
    ...
    public partial class ucMain : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        public ucMain()
        {
            ...
            //al.Add(obj);
            //obj = new FS.FrameWork.Models.NeuObject("C", "门诊", "");
            //al.Add(obj);
            obj = new FS.FrameWork.Models.NeuObject("I", "住院", "");
            al.Add(obj);
        ...
        private void QueryNeedUploadDetail()
        {
            ...
            string patientNo = this.lblPatientNo.Text.Trim();
            ...
            //{
            //    //待上传的门诊患者
            //    DataTable dtOutpatient = this.outMgr.QueryNeedUploadDetail(strBeginTime, strEndTime, patientNo);
            ...
            {
                //待上传的住院患者
                DataTable dtInpatient = this.inMgr.QueryNeedUploadDetail(strBeginTime, strEndTime, patientNo);
        ...
        private void AddNeedUpload(DataTable dtPatient)
        {
            if (dtPatient != null && dtPatient.Rows.Count > 0)
            {
                ...
                foreach (DataRow dRow in dtPatient.Rows)
                {
                    ...
                    string typeName = "";
                    if (string.IsNullOrEmpty(dRow[(int)ColPatient.ColUploadType].ToString()))
                    {
                        typeName = "都未上传";
                    }
                    else if (dRow[(int)ColPatient.ColUploadType].ToString() == "1")
                    {
                        typeName = "就诊明细已上传";
                    }
                    else if (dRow[(int)ColPatient.ColUploadType].ToString() == "2")
                    {
                        typeName = "病案首页信息已上传";
                    }
                    else
                    ...
                    this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColPatient.ColUploadType].Value = typeName;
                    this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColPatient.ColErr].Value = dRow[(int)ColPatient.ColErr].ToString();
        ...
        private void AllUploadDetail()
        {
                ...
                if (isChoose)
                {
                    ...
                    if (patientType == "1")
                    {
                        ...
                        if (rev <= 0)
                        {
                            ...
                            rev = this.inMgr.UpdateLog(patientType, patientID, invoiceNO, "0");
                            continue;
                        }
                        ...
                        rev = this.inMgr.UpdateLog(patientType, patientID, invoiceNO, "3");
                        if (rev <= 0)
                        {
                    ...
                    else
                    {
                        ...
                        if (rev <= 0)
                        {
                            rev = this.inMgr.UpdateLog(patientType, patientID, invoiceNO, "0");
                            isTrue = false;
                            if (MessageBox.Show("就诊信息上传失败！是否继续上传病案信息?", "疑问", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.No)
                            {
                                continue;
                            }
                            //continue;
                        ...
                        else
                        {
                            ...
                            if (this.inMgr.UpdateLog(patientType, patientID, invoiceNO, "1") <= 0)
                            {
                                rev = this.inMgr.SaveLog(patientType, invoiceNO, patientNo, patientID, idenNo, sfdjh, wylsh, this.inMgr.Operator.ID, "1");
                                if (rev == -1)
                                {
                                    MessageBox.Show(this.inMgr.Err, "错误");
                                    continue;
                        ...
                        else
                        {
                            if (!isTrue)
                            {
                                if (this.inMgr.UpdateLog(patientType, patientID, invoiceNO, "2") <= 0)
                                {

                                    rev = this.inMgr.SaveLog(patientType, invoiceNO, patientNo, patientID, idenNo, sfdjh, wylsh, this.inMgr.Operator.ID, "2");
                                    if (rev == -1)
                                    {
                                        MessageBox.Show(this.inMgr.Err, "错误");
                                        continue;
            ...
            {
                if (FS.FrameWork.Management.Connection.Hospital.Name == "佛山市中医院禅城高新区医院")
                {
                    MessageBox.Show("还有数据未上传!请处理完后继续上传");
                }
        ...
        /// 就诊明细上传
        /// </summary>
        private void AllUploadMainInfo()
        {
            if (this.fpNeedUpload_Sheet1.Rows.Count <= 0)
            {
                MessageBox.Show("无需要上传信息!");
            ...
            for (int k = 0; k < this.fpNeedUpload_Sheet1.Rows.Count; k++)
            {
                ...
                if (isChoose)
                {
                    ...
                    if (patientType == "1")
                    {
                        ...
                        //1、就诊单据信息上传(HJ001)
                        //rev = this.UploadMainInfo(patientID, patientType, invoiceNO, transType, ref sfdjh, ref wylsh, ref patientNo, ref idenNo, ref errInfo);
                        rev = this.UploadDetailByPatient(patientID, patientType, invoiceNO, transType, ref errInfo);
                        if (rev <= 0)
                        {
                            continue;
                        }
                    ...
                    else
                    {
                        ...
                        string wylsh = string.Empty;
                        //住院号
                        string patientNo = this.fpNeedUpload_Sheet1.Cells[k, (int)ColPatient.ColPatientNO].Value.ToString();
                        //身份证号
                        string idenNo = this.fpNeedUpload_Sheet1.Cells[k, (int)ColPatient.ColIdNO].Value.ToString();
        ...
        /// 病案首页上传
        /// </summary>
        private void AllUploadPatientCase()
        {
            if (this.fpNeedUpload_Sheet1.Rows.Count <= 0)
            {
                MessageBox.Show("无需要上传信息!");
            ...
            //提示
            string strTips = "是否确定上传病案首页信息，上传前请确定就诊明细是否已经全部上传成功！";
            if (MessageBox.Show(strTips, "警告", MessageBoxButtons.YesNo, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1) == DialogResult.No)
            {
                return;
            }
            ...
            else
            {
                if (FS.FrameWork.Management.Connection.Hospital.Name == "佛山市中医院禅城高新区医院")
                {
                    MessageBox.Show("全部操作上传完成，请确定列表内还未上传的数据！");
                }
                else
                {
        ...
        /// <returns></returns>
        public int UploadDetailByPatient(string patientID, string patientType, string invoiceNo, string transType, ref string errInfo)
        {
            
            if (string.IsNullOrEmpty(patientID) ||  string.IsNullOrEmpty(invoiceNo))
            {
                errInfo = "获取患者基本信息为空！";
                return -1;
            }
            if ( string.IsNullOrEmpty(patientType) || string.IsNullOrEmpty(invoiceNo) || string.IsNullOrEmpty(transType))
            {
                errInfo = "传入参数有为空！";
                return -1;
            }
            ...
            int rev = UploadMainInfo(patientID, patientType, invoiceNo, transType, ref ybMedNo, ref pkNo, ref patientNo, ref idenNo, ref errInfo);
            if (rev < 0)
            {
                rev = this.inMgr.UpdateLog(patientType, patientID, invoiceNo, "0");
                return -1;
            ...
            rev = this.inMgr.UpdateLog(patientType, patientID, invoiceNo, Uptype);
            if (rev <= 0)
            {
                rev = this.inMgr.SaveLog(patientType, invoiceNo, patientNo, patientID, idenNo, ybMedNo, pkNo, this.inMgr.Operator.ID, Uptype);
           
            }
            if (rev == -1)
            {
                errInfo = "上传就诊明细成功，但保存上传日志失败！" + this.inMgr.Err;
                return -1;
            }
            else
            {
        ...
        private int UploadMainInfo(string patientID, string patientType, string invoiceNO, string transType, ref string ybMedNo, ref string pkNo, ref string patientNo, ref string idenNo, ref string errInfo)
        {
            ...
            if (patientType == "1")
            {
                ...
                if (siTotCost <= 0)
                {
                    this.outMgr.UpdateSiInvoice(patientID, invoiceNO);
                }
                ...
                {
                    FS.HISFC.Models.Fee.Outpatient.FeeItemList fTemp = alFee[k] as FS.HISFC.Models.Fee.Outpatient.FeeItemList;
                ...
                foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList fTemp in alFee)
                {
                    ...
                    detailTotCost += fTemp.FT.OwnCost + fTemp.FT.PubCost + fTemp.FT.PayCost;
                    if (this.specialItemCode != fTemp.Item.ID)
                    {
                        alTemp.Add(fTemp);
                    }
                }
                if (string.IsNullOrEmpty(firstDiag) && !isHaveDrug && 
                    (FS.FrameWork.Management.Connection.Hospital.Name == "佛山市第一人民医院禅城医院"
                    || FS.FrameWork.Management.Connection.Hospital.Name == "佛山市中医院禅城高新区医院"))
                {
                    firstDiag = "Z00.000";
                }
                alFee = alTemp;   //重新赋值
                ...
                //MessageBox.Show("siTotCost:" + siTotCost.ToString("F2") + "  detailTotCost:" + detailTotCost.ToString("F2"));
                //MessageBox.Show("siTotCost:" + siTotCost.ToString("F2") + " detailTotCost:" + detailTotCost.ToString("F2"));
                if (siTotCost.ToString("F2") != detailTotCost.ToString("F2"))//如果明细的总金额和报销的总金额相等，则不需要再增加诊查费，以防上传的金额和主单的不一致
                {
                    ...
                    //if (!((FS.FrameWork.Management.Connection.Hospital.Name == "佛山市第三人民医院" && (outBalance.Patient.Pact.ID == "21"))//佛三特定门诊没有报销门诊收费处收取的费用，导致上传的时候与实际报销的总金额不符
                    //    //|| FS.FrameWork.Management.Connection.Hospital.Name == "佛山市第一人民医院禅城医院" 
                    //    )
                    //    )
                    //{
                    if (true)
                    {
                        lstAccFee = new List<FS.HISFC.Models.Account.AccountCardFee>();
                    ...
                    foreach (FS.HISFC.Models.Account.AccountCardFee cardFee in lstAccFee)
                    {
                        ...
                        {
                            FS.HISFC.Models.Fee.Outpatient.FeeItemList cfItem = new FS.HISFC.Models.Fee.Outpatient.FeeItemList();
                ...
                if (feeItem != null)
                {
                    ...
                    for (int k = 0; k < alFee.Count; k++)
                    {
                        ...
                        if (this.itemCodeCompareType == "0")
                        {
                            ...
                            if (string.IsNullOrEmpty(feeItem.Item.GBCode) || feeItem.Item.GBCode == "0")
                            {
                                if (this.DealFeeItemList(feeItem, ref errMsg) == -1)
                                {
                                    ...
                                    this.InsertErrLog("1", patientID, invoiceNO, ("项目【" + feeItem.Item.Name + "(" + feeItem.Item.ID + ")" + "】1" + errMsg).ToString());
                        ...
                        //string itemPrice = System.Math.Round(feeItem.Item.Price, 4).ToString() ;
                        string itemCost = (feeItem.FT.OwnCost + feeItem.FT.PubCost + feeItem.FT.PayCost).ToString("F2");
                        totCost += feeItem.FT.OwnCost + feeItem.FT.PubCost + feeItem.FT.PayCost;
                        ...
                        //频次【字典】
                        string pc = feeItem.Order.Frequency.ID;   //11	每天一次（qd）
                        ...
                        try
                        {
                            rowJSON = string.Format(Function.FeeDetailJSON, mxsfdjh, mxwylsh, fwrq, xmbm, xmmc, xmlb, dj, sl, zj, ysbm,
                        ...
                        {
                            errInfo = "组装异常0：" + ex.Message;
                            this.InsertErrLog("1", patientID, invoiceNO, ("上传失败@" + errInfo).ToString());
                            return -1;
                        }
                        #endregion

                    }
                    //MessageBox.Show("金额：" + totCost.ToString("F2"));
                }
                ...
                try
                {
                    json = string.Format(Function.MainInfoJSON, wylsh, sfdjh, djjsrq, ddjgbm, ddjgmc, fyfsjgbm, fyfsjgmc, zwjgmc, gmsfhm, cbrmc,
                ...
                result = this.baseJob.UploadInfo("HJ001", json, ref errInfo);
                if (result <= 0)
                {
                    if (errInfo.Contains("已经上传") || errInfo.Contains("重复上传") || errInfo.Contains("重复提交"))
                    {
                        return 1;
                    }
                    else
            ...
            else
            {
                ...
                if (siReturnID == "-1")
                {
                    this.InsertErrLog("2", patientID, invoiceNO, ("结算单号【" + siReturnID + "】不正确,未找到对应的结算信息!").ToString());
                    return -1;
                ...
                //查询诊断信息
                DataTable caseDiagnose = this.inMgr.GetCaseDiagnoseByBASY(inPatientInfo.PID.PatientNO, inPatientInfo.InTimes.ToString());//inPatientInfo.ID
                string firstDiag = string.Empty;
                string secondDiag = string.Empty;
                ...
                if (caseDiagnose != null)
                {
                    ...
                    {
                        firstDiag = dRow["入院主要诊断编码"].ToString();
                        secondDiag = dRow["出院主要诊断编码"].ToString();
                    }
                }
                ...
                //{
                //    string[] invoiceNOList = invoiceNO.Split(',');
                //    if (invoiceNOList != null && invoiceNOList.Length > 0)
                //    {
                //        foreach (string invoiceNoTemp in invoiceNOList)
                //        {
                //            if (!string.IsNullOrEmpty(invoiceNoTemp))
                //            {
                //                //药品
                //                ArrayList alMedFee = this.inMgr.QueryMedItemListsByInvoiceNO(inPatientInfo.ID, invoiceNoTemp);
                //                if (alMedFee != null)
                //                {
                ...
                //{
                //    //药品
                //    ArrayList alMedFee = this.inMgr.QueryMedItemListsByInvoiceNO(inPatientInfo.ID, invoiceNO);
                //    if (alMedFee != null)
                //    {
                //        alFee.AddRange(alMedFee);
                //    }
                ...
                //            {
                //                //非药品
                //                ArrayList alUndrugFee = this.inMgr.QueryFeeItemListsByInvoiceNO(inPatientInfo.ID, invoiceNoTemp);
                //                if (alUndrugFee != null)
                //                {
                ...
                //{
                //    //非药品
                //    ArrayList alUndrugFee = this.inMgr.QueryFeeItemListsByInvoiceNO(inPatientInfo.ID, invoiceNO);
                //    if (alUndrugFee != null)
                //    {
                //        alFee.AddRange(alUndrugFee);
                //    }
                ...
                try
                {
                    ...
                    if (feeItem != null)
                    {
                        string inpatientNO = inPatientInfo.ID;//feeItem.ID;   //住院流水号，作为处方号
                        ...
                        {
                            totCost += decimal.Parse((fTemp.Item.SpecialPrice * fTemp.Item.Qty).ToString("F2"));
                            ...
                            if (hsUpLoadFeeDetails.ContainsKey(fTemp.ChargeOper.OperTime.ToString() + fTemp.Item.ID))
                            {
                                ...
                                feeItemList.FT.TotCost += decimal.Parse((fTemp.Item.SpecialPrice * fTemp.Item.Qty).ToString("F2")); //金额累加
                                feeItemList.FT.OwnCost += decimal.Parse((fTemp.Item.SpecialPrice * fTemp.Item.Qty).ToString("F2"));   //自费金额累计
                                feeItemList.FT.PubCost += 0;   //医保金额累加
                            ...
                            else
                            {
                                ...
                                fTemp.FT.TotCost = decimal.Parse((fTemp.Item.SpecialPrice * fTemp.Item.Qty).ToString("F2"));
                                fTemp.FT.OwnCost = decimal.Parse((fTemp.Item.SpecialPrice * fTemp.Item.Qty).ToString("F2"));
                        ...
                        for (int k = 0; k < feeGatherlsClone.Count; k++)
                        {
                            ...
                            if (this.itemCodeCompareType == "0")
                            {
                                ...
                                if (string.IsNullOrEmpty(feeItem.Item.GBCode) || feeItem.Item.GBCode == "0")
                                {
                                    ...
                                    {
                                        //MessageBox.Show(errMsg);
                                        this.InsertErrLog("2", patientID, invoiceNO, ("项目【" + feeItem.Item.Name + "(" + feeItem.Item.ID + ")" + "】1" + errMsg).ToString());
                            ...
                            string itemQty = feeItem.Item.Qty.ToString();
                            //string itemPrice = System.Math.Round(feeItem.Item.PackQty == 0 ? feeItem.Item.Price : feeItem.Item.Price / feeItem.Item.PackQty, 4).ToString();
                            string itemPrice = System.Math.Round(feeItem.Item.SpecialPrice, 4).ToString();
                           ...
                           //给药途径【字典】给药途径为空则取 900 其他用药途径
                            ...
                            {
                                MessageBox.Show("System.OutOfMemoryException" + e.Message);
                                errInfo = "System.OutOfMemoryException" + e.Message;
                                this.InsertErrLog("2", patientID, invoiceNO, ("调用异常：" + errInfo).ToString());
                                return -1;
                            }
                ...
                string dylx = "";
                string cblx = "";
                string zje = totCost.ToString("F2");
                string ybnzje = pubCostStr;
                string ybnzfje = "0.00";
                string zfbl = ownCostStr;
                string zhfbl = "";
                string xyf = "";    //西药费-提取自病案首页
                string zchyf = "";
                string zcyf = "";
        ...
        private int UploadPatientCase(string patientID, string patientType, string invoiceNO, string transType, string ybMedNo)
        {
            ...
            if (patientType == "1")
            {
                ...
                if (siReturnID == "-1")
                {
                    this.InsertErrLog("1", patientID, invoiceNO, ("结算单号【" + siReturnID + "】不正确,未找到对应的结算信息!").ToString());
                    return -1;
                }
                ...
                string ksmc = "";//outPatientInfo.DoctorInfo.Templet.Dept.Name;   //科室名称
                ...
                string rowJSON = string.Format(Function.InCaseJSON, sfdjh, ddjgbm, gmsfhm, mzzyh, cjrbs, zfbl, yylb, jylx, cjdjh, rybzh,
                                                                  ...
                                                                  xx, zs, zzms, lyfs, zryybm, zryymc, ksmc);
            ...
            else
            {
                ...
                //    this.InsertErrLog("2", patientID, invoiceNO, ("患者流水号【" + patientID + "】,已经退费!").ToString());
                //    return -1;
                //}
                //FS.HISFC.Models.Fee.Inpatient.Balance balance = balanceList[0] as FS.HISFC.Models.Fee.Inpatient.Balance;
                //if (balance == null || string.IsNullOrEmpty(balance.Invoice.ID))
                //{
                //    //MessageBox.Show("发票号【" + invoiceNO + "】，未找到对应的结算信息!");
                //    this.InsertErrLog("2", patientID, invoiceNO, ("患者流水号【" + patientID + "】,未找到对应的结算信息1!").ToString());
                //    return -1;
                //}
                string upType = this.outMgr.GetUpCount(patientID, invoiceNO, "2");
                //if (upType != "1")
                //{
                //    this.InsertErrLog("2", patientID, invoiceNO, ("患者流水号【" + patientID + "】,就诊明细未上传，请先上传就诊明细!").ToString());
                //    return -1;
                //}
                ...
                if (caseDiagnose != null)
                {
                    ...
                    {
                        firstDiag = dRow["入院主要诊断编码"].ToString();
                        firstDiagName = dRow["入院主要诊断"].ToString();
                        secondDiag = dRow["出院主要诊断编码"].ToString();
                        secondDiagName = dRow["出院主要诊断"].ToString();


                        jsonDiagInfo = "'DiagnosisCode1':'" + dRow["出院其他诊断1编码"].ToString().Replace("--", "")
                            + "','DiagnosisName1':'" + dRow["出院其他诊断1"].ToString().Replace("--", "") + "',"
                            + "'DiagnosisCode2':'" + dRow["出院其他诊断2编码"].ToString().Replace("--", "") 
                            + "','DiagnosisName2':'" + dRow["出院其他诊断2"].ToString().Replace("--", "")+ "',"
                            + "'DiagnosisCode3':'" + dRow["出院其他诊断3编码"].ToString().Replace("--", "")
                            + "','DiagnosisName3':'" + dRow["出院其他诊断3"].ToString().Replace("--", "") + "'," 
                            + "'DiagnosisCode4':'" + dRow["出院其他诊断4编码"].ToString().Replace("--", "")
                            + "','DiagnosisName4':'" + dRow["出院其他诊断4"].ToString().Replace("--", "") + "',"  
                            + "'DiagnosisCode5':'" + dRow["出院其他诊断5编码"].ToString().Replace("--", "")
                            + "','DiagnosisName5':'" + dRow["出院其他诊断5"].ToString().Replace("--", "") + "',"
                            + "'DiagnosisCode6':'" + dRow["出院其他诊断6编码"].ToString().Replace("--", "")
                            + "','DiagnosisName6':'" + dRow["出院其他诊断6"].ToString().Replace("--", "") + "',"
                            + "'DiagnosisCode7':'" + dRow["出院其他诊断7编码"].ToString().Replace("--", "")
                            + "','DiagnosisName7':'" + dRow["出院其他诊断7"].ToString().Replace("--", "") + "',"
                            + "'DiagnosisCode8':'" + dRow["出院其他诊断8编码"].ToString().Replace("--", "")
                            + "','DiagnosisName8':'" + dRow["出院其他诊断8"].ToString().Replace("--", "") + "',"
                            + "'DiagnosisCode9':'" + dRow["出院其他诊断9编码"].ToString().Replace("--", "")
                            + "','DiagnosisName9':'" + dRow["出院其他诊断9"].ToString().Replace("--", "") + "',"
                            + "'DiagnosisCode10':'" + dRow["出院其他诊断10编码"].ToString().Replace("--", "")
                            + "','DiagnosisName10':'" + dRow["出院其他诊断10"].ToString().Replace("--", "") + "',"
                            + "'DiagnosisCode11':'" + dRow["出院其他诊断11编码"].ToString().Replace("--", "")
                            + "','DiagnosisName11':'" + dRow["出院其他诊断11"].ToString().Replace("--", "") + "',"
                            + "'DiagnosisCode12':'" + dRow["出院其他诊断12编码"].ToString().Replace("--", "")
                            + "','DiagnosisName12':'" + dRow["出院其他诊断12"].ToString().Replace("--", "") + "',"
                            + "'DiagnosisCode13':'" + dRow["出院其他诊断13编码"].ToString().Replace("--", "")
                            + "','DiagnosisName13':'" + dRow["出院其他诊断13"].ToString().Replace("--", "") + "',"
                            + "'DiagnosisCode14':'" + dRow["出院其他诊断14编码"].ToString().Replace("--", "")
                            + "','DiagnosisName14':'" + dRow["出院其他诊断14"].ToString().Replace("--", "") + "',"
                            + "'DiagnosisCode15':'" + dRow["出院其他诊断15编码"].ToString().Replace("--", "")
                            + "','DiagnosisName15':'" + dRow["出院其他诊断15"].ToString().Replace("--", "") + "',"
                            + "'DiagnosisCode16':'" + dRow["出院其他诊断16编码"].ToString().Replace("--", "")
                            + "','DiagnosisName16':'" + dRow["出院其他诊断16"].ToString().Replace("--", "") + "',"
                            + "'DiagnosisCode17':'" + ""
                            + "','DiagnosisName17':'" + "" + "',"
                            + "'DiagnosisCode18':'" + ""
                            + "','DiagnosisName18':'" + "" + "',"
                ...
                Const conOutRecord = this.inMgr.QueryPatientOutRecord(patientID);//patientID
                ...
                FS.HISFC.Models.HealthRecord.Base metCase = this.inMgr.QueryInpatientCase(inPatientInfo.ID);
                //(inPatientInfo.PID.PatientNO.Substring(2) + inPatientInfo.InTimes);//目前写死，以后修改 截取住院号8位+住院次数
                if (metCase == null)
                {
                ...
                if (string.IsNullOrEmpty(inPatientInfo.IDCard) || inPatientInfo.IDCard.Contains("x") || inPatientInfo.IDCard.Length < 8)
                {
                    //MessageBox.Show("患者 " + inPatientInfo.Name + "【" + inPatientInfo.PID.PatientNO + "】，住院流水号：" + inPatientInfo.ID + "身份证号不正确！请修改正确的身份证号后上传");
                    this.InsertErrLog("2", patientID, invoiceNO, ("患者 " + inPatientInfo.Name + "【" + inPatientInfo.PID.PatientNO + "】，住院流水号：" + inPatientInfo.ID + "身份证号不正确！请修改正确的身份证号后上传").ToString());
                    return -1;
                }
                string hospitalId = Manager.setObj.HospitalID;   //机构号
                string admissionNo= ybMedNo + "-" + inPatientInfo.PID.PatientNO + "-" + inPatientInfo.IDCard; //住院号 【收单业务号+“-”+住院号 + “-” + 身份证号】
            
                string sciCardNo = inPatientInfo.IDCard;   //身份证号
                string sciCardIdentified = "";    //社保卡识别码
                string outBedNum = inPatientInfo.PVisit.PatientLocation.Bed.ID;   //出院床位号
                string admissionDate = metCase.PatientInfo.PVisit.InTime.ToString("yyyy-MM-dd");//inPatientInfo.PVisit.InTime.ToString("yyyy-MM-dd");   //入院时间
                string dischargeDate = metCase.PatientInfo.PVisit.OutTime.ToString("yyyy-MM-dd");//inPatientInfo.PVisit.OutTime.ToString("yyyy-MM-dd");   //出院时间
                string doctorCode = string.IsNullOrEmpty(inPatientInfo.PVisit.AdmittingDoctor.ID) ? inPatientInfo.DoctorReceiver.ID : inPatientInfo.PVisit.AdmittingDoctor.ID;   //责任医生编码
                string doctorName = string.IsNullOrEmpty(inPatientInfo.PVisit.AdmittingDoctor.ID) ? inPatientInfo.DoctorReceiver.Name : inPatientInfo.PVisit.AdmittingDoctor.Name;   //责任医生名称
                ...
                string leaveHospitalType = metCase.Out_Type;  //离院方式
                if (string.IsNullOrEmpty(leaveHospitalType))
                {
                    leaveHospitalType = "1";
                }
                ...
                string newbornCurrentWeight = "";
                if (!Regex.IsMatch(metCase.BabyInWeight, @"^[+-]?\d*[.]?\d*$"))
                {
                    weight = metCase.BabyInWeight;  //体重
                    newbornDate = metCase.PatientInfo.Birthday.ToString("yyyy-MM-dd");    //新生儿出生日期
                    //newbornDate = "-";
                    newbornWeight = metCase.BabyBirthWeight;   //新生儿出生体重
                    newbornCurrentWeight = metCase.BabyInWeight;   //新生儿入院体重
                }
                ...
                string dischargeDiseaseId = (string.IsNullOrEmpty(metCase.OutDiag.ID) ? secondDiag : metCase.OutDiag.ID);     //出院诊断编码
                string dischargeDiseaseName = (string.IsNullOrEmpty(metCase.OutDiag.ID) ? secondDiagName : metCase.OutDiag.Name);   //出院诊断名称
                string diagnosePosition2 = "";      //出院诊断方位
                string tsblbs = "0";   //特殊病例 【0普通；1长期住院的精神类患者；2、临终关怀病床；3、长期康复住院病床】
                string ksmc = metCase.InDept.Name;//出院科室名称
        ...
        private int CancelCaseInfo()
        {
            ...
            for (int rowIndex = 0; rowIndex < this.fpHaveUploaded_Sheet1.Rows.Count; rowIndex++)
            {
                ...
                if (isChoose)
                {
                    ...
                    string patientID = this.fpHaveUploaded_Sheet1.Cells[rowIndex, (int)ColUploaded.ColPatientID].Value.ToString();
                    string invoiceNO = this.fpHaveUploaded_Sheet1.Cells[rowIndex, (int)ColUploaded.ColInvoiceNO].Value.ToString();

                    if (patientType == "1")
                    {
                        MessageBox.Show("门诊没有病案信息可作废！");
                        continue;
                    }
                    else if (patientType == "2")
                    {
                        ...
                        //删除上传日志
                        result = this.inMgr.UpdateLog(patientType, patientID, invoiceNO,"1");
                        if (result == -1)
                        {
                            FS.FrameWork.WinForms.Classes.Function.HideWaitForm();
                            MessageBox.Show(this.inMgr.Err, "错误");
        ...
        /// <param name="errInfo"></param>
        /// <returns></returns>
        public int  CancelDetail(string patientID,string invoiceNo,string patientType,ref string errInfo)
        {
            if (string.IsNullOrEmpty(patientID) || string.IsNullOrEmpty(invoiceNo) || string.IsNullOrEmpty(patientType))
            {
                errInfo = "入参为空！" + "patientID:" + patientID + " invoiceNo:" + invoiceNo + " patientType:" + patientType;
                return -1;
            }
            string sfdjh = this.inMgr.GetSFDJH(invoiceNo, patientID, patientType);

            if (string.IsNullOrEmpty(sfdjh) || sfdjh == "-1")
            {
                errInfo = "获取收费单据号出错！";
                return 0;
            }
            ...
            if (result <= 0)
            {
                errInfo = "患者流水号：" + patientID + "，结算号： " + sfdjh + " 作废就诊信息查询失败@" + errInfo;
                return -1;
            }

            //删除上传日志
            result = this.inMgr.DeleteLog(invoiceNo, patientID, patientType);
            if (result == -1)
            {
                errInfo = "删除日志出错"+this.inMgr.Err;
                return -1;
            }
        ...
        private void QueryHaveUploadedDetail()
        {
            ...
            string strEndTime = this.dtEndTime.Value.ToString();
            string patientNo = this.lblPatientNo.Text.Trim();
            //string patientType1 = this.cmbPatientType.Tag.ToString();
            if (string.IsNullOrEmpty(patientNo))
            {
                patientNo = "ALL";
            }
            ...
            {
                //已上传的门诊患者
                DataTable dtOutpatient = this.outMgr.QueryHaveUploadedDetail(strBeginTime, strEndTime, patientNo);
                if (dtOutpatient != null && dtOutpatient.Rows.Count > 0)
                {
            ...
            {
                //已上传的住院患者
                DataTable dtInpatient = this.inMgr.QueryHaveUploadedDetail(strBeginTime, strEndTime, patientNo);
                if (dtInpatient != null && dtInpatient.Rows.Count > 0)
                {
        ...
        private void AddUploaded(DataTable dtPatient)
        {
            if (dtPatient != null && dtPatient.Rows.Count > 0)
            {
                ...
                foreach (DataRow dRow in dtPatient.Rows)
                {
                    ...
                    string strType = "全部都上传";
                    if (dRow[(int)ColUploaded.ColUploadType].ToString() == "3")
                    {
                        strType = "全部都上传";
                    }
                    else if (dRow[(int)ColUploaded.ColUploadType].ToString() == "2")
                    {
                        strType = "病案首页信息已上传";
                    }
                    else if (dRow[(int)ColUploaded.ColUploadType].ToString() == "1")
                    {
                        strType = "就诊明细已上传";
                    }
                    else
                    ...
                    this.fpHaveUploaded_Sheet1.Cells[rowIndex, (int)ColUploaded.ColUploadType].Value = strType;
        ...
        protected override FS.FrameWork.WinForms.Forms.ToolBarService OnInit(object sender, object neuObject, object param)
        {
            ...
            this.toolBarService.AddToolButton("已上传患者", "已上传患者查询", (int)FS.FrameWork.WinForms.Classes.EnumImageList.S手动录入, true, false, null);
            ...
            this.toolBarService.AddToolButton("验证", "验证上传信息", (int)FS.FrameWork.WinForms.Classes.EnumImageList.Y医保, true, false, null);

            this.toolBarService.AddToolButton("全部作废", "作废上传信息", (int)FS.FrameWork.WinForms.Classes.EnumImageList.Z注销, true, false, null);

            this.toolBarService.AddToolButton("病案作废", "病案信息作废", (int)FS.FrameWork.WinForms.Classes.EnumImageList.Z注销, true, false, null);
           
            //this.toolBarService.AddToolButton("出院志", "打开出院志更新界面", (int)FS.FrameWork.WinForms.Classes.EnumImageList.Z注销, true, false, null);

            this.toolBarService.AddToolButton("更新出院记录", "更新出院记录", (int)FS.FrameWork.WinForms.Classes.EnumImageList.C查询, true, false, null);

            this.toolBarService.AddToolButton("删除上传记录", "删除上传记录", (int)FS.FrameWork.WinForms.Classes.EnumImageList.C查询, true, false, null);


            this.toolBarService.AddToolButton("就诊明细上传", "就诊明细上传", (int)FS.FrameWork.WinForms.Classes.EnumImageList.X新建, true, false, null);

            this.toolBarService.AddToolButton("病案首页上传", "病案首页上传", (int)FS.FrameWork.WinForms.Classes.EnumImageList.X新建, true, false, null);
        ...
        public override void ToolStrip_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            switch (e.ClickedItem.Text)
            {
                ...
                case "更新出院记录":
                    ...
                    break;
                case "就诊明细上传":
                    this.AllUploadMainInfo();
                    break;
                case "病案首页上传":
                    this.AllUploadPatientCase();
                    break;
                case "清屏":
                    this.Clear();
                    break;
        ...
        private void DeleteLog()
        {
            ...
            if (MessageBox.Show("是否删除上传记录?", "疑问", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.No)
            {
                return;
            }
            ...
            for (int rowIndex = 0; rowIndex < this.fpHaveUploaded_Sheet1.Rows.Count; rowIndex++)
            {
                ...
                if (isChoose)
                {
                    ...
                    string idenNo = this.fpHaveUploaded_Sheet1.Cells[rowIndex, (int)ColUploaded.ColIdNO].Value.ToString();
                    string patientID = "";
                    patientID = this.fpHaveUploaded_Sheet1.Cells[rowIndex, (int)ColUploaded.ColPatientID].Value.ToString();

                    string sql = "delete from fs_upload_diseasepay y where y.clinic_code = '{0}' and y.patient_type= '2'";
                    sql = string.Format(sql, patientID);
                    if (con.ExecNoQuery(sql) < 0)
                    {
                        MessageBox.Show("删除失败！" + con.Err);
                    }
                    else
                    {
                        isSucc = true;
        ...
        private void UpdateVest()
        {
            ...
            string patientID = "";
            patientID = this.fpNeedUpload_Sheet1.Cells[rowIndex, (int)ColPatient.ColPatientID].Value.ToString();
        ...
        private enum CaseDiagnose
        {
            ...
            /// 出院其他诊断名称
            /// </summary>
            ColOutOtherDiagName = 71,

            /// <summary>
            /// 出院其他诊断描述
            /// </summary>
            ColOutOtherDiagMemo = 72,

            /// <summary>
            /// 出院其他诊断编码
            /// </summary>
            ColOutOtherDiagCode = 73,
            /// <summary>
            /// 出院诊断名称1
            /// </summary>
            ColOutDiagName1 = 74,
            ...
            /// 出院主诊断名称10
            /// </summary>
            ColOutDiagName10 = 102,

            /// <summary>
            /// 出院诊断描述10
            /// </summary>
            ColOutDiagMemo10 = 103,

            /// <summary>
            /// 出院诊断编码10
            /// </summary>
            ColOutDiagCode10 = 104,
            /// <summary>
            /// 出院主诊断名称11
            /// </summary>
            ColOutDiagName11 = 105,

            /// <summary>
            /// 出院诊断描述11
            /// </summary>
            ColOutDiagMemo11 = 106,

            /// <summary>
            /// 出院诊断编码11
            /// </summary>
            ColOutDiagCode11 = 107,
            /// <summary>
            /// 出院主诊断名称12
            /// </summary>
            ColOutDiagName12 = 108,

            /// <summary>
            /// 出院诊断描述12
            /// </summary>
            ColOutDiagMemo12 = 109,

            /// <summary>
            /// 出院诊断编码12
            /// </summary>
            ColOutDiagCode12 = 110,
            /// <summary>
            /// 出院主诊断名称13
            /// </summary>
            ColOutDiagName13 = 111,

            /// <summary>
            /// 出院诊断描述13
            /// </summary>
            ColOutDiagMemo13 = 112,

            /// <summary>
            /// 出院诊断编码13
            /// </summary>
            ColOutDiagCode13 = 113,
            /// <summary>
            /// 出院主诊断名称14
            /// </summary>
            ColOutDiagName14 = 114,

            /// <summary>
            /// 出院诊断描述14
            /// </summary>
            ColOutDiagMemo14 = 115,

            /// <summary>
            /// 出院诊断编码14
            /// </summary>
            ColOutDiagCode14 = 116,
            /// <summary>
            /// 出院主诊断名称15
            /// </summary>
            ColOutDiagName15 = 117,

            /// <summary>
            /// 出院诊断描述15
            /// </summary>
            ColOutDiagMemo15 = 118,

            /// <summary>
            /// 出院诊断编码15
            /// </summary>
            ColOutDiagCode15 = 119,
            /// <summary>
            /// 出院主诊断名称16
            /// </summary>
            ColOutDiagName16 = 120,

            /// <summary>
            /// 出院诊断描述16
            /// </summary>
            ColOutDiagMemo16 = 121,

            /// <summary>
            /// 出院诊断编码16
            /// </summary>
            ColOutDiagCode16 = 122,
            /// <summary>
            /// 出院主诊断名称17
            /// </summary>
            ColOutDiagName17 = 123,

            /// <summary>
            /// 出院诊断描述17
            /// </summary>
            ColOutDiagMemo17 = 124,

            /// <summary>
            /// 出院诊断编码17
            /// </summary>
            ColOutDiagCode17 = 125,
            /// <summary>
            /// 出院主诊断名称18
            /// </summary>
            ColOutDiagName18 = 128,

            /// <summary>
            /// 出院诊断描述18
            /// </summary>
            ColOutDiagMemo18 = 127,

            /// <summary>
            /// 出院诊断编码18
            /// </summary>
            ColOutDiagCode18 = 128,
            /// <summary>
            /// 出院主诊断名称19
            /// </summary>
            ColOutDiagName19 = 129,

            /// <summary>
            /// 出院诊断描述19
            /// </summary>
            ColOutDiagMemo19 = 130,

            /// <summary>
            /// 出院诊断编码19
            /// </summary>
            ColOutDiagCode19 = 131,
            /// <summary>
            /// 出院主诊断名称20
            /// </summary>
            ColOutDiagName20 = 132,

            /// <summary>
            /// 出院诊断描述20
            /// </summary>
            ColOutDiagMemo20 = 133,

            /// <summary>
            /// 出院诊断编码20
            /// </summary>
            ColOutDiagCode20 = 134,
        ...
        private void tabMain_Click(object sender, EventArgs e)
        {
            ...
            if (this.tabMain.SelectedIndex != 0)
            {
                ...
                this.cmbPatientType.AddItems(al);
                if (this.patientType == "Z" || patientType == "F")
                {
            ...
            else
            {
                ...
                {
                    al.Add(obj);
                    obj = new FS.FrameWork.Models.NeuObject("Z", "六大类重症患者", "");
                    al.Add(obj);
                    obj = new FS.FrameWork.Models.NeuObject("F", "非六大类重症患者", "");
                }
                al.Add(obj);
                this.cmbPatientType.ClearItems();
                this.cmbPatientType.AddItems(al);
    ...
    public class OperationSortByHappenNO : IComparer
    {
        public int Compare(object x, object y)
        {
            ...
            try
            {
                FS.HISFC.Models.HealthRecord.OperationDetail obj1 = ((ArrayList)x)[0] as FS.HISFC.Models.HealthRecord.OperationDetail;
                FS.HISFC.Models.HealthRecord.OperationDetail obj2 = ((ArrayList)y)[0] as FS.HISFC.Models.HealthRecord.OperationDetail;
                string text = obj1.OperationNO.PadLeft(5,'0');
                string text2 = obj2.OperationNO.PadLeft(5, '0');
    ...
    public class DiagnoseSortByHappenNO : IComparer
    {
        public int Compare(object x, object y)
        {
            int result;
            try
            {
                FS.HISFC.Models.HealthRecord.Diagnose obj1 = ((ArrayList)x)[0] as FS.HISFC.Models.HealthRecord.Diagnose;
                FS.HISFC.Models.HealthRecord.Diagnose obj2 = ((ArrayList)y)[0] as FS.HISFC.Models.HealthRecord.Diagnose;
                string text = obj1.DiagInfo.HappenNo.ToString().PadLeft(5, '0');
                string text2 = obj2.DiagInfo.HappenNo.ToString().PadLeft(5, '0');
                result = string.Compare(text, text2);
            }
            catch
            {
