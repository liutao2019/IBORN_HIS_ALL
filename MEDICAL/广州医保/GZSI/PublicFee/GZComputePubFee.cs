...
namespace GZSI.PublicFee
{
    /// <summary>
    /// 广州医学院附属肿瘤医院公费算法
    /// </summary>
    class GZComputePubFee
    {
        #region 属性
        ...
        public bool IsInBlackList(FS.HISFC.Models.Registration.Register r)
        {
            ...
            //    {
            //        FS.HISFC.Models.Base.Employee empl = this.interManager.GetEmployeeInfo(markNO.Remove(0, 1).PadLeft(6, 'A'));
            //        if (empl == null || string.IsNullOrEmpty(empl.ID))
            //        {
            //            //再根据0找一遍
            //            empl = this.interManager.GetEmployeeInfo(markNO.Remove(0, 1).PadLeft(6, '0'));
            //        }
                ...
                case "省离休":
                    {
                        ...
                        if (Function.IsDrug(feeItem.Item.ItemType, feeItem.Item.MinFee))
                        {
        ...
        {
            if (Function.IsSplitFeeFlag(ItemList))
            {
        ...
        public int GetCommomPactItemPubCost(Register r, ref ArrayList feeDetails)
        {
            ...
            decimal DayDrugFee = 0;
            bool isDayLimit = false;

            //区公费，一天只能记账两次（中六）
            //根据记账单号查询发票表有效记录
            //大于两次，则记账限额为零
            //如何判断是否区公医
            //if (Function.IsQuPub(r,ref this.errMsg)&& r.Pact.PayKind.ID == "03")
            //{
            //    if (Function.GetJZCount(r.PID.CardNO, r.Pact.ID, r.SSN) >= 2)
            //    {
            //        if (MessageBox.Show("此患者今天药费已记账" + DayDrugFee + "元，是否继续？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
            //        {
            //            isDayLimit = true;
            //            DayDrugFee = 0;
            //        }
            ...
            //else
            {
                DayDrugFee = MgrOutPatient.GetDayDrugFee(r.SSN, r.Name);
                if (DayDrugFee == -1)
                {
                    return -1;
                }
                ...
                if (DayDrugFee > 0)
                {
                    if (MessageBox.Show("此患者今天药费已记账" + DayDrugFee + "元，是否继续？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                    {
                        DayDrugFee = 0;
                    }
                    else
                    {
                        this.errMsg = "此患者本次公费结算已经取消！";
                        return -1;
                    }
                }
            }
            ...
            foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList f in feeDetails)
            {
                ...
                if (!string.IsNullOrEmpty(CurrentItemLimit.User01))
                {
                    f.NewItemRate = FS.FrameWork.Function.NConvert.ToDecimal(CurrentItemLimit.User01);//自负比例
                }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (isDayLimit)//区公医大于两次后限额为零
                        {
                            limitCost = 0;
                        }
                        else if (Function.IsDrug(f.Item.ItemType, f.Item.MinFee))//否则按照日限额处理
                        {
                            if (DayDrugFee > 0)
                            {
        ...
        private void ComputeFee(FS.HISFC.Models.Registration.Register r, System.Collections.ArrayList feeDetails)
        {
            ...
            if (chaePubCost != 0 && chaePayCost != 0)
            {
                ...
                roundItemInfo.OutPatientFeeRoundOff(r, chaePayCost, chaePubCost, ref feeItemList, ((FS.HISFC.Models.Fee.Outpatient.FeeItemList)feeDetails[0]).RecipeSequence);
        ...
        private ArrayList FilterFeeItem(ArrayList alFee, bool isPhamarcy)
        {
            ...
            if (isPhamarcy)
            {
                ...
                {
                    if (Function.IsDrug(item.Item.ItemType, item.Item.MinFee))
                    {
                        al.Add(item);
                    }
                }
            ...
            else//非药品项目
            {
                foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList item in alFee)
                {
                    if (Function.IsDrug(item.Item.ItemType, item.Item.MinFee) == false)
                    {
                        al.Add(item);
                    }
                }
