...
namespace API.GZSI.Business
{
    ...
    public abstract class AbstractService<T, E> : IService where E : Models.Response.ResponseBase
    {
        ...
        public virtual int ServiceSign(DateTime sNow)
        {
            ...
            requestGzsiModel9001Data.mac = "11-11-11-11-11";
            requestGzsiModel9001Data.ip = "11.11.11.11";
            requestGzsiModel9001Data.userid = Models.UserInfo.Instance.userId;
            requestGzsiModel9001Data.opter_no = FS.FrameWork.Management.Connection.Operator.ID;
            requestGzsiModel9001Data.password = Common.Function.GetMD5(sNow.ToString("yyyyMMddHHmmss") + Models.UserInfo.Instance.password);
            requestGzsiModel9001Data.currenttime = sNow.ToString("yyyyMMddHHmmss");
            requestGzsiModel9001.data = requestGzsiModel9001Data;

            //接口调用
            //string inparam = Class.JsonHelper.SerializeObject(requestGzsiModel9001);
            //string requestStr = commonService9001.ConvertModelToSendMessage(inparam, ref Infno);
            ...
            //结果处理
            Models.UserInfo.Instance.sign_no = responseGzsiModel9001.output.result.sign_no;
            Models.UserInfo.Instance.sign_time = sNow;
        ...
        public int CallService(T sendObject, ref E receiveObject, params object[] appendParams)
        {
            ...
            if (string.IsNullOrEmpty(Models.UserInfo.Instance.sign_no) ||
                Models.UserInfo.Instance.sign_time < sNow.AddHours(-1) ||
                Models.UserInfo.Instance.sign_oper != FS.FrameWork.Management.Connection.Operator.ID)
            {
        ...
        protected virtual string ConvertModelToSendMessage(T t, ref string Infno, params object[] appendParams)
        {
            ...
            Infno = wm.Infno;
            wm.Msgid = Models.UserInfo.Instance.fixmedins_code + DateTime.Now.ToString("yyyyMMddHHmmss");// + wm.Infno;
            wm.Insuplc_admdvs = string.IsNullOrEmpty(Models.UserInfo.Instance.insuplc) ?"440100": Models.UserInfo.Instance.insuplc;//异地需要改变参保地
            wm.Mdtrtarea_admvs = "440100";
            wm.Recer_admdvs = "440100";
            wm.Dev_no = "";
            wm.Dev_safe_info = "";
            wm.Signtype = "SM3";
            ...
            wm.Fixmedins_code = Models.UserInfo.Instance.fixmedins_code;
            wm.Fixmedins_name = "广州爱博恩妇产医院";
