...
namespace API.GZSI.UI
{
    ...
    /// 功能说明：医保病案上传功能
    /// </summary>
    public partial class ucInPatientUploadLX : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        protected override FS.FrameWork.WinForms.Forms.ToolBarService OnInit(object sender, object neuObject, object param)
        {
            ...
            toolBarService.AddToolButton("自费病人费用明细信息上传", "自费病人费用明细信息上传", (int)FS.FrameWork.WinForms.Classes.EnumImageList.M明细, true, false, null);
        ...
        private int refreshData()
        {
            ...
            #region 5261
            CommonService5261 CommonService5261 = new CommonService5261();
            Models.Request.RequestGzsiModel5261 RequestGzsiModel5261 = new Models.Request.RequestGzsiModel5261();
            Models.Response.ResponseGzsiModel5261 ResponseGzsiModel5261 = new Models.Response.ResponseGzsiModel5261();
            Models.Request.RequestGzsiModel5261.Data Data5261 = new Models.Request.RequestGzsiModel5261.Data();
            Data5261.begntime = begin.ToString();
            Data5261.endtime = end.ToString();
            Data5261.psn_no = "";
            RequestGzsiModel5261.data = Data5261;
            if (CommonService5261.CallService(RequestGzsiModel5261, ref ResponseGzsiModel5261) < 0)
            {
                MessageBox.Show(CommonService5261.ErrorMsg);
                return -1;
            }
            #endregion

            if (ResponseGzsiModel5261.output.setlinfo == null || ResponseGzsiModel5261.output.setlinfo.Count <= 0)
            {
                MessageBox.Show("查询零星报销数据为空！");
            }
            else {
                this.setData(ResponseGzsiModel5261.output.setlinfo);
            }
            return 1;
        }
        ...
        protected int UploadJjqd()
        {
            if (this.tabSheet.SelectedIndex != 0)
            {
                MessageBox.Show("请勾选选择要上传费用明细的患者！");
                return -1;
            }
        ...
        protected int UploadFymx() {
            ...
            List<FS.HISFC.Models.RADT.PatientInfo> selectedPatient = this.getSelectedPatient();///TOOL
            ...
            try
            {
                ...
                foreach (FS.HISFC.Models.RADT.PatientInfo patient in selectedPatient)//TOOL
                {
                    index++;
                    ...
                    if (this.uploadFymxInfo(patient, ref errMsg) < 0)
                    {
                        //回滚
                        FS.FrameWork.WinForms.Classes.Function.HideWaitForm();
                        MessageBox.Show("上传医疗保障基金结算清单发生错误：" + errMsg);
        ...
        private List<FS.HISFC.Models.RADT.PatientInfo> getSelectedPatient()
        {
            ...
            foreach (FarPoint.Win.Spread.Row row in this.fpUnupload_Sheet1.Rows)
            {
                ...
                {
                    if (row.Tag is Models.Response.ResponseGzsiModel5261.Output.Setlinfo)
                    {
                        //selectedPatient.Add(row.Tag as FS.HISFC.Models.RADT.PatientInfo);
                        Models.Response.ResponseGzsiModel5261.Output.Setlinfo setlinfo = row.Tag as Models.Response.ResponseGzsiModel5261.Output.Setlinfo;
                        FS.HISFC.Models.RADT.PatientInfo patientInfo = new FS.HISFC.Models.RADT.PatientInfo();
                        patientInfo.ID = setlinfo.inpatientNo;
                        patientInfo.SIMainInfo.Mdtrt_id = setlinfo.mdtrt_id;
                        patientInfo.SIMainInfo.Setl_id = setlinfo.setl_id;
                        patientInfo.IDCard = setlinfo.certno;
                        patientInfo.Name = setlinfo.psn_name;
                        patientInfo.SIMainInfo.Med_type = setlinfo.med_type;
                        patientInfo.PID.PatientNO = setlinfo.ipt_op_no;
                        patientInfo.SIMainInfo.Insutype = setlinfo.insutype;
                        patientInfo.SIMainInfo.Dise_name = setlinfo.dise_name;
                        patientInfo.SIMainInfo.RegNo = setlinfo.mdtrt_id;
                        selectedPatient.Add(patientInfo);
                    }
                }
            }
        ...
        private List<FS.HISFC.Models.RADT.PatientInfo> getSelectedPatientForCancel()
        {
            ...
            if (cr != null)
            {
                Models.Response.ResponseGzsiModel5261.Output.Setlinfo setlinfo = (this.fpUploaded_Sheet1.Rows[cr.Row]).Tag as Models.Response.ResponseGzsiModel5261.Output.Setlinfo;
                FS.HISFC.Models.RADT.PatientInfo patientInfo = new FS.HISFC.Models.RADT.PatientInfo();
                patientInfo.ID = setlinfo.inpatientNo;
                patientInfo.SIMainInfo.Mdtrt_id = setlinfo.mdtrt_id;
                patientInfo.SIMainInfo.Setl_id = setlinfo.setl_id;
        ...
        private void setData(List<Models.Response.ResponseGzsiModel5261.Output.Setlinfo> data5261)
        {
            foreach (Models.Response.ResponseGzsiModel5261.Output.Setlinfo data  in data5261)
            {

                if (data.med_type != "21" && data.med_type != "52")
                {
                    continue;
                }


                if (string.IsNullOrEmpty(data.ipt_op_no))
                {
                    continue;
                }
                ...
                data.ipt_op_no = data.ipt_op_no.PadLeft(10,'0');

                //判断是否保存到本地
                if (this.localMgr.HasLXInfo(data.mdtrt_id, data.setl_id) != "1")
                {
                    //插入零星表
                    if (this.localMgr.InsertLXInfo(data) < 0)
                    {
                        MessageBox.Show("插入零星数据失败：" + this.localMgr.Err);
                        return;
                    }
                }
                else {
                    //已插入数据，获取流水号
                    Models.Response.ResponseGzsiModel5261.Output.Setlinfo setlinfo = new Models.Response.ResponseGzsiModel5261.Output.Setlinfo();
                    this.localMgr.getLXInpatientNoBySetlInfo(data.mdtrt_id, data.setl_id,ref setlinfo);
                    data.inpatientNo = setlinfo.inpatientNo;
                    data.updateState = setlinfo.updateState;
                }

                //if (string.IsNullOrEmpty(data.inpatientNo))
                //{
                //    //获取零报流水号，假设只有一条数据
                //    if (this.localMgr.HasLXInpatientNoByPatientNO(data.ipt_op_no) == 1)
                //    {
                //        //插入流水号
                //        data.inpatientNo = this.localMgr.getLXInpatientNoByPatientNO(data.ipt_op_no);
                //        //插入零星表
                //        if (this.localMgr.UpdateInpatientNoToLX(data.mdtrt_id, data.setl_id, data.inpatientNo) < 0)
                //        {
                //            MessageBox.Show("更新零星流水号失败：" + this.localMgr.Err);
                //            return;
                //        }
                //        data.updateState = "-1";
                //    }
                ...
                //if (!string.IsNullOrEmpty(data.inpatientNo)) {
                //    FS.HISFC.Models.RADT.PatientInfo patientInfo = new FS.HISFC.Models.RADT.PatientInfo();
                //    patientInfo.ID = data.inpatientNo;
                //    patientInfo.SIMainInfo.Psn_name = data.psn_name;
                //    patientInfo.SIMainInfo.Mdtrt_id = data.mdtrt_id;//就医登记号
                //    patientInfo.SIMainInfo.Setl_id = data.setl_id;
                //    patientInfo.SIMainInfo.Psn_type = data.psn_type;//人员证件类型
                //    patientInfo.SIMainInfo.Insutype = data.insutype;//险种类型
                //    patientInfo.SIMainInfo.Med_type = data.med_type;
                //    patientInfo.SIMainInfo.Certno = data.certno;
                //    patientInfo.SIMainInfo.Dise_name = data.dise_name;
                //    patientInfo.SIMainInfo.Dise_code = data.dise_no;
                //    patientInfo.PID.PatientNO = data.ipt_op_no;
                //}

                #region 赋值
                if (!string.IsNullOrEmpty(data.updateState) && data.updateState == "1")
                {
                    this.addRowToUpdatedFp(data);
                }
                else {
                    this.addRowToUnupdateFp(data);
                }
                #endregion
        ...
        /// <returns></returns>
        private int addRowToUnupdateFp(Models.Response.ResponseGzsiModel5261.Output.Setlinfo data)
        {
            int rowIndex = this.fpUnupload_Sheet1.RowCount;
            this.fpUnupload_Sheet1.Rows.Add(this.fpUnupload_Sheet1.RowCount, 1);
            this.fpUnupload_Sheet1.Cells[rowIndex, 0].Value = false;
            this.fpUnupload_Sheet1.Cells[rowIndex, 1].Value = data.inpatientNo;//流水号
            this.fpUnupload_Sheet1.Cells[rowIndex, 2].Text = data.mdtrt_id;//就诊ID
            this.fpUnupload_Sheet1.Cells[rowIndex, 3].Text = data.setl_id;//结算ID
            this.fpUnupload_Sheet1.Cells[rowIndex, 4].Text = data.psn_name;//患者姓名
            this.fpUnupload_Sheet1.Cells[rowIndex, 5].Text = data.certno;//证件号
            this.fpUnupload_Sheet1.Cells[rowIndex, 6].Text = data.psn_type;//人员类型
            this.fpUnupload_Sheet1.Cells[rowIndex, 7].Text = this.constMgr.GetConstant("GZSI_med_type", data.med_type).Name;//医疗类型
            this.fpUnupload_Sheet1.Cells[rowIndex, 8].Text = data.ipt_op_no;//住院号
            this.fpUnupload_Sheet1.Cells[rowIndex, 9].Text = data.begntime;//开始时间
            this.fpUnupload_Sheet1.Cells[rowIndex, 10].Text = data.endtime;//结束时间
            this.fpUnupload_Sheet1.Cells[rowIndex, 11].Text = data.medfee_sumamt;//总金额
            this.fpUnupload_Sheet1.Cells[rowIndex, 12].Text = data.fund_pay_sumamt;//报销金额
            this.fpUnupload_Sheet1.Cells[rowIndex, 13].Value = data.dise_name;//病种名称
            this.fpUnupload_Sheet1.Cells[rowIndex, 14].Value =data.insutype;//险种类型
            this.fpUnupload_Sheet1.Cells[rowIndex, 15].Value = data.inpatientNo;//流水号
            this.fpUnupload_Sheet1.Rows[rowIndex].Tag = data;

            //参数转换 ，根据流水号，查询本地姓名，证件号，补的发票记录
            if (!string.IsNullOrEmpty(data.inpatientNo))
            {
                DataTable dt = this.localMgr.getPatientInfoByInPatientNO(data.inpatientNo);
                if (dt != null && dt.Rows.Count > 0)
                {
                    DataRow dr = dt.Rows[0];
                    this.fpUnupload_Sheet1.Cells[rowIndex, 16].Value = dr[0].ToString();//关联姓名
                    this.fpUnupload_Sheet1.Cells[rowIndex, 17].Value = dr[1].ToString();//关联证件号
                    this.fpUnupload_Sheet1.Cells[rowIndex, 18].Value = dr[2].ToString();//发票号
                    this.fpUnupload_Sheet1.Cells[rowIndex, 19].Value = dr[3].ToString();//发票代码
                }
            }
        ...
        /// <param name="patient"></param>
        /// <returns></returns>  
        private int addRowToUpdatedFp(Models.Response.ResponseGzsiModel5261.Output.Setlinfo data)
        {
            //int rowIndex = this.fpUploaded_Sheet1.RowCount;
            //this.fpUploaded_Sheet1.Rows.Add(this.fpUploaded_Sheet1.RowCount, 1);
            //this.fpUploaded_Sheet1.Cells[rowIndex, 0].Value = false;
            //this.fpUploaded_Sheet1.Cells[rowIndex, 1].Value = data.inpatientNo;//流水号
            //this.fpUploaded_Sheet1.Cells[rowIndex, 2].Text = data.mdtrt_id;//就诊ID
            //this.fpUploaded_Sheet1.Cells[rowIndex, 3].Text = data.setl_id;//结算ID
            //this.fpUploaded_Sheet1.Cells[rowIndex, 4].Text = data.psn_name;//患者姓名
            //this.fpUploaded_Sheet1.Cells[rowIndex, 5].Text = data.certno;//证件号
            //this.fpUploaded_Sheet1.Cells[rowIndex, 6].Text = data.psn_type;//人员类型
            //this.fpUploaded_Sheet1.Cells[rowIndex, 7].Text = data.med_type;//医疗类型
            //this.fpUploaded_Sheet1.Cells[rowIndex, 8].Text = data.ipt_op_no;//住院号
            //this.fpUploaded_Sheet1.Cells[rowIndex, 9].Value = data.dise_name;//病种名称
            //this.fpUploaded_Sheet1.Cells[rowIndex, 10].Value = data.insutype;//险种类型
            //this.fpUploaded_Sheet1.Cells[rowIndex, 11].Value = data.inpatientNo;//流水号
            //this.fpUploaded_Sheet1.Rows[rowIndex].Tag = data;
            ...
            //    if (dt != null && dt.Rows.Count > 0) {
            //        DataRow dr = dt.Rows[0];
            //        this.fpUploaded_Sheet1.Cells[rowIndex, 12].Value = dr[0].ToString();//关联姓名
            //        this.fpUploaded_Sheet1.Cells[rowIndex, 13].Value = dr[1].ToString();//关联证件号
            //        this.fpUploaded_Sheet1.Cells[rowIndex, 14].Value = dr[2].ToString();//发票号
            //        this.fpUploaded_Sheet1.Cells[rowIndex, 15].Value = dr[3].ToString();//发票代码
            //    }
            //}
            FS.HISFC.Models.RADT.PatientInfo p = new FS.HISFC.Models.RADT.PatientInfo();
            p.ID = data.inpatientNo;//流水号
            FS.HISFC.Models.RADT.PatientInfo obj = this.localMgr.GetSIPersonInfo(p.ID, p);
        ...
        private int uploadBrsyInfo(FS.HISFC.Models.RADT.PatientInfo patient, ref string errMsg)
        {
            ...
            if (string.IsNullOrEmpty(patient.SIMainInfo.Psn_no)) {
                ...
                {
                    patient.SIMainInfo.Psn_no = ResponseGzsiModel1101.output[0].baseinfo.psn_no;
                    requestGzsiModel4401.baseinfo.psn_no = ResponseGzsiModel1101.output[0].baseinfo.psn_no;
                }
            }
        ...
        private int uploadJsqdInfoA(FS.HISFC.Models.RADT.PatientInfo patient, ref string errMsg)
        {
            ...
            if (string.IsNullOrEmpty(patient.SIMainInfo.Psn_no))
            {
                ...
                {
                    patient.SIMainInfo.Psn_no = ResponseGzsiModel1101.output[0].baseinfo.psn_no;
                    requestGzsiModel4101A.setlinfo.psn_no = ResponseGzsiModel1101.output[0].baseinfo.psn_no;
                }
            }
            #endregion
        ...
        /// <param name="errMsg"></param>
        /// <returns></returns>
        private int uploadFymxInfo(FS.HISFC.Models.RADT.PatientInfo patient, ref string errMsg) {

            InPatient4201 inPatient4201 = new InPatient4201();
            Models.Request.RequestGzsiModel4201 requestGzsiModel4201 = new API.GZSI.Models.Request.RequestGzsiModel4201();
            Models.Response.ResponseGzsiModel4201 responseGzsiModel4201 = new Models.Response.ResponseGzsiModel4201();
            requestGzsiModel4201.feedetail = new List<API.GZSI.Models.Request.RequestGzsiModel4201.Feedetail>();
            requestGzsiModel4201.mdtrtinfo = new API.GZSI.Models.Request.RequestGzsiModel4201.Mdtrtinfo();
            requestGzsiModel4201.diseinfo = new List<API.GZSI.Models.Request.RequestGzsiModel4201.Diseinfo>();


            DataTable dtFeedetail = new DataTable();
            DataTable dtMdtrtinfo = new DataTable();
            DataTable dtDiseinfo = new DataTable();

            #region /明细信息feedetail
            if (this.localMgr.Get4201Feedetail(patient.ID, ref dtFeedetail) < 0)
            {
                errMsg = this.localMgr.Err;
                return -1;
            }
            else if (dtFeedetail.Rows.Count == 0)
            {
                errMsg = "未找到明细信息";
                return -1;
            }

            foreach (System.Data.DataRow row in dtFeedetail.Rows)
            {
                API.GZSI.Models.Request.RequestGzsiModel4201.Feedetail feedetail = new API.GZSI.Models.Request.RequestGzsiModel4201.Feedetail();
                feedetail = Common.Function.DataRowToObject<API.GZSI.Models.Request.RequestGzsiModel4201.Feedetail>(row);
                requestGzsiModel4201.feedetail.Add(feedetail);
            }
            ...
            if (this.localMgr.Get4201Mdtrtinfo(patient.ID, ref dtMdtrtinfo) < 0)
            {
                errMsg = this.localMgr.Err;
                return -1;
            }
            else if (dtMdtrtinfo.Rows.Count == 0)
            {
                errMsg = "未找到就诊信息";
                return -1;
            }

            requestGzsiModel4201.mdtrtinfo = Common.Function.DataRowToObject<API.GZSI.Models.Request.RequestGzsiModel4201.Mdtrtinfo>(dtMdtrtinfo.Rows[0]);

            #endregion


            #region /诊断diseinfo
            if (this.localMgr.Get4201Diseinfo(patient.ID, ref dtDiseinfo) < 0)
            {
                errMsg = this.localMgr.Err;
                return -1;
            }
            ...
            foreach (System.Data.DataRow row in dtDiseinfo.Rows)
            {
                API.GZSI.Models.Request.RequestGzsiModel4201.Diseinfo diseinfo = new API.GZSI.Models.Request.RequestGzsiModel4201.Diseinfo();
                diseinfo = Common.Function.DataRowToObject<API.GZSI.Models.Request.RequestGzsiModel4201.Diseinfo> (row);
                requestGzsiModel4201.diseinfo.Add(diseinfo);
            }
            #endregion

            returnvalue = inPatient4201.CallService(requestGzsiModel4201, ref responseGzsiModel4201, SerialNumber, strTransVersion, strVerifyCode);

            if (returnvalue < 0)
            {
                errMsg = inPatient4201.ErrorMsg;
                return -1;
            }
        ...
        private void ucQueryPatientInfo_myEvent()
        {
            ...
            this.p = patientTemp;
            this.txtPatientInfo.Text = "姓名：" + p.Name
                                            + "    性别：" + p.Sex.Name
                                            + "    出生日期：" + p.Birthday.ToShortDateString()
                                            + "    证件号：" + p.IDCard
                                            + "    科室：" + p.PVisit.PatientLocation.Dept.Name
                                            + "    入院日期：" + p.PVisit.InTime.ToString("yyyy-MM-dd HH:mm:ss")
                                            + "    出院日期：" + p.PVisit.OutTime.ToString("yyyy-MM-dd HH:mm:ss");
        }

        private void bSavePaitentInfo_Click(object sender, EventArgs e)
        {
            if (this.tabSheet.SelectedIndex != 0)
            {
                MessageBox.Show("请勾选选择要保存流水号的患者！");
                return;
            }
            ...
            if (string.IsNullOrEmpty(this.tbInpatientNo.Text))
            {
                MessageBox.Show("保存时，请填写流水号！");
                return;
            }
            ...
            try
            {
                ...
                string tbInpatientNo = this.tbInpatientNo.Text;
                int res = this.localMgr.UpdateInpatientNoToLX(patient.SIMainInfo.Mdtrt_id, patient.SIMainInfo.Setl_id, tbInpatientNo);
                if (res < 0)
                {
        ...
        private void bSaveInvoiceInfo_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(patient.ID)) {
                    MessageBox.Show("该零星记录未关联本地记录，请先关联本地记录，再补充发票号！");
                    return;
                }
        ...
        private void bSavePaitentInfo2_Click(object sender, EventArgs e)
        {
            if (this.tabSheet.SelectedIndex != 0)
            {
                MessageBox.Show("请勾选选择要保存流水号的患者！");
            ...
            if (this.p == null) {
                MessageBox.Show("保存时，请先选择需要关联的本地住院记录！");
                return;
            }
            ...
            try
            {
                string errMsg = string.Empty;
                FS.HISFC.Models.RADT.PatientInfo patient = selectedPatient[0];//患者信息
                string tbInpatientNo = this.p.ID;
                int res = this.localMgr.UpdateInpatientNoToLX(patient.SIMainInfo.Mdtrt_id, patient.SIMainInfo.Setl_id, tbInpatientNo);
                if (res < 0)
                {
                    MessageBox.Show("保存失败！");
