...
namespace API.GZSI.Class
{
    public class HttpHelper
    {
        ...
        private static int postData(string request,string Infno, ref string response)
        {
            ...
            if (Infno.StartsWith("42")) {
                ServiceUrl = Models.UserInfo.Instance.zf_url + Infno;
            }
            ...
            try
            {
                ...
                if (Infno == "9102")
                {
                    string strPdfPath = Environment.CurrentDirectory + "\\" + DateTime.Now.ToString("yyyyMMdd") + Infno + ".ZIP";//压缩文件的地址 
                    File.WriteAllBytes(strPdfPath, returnBytes);
                }
