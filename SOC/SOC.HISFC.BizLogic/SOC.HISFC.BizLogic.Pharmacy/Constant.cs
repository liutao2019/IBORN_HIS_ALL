...
namespace FS.SOC.HISFC.BizLogic.Pharmacy
{
    ...
    public class Constant : DataBase
    {
        ...
        public int SetFunction(FS.HISFC.Models.Pharmacy.PhaFunction FunConstant, string ifInUpDel)
        {
            ...
            {
                string pnode;
                FS.HISFC.Models.Pharmacy.PhaFunction functin = (FS.HISFC.Models.Pharmacy.PhaFunction)this.QueryFunctionByNode(FunConstant.ID)[0];//找出父节点
        ...
        public int InsertCompany(FS.HISFC.Models.Pharmacy.Company company)
        {
            ...
            try
            {
                ...
                company.ID = company.ID.PadLeft(5, '0');    //科室编码4位，区别一下
                string[] strParm = myGetParmItem(company);  //取参数列表
                strSQL = string.Format(strSQL, strParm);    //替换SQL语句中的参数。
            }
            catch (Exception ex)
        ...
        /// </summary>
        /// <returns>科室常数数组，出错返回null</returns>
        public ArrayList QueryPharmacyStatList()
        {
            string strSQL = "";
            //取SELECT语句
            if (this.GetSQL("SOC.Pharmacy.Constant.GetDeptConstant", ref strSQL) == -1)
            {
                strSQL = @"
                            SELECT COM_DEPARTMENT.DEPT_CODE, --部门编码
                                         COM_DEPARTMENT.DEPT_NAME, --部门名称
                                         '' STORE_MAX_DAYS, --库房最高库存量(天)
                                         '' STORE_MIN_DAYS, --库房最低库存量(天)
                                         '' REFERENCE_DAYS, --参考天数
                                         '' BATCH_FLAG, --是否按批号管理药品
                                         '' STORE_FLAG, --是否管理药品库存
                                         '' UNIT_FLAG, --库存管理时默认的单位，1包装单位，0最小单位
                                         '' OPER_CODE, --操作员代码
                                         '' OPER_DATE, --操作时间
                                         '' ARK_FLAG,
                                         '' IN_LIST_CODE,
                                         '' OUT_LIST_CODE
                                    FROM COM_DEPARTMENT, COM_DEPTSTAT
                                   WHERE COM_DEPTSTAT.STAT_CODE = '03' AND
                                         COM_DEPTSTAT.DEPT_CODE = COM_DEPARTMENT.DEPT_CODE
                            ";
                this.CacheSQL("SOC.Pharmacy.Constant.GetDeptConstant", strSQL);
            }
        ...
        {
            ArrayList al = new ArrayList();
            FS.HISFC.Models.Pharmacy.DeptConstant DeptConstant; //科室常数实体
            ...
            try
            {
                while (this.Reader.Read())
                {
                    ...
                    DeptConstant.StoreMaxDays = FS.FrameWork.Function.NConvert.ToInt32(this.Reader[2].ToString());   //2 库房最高库存量(天)
                    DeptConstant.StoreMinDays = FS.FrameWork.Function.NConvert.ToInt32(this.Reader[3].ToString());   //3 库房最低库存量(天)
                    DeptConstant.ReferenceDays = FS.FrameWork.Function.NConvert.ToInt32(this.Reader[4].ToString());   //4 库房最低库存量(天)
                    DeptConstant.IsBatch = FS.FrameWork.Function.NConvert.ToBoolean(this.Reader[5].ToString()); //5 是否按批号管理药品
                    DeptConstant.IsStore = FS.FrameWork.Function.NConvert.ToBoolean(this.Reader[6].ToString()); //6 是否管理药品库存
        ...
        private string[] myGetParmDeptConstant(FS.HISFC.Models.Pharmacy.DeptConstant DeptConstant)
        {
            string[] strParm ={   
								 ...
								 FS.FrameWork.Function.NConvert.ToInt32(DeptConstant.IsBatch).ToString(), //4 是否按批号管理药品
								 FS.FrameWork.Function.NConvert.ToInt32(DeptConstant.IsStore).ToString(), //5 是否管理药品库存
        ...
        public DataTable GetSpeDruCompareDepOrPerson(FS.HISFC.Models.Pharmacy.EnumDrugSpecialType speType)
        {
            ...
            {
                this.Err = "获取索引为Pharmacy.Constant.GetSpeDruCompareDepOrPerson SQL出错";
                return null;
            }
            //            strSql = @"select spe_code,spe_name,drug_code,drug_name,memo,
            //decode(valid,'0','有效','1','无效') from PHA_COM_SPEDRUG_PER_DEP
            //where spe_type = '{0}'";
            strSql = string.Format(strSql, (int)speType);
            if (ExecQuery(strSql, ref ds) == -1)
            {
                this.Err = "执行索引为Pharmacy.Constant.GetSpeDruCompareDepOrPerson SQL语句出错";
                return null;
            }
            dt = ds.Tables[0];
            return dt;
        ...
        public int DelSpeDrugDepOrPerCompare(FS.HISFC.Models.Pharmacy.EnumDrugSpecialType speType)
        {
            ...
            {
                this.Err = "获取索引为Pharmacy.Constant.DelSpeDrugDepOrPerCompare SQL出错";
                return -1;
            }
            ...
            {
                this.Err = "执行索引为Pharmacy.Constant.DelSpeDrugDepOrPerCompare SQL语句出错";
                return -1;
            }
            return 0;
        }
        ...
        public int InsertSpeDrugDepOrPerCompare(FS.HISFC.Models.Pharmacy.DrugSpecial drugSpe)
        {
            ...
            {
                this.Err = "获取索引为Pharmacy.Constant.InsertSpeDrugDepOrPerCompare SQL出错";
                return -1;
            }
            ...
            if (ExecNoQuery(strSql) == -1)
            {
                this.Err = "执行索引为Pharmacy.Constant.InsertSpeDrugDepOrPerCompare SQL语句出错";
                return -1;
            }
            return 0;
        }
        ...
        public int DelSpeDrugCompare(FS.HISFC.Models.Pharmacy.DrugSpecial drugSpe)
        {
            ...
            {
                this.Err = "获取索引为Pharmacy.Constant.DelSpeDrugCompare SQL出错";
                return -1;
            }
            //            strSql = @"delete PHA_COM_SPEDRUG_PER_DEP
            ...
            if (ExecNoQuery(strSql) == -1)
            {
                this.Err = "执行索引为Pharmacy.Constant.DelSpeDrugCompare SQL语句出错";
                return -1;
            }
            return 0;
        }
        ...
        public ArrayList QueryAllSpeDrugCode()
        {
            ...
            {
                this.Err = "获取索引为Pharmacy.Constant.QueryAllSpeDrugCode SQL出错";
                return null;
            }
            ...
            {
                this.Err = "执行索引为Pharmacy.Constant.QueryAllSpeDrugCode SQL语句出错";
                return null;
            }
            ...
            try
            {
                while (this.Reader.Read())
                {
                    ...
                    item.Qty = FS.FrameWork.Function.NConvert.ToDecimal(this.Reader[3]);//用来存储上限量
                    item.OnceDose = FS.FrameWork.Function.NConvert.ToDecimal(this.Reader[7]);//用来存储追加量{07AA9FFA-BC72-4443-99F2-85541A03E198}
            ...
            {
                this.Err = "读取索引为SQL出错" + ex.Message;
                return null;
            }
            finally
            {
        ...
        public ArrayList QueryAllSpeDrugPerAndDep()
        {
            ...
            {
                this.Err = "获取索引为Pharmacy.Constant.QueryAllSpeDrugPerAndDep SQL出错";
                return null;
            }
            ...
            if (ExecQuery(strSql) == -1)
            {
                this.Err = "执行索引为Pharmacy.Constant.QueryAllSpeDrugPerAndDep SQL语句出错";
                return null;
            }
        ...
        #region 科室、人员控药属性
        private ArrayList GetAttributeList(string SQLWhere)
        {
            string strSQL = "";
            //取SELECT语句
            if (this.GetSQL("SOC.Pharmacy.Constant.GetAttribute", ref strSQL) == -1)
            {
                strSQL = @"select object_code,
                                   object_type,
                                   attribute_code,
                                   attribute_type,
                                   valid_state,
                                   oper_code,
                                   oper_date
                              from soc_pha_com_controldrug
                            ";
                this.CacheSQL("SOC.Pharmacy.Constant.GetAttribute",strSQL);
            }
            ...
            try
            {
                FS.SOC.HISFC.Models.Pharmacy.Constant.ControlAttribute controlAttribute;
                while (this.Reader.Read())
                {
                    controlAttribute = new FS.SOC.HISFC.Models.Pharmacy.Constant.ControlAttribute();

                    controlAttribute.ObjectCode = this.Reader[0].ToString();
                    controlAttribute.ObjectType = this.Reader[1].ToString();
                    controlAttribute.AttributeCode = this.Reader[2].ToString();
                    controlAttribute.AttributeType = this.Reader[3].ToString();
                    controlAttribute.ValidState = this.Reader[4].ToString();
                    controlAttribute.OperID = this.Reader[5].ToString();
                    controlAttribute.OperTime = FS.FrameWork.Function.NConvert.ToDateTime(this.Reader[6].ToString());

                    al.Add(controlAttribute);
                }
            }
            catch (Exception ex)
            {
        ...
        public ArrayList QueryControlDrugAttribute(string objectType, string attributeType)
        {
            string SQLWhere = "where object_type = '{0}' and attribute_type = '{1}'";
            SQLWhere = string.Format(SQLWhere, objectType, attributeType);
            return this.GetAttributeList(SQLWhere);
        }

        public int DeleteControlDrugAttribute(string objectType, string attributeType)
        {
            string SQL = "delete from soc_pha_com_controldrug where object_type = '{0}' and attribute_type = '{1}'";
            SQL = string.Format(SQL, objectType, attributeType);
            return this.ExecNoQuery(SQL);
        }

        public int DeleteControlDrugAttribute(string objectType, string objectCode, string attributeType)
        {
            string SQL = "delete from soc_pha_com_controldrug where object_type = '{0}' and object_code = '{1}' and attribute_type = '{2}'";
            SQL = string.Format(SQL, objectType, objectCode, attributeType);
            return this.ExecNoQuery(SQL);
        }

        public int InsetControlDrugAttribute(FS.SOC.HISFC.Models.Pharmacy.Constant.ControlAttribute controlAttribute)
        {
            string strSQL = "";
            //取SELECT语句
            if (this.GetSQL("SOC.Pharmacy.Constant.InsertAttribute", ref strSQL) == -1)
            {
                strSQL = @"insert into soc_pha_com_controldrug
                                      (object_code,
                                       object_type,
                                       attribute_code,
                                       attribute_type,
                            ...
                            ";
                this.CacheSQL("SOC.Pharmacy.Constant.InsertAttribute", strSQL);
            }
            strSQL =string.Format(strSQL,
                controlAttribute.ObjectCode,
                controlAttribute.ObjectType,
                controlAttribute.AttributeCode,
                controlAttribute.AttributeType,
                controlAttribute.ValidState,
                controlAttribute.OperID,
                controlAttribute.OperTime.ToString("yyyy-MM-dd HH:mm:ss")
                );
