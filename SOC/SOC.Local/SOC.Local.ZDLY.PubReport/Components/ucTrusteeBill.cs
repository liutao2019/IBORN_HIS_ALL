...
namespace FS.SOC.Local.ZDLY.PubReport.Components
{
    public partial class ucTrusteeBill : UserControl, FS.FrameWork.WinForms.Classes.IControlPrintable
    {
        ...
        {
            InitializeComponent();
            this.lblTop.Text = pubMgr.Hospital.Name + "结帐通知单（托收）";
        }
        ...
        private BizLogic.PubReport pubMgr = new BizLogic.PubReport();
        private Models.PubReport pubRep = null;
        ...
        public void SetData(FS.HISFC.Models.RADT.PatientInfo pinfo, FS.SOC.Local.ZDLY.PubReport.Models.PubReport report)
        {
            //设置患者信息
            ...
            else //出院
            {
                ...
                //出院的托收单默然统计时间为出院时间
                this.lblDate.Text = "(出院)" + "日期:" + report.Begin.ToShortDateString() + "至" + pinfo.PVisit.OutTime.ToShortDateString() + "共" + dt.Days.ToString() + "天";
            ...
            SetFee(report.CancerDrugFee, "审批药", ref i);
            if (!string.IsNullOrEmpty(report.EXT_FLAG))
            {
                SetFee(FS.FrameWork.Function.NConvert.ToDecimal(report.EXT_FLAG), "高检费", ref i);
            }
                ...
                report.Pact.ID == "80" || report.Pact.ID == "90" || report.Pact.ID.Length >= 4)
            {
                ...
                lbsz.Visible = true;
                this.lbsz.Text = FS.FrameWork.Public.String.FormatNumberReturnString(report.ZhenJin - (report.Pay_Rate * report.ZhenJin), 2);
            }
            else
            {
           ...
           // this.lblPaycost.Text = Math.Round((report.Tot_Cost-report.SpDrugFeeTot) * (report.Pay_Rate),2).ToString();
           ...
           // this.lblShiJi.Text = Math.Round(report.Tot_Cost - report.SpDrugFeeTot - (report.Tot_Cost - report.SpDrugFeeTot) * report.Pay_Rate,2).ToString();
