    ...
    {  
        ...
        private int QueryFee(FS.HISFC.Models.RADT.PatientInfo info)
        {
           ...
           /* if (new Function().CheckBeforeBalance(info.ID) == -1)
            ...
            //获取上次结算日期
            ArrayList alBalance = feeMgr.QueryBalancesByInpatientNO(info.ID, "I");
            if (alBalance != null)
            {
                foreach (FS.HISFC.Models.Fee.Inpatient.Balance balancedHead in alBalance)
                {
                    if (balancedHead.CancelType == FS.HISFC.Models.Base.CancelTypes.Valid &&
                        this.dtBegin < balancedHead.EndTime)
                    {
                        this.dtBegin = balancedHead.EndTime.AddSeconds(1);
                    }
                }
            }

            this.dtEnd = info.PVisit.OutTime;
            ...
            if (info.Pact.PayKind.ID == "03")
            {
                ...
                TimeSpan DaysSpan = info.PVisit.OutTime.Date - this.dtBegin;
            ...
            if (this.pInfo.Pact.PayKind.ID == "03")//生育保险除外
            {
                ...
                iReturn = myReport.InsertStatic(this.pInfo, "1", this.dtBegin, this.dtEnd);
                if (iReturn < 0)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                ...
                SOC.Local.ZDLY.PubReport.Models.PubReport pReport = this.myReport.GetPubReport(pInfo.ID,this.dtBegin,this.dtEnd);
                if (pReport == null)
                {
                    IsPrintTr = false;
                }
        ...
        public int SetPatientFee(FS.HISFC.Models.RADT.PatientInfo info)
        {
            ...
            if (info.Pact.PayKind.ID == "03")
            {
                 ...
                 * 取患者上次报表时间,
                 * 否则取入院时间为开始时间*/
                if (RepDate < this.dtBegin)
                {
                    RepDate = this.dtBegin;
                }
                TimeSpan DaysSpan = this.dtEnd.Date - RepDate.Date;
