...
namespace FS.SOC.Local.OutpatientFee.ShenZhen.BinHai.IOutpatientInfomation
{
    public partial class ucPatientInfo : UserControl, FS.HISFC.BizProcess.Integrate.FeeInterface.IOutpatientInfomation
    {
        ...
        public FS.HISFC.Models.Registration.Register PatientInfo
        {
            ...
            set
            {
                ...
                if (patientInfo != null)
                {
                    ...
                    if (patientInfo.DoctorInfo.Templet.Dept.ID == "3110" && patientInfo.DoctorInfo.Templet.RegLevel.ID.Equals("5"))
                    {
        ...
        public ArrayList FeeDetails
        {
            ...
            set
            {
                ...
                {
                    medcareProxy.SetPactCode(this.patientInfo.Pact.ID);
                    medcareProxy.IsLocalProcess = true;
                    ArrayList noChargeFeeDetail = new ArrayList();

                    int lngRes = medcareProxy.LocalBalanceOutpatient(patientInfo, ref value, noChargeFeeDetail);
                    if (lngRes <= 0)
                    {
                        MessageBox.Show("打包收费患者本地结算失败，原因：" + medcareProxy.ErrMsg);
                        return;
                    }

                    this.feeDetails = noChargeFeeDetail;
                }
