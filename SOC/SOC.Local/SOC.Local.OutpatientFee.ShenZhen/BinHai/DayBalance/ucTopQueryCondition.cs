...
namespace FS.SOC.Local.OutpatientFee.ShenZhen.BinHai.DayBalance
{
    public partial class ucTopQueryCondition : FS.FrameWork.WinForms.Controls.ucBaseControl, ITopQueryCondition
    {
        ...
        public int Init()
        {
            ...
            string balanceNO="";
            int i = Function.GetDayReportBizProcess().GetClinicFeeLastBalanceDateAndNO(FS.FrameWork.Management.Connection.Operator.ID, ref beginTime, ref balanceNO);
            if (i < 0)
            {
            ...
            {
                int j = Function.GetDayReportBizProcess().GetClinicFeeDate(FS.FrameWork.Management.Connection.Operator.ID, ref beginFeeDate);
                ...
                if (beginTime < beginFeeDate)
                {
                    ...
                    // 作过日结
                   // this.dtEndTime.Text = beginTime.ToString();
