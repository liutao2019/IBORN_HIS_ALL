    ...
    public partial class ucExecBillFP : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.HISFC.BizProcess.Interface.IPrintTransFusion
    {
        public ucExecBillFP()
        {
            ...
            if (!DesignMode)
            {
                ...
                this.btSaveXml.Click += new EventHandler(btSaveXml_Click);
            }
        }
        ...
        /// 执行单列宽调整
        /// </summary>
        string execXML = FS.FrameWork.WinForms.Classes.Function.CurrentPath + @".\Profile\execBill.xml";

        /// <summary>
        /// 是否自动保存列宽
        /// </summary>
        private bool isAutoSaveXml = false;
        ...
        /// 是否按照现实的内容长度打印走纸
        /// </summary>
        private bool isPrintAsShow = true;
        ...
        public void Print()
        {
            try
            {
                ...
                //if (this.isPrintAsShow)
                //{
                //    pSize.Width=this
                //}
                //else
                //{
                    pSize = pgMgr.GetPageSize("execBillPaper");
                //}
        ...
        protected void SetValues(ArrayList alValues)
        {
           // curValues = alValues;
            if (alValues != null)
            {
                ...
                if (System.IO.File.Exists(execXML))
                {
                    FS.FrameWork.WinForms.Classes.CustomerFp.ReadColumnFormatProperty(this.neuSpread1_Sheet1, execXML);
                }
                else
                {
                    FS.FrameWork.WinForms.Classes.CustomerFp.SaveColumnFormatProperty(neuSpread1_Sheet1, execXML);
                }
                    ...
                    else
                    {
                        ...
                        //{
                            try
                            {
                                ...
                                {
                                    case 1:
                                        if (Classes.Function.GetPhaItem(objExc.Order.Item.ID) != null && Classes.Function.GetPhaItem(objExc.Order.Item.ID).IsAllergy)
                                        {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        void btSaveXml_Click(object sender, EventArgs e)
        {
            try
            {
                FS.FrameWork.WinForms.Classes.CustomerFp.SaveColumnFormatProperty(neuSpread1_Sheet1, execXML);
