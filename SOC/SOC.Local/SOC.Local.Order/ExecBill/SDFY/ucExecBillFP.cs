        ...
        public void Print()
        {
            try
            {
                ...
                else
                {
                    ...
                    ps.Document.DefaultPageSettings.PrinterSettings.FromPage = 1;
                    ps.Document.DefaultPageSettings.PrinterSettings.ToPage = (Int32)Math.Ceiling((decimal)al.Count / 30);

                    int fromPage = 0;
                    int toPage = 0;
                    if (ps.ShowDialog() == DialogResult.OK)
                    {
                        fromPage = ps.Document.DefaultPageSettings.PrinterSettings.FromPage;
                        toPage = ps.Document.DefaultPageSettings.PrinterSettings.ToPage;
                    }
                    #endregion
                    ...
                    for (int page = fromPage; page <= toPage; page++)
                    {
                        ...
                        //FS.FrameWork.WinForms.Classes.Function.PopShowControl(ucPrint);
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            {
                                objExc.Order.Memo += orderManager.TransHypotest((FS.HISFC.Models.Order.EnumHypoTest)hypotest);
                            }
                        }
                        catch
                        {
    ...
    public class ComparerExecOrder : IComparer
    {
        ...
        public int Compare(object x, object y)
        {
            try
            {
                ...
                //string aa = execOrder1.Order.Patient.PVisit.PatientLocation.Bed.ID + execOrder1.Order.Patient.ID + execOrder1.Order.Combo + execOrder1.Order.ID;
                //string bb = execOrder2.Order.Patient.PVisit.PatientLocation.Bed.ID + execOrder2.Order.Patient.ID + execOrder2.Order.Combo + execOrder2.Order.ID;
