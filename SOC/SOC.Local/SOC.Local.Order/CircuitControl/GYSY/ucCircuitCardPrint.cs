...
namespace FS.SOC.Local.Order.CircuitControl.GYSY
{
    public partial class ucCircuitCardPrint : UserControl
    {
        ...
        {
            string erro = "出错";
            string imgpath = FS.FrameWork.WinForms.Classes.Function.CurrentPath + FS.SOC.Local.Order.Classes.Function.GetHospitalLogo("Xml\\HospitalLogoInfo.xml", "Hospital", "Logo", erro);
        ...
        private void SetItems()//构造预览页面内容
        {
            ...
            if (this.neuSpread1_Sheet1.Rows.Count > 0)//有数据
            {
                for (int i = 0; i < this.neuSpread1_Sheet1.RowCount; i++)
                {
                    ...
                    if (FS.SOC.Local.Order.Classes.Function.SubItemNameFP(this.neuSpread1_Sheet1.Cells[i, (int)ExecBillCols.ItemName].Text, 28).Count > 1)
                    {
