...
namespace Neusoft.SOC.Local.Order.NanZhuang
{
    /// <summary>
    /// 处方批量查询
    /// </summary>
    public partial class ucRecipeQuery : Neusoft.SOC.Local.Pharmacy.Base.BaseReport
    {
        ...
        {
            string[] parm = { this.cmbMyType.Tag.ToString(), this.cmbMyDept.Tag.ToString(), this.cmbMyDoct.Tag.ToString(), this.dtpBegin.Value.ToString(), this.dtpEnd.Value.ToString(), this.cmbMyDiag.Text, this.cmbCustomType.Tag.ToString() };
            return parm;
        }
        ...
        protected override int OnPrintPreview(object sender, object neuObject)
        {
            ...
            {
                if (MessageBox.Show("共有" + alRecipeNo.Count.ToString() + "张处方，是否全部预览？", "询问", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                {
                    return 1;
                }
                ...
                foreach (string recipeno in alRecipeNo)
                {
                    ...
                    this.PrintRecipeView(regObj, recipeno);
        ...
        public override int Export(object sender, object neuObject)
        {
            ...
            {
                if (this.fpSpread1_Sheet1.Rows[i].Tag != null && this.fpSpread1_Sheet1.Rows[i].Tag.ToString() == "SpanRow")
                {
                    this.fpSpread1_Sheet1.Cells[i, 1].ColumnSpan = 1;
                }
            }

            base.Export(sender, neuObject);

            for (int i = 0; i < this.fpSpread1_Sheet1.Rows.Count; i++)
            {
                if (this.fpSpread1_Sheet1.Rows[i].Tag != null && this.fpSpread1_Sheet1.Rows[i].Tag.ToString() == "SpanRow")
                {
                    this.fpSpread1_Sheet1.Cells[i, 1].ColumnSpan = this.fpSpread1_Sheet1.ColumnCount - 2;
                }
            }
        ...
        /// <returns></returns>
        private int PrintRecipeView(Neusoft.HISFC.Models.Registration.Register regObj, string recipeNo)
        {
            if (o == null)
            {
