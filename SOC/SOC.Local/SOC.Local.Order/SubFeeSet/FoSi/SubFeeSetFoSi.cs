...
namespace FS.SOC.Local.Order.SubFeeSet
{
    ...
    /// </summary>>
    public class SubFeeSetFoSi : FS.HISFC.BizProcess.Interface.Order.IDealSubjob
    {
        #region 变量
                        ...
                        if (orderObj.ReciptDept.ID == subObj.Dept_code || subObj.Dept_code == "ROOT")
                        {
                            ...
                            if (subObj.CombArea == "0")
                            {
                                ...
                                switch (subObj.FeeRule)
                                {
                                    ...
                                    //1 最大院注
                                    case "1":
                                        undrugItemObj.Qty = orderObj.InjectCount;
                                        break;
                                    //2 组内品种数
                                    case "2":
                                        undrugItemObj.Qty = GetFeeCountByCombOrders(alOrders, orderObj.Combo.ID);
                                        break;
                                    //3 医嘱执行次数
                                    case "3":
                                    ...
                                    //4 频次数
                                    case "4":
                                        undrugItemObj.Qty = Function.GetFrequencyCountByOneDay(orderObj.Frequency.ID);
                                        break;
                                    //5 组内数量合计 目前只用于接瓶费了
                                    case "5":
                                        undrugItemObj.Qty = GetTotalCountByCombOrders(alOrders, orderObj.Combo.ID, "1");
                                        break;
                                }
        ...
        public int DealSubjobOutPatient(FS.HISFC.Models.Registration.Register r, System.Collections.ArrayList alOrders, ref System.Collections.ArrayList alSubOrders, ref string errInfo)
        {
            ...
            if (this.GetSubListByDept(FS.HISFC.Models.Base.ServiceTypes.C, ((FS.HISFC.Models.Base.Employee)orderMgr.Operator).Dept.ID, ref errInfo, ref hsSubList) == -1)
            {
                return -1;
            }
