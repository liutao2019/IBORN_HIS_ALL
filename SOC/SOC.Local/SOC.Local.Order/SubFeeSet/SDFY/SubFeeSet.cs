...
namespace FS.SOC.Local.Order.SubFeeSet.SDFY
{
    /// <summary>
    ...
    /// </summary>>
    public class SubFeeSet : FS.HISFC.BizProcess.Interface.Order.IDealSubjob
    {
        #region 变量
        ...
        /// <returns></returns>
        private bool CheckIsSubPha(FS.HISFC.Models.Pharmacy.Item phaItem, string qualityID)
        {
            ...
            if (phaItem != null && phaItem.NameCollection.RegularSpell.UserCode == "3")
            {
                return true;
            }
            return false;
        ...
        private bool CheckIsMainDrug(FS.HISFC.Models.Order.Order order)
        {
            ...
            if (CheckIsSubPha(phaItemObj, phaItemObj.Quality.ID))
            {
                return false;
            }
        ...
        private int AddToArray(FS.HISFC.Models.Order.OutPatient.Order order, OrderSubtblNew subObj, ArrayList alSubOrder, Hashtable hsSecondFeeOrder, FS.HISFC.Models.Base.Item item, DateTime birthday, ref string errInfo)
        {
            ...
            try
            {
                ...
                //特殊处理组合费用
                //组合费用：静滴才有，每组品种数*天数 合计
                if (subObj.Extend2 == "1")
                {
                    item.Qty = item.Qty * order.HerbalQty;
                }
                ...
                else
                {
                    ...
                    if (!hsSecondFeeOrder.Contains(subObj.Item.ID))
                    {
                        hsSecondFeeOrder.Add(subObj.Item.ID, subObj.Clone());
                    }


                    //if (!hsSecondFeeOrder.Contains(subObj))
                    //{
                    //    hsSecondFeeOrder.Add(subObj, order.Clone());
                    //}
        ...
        /// 收取药袋费的执行档
        /// </summary>
        ArrayList alDrugBagExecOrder = new ArrayList();
        ...
        private int AddToArray(FS.HISFC.Models.Order.Inpatient.Order inOrder, OrderSubtblNew subObj, ArrayList alSubOrder, Hashtable hsSecondFeeOrder, ArrayList alExecOrders, FS.HISFC.Models.Base.Item item, DateTime birthday, ref string errInfo)
        {
            ...
            try
            {
                ...
                {
                    //考虑到可能有时候医嘱分解不执行，所以此处取执行的次数
                    int execCount = 0;

                    //药袋费特殊处理
                    if (subObj.Extend1.Trim() == "1")
                    {
                        foreach (FS.HISFC.Models.Order.ExecOrder execOrder in alDrugBagExecOrder)
                        {
                            if (execOrder.Order.ID == inOrder.ID && execOrder.IsExec)
                            {
                                ...
                                //儿科打印药袋不区分日期
                                if (execOrder.Order.ReciptDept.ID == "0701"//住院小儿科
                                    )
                                {
                                    useTime = "";
                                }
                                ...
                                {
                                    useTime = execOrder.DateUse.ToString("yyyy-MM-dd");
                                }

                                #endregion
                                if (execOrder.Order.User03 == "临嘱")
                                {
                                    if (!hsDrugBagOrder.Contains(execOrder.Order.ID + useTime))
                                    {
                                        execCount = 1;
                                        hsDrugBagOrder.Add(execOrder.Order.ID + useTime, null);
                                    }
                                }
                                else
                                {
                                    //住院药袋按照时间点收取
                                    if (!hsDrugBagTimes.Contains(execOrder.DateUse.ToString("yyyy-MM-dd HH:mm")))
                                    {
                                        execCount += 1;
                                        hsDrugBagTimes.Add(execOrder.DateUse.ToString("yyyy-MM-dd HH:mm"), null);
                                    }
                                }
                            }
                        }
                    ...
                    //{
                    //    //长嘱计算数量 要考虑频次和首日量
                    //    if (feeDate.Date == inOrder.MOTime.Date)
                    //    {
                    //        item.Qty = item.Qty * FS.FrameWork.Function.NConvert.ToInt32(inOrder.ExtendFlag6);
                    //    }
                    //    else
                    //    {
                    //        item.Qty = item.Qty *Function.GetFrequencyCount(inOrder.Frequency.ID);
                    //    }
                    //}
                    //catch { }
        ...
        private int DealSubjobByUsageForOutPatient(ArrayList alOrders, FS.HISFC.Models.Registration.Register regInfo, ref ArrayList alSubOrders, ref string errInfo)
        {
             ...
             * 
             * 药袋费：             * 
             * 1、门诊按照药品带出药袋，即每个药品带出一个药袋
             * 
             * 特殊处理
             * 1、接瓶费：每天组合数减1的累计和=sum(每组天数)-max(天数)
             * 2、组合费： 静滴才有，每组品种数*天数 合计
             * 
             * */
                ...
                foreach (OrderSubtblNew subObj in alSubInfo)
                {
                    ...
                    if (currentOrder.ReciptDept.ID == subObj.Dept_code || subObj.Dept_code == "ROOT")
                    {
                        ...
                        else if (subObj.CombArea == "1")
                        {
                            ...
                            if (!this.hsFirstFeeOrderUsages.Contains(currentOrder.Item.ID)
                                        || hsFirstFeeOrderCombNos.Count == 0
                                ...
                                )
                            {
                                ...
                                if (!(hsSublItem.Contains(undrugItemObj.ID) && !subObj.IsAllowReFee))
                                {
                                    ...
                                    if (!hsFirstFeeOrderCombNos.Contains(currentOrder.Combo.ID))
                                    {
                        ...
                        if (!hsFeeOrderUsages.Contains(sysUsage))
                        {
                            hsFeeOrderUsages.Add(sysUsage, currentOrder.Clone());
                        }
                    }
                }
                #endregion
            ...
            //接瓶费特殊处理   这里考虑 只有VD用法才有第二组加收
            //接瓶费=sum(每天组合数-1)=sum(每组天数)-max(天数)

            //获取组合内最大天数和天数合计
            FS.HISFC.Models.Order.Order orderTemp = null;
            ...
            foreach (ArrayList alCombOrder in alAllCombOrder)
            {
                ...
                //这里用克隆吧，后续可能修改里面的内容
                orderTemp = (alCombOrder[0] as FS.HISFC.Models.Order.Order).Clone();

                //静滴
                if (Function.GetSysType(orderTemp.Usage.ID, ref errInfo).Trim() == "IVD")
                {
                    sumDays += orderTemp.HerbalQty;
                    if (maxDays < orderTemp.HerbalQty)
                    {
                        maxDays = orderTemp.HerbalQty;
                    }
                }
            }

            foreach (string itemCode in hsSecondFeeOrder.Keys)
            {
                outSubOrder = new FS.HISFC.Models.Order.OutPatient.Order();
                outSubOrder = this.CreateOrder(this.curentOrder, itemCode, sumDays - maxDays, ref errInfo);
                if (outSubOrder == null)
                {
                    return -1;
                }
            ...
            //Hashtable hsSecondFeeItems = new Hashtable();
            //Hashtable hsMaxDays = new Hashtable();

            //foreach (FS.HISFC.Models.Order.OrderSubtblNew subObj in hsSecondFeeOrder.Keys)
            //{
            //    FS.HISFC.Models.Order.Order order = hsSecondFeeOrder[subObj] as FS.HISFC.Models.Order.Order;
            //    if (!hsMaxDays.Contains(subObj.Item.ID))
            //    {
            //        hsMaxDays.Add(subObj.Item.ID, order.HerbalQty);
            //    }
            //    else
            //    {
            //        if (FS.FrameWork.Function.NConvert.ToDecimal(hsSecondFeeOrder[subObj.Item.ID]) < order.HerbalQty)
            //        {
            //            hsSecondFeeItems[subObj.Item.ID] = order.HerbalQty;
            //        }
            //    }

            //    if (!hsSecondFeeItems.Contains(subObj.Item.ID))
            //    {
            //        hsSecondFeeItems.Add(subObj.Item.ID, subObj.Qty * order.HerbalQty);
            //    }
            //    else
            //    {
            //        hsSecondFeeItems[subObj.Item.ID] = FS.FrameWork.Function.NConvert.ToDecimal(hsSecondFeeItems[subObj.Item.ID]) + subObj.Qty * order.HerbalQty;
            //    }
            //}

            //decimal itemQty = 0;
            //foreach (string itemCode in hsSecondFeeItems.Keys)
            //{
            //    itemQty = FS.FrameWork.Function.NConvert.ToDecimal(hsSecondFeeItems[itemCode]);
            //    itemQty -= FS.FrameWork.Function.NConvert.ToDecimal(hsMaxDays[itemCode]);
            ...
            //    outSubOrder = new FS.HISFC.Models.Order.OutPatient.Order();
            //    outSubOrder = this.CreateOrder(this.curentOrder, itemCode, itemQty, ref errInfo);
            //    if (outSubOrder == null)
            //    {
            //        return -1;
            //    }
        ...
        private int DealSpecial(FS.HISFC.Models.Order.Order orderObj, ArrayList alCombOrder, FS.HISFC.Models.Base.ServiceTypes type)
        {
            ...
            foreach (FS.HISFC.Models.Order.Order obj in alCombOrder)
            {
                ...
                else
                {
                    ...
                    if (phaItem != null)
                    {
                        if (CheckIsSubPha(phaItem, phaItem.Quality.ID))
                        {
                            continue;
                        }
        ...
        /// 药袋收取的时间点
        /// </summary>
        Hashtable hsDrugBagTimes = new Hashtable();

        /// <summary>
        /// 已经收取药袋费的医嘱执行档
        /// </summary>
        Hashtable hsDrugBagOrder = new Hashtable();
        ...
        private int DealSubjobByUsageForInPatient(bool isRealTime, string usage, FS.HISFC.Models.Order.Inpatient.Order order, ArrayList alOrders, FS.HISFC.Models.RADT.PatientInfo patientInfo, ref ArrayList alSubOrders, ref string errInfo)
        {
             ...
             * 大输液不带任何附材
             * 
             * 住院药袋按照时间点带出，即对于需要带出药袋的用法，有多少个时间点 带出多少个药袋
             * */
            ArrayList alSubInfo = new ArrayList();

            alSubOrders = new ArrayList();

            hsDrugBagTimes.Clear();
            ...
            else
            {
                ...
                //处理收取药袋的执行档
                alDrugBagExecOrder = this.GetOrderListForDrugBag(alExecOrders);

                FS.HISFC.Models.Order.Inpatient.Order orderObj = null;

                //需要带药袋费的医嘱列表
                Hashtable hsDrugBag = new Hashtable();
                        ...
                        foreach (OrderSubtblNew subObj in alSubInfo)
                        {
                            ...
                            if (orderObj.ReciptDept.ID == subObj.Dept_code || subObj.Dept_code == "ROOT")
                            {
                                ...
                                if (subObj.CombArea == "1")
                                {
                                    ...
                                    if (!hsFirstFeeOrderUsages.Contains(orderObj.Item.ID)
|| (hsFirstFeeOrderCombNos.Count > 0 && hsFirstFeeOrderCombNos.Contains(orderObj.Combo.ID))
                                        )
                                    {
                                        switch (subObj.FeeRule)
                ...
                Hashtable hsItemTemp = new Hashtable();
                foreach (FS.HISFC.Models.Fee.Item.Undrug undrugObj in alSubOrders)
                {
                    if (!hsItemTemp.Contains(undrugObj.ID))
                    {
                        hsItemTemp.Add(undrugObj.ID, undrugObj);
                    }
                    else
                    {
                        ((FS.HISFC.Models.Fee.Item.Undrug)hsItemTemp[undrugObj.ID]).Qty += undrugObj.Qty;
                    }
                }

                alSubOrders = new ArrayList();
                foreach (FS.HISFC.Models.Fee.Item.Undrug undrugObj in hsItemTemp.Values)
                {
                    alSubOrders.Add(undrugObj);
                }
        ...
        /// 特殊科室 用于长嘱打印临嘱单
        /// </summary>
        Hashtable hsSpecialDept = null;

        /// <summary>
        /// 特殊项目 用于长嘱打印临嘱单
        /// </summary>
        Hashtable hsSpecialItem = null;

        /// <summary>
        /// 特殊剂型 用于长嘱打印临嘱单
        /// </summary>
        Hashtable hsSpecialDosageForm = null;
        ...
        /// <returns></returns>
        private ArrayList GetOrderListForDrugBag(ArrayList alExecOrder)
        {
            //---------------------------------------------------------------------
            //修改此处要注意！ 修改收取规则要对应打印规则
            //FS.SOC.Local.DrugStore.FuYou.Inpatient.PrintInterfaceImplement
            //PrintDrugBagBase方法
            //---------------------------------------------------------------------
            ...
            //DateTime dtDrugStoreWorkTime = new DateTime();
            ...
            //if (dtDrugStoreWorkTime == DateTime.MinValue)
            //{
            //    return alExecOrder;
            //}
            ...
            ArrayList alTemp = new ArrayList();
            if (hsSpecialDept == null)
            {
                hsSpecialDept = new Hashtable();
                FS.HISFC.BizProcess.Integrate.Manager inteMgr = new FS.HISFC.BizProcess.Integrate.Manager();
                alTemp = inteMgr.GetConstantList("SpecialDept");
                if (alTemp != null)
                {
                    foreach (FS.HISFC.Models.Base.Const con in alTemp)
                    {
                        if (!hsSpecialDept.Contains(con.ID) && con.IsValid)
                        {
                            hsSpecialDept.Add(con.ID, null);
                        }
                    }
                }
            }
            if (hsSpecialDosageForm == null)
            {
                hsSpecialDosageForm = new Hashtable();
                FS.HISFC.BizProcess.Integrate.Manager inteMgr = new FS.HISFC.BizProcess.Integrate.Manager();
                alTemp = inteMgr.GetConstantList("SpecialDosageForm");
                if (alTemp != null)
                {
                    foreach (FS.HISFC.Models.Base.Const con in alTemp)
                    {
                        if (!hsSpecialDosageForm.Contains(con.ID) && con.IsValid)
                        {
                            hsSpecialDosageForm.Add(con.ID, null);
                        }
                    }
                }
            }
            if (hsSpecialItem == null)
            {
                hsSpecialItem = new Hashtable();
                FS.HISFC.BizProcess.Integrate.Manager inteMgr = new FS.HISFC.BizProcess.Integrate.Manager();
                alTemp = inteMgr.GetConstantList("SpecialItem");
                if (alTemp != null)
                {
                    foreach (FS.HISFC.Models.Base.Const con in alTemp)
                    {
                        if (!hsSpecialItem.Contains(con.ID) && con.IsValid)
                        {
                            hsSpecialItem.Add(con.ID, null);
                        }
                    }
                }
            }
            ...
            ArrayList alDrugBagOrder = new ArrayList();
            foreach (FS.HISFC.Models.Order.ExecOrder execOrder in alExecOrder)
            {
                //药剂科主任描述：即使药房没有打印药袋，科室自行给患者用药时也使用了药袋，所以此处不判断上班时间
                //if (execOrder.DateUse < dtDrugStoreWorkTime)
                //{
                //    continue;
                //}
                ...
                //    1）、住院小儿科的都按照临嘱格式打印
                //    2）、住院的剂型 02 注射剂；03 口服溶液剂 都是按照临嘱格式打印
                //if (execOrder.Order.Patient.PVisit.PatientLocation.Dept.ID == "0701"//住院小儿科
                //    || execOrder.Order.Patient.PVisit.PatientLocation.NurseCell.ID == "8255"//儿科一区护士站
                //    || FS.SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(execOrder.Order.Item.ID).DosageForm.ID == "02"//注射剂
                //    || FS.SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(execOrder.Order.Item.ID).DosageForm.ID == "03"//口服溶液剂
                //   )

                if (hsSpecialDept.Contains(execOrder.Order.Patient.PVisit.PatientLocation.Dept.ID) //特殊科室长嘱按照临嘱打印
                    || hsSpecialDept.Contains(execOrder.Order.Patient.PVisit.PatientLocation.NurseCell.ID) //特殊科室长嘱按照临嘱打印
                    || hsSpecialDosageForm.Contains(FS.SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(execOrder.Order.Item.ID).DosageForm.ID) //所有科室的特殊剂型长嘱都按照临嘱打印
                    || hsSpecialDosageForm.Contains(FS.SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(execOrder.Order.Item.ID).DosageForm.ID + execOrder.Order.Patient.PVisit.PatientLocation.Dept.ID)//某些科室的特殊剂型长嘱按照临嘱打印
                    || hsSpecialDosageForm.Contains(FS.SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(execOrder.Order.Item.ID).DosageForm.ID + execOrder.Order.Patient.PVisit.PatientLocation.NurseCell.ID)//某些科室的特殊剂型长嘱按照临嘱打印
                    || hsSpecialItem.Contains(execOrder.Order.Item.ID)//所有科室的特殊项目长嘱按照临嘱打印
                    || hsSpecialItem.Contains(execOrder.Order.Item.ID + execOrder.Order.Patient.PVisit.PatientLocation.Dept.ID)//某些科室的特殊剂型长嘱按照临嘱打印
                    || hsSpecialItem.Contains(execOrder.Order.Item.ID + execOrder.Order.Patient.PVisit.PatientLocation.NurseCell.ID)//某些科室的特殊剂型长嘱按照临嘱打印
                   )
                {
                    //儿科以外的 按照日期收取
                    execOrder.Order.User03 = "临嘱";
                }
                else
                {
                ...
                alDrugBagOrder.Add(execOrder);
            }
            return alDrugBagOrder;
        }
