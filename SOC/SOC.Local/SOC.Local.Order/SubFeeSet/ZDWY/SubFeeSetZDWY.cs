...
namespace FS.SOC.Local.Order.SubFeeSet.ZDWY
{
    /// <summary>
    /// [功能描述:  ]<br></br>
    /// [创 建 者: ]<br></br>
    /// [创建时间: ]<br></br>
    /// </summary>>
    public class SubFeeSetZDWY : FS.HISFC.BizProcess.Interface.Order.IDealSubjob
    {
        #region 变量
        ...
        private int DealSubjobByUsageForOutPatient(ArrayList alOrders, FS.HISFC.Models.Registration.Register regInfo, ref ArrayList alSubOrders, ref string errInfo)
        {
            ...
            if (!isPopForChose)
            {
                if (this.ctrlMgr.GetControlParam<bool>("HNMZ16", false, true))
                {
                    ...
                    if (lisOrder.Count > 0)
                    {
                         ...
                         * CalculateContainer 
                         * 根据传入的申请项目清单计算使用容器数量。
                         * 入参：aply_itm_type - 申请项目来源(0 - 来自HIS ; 1 - 来自体检 ; 2 - 来自其他)
                         * 入参：dt_aply_itms - 包含申请项目Key的DataTable对象(该DataTable只包含一个名为“src_itm_key”的string类型字段。)
                         * 返回值：包含容器清单的DataSet对象，可能为null。
                         * smpl_id：样本类型ID ， grup_num：项目分组号 ， container_key：容器的Key值 ，fee_code 收费编码, qty：容器数量 
                         * 
                         * CallbackContainerPayState 
                         * 根据容器流水号更新容器的计费状态。
                         * 入参：smpl_flow_id - 容器流水号(唯一性)
                         * 入参：state - 计费状态( 0 - 未计费 ， 1 - 已计费)
                         * 返回值：更新是否成功。(true - 成功 ， false - 失败)
                         * 
                         * 
                         * LoadContainerDict 
                         * LIS系统定义的容器字典列表。 
                         * 
                         * SearchContainerListForHis 
                         * 根据条件查询目标病人的容器清单。
                         * 入参：ptnt_no_type - 病历号类型( 0 - 门诊号 ， 1 - 住院号)
                         * 入参：ptnt_no - 病历号
                         * 入参：sqnc_no - 就诊序号
                         * 入参：state - 计费状态( 0 - 未计费 ， 1 - 已计费)
                         * 特别说明：ptnt_no，sqnc_no两个入参至少传递一个。
                         *      当只传递一个时，另一个传“空字符串”；当两个都传时，忽略sqnc_no
                         * 返回值：包含容器清单的DataSet对象，可能为null。
                         * smpl_flow_id：容器流水号(唯一性)， smpl_id：样本类型ID ， 
                         *      grup_num：项目分组号 ， container_key：容器的Key值 ， qty：容器数量 
                         * 
                         * 
                         * */
                        ...
                        try
                        {
                            ...
                            //LISService.ServiceSoapClient Client = new FS.SOC.Local.Order.LISService.ServiceSoapClient();

                            string url = "http://172.16.92.203:8090/Service.asmx?wsdl";
                            FS.SOC.Local.Order.LISService.ServiceSoapClient Client = new FS.SOC.Local.Order.LISService.ServiceSoapClient(new System.ServiceModel.BasicHttpBinding(), new System.ServiceModel.EndpointAddress(url));
                            

                            DataTable dtLis = new DataTable("LISTable");
                            dtLis.Columns.Add("src_itm_key", System.Type.GetType("System.String"));


                            foreach (FS.HISFC.Models.Order.OutPatient.Order order in lisOrder)
                            {
                                DataRow dr = dtLis.NewRow();
                                dr["src_itm_key"] = order.Item.ID;

                                dtLis.Rows.Add(dr);
                            }

                            FS.HISFC.Models.Order.OutPatient.Order lisOrderObj = lisOrder[0] as FS.HISFC.Models.Order.OutPatient.Order;

                            DataSet dsLis = Client.CalculateContainer(0, dtLis);

                            if (dsLis != null
                                && dsLis.Tables.Count > 0)
                            {
                                DataTable dtLisResult = dsLis.Tables[0];
                                foreach (DataRow drow in dtLisResult.Rows)
                                {
                                    //smpl_id：样本类型ID ， grup_num：项目分组号 ， container_key：容器的Key值 ， fee_code 收费编码, qty：容器数量 
                                    string itemCode = drow["fee_code"].ToString();
                                    if (!itemCode.Contains("F"))
                                    {
                                        string sql=@"select o.item_code from fin_com_undruginfo o
                                                        where o.input_code='63903501'
                                                        and o.valid_state='1'
                                                        and rownum=1";
                                        string strRev = this.orderMgr.ExecSqlReturnOne(sql);
                                        if (!string.IsNullOrEmpty(strRev)
                                            && strRev != "-1")
                                        {
                                            itemCode = strRev;
                                        }
                                    ...
                                    {
                                        string simpID = drow["smpl_id"].ToString();
                                        string grupNo = drow["grup_num"].ToString();
                                        decimal qty = FS.FrameWork.Function.NConvert.ToDecimal(drow["qty"]);

                                        outSubOrder = new FS.HISFC.Models.Order.OutPatient.Order();
                                        outSubOrder = this.CreateOrder(lisOrderObj, itemCode, qty, ref errInfo);
                                        if (outSubOrder == null)
                                        {
                                            FS.FrameWork.WinForms.Classes.Function.ShowBalloonTip(3, "错误", "计算LIS附材出现错误！\r\n\r\n" + errInfo, System.Windows.Forms.ToolTipIcon.Error);
                                            //return -1;
                                        }
                                        else
                                        {
                                ...
                                if (alLisFee.Count > 0)
                                {
                                    foreach (FS.FrameWork.Models.NeuObject con in alLisFee)
                                    {
                                        ...
                                        {
                                            outSubOrder = new FS.HISFC.Models.Order.OutPatient.Order();
                                            outSubOrder = this.CreateOrder(lisOrderObj, con.ID, 1, ref errInfo);
                        ...
                        {
                            Classes.Function.ShowBalloonTip(3, "错误提示", "处理检验项目相关的材料费出现错误！\r\n请联系管理员！\r\n\r\n" + ex.Message, System.Windows.Forms.ToolTipIcon.Warning);
                        }
                    }
                        ...
                        if (order.Item.SysClass.ID.ToString() == "UL")
                        {
                            ...
                            if (isSubFlag)
                            {
                                if (lisOrder.Count > 0)
                                {
                                    ...
                                    foreach (FS.HISFC.Models.Order.Inpatient.Order ordObj in lisOrder)
                                    {
                                        DataRow dr = dtLis.NewRow();
                                        dr["src_itm_key"] = ordObj.Item.ID;

                                        dtLis.Rows.Add(dr);
                                    }

                                    FS.HISFC.Models.Order.Inpatient.Order lisOrderObj = lisOrder[0] as FS.HISFC.Models.Order.Inpatient.Order;
                                        ...
                                        && dsLis.Tables.Count > 0)
                                    {
                                        ...
                                        foreach (DataRow drow in dtLisResult.Rows)
                                        {
                                            ...
                                            if (!string.IsNullOrEmpty(itemCode))
                                            {
                                                ...
                                                FS.HISFC.Models.Fee.Item.Undrug item = inteFeeMgr.GetItem(itemCode);//获得最新项目信息
                                                if (item == null)
                                                {
                                                    errInfo = "获取项目失败，尝试获取项目的项目编码为" + itemCode + inteFeeMgr.Err;
                                                    return -1;
                                                }
                                                item.Qty = qty;
                                            ...
                                            foreach (FS.FrameWork.Models.NeuObject con in alLisFee)
                                            {
                                                if (con.Memo == "UL")
                                                {
                                                    ...
                                                    if (item == null)
                                                    {
                                                        ...
                                                        FS.FrameWork.WinForms.Classes.Function.ShowBalloonTip(3, "错误", "计算LIS附材出现错误！\r\n\r\n" + "获取项目失败，尝试获取项目的项目编码为" + con.ID + inteFeeMgr.Err, System.Windows.Forms.ToolTipIcon.Error);
                                                    }
                                                    else
                                                    {
                                                        item.Qty = 1;
...
//                                    {
//                                        //获得最新项目信息
//                                        FS.HISFC.Models.Fee.Item.Undrug item = inteFeeMgr.GetItem(con.ID);
//                                        if (item == null)
//                                        {
//                                            errInfo = "获取项目失败，尝试获取项目的项目编码为" + con.ID + inteFeeMgr.Err;
//                                            return -1;
                            ...
                            if (subObj.CombArea == "0")
                            {
                                ...
                                switch (subObj.FeeRule)
                                {
                                    ...
                                    case "3":
                                        ...
                                        {
                                            //{01CA12BB-49A1-483e-B329-740C5A21DFB1}
                                            //中草药无视频次
