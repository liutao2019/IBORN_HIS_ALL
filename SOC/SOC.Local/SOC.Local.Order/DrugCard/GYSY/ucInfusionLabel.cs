    ...
    public partial class ucInfusionLabel : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Classes.IControlPrintable
    {
        ...
        public object ControlValue
        {
            ...
            set
            {
                ...
                int iIndex = 1;
                int length = "淫荡邪恶猥琐的长度标咔咔又加啊".Length * 2; //汉字双字节
                ...
                foreach (FS.HISFC.Models.Order.ExecOrder info in alInfusionData)
                {
                    ...
                    if (info.Order.OrderType.Type == FS.HISFC.Models.Order.EnumType.SHORT) //临嘱不显示执行时间
                    {
                        this.lblExeTime.Visible = false;
                    }
                    else if (info.Order.OrderType.Type == FS.HISFC.Models.Order.EnumType.LONG)  //长嘱显示执行时间，隐藏“临”字
                    {
                    ...
                    {
                        FS.HISFC.Models.Pharmacy.Item phaItem = FS.SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(info.Order.Item.ID) as FS.HISFC.Models.Pharmacy.Item;
                    ...
                    {
                        #region 处理瓶签名过行
                        ArrayList al = Function.SubItemNameFP(itemName, length);
                        ...
                        else
                        {
                            ...
                            {
                                iIndex++;
                                this.fpSpread1_Sheet1.Cells[iIndex, 0].Text = al[i].ToString();
