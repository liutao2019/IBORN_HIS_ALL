...
namespace FS.SOC.Local.GYZL.PubReport.BizLogic
{
    /// <summary>
    /// PublicReport 的摘要说明。
    /// </summary>
    public class PubReport : FS.FrameWork.Management.Database
    {
        ...
        public int InsertStatic(FS.HISFC.Models.RADT.PatientInfo p, string flag, DateTime dtBegin, DateTime dtEnd)
        {
            ...
            FS.SOC.Local.GYZL.PubReport.BizLogic.PubReport rp = new PubReport();
            ...
            FS.SOC.Local.GYZL.PubReport.BizLogic.Fee locFee = new Fee();
            ...
            if (flag == "0")//月统计
            {
                ...
                if (isExist)
                {
                    FS.SOC.Local.GYZL.PubReport.Models.PubReport tempReport = new FS.SOC.Local.GYZL.PubReport.Models.PubReport();
            ...
            decimal overLimitDrugFee = rp.GetOverLimitDrugFee(p.ID, pBeginDate, pEndDate);

            FS.SOC.Local.GYZL.PubReport.Models.PubReport report = new FS.SOC.Local.GYZL.PubReport.Models.PubReport();
            ...
            foreach (DataRow row in ds.Tables[0].Rows)
            {
                ...
                if ((report.Pact.ID == "80" || report.Pact.ID == "81" || report.Pact.ID == "82" ||
                    ...
                    report.Pact.ID == "80" || report.Pact.ID == "90" || report.Pact.ID.Length >= 4) && feeCode == "030")
                {
                    totPubCost += pubCost;
            ...
            //查询高检费
            DataSet dsGJundrug = GetPatientTPundrugFee(p.ID, pBeginDate, pEndDate);
            if (dsGJundrug != null && dsGJundrug.Tables.Count > 0 && dsGJundrug.Tables[0].Rows.Count > 0)
            {
                ...
                foreach (DataRow TPdrow in dsGJundrug.Tables[0].Rows)
                {
                    ...
                    {
                        case "004": //床位费
                            report.Bed_Fee -= (gjPayCost + gjPubCost);
                            break;
                        case "015"://化验(2)
           ...
           // report.EXT_FLAG = p.Patient.Sex.User01;  //高检
            ...
            //查询审批肿瘤药
            DataSet dsTPdrug = GetPatientTPdrugFee(p.ID, pBeginDate, pEndDate);
        ...
        public FS.SOC.Local.GYZL.PubReport.Models.PubReport GetPubReport(string InpatientNo, string staticMonth)
        {
            ...
            if (al.Count == 0)
                return null;
            return al[0] as FS.SOC.Local.GYZL.PubReport.Models.PubReport;
        }

        public FS.SOC.Local.GYZL.PubReport.Models.PubReport GetPubReportExt(string InpatientNo, string staticMonth)
        {
            string strSql = "select * from gfhz_main where inpatient_no = '" + InpatientNo + "'  and end_date <= to_date('" + staticMonth + "', 'YYYY-MM-DD HH24:MI:SS')";
        ...
        private ArrayList myPubReport(string SQLString)
        {
            ...
            FS.SOC.Local.GYZL.PubReport.Models.PubReport info;            //返回数组中的摆药单分类信息
            ...
            try
            {
                while (this.Reader.Read())
                {
                    info = new FS.SOC.Local.GYZL.PubReport.Models.PubReport();
                    try
                    {
        ...
        public FS.SOC.Local.GYZL.PubReport.Models.PubReport GetPubReport(string InpatientNo, string begin, string enddate)
        {
            string strSql = "select * from gfhz_main where inpatient_no = '" + InpatientNo + "' and end_date > to_date('" + begin + "', 'YYYY-MM-DD HH24:MI:SS') and end_date <= to_date('" + enddate + "', 'YYYY-MM-DD HH24:MI:SS')";
 ...
 where p.CARD_NO = '{0}'";
        ...
        /// <returns></returns>
        public int InsertPubReport(FS.SOC.Local.GYZL.PubReport.Models.PubReport info)
        {
            string strSql = "";
        ...
        private string[] myGetParmPubReport(FS.SOC.Local.GYZL.PubReport.Models.PubReport info)
        {
       ...
       "and a.charge_date >= to_date('{1}','yyyy-mm-dd hh24:mi:ss') and a.charge_date <= to_date('{2}','yyyy-mm-dd hh24:mi:ss') and a.inpatient_no = '{0}' and a.EXT_FLAG4='4'" +
       "group by b.sort_id,a.fee_code order by b.sort_id,a.fee_code";
       ...
       "group by b.sort_id,a.fee_code order by b.sort_id,a.fee_code";
        ...
        /// <param name="info"></param>
        /// <returns></returns>
        public int UpdatePubReport(FS.SOC.Local.GYZL.PubReport.Models.PubReport info)
        {
            string strSql = "";
