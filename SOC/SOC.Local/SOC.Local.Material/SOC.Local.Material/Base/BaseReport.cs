...
namespace FS.SOC.Local.Material.Base
{
    ...
    public partial class BaseReport : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        protected FS.HISFC.BizLogic.Manager.UserPowerDetailManager priPowerMgr = new FS.HISFC.BizLogic.Manager.UserPowerDetailManager();
        ...
        /// 获取月结时间段的sql
        /// </summary>
        private string getMonthStoreTimeSQL = @"select to_char(from_date,'yyyy-mm-dd hh24:mi:ss')||' 到 '||to_char(to_date,'yyyy-mm-dd hh24:mi:ss') from mat_com_msdept where storage_code ='{0}' order by to_date desc";
        ...
        public string DetailSettingFilePatch
        {
            get
            {
                if (this.detailSettingFilePatch == string.Empty)
                {
                    try
                    {
                        return FS.FrameWork.WinForms.Classes.Function.CurrentPath + @"\Profile\Mat\" + this.FindForm().Text + "detail.xml";
                    }
                    catch
                    {
                        return ""; 
        ...
        public string SettingFilePatch
        {
            get 
            {
                if (this.settingFilePatch == string.Empty)
                {
                    try
                    {
                        return FS.FrameWork.WinForms.Classes.Function.CurrentPath + @"\Profile\Mat\" + this.FindForm().Text + "tot.xml";
                    }
                    catch
                    {
                        return "";
        ...
        /// </summary>
        /// <returns>-1失败 0成功</returns>
        public virtual int queryDetailData()
        {
            if (!this.isNeedDetailData)
            {
                return 0;
        ...
        void cmbDept_SelectedIndexChanged(object sender, EventArgs e)
        {
            //if (this.IsUseMonthStoreTime)
            {
                ...
                if (!string.IsNullOrEmpty(this.getMonthStoreTimeSQL))
                {
                    ...
                    FS.HISFC.BizLogic.Material.BizLogic.Store.StockLogic stockLogic = new FS.HISFC.BizLogic.Material.BizLogic.Store.StockLogic();
                    if (stockLogic.ExecQuery(string.Format(this.getMonthStoreTimeSQL, (this.cmbDept.Tag == null ? "" : this.cmbDept.Tag.ToString())), ref ds) == -1)
                    {
