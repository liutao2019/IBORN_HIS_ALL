...
namespace FS.SOC.Local.Material.Base
{
    public class DataSetHelper
    {
        ...
        public DataTable SelectDistinctByIndexs(string tableName, DataTable sourceTable, string[] fieldIndexs)
        {
            ...
            {
                dt.Columns.Add(sourceTable.Columns[int.Parse(fieldIndexs[i])].Caption, sourceTable.Columns[int.Parse(fieldIndexs[i])].DataType);
            ...
            foreach (DataRow dr in sourceTable.Select("", fields))
            {
                if (lastRow == null || !(RowEqual(lastRow, dr, dt.Columns)))
                {
                    ...
                    for (int i = 0; i < fieldIndexs.Length; i++)
                    {
                        values[i] = dr[int.Parse(fieldIndexs[i])         ];
                    }
                    dt.Rows.Add(values);
                }
            }
