...
namespace SOC.Local.InpatientFee.HeYuan.Hprm.IBalanceInvoicePrintmy
{
    public partial class ucInInvoice : UserControl, FS.HISFC.BizProcess.Interface.FeeInterface.IBalanceInvoicePrintmy
    {
        ...
        {
            InitializeComponent();
            al = conMgr.QueryConstantList("INVOICEPACTCODE");
        }
        ...
        public int Print()
        {
            ...
            FS.HISFC.Models.Base.PageSize ps;
            ps = psManager.GetPageSize("ZYFP");
            if (ps == null)
            {
                ps = new FS.HISFC.Models.Base.PageSize("ZYFP", 787, 400);
                ps.Top = 0;
                ps.Left = 0;
                ps.Printer = this.controlIntegrate.GetControlParam<string>("ZYFP", true, "");
            }
            p.SetPageSize(ps);
            ...
            {
                p.PrintPreview(ps.Left, ps.Top, this);
            }
            else
            {
        ...
        protected int SetPrintValue(FS.HISFC.Models.RADT.PatientInfo patientInfo, FS.HISFC.Models.Fee.Inpatient.Balance balanceHead, ArrayList alBalanceList, bool IsPreview)
        {
            ...
            if (balanceHead.FT.ArrearCost <= 0)
            {
                this.lblPriOwe.Text = string.Empty;
            }
            else
            {
                //欠费
                this.lblPriOwe.Text = FS.FrameWork.Public.String.FormatNumberReturnString(balanceHead.FT.ArrearCost, 2);
            }
