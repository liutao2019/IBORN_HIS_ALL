    ...
    public partial class ucDrugList : UserControl
    {
        ...
        private void SetDrugDeatail(int iRow, FS.HISFC.Models.Pharmacy.ApplyOut info, bool isValid)
        {
            ...
            else
            {
                ...
                this.neuSpread1_Sheet1.SetText(iRow, 8, this.GetOnceDose(info).ToString());
            }
            ...
            if (!string.IsNullOrEmpty(info.Frequency.ID))
            {
                ...
                this.neuSpread1_Sheet1.SetText(iRow, 9 ,this.GetFrequency(info).ToString());
            }
            try
            {
                this.neuSpread1_Sheet1.Cells[iRow, 11].Text = Common.Function.GetOrder(info.OrderNO).HerbalQty.ToString();
            }
            catch { }
