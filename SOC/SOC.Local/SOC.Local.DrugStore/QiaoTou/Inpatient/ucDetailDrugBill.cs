...
namespace FS.SOC.Local.DrugStore.QiaoTou.Inpatient
{
    /// <summary>
    ...
    public partial class ucDetailDrugBill : UserControl, FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.IInpatientBill
    {
        ...
        private void ShowDetailData(ArrayList alData, FS.HISFC.Models.Pharmacy.DrugBillClass drugBillClass, FS.FrameWork.Models.NeuObject stockDept)
        {
            ...
            foreach (FS.HISFC.Models.Pharmacy.ApplyOut info in alData)
            {
                ...
                {
                    string age = "";
                    //>>{57612C38-AA8A-452c-9EE6-40992338F2D1}取住院号
                    string patientNo = "";
                    FS.HISFC.Models.RADT.PatientInfo currentPatient;
                    try
                    {
                        currentPatient = inpatientManager.GetPatientInfoByPatientNO(info.PatientNO);
                        age = inpatientManager.GetAge(currentPatient.Birthday);
                        patientNo = currentPatient.PID.PatientNO;
                    ...
                    patientInfo = string.Format("{0}  {1}住院号：{2}   年龄：{3}", bedNO, SOC.Public.String.PadRight(privPatientName,8,' '), patientNo, age);
                    //<<
                    //如果不是在本页的第一行，需要患者信息
        ...
        private class CompareApplyOutByPatient : IComparer
        {
            ...
            public int Compare(object x, object y)
            {
                ...
                //{B27F9742-DC49-4515-9975-5049C540C720}20120606排序修改
                oX = o1.BedNO.Replace("A", "1").Replace("B","2").Replace("+", "").PadLeft(4,'0') + o1.PatientName + this.GetFrequencySortNO(o1.Frequency) + this.GetOrderNo(o1) + o1.UseTime.ToString();
                oY = o2.BedNO.Replace("A", "1").Replace("B","2").Replace("+", "").PadLeft(4, '0') + o2.PatientName + this.GetFrequencySortNO(o2.Frequency) + this.GetOrderNo(o2) + o2.UseTime.ToString();
