...
{
    public partial class ucComboDrugLabel5 : UserControl
    {
        public ucComboDrugLabel5()
        {
            InitializeComponent();
        }
        ...
        {
            string onceDose = string.Empty;
            if (SOC.HISFC.BizProcess.Cache.Common.IsInjectUsage(applyOut.Usage.ID))
            {
                onceDose = onceDose + applyOut.DoseOnce.ToString() + applyOut.Item.DoseUnit;
            }
            else
            {
                onceDose = onceDose + Common.Function.GetOnceDose(applyOut);
            }
        ...
        public int PrintDrugLabel(System.Collections.ArrayList alData, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal, bool isPrintRegularName, string hospitalName, bool isPrintMemo, string qtyShowType, DateTime printTime, string pageNO)
        {
            ...
            {
                FS.HISFC.Models.Pharmacy.ApplyOut apply = alData[index] as FS.HISFC.Models.Pharmacy.ApplyOut;

                FS.HISFC.Models.Order.OutPatient.Order ord = FS.SOC.Local.DrugStore.Common.Function.GetOrder(apply.OrderNO);
                if (ord == null)
                {
                    return 0;
                }
            ...
            this.nlbDrugInfo5.Text = itemName;
            this.nlbSpecs5.Text = applyOut.Item.Specs;
            this.nlbOnceDose5.Text = "每次" + this.GetOnceDose(applyOut);
            //数量显示处理
            applyPackQty = "";
            if (qtyShowType == "包装单位")
            {
            ...
            this.nlbDrugQty5.Text = "总量" + applyPackQty;
            order = SOC.Local.DrugStore.Common.Function.GetOrder(applyOut.OrderNO);
            if (order != null)
            {
                switch ((Int32)order.HypoTest)
                {
                    case 1:
                        if (item.IsAllergy)
                        {
                            this.nlbDrugInfo5.Text += "(免试)";
                        }
                        break;
                    case 2:
                        this.nlbDrugInfo5.Text += "(皮试)";
                        break;
                    case 3:
                        this.nlbDrugInfo5.Text += "(阳性)";
                        break;
                    case 4:
                        this.nlbDrugInfo5.Text += "(阴性)";
                        break;
                    default:
                        break;
                }
