...
namespace FS.SOC.Local.DrugStore.ZhangCha.Inpatient
{
    /// <summary>
    ...
    public partial class ucDetailDrugBill : UserControl, FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.IInpatientBill
    {
        ...
        private void PrintPage()
        {
            ...
            {
                //因为打印机驱动Epson LQ1600K不能识别纸张高度 > 9000，所以纸张的高度限制在9000
                if (paperSize.Height > 9000)
                {
                    int allH = paperSize.Height;
                    int rowH = (int)this.neuSpread1.ActiveSheet.Rows[0].Height;
                    int pageIndex = 0;
                    int pageNum = FS.FrameWork.Function.NConvert.ToInt32(Math.Floor(FS.FrameWork.Function.NConvert.ToDecimal((9000 - 120) / rowH)));

                    for (int j = 0; allH > 0; j++)
                    {
                        if (allH - 9000 > 0)
                        {
                            for (int k = 0; k < this.neuSpread1_Sheet1.Rows.Count; k++)
                            {
                                if (k >= pageNum * pageIndex && k < pageNum * (pageIndex + 1))
                                {
                                    this.neuSpread1_Sheet1.Rows[k].Visible = true;
                                }
                                else
                                {
                                    this.neuSpread1_Sheet1.Rows[k].Visible = false;
                                }
                            }
                            
                            paperSize.Height = 9000;
                        ...
                        else
                        {
                            for (int k = 0; k < this.neuSpread1_Sheet1.Rows.Count; k++)
                            {
                                if (k >= pageNum * pageIndex && k < this.neuSpread1_Sheet1.Rows.Count)
                                {
                                    this.neuSpread1_Sheet1.Rows[k].Visible = true;
                                }
                                else
