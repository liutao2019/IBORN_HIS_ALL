...
namespace FS.SOC.Local.DrugStore.Common
{
    /// <summary>
    /// [功能描述: 本地化公用函数]<br></br>
    /// [创 建 者: cube]<br></br>
    ...
    public class Function
    {
        ...
        /// <returns></returns>
        public static string GetOnceDoseFoSi(FS.HISFC.Models.Pharmacy.ApplyOut applyOut)
        {
            //如果剂量单位空
            ...
            decimal baseDose = item.BaseDose;
            if (baseDose == 0)
            {
                return ConvertDoseOnce(applyOut.DoseOnce, applyOut.Item.DoseUnit) + applyOut.Item.DoseUnit;
            }
            ...
            else if (doseQty >= 0.48m && doseQty <= 0.52m)
            {
                onceDose = "半" + applyOut.Item.MinUnit;
            }
            ...
            else if (doseQty >= 0.13m && doseQty <= 0.17m)
            {
                onceDose = "1/6" + applyOut.Item.MinUnit;
            }
            else if (doseQty >= 0.12m && doseQty <= 0.13m)
            {
                onceDose = "1/8" + applyOut.Item.MinUnit;
            }
            else if (doseQty == 0.1m)
            {
                onceDose = "1/10" + applyOut.Item.MinUnit;
            }
        ...
        /// <returns></returns>
        private static string ConvertDoseOnceFoSi(decimal value, string doseUnit)
        {
            ...
            else if (value >= 0.48m && value <= 0.52m)
            {
                return "半";
            }
            ...
            else if (value >= 0.13m && value <= 0.17m)
            {
                return "1/6";
            }
            else if (value == 0.125m)
            {
                return "1/8";
            }
            else if (value == 0.1m)
