...
namespace Neusoft.SOC.Local.DrugStore.FuYou.Outpatient
{
    ...
    public partial class frmWorkLoad : Form
    {
        ...
        public void Init(string deptNO)
        {
            ...
            this.isNeedConfirmOper = Neusoft.FrameWork.Function.NConvert.ToBoolean(controlValue);
            this.cmbConfirmOper.Enabled = this.isNeedConfirmOper;

            ArrayList al = personMgr.GetEmployee(Neusoft.HISFC.Models.Base.EnumEmployeeType.P);
            ArrayList alOwnDeptEmployee = new ArrayList();
            foreach (Neusoft.HISFC.Models.Base.Employee e in al)
            {
                if (e.Dept.ID == deptNO)
                {
        ...
        private void SetOper(string operCode, Neusoft.FrameWork.WinForms.Controls.NeuComboBox neuComboBox)
        {
            if (!string.IsNullOrEmpty(operCode))
            {
                neuComboBox.Tag = operCode;
                foreach (Neusoft.HISFC.Models.Base.Employee e in neuComboBox.alItems)
                {
                    if (e.ID == operCode)
                    {
