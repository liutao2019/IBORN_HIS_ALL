...
namespace FS.SOC.Local.DrugStore.GYSY.Outpatient
{
    ...
    public class WorkLoadInterfaceImplement : FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.IOutpatientWorkLoad
    {
        ...
        public string Set(FS.FrameWork.Models.NeuObject dept, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, string type, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal)
        {
            ...
            {
                //直接发药，计入发药工作量
                Outpatient.frmWorkLoad frmWorkLoad = new frmWorkLoad("1", drugRecipe.StockDept.ID, dept.ID, drugRecipe.RecipeQty, drugRecipe.RecipeNO, drugRecipe.PatientName);
                frmWorkLoad.emplID = drugRecipe.SendOper.ID;
                frmWorkLoad.SetWorkLoad();//不弹出扫描框，直接记录当前登录人员
            }
            else if (type == "1")
            {
                //配药发药，计入配药工作量
                Outpatient.frmWorkLoad frmWorkLoad = new frmWorkLoad("0", drugRecipe.StockDept.ID, dept.ID, drugRecipe.RecipeQty, drugRecipe.RecipeNO, drugRecipe.PatientName);
                frmWorkLoad.ShowDialog();
            }
            else if (type == "2")
            {
                //发药，计入发药工作量
                //FS.SOC.Local.DrugStore.Common.WorkLoadManager workLoadManager = new FS.SOC.Local.DrugStore.Common.WorkLoadManager();
                ...
                Outpatient.frmWorkLoad frmWordLoad = new frmWorkLoad("1", drugRecipe.StockDept.ID, dept.ID, drugRecipe.RecipeQty, drugRecipe.RecipeNO, drugRecipe.PatientName);
                frmWordLoad.ShowDialog();
            }
            return "";
        }
