    ...
    public class PrintInterfaceImplement : FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.IOutpatientDrug
    {
            ...
            if (al.Count == 0)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < al.Count; i++)
                    {
                        ...
                        diagNose += (i + 1)+"、" + diag.DiagInfo.ICD10.Name + " ";
                    }
                    return diagNose;
                }
            }
        ...
        public int OnAutoPrint(System.Collections.ArrayList alData, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, string type, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal)
        {
            ...
            else if (drugTerminal.TerimalPrintType == FS.HISFC.Models.Pharmacy.EnumClinicPrintType.扩展)
            {
                ...
                if (alPZ.Count > 0)
                {
                    ...
                    //借用字段，做提示使用
                    if (alPO.Count > 0 && alZJ.Count > 0)
                    {
                        drugRecipe.DrugedOper.Memo = "该方有:\n" + "口服药:" + alPO.Count + "种\n" + "针剂  :" + alZJ.Count + "种";
                    }
                    //西药、中成药的清单格式
                   // Outpatient.ucDrugList ucDrugList = new ucDrugList(); 
                    if (alPO.Count > 0)
                    {
                        //ucDrugList.PrintDrugBill(alPO, this.GetDiagnose(drugRecipe.ClinicNO), drugRecipe, drugTerminal, HospitalName);
                        this.myPrintDrugLabel(alPO, drugRecipe, type, drugTerminal);
                    }
                    if (alZJ.Count > 0)
                    {
                        //ucDrugList.PrintDrugBill(alZJ, this.GetDiagnose(drugRecipe.ClinicNO), drugRecipe, drugTerminal, HospitalName);
                        this.myPrintDrugLabel(alZJ, drugRecipe, type, drugTerminal);
                    }
        ...
        private int myPrintDrugLabel(System.Collections.ArrayList alData, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, string type, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal)
        {
            ...
            //张槎项目（南庄模式）
             ...
             if (SOC.HISFC.BizProcess.Cache.Common.IsInjectUsage(applyInfo.Usage.ID) && drugTerminal.TerimalPrintType == FS.HISFC.Models.Pharmacy.EnumClinicPrintType.扩展 && (drugRecipe.RecipeState == "0"))
             {

                 Outpatient.ucReicpeInfoLabelZJ ucReicpeInfoLabelZJ = new Outpatient.ucReicpeInfoLabelZJ();
            ...
            foreach (string comboNO in alSort)
            {
                ...
                if (alPrintData.Count == 1)
                {
                    ...
                    if (SOC.HISFC.BizProcess.Cache.Common.IsInjectUsage(appInfo.Usage.ID) && drugTerminal.TerimalPrintType == FS.HISFC.Models.Pharmacy.EnumClinicPrintType.扩展&&(drugRecipe.RecipeState=="0"))
                    {
                        Outpatient.ucDrugLabelZJ ucDrugLabelZJ = new ucDrugLabelZJ();
                ...
                else if (alPrintData.Count == 2)
                {
                    ...
                    if (SOC.HISFC.BizProcess.Cache.Common.IsInjectUsage(appInfo.Usage.ID) && drugTerminal.TerimalPrintType == FS.HISFC.Models.Pharmacy.EnumClinicPrintType.扩展 && (drugRecipe.RecipeState == "0"))
                    {
                        Outpatient.ucComboDrugLabelZJ ucDrugLabelZJ = new ucComboDrugLabelZJ();
                        ucDrugLabelZJ.PrintDrugLabel(alPrintData, drugRecipe, drugTerminal, this.IsLabelPrintRegularName, HospitalName, IsPrintMemo, QtyShowType, printTime, curPageNO.ToString() + "/" + totPageNO.ToString());
                    }
                    else
                    {
                ...
                else if (alPrintData.Count == 3)
                {
                    ...
                    if (SOC.HISFC.BizProcess.Cache.Common.IsInjectUsage(appInfo.Usage.ID) && drugTerminal.TerimalPrintType == FS.HISFC.Models.Pharmacy.EnumClinicPrintType.扩展 && (drugRecipe.RecipeState == "0"))
                    {
                        Outpatient.ucComboDrugLabel1ZJ ucDrugLabelZJ = new Outpatient.ucComboDrugLabel1ZJ();
                        ucDrugLabelZJ.PrintDrugLabel(alPrintData, drugRecipe, drugTerminal, this.IsLabelPrintRegularName, HospitalName, IsPrintMemo, QtyShowType, printTime, curPageNO.ToString() + "/" + totPageNO.ToString());
                    }
                    else
                    {
                ...
                else if (alPrintData.Count == 4)
                {
                    ...
                    if (SOC.HISFC.BizProcess.Cache.Common.IsInjectUsage(appInfo.Usage.ID) && drugTerminal.TerimalPrintType == FS.HISFC.Models.Pharmacy.EnumClinicPrintType.扩展 && (drugRecipe.RecipeState == "0"))
                    {
                        Outpatient.ucComboDrugLabel2ZJ ucDrugLabelZJ = new ucComboDrugLabel2ZJ();
                        ucDrugLabelZJ.PrintDrugLabel(alPrintData, drugRecipe, drugTerminal, this.IsLabelPrintRegularName, HospitalName, IsPrintMemo, QtyShowType, printTime, curPageNO.ToString() + "/" + totPageNO.ToString());
                    }
                    else
                    {
