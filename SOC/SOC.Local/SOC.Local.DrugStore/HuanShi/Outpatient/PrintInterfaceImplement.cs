    ...
    public class PrintInterfaceImplement : FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.IOutpatientDrug
    {
        ...
        public int OnAutoPrint(System.Collections.ArrayList alData, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, string type, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal)
        {
            ...
            if (drugTerminal.TerimalPrintType == FS.HISFC.Models.Pharmacy.EnumClinicPrintType.标签)
            {
                ...
                if (alPCC.Count > 0)
                {
                    HuanShi.Outpatient.ucHerbalDrugList ucHerbalDrugList = new ucHerbalDrugList();
            ...
            else if (drugTerminal.TerimalPrintType == FS.HISFC.Models.Pharmacy.EnumClinicPrintType.扩展)
            {
                ...
                {
                    //西药、中成药的清单格式
                    HuanShi.Outpatient.ucDrugListHS ucDrugListHS = new ucDrugListHS();
                   
                    //南庄口服和针剂分开   环市有独立针剂药房，并且部分外用药需要在针剂药房出注射单，固屏蔽这段
                    ...
                    //if (alPO.Count > 0)
                    //{
                        ucDrugListHS.PrintDrugBill(alPZ, this.GetDiagnose(drugRecipe.ClinicNO), drugRecipe, drugTerminal, HospitalName);
                    //}
                    //if (alZJ.Count > 0)
                    //{
                    //    ucDrugListHS.PrintDrugBill(alZJ, this.GetDiagnose(drugRecipe.ClinicNO), drugRecipe, drugTerminal, HospitalName);
                    //}

                        ArrayList alIV = new ArrayList();
                        FS.HISFC.BizProcess.Integrate.Manager controlParam = new FS.HISFC.BizProcess.Integrate.Manager();
                        ArrayList usageControlParam = controlParam.GetConstantList("USAGE");

                        foreach (FS.HISFC.Models.Pharmacy.ApplyOut applyOut in alPZ)
                        {
                            foreach (FS.HISFC.Models.Base.Const applyOut2 in usageControlParam)
                            {
                                if ((applyOut.Usage.ID == applyOut2.ID)&&(applyOut2.UserCode=="IVD"))
                                {
                        ...
                        this.myPrintDrugLabel2(alIV, drugRecipe, type, drugTerminal);
            ...
            else if (drugTerminal.TerimalPrintType == FS.HISFC.Models.Pharmacy.EnumClinicPrintType.清单)
            {
                ...
                {
                    //西药、中成药的清单格式
                    Outpatient.ucDrugList ucDrugList = new ucDrugList();
                    //南庄口服和针剂分开
                    ArrayList alPO = new ArrayList();
                    ArrayList alZJ = new ArrayList();
                    foreach (FS.HISFC.Models.Pharmacy.ApplyOut applyOut in alPZ)
        ...
        public int PrintDrugBill(System.Collections.ArrayList alData, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal)
        {
            ...
            {
                //西药、中成药的清单格式
                HuanShi.Outpatient.ucDrugList ucDrugList = new ucDrugList();
                //南庄口服和针剂分开
                ArrayList alPO = new ArrayList();
                ArrayList alZJ = new ArrayList();
                foreach (FS.HISFC.Models.Pharmacy.ApplyOut applyOut in alPZ)
        ...
        private int myPrintDrugLabel(System.Collections.ArrayList alData, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, string type, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal)
        {
            ...
            //南庄项目打印标签
            NanZhuang.Outpatient.ucReicpeInfoLabel ucReicpeInfoLabel = new NanZhuang.Outpatient.ucReicpeInfoLabel();
            ...
            if (hsCombo.Values.Count == 0)
            {
                foreach (FS.HISFC.Models.Pharmacy.ApplyOut applyOut in alData)
                {
                    ...
                    NanZhuang.Outpatient.ucDrugLabel ucDrugLabel = new NanZhuang.Outpatient.ucDrugLabel();
            ...
            foreach (string comboNO in alSort)
            {
                ...
                {
                    NanZhuang.Outpatient.ucComboDrugLabel ucDrugLabel = new NanZhuang.Outpatient.ucComboDrugLabel();
                    ucDrugLabel.PrintDrugLabel(alPrintData, drugRecipe, drugTerminal, this.IsLabelPrintRegularName, HospitalName, IsPrintMemo, QtyShowType, printTime, curPageNO.ToString() + "/" + totPageNO.ToString());

                }
                else if (alPrintData.Count == 3)
                {
                    NanZhuang.Outpatient.ucComboDrugLabel1 ucDrugLabel = new NanZhuang.Outpatient.ucComboDrugLabel1();
                    ucDrugLabel.PrintDrugLabel(alPrintData, drugRecipe, drugTerminal, this.IsLabelPrintRegularName, HospitalName, IsPrintMemo, QtyShowType, printTime, curPageNO.ToString() + "/" + totPageNO.ToString());

                }
                else if (alPrintData.Count == 4)
                {
                    NanZhuang.Outpatient.ucComboDrugLabel2 ucDrugLabel = new NanZhuang.Outpatient.ucComboDrugLabel2();
                    ucDrugLabel.PrintDrugLabel(alPrintData, drugRecipe, drugTerminal, this.IsLabelPrintRegularName, HospitalName, IsPrintMemo, QtyShowType, printTime, curPageNO.ToString() + "/" + totPageNO.ToString());
        ...
        //针剂药房打印的是环市本地化的标签
        private int myPrintDrugLabel2(System.Collections.ArrayList alData, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, string type, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal)
        {
            DateTime printTime = DateTime.Now;


            //环市项目打印标签,针剂标签不打印总签
            //HuanShi.Outpatient.ucReicpeInfoLabel ucReicpeInfoLabel = new HuanShi.Outpatient.ucReicpeInfoLabel();
            //ucReicpeInfoLabel.PrintDrugLabel(alData, drugRecipe, drugTerminal, this.IsLabelPrintRegularName, HospitalName, IsPrintMemo, this.GetDiagnose(drugRecipe.ClinicNO), printTime);
            ...
            if (hsCombo.Values.Count == 0)
            {
                foreach (FS.HISFC.Models.Pharmacy.ApplyOut applyOut in alData)
                {
                    ...
                    ArrayList alPrintData = new ArrayList();
                    alPrintData.Add(applyOut);
                    HuanShi.Outpatient.ucDrugLabel ucDrugLabel = new HuanShi.Outpatient.ucDrugLabel();
                    ucDrugLabel.PrintDrugLabel(alPrintData, drugRecipe, drugTerminal, this.IsLabelPrintRegularName, HospitalName, IsPrintMemo, QtyShowType, printTime, curPageNO.ToString() + "/" + totPageNO.ToString());
                }
            }
            foreach (string comboNO in alSort)
            {
                ...
                {
                    HuanShi.Outpatient.ucComboDrugLabel ucDrugLabel = new HuanShi.Outpatient.ucComboDrugLabel();
                    ucDrugLabel.PrintDrugLabel(alPrintData, drugRecipe, drugTerminal, this.IsLabelPrintRegularName, HospitalName, IsPrintMemo, QtyShowType, printTime, curPageNO.ToString() + "/" + totPageNO.ToString());

                }
                else if (alPrintData.Count == 3)
                {
                    HuanShi.Outpatient.ucComboDrugLabel1 ucDrugLabel = new HuanShi.Outpatient.ucComboDrugLabel1();
                    ucDrugLabel.PrintDrugLabel(alPrintData, drugRecipe, drugTerminal, this.IsLabelPrintRegularName, HospitalName, IsPrintMemo, QtyShowType, printTime, curPageNO.ToString() + "/" + totPageNO.ToString());

                }
                else if (alPrintData.Count == 4)
                {
                    HuanShi.Outpatient.ucComboDrugLabel2 ucDrugLabel = new HuanShi.Outpatient.ucComboDrugLabel2();
                    ucDrugLabel.PrintDrugLabel(alPrintData, drugRecipe, drugTerminal, this.IsLabelPrintRegularName, HospitalName, IsPrintMemo, QtyShowType, printTime, curPageNO.ToString() + "/" + totPageNO.ToString());
