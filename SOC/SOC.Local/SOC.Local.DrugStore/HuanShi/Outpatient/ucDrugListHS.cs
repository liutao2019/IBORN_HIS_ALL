...
{
    public partial class ucDrugListHS : UserControl
    {
        public ucDrugListHS()
        {
            InitializeComponent();
        ...
        private void AddAllData(System.Collections.ArrayList al, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, string diagnose, string hospitalName)
        {
            if (al.Count > 0)
            {
                ...
                foreach (KeyValuePair<int, List<FS.HISFC.Models.Pharmacy.ApplyOut>> applist in applyOutPageList)
                {
                    ...
                    //增加组合画线分栏
                    FarPoint.Win.LineBorder lineBorder1 = new FarPoint.Win.LineBorder(System.Drawing.SystemColors.WindowText, 1, false, false, false, true);
                    FarPoint.Win.LineBorder lineBorder2 = new FarPoint.Win.LineBorder(System.Drawing.SystemColors.WindowText, 1, false, true, false, false);
                    //FarPoint.Win.LineBorder lineBorder3 = new FarPoint.Win.LineBorder(System.Drawing.SystemColors.WindowText, 1, false, true, true, false);
                    //FarPoint.Win.LineBorder lineBorder4 = new FarPoint.Win.LineBorder(System.Drawing.SystemColors.WindowText, 1, false, true, false, false);

                    for (int i = 1; i < this.neuSpread1_Sheet1.RowCount; i++)
                    {
                        if (this.neuSpread1_Sheet1.Cells[i, 8].Text == "┓")
                        {
                            for (int k = 0; k < this.neuSpread1_Sheet1.ColumnCount; k++)
                            {
                                this.neuSpread1_Sheet1.Cells[i,k].Border = lineBorder2;
                        ...
                        if (this.neuSpread1_Sheet1.Cells[i, 8].Text == "┛")
                        {
                            for (int k = 0; k < this.neuSpread1_Sheet1.ColumnCount; k++)
                            {
                                this.neuSpread1_Sheet1.Cells[i, k].Border = lineBorder1;
        ...
        private void Print()
        {
            if (isPrint == true)
            {
                ...
                print.SetPageSize(new FS.HISFC.Models.Base.PageSize("OutPatientDrugBill", 860, 550));
        ...
        private void SetLableValue(FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, string diagnose, string hospitalName)
        {
            ...
            this.lbCardNo.Text = "ID：" + drugRecipe.CardNO.ToString();
            //发票号
            ...
            //就诊号
            this.lbClinicNO.Text = "门诊号：" + drugRecipe.ClinicNO;
            //科室名称
            lbDeptName.Text = "科室：" + SOC.HISFC.BizProcess.Cache.Common.GetDeptName(drugRecipe.DoctDept.ID);
            //诊断
            ...
            lbRecipeDate.Text = "收费时间："+FS.FrameWork.Function.NConvert.ToDateTime(drugRecipe.FeeOper.OperTime).ToString("yyyy-MM-dd HH:mm:ss");
            //配药人
            neuLabel1.Text = "发药人：" + SOC.HISFC.BizProcess.Cache.Common.GetEmployeeName(drugRecipe.DrugedOper.ID);
            //打印时间
            lbPrintTime.Text = "打印时间：" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            //就诊时间
            lbRegDate.Text = "就诊时间：" + drugRecipe.RegTime;
            //合同单位
            ...
            //条形码
            this.npbBarCode2.Image = this.CreateBarCode(drugRecipe.RecipeNO);
        ...
        private void SetDrugDeatail(int iRow, FS.HISFC.Models.Pharmacy.ApplyOut info, bool isValid)
        {
            ...
            this.neuSpread1_Sheet1.SetText(iRow, 1, SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(info.Item.ID).NameCollection.RegularName);
            //规格
            //if (string.IsNullOrEmpty(info.Item.SpecialFlag1))
            //{
            //    info.Item.SpecialFlag1 = "  ";
            //}
            this.neuSpread1_Sheet1.SetText(iRow, 2, SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(info.Item.ID).BaseDose+info.Item.DoseUnit );
            //数量
            int outMinQty;
            ...
            try
            {
                this.neuSpread1_Sheet1.SetText(iRow,9, SOC.HISFC.BizProcess.Cache.Common.GetUsage(info.Usage.ID).Name);
            }
            catch { }
            //每次用量
            if (info.DoseOnce == 0)
            {
                this.neuSpread1_Sheet1.SetText(iRow, 7, "遵医嘱");
            }
            else
            {
                this.neuSpread1_Sheet1.SetText(iRow, 7, info.DoseOnce.ToString() + info.Item.DoseUnit);
            }
            //频次
            if (!string.IsNullOrEmpty(info.Frequency.ID))
            {
                ...
                this.neuSpread1_Sheet1.SetText(iRow, 10, info.Frequency.ID);
            }
            //天数
            try
            {
                this.neuSpread1_Sheet1.Cells[iRow, 12].Text = Common.Function.GetOrder(info.OrderNO).HerbalQty.ToString();
            }
            catch { }
            ...
            else
            {
                ...
                FS.SOC.HISFC.Components.Common.Function.DrawCombo(this.neuSpread1_Sheet1, 13, 8);

            }
            //备注添加信息
              try
            {
            this.neuSpread1_Sheet1.Cells[iRow, 17].Text = SOC.Local.DrugStore.Common.Function.GetOrder(info.OrderNO).Memo.ToString();
            }
              catch { }
        }
        ...
        {
            this.neuSpread1_Sheet1.Models.Span.Add(this.rowMaxCount - 1, 0, 1, 17);
            try
            {
