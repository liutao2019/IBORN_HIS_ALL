...
namespace FS.SOC.Local.DrugStore.HuanShi.Outpatient
{
    public partial class ucHerbalDrugList : UserControl
    {
        ...
        public void ShowData(ArrayList alData, string diagnose, DrugRecipe drugRecipe, DrugTerminal drugTerminal, string hospitalName, DateTime printTime, int pageNO, int maxPageNO)
        {
            ...
            {
                this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.RowCount - 1, 0).Value ="共"+(alData.Count)+"种  "+ "共 " + days.ToString("F4").TrimEnd('0').TrimEnd('.') + " 剂  " + memo + "  " + pactInfo;
        ...
        public int Print(ArrayList alData, string diagnose, DrugRecipe drugRecipe, DrugTerminal drugTerminal, string hospitalName, DateTime printTime)
        {
            ...
            #region 标题信息
            //姓名
            lbName.Text = drugRecipe.PatientName + "  " + drugRecipe.Sex.Name + "  " + (new FS.FrameWork.Management.DataBaseManger()).GetAge(drugRecipe.Age)+" 处方号  "+drugRecipe.RecipeNo+" 诊断: "+diagnose;
            //处方号
