    ...
    public partial class ucPrintItinerateLarge : System.Windows.Forms.UserControl, Neusoft.HISFC.BizProcess.Interface.Nurse.IInjectItineratePrint
    {
        ...
        public void Init(ArrayList al)
        {
            ...
            for (int i = 0; i < al.Count; i++)
            {
                ...
                if (info.Item.CancelType == Neusoft.HISFC.Models.Base.CancelTypes.Canceled)
                {
                ...
                //如果是非药品每次量等于总量
                if (info.Item.Item.ItemType == HISFC.Models.Base.EnumItemType.UnDrug)
                {
            ...
            if (pSize == null)
            {
                pSize = new Neusoft.HISFC.Models.Base.PageSize("MZXSK", 812, 583);
                pSize.Top = 0;
                pSize.Left = 0;
            }
            ...
            //屏蔽非药品项目
            foreach (Neusoft.HISFC.Models.Nurse.Inject inject in al)
            {
                if (inject.Item.Item.ItemType == HISFC.Models.Base.EnumItemType.UnDrug)
                {
                    continue;
                }
                DrugList.Add(inject);
            ...
            //总页数
            int totPageNO = 0;
            foreach (Neusoft.HISFC.Models.Nurse.Inject inject in DrugList)
            {
