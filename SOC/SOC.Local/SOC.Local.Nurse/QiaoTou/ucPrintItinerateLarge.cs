...
namespace Neusoft.SOC.Local.Nurse.QiaoTou
{
    public partial class ucPrintItinerateLarge : System.Windows.Forms.UserControl, Neusoft.HISFC.BizProcess.Interface.Nurse.IInjectItineratePrint
    {
        ...
        Neusoft.HISFC.BizLogic.Manager.PageSize pgMgr = null;
        ...
        public void Init(ArrayList al)
        {
            try
            {
                ...
                //重新获取患者信息
                Neusoft.HISFC.Models.Registration.Register regsiter = registerManager.GetByClinic(info.Patient.ID);
                ...
                try
                {
                    ...
                    if (alRecipeNo != null && al.Count > 0)
                    {
                        ...
                        foreach (string recipeNo in alRecipeNo)
                        {
                            ...
                            {
                                if (!invoiceNo.Contains((alFeeDetail[0] as Neusoft.HISFC.Models.Fee.Outpatient.FeeItemList).Invoice.ID))
                                {
                                    if (invoiceNo == "")
                                    {
                                        invoiceNo = (alFeeDetail[0] as Neusoft.HISFC.Models.Fee.Outpatient.FeeItemList).Invoice.ID;
                                    }
                                    else
                                    {
                                        invoiceNo += "/" + (alFeeDetail[0] as Neusoft.HISFC.Models.Fee.Outpatient.FeeItemList).Invoice.ID;
                ...
                pgMgr = new Neusoft.HISFC.BizLogic.Manager.PageSize();
