...
namespace SOC.Local.Nurse.HSHIS
{
    ...
    /// 右下角控制发票号查询还是卡号查询
    /// </summary>
    public partial class ucTreatmentQuery : Neusoft.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        /// 常数维护
        /// </summary>
        private Neusoft.HISFC.BizProcess.Integrate.Manager constantMgr = new Neusoft.HISFC.BizProcess.Integrate.Manager();
        ...
        private Neusoft.HISFC.BizProcess.Interface.Nurse.ITreatmentPrint ucPrint = null;
        #endregion
        ...
        /// 系统类别（默认治疗）
        /// </summary>
        private Neusoft.HISFC.Models.Base.EnumSysClass sysClass = Neusoft.HISFC.Models.Base.EnumSysClass.UZ;
        [Description("显示当前系统类别的项目"), Category("参数设置")]
        public Neusoft.HISFC.Models.Base.EnumSysClass SysClass
        {
            get
            {
        ...
        protected override Neusoft.FrameWork.WinForms.Forms.ToolBarService OnInit(object sender, object neuObject, object param)
        {
            ...
            this.toolBarService.AddToolButton("全不选", "", (int)Neusoft.FrameWork.WinForms.Classes.EnumImageList.Q取消, true, false, null);
        ...
        private int Init()
        {
            ...
            //加载门诊科室
            deptHelper.ArrayObject = this.constantMgr.GetDepartment(Neusoft.HISFC.Models.Base.EnumDepartmentType.C);
            //加载医生列表
            doctHelper.ArrayObject = this.constantMgr.QueryEmployee(Neusoft.HISFC.Models.Base.EnumEmployeeType.D);
            ...
            //加载打印预览
            //获取接口
            ucPrint = Neusoft.FrameWork.WinForms.Classes.UtilInterface.CreateObject(this.GetType(), typeof(Neusoft.HISFC.BizProcess.Interface.Nurse.ITreatmentPrint)) as Neusoft.HISFC.BizProcess.Interface.Nurse.ITreatmentPrint;

            if (ucPrint == null)
            {
                ucPrint = new SOC.Local.Nurse.Print.ucPrintTreatment();
            }
        ...
        private void QueryRegInfo(string cardNo, DateTime beginDate)
        {
            ...
            foreach (Neusoft.HISFC.Models.Registration.Register reg in alRegs)
            {
                ...
                {
                    Neusoft.FrameWork.Models.NeuObject obj = this.constantMgr.GetEmployeeInfo(reg.SeeDoct.ID);
                    if (obj != null)
                    {
                        doctName = obj.Name;
                    }
        ...
        private void QueryInvoiceInfoByCardNo(string cardNo, DateTime beginDate, DateTime endDate)
        {
            ...
            List<Neusoft.HISFC.Models.Fee.Outpatient.Balance> balanceList = new List<Neusoft.HISFC.Models.Fee.Outpatient.Balance>();
            ...
            foreach (Neusoft.HISFC.Models.Fee.Outpatient.Balance balance in balanceList)
            {
                TreeNode treeNode = new TreeNode();
        ...
        private string GetInvoiceTypeName(Neusoft.HISFC.Models.Base.CancelTypes cancelTypes)
        {
            switch (cancelTypes)
            {
        ...
        private void AddDetail(int row, Neusoft.HISFC.Models.Fee.Outpatient.FeeItemList feeItem)
        {
            this.neuSpread1_Sheet1.Rows[row].Tag = feeItem;
            string doctName = this.doctHelper.GetName(feeItem.RecipeOper.ID);
            if (string.IsNullOrEmpty(doctName))
            {
                Neusoft.FrameWork.Models.NeuObject obj = this.constantMgr.GetEmployeeInfo(feeItem.RecipeOper.ID);
                if (obj != null)
                {
                    doctName = obj.Name;
                }
            ...
            {
                Neusoft.FrameWork.Models.NeuObject obj = this.constantMgr.GetDepartment(feeItem.RecipeOper.Dept.ID);
                if (obj != null)
                {
                    deptName = obj.Name;
                }
            ...
            {
                Neusoft.FrameWork.Models.NeuObject obj = this.constantMgr.GetDepartment(feeItem.ExecOper.Dept.ID);
                if (obj != null)
                {
            ...
            if (string.IsNullOrEmpty(feeItem.Order.Memo))
            {
                ...
                Neusoft.HISFC.Models.Order.OutPatient.Order order = orderMgr.GetOneOrder(feeItem.Patient.ID, feeItem.Order.ID);
                if (order != null)
                {
        ...
        private void SetPrintInfo()
        {
            ...
            for (int i = 0; i < this.neuSpread1_Sheet1.RowCount; i++)
            {
                if (Neusoft.FrameWork.Function.NConvert.ToBoolean(this.neuSpread1_Sheet1.Cells[i, (int)ColFeeItem.Selected].Value))
                {
                    if (this.neuSpread1_Sheet1.Rows[i].Tag != null && this.neuSpread1_Sheet1.Rows[i].Tag is Neusoft.HISFC.Models.Fee.Outpatient.FeeItemList)
                    {
        ...
        private void SetFeeInfo(ArrayList alFee)
        {
            //清空表格
            this.neuSpread1_Sheet1.RowCount = 0;
            string currentDept = ((Neusoft.HISFC.Models.Base.Employee)this.registerManager.Operator).Dept.ID;
            //没有任何费用信息
            foreach (Neusoft.HISFC.Models.Fee.Outpatient.FeeItemList feeItem in alFee)
            {
                //显示有效信息
                if (feeItem.CancelType != Neusoft.HISFC.Models.Base.CancelTypes.Valid)
                {
                    continue;
                }
                //药品不显示
                ...
                {
                    if (feeItem.Item.ItemType == Neusoft.HISFC.Models.Base.EnumItemType.UnDrug)
                    {
                        continue;
                    }
                }
                ...
                if (this.isShowAllUsage == false)
                {
                    Neusoft.HISFC.Models.Base.Const constObject = usageHelper.GetObjectFromID(feeItem.Order.Usage.ID) as Neusoft.HISFC.Models.Base.Const;
            ...
            if (this.neuSpread1_Sheet1.RowCount > 0)
            {
                Neusoft.HISFC.Components.Common.Classes.Function.DrawCombo(this.neuSpread1_Sheet1, (int)ColFeeItem.ComboCode, (int)ColFeeItem.Combo);
            }
        }
        ...
        private void SetPatient(Neusoft.HISFC.Models.RADT.Patient reg)
        {
            if (reg == null || reg.ID == "")
            {
                return;
        ...
        {
            if (e.Node != null && e.Node.Tag is Neusoft.HISFC.Models.Registration.Register)
            {
                //查询所有的费用信息
                Neusoft.HISFC.Models.Registration.Register reg = e.Node.Tag as Neusoft.HISFC.Models.Registration.Register;
        ...
        {
            if (e.Node != null && e.Node.Tag is Neusoft.HISFC.Models.Fee.Outpatient.Balance)
            {
                //查询所有的费用信息
                Neusoft.HISFC.Models.Fee.Outpatient.Balance balance = e.Node.Tag as Neusoft.HISFC.Models.Fee.Outpatient.Balance;
                if (balance == null || string.IsNullOrEmpty(balance.Invoice.ID))
                {
                    return;
                }
                Neusoft.HISFC.Models.Registration.Register reg = this.registerManager.GetByClinic(balance.Patient.ID);
                if (reg == null || string.IsNullOrEmpty(reg.ID))
                {
                    return;
                }
        ...
        private void neuSpread1_ButtonClicked(object sender, FarPoint.Win.Spread.EditorNotifyEventArgs e)
        {
            ...
            {
                this.SelectedComb(Neusoft.FrameWork.Function.NConvert.ToBoolean(this.neuSpread1_Sheet1.Cells[e.Row, e.Column].Value));
                this.SetPrintInfo();
            }
        }
