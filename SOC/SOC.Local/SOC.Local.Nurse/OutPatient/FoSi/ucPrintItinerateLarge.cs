    ...
    public partial class ucPrintItinerateLarge : System.Windows.Forms.UserControl, FS.HISFC.BizProcess.Interface.Nurse.IInjectItineratePrint
    {
        ...
        public void Init(ArrayList al)
        {
            ...
            for (int i = 0; i < al.Count; i++)
            {
                ...
                {
                    string str = this.SetStr((info.Item.Order.DoseOnce + info.Item.Order.DoseUnit).ToString());
                    this.neuSpread1_Sheet1.Cells.Get(row, 3).Font = new System.Drawing.Font("宋体", 10.5F, System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, ((byte)(134)));
                    this.neuSpread1_Sheet1.Cells[row, 3].Text = str + remark;
                }
                else
                {
                ...
                this.neuSpread1_Sheet1.Cells[row, 5].Text = info.Item.Order.Frequency.ID.ToLower();
                this.neuSpread1_Sheet1.Cells[row, 5].Tag = info.User03;
                this.neuSpread1_Sheet1.Cells[row, 6].Text = info.Item.Days.ToString();
                //根据处方号，处方内流水号获得费用明细实体
                if (info.Item.CancelType == FS.HISFC.Models.Base.CancelTypes.Canceled)
                {
                    feeItemList = feeManager.GetFeeItemListAndQuitForFee(recipeNO, sequenceNO);
                ...
                //{
                //    this.neuSpread1_Sheet1.Cells[row, 7].Text = feeItemList.Item.Qty.ToString() + feeItemList.Item.PriceUnit.ToString();
                //}
                //else
                //{
                //    this.neuSpread1_Sheet1.Cells[row, 7].Text = (feeItemList.Item.Qty / feeItemList.Item.PackQty).ToString() + feeItemList.Item.PriceUnit.ToString();
                //}
                // 不显示单位
                if (feeItemList.FeePack == "0")
                {
                    this.neuSpread1_Sheet1.Cells[row, 7].Text = feeItemList.Item.Qty.ToString();
                }
                else
                {
                    this.neuSpread1_Sheet1.Cells[row, 7].Text = (feeItemList.Item.Qty/feeItemList.Item.PackQty).ToString();
                }
                //如果是非药品每次量等于总量
                if (info.Item.Item.ItemType == FS.HISFC.Models.Base.EnumItemType.UnDrug)
                {
                    this.neuSpread1_Sheet1.Cells[row, 3].Text = this.neuSpread1_Sheet1.Cells[row, 7].Text;
                }
            }
            this.SetComb(al);
            int seq = 1;
            ...
            string pAge = this.injectMgr.GetAge(info.Patient.Birthday, this.feeManager.GetDateTimeFromSysDateTime());
            string pSex = "";
            ...
            //屏蔽非药品项目
            foreach (FS.HISFC.Models.Nurse.Inject inject in al)
            {
                if (inject.Item.Item.ItemType == FS.HISFC.Models.Base.EnumItemType.UnDrug)
                {
                    continue;
                }
                DrugList.Add(inject);
            ...
            //总页数
            int totPageNO = 0;
            foreach (FS.HISFC.Models.Nurse.Inject inject in DrugList)
            {
                if (hsCombo.Contains(inject.Item.Order.Combo.ID))
                {
                    ArrayList alCombo = hsCombo[inject.Item.Order.Combo.ID] as ArrayList;
                    ...
                    //}
                    alCombo.Add(inject);
                    hsCombo[inject.Item.Order.Combo.ID] = alCombo;
                }
                else
                {
                    ArrayList al1 = new ArrayList();
                    al1.Add(inject);
                    hsCombo.Add(inject.Item.Order.Combo.ID, al1);
                    //alSort.Add(inject.Item.Order.Combo.ID);
                    totPageNO++;
                }
            }
            int curPageNO = 0;
            ...
            //        alPrintData.Add(inject);
            //        if (this.isPrintUseName(alPrintData))
            //        {
            //            FoSi.ucPrintCard ucPrintCard = new FoSi.ucPrintCard();
            //            ucPrintCard.Init(alPrintData);
            //        }
            //    }
            //}
            foreach (string comboNO in hsCombo.Keys)
            {
                curPageNO++;
        ...
        /// <param name="code"></param>
        /// <returns></returns>
        private bool isPrintUseName(ArrayList al)
        {
            ...
            foreach (FS.HISFC.Models.Nurse.Inject inject in al)
            {
                string sql = @"select * from com_dictionary t
                               where t.type ='MZZSBQDYYFWH'
                               and t.name ='{0}'
                               and t.valid_state ='1'";
                sql = string.Format(sql, inject.Item.Order.Usage.Name);
                if (injectMgr.ExecSqlReturnOne(sql) == "-1")
                {
                    return false;
                }
                else
        ...
        private void SetComb(ArrayList al)
        {
            ...
            for (i = 2; i < myCount - 1; i++)
            {
                ...
                // 患者当次注射处方信息
                string currentRowInjectTime = this.neuSpread1_Sheet1.Cells[i, 5].Tag.ToString();
                string priorRowInjectTime = this.neuSpread1_Sheet1.Cells[prior, 5].Tag.ToString();
                string nextRowInjectTime = this.neuSpread1_Sheet1.Cells[next, 5].Tag.ToString();
            ...
            if (myCount == 3)
            {
                ...
                if (this.neuSpread1_Sheet1.Cells[1, 5].Tag.ToString().Trim() != this.neuSpread1_Sheet1.Cells[2, 5].Tag.ToString().Trim())
                {
                    this.neuSpread1_Sheet1.SetValue(1, 2, "");
                    this.neuSpread1_Sheet1.SetValue(2, 2, "");
                }
