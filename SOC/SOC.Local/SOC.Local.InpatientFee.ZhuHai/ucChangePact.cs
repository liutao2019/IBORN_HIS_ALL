...
namespace FS.SOC.Local.InpatientFee.ZhuHai
{
    public partial class ucChangePact : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        protected virtual int DisplayDrugDetail(string inpatientNO)
        {
            ...
            if (alDrung.Count > 0)
            {
                for (int i = 0; i < alDrung.Count; i++)
                {
                    ...
                    if (medcineList.FT.TotCost == 0)
                    {
                        this.fpDrug_Sheet1.Cells[i, 7].Value = "1.00";
                        this.fpDrug_Sheet1.Cells[i, 9].Value = "1.00";
                    }
                    else
                    {
        ...
        protected virtual int DisplayUndrugDetail(string inpatientNO)
        {
            ...
            if (alUnDrung.Count > 0)
            {
                for (int i = 0; i < alUnDrung.Count; i++)
                {
                    ...
                    if (undrugItem.FT.TotCost == 0)
                    {
                        this.fpUndrug_Sheet1.Cells[i, 6].Value = "1.00";
                        this.fpUndrug_Sheet1.Cells[i, 8].Value = "1.00";
                    }
                    else
                    {
        ...
        protected virtual int ChangePact()
        {
            ...
            {
                this.medcareInterfaceProxy.CancelRegInfoInpatient(pubPatientInfo);
                FS.FrameWork.Management.PublicTrans.RollBack();
        ...
        private void ucQueryInpatientNo1_myEvent()
        {
            ...
            {
                FS.FrameWork.WinForms.Classes.Function.Msg("该患者已经无费退院!", 111);
