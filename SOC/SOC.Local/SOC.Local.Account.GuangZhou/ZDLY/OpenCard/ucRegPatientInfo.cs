...
{
    public partial class ucRegPatientInfo : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        private bool isInputLinkPhone = false;
        #endregion
        ...
        [Category("控件设置"), Description("联系电话是否必须输入！")]
        ...
        [Category("控件设置"), Description("家庭地址是否必须输入！")]
        ...
        public virtual bool InputValid()
        {
            ...
            //联系人地址
            if (!FS.FrameWork.Public.String.ValidMaxLengh(this.cmbLinkAddress.Text, 12))
            {
            ...
            //母亲姓名
            if (!FS.FrameWork.Public.String.ValidMaxLengh(this.txtMatherName.Text, 12))
            {
            ...
            if (!FS.FrameWork.Public.String.ValidMaxLengh(this.txtＨomeAddressZip.Text, 6))
            {
        ...
        public int Save()
        {
            ...
            if (string.IsNullOrEmpty(patientInfo.IDCard))
            {
                MessageBox.Show("病人身份证信息为空,请检查输入操作");
                return -1;
            }
        ...
        public int ReadIDCard()
        {
            ...
            {
                //在CardNo为空的时候,去查一下数据库是否存在这个病人
                if (string.IsNullOrEmpty(this.CardNO))
                {
                    //查找是否已存在使用这个身份证的病人信息,有则使用之前的信息
                    string CardNo = accountManager.GetCardNoByIDCard(code);
                ...
                //}
                //根据身份证号获取患者性别
                FS.FrameWork.Models.NeuObject obj = Function.GetSexFromIdNO(code, ref error);
                if (obj == null)
                {
            ...
            try
            {
                if (string.IsNullOrEmpty(photoFileName)) photoFileName = "photo.bmp";
                System.IO.MemoryStream m = new System.IO.MemoryStream();
     ...
     public int AutoReadIDCard()
        ...
        private void ucPatientInfo_Load(object sender, EventArgs e)
        {
            ...
            if (System.Diagnostics.Process.GetCurrentProcess().ProcessName.ToLower() != "devenv")
            {
                ...
                NeuObject dept = (accountManager.Operator as FS.HISFC.Models.Base.Employee).Dept;
                //判断是否有加密权限
                ...
                if (this.txtIDNO.Focused)
                {
                    if (this.cmbCardType.Tag.ToString() == "01")
                    {
                        ...
                        if (idNO != string.Empty)
                        {
                            ...
                            if (string.IsNullOrEmpty(this.cardNO))
                            {
                                string CardNo = accountManager.GetCardNoByIDCard(idNO);
                                if (!string.IsNullOrEmpty(CardNo))
                                {
                                    this.CardNO = CardNo;
                                    return true;
                            ...
                            //根据身份证号获取患者性别
                            FS.FrameWork.Models.NeuObject obj = Function.GetSexFromIdNO(idNO, ref error);
                            if (obj == null)
                            {
                                MessageBox.Show(error);
                                return true;
