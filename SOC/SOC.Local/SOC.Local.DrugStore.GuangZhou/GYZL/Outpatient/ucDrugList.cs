...
//using FS.SOC.Local.DrugStore.GuangZhou.Common;
...
namespace FS.SOC.Local.DrugStore.GuangZhou.GYZL.Outpatient
{
    public partial class ucDrugList : UserControl
    {
        ...
        private void AddAllData(System.Collections.ArrayList al, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, string diagnose,string hospitalName)
        {
            if (al.Count > 0)
            {
                ...
                foreach (KeyValuePair<int, List<FS.HISFC.Models.Pharmacy.ApplyOut>> applist in applyOutPageList)
                {
                    ...
                    {
                        string placeNo = SOC.HISFC.BizProcess.Cache.Pharmacy.GetItemPlaceNo(applayOut.StockDept.ID, applayOut.Item.ID);
                        applayOut.PlaceNO = placeNo;
                        alNew.Add(applayOut);
                    }
                    alNew.Sort(new CompareByPlaceNO());
                    foreach (FS.HISFC.Models.Pharmacy.ApplyOut applayOut in alNew)//applist.Value)
                    {
        ...
        {
            //增加货位号
            string placeNo = SOC.HISFC.BizProcess.Cache.Pharmacy.GetItemPlaceNo(info.StockDept.ID, info.Item.ID);
            ...
            {
                this.neuSpread1_Sheet1.SetText(iRow, 0, "[" + placeNo + "]");
            }
            //药品名
            this.neuSpread1_Sheet1.SetText(iRow, 1, info.Item.Name + "(" + info.Item.Specs + ")");
            //规格
            ...
            if (outPackQty == 0)
            {
                this.neuSpread1_Sheet1.SetText(iRow, 3, ((int)(info.Operation.ApplyQty)).ToString("F4").TrimEnd('0').TrimEnd('.') + info.Item.MinUnit);
                //单位
            ...
            //总药价
            this.drugListTotalPrice += (info.Item.PriceCollection.RetailPrice / info.Item.PackQty) * (info.Operation.ApplyQty );
            ...
            try
            {
                this.neuSpread1_Sheet1.Cells[iRow,11].Text = FS.SOC.Local.DrugStore.GuangZhou.Common.Function.GetOrder(info.OrderNO).HerbalQty.ToString();
            }
            catch { }
        ...
        {
            this.neuSpread1_Sheet1.Models.Span.Add(this.rowMaxCount - 2, 0, 1, 12);
            try
            {
                this.neuSpread1_Sheet1.SetText(this.rowMaxCount - 2, 0, string.Format("总药价：￥{0}               打印时间：{1}    {2}",
            ...
            this.neuSpread1_Sheet1.Cells.Get(this.rowMaxCount - 2, 0).Border = lineBorder11;
            this.neuSpread1_Sheet1.Cells.Get(this.rowMaxCount - 2, 0).Font = new System.Drawing.Font("宋体", 9F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(134)));
            this.neuSpread1_Sheet1.Cells.Get(this.rowMaxCount - 2, 0).HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Left;

            
            this.neuSpread1_Sheet1.Models.Span.Add(this.rowMaxCount - 1, 0, 1, 12);
            try
            {
                this.neuSpread1_Sheet1.SetText(this.rowMaxCount - 1, 0, this.lbDoctor.Text + "        " + "配药人：     " + "发药人：" + this.lbRecipeDate.Text);
            }
            catch { }
    ...
    public class CompareByPlaceNO : System.Collections.IComparer
    {
        public int Compare(object x, object y)
        {
            ...
            {
                oX = (x as FS.HISFC.Models.Pharmacy.ApplyOut).Clone().PlaceNO;
                oY = (y as FS.HISFC.Models.Pharmacy.ApplyOut).Clone().PlaceNO;
            }
