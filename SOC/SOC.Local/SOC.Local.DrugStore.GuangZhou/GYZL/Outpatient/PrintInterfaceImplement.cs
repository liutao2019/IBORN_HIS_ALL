    ...
    public class PrintInterfaceImplement : FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.IOutpatientDrug
    {
        ...
        public int OnAutoPrint(System.Collections.ArrayList alData, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, string type, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal)
        {
            ...
            if (drugTerminal.TerimalPrintType == FS.HISFC.Models.Pharmacy.EnumClinicPrintType.标签)
            {
                ...
                //清单打印院注项目
                ArrayList alInjectItem = new ArrayList();
                ...
                if (alPZ.Count > 0)
                {
                    ...
                    foreach (FS.HISFC.Models.Pharmacy.ApplyOut applyInfo in alValid)
                    {
                        if (SOC.HISFC.BizProcess.Cache.Common.IsInjectUsage(applyInfo.Usage.ID))
                        {
                            alInjectItem.Add(applyInfo);
                        }
                    }
                    if (alInjectItem.Count > 0)
                    {
                        Outpatient.ucDrugList ucDrugList = new ucDrugList();
                        ucDrugList.PrintDrugBill(alInjectItem, this.GetDiagnose(drugRecipe.ClinicNO), drugRecipe, drugTerminal, HospitalName);
                    }
                }
            }
        ...
        public int PrintDrugBill(System.Collections.ArrayList alData, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal)
        {
            ...
            {
                Outpatient.ucHerbalDrugList ucHerbalDrugList = new ucHerbalDrugList();
                ucHerbalDrugList.Print(alPCC, this.GetDiagnose(drugRecipe.ClinicNO), drugRecipe, drugTerminal, HospitalName, printTime,2);
            }
        ...
        public int PrintRecipe(System.Collections.ArrayList alData, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal)
        {
            ...
            /// 处方打印接口
            /// </summary>
            //FS.HISFC.BizProcess.Interface.IRecipePrint iRecipePrint = FS.FrameWork.WinForms.Classes.UtilInterface.CreateObject(typeof(FS.SOC.Local.DrugStore.GuangZhou.GYZL.Outpatient.PrintInterfaceImplement), typeof(FS.HISFC.BizProcess.Interface.IRecipePrint), 0) as FS.HISFC.BizProcess.Interface.IRecipePrint;
            FS.HISFC.BizProcess.Interface.Pharmacy.IDrugPrint iDrugPrint = FS.FrameWork.WinForms.Classes.UtilInterface.CreateObject(typeof(FS.SOC.Local.DrugStore.GuangZhou.GYZL.Outpatient.PrintInterfaceImplement), typeof(FS.HISFC.BizProcess.Interface.Pharmacy.IDrugPrint), 0) as FS.HISFC.BizProcess.Interface.Pharmacy.IDrugPrint;
            ...
            //iRecipePrint.SetPatientInfo(register);
            //iRecipePrint.RecipeNO = drugRecipe.RecipeNO;
            
            //return iRecipePrint.PrintRecipe();

            iDrugPrint.OutpatientInfo = register;
            iDrugPrint.Print();
        ...
        private int myPrintDrugLabel(System.Collections.ArrayList alData, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, string type, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal)
        {
            ...
            {
                //广医肿瘤排除针剂打印标签
                if (FS.SOC.HISFC.BizProcess.Cache.Common.IsInjectUsage(applyOut.Usage.ID.ToString()))
                {
                    continue;
                }
                if (!string.IsNullOrEmpty(applyOut.CombNO) && hsCombo.Contains(applyOut.CombNO))
                {
                ...
                else if (hsCombo.Contains(applyOut.CombNO))
                {
                ...
                else
                {
                    ArrayList al = new ArrayList();
                    al.Add(applyOut);
                    hsCombo.Add(applyOut.OrderNO, al);
                    alSort.Add(applyOut.OrderNO);
                    totPageNO++;
                }
            }
            int curPageNO = 0;
            ...
            foreach (string comboNO in alSort)
            {
                ...
                {
                    Outpatient.ucComboDrugLabel ucDrugLabel = new Outpatient.ucComboDrugLabel();
                    ucDrugLabel.PrintDrugLabel(alPrintData, drugRecipe, drugTerminal, this.IsLabelPrintRegularName, HospitalName, IsPrintMemo, QtyShowType, printTime, curPageNO.ToString() + "/" + totPageNO.ToString());

                }
                else if (alPrintData.Count == 3)
                {
                    Outpatient.ucComboDrugLabel1 ucDrugLabel = new Outpatient.ucComboDrugLabel1();
                    ucDrugLabel.PrintDrugLabel(alPrintData, drugRecipe, drugTerminal, this.IsLabelPrintRegularName, HospitalName, IsPrintMemo, QtyShowType, printTime, curPageNO.ToString() + "/" + totPageNO.ToString());

                }
                else if (alPrintData.Count == 4)
                {
                    Outpatient.ucComboDrugLabel2 ucDrugLabel = new Outpatient.ucComboDrugLabel2();
                    ucDrugLabel.PrintDrugLabel(alPrintData, drugRecipe, drugTerminal, this.IsLabelPrintRegularName, HospitalName, IsPrintMemo, QtyShowType, printTime, curPageNO.ToString() + "/" + totPageNO.ToString());
