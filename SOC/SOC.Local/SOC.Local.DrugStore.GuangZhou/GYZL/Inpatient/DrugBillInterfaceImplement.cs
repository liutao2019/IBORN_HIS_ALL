...
namespace FS.SOC.Local.DrugStore.GuangZhou.GYZL.Inpatient
{
    ...
    public class DrugBillInterfaceImplement : FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.IDrugBillClass
    {
        ...
        public FS.HISFC.Models.Pharmacy.DrugBillClass GetDrugBillClass(FS.HISFC.Models.Pharmacy.ApplyOut applyOut)
        {
            ...
            string drugQuality = SOC.HISFC.BizProcess.Cache.Common.GetDrugQualitySystem(applyOut.Item.Quality.ID);
            ...
            if (applyOut.OrderType.ID == "CD" || applyOut.OrderType.ID == "QL")
            {
                ...
                if (drugQuality == "S" || drugQuality == "P1")
                {
                    billClass.ID = "OP1";
                    if (hsDrugBillClass.Contains(billClass.ID))
                    {
            ...
            ////根据药品性质，把精一方发送到精一摆药单
            //if (SOC.HISFC.BizProcess.Cache.Common.GetDrugQualitySystem(applyOut.Item.Quality.ID) == "P1")
            //{
            //    billClass.ID = "P1";
            //    if (hsDrugBillClass.Contains(billClass.ID))
            //    {
        ...
        private ArrayList SaveDefaultBill()
        {
            ...
            FS.HISFC.Models.Pharmacy.DrugBillClass p1DrugBill = new FS.HISFC.Models.Pharmacy.DrugBillClass();
            sDrugBill.ID = "P1";
            sDrugBill.Name = "精一摆药单";                       //摆药分类名称
            sDrugBill.PrintType.ID = FS.HISFC.Models.Pharmacy.BillPrintType.enuBillPrintType.T;           //打印类型
            sDrugBill.DrugAttribute.ID = FS.HISFC.Models.Pharmacy.DrugAttribute.enuDrugAttribute.T; //摆药类型
            sDrugBill.IsValid = true;              //是否有效
            sDrugBill.Memo = "系统默认的摆药单";               //备注
            ...
            FS.HISFC.Models.Pharmacy.DrugBillClass p2DrugBill = new FS.HISFC.Models.Pharmacy.DrugBillClass();
            sDrugBill.ID = "P2";
            sDrugBill.Name = "精二摆药单";                       //摆药分类名称
            sDrugBill.PrintType.ID = FS.HISFC.Models.Pharmacy.BillPrintType.enuBillPrintType.T;           //打印类型
            sDrugBill.DrugAttribute.ID = FS.HISFC.Models.Pharmacy.DrugAttribute.enuDrugAttribute.T; //摆药类型
            sDrugBill.IsValid = true;              //是否有效
            sDrugBill.Memo = "系统默认的摆药单";               //备注
