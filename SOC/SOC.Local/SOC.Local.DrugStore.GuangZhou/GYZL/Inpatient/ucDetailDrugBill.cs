...
namespace FS.SOC.Local.DrugStore.GuangZhou.GYZL.Inpatient
{
    ...
    public partial class ucDetailDrugBill : UserControl, FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.IInpatientBill
    {
        ...
        private void ShowDetailData(ArrayList alDataOrg, FS.HISFC.Models.Pharmacy.DrugBillClass drugBillClass, FS.FrameWork.Models.NeuObject stockDept)
        {
            ...
            Hashtable hsTotal = new Hashtable();
            foreach (FS.HISFC.Models.Pharmacy.ApplyOut objOrg in alDataOrg)
            {
                ...
                if (!hsTotal.ContainsKey(objOrg.Item.ID))
                {
                    hsTotal.Add(objOrg.Item.ID, objOrg.Operation.ApplyQty);
                }
                else
                {
                    decimal totQty = (decimal)(hsTotal[objOrg.Item.ID]) + objOrg.Operation.ApplyQty;
                    hsTotal.Remove(objOrg.Item.ID);
                    hsTotal.Add(objOrg.Item.ID, totQty);
                }
            }
            ...
            this.nlbTitle.Text =  drugBillClass.Name + "(明细)";
            this.lblApplyDept.Text = "领药科室： " + applyDeptName;

            this.nlbRowCount.Text = "记录数：" + alData.Count.ToString();
            this.nlbBillNO.Text = "单据号：" + drugBillClass.DrugBillNO;// (drugBillClass.DrugBillNO.Length > 8 ? drugBillClass.DrugBillNO.Substring(8) : drugBillClass.DrugBillNO);
            ...
            {          
                ...
                patientInfo = string.Format("{0}    {1}住院号：{2}   年龄：{3}    性别：{4}", bedNO, SOC.Public.String.PadRight(privPatientName, 8, ' '), patientNO, age, p.Sex.Name);
                ...
                this.neuSpread1_Sheet1.Cells[iRow, 0].Font = new System.Drawing.Font("楷体", 11F, System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, ((byte)(134)));
                ...
                foreach (FS.HISFC.Models.Pharmacy.ApplyOut info in alPatientData)
                {
                    ...
                    this.neuSpread1.SetCellValue(0, iRow, "每次", doseOnce);
                    //this.neuSpread1.SetCellValue(0, iRow, "每次用量", info.DoseOnce.ToString("F4").TrimEnd('0').TrimEnd('.') + info.Item.DoseUnit);


                    this.neuSpread1.SetCellValue(0, iRow, "总量", info.Operation.ApplyQty.ToString("F4").TrimEnd('0').TrimEnd('.') + info.Item.MinUnit);
                    this.neuSpread1.SetCellValue(0, iRow, "零售价", FS.FrameWork.Public.String.FormatNumberReturnString(info.Item.PriceCollection.RetailPrice / info.Item.PackQty, 4));
                    ...
                    //                this.neuSpread1.SetCellValue(0, iRow, "零售价", FS.FrameWork.Public.String.FormatNumberReturnString(price,2));
                    this.neuSpread1.SetCellValue(0, iRow, "零售金额", FS.FrameWork.Public.String.FormatNumberReturnString(info.Operation.ApplyQty / info.Item.PackQty * info.Item.PriceCollection.RetailPrice, 2));
                ...
                this.neuSpread1_Sheet1.Rows[iRow].Font = new System.Drawing.Font("楷体", 11F, System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, ((byte)(134)));
                this.neuSpread1.SetCellValue(0, iRow, "零售金额", totCost.ToString());
                this.neuSpread1_Sheet1.Cells[iRow, 11].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Right;
                this.neuSpread1.SetCellValue(0, iRow, "零售价", "总金额：");
                iRow++;
                this.neuSpread1_Sheet1.Rows.Add(iRow, 1);
            ...
            #region 汇总显示
            int totRow = this.neuSpread1_Sheet1.Rows.Count;
            int totFrow = totRow;
            foreach (string s in hsTotal.Keys)
            {
                if (!string.IsNullOrEmpty(s))
                {
                    //自定义码
                    this.neuSpread1_Sheet1.AddRows(totRow, 1);
                    FS.HISFC.Models.Pharmacy.Item itemTmp = FS.SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(s);
                    if (totFrow == totRow)
                    {
                        this.neuSpread1_Sheet1.Rows[totRow].Border = topBorder;
                    }
                    this.neuSpread1.SetCellValue(0, totRow, "编码", SOC.HISFC.BizProcess.Cache.Pharmacy.GetItemCustomNO(s));
                    
                    //药品名称
                    this.neuSpread1_Sheet1.Cells[totRow, 1].ColumnSpan = 3;
                    this.neuSpread1.SetCellValue(0, totRow, "名称", itemTmp.Name);

                    //频次
                    this.neuSpread1_Sheet1.Cells[totRow, 4].ColumnSpan = 2;
                    this.neuSpread1.SetCellValue(0, totRow, "频次", itemTmp.Specs);

                    //总量
                    this.neuSpread1_Sheet1.Cells[totRow, 6].ColumnSpan = 4;
                    this.neuSpread1.SetCellValue(0, totRow, "使用时间", hsTotal[s]);

                    //单位
                    this.neuSpread1.SetCellValue(0, totRow, "备注", itemTmp.MinUnit);
                    totRow++;
                }
            }
            #endregion
                ...
                if (this.neuSpread1_Sheet1.Rows.Count > 0)
                {
                    ...
                    for (int page = totPageNO; page > 0; page--)
                    {
                        if (page == totPageNO)
                        {
                            ...
                            this.neuSpread1_Sheet1.Cells[index, 0].Font = new Font("楷体", 10f);
        ...
        public void Print()
        {
            ...
            if (this.neuSpread1_Sheet1.RowCount==0||this.nlbRowCount.Text == "记录数：0")
            {
                return;
            }
            //启用华南打印基类打印
