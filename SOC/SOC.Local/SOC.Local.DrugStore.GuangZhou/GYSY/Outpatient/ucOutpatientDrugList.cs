...
{
    /// <summary>
    /// [功能描述: 门诊药房西药、中成药配药清单本地化实例]<br></br>
    /// [创 建 者: cube]<br></br>
    /// [创建时间: 2010-12]<br></br>
    /// 说明：
    /// 1、作为一个例子保留下来，不要更改
    /// </summary>
    public partial class ucOutpatientDrugList : UserControl
    {
        public ucOutpatientDrugList()
        {
            InitializeComponent();
        ...
        private void AddAllData(System.Collections.ArrayList al, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, string hospitalName)
        {
            if (al.Count > 0)
            {
                //有效的药品页数
                ...
                foreach (FS.HISFC.Models.Pharmacy.ApplyOut applyOut in al)
                {
                    ...
                    if (applyOutList.Count == 5 || (num == al.Count && applyOutList.Count > 0))
                    {
                        applyOutPageList.Add(pageNO, applyOutList);
                        applyOutList = new List<FS.HISFC.Models.Pharmacy.ApplyOut>();
                        pageNO++;
        ...
        private void SetDrugDeatail(int iRow, FS.HISFC.Models.Pharmacy.ApplyOut info, bool isValid)
        {
            ...
            {
                this.neuSpread1_Sheet1.SetText(iRow, 1, item.NameCollection.RegularName);
            }
            else
            {
                this.neuSpread1_Sheet1.SetText(iRow, 1, item.Name);
            }
            ...
            if (outPackQty == 0)
            {
                this.neuSpread1_Sheet1.SetText(iRow, 3, ((int)(info.Operation.ApplyQty)).ToString("0.00"));
                //单位
                this.neuSpread1_Sheet1.SetText(iRow, 4, info.Item.MinUnit);
            }
            else if (outMinQty == 0)
            {
                this.neuSpread1_Sheet1.SetText(iRow, 3, outPackQty.ToString("0.00"));
                //单位
            ...
            //金额
            this.neuSpread1_Sheet1.SetText(iRow, 9, ((decimal)((info.Item.PriceCollection.RetailPrice / info.Item.PackQty) * (info.Operation.ApplyQty))).ToString("0.00"));
            //总药价
            this.drugListTotalPrice += (info.Item.PriceCollection.RetailPrice / info.Item.PackQty) * (info.Operation.ApplyQty);

            //用法
            this.neuSpread1_Sheet1.SetText(iRow, 5, SOC.HISFC.BizProcess.Cache.Common.GetUsageName(info.Usage.ID));
            //每次用量
            if (info.DoseOnce == 0)
            {
                this.neuSpread1_Sheet1.SetText(iRow, 6, "遵医嘱");
            }
            else
            {
                this.neuSpread1_Sheet1.SetText(iRow, 6, info.DoseOnce.ToString() + info.Item.DoseUnit);
            }

            if (!string.IsNullOrEmpty(info.Frequency.ID))
            {
                this.neuSpread1_Sheet1.SetText(iRow, 7, info.Frequency.ID.ToLower());
            }
        ...
        public int PrintDrugBill(ArrayList alData, string diagnose, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal,string hospitalName)
        {
            ...
            this.AddAllData(alData, drugRecipe, hospitalName);
