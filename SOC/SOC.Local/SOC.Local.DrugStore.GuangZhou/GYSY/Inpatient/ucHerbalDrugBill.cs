    ...
    public partial class ucHerbalDrugBill : UserControl, FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.IInpatientBill
    {
        ...
        private void ShowDetailData(ArrayList alData, FS.HISFC.Models.Pharmacy.DrugBillClass drugBillClass, FS.FrameWork.Models.NeuObject stockDept)
        {
            ...
            foreach (FS.HISFC.Models.Pharmacy.ApplyOut info in alData)
            {
                ...
                //患者不同、组合号不同需要插入一行患者信息
                if (samePatient != info.PatientNO + info.CombNO)
                {
                    string age = "";
                    ...
                    try
                    {
                        ...
                        patientNO = p.PID.PatientNO;
                    }
                    catch { }

                    privPatientName = info.PatientName;
                    samePatient = info.PatientNO + info.CombNO;
                    patientInfo = string.Format("{0}  {1}住院号：{2}   年龄：{3}    剂数：{4}",
                        bedNO, 
                        SOC.Public.String.PadRight(privPatientName, 8, ' '), 
                        patientNO, 
                        age,
                        info.Days.ToString("F4").TrimEnd('0').TrimEnd('.'));
                    //如果不是在本页的第一行，需要患者信息
                    ...
                    this.neuSpread1_Sheet1.Cells[iRow, 0].Font = new System.Drawing.Font("宋体", 12F, System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, ((byte)(134)));
                ...
                this.neuSpread1.SetCellValue(0, iRow, "毎剂量", doseOnce);
        ...
        {
            this.neuSpread1.SaveSchema(FS.FrameWork.WinForms.Classes.Function.CurrentPath + "\\Profile\\InpatientDrugStoreHerbalDrugBill.xml");
        }
        ...
        private class CompareApplyOutByPatient : IComparer
        {
            ...
            public int Compare(object x, object y)
            {
                ...
                oX = o1.BedNO + o1.PatientName + o1.CombNO;
                oY = o2.BedNO + o2.PatientName + o2.CombNO;
