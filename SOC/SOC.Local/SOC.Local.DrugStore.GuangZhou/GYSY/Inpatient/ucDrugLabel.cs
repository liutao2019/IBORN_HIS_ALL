...

    {
        ...
        private void Print()
        {
            ...
            if (pageSize == null)
            {
                ...
                if (pageSize == null)
                {
                    pageSize = new FS.HISFC.Models.Base.PageSize("InPatientDrugLabel", 365, 151);
                }
            }
        ...
        public int PrintDrugLabel(System.Collections.ArrayList alData, FS.HISFC.Models.Pharmacy.DrugBillClass drugBillClass, FS.FrameWork.Models.NeuObject stockDept)
        {
            ...
            foreach (FS.HISFC.Models.Pharmacy.ApplyOut applyOut in alData)
            {
                ...
                string patientInfo = string.Format("{0}  {1}住院号：{2}   年龄：{3}", bedNO, SOC.Public.String.PadRight(privPatientName, 8, ' '), applyOut.PatientNO, age);
