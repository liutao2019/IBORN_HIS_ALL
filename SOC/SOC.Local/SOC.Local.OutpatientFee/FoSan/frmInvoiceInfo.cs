...
{
    public partial class frmInvoiceInfo : Form,SOC.Local.OutpatientFee.FoSan.Interface.IOutPatientInvoiceShow
    {
        #region 变量
        /// <summary>
        /// 患者挂号信息
        ...
        public bool SetInvoiceInfo(FS.HISFC.Models.Fee.Outpatient.Balance invoice)
        {
            ...
            if (isShowTotal)
            {
                alFee = outPatientManager.QueryFeeItemListsTogetherByInvoiceNO(invoice.Invoice.ID);
            }
            else
            {
                alFee = outPatientManager.QueryFeeItemListsByInvoiceNO(invoice.Invoice.ID);
            }
        ...
        private bool SetFeeInfo(ArrayList alFee)
        {
            ...
            foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList f in alFee)
            {
                ...
                if (f.PayType == PayTypes.Balanced && f.CancelType == CancelTypes.Valid)
                {
                    ...
                    {
                        this.neuSpread1_Sheet1.Columns[9].Width = 0F;
                    }
