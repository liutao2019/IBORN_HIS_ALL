...
{                                                         
    public partial class ucSDFYOutpatientFeeInvoicePrint : System.Windows.Forms.UserControl, FS.HISFC.BizProcess.Interface.FeeInterface.IInvoicePrint, FS.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        public ucSDFYOutpatientFeeInvoicePrint()
        {
            InitializeComponent();
        }
            ...
            ArrayList alInvoiceDetail, ArrayList alFeeItemList, ArrayList alPayModes, bool isPreview)
        {
            ...
            foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList item in alFeeItemList)
            {
                if (item.Item.MinFee.ID == "002" || item.Item.MinFee.ID == "003" || item.Item.MinFee.ID == "004")
                {
                    if (item.ItemRateFlag == "1")
                    {
                        ZFPha += FS.FrameWork.Function.NConvert.ToDecimal(item.FT.DrugOwnCost);
