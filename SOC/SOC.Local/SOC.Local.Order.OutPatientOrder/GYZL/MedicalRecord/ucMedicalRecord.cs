        ...
        FS.SOC.HISFC.BizProcess.OrderInterface.OutPatientOrder.IOutPatientOrderPrint
    {
        ...
        public void SetPatientInfo(FS.HISFC.Models.Registration.Register register)
        {
            ...
            if (this.caseHistory != null)
            {
                ...
                {
                    this.lblCheckBody.Text = this.caseHistory.CheckBody;
                }
                if (!string.IsNullOrEmpty(this.caseHistory.CaseMain))
                {
                    this.lblCaseMain.Text = this.caseHistory.CaseMain;
                }
                if (!string.IsNullOrEmpty(this.caseHistory.CaseNow))
                {
                    this.lblCaseNow.Text = this.caseHistory.CaseNow;
                }
                if (!string.IsNullOrEmpty(this.caseHistory.CaseAllery))
                {
                    this.lbCaseAllery.Text = this.caseHistory.CaseAllery;
                }
            }
        ...
        public int PrintOutPatientOrderBill(FS.HISFC.Models.Registration.Register regObj, FS.FrameWork.Models.NeuObject reciptDept, FS.FrameWork.Models.NeuObject reciptDoct, IList<FS.HISFC.Models.Order.OutPatient.Order> orderList, bool isPreview)
        {
            ...
            //        ////查询门诊处方记录
            //        //List<FS.FrameWork.Models.NeuObject> IRecipe = this.OrderManagement.GetRecipeNoByClinicNoAndSeeNo(regObj.ID, regObj.DoctorInfo.SeeNO.ToString(), "0").ToList();

            //        ////取出处方信息
            //        //foreach (FS.FrameWork.Models.NeuObject NeuObject in IRecipe)
            //        //{
            ...
            //        /////循环打印信息
            //        //foreach (KeyValuePair<string, IList<FS.HISFC.Models.Order.OutPatient.Order>> infoDic in recipeInfoDic)
            //        //{
            ...
            //        int icount = FS.FrameWork.Function.NConvert.ToInt32(Math.Ceiling(Convert.ToDouble(IList.Count) / iSet));
            //        ArrayList tempList;
            //        for (int i = 1; i <= icount; i++)
            //        {
            //            alPrint.Clear();
            ...
            //            {
            //                //最后一页
            //                int num = IList.Count % iSet;
            //                if (num == 0)
            //                {
            //                    num = iSet;
            //                }
            //                tempList = ArrayList.Adapter(IList.ToArray()).GetRange(iSet * (i - 1), num);
            //                for (int j = 0; j < tempList.Count; j++)
            //                {
            //                    alPrint.Add(tempList[j] as FS.HISFC.Models.Order.OutPatient.Order);
            //                }
            //                this.MakaLabel(alPrint);
            //                this.Print();
            //            }
            //            else
            //            {
            //                tempList = ArrayList.Adapter(IList.ToArray()).GetRange(iSet * (i - 1), iSet);
            //                for (int j = 0; j < tempList.Count; j++)
            //                {
            //                    alPrint.Add(tempList[j] as FS.HISFC.Models.Order.OutPatient.Order);
            //                }
