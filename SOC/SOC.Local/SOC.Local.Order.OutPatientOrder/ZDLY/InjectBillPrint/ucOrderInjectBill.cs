...
namespace FS.SOC.Local.Order.OutPatientOrder.ZDLY.InjectBillPrint
{
    /// <summary>
    /// 中大六院注射单打印
    /// </summary>
    public partial class ucOrderInjectBill : UserControl, FS.SOC.HISFC.BizProcess.OrderInterface.OutPatientOrder.IOutPatientOrderPrint
    {
        /// <summary>
        /// 初始化
        /// </summary>
        ...
        private void PrintOnePage(ArrayList alData, int current, int total, FS.HISFC.Models.Registration.Register regObj, FS.FrameWork.Models.NeuObject reciptDept, FS.FrameWork.Models.NeuObject reciptDoct, bool isPreview)
        {
            ...
            try
            {
                ...
                if (alData.Count > 0)
                {
                    alFee = this.outPatientManager.QueryFeeDetailByClinicCodeAndRecipeSeq(regObj.ID, ((FS.HISFC.Models.Order.OutPatient.Order)alData[0]).ReciptSequence, "ALL");
                }
                ...
                //画组合号
                FS.HISFC.Components.Common.Classes.Function.DrawCombo(neuSpread1_Sheet1, 9, 2);
                if(!isPreview)
                {
                    PrintPage();
                }
        ...
        public int PrintOutPatientOrderBill(FS.HISFC.Models.Registration.Register regObj, FS.FrameWork.Models.NeuObject reciptDept, FS.FrameWork.Models.NeuObject reciptDoct, System.Collections.ArrayList alOrder, bool isPreview)
        {
            ...
            /*
            Dictionary<string, List<FS.HISFC.Models.Order.OutPatient.Order>> PrintDic = null;
            foreach (string days in injectDictionary.Keys)
            {
                PrintDic = new Dictionary<string, List<FS.HISFC.Models.Order.OutPatient.Order>>();
                for (int t = 0; t < injectDictionary[days].Count; t++)
                {
                    ...
                    {
                        List<FS.HISFC.Models.Order.OutPatient.Order> usageorderList = new List<FS.HISFC.Models.Order.OutPatient.Order>();
