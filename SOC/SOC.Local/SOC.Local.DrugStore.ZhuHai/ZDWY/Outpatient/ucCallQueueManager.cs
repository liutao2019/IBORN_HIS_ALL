...
namespace FS.SOC.Local.DrugStore.ZhuHai.ZDWY.Outpatient
{
    public partial class ucCallQueueManager : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        private FS.SOC.Local.DrugStore.ZhuHai.ZDWY.Outpatient.Bizlogic.DrugStoreAsign drugStoreAsignMgr = new FS.SOC.Local.DrugStore.ZhuHai.ZDWY.Outpatient.Bizlogic.DrugStoreAsign();

        private FS.SOC.HISFC.BizLogic.Pharmacy.DrugStore drugStoreMgr = new FS.SOC.HISFC.BizLogic.Pharmacy.DrugStore();

        private FS.SOC.Local.DrugStore.ZhuHai.Common.CallSpeak callSpeak = new FS.SOC.Local.DrugStore.ZhuHai.Common.CallSpeak();
        ...
        private void AssignCallback()
        {
            ...
            try
            {
                ...
                while (queue.Count > 0)
                {
                    ...
                    //根据护士站和午别查找所有的叫号申请信息
                    List<FS.SOC.Local.DrugStore.ZhuHai.ZDWY.Outpatient.Models.DrugStoreAsign> list = drugStoreAsignMgr.Query(this.curDept.ID);
                    if (list != null && list.Count > 0)
                    {
                        foreach (FS.SOC.Local.DrugStore.ZhuHai.ZDWY.Outpatient.Models.DrugStoreAsign drugStoreAsign in list)
                        {
                            //开始逐一叫号
                            callSpeak.Speech(drugStoreAsign);
                            //更新叫号状态
                            string state = this.drugStoreAsignMgr.GetRecipeState(drugStoreAsign.RecipeNO);
                            ...
                            {
                                returnValue = this.drugStoreAsignMgr.UpdateQueueState(drugStoreAsign.RecipeNO, drugStoreAsign.DrugDeptCode, "2");
                            }
                            else
                            {
                                returnValue = this.drugStoreAsignMgr.UpdateQueueState(drugStoreAsign.RecipeNO, drugStoreAsign.DrugDeptCode, "1");
                            }
                            if (returnValue >= 1)
                            {
