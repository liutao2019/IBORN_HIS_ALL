...
namespace FS.SOC.Local.DrugStore.ZhuHai.ZDWY.Inpatient
{
    ...
    public class DrugBillInterfaceImplement : FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.IDrugBillClass,FS.HISFC.BizProcess.Interface.Pharmacy.IDrugBillClassP
    {
        ...
        public FS.HISFC.Models.Pharmacy.DrugBillClass GetDrugBillClass(FS.HISFC.Models.Pharmacy.ApplyOut applyOut)
        {
            ...
            //临购药品，发送到临购摆药单
            if (SOC.HISFC.BizProcess.Cache.Pharmacy.GetItemSpecial(applyOut.Item.ID) == "3" || SOC.HISFC.BizProcess.Cache.Pharmacy.GetItemSpecial(applyOut.Item.ID) == "6")
            {
                billClass.ID = "L";
                if (hsDrugBillClass.Contains(billClass.ID))
                {
            ...
            if ((applyOut.OrderType.ID == "CD" || applyOut.OrderType.ID == "QL") && (SOC.HISFC.BizProcess.Cache.Common.GetDrugQualitySystem(applyOut.Item.Quality.ID) == "S" || SOC.HISFC.BizProcess.Cache.Common.GetDrugQualitySystem(applyOut.Item.Quality.ID) == "P"))
            {
                billClass.ID = "OP1";
                if (hsDrugBillClass.Contains(billClass.ID))
                {
            ...
            //盐酸曲马多注射液【精二】发送到重点精二摆药单 {E81700F1-6DAB-4664-AFC5-9175D601BE02}
            if (SOC.HISFC.BizProcess.Cache.Common.GetDrugQualitySystem(applyOut.Item.Quality.ID) == "P2")
            {
                if (SOC.HISFC.BizProcess.Cache.Common.GetP2DrugUserCode(applyOut.Item.ID) == "ZDJE")
                {
                    billClass.ID = "ZDP2";
                    if (hsDrugBillClass.Contains(billClass.ID))
                    {
            ...
            //☆丙泊酚中/长链脂肪乳注射液 发送到全身麻醉剂摆药单 {E81700F1-6DAB-4664-AFC5-9175D601BE02}
            //☆吸入用七氟烷(喜保福宁Sevofrane) 发送到全身麻醉剂摆药单
            if (SOC.HISFC.BizProcess.Cache.Common.GetDrugQualitySystem(applyOut.Item.Quality.ID) == "Q")
            {
                if (SOC.HISFC.BizProcess.Cache.Common.GetQsAnesthesiaDrugUserCode(applyOut.Item.ID) == "QSMZJ")
                {
                    billClass.ID = "QS";
                    if (hsDrugBillClass.Contains(billClass.ID))
                    {
            ...
            if (SOC.HISFC.BizProcess.Cache.Common.IsPOUsage(applyOut.Usage.ID))
            {
                if (applyOut.OrderType.ID == "CZ")// {7AAC605A-0755-451b-8848-19ACBA5AE26E}
                {
                    billClass.ID = "CP";
                    if (hsDrugBillClass.Contains(billClass.ID))
                    {
                ...
                else if (applyOut.OrderType.ID == "LZ")
                {
                    billClass.ID = "LP";
                    if (hsDrugBillClass.Contains(billClass.ID))
                    {
            ...
            if (SOC.HISFC.BizProcess.Cache.Common.IsInjectionUsage(applyOut.Usage.ID))
            {
                //针剂摆药单
                billClass.ID = "ZJ";
            ...
            if (SOC.HISFC.BizProcess.Cache.Common.IsExternalUsage(applyOut.Usage.ID))
            {
                //外用药摆药单
                billClass.ID = "WY";
        ...
        private ArrayList SaveDefaultBill()
        {
            ...
            lDrugBill.PrintType.ID = FS.HISFC.Models.Pharmacy.BillPrintType.enuBillPrintType.D;           //打印类型
            ...
            op1DrugBill.PrintType.ID = FS.HISFC.Models.Pharmacy.BillPrintType.enuBillPrintType.D;           //打印类型
            ...
            // {7AAC605A-0755-451b-8848-19ACBA5AE26E}
            ...
            lpDrugBill.PrintType.ID = FS.HISFC.Models.Pharmacy.BillPrintType.enuBillPrintType.D;           //打印类型
