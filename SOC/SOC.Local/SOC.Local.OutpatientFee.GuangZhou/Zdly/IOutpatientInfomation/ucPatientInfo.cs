...
namespace FS.SOC.Local.OutpatientFee.GuangZhou.Zdly.IOutpatientInfomation
{
    public partial class ucPatientInfo : UserControl, FS.HISFC.BizProcess.Integrate.FeeInterface.IOutpatientInfomation
    {
        ...
        /// 是否可以修改结算种类
        /// </summary>
        private bool isCanModifyPact = true;
        public bool IsCanModifyPact
        {
            set
            {
                this.isCanModifyPact = value;
                this.cmbPact.Enabled = this.isCanModifyPact;
            }
        }
        ...
        private void tbCardNO_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                else //正常输入患者门诊卡号情况
                {
                    ...
                    {
                        //否则9开头的病历号
                        if (cardNO.StartsWith(this.noRegFlagChar) && cardNO.Length == 10)
                        {
                        }
                        else
                        {
        ...
        private void cmbPact_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else//有限额
            {
                ...
                foreach (FS.HISFC.Models.Base.PactStatRelation p in relations)
                {
                    if (true)
                    {
                        if (this.patientInfo.DoctorInfo.Templet.RegLevel.IsEmergency == FrameWork.Function.NConvert.ToBoolean(p.SortID))
                        {
                            if (obj.ID != p.Group.ID)
                            {
                                obj = new FS.FrameWork.Models.NeuObject();
        ...
        private void tbAge_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                try
                {
                    if (this.tbAge.Tag == null || text.Equals(this.tbAge.Tag.ToString()) == false)
                    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            int result = opercard.ReadCardNO(ref McardNo, ref  error);
            if (result != -1)
            {
                tbCardNO.Text = McardNo;
            }
            else
            {
            ...
            tbCardNO_KeyDown(null, new KeyEventArgs(Keys.Enter));
