...
namespace FS.SOC.Local.OutpatientFee.GuangZhou.Zdly.IChooseItemForOutpatient
{
    ...
    public partial class ucChooseItemForOutPatient : UserControl, FS.HISFC.BizProcess.Integrate.FeeInterface.IChooseItemForOutpatient
    {
        ...
        void ckCustomCode_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                SOC.Public.XML.SettingFile.SaveSetting(fileName, "排序" + FS.FrameWork.Management.Connection.Operator.Name, "currentmodel", "2");
            }
            else if (((Control)sender).Name == this.ckSpellCode.Name && this.ckSpellCode.Checked)
            {
                SOC.Public.XML.SettingFile.SaveSetting(fileName, "排序" + FS.FrameWork.Management.Connection.Operator.Name, "currentmodel", "0");
            }
            else if (((Control)sender).Name == this.ckWBCode.Name && this.ckWBCode.Checked)
            {
                SOC.Public.XML.SettingFile.SaveSetting(fileName, "排序" + FS.FrameWork.Management.Connection.Operator.Name, "currentmodel", "1");
            }
            else if (((Control)sender).Name == this.cbFilter.Name)
            {
                SOC.Public.XML.SettingFile.SaveSetting(fileName, "模糊查询" + FS.FrameWork.Management.Connection.Operator.Name, "currentmodel", this.cbFilter.Checked ? "1" : "0");
            }
        ...
        private void InitDataTable()
        {
            ...
            else
            {
                ...
                dtItem.Columns.AddRange(new DataColumn[]{   new DataColumn("自定义码", str),
                                                        ...
                                                        new DataColumn("医保等级",str),
                                                        new DataColumn("别名", str),
                                                        new DataColumn("别名拼音码", str),
                                                        new DataColumn("别名五笔码", str),
                                                        new DataColumn("别名自定义码", str),   //{E91E0D33-FCC6-4982-BA74-320A6E8A373C}
                                                            //{ED51E97B-B752-4c32-BD93-F80209A24879}增加输入次数排序
        ...
        private string QuerySortString()
        {
            ...
            {
                SOC.Public.XML.SettingFile.SaveSetting(fileName, "排序" + FS.FrameWork.Management.Connection.Operator.Name, "currentmodel", "0");
                SOC.Public.XML.SettingFile.SaveSetting(fileName, "模糊查询" + FS.FrameWork.Management.Connection.Operator.Name, "currentmodel", "0");

            }
            int iModel = 0;

            iModel = FS.FrameWork.Function.NConvert.ToInt32(SOC.Public.XML.SettingFile.ReadSetting(fileName, "模糊查询" + FS.FrameWork.Management.Connection.Operator.Name, "currentmodel", "1"));
            if (iModel == 0)
            {
            ...
            iModel = FS.FrameWork.Function.NConvert.ToInt32(SOC.Public.XML.SettingFile.ReadSetting(fileName, "排序" + FS.FrameWork.Management.Connection.Operator.Name, "currentmodel", "0"));
        ...
        public void SetDataSet(DataSet dsItem)
        {
            ...
            foreach (DataRow row in dsItem.Tables[0].Rows)
            {
                ...
                IRow["规格"] = row["SPECS"];
                if (Function.IsContainYKDept(employee.Dept.ID))//如果是宜康门诊
                {
                    IRow["价格"] = row["SP_PRICE"];//显示特诊价
                }
                else
                {
                    IRow["价格"] = row["UNIT_PRICE"];//显示正常价格
                }
                ...
                IRow["别名自定义码"] = row["CUS_USER_CODE"];
                IRow["别名"] = row["cus_name"];
                IRow["别名拼音码"] = row["cus_spell_code"];
                IRow["别名五笔码"] = row["cus_wb_code"];
                //{ED51E97B-B752-4c32-BD93-F80209A24879}增加输入次数排序
        ...
        private void SetFilter(string inputChar)
        {
            ...
            {
                filterString = "(拼音码 LIKE '" + inputChar + "%') OR " + "(项目名称 LIKE '" + inputChar + "%') OR " + "(自定义码 LIKE '" + inputChar + "%') " + " or (五笔码 LIKE '" + inputChar + "%') " + " or (别名 LIKE '" + inputChar + "%') ";
            }
            else if (cbFilter.Checked)
            {
                filterString = "(拼音码 LIKE '%" + inputChar + "%') OR " + "(项目名称 LIKE '%" + inputChar + "%') OR " + "(自定义码 LIKE '%" + inputChar + "%') " + " or (五笔码 LIKE '%" + inputChar + "%') " + " or (别名五笔码 LIKE '%" + inputChar + "%') " + " or (别名拼音码 LIKE '%" + inputChar + "%') " + " or (别名自定义码 LIKE '%" + inputChar + "%') ";
            }
