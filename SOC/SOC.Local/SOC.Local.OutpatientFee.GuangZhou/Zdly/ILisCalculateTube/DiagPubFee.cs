...
namespace FS.SOC.Local.OutpatientFee.GuangZhou.Zdly.ILisCalculateTube
{
    public class DiagPubFee
    {
        ...
        private string roundFeeItemCode = "F00002237128";
        ...
        public FS.HISFC.Models.Fee.Outpatient.FeeItemList  GetFeeItemList(FS.HISFC.Models.Registration.Register r,decimal qty , string recipeSequence,ref string errorInfo)
        {
            ...
            feeItemList.Item.Qty = qty;//数量
            //包装单位单价，保留4位小数
            feeItemList.Item.Price = 1M;
            feeItemList.OrgPrice = 1M;
            feeItemList.SpecialPrice = 1M;
        ...
        public decimal GetNumber(FS.HISFC.Models.Registration.Register r, ref string errInfo)
        {
            ...
            decimal paycost = 0;// FS.FrameWork.Function.NConvert.ToDecimal(num);

            if (paycost >= 0)
            {
                //if (string.IsNullOrEmpty(r.DoctorInfo.Templet.RegLevel.ID))
                //{
                //    //如果是挂号级别为空，那么直接获取一个
