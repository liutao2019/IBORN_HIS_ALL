    ...
    public partial class ucInvoicePrint : System.Windows.Forms.UserControl, FS.HISFC.BizProcess.Interface.FeeInterface.IInvoicePrint, FS.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        ...
        public int Print()
        {
            try
            {
                ...
                FS.HISFC.Models.Base.PageSize ps = psManager.GetPageSize("MZFP");
                if (ps == null || string.IsNullOrEmpty(ps.Printer))
                {
                    ...
                    MessageBox.Show("没有找到打印机【" + ps.Printer + "】");
                    return -1;
                }
            ...
            ArrayList alInvoiceDetail, ArrayList alFeeItemList, ArrayList alPayModes, bool isPreview)
        {
            ...
            if (CommonController.Instance.GetSystemTime().Date >= invoice.PrintTime.Date.AddDays(1))
            {
                if (DialogResult.Yes == CommonController.Instance.MessageBox("是否修改发票的打印时间？", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
                {
                    ChooseDate frm = new ChooseDate();
                    frm.OkEvent += new ChooseDate.ButtonHandle(frm_OkEvent);
                    frm.CloseEvent += new ChooseDate.ButtonHandle(frm_CloseEvent);
                    frm.ShowDialog();
                    if (myPrintDate != System.DateTime.MinValue)
                    {
                        invoice.PrintTime = myPrintDate;
                    }
            ...
            ///ZFItem: 自费非药品
            ///CBPha: 超标药品
            ///JZPha：记账药品金额（pub+pay）
            decimal ZFPha = 0m, CBPha = 0m, ZFItem = 0m, JZPha = 0m;
            ...
            foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList item in alFeeItemList)
            {
                if (item.Item.ItemType== FS.HISFC.Models.Base.EnumItemType.Drug)
                {
                    ...
                    JZPha += item.FT.PayCost + item.FT.PubCost;
                }
                else
                {
                    ZFItem += item.FT.OwnCost;
                ...
                this.lblDoctor.Text = item.RecipeOper.ID;
            }

            //其它药 = 自费药+超标药
            if (ZFPha + CBPha + JZPha > 0&&regInfo.Pact.PayKind.ID=="03")
            {
                this.lblDrugFeeInfo.Text = string.Format("药费记账：{0} 超标：{1} 自费：{2}", JZPha, CBPha, ZFPha);
            }
            else
            {
                this.lblDrugFeeInfo.Text = string.Empty;
            }
            ...
            this.lblPriPatientNo.Text = invoice.Patient.PID.CardNO;//病历号
            this.lblPriName.Text = regInfo.Name;//姓名
            string tempRate = "";
            ...
            this.lbPactName.Text = tempRate;
            if (regInfo.SSN != "")
            {
                if (regInfo.Pact.PayKind.ID == "03")
                {
                    this.lbPactName.Text += "," + "公医证号：" + regInfo.SSN + "(" + (regInfo.Pact.Rate.PayRate * 100).ToString("00") + "%)";
                }
                else
                {
                    this.lbPactName.Text += "," + "公医证号：" + regInfo.SSN;
                }
            }
            //ArrayList al;
            //try
            ...
            string deptid = ((managerIntegrate.Operator) as FS.HISFC.Models.Base.Employee).Dept.ID;
            ...
            this.lblnan.Visible = false;

            if (regInfo.Sex.ID.ToString() == "M")
            {
                this.lblnan.Visible = true;
            }
            else
            {
                this.lblnv.Visible = true;
            }
            ...
            for (int i = 0; i < alPayModes.Count; i++)
            {
                ...
                if (isyk)
                {
                    strPayMode += " " + helper.GetObjectFromID(payMode.PayType.ID.ToString()).Name+"  ";//结算操作员
                }
                else
                {
                    strPayMode += " " + helper.GetObjectFromID(payMode.PayType.ID.ToString()).Name + " " + FS.FrameWork.Public.String.FormatNumber(payMode.FT.TotCost, 2);//结算操作员
                }
            }
            ...
            if (regInfo.Pact.PayKind.ID == "03"  || regInfo.Pact.PayKind.ID == "02")//公费 增加医保
            {
                ...
                if (pactObj != null && pactObj.Rate.PayRate > 0)
                {
                    if (invoice.FT.PayCost > decimal.Zero)
                    {
                        string priPay = FS.FrameWork.Public.String.FormatNumberReturnString(DealOwnPay, 2) + " (" +
                                    FS.FrameWork.Public.String.FormatNumberReturnString(pactObj.Rate.PayRate * 100, 0).Replace(".", "") + "%:" +
                                    FS.FrameWork.Public.String.FormatNumberReturnString(invoice.FT.PayCost, 2) + ")";
                        if (priPay.Length > 15)
                        {
                            priPay.Insert(DealOwnPay.ToString().Length, "\n");
                            Font f = new Font("宋体", 8);
                            this.lblPriPay.Font = f;
                        }
                        this.lblPriPay.Text = priPay;
        ...
        #region 重选打印发票时间
        DateTime myPrintDate = new DateTime();
        private void frm_OkEvent(DateTime dt)
        {
            myPrintDate = dt;
        }
        private void frm_CloseEvent(DateTime dt)
        {
            myPrintDate = dt;
        }
        #endregion
