...
using FS.SOC.Local.OutpatientFee.GuangZhou.Gyzl;
using FS.HISFC.Models.Base;
using System.Xml.Linq;

namespace FS.SOC.Local.OutpatientFee.GuangZhou.Zdly
{
    /// <summary>
    /// 功能函数
    /// </summary>
    public class Function : FS.FrameWork.Management.Database
    {
        ...
        public static DataTable GetGFJZ()
        {
            ...
            {
                CommonController.Instance.MessageBox("请维护公费记账单统计大类[MZJZ]", System.Windows.Forms.MessageBoxIcon.Information);
                return null;
            }
        ...
        /// <returns></returns>
        public static bool IsShenPub(FS.HISFC.Models.Registration.Register r, ref string errInfo)
        {
            ...
            string sql = @"select 1 from com_pactcompare cd where cd.parent_pact='0304' and cd.pact_code='{0}'";
        ...
        public static ArrayList ConvertItemToPackage(ArrayList feeDetails,ref string errorInfo)
        {
            ...
            foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList feeItemList in feeDetails)
            {
                ...
                else//复合项目合并
                {
                    ...
                    {
                       ( (FS.HISFC.Models.Fee.Outpatient.FeeItemList)(packageItem[packageOrderID])).Item.Price += feeItemList.FT.TotCost;
                        continue;
                    }
                    else
                    {
