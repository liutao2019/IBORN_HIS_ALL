...
namespace FS.SOC.Local.InpatientFee.ShenZhen.BinHai.HISTIMEJOB
{
    /// <summary>
    /// FixedFee<br></br>
    /// [功能描述: 固定费用收取类]<br></br>
    ...
    public partial class FixedFee : FS.FrameWork.Management.Database, IJob
    {
        ...
        //床位费管理类
        private FS.SOC.Local.InpatientFee.ShenZhen.BinHai.BizLogic.Fee.BedFeeItem feeBedFeeItem = new FS.SOC.Local.InpatientFee.ShenZhen.BinHai.BizLogic.Fee.BedFeeItem();
        //床位管理类
        private FS.HISFC.BizLogic.Manager.Bed managerBed = new FS.HISFC.BizLogic.Manager.Bed();

        private FS.SOC.Local.InpatientFee.ShenZhen.BinHai.BizProcess.Integrate.Fee interageFee = new FS.SOC.Local.InpatientFee.ShenZhen.BinHai.BizProcess.Integrate.Fee();
        #endregion
        ...
        public int SetFeeByPerson(FS.HISFC.Models.Base.Bed bed, DateTime operDate)
        {
            ...
            if (patientInfo == null)
            {
                this.Err = bed.ID + this.radtInpatient.Err;
                WriteErr();
                return -1;
            }
            //如果是婴儿，不收床位费
            ...
            if (patientInfo.FT.PreFixFeeDateTime == date)
            {
                ...
                foreach (FS.HISFC.Models.Base.Bed objBed in alBed)
                {
                    ...
                    //收取1天的费用
                    if (interageFee.DoBedItemFee(itemsSupply, patientInfo, 1, operDate, patientInfo.FT.PreFixFeeDateTime, objBed) == -1)
                    {
                        return -1;
                    }
                }
            ...
            for (int i = 0; i < days; i++)
            {
                ...
                foreach (FS.HISFC.Models.Base.Bed objBed in alBed)
                {
                    ...
                    if (interageFee.DoBedItemFee(items, patientInfo, 1, operDate, patientInfo.FT.PreFixFeeDateTime, objBed) == -1)
                    {
                        return -1;
                    }
                }
