...
namespace FS.SOC.Local.Pharmacy.Extend.FuYou
{
    public class BizExtendInterfaceImplement: FS.SOC.HISFC.BizProcess.PharmacyInterface.Pharmacy.IPharmacyBizExtend
    {
        ...
        public System.Collections.ArrayList SetInputPlan(string stockDeptNO, System.Collections.ArrayList alData)
        {
            ...
            return setPlan.GetAllStockinfoAsPlan(stockDeptNO, "All");
        ...
        {
            string ListNO = "repeated";
            int index = 1;

            while (ListNO == "repeated")
            {
                if (index > 100)
                {
                    ListNO = "";
                ...
                index++;
                ListNO = this.MyGetBillNO(stockDeptNO, class2Code, class3code, ref errInfo);
            }
        ...
        private string MyGetBillNO(string stockDeptNO, string class2Code, string class3code, ref string errInfo)
        {
            ...
            else
            {
                ...
                ListNO = stockDeptNO + sysDate.Year.ToString().Substring(2, 2) + sysDate.Month.ToString().PadLeft(2, '0') + sysDate.Day.ToString().PadLeft(2, '0')
                    + inOutFlag + iSequence.ToString().PadLeft(3, '0');
                ...
                string SQL = @"select count(*) from pha_com_input where drug_dept_code = '{0}' and in_list_code = '{1}'";
                if (inOutFlag == "2")
                {
                    SQL = @"select count(*) from pha_com_output where drug_dept_code = '{0}' and out_list_code = '{1}'";
                }

                SQL = string.Format(SQL, stockDeptNO, ListNO);

                string returnVal = extentManager.ExecSqlReturnOne(SQL);
                if (returnVal == "-1" || string.IsNullOrEmpty(returnVal))
                {
                    errInfo = extentManager.Err;
                    return null;
                }
                if (FS.FrameWork.Function.NConvert.ToInt32(returnVal) > 0)
                {
                    return "repeated";
                }
