...
namespace FS.SOC.Local.Pharmacy.Extend.LSYY
{
    ...
    public partial class ucCommonInput : UserControl,SOC.HISFC.BizProcess.PharmacyInterface.Pharmacy.IInputInfoControl
    {
        ...
        /// 药品管理业务层
        /// </summary>
        FS.SOC.HISFC.BizLogic.Pharmacy.Storage stoMgr = new FS.SOC.HISFC.BizLogic.Pharmacy.Storage();
        ...
        void ncmbItemUnit_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                    ...
                    if (this.ncmbItemUnit.Tag != null && this.ncmbItemUnit.Tag.ToString() == "1")
                    {
                        this.ntxtPurchasePrice.Text = (curItem.PriceCollection.PurchasePrice/ curItem.PackQty).ToString("F4");
                        this.ntxtRetailPrice.Text = (curItem.PriceCollection.RetailPrice / curItem.PackQty).ToString("F4");
                        this.ntxtPurchaseCost.Text = (curItem.PriceCollection.PurchasePrice * qty / this.curItem.PackQty).ToString("F4");
                        this.ntxtRetailCost.Text = (curItem.PriceCollection.RetailPrice * qty / this.curItem.PackQty).ToString("F4");
                    }
                    else
                    {
                        this.ntxtPurchasePrice.Text = (curItem.PriceCollection.PurchasePrice).ToString("F4");
                        this.ntxtRetailPrice.Text = (curItem.PriceCollection.RetailPrice).ToString("F4");
                        this.ntxtPurchaseCost.Text = (curItem.PriceCollection.PurchasePrice * qty).ToString("F4");
                        this.ntxtRetailCost.Text = (curItem.PriceCollection.RetailPrice * qty).ToString("F4");
                    }
        ...
        void ntxtPurchasePrice_TextChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (this.cbAutoAdjustPrice.Checked)
                {
                    ...
                    try
                    {
                        this.ntxtRetailPrice.Text = FS.FrameWork.Function.NConvert.ToDecimal(FS.FrameWork.Public.String.ExpressionVal(formula)).ToString("F4");
        ...
        public int SetInputObject(FS.HISFC.Models.Pharmacy.Item item, FS.HISFC.Models.Pharmacy.Storage storage, bool isUseBatchNO, DateTime defaultValidDate)
        {
            ...
            if (this.cbAutoAdjustPrice.Checked)
            {
                ...
                try
                {
                    this.ntxtRetailPrice.Text = FS.FrameWork.Public.String.ExpressionVal(formula).ToString();
                }
                catch
                {
                }
        ...
        public int SetInputObject(FS.HISFC.Models.Pharmacy.Item item)
        {
            ...
            if (!string.IsNullOrEmpty(deptCode))
            {
                ...
                storage = stoMgr.GetLatestStorage(deptCode, item.ID);
                return this.SetInputObject(item, storage, true, DateTime.Now.AddYears(-1));
            }
            return this.SetInputObject(item,null,true,DateTime.Now.AddYears(-1));
        }
        ...
        public FS.HISFC.Models.Pharmacy.Input GetInputObject()
        {
            ...
            if (input.Item.PriceCollection.RetailPrice != retailPrice)
            {
                if (this.cbAutoAdjustPrice.Checked)
                {
                    ...
                    info.Memo = this.lbRetailPriceRate.Text;
            ...
            {
                input.ValidTime = DateTime.Now.AddYears(3).Date;
            }
            ...
            {
                DateTime validDate;
                if (DateTime.TryParse(this.nTxtDeliverTime.Text, out validDate))
                {
                    input.User01 = validDate.Date.ToString();
                }
                else
                {
                    Function.ShowMessage("送单不是时间格式：yyyy-MM-dd，请修改！", MessageBoxIcon.Information);
                    this.nTxtDeliverTime.Select();
                    this.nTxtDeliverTime.Focus();
                    return null;
                }
            }
            else
        ...
        protected string GetRetailPriceComputeFormula(FS.HISFC.Models.Pharmacy.Item item)
        {
            ...
            this.lbRetailPriceRate.Text = formula;
