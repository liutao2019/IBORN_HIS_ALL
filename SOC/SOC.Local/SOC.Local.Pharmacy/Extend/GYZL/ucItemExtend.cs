...
namespace FS.SOC.Local.Pharmacy.Extend.GYZL
{
    ...
    public partial class ucItemExtend : UserControl, FS.SOC.HISFC.BizProcess.PharmacyInterface.Pharmacy.IItemExtendControl
    {
        ...
        public FS.HISFC.Models.Pharmacy.Item Get(ref string errInfo)
        {
            ...
            item.ExecuteStandard = this.GetSIGrade(item);
            return item;
        }
        ...
        public int Set(FS.HISFC.Models.Pharmacy.Item item, ref string errInfo)
        {
            ...
            this.ncmbExtend4.Text = this.GetSIGrade(item);
            this.lblSIGrade.Text = "医保等级：" + this.ncmbExtend4.Text;
            return 1;
        }
        ...
        public int Init(ref string errInfo)
        {
            ...
            if (!System.IO.File.Exists(this.settingFile))
            {
                ...
                SOC.Public.XML.SettingFile.SaveSetting(this.settingFile, this.ncmbExtend3.Name, "DataType", "Items");
                //string items = "";
                //for (int index = 0; index < (int)FS.SOC.HISFC.Components.Pharmacy.Maintenance.EnumSplitType.End; index++)
                //{
                //    items += ((FS.SOC.HISFC.Components.Pharmacy.Maintenance.EnumSplitType)index).ToString() + ",";
                //}
                //items = items.TrimEnd(',');
                //SOC.Public.XML.SettingFile.SaveSetting(this.settingFile, this.ncmbExtend3.Name, "Items", items);
                SOC.Public.XML.SettingFile.SaveSetting(this.settingFile, this.ncmbExtend3.Name, "DropDownStyle", "DropDownList");
                SOC.Public.XML.SettingFile.SaveSetting(this.settingFile, this.ncmbExtend3.Name, "Enabled", this.ncmbExtend3.Enabled.ToString());
        ...
        private string GetSIGrade(FS.HISFC.Models.Pharmacy.Item item)
        {
            string siGrade = "未匹配(自费)";            
            string sql = @"select (case c.center_item_grade when '1' then '甲类' when '2' then '乙类' when '3' then '自费'  end) item_grade
            ...
            {                
                siGrade = dbMgr.Reader[0].ToString();             
            }
