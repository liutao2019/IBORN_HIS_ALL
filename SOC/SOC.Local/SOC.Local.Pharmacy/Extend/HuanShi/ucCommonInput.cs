...
namespace FS.SOC.Local.Pharmacy.Extend.HuanShi
{
    ...
    public partial class ucCommonInput : UserControl,SOC.HISFC.BizProcess.PharmacyInterface.Pharmacy.IInputInfoControl
    {
        ...
        /// 库存管理业务层
        /// </summary>
        FS.SOC.HISFC.BizLogic.Pharmacy.Storage sto = new FS.SOC.HISFC.BizLogic.Pharmacy.Storage();
        /// <summary>
        /// 当前药品基本信息，必须是和字典一致的
        ...
        void ncmbItemUnit_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (!this.cbAutoAdjustPrice.Checked)
            {
                if (this.ncmbItemUnit.Tag != null && this.ncmbItemUnit.Tag.ToString() == "1")
                {
                    this.ntxtRetailPrice.Text = (this.curItem.PriceCollection.RetailPrice / this.curItem.PackQty).ToString("F2");
                ...
                {
                    this.ntxtRetailPrice.Text = this.curItem.PriceCollection.RetailPrice.ToString("F2");
                    ...
                    {
                        this.ntxtPurchasePrice.Text = this.curItem.PriceCollection.PurchasePrice.ToString("F2");
                    }
                    else
                    {
                        this.ShowBalloonTip(4000, "提示：", "请注意【购入价】和【单位】是否匹配!", ToolTipIcon.Info);
            ...
            {
                //如果自动调价此处不处理单价，只处理金额,且人为选择大小单位展示的是大小单位的价格，这样处理保证所见即所得。
                decimal qty = FS.FrameWork.Function.NConvert.ToDecimal(this.ntxtInputQty.Text);
                if (this.ncmbItemUnit.Tag != null && this.ncmbItemUnit.Tag.ToString() == "1")
                {
                    this.ntxtPurchasePrice.Text = (curItem.PriceCollection.PurchasePrice / curItem.PackQty).ToString("F2");
                    this.ntxtRetailPrice.Text = (curItem.PriceCollection.RetailPrice / curItem.PackQty).ToString("F2");
                    this.ntxtPurchaseCost.Text = (curItem.PriceCollection.PurchasePrice * qty / this.curItem.PackQty).ToString("F2");
                    this.ntxtRetailCost.Text = (curItem.PriceCollection.RetailPrice * qty / this.curItem.PackQty).ToString("F2");
                }
                else
                {
                    this.ntxtPurchasePrice.Text = (curItem.PriceCollection.PurchasePrice).ToString("F2");
                    this.ntxtRetailPrice.Text = (curItem.PriceCollection.RetailPrice).ToString("F2");
                    this.ntxtPurchaseCost.Text = (curItem.PriceCollection.PurchasePrice * qty).ToString("F2");
                    this.ntxtRetailCost.Text = (curItem.PriceCollection.RetailPrice * qty).ToString("F2");
                }
        ...
        void ntxtPurchasePrice_TextChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    purchaseCost = price * qty / curItem.PackQty;
                }
                else
                {
                ...
                if (this.cbAutoAdjustPrice.Checked)
                {
                    FS.HISFC.Models.Pharmacy.Item itemNewPurchasePrice = curItem.Clone();
                    ////使用的是最小单位，价格乘以包装数量
                    //if (this.ncmbItemUnit.Tag.ToString() == "1")
                    //{
                    //    price = price * curItem.PackQty;
                    //}
                    itemNewPurchasePrice.PriceCollection.PurchasePrice = price;
                    string formula = this.GetRetailPriceComputeFormula(itemNewPurchasePrice);
                    try
                    {
                        this.ntxtRetailPrice.Text = FS.FrameWork.Function.NConvert.ToDecimal(FS.FrameWork.Public.String.ExpressionVal(formula)).ToString("F2");
                        //decimal retailPrice = FS.FrameWork.Function.NConvert.ToDecimal(this.ntxtRetailPrice.Text);
                        //itemNewPurchasePrice.PriceCollection.RetailPrice = retailPrice;
                    }
                    catch
                    {
                    }
        ...
        void ntxtValidDate_Leave(object sender, EventArgs e)
        {
         ...
         void nTxtDeliverTime_Leave(object sender, EventArgs e)
        {
            this.nTxtDeliverTime.Text = this.TryConvertToDateTimeFormat(this.nTxtDeliverTime.Text);
        }
        ...
        void cbAutoAdjustPrice_CheckStateChanged(object sender, EventArgs e)
        {
            SOC.Public.XML.SettingFile.SaveSetting(this.settingFile, "AutoAjustPrice", "Value", this.cbAutoAdjustPrice.Checked ? "True" : "False");
            //if (this.cbAutoAdjustPrice.Checked)
            //{
            //    this.ntxtRetailPrice.Enabled = true;
            //    this.ntxtRetailPrice.ReadOnly = false;
            //}
        }
        ...
        public int Clear(bool isClearInvoiceInfo, bool isClearDeliveryInfo,bool isClearDeliveryTime)
        {
            foreach (Control c in ngbInputInfo.Controls)
            {
                if (c is Label)
                ...
                if (!isClearInvoiceInfo)
                {
                    ...
                    //else if (c == this.ncmbInvoiceType)
                    //{
                    //    continue;
                    //}
                    ...
                    else if (c == this.nTxtDeliverTime)
                    {
                        continue;
                    }
                }
        ...
        public int Clear(bool isClearInvoiceInfo, bool isClearDeliveryInfo)
        {
            foreach (Control c in ngbInputInfo.Controls)
            {
                ...
                if (!isClearDeliveryInfo && (c == this.ntxtDeliveryNO|| c == this.nTxtDeliverTime))
                {
                    continue;
                }
                c.Text = "";
        ...
        public int Init()
        {
            ...
            this.ntxtInvoiceDate.Leave += new EventHandler(ntxtInvoiceDate_Leave);
            this.nTxtDeliverTime.Leave -= new EventHandler(nTxtDeliverTime_Leave);
            this.nTxtDeliverTime.Leave += new EventHandler(nTxtDeliverTime_Leave);
            ...
            this.cbAutoAdjustPrice.CheckStateChanged -= new EventHandler(cbAutoAdjustPrice_CheckStateChanged);
            this.cbAutoAdjustPrice.CheckStateChanged += new EventHandler(cbAutoAdjustPrice_CheckStateChanged);
            ...
            string autoAjustPrice = SOC.Public.XML.SettingFile.ReadSetting(this.settingFile, "AutoAjustPrice", "Value", this.cbAutoAdjustPrice.Checked ? "True" : "False");
            this.cbAutoAdjustPrice.Checked = FS.FrameWork.Function.NConvert.ToBoolean(autoAjustPrice);
            //if (this.cbAutoAdjustPrice.Checked)
            //{
            //    this.ntxtRetailPrice.Enabled = true;
            //    this.ntxtRetailPrice.ReadOnly = false;
        ...
        public int SetInputObject(FS.HISFC.Models.Pharmacy.Item item, FS.HISFC.Models.Pharmacy.Storage storage, bool isUseBatchNO, DateTime defaultValidDate)
        {
            ...
            if (storage != null && storage.Item != null)
            {
                ...
                this.ntxtRetailPrice.Text = storage.Item.PriceCollection.RetailPrice.ToString("F2");
            }


            if (this.cbAutoAdjustPrice.Checked)
            {
                string formula = this.GetRetailPriceComputeFormula(curItem);
                try
                {
                    this.ntxtRetailPrice.Text = FS.FrameWork.Function.NConvert.ToDecimal(FS.FrameWork.Public.String.ExpressionVal(formula)).ToString("F2");
                }
            ...
            if (storage != null)
            {
                if (curItem.SysClass.ID.ToString() != "PCC")
                {
                    this.ntxtValidDate.Text = storage.ValidTime.ToString("yyyyMMdd");
                }
                else
                {
                    this.ntxtValidDate.Text = DateTime.Now.AddYears(2).ToString("yyyyMMdd");
 
                }
            }


            if (this.nTxtDeliverTime.Text == null || this.nTxtDeliverTime.Text == "")
            {
        ...
        {
            string deptCode = controler.QueryControlerInfo("pCode");
            FS.HISFC.Models.Pharmacy.Storage storage = new FS.HISFC.Models.Pharmacy.Storage();
            storage = sto.GetLatestStorage(deptCode, item.ID);
            return this.SetInputObject(item, storage, true, DateTime.Now.AddYears(2));
        }
        ...
        public int SetInputObject(FS.HISFC.Models.Pharmacy.Input input, FS.HISFC.Models.Pharmacy.Item item)
        {
            ...
            //单价
            this.ntxtPurchasePrice.Text = purchasePrice.ToString("F2").TrimEnd('0').TrimEnd('.');
            this.ntxtRetailPrice.Text = retailPrice.ToString("F2").TrimEnd('0').TrimEnd('.');
        ...
        public FS.HISFC.Models.Pharmacy.Input GetInputObject()
        {
            ...
            if (decimal.TryParse(this.ntxtRetailPrice.Text, out retailPrice))
            {
                //使用的是最小单位，价格乘以包装数量
                if (this.ncmbItemUnit.Tag.ToString() == "1")
                {
            ...
            {
                Function.ShowMessage("零售价不是数字，请修改！", MessageBoxIcon.Information);
                this.ntxtRetailPrice.Select();
                this.ntxtRetailPrice.Focus();
                return null;
            }
            if (retailPrice < 0)
            {
                Function.ShowMessage("零售价必须大于等于零，请修改！", MessageBoxIcon.Information);
                this.ntxtRetailPrice.Select();
                this.ntxtRetailPrice.Focus();
                return null;
            }
            if (input.Item.PriceCollection.RetailPrice != retailPrice)
            {
                if (this.cbAutoAdjustPrice.Checked)
                {
                    FS.FrameWork.WinForms.Classes.Function.ShowWaitForm("正在调价，请稍后...");
                    ...
                    string adjustPriceBillNO = this.adjustMgr.GetAdjustPriceBillNO();
                    if (adjustPriceBillNO == null)
                    {
                        FS.FrameWork.WinForms.Classes.Function.HideWaitForm();
                        Function.ShowMessage("调价失败，请向系统管理员联系并报告错误：获取新调价单号出错，" + this.adjustMgr.Err, MessageBoxIcon.Error);
                        return null;
                    }

                    info.ID = adjustPriceBillNO;                        //调价单号
                    info.SerialNO = 1;                       //调价单内序号
                    info.StockDept.ID = "7005";                 //调价科室
                    ...
                    info.Operation.Oper.OperTime = this.adjustMgr.GetDateTimeFromSysDateTime();                     //操作时间


                    info.InureTime = info.Operation.Oper.OperTime;                    //生效时间
                    info.IsDDAdjust = false;

                    info.AfterRetailPrice = retailPrice; //调价后零售价
                    info.AfterWholesalePrice = info.Item.PriceCollection.WholeSalePrice;
                    info.Memo = this.nlbRate.Text;
                    ...
                    if (this.adjustMgr.InsertAdjustPriceInfo(info) == -1)
                    {
                        ...
                        Function.ShowMessage("调价失败，请向系统管理员联系并报告错误：" + this.adjustMgr.Err, MessageBoxIcon.Error);
                        return null;
                    }
                    ...
                    FS.HISFC.Models.Pharmacy.Item newPriceItem = this.adjustMgr.GetItem(info.Item.ID);
                    if (newPriceItem == null)
                    {
                        FS.FrameWork.WinForms.Classes.Function.HideWaitForm();
                        Function.ShowMessage("获取药品基本信息失败，请向系统管理员联系并报告错误：" + this.adjustMgr.Err, MessageBoxIcon.Error);
                        return null;
                    }
                    if (newPriceItem.PriceCollection.RetailPrice != retailPrice)
                    {
                        FS.FrameWork.WinForms.Classes.Function.HideWaitForm();
                        Function.ShowMessage("调价没有执行，请手工调价后入库", MessageBoxIcon.Error);
                        return null;
                    }
                    FS.FrameWork.WinForms.Classes.Function.HideWaitForm();
                    input.Item.PriceCollection.RetailPrice = retailPrice;
        ...
        protected string GetRetailPriceComputeFormula(FS.HISFC.Models.Pharmacy.Item item)
        {
            ...
            if (hsFormula.Contains(item.ID))
            {
                formula = hsFormula[item.ID].ToString();
            ...
            this.nlbRate.Text = formula;
            hsFormula.Add(item.ID, formula);
