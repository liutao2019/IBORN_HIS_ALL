...
namespace FS.SOC.Local.Pharmacy.Print.HuanShi
{
    ...
    public partial class ucPhaOutputBill : ucBaseControl, Base.IPharmacyBillPrint
    {
        ...
        private void SetPrintData(ArrayList al, int inow, int icount, string title)
        {
            ...
            //新增备注
            if (string.IsNullOrEmpty(info.Memo)||"即入即出"== info.Memo)
            {
            ...
            for (int i = 0; i < al.Count; i++)
            {
                ...
                this.neuFpEnter1_Sheet1.Cells[i, 2].Text = SOC.HISFC.BizProcess.Cache.Common.GetDrugTypeName(output.Item.Type.ID);//药品类别
                this.neuFpEnter1_Sheet1.Cells[i, 3].Text = output.Item.Specs;//规格		
                ...
                if (output.ShowState=="0")
                {
                    ...
                    this.neuFpEnter1_Sheet1.Cells[i, 4].Text = (count / output.Item.PackQty).ToString();//数量				
            ...
            this.neuLabel2.Text = "总毛利: " + (((TotCost)hsTotCost[info.OutListNO]).retailCost - ((TotCost)hsTotCost[info.OutListNO]).purchaseCost).ToString(Function.GetCostDecimalString());
