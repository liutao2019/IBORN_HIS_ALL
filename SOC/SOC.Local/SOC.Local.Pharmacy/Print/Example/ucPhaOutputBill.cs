...
using FS.SOC.Local.Pharmacy.Print.Example;
        ...
        private void SetPrintData(ArrayList al, int inow, int icount, string title)
        {
            ...
            for (int i = 0; i < al.Count; i++)
            {
                ...
                if (output.ShowState=="0")
                {
                    ...
                    this.neuFpEnter1_Sheet1.Cells[i, 8].Value = output.PriceCollection.WholeSalePrice / output.Item.PackQty;
                    this.neuFpEnter1_Sheet1.Cells[i, 10].Value = output.StoreQty.ToString();
                ...
                else
                {
                    ...
                    this.neuFpEnter1_Sheet1.Cells[i, 8].Value = output.PriceCollection.WholeSalePrice;
                    this.neuFpEnter1_Sheet1.Cells[i, 10].Value = (output.StoreQty / output.Item.PackQty).ToString();
                }
                //this.neuFpEnter1_Sheet1.Cells[i, 5].Text = BillPrintFun.GetItem(output.Item.ID).Product.ProducingArea;
               
                //出库单将产地改为生产厂家
                if (string.IsNullOrEmpty(output.Producer.ID))
                {
                    this.neuFpEnter1_Sheet1.Cells[i, 5].Text = "";
                }
                else
                {
                    //FS.HISFC.Management.Pharmacy.Constant constant = new FS.HISFC.Management.Pharmacy.Constant();
                    //FS.HISFC.Models.Pharmacy.Company company = new FS.HISFC.Models.Pharmacy.Company();
                    //company = constant.QueryCompanyByCompanyID(output.Producer.ID);
                    //this.neuFpEnter1_Sheet1.Cells[i, 5].Text = company.Name;
                    this.neuFpEnter1_Sheet1.Cells[i, 5].Text = FS.SOC.HISFC.BizProcess.Cache.Pharmacy.GetProducerName(output.Producer.ID);
                }
                ...
                switch(output.Item.Type.ID)
                {
                    case "B":
                        ...
                        sumBWholeCost += output.WholeSaleCost;
                        break;

                    default:
                        sumDrugPurCost += output.PurchaseCost;
                        sumDrugWholeCost += output.WholeSaleCost;
                        break;
                ...
                this.neuFpEnter1_Sheet1.Cells[i, 6].Text = output.BatchNO;
                this.neuFpEnter1_Sheet1.Cells[i, 9].Value = output.WholeSaleCost.ToString("F4");
