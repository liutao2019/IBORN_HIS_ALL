                ...
                foreach (System.Data.DataRow r in dsTmp.Tables[0].Rows)
                {
                    if (r[FS.SOC.Local.Pharmacy.Base.PrintBill.ColSet.二级权限代码.ToString()].ToString() == class2Code)
                    {
                        if (r[FS.SOC.Local.Pharmacy.Base.PrintBill.ColSet.三级权限代码.ToString()].ToString() == class3Code)
                        {
                            ...
                            try
                            {
                                string sortTemp = r[FS.SOC.Local.Pharmacy.Base.PrintBill.ColSet.排序方式.ToString()].ToString();
                                if (sortTemp == FS.SOC.Local.Pharmacy.Base.PrintBill.SortType.货位号.ToString())
                                {
                                    pringBill.Sort = FS.SOC.Local.Pharmacy.Base.PrintBill.SortType.货位号;
                                }
                                else if (sortTemp == FS.SOC.Local.Pharmacy.Base.PrintBill.SortType.类别货位号.ToString())
                                {
                                    pringBill.Sort = FS.SOC.Local.Pharmacy.Base.PrintBill.SortType.类别货位号;
                                }
                                else if (sortTemp == FS.SOC.Local.Pharmacy.Base.PrintBill.SortType.物理顺序.ToString())
                                {
                                    pringBill.Sort = FS.SOC.Local.Pharmacy.Base.PrintBill.SortType.物理顺序;
                                }
