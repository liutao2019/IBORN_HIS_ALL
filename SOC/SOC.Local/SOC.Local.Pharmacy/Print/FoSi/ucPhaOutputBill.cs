...
namespace FS.SOC.Local.Pharmacy.Print.FoSi
{
    ...
    public partial class ucPhaOutputBill : ucBaseControl, Base.IPharmacyBillPrint
    {
        ...
        private void SetPrintData(ArrayList al, int inow, int icount, string title)
        {
            ...
            if (!string.IsNullOrEmpty(FS.SOC.HISFC.BizProcess.Cache.Common.GetDeptName(info.TargetDept.ID)))
            {
            ...
            this.lblSignOper1.Text = "制表人:" + FS.SOC.HISFC.BizProcess.Cache.Common.GetEmployeeName(info.Operation.ExamOper.ID) + " " + DateTime.Now.ToString();
            ...
            for (int i = 0; i < al.Count; i++)
            {
                ...
                if (output.ShowState=="0")
                {
                    ...
                    this.neuFpEnter1_Sheet1.Cells[i, 6].Value = output.PriceCollection.PurchasePrice / output.Item.PackQty;
                ...
                this.neuFpEnter1_Sheet1.Cells[i, 7].Value = output.PurchaseCost.ToString(Function.GetCostDecimalString());
        ...
        public int Print()
        {
            ...
            foreach (ArrayList alPrint in hs.Values)
            {
                ...
                ////保证只有第一页有标题，最后一页有页尾，要修改分页
                //int pageTotNum=1;
                //int currentCount=0;
                //while (alPrint.Count > currentCount)
                //{
                //    if (pageTotNum == 1)
                //    {
                //        currentCount = printBill.RowCount;
                //    }
                //}
