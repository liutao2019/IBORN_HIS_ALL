...
namespace FS.SOC.Local.Pharmacy.Print.FoSi
{
    ...
    public partial class ucPhaBackOutBill : ucBaseControl, Base.IPharmacyBillPrint
    {
        ...
        private void SetPrintData(ArrayList al, int inow, int icount, string title)
        {
            ...
            this.lblSignOper1.Text = "制单人:" + SOC.HISFC.BizProcess.Cache.Common.GetEmployeeName(info.Operation.ExamOper.ID) + " " + DateTime.Now.ToString(); //BillPrintFun.GetEmplName(info.Operation.ExamOper.ID);
            ...
            for (int i = 0; i < al.Count; i++)
            {
                ...
                if (SOC.HISFC.BizProcess.Cache.Common.GetDept(output.StockDept.ID).DeptType.ID.ToString() == "P")
                {
                    ...
                    this.neuFpEnter1_Sheet1.Cells[i, 9].Value = (output.Item.PriceCollection.RetailPrice / output.Item.PackQty).ToString("F4");//零售价
                }
                else
                {
                ...
                this.neuFpEnter1_Sheet1.Cells[i, 10].Value = output.RetailCost.ToString(Function.GetCostDecimalString()); //退库金额
