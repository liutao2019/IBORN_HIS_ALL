...
namespace FS.SOC.Local.Pharmacy.Base
{
    ...
    public partial class BaseReport : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        /// 是否需要将开始时间设置为当前月初1号
        /// </summary>
        private bool isNeedInitBeginDateToFirst = false;

        /// <summary>
        /// 是否需要将开始时间设置为当前月初1号
        /// </summary>
        [Description("是否需要将开始时间设置为当前月初1号"), Category("Time时间设置"), Browsable(true), DefaultValue(false)]
        public bool IsNeedInitBeginDateToFirst
        {
            get
            {
                return this.isNeedInitBeginDateToFirst;
            }
            set
            {
                this.isNeedInitBeginDateToFirst = value;
            }
        }
        ...
        /// 是否锁定全部单元格
        /// </summary>
        private bool isLockAllCells = true;

        [Description("是否锁定全部单元格"), Category("FarPoint设置"), Browsable(true), DefaultValue(true)]
        public bool IsLockAllCells
        {
            get
            {
                return isLockAllCells;
            }
            set
            {
                isLockAllCells = value;
            }
        }
        ...
        private int initTime()
        {
            ...
            if (this.isNeedInitBeginDateToFirst && this.dtStart.Value.Day > 1)
            {
                this.dtStart.Value = this.dtStart.Value.AddDays(-(this.dtStart.Value.Day - 1));
            }
            else
            {
        ...
        public void LockAllCells()
        {
            if (isLockAllCells)
            {
                for (int i = 0; i < fpSpread1_Sheet1.RowCount; i++)
                {
                    ...
                    {
                        fpSpread1_Sheet1.Cells[i, j].Locked = true;
                    }
                }
                for (int i = 0; i < fpSpread1_Sheet2.RowCount; i++)
                {
                    for (int j = 0; j < fpSpread1_Sheet2.ColumnCount; j++)
                    {
                        fpSpread1_Sheet2.Cells[i, j].Locked = true;
                    }
                }
            }
        }
        ...
        private int sumTot()
        {
            try
            {
                ...
                if (this.sumColIndexs[0] != '0')
                {
                    ...
                    this.fpSpread1_Sheet1.Cells[this.fpSpread1_Sheet1.RowCount - 1, 0].Tag = "合计：";
                }
            }
            catch (Exception ex)
            {
        ...
        public int Init()
        {
            ...
            this.fpSpread1.AutoSortingColumn += new FarPoint.Win.Spread.AutoSortingColumnEventHandler(fpSpread1_AutoSortingColumn);

            this.fpSpread1.AutoSortedColumn += new FarPoint.Win.Spread.AutoSortedColumnEventHandler(fpSpread1_AutoSortedColumn);
            return 0;


        }

        void fpSpread1_AutoSortedColumn(object sender, FarPoint.Win.Spread.AutoSortedColumnEventArgs e)
        {
            this.sumTot();
        }
        ...
        {
            if (this.sumColIndexs.Length > 0)
            {
                if (this.fpSpread1_Sheet1.Rows.Count > 0)
                {
                    int rowIndex = this.fpSpread1_Sheet1.Cells["合计："].Row.Index;
        ...
        public string[] GetQueryConditions()
        {
            if (this.IsDeptAsCondition && this.IsTimeAsCondition && this.IsUseCustomType)
            {
                ...
                else if (!string.IsNullOrEmpty(this.cmbCustomType.Text))
                {
                    parm[3] = this.cmbCustomType.Text.ToString();
                }
                return parm;
            }
            ...
            else if (!this.IsDeptAsCondition && this.IsTimeAsCondition && this.IsUseCustomType)
            {
                ...
                else if(!string.IsNullOrEmpty(this.cmbCustomType.Text.Trim()))
                {
                    parm[2] = this.cmbCustomType.Text.Trim();
                }
                return parm;
            }
            else if (!this.IsDeptAsCondition && this.IsTimeAsCondition && !this.IsUseCustomType)
            ...
            else if (this.IsDeptAsCondition && !this.IsTimeAsCondition && this.IsUseCustomType)
            {
                ...
                else if (string.IsNullOrEmpty(this.cmbCustomType.Tag.ToString()) && !string.IsNullOrEmpty(this.cmbCustomType.Text))
                {
                    parm[1] = this.cmbCustomType.Text.ToString();
                }
                return parm;
            }
            else if (this.IsDeptAsCondition && !this.IsTimeAsCondition && !this.IsUseCustomType)
