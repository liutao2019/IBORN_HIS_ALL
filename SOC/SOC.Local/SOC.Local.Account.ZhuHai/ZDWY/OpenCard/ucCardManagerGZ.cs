...
namespace FS.SOC.Local.Account.ZhuHai.ZDWY.OpenCard
{
    ...
    /// 就诊卡发放
    /// </summary>
    public partial class ucCardManagerGZ : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        ...
        /// 预交金打印实体
        /// </summary>
        FS.HISFC.BizProcess.Interface.Account.IPrintPrePayRecipe Iprint = null;
        ...
        /// 是否根据身份证号判断是否办过卡
        /// </summary>
        private bool isJudgeByIDCard = true;
        ...
        /// 是否是默认密码
        /// </summary>
        private bool isDefaultPassword = false;
        ...
        /// 是否是默认密码
        /// </summary>
        [Category("控件设置"), Description("办卡时是否使用默认密码")]
        public bool IsDefaultPassword
        {
            get { return isDefaultPassword; }
            set { isDefaultPassword = value; }
        }

        private bool isStopHisCard = false;
        /// <summary>
        /// 办卡时是否将之前的卡停了 默认为false
        /// </summary>
        [Category("控件设置"), Description("办卡时是否将之前的卡停了")]
        public bool IsStopHisCard
        {
            get { return isStopHisCard; }
            set { isStopHisCard = value; }
        }
        ...
        /// </summary>
        private bool isDefaultInvoiceNO = true;
        [Category("控件设置"), Description("是否自己根据规则生成发票号")]
        protected bool IsDefaultInvoiceNO
        {
        ...
        /// <summary>
        /// 退卡时退费 0=不退，1=退费 默认为0
        /// </summary>
        [Category("控件设置"), Description("退卡时是否退费 0不退 1退费")]
        public string ReturnCardReturnFee
        {
            get { return returnCardReturnFee; }
            set { returnCardReturnFee = value; }
        }
        /// <summary>
        /// 新建卡是否收取卡成本费，0=不收取，1=收取，2=按卡类别收取 默认为0
        /// </summary>
        [Category("控件设置"), Description("办卡时是否收费 0不收 1收取 2按卡类别收取")]        
        public string IsAcceptCardFee { get { return this.isAcceptCardFee; } set { this.isAcceptCardFee = value; } }
        /// <summary>
        /// 补卡是否收取卡成本费 0=不收取，1=收取，2=按卡类别收取 默认为0
        /// </summary>
        [Category("控件设置"), Description("补卡时是否收费 0不收取 1收取 2按卡类别收取")]
        public string IsAcceptChangeCardFee
        {
            get { return isAcceptChangeCardFee; }
            set { isAcceptChangeCardFee = value; }
        }
        ...
        [Category("输入设置"), Description("是否输入电话号码")]
        public bool IsInputLinkPhone
        {
            get { return ucRegPatientInfo1.IsInputLinkPhone; }
            set { this.ucRegPatientInfo1.IsInputLinkPhone = value; }
        }
        [Category("输入设置"), Description("是否输入手机")]
        public bool IsInputPhoneNumber
        {
            get { return ucRegPatientInfo1.IsInputPhoneNumber; }
            set { this.ucRegPatientInfo1.IsInputPhoneNumber = value; }
        }
        ...
        private bool Valid()
        {
            ...
            if (MarkType.Memo == "01")
            {
                ...
                int result = Function.OperMCard(ref McardNo, ref  error);
                if (result != -1)
                {
        ...
        protected virtual void Save()
        {
            ...
            if (blnIDCardNoOnly)
            {
                ...
                if (!string.IsNullOrEmpty(strIDCardType) && !string.IsNullOrEmpty(strIDCardNo))
                {
                    ...
                    //if (list != null) list = list.Where(a => a.MarkType.ID == MarkType.ID).ToList();

                    //如果患者信息有多条,如果界面上输入了病历号,那么将这些病历号相同的信息移出,剩下的是病历号不同的
            ...
            //当正常发卡操作的时候回新建立CardNO形成新的患者信息
            //在补发的时候只更新患者信息
            if (!string.IsNullOrEmpty(tempCardNO) && IsStopHisCard)
            {
                //原来方法查询卡类型有问题，暂时传入全部卡类型
                lstHistoryCard = accountManager.GetMarkList(tempCardNO, MarkType.ID, "1");
            ...
            accountCard.MarkType = this.cmbMarkType.SelectedItem;
            ...
            #region 发卡计费实体处理
            string CardFeeInvoiceNO = "";
            if (IsDefaultInvoiceNO) //中大六院获取发票号码
            {
                CardFeeInvoiceNO = this.accountManager.GetNewInvoiceNO("A");
            }
            else
            {
                FS.HISFC.BizProcess.Integrate.Fee feeIntegrate = new FS.HISFC.BizProcess.Integrate.Fee();
                CardFeeInvoiceNO = feeIntegrate.GetNewInvoiceNO("A");
                if (CardFeeInvoiceNO == null || CardFeeInvoiceNO == string.Empty)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                    MessageBox.Show("获得发票号出错!" + feeIntegrate.Err);
                    this.txtpay.Focus();
                    return;
                }
            }
            if ((accountCard.ReFlag == "0" && this.IsAcceptCardFee != "0") || (accountCard.ReFlag == "1" && this.IsAcceptChangeCardFee != "0"))
            {
                ...
                cardFee.InvoiceNo = accountCard.MarkNO;
                cardFee.Print_InvoiceNo = accountCard.MarkNO;
                cardFee.CardNo = tempCardNO;
                cardFee.ClinicNO = cardFee.CardNo;
                ...
                cardFee.PayType.ID = "CA";
                cardFee.PayType.Name = "现金";
                cardFee.InvoiceNo = CardFeeInvoiceNO;
                cardFee.Tot_cost = FS.FrameWork.Function.NConvert.ToDecimal(controlParamIntegrate.GetControlParam<string>(FS.HISFC.BizProcess.Integrate.AccountConstant.AcceptCardFee, false));
            ...
            if (ReturnCardReturnFee == "1" && lstStopBackCard != null && lstStopBackCard.Count > 0 && IsStopHisCard)
            {
                    ...
                    if (backCard.MarkStatus == MarkOperateTypes.Cancel)
                    {
                        ...
                        if (iRes > 0 && lstTempCardFee != null && lstTempCardFee.Count > 0)
                        {
                            for (int idx = 0; idx < lstTempCardFee.Count; idx++)
                            {
                                ...
                                if (string.IsNullOrEmpty(tempCardFee.ClinicNO))
                                    tempCardFee.ClinicNO = tempCardFee.Patient.PID.CardNO;
            ...
            //停掉历史卡记录
            if (lstStopBackCard != null && lstStopBackCard.Count > 0&&IsStopHisCard)
            {
            ...
            if (accountCard.MarkType.ID=="1")
            {
                #region 开户,并充值预交金
                //如果历史账户为空,则进行开户操作

                //预交金
                decimal decPay = FS.FrameWork.Function.NConvert.ToDecimal(txtpay.NumericValue);
                txtpay.Text = "0.00";
                decimal decCardFee = 0;
                if (IsAcceptCardFee == "1")
                    if (lstCardFee != null)
                        decCardFee = lstCardFee.Sum(c => c.Tot_cost);
                if (decPay < decCardFee)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                    MessageBox.Show("金额不足工本费" + decCardFee + "元", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                try
                {
                    ...
                    {
                        account = accountManager.GetAccountByCardNoEX(accountCard.Patient.PID.CardNO);
                        if (account == null)
                        {
                            FS.FrameWork.Management.PublicTrans.RollBack();
                            MessageBox.Show("建立账号错误", "错误");
                            return;
                        }
                    }
                    ...
                    prePay.FT.PrepayCost = decPay - decCardFee;
                    prePay.InvoiceNO = invoiceNO;
                    prePay.ValidState = FS.HISFC.Models.Base.EnumValidState.Valid;
                    prePay.PrePayOper.ID = accountManager.Operator.ID;
                    prePay.PrePayOper.Name = accountManager.Operator.Name;
                    prePay.PrePayOper.OperTime = nowTime;
                    prePay.AccountNO = account.ID;
                    prePay.IsHostory = false;
                    prePay.Memo = accountCard.Patient.PID.CardNO;
                ...
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                    MessageBox.Show("建立账户失败"+ex.Message, "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
           ...
           // FrameWork.Models.NeuObject MarkType = this.cmbMarkType.SelectedItem as FrameWork.Models.NeuObject;
            ...
            //    IOperCard opercard = FS.FrameWork.WinForms.Classes.
            ...
            decimal van = 0;
            int result = accountManager.GetVacancy(prePay.Patient.PID.CardNO, ref van);
            if (result > 0)
                prePay.FT.TotCost = van;
            // 打印卡费用凭证
            ...
            //ArrayList ykal = FS.SOC.HISFC.BizProcess.CommonInterface.CommonController.Instance.QueryConstant("YkDept");

            //string ykdeptid = ((controlParamIntegrate.Operator) as FS.HISFC.Models.Base.Employee).Dept.ID;
            //if (ykal != null)
            //{
            ...
            //        if (ykdeptid == obj.ID)
            //        {
            //            txtMarkNo.Text = accountManager.GetMaxMcard("2");

            //            this.cmbMarkType.SelectedIndex = 1;
            //            break;
            //        }
            //    }
            //}
        ...
        private void PrintPrePayRecipe(PrePay temprePay)
        {
            if (Iprint == null)
            {
                Iprint = FS.FrameWork.WinForms.Classes.
                UtilInterface.CreateObject<FS.HISFC.BizProcess.Interface.Account.IPrintPrePayRecipe>(this.GetType());
        ...
        private void PrintCardFee(List<AccountCardFee> lstCardFee)
        {
            ...
            foreach (AccountCardFee cardFee in lstCardFee)
            {
                if (cardFee.TransType == FS.HISFC.Models.Base.TransTypes.Positive)
                {
                    if (iPrint == null)
                    {
                        iPrint = FS.FrameWork.WinForms.Classes.UtilInterface.CreateObject<IPrintCardFee>(this.GetType());
                        if(iPrint==null)
                            continue;
                    }
                ...
                else
                {
                    ...
                    {
                        iPrintReturn = FS.FrameWork.WinForms.Classes.UtilInterface.CreateObject<IPrintReturnCardFee>(this.GetType());
                        if (iPrintReturn == null)
                            continue;
                    }
        ...
        protected virtual int QueryPatientInfo(string patientName, string sexId, string pactId, string CassNo, string idCardType, string idCard, string SSN,string phoneNumber)
        {
            ...
            //{222C3EA1-F613-4904-89A0-AD01CAB96166}
        ...
        private void ucCardManager_Load(object sender, EventArgs e)
        {
            ...
            iPrintLable = FS.FrameWork.WinForms.Classes.UtilInterface.CreateObject<IPrintLable>(this.GetType());
            iPrint = FS.FrameWork.WinForms.Classes.UtilInterface.CreateObject<IPrintCardFee>(this.GetType());
            iPrintReturn = FS.FrameWork.WinForms.Classes.UtilInterface.CreateObject<IPrintReturnCardFee>(this.GetType());
