...
namespace FS.SOC.Local.DrugStore.ShenZhen.BinHai.Inpatient
{
    ...
    public class DrugBillInterfaceImplement : FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.IDrugBillClass
    {
        ...
        public FS.HISFC.Models.Pharmacy.DrugBillClass GetDrugBillClass(FS.HISFC.Models.Pharmacy.ApplyOut applyOut)
        {
            ...
            //口服非固态摆药单
            if (SOC.HISFC.BizProcess.Cache.Common.IsPOUsage(applyOut.Usage.ID) && SOC.Local.DrugStore.ShenZhen.Common.Function.IsSpecialDoseModel(applyOut.Item.DosageForm.ID))
            {
                billClass.ID = "K";// "120821153455";
                if (hsDrugBillClass.Contains(billClass.ID))
                {
                    billClass = hsDrugBillClass[billClass.ID] as FS.HISFC.Models.Pharmacy.DrugBillClass;
                }
            ...
            if (SOC.HISFC.BizProcess.Cache.Common.IsInjectUsage(applyOut.Usage.ID) && !SOC.Local.DrugStore.ShenZhen.Common.Function.IsSpecialDoseModel(applyOut.Item.DosageForm.ID))
            {  //针剂长期医嘱摆药单
                if (applyOut.OrderType.ID == "CZ")
                {
                    billClass.ID = "TC";
        ...
        private ArrayList SaveDefaultBill()
        {
            ...
            FS.HISFC.Models.Pharmacy.DrugBillClass kDrugBill = new FS.HISFC.Models.Pharmacy.DrugBillClass();
            kDrugBill.ID = "K";
            kDrugBill.Name = "口服非固态摆药单";                       //摆药分类名称
            kDrugBill.PrintType.ID = FS.HISFC.Models.Pharmacy.BillPrintType.enuBillPrintType.D;           //打印类型
            kDrugBill.DrugAttribute.ID = FS.HISFC.Models.Pharmacy.DrugAttribute.enuDrugAttribute.T; //摆药类型
            kDrugBill.IsValid = true;              //是否有效
            kDrugBill.Memo = "系统默认的摆药单";               //备注
            al.Add(kDrugBill);
