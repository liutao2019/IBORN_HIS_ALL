...
//using Neusoft.SOC.Local.DrugStore.Common;

namespace Neusoft.SOC.Local.DrugStore.ShenZhen.BinHai.Outpatient
{
    public partial class ucDrugList1 : UserControl
    {
        public ucDrugList1()
        {
            InitializeComponent();
        ...
        /// 药库管理类
        /// </summary>
        Neusoft.SOC.HISFC.BizLogic.Pharmacy.Item itemManager = new Neusoft.SOC.HISFC.BizLogic.Pharmacy.Item();
        ...
        private void AddAllData(System.Collections.ArrayList al, Neusoft.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, string diagnose,string hospitalName)
        {
            if (al.Count > 0)
            {
                //有效的药品页数
                ...
                //有效的药品数据
                Dictionary<int, List<Neusoft.HISFC.Models.Pharmacy.ApplyOut>> applyOutPageList = new Dictionary<int, List<Neusoft.HISFC.Models.Pharmacy.ApplyOut>>();
                List<Neusoft.HISFC.Models.Pharmacy.ApplyOut> applyOutList = new List<Neusoft.HISFC.Models.Pharmacy.ApplyOut>();
                //无效的药品数据
                Dictionary<int, List<Neusoft.HISFC.Models.Pharmacy.ApplyOut>> applyOutInValidPageList = new Dictionary<int, List<Neusoft.HISFC.Models.Pharmacy.ApplyOut>>();
                List<Neusoft.HISFC.Models.Pharmacy.ApplyOut> applyOutInValidList = new List<Neusoft.HISFC.Models.Pharmacy.ApplyOut>();
                ...
                else
                {
                    ...
                    if (!hsApplyOut.Contains(applyOut.ID))
                    {
                        ...
                        if (applyOut.ValidState == Neusoft.HISFC.Models.Base.EnumValidState.Valid)
                        {
                            ...
                            if(applyOutList.Count==10)
                            {
                                ...
                                applyOutList = new List<Neusoft.HISFC.Models.Pharmacy.ApplyOut>();
                                pageNO++;
                            }

                            Neusoft.HISFC.Models.Pharmacy.ApplyOut lastApplyOut = new Neusoft.HISFC.Models.Pharmacy.ApplyOut();
                            foreach (Neusoft.HISFC.Models.Pharmacy.ApplyOut aTmp in alTmp)
                            {
                                //退费数据状态不一样
                                if(aTmp.ID == applyOut.ID)
                                {
                                    //退费后状态改变，这样从数据库查询的数据就是无效的
                                    if (aTmp.ValidState == Neusoft.HISFC.Models.Base.EnumValidState.Invalid)
                                    {
                                        return;
                                    }
                                    continue;
                                ...
                                //状态相同
                                if (aTmp.ValidState == Neusoft.HISFC.Models.Base.EnumValidState.Valid)
                                {
                    ...
                    if (applyOutInValidList.Count == 5 || (num == al.Count && applyOutInValidList.Count > 0))
                    {
                        ...
                        applyOutInValidList = new List<Neusoft.HISFC.Models.Pharmacy.ApplyOut>();
                        pageInValidNO++;
                    }
                }
                ...
                #region 设置数据并打印
                foreach (KeyValuePair<int, List<Neusoft.HISFC.Models.Pharmacy.ApplyOut>> applist in applyOutInValidPageList)
                {
                ...
                foreach (KeyValuePair<int, List<Neusoft.HISFC.Models.Pharmacy.ApplyOut>> applist in applyOutPageList)
                {
                    // 设置Lable的值
                    SetLableValue(drugRecipe,diagnose,hospitalName);
                    this.isMoreOnePage = (applyOutPageList.Count > 1);
                    foreach (Neusoft.HISFC.Models.Pharmacy.ApplyOut applayOut in applist.Value)
                    {
        ...
        private void Print()
        {
            if (isPrint == true)
            {
                ...
                print.SetPageSize(new Neusoft.HISFC.Models.Base.PageSize("OutPatientDrugBill",400, 400));
        ...
        private void SetLableValue(Neusoft.HISFC.Models.Pharmacy.DrugRecipe drugRecipe,string diagnose,string hospitalName)
        {
            ...
            //处方日期
            //lbRecipeDate.Text = "处方日期：" + Neusoft.FrameWork.Function.NConvert.ToDateTime(drugRecipe.FeeOper.OperTime).ToString("yyyy-MM-dd");
            lbRecipeDate.Text = Neusoft.FrameWork.Function.NConvert.ToDateTime(drugRecipe.FeeOper.OperTime).ToString("yyyy-MM-dd");
        ...
        /// <param name="info"></param>
        private void SetDrugDeatail(int iRow, Neusoft.HISFC.Models.Pharmacy.ApplyOut info, bool isValid)
        {
            //自定义码
            ...
            try
            {
                this.neuSpread1_Sheet1.Cells[iRow,11].Text = Neusoft.SOC.Local.DrugStore.Common.Function.GetOrder(info.OrderNO).HerbalQty.ToString();
            }
            catch { }
            ...
            else
            {
                ...
                Neusoft.SOC.HISFC.Components.Common.Function.DrawCombo(this.neuSpread1_Sheet1, 13, 12);
            }
        ...
        /// <returns></returns>
        public int PrintDrugBill(ArrayList alData, string diagnose, Neusoft.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, Neusoft.HISFC.Models.Pharmacy.DrugTerminal drugTerminal,string hospitalName)
        {
