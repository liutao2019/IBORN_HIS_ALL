...
using FS.SOC.HISFC.Components.DrugStore.Outpatient.Common;
...
namespace FS.SOC.Local.DrugStore.ShenZhen.BinHai.Outpatient.Common
{
    public partial class ucDrug : ucDrugBase, FS.FrameWork.WinForms.Classes.IPreArrange
    {
        ...
        private void Init()
        {
            ...
            if (!isCanUseRowaDrug)
            {
                if (this.ToolBarService.GetToolButton("机配") != null)
                    this.ToolBarService.GetToolButton("机配").Visible = false;
               
            }

            // 设置了出药口才可以显示机配按钮
            if (((FS.HISFC.Models.Base.Spell)(rowaTermial)).UserCode == null)
            {
                if (this.ToolBarService.GetToolButton("设备状态") != null)
                {
                    this.ToolBarService.GetToolButton("机配(F4)").Visible = false;
                    this.ToolBarService.GetToolButton("设备状态").Visible = false;
                }
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        private void RowaDrug()
        {            
            if (this.ucRecipeDetail1 != null)
            {
                string rtnText = string.Empty;
                if (this.ucRecipeDetail1.RowaDrug(ref rtnText) == -1)  
                    this.ShowMessage("机配信息: \r\n" + rtnText, MessageBoxIcon.Exclamation); 
                else
                    this.ShowMessage("机配信息: \r\n" + rtnText, MessageBoxIcon.Information); 
            }
            else
                this.ShowMessage("机配信息: \r\n ucDrug.ucRecipeDetail1对象为空!", MessageBoxIcon.Exclamation);             
        }
        ...
        private void RowaDrugStatus()
        {
            ...
            if (FS.SOC.Local.DrugStore.ShenZhen.Common.InterfaceManager.GetIMachineSender() != null)
            {

                FS.SOC.Local.DrugStore.ShenZhen.Common.InterfaceManager.GetIMachineSender().Send(applyOut, ref resultInfo, ref ERR);
            ...
            if(rowaStore != null)
               ...
               this.ShowMessage("[" + drugStoreMgr.GetDateTimeFromSysDateTime().ToString() + "]当前设备状态: " + rowaStore.User01.ToString() + "  \r\n" + "消息接口返回", MessageBoxIcon.Information); 
            else
                this.ShowMessage("[" + drugStoreMgr.GetDateTimeFromSysDateTime().ToString() + "]当前设备状态:  未获取机器状态，请检查设备情况！！"  + "  \r\n" + "消息接口返回", MessageBoxIcon.Information); 
        }
        ...
        private int Save()
        {
            ...
            //配药保存：更新调剂数
            parm = FS.SOC.Local.DrugStore.ShenZhen.Common.Function.OutpatientDrug(alData, DrugTerminal, this.StockDept, this.drugStoreMgr.Operator, true);
        ...
        protected override FS.FrameWork.WinForms.Forms.ToolBarService OnInit(object sender, object neuObject, object param)
        {
            ...
            this.ToolBarService.AddToolButton("机配", "欧娲设备配药", FS.FrameWork.WinForms.Classes.EnumImageList.D打包, true, false, null);
            this.ToolBarService.AddToolButton("设备状态", "欧娲设备状态查看", FS.FrameWork.WinForms.Classes.EnumImageList.S设备, true, false, null);
        ...
        public override void ToolStrip_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            else if (e.ClickedItem.Text == "机配")
            {
                this.RowaDrug();
            }
            else if (e.ClickedItem.Text == "设备状态")
            {
                this.RowaDrugStatus();
            }
