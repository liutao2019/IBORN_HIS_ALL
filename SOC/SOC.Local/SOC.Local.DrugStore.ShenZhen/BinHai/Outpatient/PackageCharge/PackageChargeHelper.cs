...
namespace FS.SOC.Local.DrugStore.ShenZhen.BinHai.Outpatient.PackageCharge
{
    public class PackageChargeHelper
    {
        ...
        /// <returns></returns>
        public int InsertApplyOut(FS.HISFC.Models.Registration.Register register, ref string errInfo)
        {
            if (register == null)
            {
                errInfo = "患者信息为空！";
            ...
            phaIntegrate.SetTrans(FS.FrameWork.Management.PublicTrans.Trans);
          ...
          DateTime nowTime = this.outPatientMgr.GetDateTimeFromSysDateTime() ;

          ArrayList alNoConfirmedFeeDetail = new ArrayList();
          foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList itemList in alFeeDetail)
          {
              //如果已经确认过了，就不需要重新确认
              if (itemList.Item.ItemType == FS.HISFC.Models.Base.EnumItemType.UnDrug || itemList.IsConfirmed)
              {
                  continue;
              }
              ...
              {
                  itemList.FeeOper.OperTime = nowTime;
                  itemList.FeeOper.ID = "000000";
                  FS.HISFC.Models.Pharmacy.ApplyOut applyOut = phaIntegrate.GetApplyOut(itemList.RecipeNO, itemList.SequenceNO);
                  if (string.IsNullOrEmpty(applyOut.ID))
                  {
                      //没有确认过
                      alNoConfirmedFeeDetail.Add(itemList);
                      continue;
                  }
                  int param2 = phaIntegrate.DelApplyOut(itemList.RecipeNO, itemList.SequenceNO.ToString());
                  if (param2 == -1)
                  {
                      errInfo = "删除原有出库申请数据失败,原因" + phaIntegrate.Err;
                  }
              }

          }
          int returnValue = phaIntegrate.ApplyOut(register, alNoConfirmedFeeDetail, string.Empty, nowTime, false, out  drugSendInfo);

          if (returnValue == -1)
          {
              errInfo = "插入出库申请表失败，原因：" + phaIntegrate.Err;
              return -1;
          }
          int param = this.DrugConfirm(alNoConfirmedFeeDetail);
          if (param == -1)
          {
              errInfo = "更新费用确认信息失败";
              return -1;
        ...
        /// <param name="register"></param>
        /// <returns></returns>
        public int Balance(FS.HISFC.Models.Registration.Register register, ref string errInfo)
        {
            if (register == null)
            {
                errInfo = "患者信息为空！";
        ...
        /// 根据病历号获取一天内有效挂号信息
        /// </summary>
        public ArrayList GetRegisterByCardNO(string cardNO)
        {
            if (string.IsNullOrEmpty(cardNO))
            {
                return null;
            }
            DateTime endTime = this.outPatientMgr.GetDateTimeFromSysDateTime() ;
            ...
            DateTime beginTime = endTime.AddDays(-iRegValidDays);
            //ArrayList allRegister = this.registerIntegrate.GetRegisterByCardNODate(cardNO,beginTime,endTime);
            ArrayList allRegister = this.registerIntegrate.QueryRegList(cardNO, beginTime, endTime, "1");
            if(allRegister.Count == 0 || allRegister == null)
            {
                return null;
            }
            return allRegister;
        ...
        /// </summary>
        /// <returns></returns>
        public int DrugConfirm(ArrayList allFeeItemList)
        {
            FS.HISFC.Models.Base.Employee employee = this.outPatientMgr.Operator as FS.HISFC.Models.Base.Employee;
            DateTime conFirmTime = this.outPatientMgr.GetDateTimeFromSysDateTime();
            string operCode = employee.ID;
            ArrayList allDrugItemList = null;
            foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList feeItemList in allFeeItemList)
            {
                if (feeItemList.Item.ItemType == FS.HISFC.Models.Base.EnumItemType.Drug)
                { 
                    feeItemList.ConfirmedQty = feeItemList.Item.Qty;
                    feeItemList.ConfirmOper.Dept.ID = feeItemList.ExecOper.Dept.ID;
                    feeItemList.ConfirmOper.ID = operCode;
                    feeItemList.ConfirmOper.OperTime = conFirmTime;

                    if (this.outPatientMgr.UpdateSingleTable("Fee.OutPatient.ItemDetail.UpdateBeforCharge", "1", operCode, feeItemList.ConfirmOper.Dept.ID, conFirmTime.ToString(), feeItemList.ConfirmedQty.ToString(), feeItemList.RecipeNO, feeItemList.SequenceNO.ToString(), "1") == -1)
                    {
                        return -1;
                    }
                }
        ...
        /// 更新调剂终端
        /// </summary>
        /// <param name="drugRecicpe"></param>
        /// <param name="drugTerminal"></param>
        /// <returns></returns>
        public int UpdateTerminal(FS.HISFC.Models.Pharmacy.DrugRecipe drugRecicpe, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal)
        {
            if (drugRecicpe == null || drugTerminal == null)
            {
                return -1;
            }
            if (this.outPatientMgr.UpdateSingleTable("Pharamacy.DrugStore.Update.UpdateDrugTerminal", drugTerminal.ID, drugTerminal.SendWindow.ID, drugRecicpe.StockDept.ID, drugRecicpe.RecipeNO) == -1)
            {
                return -1;
            }
            return 1;
