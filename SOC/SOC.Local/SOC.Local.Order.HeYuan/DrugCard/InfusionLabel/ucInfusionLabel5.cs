...
namespace FS.SOC.Local.Order.HeYuan.DrugCard.InfusionLabel
{
    ...
    /// [创建时间: 2010-01]<br></br>
    /// </summary>
    public partial class ucInfusionLabel5 : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Classes.IControlPrintable
    {
        public ucInfusionLabel5()
        {
            InitializeComponent();
        }
        ...
        public object ControlValue
        {
            ...
            set
            {
                ...
                foreach (FS.HISFC.Models.Order.ExecOrder info in alInfusionData)
                {
                    ...
                    //用量 因为存在合并列 列索引+1
                    this.sheetView1.Cells[iIndex, 2].Text = info.Order.DoseOnce.ToString("F4").TrimEnd('0').TrimEnd('.') + info.Order.DoseUnit.ToString();
                    this.sheetView1.Cells[iIndex, 3].Text = info.Order.Frequency.ID;
                    ...
                    if (blnLineFeeds)
                    {
                        ...
                        {
                            this.sheetView1.Cells[iIndex, 0].Text = itemName.Substring(0, length);
                            iIndex++;
                            this.sheetView1.Cells[iIndex, 0].Text = itemName.Substring(length);
                        }
                        else
                        {
                            this.sheetView1.Cells[iIndex, 0].Text = info.Order.Item.Name;//药品名称
                        }
                    }
                    else
                    {
                        this.sheetView1.Cells[iIndex, 0].Text = itemName;
                    }
                ...
                #endregion
                float height = this.sheetView1.ColumnHeader.Rows[0].Height + this.sheetView1.Rows[0].Height * (iIndex + 1) + 10;
        ...
        public bool IsSelected
        {
            ...
            set
            {
                ...
                if (value)
                {
                    ...
                    //{
                    //    this.sheetView1.Rows[i].BackColor = Color.FromArgb(224, 224, 224);
                    //}
                }
                else
                {
                    ...
                    //for (int i = 0; i < this.sheetView1.Rows.Count; i++)
                    //{
                    //    this.sheetView1.Rows[i].BackColor = Color.FromArgb(255, 255, 255);
                    //}
