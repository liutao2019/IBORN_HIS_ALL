...
namespace FS.SOC.Local.InpatientFee.GuangZhou.BizLogic
{
    /// <summary>
    /// 审批项目
    /// </summary>
    public class ApproveItemBizLogic : FS.FrameWork.Management.Database
    {
        ...
        {
            string sql = @"INSERT INTO LOCAL_FINIPB_APPROVEITEM
	                                    (
	                                    RECIPE_NO,
	                                    SEQUENCE_NO,
	                                    INPATIENT_NO,
        ...
        {
            string sql = @"UPDATE LOCAL_FINIPB_APPROVEITEM
                                        SET 
	                                        INPATIENT_NO = '{2}',
	                                        NAME = '{3}',
	                                        REG_NO ='{4}',
        ...
        {
            string sql = @"DELETE FROM LOCAL_FINIPB_APPROVEITEM
        ...
        {
            string sql = @"select max(a.org_unit_price) from fin_ipb_itemlist a where a.inpatient_no='{0}' and a.mo_exec_sqn='{1}' and a.trans_type='1' and a.package_code='{2}'";
        ...
        {
            string sql = @"select count(distinct export_filename) from local_finipb_approveitem a where a.inpatient_no='{0}' and a.export_flag='1' and trunc(a.export_date)=to_date('{1}','yyyy-mm-dd hh24:mi:ss')";
        ...
        {
            string sql = @"update local_finipb_approveitem a 
                                       set a.export_flag='1',
                                             a.export_filename='{1}',
        ...
        /// <returns></returns>
        public int ProcedueApproveFeeInfo(string inpatientNO,  string recipeNO,  int sequenceNO, FS.HISFC.Models.Base.EnumItemType itemType, FS.HISFC.Models.Base.TransTypes transType, string operCode,ref string newRecipeNO,ref int newSequenceNO)
        {
            string strSQL = @"PKG_IPB.prc_ipb_ApproveFeeInfo,P_InpatientNO,22,1,{0}, P_RecipeNO,22,1,{1},P_SequenceNO,13,1,{2},P_ItemType,22,1,{3},P_TransType,22,1,{4},P_OperCode,22,1,{5}, Par_ErrCode,13,2,1,Par_ErrText,22,2,1,par_RecipeNO,22,2,1,par_SequenceNO,13,2,1";
            string strReturn = string.Empty;

            try
            {
                strSQL = string.Format(strSQL, inpatientNO, recipeNO, sequenceNO,(int)itemType ,(int)transType, operCode);
                ...
                {
                    newRecipeNO = str[2];
                    newSequenceNO = FS.FrameWork.Function.NConvert.ToInt32(str[3]);
                }
                return 1;
            }
            catch (Exception ex)
