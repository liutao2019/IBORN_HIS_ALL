...
namespace FS.SOC.Local.InpatientFee.QiaoTou
{
    public class Function
    {
        ...
        /// 控制参数管理类
        /// </summary>
        private static FS.HISFC.BizProcess.Integrate.Common.ControlParam controlIntegrate = new FS.HISFC.BizProcess.Integrate.Common.ControlParam();
    ...
    public class PatientFee : FS.FrameWork.Management.Database
    {

        public DataSet GetPatientBalanceDayFeeList(string inpatientNo, DateTime begintime, DateTime endtime)
        {
            string strSql = "";
            DataSet dsResult = new DataSet();

            if (this.Sql.GetSql("FS.InpatientFee.BalanceFee.DayCostList", ref strSql) == -1)
            {
                this.Err = "获取sqlid为FS.InpatientFee.BalanceFee.DayCostList出错!";
                this.WriteErr();
                return null;
            }
            ...
            try
            {
                ...
                strSql = string.Format(strSql, inpatientNo, begintime.ToString(), endtime.ToString());
                if (this.ExecQuery(strSql,ref dsResult) == -1)
                {
                    this.Err = "执行sql语句FS.InpatientFee.BalanceFee.DayCostList出错!";
                    this.WriteErr();
                    return null;
                }
            }
