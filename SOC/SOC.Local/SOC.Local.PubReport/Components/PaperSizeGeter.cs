...
namespace FS.SOC.Local.PubReport.Components
{
    public class PaperSizeGetter
    {
        ...
        public static extern int DeviceCapabilities(string pDevice, string pPort, short
        fwCapabilities, IntPtr pOutput, IntPtr pDevMode);
        public static int[] Get_PaperSizes(string printer)
        {
            ...
            {
                string text2 = Marshal.PtrToStringAuto((IntPtr)(((long)ptr1) + (num2 *
                num3)), 0x40);
        ...
        /// <param name="paper">纸张名</param>
        /// <returns></returns>
        public static int Get_PaperSizes(string printer,string paper)
        {
            int iPaper = 0;
            string text1 = printer;
            int num1 = FastDeviceCapabilities(0x10, IntPtr.Zero, -1, text1);
            ...
            for (int num3 = 0; num3 < num1; num3++)
            {
                ...
                if (num4 > -1)
                {
                    ...
                    sizeArray1[num3] = System.Convert.ToInt32(num5);
                     if (text2 == paper)
                    {
                        iPaper = System.Convert.ToInt32(num5);
                    }
                }
                Marshal.FreeCoTaskMem(ptr1);
                Marshal.FreeCoTaskMem(ptr2);
        ...
        private static int FastDeviceCapabilities(short capability, IntPtr pointerToBuffer, int
        defaultValue, string printerName)
        {
            int num1 = DeviceCapabilities(printerName, OutputPort, capability,
