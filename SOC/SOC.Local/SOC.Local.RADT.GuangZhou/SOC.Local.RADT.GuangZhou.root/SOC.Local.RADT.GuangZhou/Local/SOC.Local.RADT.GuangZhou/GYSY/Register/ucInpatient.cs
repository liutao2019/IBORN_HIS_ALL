...
using Neusoft.SOC.Local.RADT.GuangZhou.GYSY.Interface;

namespace Neusoft.SOC.Local.RADT.GuangZhou.GYSY.Register
{
    /// <summary>
    /// 入院登记主界面
    /// 用接口方式实现 列表显示和登记界面
    /// </summary>
    public partial class ucInpatient : Neusoft.FrameWork.WinForms.Controls.ucBaseControl
    {
        public ucInpatient()
        {
            InitializeComponent();
        ...
        /// 列表 入院登记接口
        /// </summary>
        private Neusoft.SOC.HISFC.RADT.Interface.Register.IPatientList IRegisterList = null;
        ...
        private Neusoft.SOC.HISFC.BizProcess.CommonInterface.Common.ISave<PatientInfo> ISave = null;
        ...
        protected override ToolBarService OnInit(object sender, object neuObject, object param)
        {
            ...
            toolBarService.AddToolButton("医保", "获取医保患者", (int)Neusoft.FrameWork.WinForms.Classes.EnumImageList.Y医保, true, false, null);
        ...
        private int initInterface()
        {
            ...
            this.IRegisterList.OnSelectPatientInfo += new Neusoft.SOC.HISFC.RADT.Interface.Register.SelectPatientInfo(IRegisterListInterface_OnSelectPatientInfo);
            ...
            this.IRegister.FileName = Neusoft.FrameWork.WinForms.Classes.Function.SettingPath + "\\SOC.Components.Radt.ucRegisterInfo.xml"; 
        ...
        public int setPatientInfo(string patientNO)
        {
            ...
            else
            {
                ...
                {
                    //判断在院状态
                    if (patient.PVisit.InState.ID.ToString() == Neusoft.HISFC.Models.RADT.EnumPatientType.R.ToString()
                       || patient.PVisit.InState.ID.ToString() == Neusoft.HISFC.Models.RADT.EnumPatientType.I.ToString()
                       || patient.PVisit.InState.ID.ToString() == Neusoft.HISFC.Models.RADT.EnumPatientType.P.ToString())
        ...
        private int setOldPatientInfo(Neusoft.HISFC.Models.RADT.PatientInfo oldPatientInfo)
        {
        ...
        protected virtual int updatePatientInfo()
        {
            ...
            if (this.patientInfo.PVisit.InState.ID.ToString() != Neusoft.HISFC.Models.Base.EnumInState.R.ToString())
            {
            ...
            if (this.ISave.Saving(Neusoft.SOC.HISFC.BizProcess.CommonInterface.Common.EnumSaveType.Update, this.patientInfo) == -1)
            {
            ...
            //添加变更记录表	
            if (this.radtIntegrate.InsertShiftData(this.patientInfo.ID, Neusoft.HISFC.Models.Base.EnumShiftType.CD, "修改科室", oldDept,
            ...
            //添加变转病区更记录表
            if (this.radtIntegrate.InsertShiftData(this.patientInfo.ID, Neusoft.HISFC.Models.Base.EnumShiftType.CN, "修改病区", oldNurseCell, this.patientInfo.PVisit.PatientLocation.NurseCell) < 0)
            {
                Neusoft.FrameWork.Management.PublicTrans.RollBack();
            ...
            if (this.ISave.SaveCommitting(Neusoft.SOC.HISFC.BizProcess.CommonInterface.Common.EnumSaveType.Update, this.patientInfo) == -1)
            {
                Neusoft.FrameWork.Management.PublicTrans.RollBack();
                this.myMessageBox(this.ISave.Err, MessageBoxIcon.Stop);
                return -1;
            ...
            if (this.ISave.Saved(Neusoft.SOC.HISFC.BizProcess.CommonInterface.Common.EnumSaveType.Update, this.patientInfo) == -1)
            {
                this.myMessageBox(this.ISave.Err, MessageBoxIcon.Stop);
            }
        ...
        protected virtual int insertPatientInfo()
        {
            ...
            if (this.ISave.Saving(Neusoft.SOC.HISFC.BizProcess.CommonInterface.Common.EnumSaveType.Insert, this.patientInfo) == -1)
            {
                this.myMessageBox(this.ISave.Err, MessageBoxIcon.Stop);
                return -1;
            }
            ...
            if (isCreateMoneyAlert)
            {
                ...
                /*
                Neusoft.FrameWork.Models.NeuObject conObj = Neusoft.SOC.HISFC.BizProcess.CommonInterface.CommonController.CreateInstance().GetPact(this.patientInfo.Pact.ID);          
                if (conObj == null)
                {
                    Neusoft.FrameWork.Management.PublicTrans.RollBack();
                ...
                {
                    this.patientInfo.PVisit.MoneyAlert = Neusoft.FrameWork.Function.NConvert.ToDecimal(conObj.Memo);
                }
                */
                #endregion
                
                Neusoft.HISFC.BizLogic.Manager.Constant conStant = new Neusoft.HISFC.BizLogic.Manager.Constant();
                Neusoft.FrameWork.Models.NeuObject conStantObj = conStant.GetConstant("MONEYALERT", this.patientInfo.Pact.PayKind.ID);
                if (conStantObj==null)
                {
                    Neusoft.FrameWork.Management.PublicTrans.RollBack();
                ...
                {
                    this.patientInfo.PVisit.MoneyAlert = Neusoft.FrameWork.Function.NConvert.ToDecimal(conStantObj.Memo);
                }                          
                else
                {
                    this.patientInfo.PVisit.MoneyAlert = 0m;
                }
                if (this.radtIntegrate.UpdatePatientAlert(this.patientInfo.ID, patientInfo.PVisit.MoneyAlert, Neusoft.HISFC.Models.Base.EnumAlertType.M.ToString(), DateTime.MinValue, DateTime.MinValue) <= 0)
                {
                    Neusoft.FrameWork.Management.PublicTrans.RollBack();
            ...
            {
                Neusoft.HISFC.Models.Base.Bed bedObjTemp = this.patientInfo.PVisit.PatientLocation.Bed;
            ...
            //预交金实体
            Neusoft.HISFC.Models.Fee.Inpatient.Prepay prepay = this.IRegister.GetPrepay();
            ...
            if (this.ISave.SaveCommitting(Neusoft.SOC.HISFC.BizProcess.CommonInterface.Common.EnumSaveType.Insert, this.patientInfo) == -1)
            {
                Neusoft.FrameWork.Management.PublicTrans.RollBack();
            ...
            if (this.ISave.Saved(Neusoft.SOC.HISFC.BizProcess.CommonInterface.Common.EnumSaveType.Insert, this.patientInfo) == -1)
            {
                this.myMessageBox(this.ISave.Err, MessageBoxIcon.Warning);
            }
        ...
        /// <param name="patientInfo"></param>
        /// <returns></returns>
        private int IRegisterListInterface_OnSelectPatientInfo(Neusoft.HISFC.Models.RADT.PatientInfo pInfo)
        {
            this.clear();
        ...
        public int ReadCard()
        {
            ...
            clear();
         Neusoft.HISFC.Models.RADT.PatientInfo SiPatient = new PatientInfo();
         SiPatient.Pact.ID = defaultPactCode;
            Base.Inpatient.frmSiRegisterInfo frmSiRegisterInfo = new Neusoft.SOC.Local.RADT.GuangZhou.GYSY.Base.Inpatient.frmSiRegisterInfo();
            ...
            string errText = "";
            if (Function.GetRegInfoInpatient(SiPatient, ref errText) < 0)
            {
        ...
        public int SetSIPatientInfo(Neusoft.HISFC.Models.RADT.PatientInfo SiPatient)
        {
