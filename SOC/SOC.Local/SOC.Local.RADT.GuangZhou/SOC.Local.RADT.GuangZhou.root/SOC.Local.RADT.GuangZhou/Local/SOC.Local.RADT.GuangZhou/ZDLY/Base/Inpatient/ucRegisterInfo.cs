...
namespace Neusoft.SOC.Local.RADT.GuangZhou.ZDLY.Base.Inpatient
{
    ...
    public partial class ucRegisterInfo : Neusoft.FrameWork.WinForms.Controls.ucBaseControl, Neusoft.SOC.HISFC.RADT.Interface.Register.IInpatient
    {
        ...
        {
            //性别列表
            ArrayList talSex = Neusoft.HISFC.Models.Base.SexEnumService.List();
            ...
            foreach (Neusoft.HISFC.Models.Base.Spell s in commonController.QueryDepartment(true))
            {
            ...
            foreach (Neusoft.HISFC.Models.Base.Spell s in alDist)
            {
                Neusoft.HISFC.Models.Base.Spell sClone = s.Clone();
        ...
        private PatientInfo getPatientInfomation(PatientInfo patient)
        {
            ...
            patient.AddressHome = this.txtAddressNow.Text;//单位地址
        ...
        private bool isInputValid()
        {
            ...
            try
            {
                ...
                {
                    long inpatientNO = Neusoft.FrameWork.Public.String.GetNumber(this.txtInpatientNO.Text);
            ...
            //判断字符超长工作单位
            if (!Neusoft.FrameWork.Public.String.ValidMaxLengh(this.txtWorkAddress.Text, 100))
            {
        ...
        private string getAge(int year, int month, int day)
        {
            if (year == 0 && month == 0)
            {
                DateTime dtBirthBegin = Neusoft.FrameWork.Function.NConvert.ToDateTime(this.dtpBirthDay.Text);
                DateTime dtBirthEnd = commonController.GetSystemTime();
                if (dtBirthBegin <= new DateTime(1700, 1, 1))
                {
                    dtBirthBegin = dtBirthEnd;
                }
        ...
        private int processIDENNO(string idNO, EnumCheckIDNOType enumType)
        {
            ...
            foreach (Neusoft.HISFC.Models.Base.Const tcom in list)
            {
                if (tempstr == tcom.ID)
                {
