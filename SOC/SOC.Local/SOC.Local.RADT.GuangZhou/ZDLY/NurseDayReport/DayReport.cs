...
namespace FS.SOC.Local.RADT.GuangZhou.ZDLY.NurseDayReport
{
    /// <summary>
    /// 中六本地化床位日报表管理
    /// </summary>
...
类修改：1>FS.HISFC.Models.Base.EnumShiftType中添加  包床（ABD）, 解除包床（RBD）（行643）
2>FS.HISFC.Models.RADT.InPatientBedTransReord 新增床位记录实体
3>FS.HISFC.BizLogic.RADT.InpatientDayReport 新增三个函数
ArriveBed(FS.HISFC.Models.RADT.PatientInfo patientInfo, FS.HISFC.Models.Base.EnumShiftType shiftType) 
接诊用法：入院登记，护士站接诊，无费退院、出院登记、出院召回用，传入操作时的病人信息实体即可，类型为对应枚举，如入院登记 FS.HISFC.Models.Base.EnumShiftType.B，每个操作对应各自的枚举，不能相同。
TransBed(FS.HISFC.Models.RADT.PatientInfo patientInfoOld,FS.HISFC.Models.RADT.PatientInfo patientInfoNew) 
转科用方法，转科前病人信息实体，转科后病人信息实体
AddExtentBed(FS.HISFC.Models.RADT.PatientInfo patientInfo, FS.HISFC.Models.Base.Bed bedInfo,bool isAdd)   
