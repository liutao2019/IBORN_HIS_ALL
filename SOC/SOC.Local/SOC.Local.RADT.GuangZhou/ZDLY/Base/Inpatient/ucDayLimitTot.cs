...
namespace FS.SOC.Local.RADT.GuangZhou.ZDLY.Base.Inpatient
{
    public partial class ucDayLimitTot : UserControl
    {
        ...
        {
            this.txtCost.Text = FS.FrameWork.Public.String.FormatNumber(oEPatientInfo.FT.DayLimitTotCost, 2).ToString();//日限额累计
        ...
        private int updateLimCost()
        {
            ...
            dCost = FS.FrameWork.Function.NConvert.ToDecimal(this.txtCost.Text.ToString());
            ...
            //开始一个事物
            //FS.neuFC.Management.Transaction t = new FS.neuFC.Management.Transaction(this.inPatient.Connection);
            //t.BeginTransaction();
            FS.FrameWork.Management.PublicTrans.BeginTransaction();
            this.inPatient.SetTrans(FS.FrameWork.Management.PublicTrans.Trans);
            this.patientInfo.SetTrans(FS.FrameWork.Management.PublicTrans.Trans);
            //更新日限额方法
            if (this.inPatient.UpdateLimitTot(this.oEPatientInfo.ID, dCost) < 0)
            {
                FS.FrameWork.Management.PublicTrans.RollBack();
                FS.FrameWork.WinForms.Classes.Function.Msg("修改日限额累计出错!" + this.inPatient.Err, 211);
                return -1;
            }
            //插入变更日志
            if (this.patientInfo.SetShiftData(this.txtInpatientNo.InpatientNo, FS.HISFC.Models.Base.EnumShiftType.BT, "日限额累计", oldObj, newObj,oEPatientInfo.IsBaby) != 1)
            {
                FS.FrameWork.Management.PublicTrans.RollBack();
                FS.FrameWork.WinForms.Classes.Function.Msg("插入变更出错!" + this.patientInfo.Err, 211);
                return -1;
            }
        ...
        private bool validCost()
        {
            ...
            {
                FS.FrameWork.WinForms.Classes.Function.Msg("请输入患者!", 111);
                this.txtInpatientNo.Focus();
                return false;
            }
            ...
            {//判断输入是否数值
                if (FS.FrameWork.Function.NConvert.ToDecimal(this.txtCost.Text) < 0)
                {
            ...
            //判断修改前后金额是否一致
            if (this.oEPatientInfo.FT.DayLimitTotCost == FS.FrameWork.Function.NConvert.ToDecimal(this.txtCost.Text))
            {
                FS.FrameWork.WinForms.Classes.Function.Msg("修改前累计金额等于修改后累计金额,请重新输入", 111);
                return false;
            }
            return true;
        }
