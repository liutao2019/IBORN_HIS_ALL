...
namespace FS.SOC.Local.RADT.GuangZhou.ZDLY.Base.Inpatient
{
    ...
    /// 住院登记患者列表（默认）
    /// </summary>
    public partial class ucPatientList : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.SOC.HISFC.RADT.Interface.Register.IPatientList
    {
        ...
        private int getImageKey(DateTime now, FS.HISFC.Models.RADT.PatientInfo patientInfo)
        {
            if (patientInfo == null)
            {
                return 0;
            ...
            if (patientInfo.Sex.ID.ToString() == FS.HISFC.Models.Base.EnumSex.M.ToString())
            {
                return imageKey;
            }
            else if (patientInfo.Sex.ID.ToString() == FS.HISFC.Models.Base.EnumSex.F.ToString())
            {
                return imageKey + 1;
            }
        ...
        private void addGroupItem(FS.HISFC.Models.RADT.PatientInfo patientInfo, DateTime dtNow)
        {
        ...
        private void refreshGroupItem()
        {
            ...
            foreach (FS.HISFC.Models.RADT.PatientInfo item in list)
            {
        ...
        {
            if (inState == FS.HISFC.Models.Base.EnumInState.R.ToString())
            {
                return Color.Blue;
            }
            else if (inState == FS.HISFC.Models.Base.EnumInState.N.ToString())
            {
                return Color.Red;
            }
            else
        ...
        private void lvPatientList_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            if (item != null && item.Tag != null && item.Tag is FS.HISFC.Models.RADT.PatientInfo)
            {
                if (OnSelectPatientInfo != null)
                {
                    OnSelectPatientInfo(item.Tag as FS.HISFC.Models.RADT.PatientInfo);
                }
            }
        }
        ...
        private void lvPatientList_Select(object sender, EventArgs e)
        {
            if (this.lvPatientList.SelectedItems != null && this.lvPatientList.SelectedItems.Count > 0)
            {
                if (OnSelectPatientInfo != null)
                {
                    OnSelectPatientInfo(this.lvPatientList.SelectedItems[0].Tag as FS.HISFC.Models.RADT.PatientInfo);
                }
            }
        ...
        public int LoadPatient(System.Collections.ArrayList al)
        {
            ...
            {
                if (Function.IsContainYKDept().Equals(patientInfo.PID.PatientNO.Contains("V")))
                {
                    alDept.Add(patientInfo);
                }
            }
            ...
            foreach (FS.HISFC.Models.RADT.PatientInfo patientInfo in alDept)
            {
                this.addGroupItem(patientInfo, dtNow);
            }

            this.lvPatientList.Tag = alDept;
        ...
        public event FS.SOC.HISFC.RADT.Interface.Register.SelectPatientInfo OnSelectPatientInfo;
