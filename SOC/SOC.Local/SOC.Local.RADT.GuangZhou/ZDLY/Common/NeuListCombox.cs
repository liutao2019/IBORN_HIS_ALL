...
using Neusoft.SOC.Public.Ini;
using System.Xml.Linq;

namespace Neusoft.SOC.Local.RADT.GuangZhou.GYSY.Common
{
    public partial class NeuCombox : Neusoft.FrameWork.WinForms.Controls.NeuComboBox, Neusoft.SOC.HISFC.RADT.Interface.Common.IInputControl
    {
        #region  私有变量       
        ...
        public void ReadConfig(System.Xml.Linq.XElement doc)
        {
            if (doc.Element(this.Name) != null)
            {
                ...
                this.IsTagInput = Neusoft.FrameWork.Function.NConvert.ToBoolean(doc.Element(this.Name).Attribute("IsTagInput").Value);
                this.IsTextInput = Neusoft.FrameWork.Function.NConvert.ToBoolean(doc.Element(this.Name).Attribute("IsTextInput").Value);
                this.IsDefaultCHInput = Neusoft.FrameWork.Function.NConvert.ToBoolean(doc.Element(this.Name).Attribute("IsDefaultCHInput").Value);
                this.Enabled = Neusoft.FrameWork.Function.NConvert.ToBoolean(doc.Element(this.Name).Attribute("Enabled").Value);
                this.TabIndex = Neusoft.FrameWork.Function.NConvert.ToInt32(doc.Element(this.Name).Attribute("TabIndex").Value); ;
            }
