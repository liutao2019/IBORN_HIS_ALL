...
namespace FS.SOC.Local.RADT.GuangZhou.GYZL.Base.Inpatient
{
    ...
    public partial class ucRegisterInfo : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.SOC.HISFC.RADT.Interface.Register.IInpatient
    {
        ...
        private void initCombo()
        {
            ...
            ArrayList alDist = commonController.QueryConstant("ZIP");
        ...
        private bool isInputValid()
        {
            ...
            //判断联系人电话
            if (this.txtLinkPhone.Text==string.Empty)
            {
                this.myMessageBox("请录入联系人电话！", MessageBoxIcon.Warning);
        ...
        private string getAge(int year, int month, int day)
        {
            if (year == 0 && month == 0)
            {
                DateTime dtBirthBegin = FS.FrameWork.Function.NConvert.ToDateTime(this.dtpBirthDay.Text);
                DateTime dtBirthEnd = commonController.GetSystemTime();
                TimeSpan ts = dtBirthEnd - dtBirthBegin;
                int days = ts.Days;
                int hours = ts.Hours;
                int minutes = ts.Minutes;
                return string.Format("{0}天{1}时{2}分", days.ToString().PadLeft(2, '_'), hours.ToString().PadLeft(2, '_'), minutes.ToString().PadLeft(2, '_'));
