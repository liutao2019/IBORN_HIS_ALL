...
namespace SOC.Local.Assign.ZhuHai.ZDWY.ITriaggedPatient
{
    public partial class ucTriaggedPatient : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.SOC.HISFC.Assign.Interface.Components.ITriaggedPatient
    {
        ...
        private void addNode(FS.SOC.HISFC.Assign.Models.Assign assign)
        {
            ...
            ArrayList regList = regIntegrate.QueryPatient(assign.Register.ID);
            ...
            if (assign.TriageStatus == FS.HISFC.Models.Nurse.EnuTriageStatus.Delay)
            {
                ...
                {                   
                    if (regList.Count > 0&&register.DoctorInfo.Templet.Begin!=register.DoctorInfo.Templet.Begin.Date)
                    {
                        node.Text = str + "[" + assign.SeeNO.ToString() + "号]" + assign.Register.Name
                            + "(" + register.DoctorInfo.Templet.Begin.ToString("HH:mm") + "-" + register.DoctorInfo.Templet.End.ToString("HH:mm") + ")"
                            + "--队列：" + assign.Queue.Name + strHot;
                    }
                    else
                    {
                        node.Text = str + "[" + assign.SeeNO.ToString() + "号]" + assign.Register.Name + "--队列：" + assign.Queue.Name + strHot;
                    }
                }
                //else
                //{
            ...
            else if (assign.TriageStatus == FS.HISFC.Models.Nurse.EnuTriageStatus.Arrive || assign.TriageStatus == FS.HISFC.Models.Nurse.EnuTriageStatus.Out)
            {
                //if (assign.Register.RegType == FS.HISFC.Models.Base.EnumRegType.Pre)
                {
                    if (regList.Count > 0 && register.DoctorInfo.Templet.Begin != register.DoctorInfo.Templet.Begin.Date)
                    {
                        if (assign.Queue.QueueType == FS.SOC.HISFC.Assign.Models.EnumQueueType.RegLevel || assign.Queue.QueueType == FS.SOC.HISFC.Assign.Models.EnumQueueType.Custom)
                        {
                            FS.HISFC.Models.Base.Employee emp = FS.SOC.HISFC.BizProcess.Cache.Common.GetEmployeeInfo(assign.Queue.Doctor.ID);
                            if (emp != null)
                            {
                                node.Text = str + "[" + assign.SeeNO.ToString() + "号]" + assign.Register.Name + "(" + emp.Name + " " + assign.Queue.SRoom.Name + ")" + strHot;
                            }
                            else
                            {
                                node.Text = str + "[" + assign.SeeNO.ToString() + "号]" + assign.Register.Name + "(" + "无" + " " + assign.Queue.SRoom.Name + ")" + strHot;
                            }
                        }
                        else
                        {
                            node.Text = str + "[" + assign.SeeNO.ToString() + "号]" + assign.Register.Name + "(" + register.DoctorInfo.Templet.Begin.ToString("HH:mm")
                                + "-" + register.DoctorInfo.Templet.End.ToString("HH:mm") + ")" + strHot;
                        }
                    }
                    else
                    {
                        node.Text = str + "[" + assign.SeeNO.ToString() + "号]" + assign.Register.Name + strHot;
                    }
                }
                //else
                //{
