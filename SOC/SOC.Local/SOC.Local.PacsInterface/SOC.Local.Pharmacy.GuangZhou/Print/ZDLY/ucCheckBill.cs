...
namespace Neusoft.SOC.Local.Pharmacy.Print.ZDLY
{
    ...
    public partial class ucCheckBill : ucBaseControl, Base.IPharmacyBillPrint
    {
        ...
        public int SetPrintData(ArrayList alPrintData, Base.PrintBill printBill)
        {
            ...
            {
                Neusoft.HISFC.Models.Pharmacy.Check check = alPrintData[0] as Neusoft.HISFC.Models.Pharmacy.Check;
                fOperDate = check.FOper.OperTime;

                Neusoft.SOC.HISFC.BizLogic.Pharmacy.Check itemMgr = new Neusoft.SOC.HISFC.BizLogic.Pharmacy.Check();
                Neusoft.HISFC.BizProcess.Integrate.Manager manager = new Neusoft.HISFC.BizProcess.Integrate.Manager();
                ArrayList alCheckDetail = itemMgr.QueryCheckDetailByCheckCode(check.StockDept.ID, check.CheckNO);

                foreach (Neusoft.HISFC.Models.Pharmacy.Check checkItem in alCheckDetail)
                {
                    if (checkItem.AdjustQty > 0 || checkItem.FStoreQty > 0)
                    {
                        ...
                        totPurchaseCost += Neusoft.FrameWork.Public.String.FormatNumber(checkItem.AdjustQty * checkItem.Item.PriceCollection.PurchasePrice / checkItem.Item.PackQty, 2);
                        totRetailCost += Neusoft.FrameWork.Public.String.FormatNumber(checkItem.AdjustQty * checkItem.Item.PriceCollection.RetailPrice / checkItem.Item.PackQty, 2);
                    }
                }
        ...
        private void SetPrintData(ArrayList al, int inow, int icount, string title)
        {
            ...
            Neusoft.HISFC.Models.Pharmacy.Check info = (Neusoft.HISFC.Models.Pharmacy.Check)al[0];

            #region label赋值
            if (string.IsNullOrEmpty(title))
            {
                this.lblTitle.Text = string.Format(this.lblTitle.Text, Neusoft.SOC.HISFC.BizProcess.Cache.Common.GetDeptName(info.StockDept.ID), drugType);
            }
            else
            {
            ...
            this.lblOper.Text = "制表人:" + Neusoft.SOC.HISFC.BizProcess.Cache.Common.GetEmployeeName(Neusoft.FrameWork.Management.Connection.Operator.ID);
            ...
            for (int i = 0; i < al.Count; i++)
            {
                ...
                Neusoft.HISFC.Models.Pharmacy.Check check = al[i] as Neusoft.HISFC.Models.Pharmacy.Check;
                Neusoft.HISFC.Models.Base.Department departMent = dept.GetDeptmentById(info.StockDept.ID);
                this.neuFpEnter1_Sheet1.Cells[i, 0].Text = Neusoft.SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(check.Item.ID).UserCode; //药品自定义码
                ...
                if (departMent.DeptType.ID.ToString() == "PI")
                {
                    ...
                    this.neuFpEnter1_Sheet1.Cells[i, 6].Value = FrameWork.Public.String.FormatNumber(check.FStoreQty / check.Item.PackQty, 2);
                    this.neuFpEnter1_Sheet1.Cells[i, 7].Value = FrameWork.Public.String.FormatNumber(check.AdjustQty / check.Item.PackQty, 2);
                }
                else
                {
                    this.neuFpEnter1_Sheet1.Cells[i, 3].Text = check.Item.MinUnit;
                    this.neuFpEnter1_Sheet1.Cells[i, 6].Value = FrameWork.Public.String.FormatNumber(check.FStoreQty, 2);
                    this.neuFpEnter1_Sheet1.Cells[i, 7].Value = FrameWork.Public.String.FormatNumber(check.AdjustQty, 2);
                }
