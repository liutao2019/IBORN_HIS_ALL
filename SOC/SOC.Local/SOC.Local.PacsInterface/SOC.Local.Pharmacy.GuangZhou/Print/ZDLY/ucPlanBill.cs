...
namespace Neusoft.SOC.Local.Pharmacy.Print.ZDLY
{
    ...
    public partial class ucPlanBill : ucBaseControl, Base.IPharmacyBillPrint
    {
        ...
        private void SetPrintData(ArrayList al, int inow, int icount, string title)
        {
            ...
            Neusoft.HISFC.Models.Pharmacy.InPlan plan = (Neusoft.HISFC.Models.Pharmacy.InPlan)al[0];

            #region label赋值
            if (string.IsNullOrEmpty(title))
            {
                this.lblTitle.Text = string.Format(this.lblTitle.Text, Neusoft.SOC.HISFC.BizProcess.Cache.Common.GetDeptName(plan.Dept.ID), drugType);
            }
            else
            {
                if (title.IndexOf("[库存科室]") != -1)
                {
                    title = title.Replace("[库存科室]", Neusoft.SOC.HISFC.BizProcess.Cache.Common.GetDeptName(plan.Dept.ID));
                }
            ...
            for (int i = 0; i < al.Count; i++)
            {
                ...
                Neusoft.HISFC.Models.Base.Department departMent = dept.GetDeptmentById(info.Dept.ID);
                this.neuFpEnter1_Sheet1.Cells[i, 0].Text = Neusoft.SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(info.Item.ID).UserCode; //药品自定义码

                Neusoft.HISFC.Models.Pharmacy.Item itemObj = Neusoft.SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(info.Item.ID);
        ...
        public int SetPrintData(ArrayList alPrintData, Neusoft.SOC.Local.Pharmacy.Base.PrintBill printBill)
        {
           Base.PrintBill.SortByCustomerCode(ref this.alPrintData);
           string billNo = (alPrintData[0] as Neusoft.HISFC.Models.Pharmacy.InPlan).BillNO;
           string deptNo = (alPrintData[0] as Neusoft.HISFC.Models.Pharmacy.InPlan).Dept.ID;
           SOC.HISFC.BizLogic.Pharmacy.Plan plan = new Neusoft.SOC.HISFC.BizLogic.Pharmacy.Plan();
           List<Neusoft.HISFC.Models.Pharmacy.InPlan> al = plan.MergeInPlan(deptNo, billNo);
           for (int i = 0; i < al.Count; i++)
           {
