...
namespace Neusoft.SOC.Local.Pharmacy.Print.ZDLY
{
    ...
    public partial class ucPhaOutputBill : ucBaseControl, Base.IPharmacyBillPrint
    {
        ...
        private void SetPrintData(ArrayList al, int inow, int icount, string title)
        {
            ...
            this.lblCompany.Text = "领用部门: (" + info.TargetDept.ID + ")" + Neusoft.SOC.HISFC.BizProcess.Cache.Common.GetDeptName(info.TargetDept.ID);
            this.lblBillID.Text = "单据号: " + info.OutListNO;
            this.lblInputDate.Text = "领用日期: " + info.OutDate.ToShortDateString();
            this.lblOper.Text = "制单:" + Neusoft.SOC.HISFC.BizProcess.Cache.Common.GetEmployeeName(info.Operation.ExamOper.ID);

            lblStorageDept.Text = "发出仓库: (" + info.StockDept.ID + ")" +Neusoft.SOC.HISFC.BizProcess.Cache.Common.GetDeptName(info.StockDept.ID);
            lblUser.Text = "领用: " +Neusoft.SOC.HISFC.BizProcess.Cache.Common.GetEmployeeName(info.GetPerson);
            ...
            for (int i = 0; i < al.Count; i++)
            {
                ...
                this.neuFpEnter1_Sheet1.Cells[i, 0].Text = Neusoft.SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(output.Item.ID).UserCode; //药品自定义码
