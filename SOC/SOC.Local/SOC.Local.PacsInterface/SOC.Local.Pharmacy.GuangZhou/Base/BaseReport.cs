...
namespace Neusoft.SOC.Local.Pharmacy.Base
{
    ...
    ///  />
    /// </summary>
    public partial class BaseReport : Neusoft.FrameWork.WinForms.Controls.ucBaseControl
    {
        public BaseReport()
        {
            InitializeComponent();
        ...
        public string DetailSettingFilePatch
        {
            get
            {
                if (this.detailSettingFilePatch == string.Empty)
                {
                    try
                    {
                        return Neusoft.FrameWork.WinForms.Classes.Function.CurrentPath + @"\Profile\" + this.FindForm().Text + "detail.xml";
                    }
                    catch
                    {
                        return ""; 
        ...
        public string SettingFilePatch
        {
            get 
            {
                if (this.settingFilePatch == string.Empty)
                {
                    try
                    {
                        return Neusoft.FrameWork.WinForms.Classes.Function.CurrentPath + @"\Profile\" + this.FindForm().Text + "tot.xml";
                    }
                    catch
                    {
                        return "";
        ...
        private int initDeptByPriPower()
        {
            ...
            if (alDept.Count == 1)
            {
                ...
                this.cmbDept.Tag = ((Neusoft.FrameWork.Models.NeuObject)alDept[0]).ID;
        ...
        private int initCustomType()
        {
            ...
            {
                Neusoft.HISFC.Models.Base.Spell customType = new Neusoft.HISFC.Models.Base.Spell();
                try
                {
                ...
                if (this.avgType == "Tot" || this.avgType == "Both")
                {
                    ...
                    foreach (string colLetter in cols)
                    {
                        try
                        {
                            ...
                            int colIndex = Neusoft.FrameWork.Function.NConvert.ToInt32(colLetter.Split('|')[0]);
                            if (colIndex > this.fpSpread1_Sheet1.ColumnCount - 1 || this.fpSpread1_Sheet1.RowCount == 1)
                            {
                                continue;
                            }
                            ...
                            {
                                decimalPlaces = Neusoft.FrameWork.Function.NConvert.ToInt32(colLetter.Split('|')[1]);
                            }
                    ...
                    foreach (string colLetter in cols)
                    {
                        try
                        {
                            ...
                            if (!string.IsNullOrEmpty(formula))
                            {
                                ...
                                {
                                    decimalPlaces = Neusoft.FrameWork.Function.NConvert.ToInt32(colLetter.Split('|')[2]);
                                }
                                ...
                                if (colLetter.Split('|').Length > 4)
                                {
                                    try
                                    {
                                        ...
                                        if (t.Split('.').Length > 1)
                                        {
                                            this.fpSpread1_Sheet1.Cells[this.fpSpread1_Sheet1.RowCount - 1, Neusoft.FrameWork.Function.NConvert.ToInt32(t.Split('.')[0])].Value = (t.Split('|')[1]).ToString();
                                        }
                                        else
                                        {
        ...
        {
            toolBarService.AddToolButton("下一天", "查询下一天数据", (int)Neusoft.FrameWork.WinForms.Classes.EnumImageList.X下一个, true, false, null);
        ...
        {
            //Neusoft.SOC.Windows.Forms.Print p = new Neusoft.SOC.Windows.Forms.Print();
