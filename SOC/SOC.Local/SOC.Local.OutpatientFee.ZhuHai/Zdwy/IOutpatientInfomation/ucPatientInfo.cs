...
namespace FS.SOC.Local.OutpatientFee.ZhuHai.Zdwy.IOutpatientInfomation
{
    public partial class ucPatientInfo : UserControl, FS.HISFC.BizProcess.Integrate.FeeInterface.IOutpatientInfomation
    {
        ...
        /// 是否按照超出项目次数分收费序列
        /// </summary>
        private bool isExceededItem = false;
        #endregion
        ...
        public FS.HISFC.Models.Registration.Register PatientInfo
        {
            ...
            set
            {
                ...
                if (patientInfo != null)
                {
                    ...
                    this.chkIsFirst.Checked = patientInfo.IsFirst;
                    ...
                    txtRemark.Text = patientInfo.Memo;////{187A73EB-008A-4A25-A6CB-28CAE0E629A7}添加备注信息
        ...
        private void setAccountInfo()
        {
            ...
            if (this.levelDiscount.Equals(1))
            {
                ...
                accountInfo = string.Format(accountInfo, this.levelName);
            }
            else
            {
                accountInfo = string.Format(accountInfo, this.levelName, (this.levelDiscount * 10).ToString().Trim());
            }
        ...
        {
            //{437D8967-CA6A-4024-A810-8BFCDB4F9B13}
            try
            {
                string resultCode = "0";
                ...
                this.levelID = this.patientInfo.WBCode;
                this.levelName = this.patientInfo.SpellCode;
                this.levelDiscount = decimal.Parse(this.patientInfo.UserCode);
                //if (FS.HISFC.BizProcess.Integrate.WSHelper.QueryAccountDiscount(this.PatientInfo.PID.CardNO, out levelDiscount, out levelID, out levelName, out resultCode, out errMsg) < 0)
                //{
        ...
        public void Clear()
        {
            ...
            this.txtRemark.Text = string.Empty;//{187A73EB-008A-4A25-A6CB-28CAE0E629A7}添加备注信息
            this.tbCardNO.Focus();
            this.lbAccountInfo.Text = "";
        }
        ...
        private int InitControlParams() 
        {
            ...
            //是否显示“超”
            this.isExceededItem = this.controlParamIntegrate.GetControlParam<bool>("MZ0203", false, false);
        ...
        public void DealModifyDetails()
            ...
            foreach (ArrayList temp in sortList)
            {
                ...
                for (int i = 0; i < this.fpRecipeSeq_Sheet1.RowCount; i++)
                {
                    if (this.fpRecipeSeq_Sheet1.Cells[i, 3].Tag == null)
                        continue;
        ...
        public void GetRegInfo()
        {
            ...
            this.patientInfo.Memo = this.txtRemark.Text;//{187A73EB-008A-4A25-A6CB-28CAE0E629A7}添加备注信息
            //记账单号
        ...
        public void SetRegInfo()
        {
            ...
            this.txtRemark.Text = this.patientInfo.Memo;//{187A73EB-008A-4A25-A6CB-28CAE0E629A7}添加备注信息
        ...
        private void SetChargeInfo()
        {
            ...
            for (int i = 0; i < sortList.Count; i++)
            {
                ...
                string dep = this.fpRecipeSeq_Sheet1.Cells[i, 1].Text;
                if (tempFeeItemRowOne.IsExceeded && isExceededItem)
                {
                    this.fpRecipeSeq_Sheet1.Cells[i, 1].Text = dep + "【医保】";
                }
        ...
        private void tbCardNO_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)// {D55B4DFA-DA91-42b0-8163-27036100E89E}
            {
                ...
                else //正常输入患者门诊卡号情况
                {
                    ...
                    {
                        //9开头的门诊临时号。可以调出未收费的明细 gumzh
                        if (cardNO.Length != 10 || !cardNO.StartsWith(this.noRegFlagChar))
                        {
            ...
            //通过姓名查询当天的有效挂号记录 gumzh
            if (e.KeyCode == Keys.F12)
            {
                string pName = this.tbCardNO.Text.Trim();
                if (!string.IsNullOrEmpty(pName))
                {
                    frmQueryRegByName frmName = new frmQueryRegByName();
                    frmName.PatientName = pName;
                    if (frmName.Init() == 1)
                    {
                        if (frmName.ShowDialog() == DialogResult.OK)
                        {
                            string cNO = frmName.CardNo;
                            this.tbCardNO.Text = cNO;
                            KeyEventArgs eKey = new KeyEventArgs(Keys.Enter);
                            this.tbCardNO_KeyDown(null, eKey);
                        }
                    }
                    else
                    {
                        MessageBox.Show(frmName.ErrInfo);
                        return;
                    }
        ...
        private void tbName_KeyDown(object sender, KeyEventArgs e)// {D55B4DFA-DA91-42b0-8163-27036100E89E}
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (frmQuery.DialogResult == DialogResult.OK)
                {
                    ...
                    this.tbCardNO.Text = frmQuery.PatientInfo.PID.CardNO;
                    tbCardNO_KeyDown(new object(), new KeyEventArgs(Keys.Enter));
                }
