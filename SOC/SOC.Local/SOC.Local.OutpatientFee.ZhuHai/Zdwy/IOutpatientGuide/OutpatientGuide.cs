...
namespace FS.SOC.Local.OutpatientFee.ZhuHai.Zdwy.IOutpatientGuide
{
    public class OutpatientGuide : FS.HISFC.BizProcess.Interface.Fee.IOutpatientGuide
    {
        ...
        public void SetValue(FS.HISFC.Models.Registration.Register rInfo, System.Collections.ArrayList invoices, System.Collections.ArrayList feeDetails)
        {
            ...
            else if (rInfo.PrintInvoiceCnt == 4)//医保患者自费清单
            {
                if (rInfo.Pact.PayKind.ID == "02")
                {
                    IBorn.GZSI.ucSIFeeDetail ucSIFeeDetail = new FS.SOC.Local.OutpatientFee.ZhuHai.IBorn.GZSI.ucSIFeeDetail();
                    ucSIFeeDetail.SetValue(rInfo, invoices, null);
            ...
            else//收费时全部都提示
            {
                ...
                {
                    IBorn.GZSI.ucSIFeeDetail ucSIFeeDetail = new FS.SOC.Local.OutpatientFee.ZhuHai.IBorn.GZSI.ucSIFeeDetail();
                    ucSIFeeDetail.SetValue(rInfo, invoices, feeDetails);
                    ucSIFeeDetail.PrintPage();
                }
                #endregion
