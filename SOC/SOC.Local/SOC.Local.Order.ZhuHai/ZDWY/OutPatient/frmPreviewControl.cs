...
namespace FS.SOC.Local.Order.ZhuHai.ZDWY.OutPatient
{
    public partial class frmPreviewControl : Form
    {
        ...
        public frmPreviewControl(Dictionary<Int32, List<Control>> previewDictionary)
        {
            ...
            foreach (Int32 keysType in previewDictionary.Keys)
            {
                ...
                for (int i = 0; i < previewDictionary[keysType].Count; i++)
                {
                    ...
                    neuObject.Name = FS.FrameWork.Public.EnumHelper.Current.GetEnumName((EnumOutPatientBill)keysType) + iCount;
        ...
        {
            this.tbAllPage.Image = FS.FrameWork.WinForms.Classes.Function.GetImage(FS.FrameWork.WinForms.Classes.EnumImageList.D打印);
            this.tbCurrenPage.Image = FS.FrameWork.WinForms.Classes.Function.GetImage(FS.FrameWork.WinForms.Classes.EnumImageList.D打印清单);
            this.tbFrontPage.Image = FS.FrameWork.WinForms.Classes.Function.GetImage(FS.FrameWork.WinForms.Classes.EnumImageList.S上一个);
            this.tbNextPage.Image = FS.FrameWork.WinForms.Classes.Function.GetImage(FS.FrameWork.WinForms.Classes.EnumImageList.X下一个);
        ...
        private void InitControl()
        {
            ...
            foreach (FS.FrameWork.Models.NeuObject neuObject in previewList)
            {
        ...
        private void ShowPrintDialog(Control uc)
        {
            ...
            FS.SOC.HISFC.BizProcess.OrderInterface.OutPatientOrder.IOutPatientOrderPrint IOutPatientOrderPrint = uc as FS.SOC.HISFC.BizProcess.OrderInterface.OutPatientOrder.IOutPatientOrderPrint;
            ...
            print.SetPageSize(new FS.HISFC.Models.Base.PageSize("liuww", uc.Width, uc.Height));

            print.SetPrintDocument(5, 5, uc);
        ...
        private void PrintPage(Control printControl)// {F9B890A9-D02C-4e38-BB39-F64251AF8F64}
        {
            if (printControl.ToString() == "FS.SOC.Local.Order.ZhuHai.ZDWY.OutPatient.RegistionExtend.ucMedicalReportIBORN"
                || printControl.ToString() == "FS.SOC.Local.Order.ZhuHai.ZDWY.InPatient.PacsBillPrint.ucPacsBillPrintIBORNA4" || printControl.ToString() == "FS.SOC.Local.Order.ZhuHai.ZDWY.OutPatient.RegistionExtend.ucMedicalReportIBORNA5")
            {
                print.IsLandScape = false;
                print.SetPageSize(new FS.HISFC.Models.Base.PageSize("A4", printControl.Width + 5, printControl.Height + 5));
            ...
            else
            {
                ...
                if (printControl.Width > printControl.Height)
                {
                    ...
                    print.SetPageSize(new FS.HISFC.Models.Base.PageSize("A5", printControl.Height + 5, printControl.Width + 5));
                }
                else
                {
                    print.IsLandScape = false;
                    print.SetPageSize(new FS.HISFC.Models.Base.PageSize("A5", printControl.Width + 5, printControl.Height + 5));
                }
            }

            if (printControl.ToString() == "FS.SOC.Local.Order.ZhuHai.ZDWY.OutPatient.RecipePrint.IBORN.ucRecipePrintIBORN")
            {
                FS.SOC.Local.Order.ZhuHai.ZDWY.OutPatient.RecipePrint.IBORN.ucRecipePrintIBORN printRecipe = printControl as FS.SOC.Local.Order.ZhuHai.ZDWY.OutPatient.RecipePrint.IBORN.ucRecipePrintIBORN;
                if (printRecipe != null && printRecipe.IsSpeRecipe)
                {
                    MessageBox.Show("当前打印麻精处方，请注意换纸！！！");
                }
            }
            print.IsDataAutoExtend = false;
        ...
        private void clbPreview_ItemCheck(object sender, ItemCheckEventArgs e)
        {
            ...
            string controlType = (this.clbPreview.SelectedItem as FS.FrameWork.Models.NeuObject).ID;
            for (int i = 0; i < this.checkedListBox1.Items.Count; i++)
            {

                if ((this.checkedListBox1.Items[i] as FS.FrameWork.Models.NeuObject).ID.Equals(controlType))
                {
        ...
        {
            currenPage = Convert.ToInt32(((FS.FrameWork.Models.NeuObject)(((System.Windows.Forms.ListBox)(sender)).SelectedItem)).Memo) - 1;
            this.ShowPrintDialog(ControlList[currenPage]);
        }
    }
}
