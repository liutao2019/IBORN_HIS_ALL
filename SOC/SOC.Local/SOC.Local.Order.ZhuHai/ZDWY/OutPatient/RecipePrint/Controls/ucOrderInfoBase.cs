...
namespace FS.SOC.Local.Order.ZhuHai.ZDWY.OutPatient.RecipePrint.Controls
{
    public partial class ucOrderInfoBase : UserControl, RecipePrint.Interface.IOrderInfo
    {
        ...
        {
            return FS.FrameWork.Public.String.ToSimpleString(dec);
        }
        ...
        /// <returns></returns>
        private string GetName(FS.HISFC.Models.Order.OutPatient.Order outOrder)
        {
            ...
            {
                phaItem = SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(outOrder.Item.ID);
        ...
        public int SetOrderInfo(System.Collections.ArrayList alOrder)
        {
            ...
            //按照sortID排序
            alOrder.Sort(new FS.SOC.Local.Order.ZhuHai.ZDWY.Common.Comparer.OrderCompare());
            ...
            foreach (string key in dicCombOrder.Keys)
            {
                ...
                {
                    FS.HISFC.Models.Order.OutPatient.Order ord = alCombOrder[i] as FS.HISFC.Models.Order.OutPatient.Order;
                    ...
                    if (alCombOrder.Count == 1)
                    {
                        ...
                        //用量    外用药不显示每次量
                        if (SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(ord.Item.ID).SpecialFlag2 != "1")
                        {
                            buffer.Append(" ");
                        ...
                        buffer.Append(SOC.HISFC.BizProcess.Cache.Order.GetFrequencyName(ord.Frequency.ID));
                        buffer.Append(" ");
                    ...
                    else
                    {
                        ...
                        else if (i == alCombOrder.Count - 1)
                        {
                            ...
                            {
                                FS.HISFC.Models.Order.OutPatient.Order orderM = alCombOrder[m] as FS.HISFC.Models.Order.OutPatient.Order;
                                ...
                                if (m == 0)
                                {
                                    ...
                                    if (SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(ord.Item.ID).SpecialFlag2 != "1")
                                    {
                                        ...
                                        {
                                            FS.HISFC.Models.Order.OutPatient.Order orderJ = alCombOrder[j] as FS.HISFC.Models.Order.OutPatient.Order;
                                            FS.HISFC.Models.Pharmacy.Item item = orderJ.Item as FS.HISFC.Models.Pharmacy.Item;
                    ...
                    //if (FS.SOC.HISFC.BizProcess.Cache.Common.IsInjectUsage(ord.Usage.ID))
                    //{
                    //    buffer.Append("(余液弃去)");
                    //}
                    buffer.Append("\n");
