...
namespace FS.SOC.Local.Order.ZhuHai.ZDWY.OutPatient.SplitRecipe
{
    /// <summary>
    /// 默认分处方接口
    /// </summary>
    public class SplitRecipeClass : FS.HISFC.BizProcess.Interface.FeeInterface.ISplitRecipe
    {
        ...
        /// 药品性质转换类
        /// </summary>
        private static FS.FrameWork.Public.ObjectHelper hsDrugQuaulity = new FS.FrameWork.Public.ObjectHelper();
        ...
        /// <returns></returns>
        public bool SplitRecipe(FS.HISFC.Models.Registration.Register r, System.Collections.ArrayList feeItemList, ref string errText)
        {
        ...
        /// <param name="r"></param>
        /// <returns></returns>
        public bool SetNoteNo(ArrayList feeDetails, ref string errText, FS.HISFC.Models.Registration.Register r)
        {
            //存储已有的处方号和最大的处方流水号
            ...
            //是否忽略系统类别
            bool isDecSysClassWhenGetRecipeNO = controlParamIntegrate.GetControlParam<bool>(FS.HISFC.BizProcess.Integrate.Const.DEC_SYS_WHENGETRECIPE, false, false);
            ...
            foreach (ArrayList temp in sortList)
            {
                ...
                foreach (ArrayList tempCounts in counts)
                {
                    ...
                    string tempRecipeNo = "";
                    int recipeSeqNew = 0;
                    GetRecipeNoAndMaxSeq(tempCounts, ref tempRecipeNo, ref recipeSeqNew, r.ID);
                    recipeSeqNew = recipeSeqNew + 1;
                    if (tempRecipeNo != "" && recipeSeqNew > 0)
                    {
                        foreach (FeeItemList f in tempCounts)
                        {
                            feeDetails.Add(f);
            ...
            foreach (FeeItemList feeItem in feeDetails)
            {
                if (hsInvertDept.GetObjectFromID(feeItem.UndrugComb.ID) != null && feeItem.Item.ItemType == FS.HISFC.Models.Base.EnumItemType.Drug)
                {
        ...
        /// </summary>
        /// <returns></returns>
        private bool RecipeCountLimit(FS.HISFC.Models.Registration.Register reg, ArrayList itemList)
        {

            //取药房替换科室
            ArrayList alEmergencyDept = managerIntegrate.GetConstantList("EmergencyDept");
            bool isEmergency = false;
            foreach (FS.HISFC.Models.Base.Const dept in alEmergencyDept)
            {
                if (dept.ID == ((FS.HISFC.Models.Base.Employee)this.myOutPatient.Operator).Dept.ID)
                {
                    isEmergency = true;
                    break;
                }
