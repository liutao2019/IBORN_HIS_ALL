...
namespace FS.SOC.Local.Order.ZhuHai.ZDWY.Common.IMedicalResult.LIS
{
    public partial class frmLisResultShow : FS.FrameWork.WinForms.Forms.BaseForm
    {
        ...
        public int ShowResultByPatient()
        {
            ...
            {
                if (MessageBox.Show("是否使用测试数据？", "询问", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    patientType = "1";
                    patientName = "陈渭泉";
                    clinicNo = "";
        ...
        public int SetPatient(FS.HISFC.Models.RADT.Patient patient)
        {
            ...
            try
            {
                if (myPatient.GetType() == typeof(FS.HISFC.Models.Registration.Register))
                {
                    ...
                    patientNo = regObj.PID.CardNO;
                    this.patientType = "0";
                    this.clinicNo = regObj.ID;
                    this.deptCode = "";

                    this.lblDeptInfo.Text = "  科室：" + regObj.SeeDoct.Dept.Name;
                    this.lblPatientInfo.Text = "类型：门诊" + "  门诊号：" + myPatient.PID.PatientNO + "  姓名：" + myPatient.Name + "  性别：" + myPatient.Sex.Name + "  年龄：" + funMgr.GetAge(myPatient.Birthday, regObj.DoctorInfo.SeeDate);
                }
                else
                {
                    FS.HISFC.Models.RADT.PatientInfo patientInfo = (FS.HISFC.Models.RADT.PatientInfo)myPatient;
                    isOutPatient = false;
                    patientNo = patientInfo.PID.PatientNO;
                    this.patientType = "1";
                    ...
                    this.lblPatientInfo.Text = "类型：住院" + "  住院号：" + myPatient.PID.PatientNO + "  姓名：" + myPatient.Name + "  性别：" + myPatient.Sex.Name + "  年龄：" + funMgr.GetAge(myPatient.Birthday, patientInfo.PVisit.InTime);
                }
            }
            catch
            {
