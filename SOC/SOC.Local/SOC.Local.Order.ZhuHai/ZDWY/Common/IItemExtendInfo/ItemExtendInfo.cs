...
namespace FS.SOC.Local.Order.ZhuHai.ZDWY.Common.IItemExtendInfo
{
    /// <summary>
    /// 获取项目医保对照信息
    /// </summary>
    public class ItemExtendInfoAchieve : FS.HISFC.BizProcess.Interface.Common.IItemCompareInfo
    {
        ...
        {
            if (pactInfo == null || item == null || item.ID == "999" || string.IsNullOrEmpty(pactInfo.ID))// {FDC20D82-91D7-4741-8365-2CD057265B6E}
            {
                return 1;
            }
            ...
            else if (pactInfo.PayKind.ID == "02" && !isCivilworker)
            {
                ...
                //珠海医保都对照PACTCODE是14的，
                //中山医保在本地不知道是不是自费项目
                if (pact.PactDllName == "ZhuHaiSI.dll")
                {
                    string strPactCode = pactInfo.ID;
                    strPactCode = "14";

                    FS.HISFC.Models.SIInterface.Compare compareItem = this.GetCompareItemInfo(strPactCode, item.ID);
