...
namespace Neusoft.SOC.Local.Order.ZhuHai.ZDWY.InPatient.ExecBill
{
    ...
    public partial class ucExecBillFP : Neusoft.FrameWork.WinForms.Controls.ucBaseControl, Neusoft.HISFC.BizProcess.Interface.IPrintTransFusion
    {
        ...
        protected string sheetXMLFileName = Neusoft.FrameWork.WinForms.Classes.Function.CurrentPath +
            Neusoft.FrameWork.WinForms.Classes.Function.SettingPath + "ucExecBillFPItem.xml";
        ...
        public void Print()
        {
            try
            {
                ...
                if (!((Neusoft.HISFC.Models.Base.Employee)orderManager.Operator).IsManager)
                {
        ...
        private void AddLine()
        {
            ...
            Neusoft.HISFC.Components.Common.Classes.Function.DrawComboLeft(neuSpread1_Sheet1, (int)EnumCol.CombNo, (int)EnumCol.CombFlag);
            ...
            if (this.neuSpread1_Sheet1.Rows.Count > 0)
            {
                for (int index = 0; index < this.neuSpread1_Sheet1.RowCount; index++)
                {
                    ...
                    try
                    {
                        Neusoft.HISFC.Models.Order.ExecOrder execOrder = this.neuSpread1_Sheet1.Rows[index].Tag as Neusoft.HISFC.Models.Order.ExecOrder;
                        if (execOrder != null
        ...
        private void AddToFP(ArrayList list)
        {
            ...
            foreach (Neusoft.HISFC.Models.Order.ExecOrder execOrder in list)
            {
                ...
                if (!hsPatientInfo.Contains(execOrder.Order.Patient.ID))
                {
                    ...
                    Neusoft.HISFC.BizLogic.RADT.InPatient inMgr = new Neusoft.HISFC.BizLogic.RADT.InPatient();
                ...
                {
                    Neusoft.HISFC.Models.RADT.PatientInfo patientInfo = hsPatientInfo[execOrder.Order.Patient.ID] as Neusoft.HISFC.Models.RADT.PatientInfo;
                    execOrder.Order.Patient = patientInfo;
                }
        ...
        public void Query(List<Neusoft.HISFC.Models.RADT.PatientInfo> patients, string usagecode, DateTime dtBegin, DateTime dtEnd, bool isRePrint, string orderType, bool isFirst)
        {
            ...
            Dictionary<string, Neusoft.HISFC.Models.RADT.PatientInfo> hsPatientInfo = new Dictionary<string, Neusoft.HISFC.Models.RADT.PatientInfo>();
            ...
            foreach (Neusoft.HISFC.Models.Order.ExecOrder exeOrder in alOrder)
            {
                ...
                {
                    Neusoft.HISFC.Models.Order.Frequency freqObj = SOC.HISFC.BizProcess.Cache.Order.GetFrequency(exeOrder.Order.Frequency.ID);
                    ...
                    //{
                        ...
                        //{
                        //    DateTime dt = exeOrder.DateUse.Date.AddHours(Neusoft.FrameWork.Function.NConvert.ToInt32(times[i].Substring(0, times[i].IndexOf(':'))));
                        //    if (!hsMorder_useTime.ContainsKey(exeOrder.Order.ID))
                        //    {
            ...
            for (int k = 0; k < alTemp.Count; k++)
            {
                ...
                Neusoft.HISFC.Models.Order.ExecOrder execOrd = ((Neusoft.HISFC.Models.Order.ExecOrder)alTemp[k]);
                ...
                {
                    sMing = "";
                    Neusoft.HISFC.Models.Order.ExecOrder execOrder = ((Neusoft.HISFC.Models.Order.ExecOrder)alOrder[j]);
                ...
                if (!isHave)
                {
                    ...
                    if (execOrd.Order.OrderType.Type == Neusoft.HISFC.Models.Order.EnumType.LONG)//临时医嘱暂时不显示执行时间
                    {
                        if (execOrd.DateUse.Hour <= 12)
                        {
        ...
        private void ucExecBillFP_Load(object sender, EventArgs e)
        {
            ...
            if (this.neuSpread1_Sheet1 != null)
            {
                ...
                {
                    Neusoft.FrameWork.WinForms.Classes.CustomerFp.ReadColumnFormatProperty(this.neuSpread1_Sheet1, this.sheetXMLFileName);
                }
                else
                {
                    Neusoft.FrameWork.WinForms.Classes.CustomerFp.SaveColumnFormatProperty(this.neuSpread1_Sheet1, this.sheetXMLFileName);
                }
            }
        }
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            if (al.Count > 0)
            {
                ...
                if (al.Count > 0)
                {
                    patientNO = (al[0] as Neusoft.HISFC.Models.Order.ExecOrder).Order.Patient.ID;
                }
                Dictionary<string, List<Neusoft.HISFC.Models.Order.Inpatient.Order>> lisPrint = new Dictionary<string, List<Neusoft.HISFC.Models.Order.Inpatient.Order>>();
                for (int i = 0; i < al.Count; i++)
                {
                    Neusoft.HISFC.Models.Order.ExecOrder exeord = al[i] as Neusoft.HISFC.Models.Order.ExecOrder;

                    if (patientNO != exeord.Order.Patient.ID)
                    {
                        foreach (string ApplyNo in lisPrint.Keys)
                        {
                            List<Neusoft.HISFC.Models.Order.Inpatient.Order> alPrint = lisPrint[ApplyNo];
                        ...
                        lisPrint = new Dictionary<string, List<Neusoft.HISFC.Models.Order.Inpatient.Order>>();

                    }

                    Neusoft.HISFC.Models.Order.Inpatient.Order orderObj = this.orderManager.QueryOneOrder(exeord.Order.ID);
                    if (orderObj == null)
                    {
                    ...
                    {
                        List<Neusoft.HISFC.Models.Order.Inpatient.Order> list = new List<Neusoft.HISFC.Models.Order.Inpatient.Order>();
                        list.Add(orderObj);
