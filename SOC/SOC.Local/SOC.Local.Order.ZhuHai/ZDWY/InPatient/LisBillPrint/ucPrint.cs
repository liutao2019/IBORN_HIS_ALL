...
namespace Neusoft.SOC.Local.Order.ZhuHai.ZDWY.InPatient.ExecBill
{
    public partial class ucPrint : UserControl
    {
        ...
        private void printPage()
        {
            ...
            printer.SetPageSize(new Neusoft.HISFC.Models.Base.PageSize("A4", 827, 1169));
        ...
        public void SetData(string execBill, ArrayList allDataByExec, Dictionary<string, ArrayList> alExecByOrderNo)
        {
            ...
            foreach (Neusoft.HISFC.Models.Order.ExecOrder execOrder in allDataByExec)
            {
            ...
            allPatientBed.Sort(new Neusoft.SOC.Local.Order.ZhuHai.ZDWY.Common.Comparer.BedCompare());

            int curRowNum = 0;
            int curPageNo = 1;
            int maxPagNo = 1;
            //计算总页数
            foreach (Neusoft.FrameWork.Models.NeuObject patientBed in allPatientBed)
            {
            ...
            foreach (Neusoft.FrameWork.Models.NeuObject patientBed in allPatientBed)
            {
                ...
                //Neusoft.HISFC.Models.RADT.PatientInfo inPatient = this.inPatientMgr.QueryPatientInfoByInpatientNO((allPatientData[0] as Neusoft.HISFC.Models.Order.ExecOrder).Order.Patient.ID);

                string strSQL = @"select name from fin_ipr_inmaininfo i where i.inpatient_no='{0}'";
                strSQL = string.Format(strSQL, (allPatientData[0] as Neusoft.HISFC.Models.Order.ExecOrder).Order.Patient.ID);
                ...
                if (reMainCount > 0)
                {
                    for (int index = 0; index <= reMainCount; index++)
                    {
                        ...
                        {
                            this.SetTitl((allPatientData[0] as Neusoft.HISFC.Models.Order.ExecOrder), curPageNo, maxPagNo);
        ...
        private int SetDetail(string name, string bedNO, ArrayList allExecBill, Dictionary<string, ArrayList> alExecByOrderNO, string printType)
        {
            ...
            allExecBill.Cast<Neusoft.HISFC.Models.Order.ExecOrder>()
                ...
                {
                    //Neusoft.HISFC.Models.RADT.PatientInfo inPatient = this.inPatientMgr.QueryPatientInfoByInpatientNO((allExecBill[0] as Neusoft.HISFC.Models.Order.ExecOrder).Order.Patient.ID);
                    Neusoft.HISFC.Models.RADT.PatientInfo inPatient = aGroup.First().Order.Patient;
                    ...
                    aGroup.ToList().ForEach(execOrder =>
                    {
                        ...
                        if (execOrder.Order.Item.ItemType == Neusoft.HISFC.Models.Base.EnumItemType.Drug)
                        {
                            ...
                            {
                                this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.Rows.Count - 1, 1).Text = indexCount + "." + execOrder.Order.Item.Name + "(" + ((Neusoft.HISFC.Models.Pharmacy.Item)execOrder.Order.Item).BaseDose + ((Neusoft.HISFC.Models.Pharmacy.Item)execOrder.Order.Item).DoseUnit + "/" + ((Neusoft.HISFC.Models.Pharmacy.Item)execOrder.Order.Item).MinUnit + ")";
                            }
                            else
                            {
                                this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.Rows.Count - 1, 1).Text = "".PadLeft((indexCount + ".").Length, ' ') + execOrder.Order.Item.Name + "(" + ((Neusoft.HISFC.Models.Pharmacy.Item)execOrder.Order.Item).BaseDose + ((Neusoft.HISFC.Models.Pharmacy.Item)execOrder.Order.Item).DoseUnit + "/" + ((Neusoft.HISFC.Models.Pharmacy.Item)execOrder.Order.Item).MinUnit + ")";
                            }
                        }
                        else
                        {
                        ...
                        if (curcomboNo != execOrder.Order.Combo.ID)
                        {
                            ...
                            if (execOrder.Order.OrderType.IsDecompose)
                            {
                                ...
                                if (execOrder.Order.Item.ItemType == Neusoft.HISFC.Models.Base.EnumItemType.Drug)
                                {
                                    ...
                                    {
                                        int hour = (allDataByOrderNo[index] as Neusoft.HISFC.Models.Order.ExecOrder).DateUse.Hour;
                                        hourText += (allDataByOrderNo[index] as Neusoft.HISFC.Models.Order.ExecOrder).DateUse.Hour.ToString().PadLeft(2, '0') + ":" + (allDataByOrderNo[index] as Neusoft.HISFC.Models.Order.ExecOrder).DateUse.Minute.ToString().PadLeft(2, '0');
