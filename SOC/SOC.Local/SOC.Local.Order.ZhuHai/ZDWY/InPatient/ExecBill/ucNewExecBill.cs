...
namespace FS.SOC.Local.Order.ZhuHai.ZDWY.InPatient.ExecBill
{
    public partial class ucNewExecBill : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        /// 患者列表
        /// </summary>
        private List<FS.HISFC.Models.RADT.PatientInfo> _patientList = new List<FS.HISFC.Models.RADT.PatientInfo>();
        ...
        private ExecTreatmentBillPrintControl _printTreatmentControl = new ExecTreatmentBillPrintControl();
        ...
        {
            _alExecBill = execBillManager.QueryExecBill(((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Nurse.ID);
            if (_alExecBill == null)
            {
            ...
            for (int i = 0, count = 0; count < _alExecBill.Count && i < this.gbUsage.Controls.Count; i++)
            {
                ...
                this.gbUsage.Controls[i].Text = ((FS.FrameWork.Models.NeuObject)_alExecBill[count]).Name;
        ...
        private void Query(List<FS.HISFC.Models.RADT.PatientInfo> patientList)
        {
            ...
            for (int i = 0; i < this.gbUsage.Controls.Count; i++)
            {
                if (this.gbUsage.Controls[i].Visible == true && !(this.gbUsage.Controls[i].Tag is string))
                {
                    ...
                    {
                        FS.FrameWork.Models.NeuObject billObj = (FS.FrameWork.Models.NeuObject)gbUsage.Controls[i].Tag;
            ...
            foreach (FS.HISFC.Models.RADT.PatientInfo p in patientList)
            {
                strInpatientNo += ",'" + p.ID + "'";
            }
            ...
            Dictionary<string, FS.FrameWork.Models.NeuObject> dicExecBill = new Dictionary<string, FS.FrameWork.Models.NeuObject>();
        ...
        private void query(FS.HISFC.Models.RADT.PatientInfo patient, ArrayList alPrintExecBill)
        {
            ...
            foreach (FS.FrameWork.Models.NeuObject obj in alPrintExecBill)
            {
                ...
                foreach (FS.HISFC.Models.Order.ExecOrder execOrder in alExecOrder)
                {
                    string deptType = (FS.SOC.HISFC.BizProcess.Cache.Common.GetDept(execOrder.Order.ExeDept.ID)).DeptType.ID.ToString();
                ...
                foreach (FS.HISFC.Models.Order.ExecOrder execOrder in alDiffDeptOrder)
                {
        ...
        {
            alExecOrder.Sort(new FS.SOC.Local.Order.ZhuHai.ZDWY.Common.Comparer.OrderCompareByPatientAndUsage());
        ...
        private void addRow(FS.HISFC.Models.RADT.PatientInfo patient, FS.HISFC.Models.Order.ExecOrder execOrder)
        {
            ...
            this.neuSpread1_Sheet1.Cells[curRow, (int)Columns.备注].Text = execOrder.Order.Memo; //{0A476EA2-9BBF-48bb-99FF-34E08D516AD6}
        ...
        private void printDt_Click(object sender, EventArgs e)
        {
            if (this._patientList.Count == 0)
            {
                MessageBox.Show("请先选择客户查询再打印！！！");
                return;
            }

            string strInpatientNo = "";

            foreach (FS.HISFC.Models.RADT.PatientInfo p in _patientList)
            {
                strInpatientNo += ",'" + p.ID + "'";
            }

            ArrayList alExecOrder = localMgr.GetZLPrint(strInpatientNo.Substring(1));
            ...
            _printTreatmentControl.SetExecBill(alExecOrder, "", "1", _dicExecByOrderNO);
        ...
        protected override int OnSetValues(ArrayList alValues, object e)
        {
            ...
            {
                FS.FrameWork.WinForms.Classes.Function.ShowBalloonTip(3, "错误", ex.Message, ToolTipIcon.Error);
            }
            finally
            {
        ...
        enum Columns
        {
            ...
            频率,
            执行时间,
            备注, //{0A476EA2-9BBF-48bb-99FF-34E08D516AD6}
            是否打印
