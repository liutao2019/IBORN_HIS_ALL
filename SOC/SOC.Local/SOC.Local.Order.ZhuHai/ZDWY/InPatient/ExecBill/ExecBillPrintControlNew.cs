...
namespace FS.SOC.Local.Order.ZhuHai.ZDWY.InPatient.ExecBill
{
    ...
    /// 执行单打印界面
    /// </summary>
    public partial class ExecBillPrintControlNew : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        private void printPage()
        {
            ...
            //if (pSize == null)
            //{
            //    printer.SetPageSize(new FS.HISFC.Models.Base.PageSize("execBillPaper", 1100, 850));
            //}
            //else
            //{
        ...
        private int SetDetail(string name, string bedNO, ArrayList allExecBill, Dictionary<string, ArrayList> alExecByOrderNO, string printType)
        {
                ...
                .ToList().ForEach(aGroup =>
                {
                    ...
                    aGroup.ToList().ForEach(execOrder =>
                    {
                        ...
                        if (string.IsNullOrEmpty(name))
                        {
                            ...
                            // {CDB01BF4-B40F-4cdc-9F0D-23F074290136}
                            FS.HISFC.Models.RADT.PatientInfo patientInfo = FS.SOC.HISFC.BizProcess.Cache.BIZManager.InPatientMgr.GetPatientInfoByPatientNO(execOrder.Order.Patient.ID.ToString());
                            name = patientInfo.Name;
                            age = FS.SOC.HISFC.BizProcess.Cache.BIZManager.InPatientMgr.GetAge(patientInfo.Birthday);

                            //FZC 修改 
                            this.neuSpread1_Sheet1.Cells.Get(patientRowIndex, 2).Text = "住院号:" + patientInfo.PID.PatientNO.TrimStart('0') + "    床号:" + bedNo + "    姓名:" + name.ToString() + "    年龄:" + age;
                        }
                        ...
                        {
                            this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.Rows.Count - 1, 2).Text = execOrder.Order.MOTime.ToString();
                            
                            this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.Rows.Count - 1, 4).Text = execOrder.Order.DoseOnce + execOrder.Order.DoseUnit;
                            ...
                            if (curcomboNo != execOrder.Order.Combo.ID)
                            {
                                ...
                                if (!execOrder.Order.OrderType.IsCharge)
                                {
                                    if (execOrder.Order.Item.ItemType == FS.HISFC.Models.Base.EnumItemType.Drug)
                                    {
                                        this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.Rows.Count - 1, 3).Text = "(自备)" + execOrder.Order.Item.Name + orderManager.TransHypotest(execOrder.Order.HypoTest) + memo;
                                    }
                                    else
                                    {
                                        this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.Rows.Count - 1, 3).Text = "(自备)" + execOrder.Order.Item.Name + orderManager.TransHypotest(execOrder.Order.HypoTest) + memo;
                                ...
                                {
                                    this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.Rows.Count - 1, 3).Text = execOrder.Order.Item.Name + orderManager.TransHypotest(execOrder.Order.HypoTest) + memo;
                                }
                                ...
                                this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.Rows.Count - 1, 7).Text = mess;                                
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.Rows.Count - 1, 2).Text = execOrder.Order.MOTime.ToString();
                            this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.Rows.Count - 1, 4).Text = execOrder.Order.Qty + execOrder.Order.Unit + " ";
                            this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.Rows.Count - 1, 1).Tag = execOrder.Order.Combo.ID;
                            this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.Rows.Count - 1, 1).Text = execOrder.Order.Combo.Memo;
                            ...
                            if (curcomboNo != execOrder.Order.Combo.ID)
                            {
                                ...
                                if (!execOrder.Order.OrderType.IsCharge)
                                {
                                    if (execOrder.Order.Item.ItemType == FS.HISFC.Models.Base.EnumItemType.Drug)
                                    {
                                        this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.Rows.Count - 1, 3).Text = "(自备)" + execOrder.Order.Item.Name + memo;
                                    }
                                    else
                                    {
                                        this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.Rows.Count - 1, 3).Text = "(嘱托)" + execOrder.Order.Item.Name + orderManager.TransHypotest(execOrder.Order.HypoTest) + memo;
                                    }
                                }
                                else
                                {
                        ...
                        if (curcomboNo != execOrder.Order.Combo.ID)
                        {
                            this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.Rows.Count - 1, 5).Text = FS.SOC.HISFC.BizProcess.Cache.Common.GetUsageName(execOrder.Order.Usage.ID);
                            if (execOrder.Order.OrderType.IsDecompose)// {407F4A63-CC38-4842-BFDA-995E1C3FC664}
                            {
                                this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.Rows.Count - 1, 6).Text = execOrder.Order.Frequency.ID;

                            }
                            else
                            {
                                if (execOrder.Order.Item.ItemType == FS.HISFC.Models.Base.EnumItemType.UnDrug)
                                {
                        ...
                        curcomboNo = execOrder.Order.Combo.ID;
                        this.neuSpread1_Sheet1.Cells.Get(this.neuSpread1_Sheet1.Rows.Count - 1, 8).Text = ". ";
                    });
                    #endregion
                });
            ...
            FS.SOC.Local.Order.ZhuHai.Classes.Function.DrawComboLeft(this.neuSpread1_Sheet1, 1, 1);//画组
        ...
        private void PrintDocumentPrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            #region Farpoint绘制// {407F4A63-CC38-4842-BFDA-995E1C3FC664}
            int drawingWidth = 850 - this.DrawingMargins.Left - this.DrawingMargins.Right;
            int drawingHeight = 1100 - this.DrawingMargins.Top - this.DrawingMargins.Bottom - this.panelTitle.Height;
        ...
        private void SetPaperSize(System.Drawing.Printing.PaperSize paperSize)
        {
            //if (paperSize == null)
            //{
            //    paperSize = new System.Drawing.Printing.PaperSize("execBillPaper", 850, 1100);
            //}
            // {407F4A63-CC38-4842-BFDA-995E1C3FC664}
            if (paperSize == null)
            {
