...
namespace FS.SOC.Local.Order.ZhuHai.ZDWY.InPatient.DrugCard
{
    ...
    /// </summary>
    public partial class ucInfusionLabelNew : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Classes.IControlPrintable
    {
            ...
            set
            {
                ...
                foreach (FS.HISFC.Models.Order.ExecOrder execOrder in alInfusionData)
                {
                    ...
                    if (patientnifo != null)
                    {
                        ...
                        this.txtPatientNo.Text = patientnifo.PID.PatientNO.TrimStart('0');
                        //{D20C82C8-71C1-4f77-A380-09776338A590}
                        //this.lblBedNo.Text = patientnifo.PVisit.PatientLocation.Bed.ID.Substring(4) + "床 "; {14f06e15-1970-4fdd-a12b-70dfeaafad6d}
                        string bed = patientnifo.PVisit.PatientLocation.Bed.Name;
                        this.lblBedNo.Text = bed.Contains("VIP待产室") ? bed : bed + "床 ";
                        lblName.Text = patientnifo.Name + " " + patientnifo.Sex.Name + " " + this.orderManager.GetAge(patientnifo.Birthday);
                    }
                    ...
                    this.lblUsage.Text = "用法：" + execOrder.Order.Usage.Name;

                    if (rowIndex == 0)
                    {
                        this.fpSpread1_Sheet1.Cells[rowIndex, 0].Text = (execOrder.Order.OrderType.IsDecompose ? "长" : "临") + execOrder.Order.SubCombNO.ToString();

                        this.fpSpread1_Sheet1.Cells[rowIndex, 4].Text = execOrder.Order.Frequency.ID;
                    }
                    ...
                    itemName = byoStr + execOrder.Order.Item.Name + "【" + execOrder.Order.Item.Specs + "】";
