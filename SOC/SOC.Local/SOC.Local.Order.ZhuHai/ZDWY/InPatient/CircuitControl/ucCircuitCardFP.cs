    ...
    public partial class ucCircuitCardFP : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.HISFC.BizProcess.Interface.IPrintTransFusion
    {
        ...
        private void AddLine()
        {
            ...
            if (this.neuSpread1_Sheet1.Rows.Count > 0)//有数据
            {
                for (int i = 0; i < this.neuSpread1_Sheet1.RowCount; i++)
                {
                    ...
                    neuSpread1_Sheet1.Rows[i].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Center;
        ...
        private void AddToFP(ArrayList list)
        {
            ...
            foreach (FS.HISFC.Models.Order.ExecOrder execOrder in list)
            {
                ...
                //名称
                this.neuSpread1_Sheet1.Cells[index, (int)ExecBillCols.ItemName].Text = byoStr + execOrder.Order.Item.Name;
                ...
                //这里存储了实际的执行时间
                this.neuSpread1_Sheet1.Cells[index, (int)ExecBillCols.Memo].Text = execOrder.DateUse.ToString("yyyy-MM-dd HH:mm");
        ...
        public void Print()
        {
            try
            {
                ...
                printerName = FS.HISFC.Components.Common.Classes.Function.ChoosePrinter();
                ...
                if (pSize == null)
                {
                    pSize = new FS.HISFC.Models.Base.PageSize("Letter", 690, 1100);
                }
                ...
                else //打印程序段
                {
                    ...
                    //        ucPrint.execTime = this.dt1.ToString("MM月dd日");
                    ...
                    //}
                    #endregion
                    IList<FS.HISFC.Models.Order.ExecOrder> allDataList = new List<FS.HISFC.Models.Order.ExecOrder>();
                    IDictionary<string, string> makeMessDic = new Dictionary<string, string>();

                    foreach (FS.HISFC.Models.Order.ExecOrder order in alPrint)// {E97273E4-CF5A-47bf-97C6-8025504486C4}
                    {
                        //临嘱无需合并
                        if (!order.Order.OrderType.IsDecompose)
                        {

                            order.DateUse = order.DateUse.Date;
                        }
                        else
                        {
                            if (!makeMessDic.ContainsKey(order.Order.Combo.ID + order.Order.Item.ID))
                            {
                                makeMessDic.Add(order.Order.Combo.ID + order.Order.Item.ID, Function.GetShowTime(order.DateUse, order.DateUse));
                            ...
                            {
                                makeMessDic[order.Order.Combo.ID + order.Order.Item.ID] += "," + Function.GetShowTime(order.DateUse, order.DateUse);
                            }
                    ...
                    #region 新打印 FZC
                    //转成高效且易用的IList
                    IList<FS.HISFC.Models.Order.ExecOrder> allDataList1 = new List<FS.HISFC.Models.Order.ExecOrder>();

                    foreach (var order in allDataList)
                    {
                        ...
                        allDataList1.Add(order);
                    }
                    allDataList = allDataList1;// {E97273E4-CF5A-47bf-97C6-8025504486C4}
                    //foreach (FS.HISFC.Models.Order.ExecOrder order in alPrint)
                    //{
                    //    order.DateUse = order.DateUse.Date;
                    //    allDataList.Add(order);
                    //}
                    //按日期分组 不同日期分开打印
                    var daySplitList = allDataList.GroupBy(o => o.DateUse.Date);

                    foreach (var oneDayData in daySplitList)
                    {
                        //过滤出一天的数据 再按患者分组
                        var personSplitList = oneDayData.ToList().GroupBy(o => o.Order.Patient.Name);
                        foreach (var onePersonData in personSplitList)
                        {
                            //患者数据列表
                            IList<FS.HISFC.Models.Order.ExecOrder> orderList = new List<FS.HISFC.Models.Order.ExecOrder>();
                            IList<string> str = new List<string>();
                            //去掉重复的长嘱
                            foreach (var order in onePersonData.ToList())
                            {
                                if (order.Order.OrderType.ID == "LZ")
                                {
                                    orderList.Add(order);
                                }
                                else
                                {
                                    if (!str.Contains(order.Order.SortID + order.Order.Item.ID))
                                    {
                                        orderList.Add(order);
                                        str.Add(order.Order.SortID + order.Order.Item.ID);
                                    }
                                }
                            }

                            //按组号分组 用于使同组出现在一张纸上
                            var comboSplitList = orderList.GroupBy(o => o.Order.Combo.ID);
                            IList<FS.HISFC.Models.Order.ExecOrder> orderPrintList = new List<FS.HISFC.Models.Order.ExecOrder>();
                            int dataCount = 0;
                            foreach (var oneComboList in comboSplitList)
                            {
                                //本页内容还未满 继续添加
                                if ((dataCount + oneComboList.ToList().Count()) < 12)
                                {
                                    foreach (var order in oneComboList.ToList())
                                    {
                                        orderPrintList.Add(order);
                                        dataCount++;
                                    }
                                }
                                else if((dataCount + oneComboList.ToList().Count())== 12)
                                {
                                    //加完这组正好满了 添加后打印并清空数据
                                    foreach (var order in oneComboList.ToList())
                                    {
                                    ...
                                    //打印方法
                                    PrintFZC(print, dicPatientInfo, orderPrintList);

                                    //清空
                                    orderPrintList = new List<FS.HISFC.Models.Order.ExecOrder>();
                                    dataCount = 0;
                                }
                                else
                                {
        ...
        /// <param name="dicPatientInfo"></param>
        /// <param name="orderList"></param>
        private void PrintFZC(FS.FrameWork.WinForms.Classes.Print print, Dictionary<string, FS.HISFC.Models.RADT.PatientInfo> dicPatientInfo, IList<FS.HISFC.Models.Order.ExecOrder> orderList)
        {
            //打印机名称
            ...
            {
                ucPrint.isPrintMemo = true;
            }
            ucPrint.execTime = orderList[0].DateUse.ToShortDateString();
            ucPrint.SetHeader(this.lblTitle.Text, "", this.orderBizLogic.GetDateTimeFromSysDateTime().ToString(), 1, 1);
            ucPrint.SetItemsFZC(orderList);
            ucPrint.SetPatientInfo(dicPatientInfo[orderList[0].Order.Patient.ID]);
        ...
        protected void SetValues(ArrayList alValues)
        {
            ...
            if (alValues != null)
            {
                ...
                FS.SOC.Local.Order.ZhuHai.Classes.Function.DrawComboLeft(this.neuSpread1_Sheet1, (int)(int)ExecBillCols.ComboNo, (int)ExecBillCols.ComboMemo);
        ...
        public void Query(List<FS.HISFC.Models.RADT.PatientInfo> patients, string usageCode, DateTime dtBegin, DateTime dtEnd, bool isPrinted, string orderType, bool isFirst)
        {
            ...
            {
                if (!exeOrder.IsValid || exeOrder.Order.Status == 3||exeOrder.Order.Status==4)
                {

                }
                else
                {
                    validList.Add(exeOrder);
                }
            }
    ...
    public class ComparerExecOrder : IComparer
    {
        ...
        public int Compare(object x, object y)
        {
            try
            {
                ...
                string cc = execOrder1.Order.MOTime.ToString("yyyyMMdd") + execOrder1.Order.Combo.ID + execOrder1.DateUse.ToString() + execOrder1.Order.ID;
                string dd = execOrder2.Order.MOTime.ToString("yyyyMMdd") + execOrder2.Order.Combo.ID + execOrder2.DateUse.ToString() + execOrder2.Order.ID;
