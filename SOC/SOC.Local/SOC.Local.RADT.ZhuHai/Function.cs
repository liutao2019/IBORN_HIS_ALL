...
namespace FS.SOC.Local.RADT.ZhuHai
{
    public class Function
    {
        ...
        {
            //if (IsContainYKDept())// {5B3B503C-8CF5-415b-89EB-C11A4FEE8A19}
            //{
        ...
        /// <summary>
        /// 根据患者类型获取临时住院号// {F6204EF5-F295-4d91-B81A-736A268DD394}
        /// </summary>
        /// <param name="tempPatientNO"></param>
        /// <returns></returns>
        public static int GetAutoTempPatientNOByType(string typeCode,ref string tempPatientNO)
        {
            FS.HISFC.BizLogic.RADT.InPatient inpatientMgr = new FS.HISFC.BizLogic.RADT.InPatient();
            tempPatientNO = inpatientMgr.GetNewTempPatientNoByType(typeCode);
            if (string.IsNullOrEmpty(tempPatientNO) || tempPatientNO == "-1")
            {
                err = "获取临时住院号失败";
                return -1;
        ...
        {
            if (FS.SOC.Local.RADT.ZhuHai.ZDWY.Interface.InterfaceManager.GetIOperCard() == null)
            {
                errInfo = "没有维护读卡接口！";
                return -1;
            }

            int result = FS.SOC.Local.RADT.ZhuHai.ZDWY.Interface.InterfaceManager.GetIOperCard().ReadCardNO(ref cardNO, ref  errInfo);
            if (result == -1)
            {
        ...
        /// <summary>
        /// 读卡接口// {DC68D3DF-1CB9-4d58-93E0-4F85B58E1647}
        /// </summary>
        /// <param name="cardNO"></param>
        /// <param name="errInfo"></param>
        /// <returns></returns>
        public static int OperMCard(ref string mCardNO, ref string errInfo)
        {
            ...
            int result = FS.SOC.Local.RADT.ZhuHai.ZDWY.Interface.InterfaceManager.GetIOperCard().ReadMCardNO(ref mCardNO, ref  errInfo);
            if (result == -1)
            {
                errInfo = "读卡失败，请确认是否正确放置诊疗卡！";
                return -1;
