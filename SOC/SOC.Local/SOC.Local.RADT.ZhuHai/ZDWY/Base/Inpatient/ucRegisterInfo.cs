...
namespace FS.SOC.Local.RADT.ZhuHai.ZDWY.Base.Inpatient
{
    ...
    public partial class ucRegisterInfo : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.SOC.HISFC.RADT.Interface.Register.IInpatient
    {
        public ucRegisterInfo()
        {
            ...
            this.ckHour.CheckedChanged += new EventHandler(ckHour_CheckedChanged);
        }
        ...
        private bool isCanModifyName = false;
        private bool isCanModifyPatientNo = false;
        private bool isShowPrePay = true;
        // {1470D480-E0E3-4ab7-AECE-2B75E9101D4E}
        ...
        private string fertilityPatientNO = ""; //自动生成的生殖号
        //---add--
        decimal DayLimit = 0;
        ...
        {
            get { return cardNo; }// {249D5ADE-8BE6-4c90-A8D1-F99540D24A64}
            set
            {
                this.txtClinicNO.Focus();
                this.txtClinicNO.SelectAll();
                this.txtClinicNO.Text = value;
                this.txtClinicNO_KeyDown(null,new KeyEventArgs(Keys.Enter));
        ...
        {
            //性别列表
            ArrayList talSex = FS.HISFC.Models.Base.SexEnumService.List();
            ArrayList alSex = new ArrayList();
            //去掉全部和其他选项
            alSex.Add(talSex[0]);
            alSex.Add(talSex[1]);
            ...
            this.cmbCountry.Tag = "156";
            //入院诊断
            this.txtDiagnose.AddItems(commonController.QueryConstantBySql(@"select 'ICD10' as type,
                                                                            icd_code as code,
                                                                            icd_name as name,
                                                                            '' as mark,
            ...
            foreach (FS.HISFC.Models.Base.Spell s in commonController.QueryDepartment(true))
            {
                if (Function.IsContainYKDept(s.ID).Equals(Function.IsContainYKDept()))
                {
                    alDept.Add(s);
                }
            }
            ...
            //患者类型信息
            this.cmbPatientType.AddItems(commonController.QueryConstant("PatientType"));// {F6204EF5-F295-4d91-B81A-736A268DD394}
            //转科的原科室
            ...
            //ArrayList alDist = commonController.QueryConstant(EnumConstant.cased);
            //定义变量
            ...
            this.cmbPatientType.Tag = "P";// {F6204EF5-F295-4d91-B81A-736A268DD394}
            this.txtHomeZip.AddItems(alTemp);

            //住院号是否可以编辑
            this.rbtnTempPatientNO.Enabled = this.isCanModifyPatientNo;
            this.rdoInpatientNO.Enabled = this.isCanModifyPatientNo;
            this.txtInpatientNO.Enabled = this.isCanModifyPatientNo;
            this.PrePayVisible(this.isShowPrePay);
        ...
        public bool IsSpReg
        {
            set
            {
                ...
                if (isSpReg)
                {
                    ...
                    foreach (FS.HISFC.Object.Fee.PactUnitInfo info in alPact)
                    {
                    ...
                    else
                    {
                        ...
                        //							bool parm = false;
                        //							//在通用的合同单位中去除受限制的合同单位
                        //							foreach(FS.neuFC.Object.neuObject neuobj in alPactForInpatient) 
                        //							{
                        //								foreach(FS.HISFC.Object.Fee.PactUnitInfo info in alPact)
                        //								{
        ...
        private void PrePayVisible(bool enab)
        {
            this.lblBloodFee.Visible = enab;
            this.mtxtBloodFee.Visible = enab;
            this.lblComputerNO.Visible = enab;
            this.txtComputerNO.Visible = enab;
            this.txtDayLimit.Visible = enab;
            this.lblDayLimit.Visible = enab;
            this.lblBedLimit.Visible = enab;
            this.txtBedLimit.Visible = enab;
            this.mTxtPrepay.Visible = enab;
            this.lblPrepay.Visible = enab;
            this.lblPayMode.Visible = enab;
            this.cmbPayType.Visible = enab;
            this.cmbBedOverDeal.Visible = enab;
            this.lblBedOverDeal.Visible = enab;
            this.lblOverLop.Visible = enab;
            this.cmbOverLop.Visible = enab;
            this.lblAirLimit.Visible = enab;
            this.txtAirLimit.Visible = enab;
            this.lblRate.Visible = enab;
            this.vtbRate.Visible = enab;
            this.lbRealInvoiceNO.Visible = enab;
            this.txtRealInvoiceNO.Visible = enab;
            this.btnUpdateRealInvoiceNO.Visible = enab;
        }
        /// <summary>
        /// 设置患者信息到控件
        /// </summary>
        ...
        private void setPatientInfo(PatientInfo patient, bool isAll)
        {

            /********************************************************
             * *           住院登记界面患者信息设置逻辑
             * *    首先根据姓名或者门诊卡号找到对应的患者信息之后，调用此
             * *函数进行界面赋值，此时 isAll 值为false,表示只针对患者基
             * *本信息进行赋值，不对住院信息进行赋值
             * *    之后根据患者的姓名去查找住院信息用户确认选择之后，返回
             * *改住院信息对应的患者最近一次住院信息对界面进行重新赋值，此
             * *时 isALL 为true,会针对住院号，住院次数等进行赋值。
             * *    2019-12-11修改增加通过姓名查询旧系统数据，此时存在有可
             * *住院号是旧系统中取出的，该住院号在新系统中未住院过，此时其
             * *它字段值为空，patient.PID.CardNO会处理为以 “T”开头的卡号，
             * *表示改住院号在新系统未找到任何住院记录，patient对象中仅存在
             * *patient.PID.patientNO、patient.InTimes有值和
             * * patient.PID.CardNO有值，此时赋值需要做特殊处理，相当于新系
             * *中的患者需要用旧系统中患者的住院号。
             *******************************************************/


            //若patient.PID.CardNO第一个字母为T，说明以住院号在新系统查询
            //患者住院信息为空，是患者之前在旧系统住院过，产生了住院号，但
            //是在新系统中未住过院，此时要做特殊处理
            //{67F0DA44-EB32-4556-96C5-88B60C59E157}
            if (patient.PID.CardNO.Substring(0, 1).Equals("T"))
            {
                ...
                if (!string.IsNullOrEmpty(patient.PID.PatientNO))
                {
                    this.patientNO = patient.PID.PatientNO.PadLeft(10, '0');
            ...
            cardNo = patient.PID.CardNO;//{94F7BB9C-A3AC-43c8-A5B0-272575A6DF75}
            this.setPatient(patient);

            this.cmbPatientType.Tag = patient.PatientType.ID;
            if (string.IsNullOrEmpty(patient.PatientType.ID))
            {
            ...
            cbxChild.Checked = p.ChildFlag == "1" ? true : false;
            ...
            if (isAll == true)
            {
                ...
                {
                    this.cmbApproach.Tag = 1;
                }
                //入院来源
                if (patient.PVisit.InSource.ID == string.Empty)
                {
                    this.cmbInSource.Tag = "2";
                }
                else
                {
                ...
                #region 屏蔽冗余代码
                //{67F0DA44-EB32-4556-96C5-88B60C59E157}
                //string strSQL = @"select f.pay_way from fin_ipb_inprepay f
                //            where f.inpatient_no ='27346'
                //            and trunc(f.oper_date)=(select trunc(i.in_date) from fin_ipr_inmaininfo i
                //            where i.inpatient_no='27346')
                //            and rownum=1";
                //FS.HISFC.BizLogic.Manager.DataBase dbMgr = new FS.HISFC.BizLogic.Manager.DataBase();
                //string strPayMode = dbMgr.ExecSqlReturnOne(strSQL);
                //if (!string.IsNullOrEmpty(strPayMode)
                //    && strPayMode != "-1")
                //{
                //    this.cmbPayType.Tag = strPayMode;
                //}
                //if (cmbPayType.Tag == null
                //    || string.IsNullOrEmpty(cmbPayType.Tag.ToString()))
                //{
                //    this.cmbPayType.Tag = "CA";
                //    this.cmbPayType.Text = "现金";
                //}
        ...
        private void setPatient(FS.HISFC.Models.RADT.Patient patient)
        {
            ...
            //string packCode = ctlParamManage.GetControlParam<string>("SI0001", true, "1");

            //if (patient.Pact.ID == "4")
            //{
            //    pact = this.PactUnit.GetPactUnitInfoByPactCode(packCode);
            ...
            //患者备注
            this.txtMemo.Text = patient.Memo;// {99D2DB54-67B7-4ed1-A59E-97BE8612FE43}
            
            cbxChild.Checked = patient.ChildFlag == "1";
            try
            {
            ...
            {
                this.processIDENNO(this.txtIDNO.Text.Trim(), EnumCheckIDNOType.BeforeSave);
                this.cmbCountry.Tag = "156";
                this.cmbMarry.Select();
                this.cmbMarry.Focus();
            }

            string SQL = @"select * from ( select diag_name from fin_opr_register  r
                                                            left join met_cas_diagnose d 
                                                              on r.clinic_code = d.inpatient_no
                                            where card_no = '{0}'
                                              and diag_kind = '1' 
                                              and happen_no = '1'
                                            order by reg_date desc) where rownum = 1";

            string strSQL = string.Format(SQL, cardNo);

            //FS.HISFC.BizLogic.Manager.DataBase dbMgr = new FS.HISFC.BizLogic.Manager.DataBase();
            //dbMgr.ExecSqlReturnOne(strSQL);

            string strPayMode = Constant.ExecSqlReturnOne(strSQL);
            if(strPayMode!=null&&strPayMode!="-1")
                txtDiagnose.Text = strPayMode;
        }
        ...
        private PatientInfo getPatientInfomation(PatientInfo patient)
        {
            ...
            patient = accountMgr.GetPatientInfoByCardNO(cardNo);//{94F7BB9C-A3AC-43c8-A5B0-272575A6DF75}获取患者信息再赋值
            if (patient == null)
            {
            ...
            {
                myMessageBox("出生日期输入错误，请修改！", MessageBoxIcon.Stop);
            ...
            if (this.txtWorkAddress.Text == "")
            {
                patient.CompanyName = "-";
            }
            else
            {
                patient.CompanyName = this.txtWorkAddress.Text;
            }
            //end modify
            ...
            patient.PatientType.ID = this.cmbPatientType.Tag.ToString();//患者类型// {F6204EF5-F295-4d91-B81A-736A268DD394}
            patient.PatientType.Name = this.cmbPatientType.Text;//患者类型// {F6204EF5-F295-4d91-B81A-736A268DD394}

            //modify by zhy
            // patient.HomeZip = this.txtHomeZip.Text;//家庭地址邮编
            if (this.txtHomeZip.Text == "")
            {
                patient.HomeZip = "-";
            }
            else
            {
                patient.HomeZip = this.txtHomeZip.Text;
            }
            //end modify
            patient.AddressBusiness = this.txtAddressNow.Text;//现地址
            ...
            {
                patient.PhoneBusiness = this.txtWorkPhone.Text;
            }
            //end modify
            patient.IDCard = SpecialFilte(this.txtIDNO.Text);//身份证
            ...
            patient.FT.AirLimitCost = FS.FrameWork.Function.NConvert.ToDecimal(txtAirLimit.Text);  //监护床(空调上限)
            patient.FT.DayLimitCost = FS.FrameWork.Function.NConvert.ToDecimal(txtDayLimit.Text);  //公费日限
            patient.FT.BedLimitCost = FS.FrameWork.Function.NConvert.ToDecimal(txtBedLimit.Text);  //普通标准(床位上限)
            patient.FT.FTRate.PayRate = FS.FrameWork.Function.NConvert.ToDecimal(vtbRate.Text);  //自付比例
            ...
            patient.FT.DayLimitTotCost = FS.FrameWork.Function.NConvert.ToDecimal(this.txtDayLimit.Text);//日限额累计     
            ...
            #region 增加备注
            patient.Memo = this.txtMemo.Text.Trim();
            //{05024624-3FF4-44B5-92BF-BD4C6FAF6897}添加儿童标签 儿童的必须填写联系人电话 非儿童则必须填写联系电话 且联系电话不能重复
            patient.ChildFlag = cbxChild.Checked?"1":"0";
            //{1A1ECA02-C14B-4c25-9962-7797FDE2F7E2}使用扩展标记2作为icd10的储存位置
            patient.ExtendFlag2 = txtDiagnose.Tag.ToString();
            if (patient.ExtendFlag2 == null || patient.ExtendFlag2 == "" || patient.ExtendFlag2 == "N05.400")
            {
                patient.ExtendFlag2 = "MS999";
            }
        ...
        {
            string idCard = txtIDNO.Text;
            if (!string.IsNullOrEmpty(idCard) && idCard != "-")
            {
                string errText = string.Empty;
                int returnValue = FS.FrameWork.WinForms.Classes.Function.CheckIDInfo(idCard, ref errText);
                if (returnValue == -1)
                {
            ...
            this.txtInpatientNO.Text = this.txtInpatientNO.Text.ToUpper();
            ...
            FS.HISFC.Models.RADT.PatientInfo patientInfo = new PatientInfo();// {5B3B503C-8CF5-415b-89EB-C11A4FEE8A19}

            //判断住院状态
            ArrayList alPatientInfo = this.radtInpatient.GetPatientInfoByCardNO(this.txtClinicNO.Text);
            foreach (FS.HISFC.Models.RADT.PatientInfo obj in alPatientInfo)
            {
                if ((obj.PVisit.InState.ID.ToString() == "R" || obj.PVisit.InState.ID.ToString() == "I"
                    || obj.PVisit.InState.ID.ToString() == "P") && obj.PID.PatientNO.Contains("Y")
                    && this.cmbPatientType.Tag.ToString() == "Y"
                    )
                {
                    this.myMessageBox("此患者已在院治疗!", MessageBoxIcon.Information);
                    return false;
                }
                //{8A1C57D4-3D37-4bc5-AFF1-BF6CC184D69E}
                //此处判断错误，应该是在院有患者，并且在院患者是留观，新登记为临时患者时
                else if ((obj.PVisit.InState.ID.ToString() == "R" || obj.PVisit.InState.ID.ToString() == "I"
                    || obj.PVisit.InState.ID.ToString() == "P") && obj.PID.PatientNO.Contains("L")
                   && this.cmbPatientType.Tag.ToString() == "Y"
                    )
                {
                    this.myMessageBox("此患者已在院治疗!", MessageBoxIcon.Information);
                    ...
                    || obj.PVisit.InState.ID.ToString() == "P") && !obj.PID.PatientNO.Contains("L")
                    && !obj.PID.PatientNO.Contains("Y") && this.cmbPatientType.Tag.ToString() != "Y"
                    )
                {
                    this.myMessageBox("此患者已在院治疗!", MessageBoxIcon.Information);
                    return false;
            ...
            if (string.IsNullOrEmpty(patientInfo.ID) == false && patientInfo.PatientNOType == EnumPatientNOType.First)
            {
                ...
                else if (rbtnTempPatientNO.Checked)
                {
                    ...
                    else if (this.isFertilityPatientNO && this.fertilityPatientNO != this.txtInpatientNO.Text)
                    {
                        this.myMessageBox("自动生成的生殖号不允许修改", MessageBoxIcon.Warning);
                        this.txtInpatientNO.Text = this.fertilityPatientNO;
                        this.txtInpatientNO.Select();
                        return false;
                    }
                }
            ...
            try
            {
                ...
                {
                    long inpatientNO = FS.FrameWork.Public.String.GetNumber(this.txtInpatientNO.Text);
            ...
            //患者类型
            if (this.cmbPatientType.Tag.ToString() == string.Empty)// {F6204EF5-F295-4d91-B81A-736A268DD394}
            {
                this.myMessageBox("患者类型不能为空，请重新输入", MessageBoxIcon.Warning);
            ...
            //判断字符超长工作单位
            if (!FS.FrameWork.Public.String.ValidMaxLengh(this.txtWorkAddress.Text, 100))
            {
            ...
            //诊断 //诊断B987DA4F-0C43-4bd2-8DD9-C3F1978A3493 增加诊断长度限制
            if (!FS.FrameWork.Public.String.ValidMaxLengh(this.txtDiagnose.Text, 200))
            {
            ...
            //诊断必须选择// {FA3F9D7A-1584-4A41-8133-004EF8242862}
            if (string.IsNullOrEmpty(this.txtDiagnose.Text) || string.IsNullOrEmpty(this.txtDiagnose.Tag.ToString())) {
                this.myMessageBox("请选择相应的门诊诊断，禁止手动录入！", MessageBoxIcon.Warning);
                this.txtDiagnose.Select();
            ...
            int  retValue = this.processIDENNO(this.txtIDNO.Text.Trim(), EnumCheckIDNOType.Saveing);
            if (retValue < 0)
            {
                this.myMessageBox("证件校验不通过!", MessageBoxIcon.Warning);
                return false;
            }

            //住院流水号与IDNO是否冲突,不能同时存在住院流水号一致而IDNO不一致的患者,且15位身份证号与18位身份证号分开比较
            ArrayList patientInfoList = this.radtInpatient.GetPatientInfoByPatientNo(this.txtInpatientNO.Text);
            if (patientInfoList != null)
            {
                foreach (FS.HISFC.Models.RADT.PatientInfo patient in patientInfoList)
                {
                    //{C40F0D34-B030-4210-BF66-1B08DFBA8CBD}
                    //无费退院的记录不算有效记录{7FDF13F6-47F9-40de-B7BD-62CBA854D0CB}
                    if (patient.PVisit.InState.ID.ToString().Trim().ToUpper() == "N")
                    {
                        continue;
                    }

                    if (patient.IDCard != null && patient.IDCard.Length == this.txtIDNO.Text.Trim().Length && patient.PID.PatientNO == this.txtInpatientNO.Text.Trim() && patient.IDCard != this.txtIDNO.Text.Trim())
                    {
                        this.myMessageBox("存在住院号相同但身份证号不同的患者信息，请验证后输入!", MessageBoxIcon.Warning);
                        return false;
                    }
                }
            }
            ...
            else
            {
             ...
             //ArrayList patientInfoList = this.radtInpatient
            if (!string.IsNullOrEmpty(txtHomePhone.Text)) //{05024624-3FF4-44B5-92BF-BD4C6FAF6897}添加儿童标签 儿童的必须填写联系人电话 非儿童则必须填写联系电话 且联系电话不能重复
            {
                ...
                list = accountManager.QueryPatientInfoByWhere("", "", txtHomePhone.Text, "");// {CDB01BF4-B40F-4cdc-9F0D-23F074290136}
                if (string.IsNullOrEmpty(CardNo))
                {
                    if (list != null && list.Count > 0)// {06A6BD92-97A6-4f01-8B6E-533B35CBF2DD}
                    {
                        FS.HISFC.Models.RADT.PatientInfo ac = list[0];
                        string strMsg = string.Format("已存在相同【联系电话】,病历号为：{0}", ac.PID.CardNO);
                ...
                {
                    if (list.Where(t => t.PID.CardNO == cardNo).DefaultIfEmpty() != null)
                    {
                        if (list.Count > 1)
                        {
                            FS.HISFC.Models.RADT.PatientInfo ac = list[0];
            ...
            if ((decimal.Parse(this.txtBedLimit.Text) > 0) && (this.cmbBedOverDeal.Text == ""))
            {
                ...
                //FS.neuFC.Interface.Classes.Function.Msg("请选择床费超标处理!",111);
            ...
            if (isSpReg)
            {
                ...
                try
                {
                    tempRate = FS.FrameWork.Function.NConvert.ToDecimal(this.vtbRate.Text);
                }
                catch (Exception ex)
                {
            ...
            ArrayList allNurseCell = new ArrayList();
            allNurseCell = Function.QueryNurseByDept(this.cmbDept.Tag.ToString());
            if (allNurseCell.Count <= 0 || allNurseCell == null)
            {
                this.myMessageBox("入院科室无病区，请重新选择！", MessageBoxIcon.Warning);
                this.cmbDept.Select();
                return false;
            }
            else
            {
                bool isPP = false;
                //FS.FrameWork.Models.NeuObject nurseCell = Function.QueryNurseByDept(this.cmbDept.Tag.ToString())[0] as FS.FrameWork.Models.NeuObject;
                foreach (FS.FrameWork.Models.NeuObject nurseCell in allNurseCell)
                {

                    if (this.cmbNurseCell.Tag.ToString() == nurseCell.ID.ToString())
                    {
                        isPP = true;
                        break;
                    }
                ...
                if (!isPP)
                {
                    this.myMessageBox("科室与病区匹配错误，请致电信息科排查原因！", MessageBoxIcon.Warning);
                    this.cmbDept.Select();
                    return false;
                }
            }
        ...
        {
            int iyear = FS.FrameWork.Function.NConvert.ToInt32(this.txtAgeOne.Text.Trim());
            int iMonth = FS.FrameWork.Function.NConvert.ToInt32(this.txtAgeTwo.Text.Trim());
            int iDay = FS.FrameWork.Function.NConvert.ToInt32(this.txtAgeThree.Text.Trim());

            DateTime birthDay = new DateTime();
            if (this.ckHour.Checked == false)
            {
        ...
        private int processIDENNO(string idNO, EnumCheckIDNOType enumType)
        {
            ...
            //    return 1;
            //}
            if (!FS.FrameWork.Public.String.IsNumeric(idNO.Trim().Substring(0, 1)))
            {
                //香港、台湾直接不验证
                return 1;
            }

            if (idNO == "-" || string.IsNullOrEmpty(idNO)) //特殊符号处理
            {
                return 1;
            }
            ...
            {
                this.dtpBirthDay.Text = reurnString[1] + " 00:00:00";
            ...
            {
                string[] temp = this.dtpBirthDay.Text.Split(' ');
                if (temp[0] != reurnString[1])
                {
            ...
            if (enumType == EnumCheckIDNOType.BeforeSave)
            {
                ...
                ArrayList list = commonController.QueryConstant(EnumConstant.DIST);
                string tempstr = idNOTmp.Substring(0, 6);
                string temps = idNOTmp.Substring(0, 2);
                cmbBirthArea.Tag = temps;

                foreach (FS.HISFC.Models.Base.Const tcom in list)
                {
                    if (tempstr == tcom.ID)
                    {
                        txtBirthArea.Text = tcom.Name;
                        cmbBirthArea.Tag = tcom;
                    }
                }
            }
            return 1;
        ...
        private int queryPatientInfo()
        {
            ...
            else
            {
                ...
                patientInfo.IDCard = this.txtIDNO.Text;
                patientInfo.PhoneHome = this.txtHomePhone.Text;
                patientInfo.Birthday = this.dtpBirthDay.Value.Date;
                //patientInfo.Sex.ID = this.cmbSex.Tag.ToString();
                //patientInfo.Sex.Name = this.cmbSex.Text;

                patientInfo.PID.CardNO = this.txtClinicNO.Text;
        ...
        private void setEnable(bool eable)
        {
            ...
            else
            {
                ...
                //this.cmbPact.EnterVisiable = eable;
                this.cmbPact.Enabled = eable;
                this.cmbPatientType.Enabled = eable;
                //this.cmbPayType.EnterVisiable = eable;
                this.cmbPayType.Enabled = eable;
            }
            ...
            //this.txtName.Enabled = this.IsCanModifyIName;
            //this.mtxtBloodFee.Enabled = eable;
            this.mTxtPrepay.Enabled = eable;
            this.rdoInpatientNO.Enabled = eable;
            this.rbtnTempPatientNO.Enabled = this.isCanModifyPatientNo;// {5B3B503C-8CF5-415b-89EB-C11A4FEE8A19}
            this.txtInpatientNO.Enabled = this.isCanModifyPatientNo;
            this.txtHomeZip.Enabled = eable;
            cbxChild.Enabled = eable;
        }
                ...
                else if (this.cmbMarry.Focused)
                {
                    switch (cmbMarry.Text)
                    {
                        ...
                        case "4": cmbMarry.Tag = "W"; //丧偶
                            break;
                        case "yh": cmbMarry.Tag = "M"; //已婚
                            break;
                        case "wh": cmbMarry.Tag = "S"; //未婚
                            break;
                        case "sh": cmbMarry.Tag = "D"; //失婚
                            break;
                        case "so": cmbMarry.Tag = "W"; //丧偶
                            break;
                        default:
                            break;
                    }
        ...
        private void cmbDept_TextChanged(object sender, EventArgs e)
        {
            ...
            this.SpecialDept(deptObj.Name, this.cmbSex.Text);
            ...
            if (al != null && al.Count > 0)
            {
                ...
                FS.FrameWork.Models.NeuObject nurseCell = al[0] as FS.FrameWork.Models.NeuObject;
                //{6F4FF33E-1EF3-4f95-9F65-DBACF15DF8A1}
                if (al.Count == 1 && nurseCell != null)
                {
                    this.cmbNurseCell.Tag = nurseCell.ID;
                }

            }
            //{83AE8811-20CC-4916-9132-DF101AED9DAD} 无护士站的科室置为空防止出现匹配错误 2014-12-16 by lixuelong
            else
            {
        ...
        private void dtpBirthDay_TextChanged(object sender, EventArgs e)
        {
            ...
            if (this.ckHour.Checked)
            {
                DateTime dtNow = commonController.GetSystemTime();
                birthday = this.dtpBirthDay.Value; //FS.FrameWork.Function.NConvert.ToDateTime(dtNow.Date.ToString("yyyy-MM-dd") + " " + this.dtpBirthDay.Text);
                iDay = (dtNow - birthday).Milliseconds;
                iMonth = (dtNow - birthday).Minutes;
                iyear = (dtNow - birthday).Hours;
                this.getAge(iyear, iMonth, iDay);
            }
            else
            {
                birthday = FS.FrameWork.Function.NConvert.ToDateTime(this.dtpBirthDay.Text);
        ...
        private void txtClinicNO_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyData == Keys.Enter)
            {
                ...
                DateTime now = feeInpatient.GetDateTimeFromSysDateTime();
                ArrayList arrPrein = this.managerIntegrate.QueryPreInPatientInfoByDateAndState("0", now.AddDays(-1).ToShortDateString() + " 00:00:00", now.ToShortDateString() + " 23:59:59");
                bool foundFlag = false;
                foreach (FS.HISFC.Models.RADT.PatientInfo obj in arrPrein)
                {
                    if (obj.PID.CardNO.Equals(Function.GetCardNO(clinicNO)))
                    {
                        patientInfo = this.managerIntegrate.QueryPreInPatientInfoByCardNO(obj.User01, obj.PID.CardNO);
                        //patientInfo.Memo = "预约患者";
                        foundFlag = true;
                        break;
                    }
                }
                ...
                if (foundFlag == false)
                {
                    //DialogResult dr = MessageBox.Show("没有找到该门诊号对应的当天预约记录\n是否继续？", "警告", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                    //if (dr == DialogResult.Yes)
                    //{
                    //    //根据查询出来的患者信息
                    //    patient = Function.GetPatient(Function.GetCardNO(clinicNO));
                    //}
                    //else
                    //{
                    //    return;
                ...
                if (patient == null && patientInfo == null)
                {
                    this.myMessageBox("获得患者基本信息出错!" + Function.Err, MessageBoxIcon.Warning);
                    return;
                }

                if (patient != null && patient.Memo == "作废患者")
                {
                    this.myMessageBox("此患者已作废" + Function.Err, MessageBoxIcon.Warning);
                    return;
                }

                else if ((patient != null && string.IsNullOrEmpty(patient.PID.CardNO)) || (patientInfo != null && string.IsNullOrEmpty(patientInfo.PID.CardNO)))
                {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void rbtnTempPatientNO_CheckedChanged(object sender, EventArgs e)
        {
            if (this.rbtnTempPatientNO.Checked)
            {
                //string tempPatientNO=string.Empty;
                //if (Function.GetAutoTempPatientNO(ref tempPatientNO) > 0)
                if (this.isFertilityPatientNO)//是否生殖中心登记号
                {
                    if (Function.GetNewFertilityPatientNo(ref fertilityPatientNO) > 0)
                    {
                        this.txtInpatientNO.Text = fertilityPatientNO;
                    ...
                    {
                        this.myMessageBox("请手动输入生殖住院号，已字母S开头，" + Function.Err, MessageBoxIcon.Warning);
                ...
                {
                    if (string.IsNullOrEmpty(this.cmbPatientType.Tag.ToString()))// {F6204EF5-F295-4d91-B81A-736A268DD394}
                    {
                        this.myMessageBox("请选择患者类型！", MessageBoxIcon.Warning);
                        return;

                    }
                    if (Function.GetAutoTempPatientNOByType(this.cmbPatientType.Tag.ToString(), ref auoPatientNOTemp) > 0)
                    {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void ckHour_CheckedChanged(object sender, EventArgs e)
        {
            this.getAge(0, 0, 0);
            if (ckHour.Checked)
            {
                txtAgeThree.Visible = false;
                lbAgeThree.Visible = false;
                //this.dtpBirthDay.Mask = "90:00:00";
                dtpBirthDay.CustomFormat = "yyyy-MM-dd HH:mm";
                this.lbAgeOne.Text = "时";
                this.lbAgeTwo.Text = "分";
                this.lbAgeThree.Text = "秒";
                this.dtpBirthDay.TextChanged -= new EventHandler(dtpBirthDay_TextChanged);
                this.dtpBirthDay.Leave -= new EventHandler(this.dtpBirthDay_Leave);
                this.dtpBirthDay.Leave += new EventHandler(this.dtpBirthDay_Leave);
                this.dtpBirthDay.Text = commonController.GetSystemTime().ToString("yyyy-MM-dd HH:mm:ss");
                this.dtpBirthDay_Leave(null, null);
                this.dtpBirthDay.Focus();
                //this.dtpBirthDay.SelectAll();
            }
            else
            {
                //{BD95226A-1653-467c-BE5D-2AEB8E8EE1F5}
                //成年人出生日期位yyyy-MM-dd HH:mm:ss
                //婴儿出生日期录入时不需要填秒
                txtAgeThree.Visible = true;
                lbAgeThree.Visible = true;
                ...
                this.dtpBirthDay.TextChanged += new EventHandler(dtpBirthDay_TextChanged);
                this.dtpBirthDay.Leave -= new EventHandler(this.dtpBirthDay_Leave);
                this.dtpBirthDay.Text = commonController.GetSystemTime().ToString("yyyy-MM-dd");
                this.dtpBirthDay.Focus();
                //this.dtpBirthDay.SelectAll();
            }
        }
        ...
        private void dtpBirthDay_Leave(object sender, EventArgs e)
        {
            ...
            {
                DateTime dtNow = commonController.GetSystemTime();
                birthday = dtpBirthDay.Value;// FS.FrameWork.Function.NConvert.ToDateTime(dtNow.Date.ToString("yyyy-MM-dd") + " " + this.dtpBirthDay.Text);
                iDay = (dtNow - birthday).Seconds;
                iMonth = (dtNow - birthday).Minutes;
                iyear = (dtNow - birthday).Hours;
            }
            else
        ...
        public int Init()
        {
            ...
            if (!this.isInputDiagnose)// {D59C1D74-CE65-424a-9CB3-7F9174664504}
            {
                this.txtDiagnose.IsTextInput = true;
                this.lblDiagnose.ForeColor = Color.Blue;
            }
            else
            {
                this.txtDiagnose.IsTextInput = false;
                this.lblDiagnose.ForeColor = Color.Black;
            }


            if (this.isInputLinkMan)// {6BF1F99D-7307-4d05-B747-274D24174895}
            {

                this.txtLinkMan.IsTextInput = true;
                this.lblLinkMan.ForeColor = Color.Blue;
            }
            else
            {
                this.txtLinkMan.IsTextInput = false;
                this.lblLinkMan.ForeColor = Color.Black;
            }
            this.cmbInSource.Tag = 2;
            this.cmbCircs.Tag = 1;
            this.cmbApproach.Tag = 1;
            ...
            if (isFertilityPatientNO)
            {
                this.rbtnTempPatientNO.Text = "生殖号(F2)";
                SendKeys.Send("{F2}");
            }
            return 1;
        }
        ...
        public void Clear()
        {
            ...
            this.cmbPatientType.Tag = "P";// {F6204EF5-F295-4d91-B81A-736A268DD394}
            this.cmbPatientType.Enabled = true;// {F6204EF5-F295-4d91-B81A-736A268DD394}
            ...
            if (this.ckHour.Checked == false)
            {
                this.ckHour_CheckedChanged(null, null);
            }
            else
            {
                this.ckHour.Checked = false;
            }
        ...
        public bool IsCanModifyIName
        {
            ...
            set
            {
                this.isCanModifyName = value;
            }
        }

        public bool IsCanModifyPatientNo// {1470D480-E0E3-4ab7-AECE-2B75E9101D4E}
        {
            get
            {
                return this.isCanModifyPatientNo;
            }
            set
            {
                this.isCanModifyPatientNo = value;
            }
        }
        ...
        public bool IsShowPrePay
        {
            get
            {
                return this.isShowPrePay;
            }
            set
            {
                this.isShowPrePay = value;
            }
        }
        ...
        {
            this.setPatientInfo(patientInfo, isAll);
            this.cmbInSource.SelectAll();
        ...
        public bool IsFertilityPatientNO
        {
            get
            {
                return this.isFertilityPatientNO;
            }
            set
            {
                this.isFertilityPatientNO = value;
            }
        }
        ...
        private void cmbPact_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (this.cmbPact.SelectedItem.ID == "PYYB")
            {
                ...
                FS.HISFC.Object.RADT.PatientInfo tempInfo = new FS.HISFC.Object.RADT.PatientInfo();
            ...
            //HZYB
            FS.neuFC.Object.neuObject obj = this.Constant.Get("HZYB", this.cmbPact.SelectedItem.ID);
            if (obj != null && obj.ID != "")
            {
            ...
            else
            {
                ...
                switch (p.PayKind.ID)
                {
                    case "01":
                        strPayKindName = "自费";
                        this.txtWorkAddress.AddItems(Constant.GetList(FS.HISFC.Object.Base.enuConstant.AREA));
                        break;
                    case "02":
                        strPayKindName = "保险";
                        break;
                    case "03":
                        strPayKindName = "公费在职";
                        this.txtWorkAddress.AddItems(Constant.GetList(FS.HISFC.Object.Base.enuConstant.SGYDW));
                        break;
                    case "04":
                        strPayKindName = "公费退休";
                        break;
        ...
        {
            this.SpecialDept(this.cmbDept.Text, this.cmbSex.Text);
        }

        private void SpecialDept(string deptName, string sexName)
        {
            #region 男性患者就诊产科、妇科提示
            bool isCK = deptName.Contains("妇") || deptName.Contains("产");
            if (isCK && sexName.Trim().Equals("男"))
            {
                string tip = "录入信息出现，{0}性别患者到{1}就诊！\n请确认录入数据与具体情况是否属实吻合！";
                tip = string.Format(tip, sexName, deptName);
                MessageBox.Show(tip, "警告");
            }
            #endregion
        }
        ...
        {
            this.rdoInpatientNO.Checked = !this.rdoInpatientNO.Checked;
            foreach (FS.HISFC.Models.Base.Const obj in this.cmbPatientType.alItems)
            {

                if (obj.ID == this.cmbPatientType.Tag.ToString())
                {
                    if (obj.UserCode == "L")
                    {
                        this.rbtnTempPatientNO.Checked = true;
                    ...
                    else
                    {
                        ...
                        this.txtInpatientNO.Text = this.patientNO;
                    }
