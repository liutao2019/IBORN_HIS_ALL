...
namespace FS.SOC.Local.RADT.ZhuHai.ZDWY.Controls
{
    ...
    /// 中大五院病房工作日志
    /// </summary>
    public partial class ucShiftRecordForNurse : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        public ucShiftRecordForNurse()
        {
            InitializeComponent();
...
where (g.nurse_cell_code='{2}' or '{2}'='ALL')
and g.clinic_no !='N'
...
select nvl(sum(入院人数),0) 入院人数 from (
select (case when s.shift_type='K' then count((s.clinic_no)) 
             when s.shift_type='C' then count((s.clinic_no)) else 0 end) 入院人数
from com_shiftdata s,fin_ipr_inmaininfo i
where s.clinic_no=i.inpatient_no
and s.shift_type in ('K'/*接诊*/,'C'/*召回*/)
and (s.old_data_code='{2}' or '{2}'='ALL')
...
and (s.new_data_code='{2}' or '{2}'='ALL')
    and s.shift_cause not in ('修改科室','修改病区')
and i.patient_no not like 'B%'
           and not exists(select 1 from com_shiftdata a
           where a.clinic_no=s.clinic_no
...
select distinct --shifttype,
  ...
  from (select (case
               ...
               (case s.shift_type
                ...
                (case s.shift_type
                 when 'O' then  i.zg end) 情况,                   
                  (case s.shift_type   when 'CN' then old_data_code    end) 转入前原病区, 
                  (case s.shift_type   when 'CNO' then new_data_code    end) 转往病区,
               s.shift_type shifttype,           
               s.oper_date
          from com_shiftdata s, fin_ipr_inmaininfo i
         where s.clinic_no = i.inpatient_no
           and ((s.shift_type in ('K' /*接诊*/, 'CNO' /*转出*/) and
               (s.old_data_code = '{2}' or '{2}'='ALL')) or
               (s.shift_type in ('O' /*出院等级*/, 'CN' /*转入*/) and
               (s.new_data_code = '{2}' or '{2}'='ALL')) or
               (s.shift_type = 'C' /*召回*/
               and (s.old_data_code = '{2}' or '{2}'='ALL')
               ))
           and s.shift_cause not in ('修改科室', '修改病区')
...
";
        ...
        string sql_现有人数 = @"select sum(人数)
                        from
                        (select case
                                 when to_date('{0}', 'yyyy-mm-dd hh24:mi:ss') =
                                      trunc(sysdate) then
                                  sum(decode(f.clinic_no, 'N', 0, 1)) 
                                 else
                                  (select sum(decode(g.clinic_no, 'N', 0, 1))
                                     from com_bedinfo_log g
                                    where f.nurse_cell_code = g.nurse_cell_code
                                      and g.bed_state != 'W'
                                      and g.log_date >=
                          ...
                          from com_bedinfo f
                         where (f.nurse_cell_code = '{2}' or '{2}' = 'ALL')
                         and f.bed_state != 'W'
                         group by f.nurse_cell_code)";
        ...
        string sql_陪人数 = @"select nvl(count(*),0)
         ...
         where r.item_name like '%陪人%'
           ...
           and a.shift_type='OF')
           and (r.nurse_cell_code='{2}' or '{2}'='ALL')
           and r.date_bgn >= to_date('{0}','yyyy-mm-dd hh24:mi:ss')
           and r.date_bgn < to_date('{1}','yyyy-mm-dd hh24:mi:ss')+1";
...
select nvl(sum(入院人数), 0) 入院人数
  from (select (case
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            ArrayList alDept = deptMgr.GetDeptment(FS.HISFC.Models.Base.EnumDepartmentType.N);
        ...
        protected override int OnQuery(object sender, object neuObject)
        {
            ...
            string fromDate = dateTimePicker1.Value.Date.ToString();
            string endDate = dateTimePicker2.Value.Date.ToString();
            if (!isShowDateBetween)
            {
                endDate = dateTimePicker1.Value.Date.AddDays(1).ToString();
            }
