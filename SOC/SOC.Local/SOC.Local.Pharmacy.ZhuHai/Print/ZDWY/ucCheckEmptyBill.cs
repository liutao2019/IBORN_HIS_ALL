...
namespace FS.SOC.Local.Pharmacy.ZhuHai.Print.ZDWY
{
    public partial class ucCheckEmptyBill : UserControl, Base.IPharmacyBillPrint
    {
        ...
        private void SetPrintData(ArrayList al, string title)
        {
            ...
            SOC.HISFC.BizLogic.Pharmacy.Storage stoMgr = new FS.SOC.HISFC.BizLogic.Pharmacy.Storage();

             //alValidDates = stoMgr.QueryValidDates(info.StockDept.ID);
            ...
            else
            {
                ...
                {
                    title = this.lblTitle.Text.Replace("[院区]", deptInfo.HospitalName);
                }
            ...
            this.lblFOperTime.Text = "封账时间:" + info.FOper.OperTime.ToString();
            ...
            for (int i = 0; i < al.Count; i++)
            {
                ...
                //this.neuFpEnter1.SetCellValue(0, i, "有效期", this.GetValidDate(checkDetail.Item.ID));
                this.neuFpEnter1.SetCellValue(0, i, "有效期", " ");
                

                decimal packQty = Math.Floor(checkDetail.FStoreQty/checkDetail.Item.PackQty);
                decimal minQty = checkDetail.FStoreQty - packQty * checkDetail.Item.PackQty;
                string fStoreQty = string.Empty;

                if (checkDetail.FStoreQty==0)
                {
                    fStoreQty = "0" + checkDetail.Item.PackUnit;
                }
                if (packQty > 0)
                {
                    fStoreQty = fStoreQty + packQty.ToString() + checkDetail.Item.PackUnit;
                }
                if (minQty > 0)
                {
                    fStoreQty = fStoreQty + minQty.ToString() + checkDetail.Item.MinUnit;
                }

                this.neuFpEnter1.SetCellValue(0, i, "封账库存", fStoreQty);
        ...
        public int SetPrintData(System.Collections.ArrayList alPrintData, FS.SOC.Local.Pharmacy.ZhuHai.Base.PrintBill printBill)
        {
            if (alPrintData == null)
            {
            ...
            foreach (FS.HISFC.Models.Pharmacy.Check checkInfo in alPrintData)
            {
                if (SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(checkInfo.Item.ID).ValidState == FS.HISFC.Models.Base.EnumValidState.Invalid && checkInfo.FStoreQty == 0)
                {
                    continue;
                }
                this.alPrintData.Add(checkInfo);
            }
        ...
        private string GetValidDate(string drugCode)
        {
            string validDate = string.Empty;
            for (int i = 0; i < alValidDates.Count; i++)
            {
                FS.HISFC.Models.Pharmacy.Storage info = alValidDates[i] as FS.HISFC.Models.Pharmacy.Storage;
                if (info.Item.ID==drugCode)
                {
                    validDate = info.ValidTime.ToShortDateString();
                    break;
                }
            }
            return validDate;
