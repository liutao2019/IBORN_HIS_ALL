...
namespace FS.SOC.Local.Pharmacy.ZhuHai.Print.ZDWY.GJ
{
    ...
    /// 东莞药品入库计划打印
    /// </summary>
    public partial class ucPlanBillIBORN : ucBaseControl, Base.IPharmacyBillPrint
    {
        /// <summary>
        /// 药品入库打印单
        /// </summary>
        public ucPlanBillIBORN()
        {
            InitializeComponent();
        ...
        private void PrintDocumentPrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            if (curPageNO == maxPageNO)
            {
                ...
                additionTitleLocalX = this.DrawingMargins.Left + this.lblCheckOper.Location.X;
                additionTitleLocalY = this.DrawingMargins.Top + this.lblCheckOper.Location.Y + this.neuPanel4.Height + FarpointHeight;
                graphics.DrawString(this.lblCheckOper.Text, this.lblCheckOper.Font, new SolidBrush(this.lblCheckOper.ForeColor), additionTitleLocalX, additionTitleLocalY);
        ...
        private void SetPrintData(ArrayList al, int inow, int icount, string title)
        {
            ...
            FS.HISFC.Models.Base.Department deptInfo = FS.SOC.HISFC.BizProcess.Cache.Common.GetDept(plan.Dept.ID);
            ...
            this.neuFpEnter1_Sheet1.Columns[5].CellType = n1;
            ...
            for (int i = 0; i < al.Count; i++)
            {
                ...
                if (departMent.DeptType.ID.ToString() == "PI")
                {
                    ...
                    this.neuFpEnter1_Sheet1.Cells[i, 5].Value = (info.PlanQty / info.Item.PackQty).ToString("F3").TrimEnd('0').TrimEnd('.');

                    this.neuFpEnter1_Sheet1.Cells[i, 6].Text = info.Item.PackUnit;//单位
                }
                else
                {
                    this.neuFpEnter1_Sheet1.Cells[i, 5].Value = info.PlanQty;
                    this.neuFpEnter1_Sheet1.Cells[i, 6].Text = info.Item.MinUnit;
                }
                this.neuFpEnter1_Sheet1.Cells[i, 8].Value = (info.PlanQty / info.Item.PackQty * info.Item.PriceCollection.PurchasePrice).ToString("F2");//预购金额

                this.neuFpEnter1_Sheet1.Cells[i, 9].Text = SOC.HISFC.BizProcess.Cache.Pharmacy.GetProducerName(info.Item.Product.Producer.ID);
