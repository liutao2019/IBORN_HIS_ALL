...
namespace FS.SOC.Local.Pharmacy.ZhuHai.Print.ZDWY.GJ
{
    ...
    /// 东莞药品出库单据打印
    /// </summary>
    public partial class ucPhaBackInBillIBORN: ucBaseControl, Base.IPharmacyBillPrint
    {
        /// <summary>
        /// 药品入库打印单
        /// </summary>
        public ucPhaBackInBillIBORN()
        {
            InitializeComponent();
        ...
        private void SetPrintData(ArrayList al, int inow, int icount, string title)
        {
            ...
            this.lblInputTime.Text = "退货日期：" + info.InDate.ToString();//+ info.InDate.Year + "." + info.InDate.Month.ToString().PadLeft(2, '0') + "."+ info.InDate.Day.ToString().PadLeft(2,'0');
            ...
            this.lblPrintDate.Text = "制单日期" + DateTime.Now;
            this.lblTotPurCost.Text = "总合计：购入总金额：" + ((TotCost)hsTotCost[info.InListNO]).purchaseCost.ToString("F2");
            this.lblTotRetail.Text = "零售金额总计：" + ((TotCost)hsTotCost[info.InListNO]).retailCost.ToString(Function.GetCostDecimalString());

            this.lblTotDiff.Text = "进销差总计：" + (((TotCost)hsTotCost[info.InListNO]).retailCost -
            ...
            for (int i = 0; i < al.Count; i++)
            {
                ...
                this.lblStoreOper.Text = "仓管员：" + FS.SOC.HISFC.BizProcess.Cache.Common.GetEmployeeName(input.User02);

                this.lblBuyer.Text = "采购员：" + FS.SOC.HISFC.BizProcess.Cache.Common.GetEmployeeName(input.User03);
                ...
                this.neuFpEnter1_Sheet1.Cells[i, 7].Text = input.PurchaseCost.ToString("F2");
                this.neuFpEnter1_Sheet1.Cells[i, 8].Text = input.InvoiceNO;
            ...
            this.lblCurPurCost.Text = "本页合计：购入金额：" + sumPurchase.ToString("F2");
            this.lblCurRetailCost.Text = "本页零售金额：" + sumRetail;
        ...
        private void PrintDocumentPrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            additionTitleLocalY = this.DrawingMargins.Top + this.lblPrintDate.Location.Y + this.neuPanel4.Height + FarpointHeight;
            ...
            graphics.DrawString("本页合计：购入金额：" + totCostByPage.purchaseCost.ToString("F2"), this.lblCurPurCost.Font, new SolidBrush(this.lblCurPurCost.ForeColor), additionTitleLocalX, additionTitleLocalY);
            ...
            //graphics.DrawString("本页购零差：" + (totCostByPage.retailCost - totCostByPage.purchaseCost).ToString(Function.GetCostDecimalString()), this.lblCurDif.Font, new SolidBrush(this.lblCurDif.ForeColor), additionTitleLocalX, additionTitleLocalY);
            #endregion
