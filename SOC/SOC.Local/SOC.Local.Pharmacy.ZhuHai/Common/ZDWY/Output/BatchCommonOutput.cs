...
{
    /// <summary>
    /// [功能描述: 药库按批一般出库：出库时指定某个批次]<br></br>
    /// [创 建 者: zhaozf]<br></br>
    /// [创建时间: 2011-6-16]<br></br>
    /// 说明：
    /// 1、按批一般出库直接增加对方科室库存
    /// 2、按批一般出库不允许向不管理库存的科室操作    
    /// </summary>
    public class BatchCommonOutput : FS.SOC.HISFC.Components.Pharmacy.Base.IBaseBiz
    {
        ...
        private bool isDirectInput = true;
        ...
        private FS.HISFC.BizProcess.Integrate.Common.ControlParam controlMgr = new FS.HISFC.BizProcess.Integrate.Common.ControlParam();
        ...
        public int Init(FS.SOC.HISFC.BizProcess.PharmacyInterface.Pharmacy.IDataChooseList IDataChooseList, SOC.HISFC.BizProcess.PharmacyInterface.Pharmacy.IDataDetail IDataDetail, ToolStrip toolStrip)
        {
            ...
            this.settingFileName = FS.FrameWork.WinForms.Classes.Function.CurrentPath + "\\Profile\\BatchOutputSetting.xml";

            if (this.curPriveType == null)
            {
                this.curPriveType = new FS.FrameWork.Models.NeuObject();
                curPriveType.ID = "10";
                curPriveType.Name = " 按批一般出库";
                curPriveType.Memo = "32";
            }
        ...
        protected int InitChooseData()
        {
            ...
            if (chooseDataSetting.IsDefault)
            {
                ...
                //五笔码regular_wb 已用做有效期，因此这里不再使用regular_wb进行过滤，
                //否则会造成过滤错误，日期与字符串不能进行直接比较
        ...
        protected int InitFarPoint()
        {
            ...
            //    {D49074A9-8311-43ce-AAC0-80108E65D204}
                ...
                //{2FFD16A6-7281-4d71-817A-E3CD6B12C43E}
        ...
        protected int InitDataTable()
        {
            ...
            {
                if (dc.ColumnName == "出库量" || dc.ColumnName == "零售金额"||dc.ColumnName=="购入金额"||dc.ColumnName == "加成金额")
                {
                    continue;
                }
                dc.ReadOnly = true;
        ...
        /// </summary>
        /// <returns></returns>
        private int InitControlCode()
        {
            isDirectInput = this.controlMgr.GetControlParam("P01018", false, true);
            return 1;
        }
        ...
        protected int AddOutputObjectToDataTable(FS.HISFC.Models.Pharmacy.Output output)
        {
            ...
            if (this.itemMgr.GetStorageNum(this.curStockDept.ID, output.Item.ID,output.GroupNO, out storageNum) == -1)
            {
        ...
        protected int Delete()
        {
            ...
            //string groupNO = this.curIDataDetail.FpSpread.Sheets[0].Cells[rowIndex, this.curIDataDetail.FpSpread.GetColumnIndex(0, "批次")].Text;
        ...
        void IDataChooseList_ChooseCompletedEvent()
        {
            ...
            else
            {
                ...
                ArrayList alDetail = this.itemMgr.QueryStorageList(this.curStockDept.ID, values[0], NConvert.ToDecimal(values[1]));
                ...
                output.Item.PriceCollection = storage.Item.PriceCollection;
                output.Item.PackQty = storage.Item.PackQty;
                output.ValidTime = storage.ValidTime;
                output.ShowState = "1";
