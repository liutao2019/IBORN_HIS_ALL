...
{
    /// <summary>
    /// [功能描述: 特殊出库：报损或者科室领药]<br></br>
    /// [创 建 者: cube]<br></br>
    /// [创建时间: 2011-4]<br></br>
    /// </summary>
    public class SpecialOutput : FS.SOC.HISFC.Components.Pharmacy.Base.IBaseBiz
    {
        ...
        public int Init(FS.SOC.HISFC.BizProcess.PharmacyInterface.Pharmacy.IDataChooseList IDataChooseList, SOC.HISFC.BizProcess.PharmacyInterface.Pharmacy.IDataDetail IDataDetail, ToolStrip toolStrip)
        {
            ...
            if (this.PriveType.ID == "12")
            {
                this.settingFileName = FS.FrameWork.WinForms.Classes.Function.CurrentPath + "\\Profile\\SpecialOutputSettingKSLY.xml";
            }
            else
            {
                this.settingFileName = FS.FrameWork.WinForms.Classes.Function.CurrentPath + "\\Profile\\SpecialOutputSetting.xml";
            }
            ...
            if (this.curPriveType == null)
            {
                ...
                curPriveType.ID = "09";
                curPriveType.Name = "特殊出库";
                curPriveType.Memo = "26";
            }
        ...
        public ArrayList GetTargetDeptArray()
        {
            ...
            //报损
            if (this.curPriveType.ID == "05" || this.curPriveType.ID == "09" || this.curPriveType.ID == "57"|| this.curPriveType.ID == "08")
            {
                alOutDept.Add(this.curStockDept);

            }
            else if (System.IO.File.Exists(this.settingFromDeptFileName) && (FS.FrameWork.Function.NConvert.ToBoolean(SOC.Public.XML.SettingFile.ReadSetting(this.settingFromDeptFileName, "PrivType", "SpecialOutputType" + this.PriveType.ID, "0"))))
            {
                    alOutDept.Add(this.curStockDept);
            }
            else
            {
                alOutDept = SOC.HISFC.Components.Pharmacy.Function.QueryCommonDept();
            }
            if (alOutDept == null)
            {
        ...
        private int AutoFillSummary()
        {
            ...
            string summary = this.curIDataDetail.FpSpread.GetCellText(0, rowIndex, "摘要");
            ...
            else
            {
                ...
                {
                    this.curIDataDetail.FpSpread.SetCellValue(0, i, "摘要", summary);
                }
            }
        ...
        protected int InitChooseData()
        {
            ...
            if (chooseDataSetting.IsDefault)
            {
                ...
                //{72708811-6675-467c-8B60-CBB42631CC0D}
                //用退库的好了
        ...
        protected int InitFarPoint()
        {
            ...
            if (!System.IO.File.Exists(this.settingFileName))
            {
                ...
                this.curIDataDetail.FpSpread.SetColumnWith(0, "国家医保代码", 100f);
                ...
                //this.curIDataDetail.FpSpread.SetColumnWith(0, "零售金额", nPrice);
                //this.curIDataDetail.FpSpread.SetColumnWith(0, "零售价", 60f);
                this.curIDataDetail.FpSpread.SetColumnWith(0, "有效期", 60f);
                this.curIDataDetail.FpSpread.SetColumnWith(0, "摘要", 60f);
                ...
                FarPoint.Win.Spread.CellType.NumberCellType tWrite = new FarPoint.Win.Spread.CellType.NumberCellType();
                tWrite.DecimalPlaces = 3;
                ...
                //{A6334C92-C1B9-4d50-93A1-2C77DD4395EA}
                if (this.PriveType.ID == "12")  //科室领药显示零售金额
                {
                ...
                this.curIDataDetail.FpSpread.SetColumnCellType(0, "摘要", t);
        ...
        protected int InitDataTable()
        {
            ...
            if (this.PriveType.ID == "12")  //科室领药显示零售金额
            {
                this.dtDetail.Columns.AddRange
                    ...
                    new System.Data.DataColumn[]
                    {
                        ...
                        new DataColumn("摘要",typeof(string)),
            ...
            foreach (DataColumn dc in this.dtDetail.Columns)
            {
                //{A6334C92-C1B9-4d50-93A1-2C77DD4395EA}
                //readonly属性，血一般的教训
                if (dc.ColumnName == "出库量" || dc.ColumnName == "零售金额" || dc.ColumnName == "购入金额" || dc.ColumnName == "备注" || dc.ColumnName == "摘要")
                {
                    continue;
                }
                dc.ReadOnly = true;
        ...
        protected int AddOutputObjectToDataTable(FS.HISFC.Models.Pharmacy.Output output)
        {
            ...
            if (this.hsOutput.Contains(output.Item.ID + output.GroupNO))
            {
            ...
            {
                hsOutput.Add(output.Item.ID + output.GroupNO, output);
            }
            ...
            //药品报损出库单，因为以包装单位为单位，数量取了两位小数之后，金额会有差异；打印的单据与系统上的数量也会有差异。
            output.ShowState = "0";
            output.ShowUnit = output.Item.MinUnit;
            if (output.ShowState == "1")
            {
                row["库存量"] = output.Quantity / output.Item.PackQty;//storageNum / output.Item.PackQty;
                if ((output.Quantity / output.Item.PackQty) < output.Quantity)
                {
                    row["出库量"] = row["库存量"];
                }
                else
            ...
            {
                row["库存量"] = output.Quantity;// storageNum;
                if ( output.Quantity < output.Quantity)
                {
                    row["出库量"] = row["库存量"];
                }
                else
            ...
            if (this.PriveType.ID == "12")  //科室领药显示零售金额
            {
                row["零售价"] = output.Item.PriceCollection.RetailPrice / output.Item.PackQty;
                row["零售金额"] = output.RetailCost / output.Item.PackQty;
            }
            else
            {
                row["购入价"] = output.Item.PriceCollection.PurchasePrice / output.Item.PackQty;
                row["购入金额"] = output.PurchaseCost / output.Item.PackQty;
            }
        ...
        public int CheckValid()
        {
            ...
            //else
            //{
                //1.为什么"09 "要空格？？？2.流程中09也不需要目标科室是本科室
                if ((this.curPriveType.ID == "05" || this.curPriveType.ID == "09 ") && this.curTargetDept.ID != this.curStockDept.ID)
                {
          ...
          //  }
            foreach (DataRow dr in this.dtDetail.Rows)
            {
                ...
                if (NConvert.ToDecimal(dr["库存量"]) < NConvert.ToDecimal(dr["出库量"]))
                {
                ...
                //    return -1;
                //}
                if (this.curPriveType.ID != "12")
                {
                    if (string.IsNullOrEmpty(dr["备注"].ToString().Trim()))
                    {
                        Function.ShowMessage(dr["名称"].ToString() + "备注未填写，请录入！", MessageBoxIcon.Information);
                        return -1;
                    }
                }
            }
            ...
            for (int rowIndex = 0; rowIndex < this.curIDataDetail.FpSpread.Sheets[0].Rows.Count; rowIndex++)
            {
                ...
                if (hsOutput.Contains(keys))
                {
                    ...
                    else
                    {
                        ...
                        purchaseCost = output.Item.PriceCollection.PurchasePrice * (qty / output.Item.PackQty);
                        purchaseCost = FS.FrameWork.Function.NConvert.ToDecimal(purchaseCost.ToString("F" + this.costDecimals.ToString()));
                        wholeSaleCost = output.Item.PriceCollection.WholeSalePrice * (qty / output.Item.PackQty);
                        wholeSaleCost = FS.FrameWork.Function.NConvert.ToDecimal(wholeSaleCost.ToString("F" + this.costDecimals.ToString()));
                        retailCost = output.Item.PriceCollection.RetailPrice * (qty / output.Item.PackQty);
                        retailCost = FS.FrameWork.Function.NConvert.ToDecimal(retailCost.ToString("F" + this.costDecimals.ToString()));
                    }
        ...
        void IDataChooseList_ChooseCompletedEvent()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (this.hsOutput.Contains(item.ID + groupNO))
                    {
                    ...
                    //{39D8B814-90A8-409b-AEBF-BD178E71B133}
                    ArrayList alDetail = new ArrayList();
                    if (this.curPriveType.ID == "12")
                    {
                        alDetail = this.itemMgr.QueryStorageList(this.curStockDept.ID, drugNO, NConvert.ToDecimal(groupNO));
                    }
                    else
                    {
                        alDetail = this.itemMgr.QueryStorageList(this.curTargetDept.ID, drugNO, NConvert.ToDecimal(groupNO));
                    }
                    ...
                    output.Item.GBCode = item.GBCode;//国家编码
