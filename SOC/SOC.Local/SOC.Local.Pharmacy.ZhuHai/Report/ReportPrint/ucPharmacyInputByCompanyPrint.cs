...
namespace FS.SOC.Local.Pharmacy.ZhuHai.Print.ZDWY.Report.ReportPrint
{
    ...
    /// 东莞药品入库单据打印
    /// </summary>
    public partial class ucPharmacyInputByCompanyPrint : ucBaseControl
    {
        ...
        /// 药品入库打印单
        /// </summary>
        public ucPharmacyInputByCompanyPrint()
        {
            InitializeComponent();
        ...
        private System.Drawing.Printing.Margins DrawingMargins = new System.Drawing.Printing.Margins(30, 0, 100, 96);
        ...
        private void PrintDocumentPrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            int drawingHeight = this.printBill.PageSize.Height - this.DrawingMargins.Top - this.DrawingMargins.Bottom - this.neuPanel4.Height - neuPanel3.Height;
            ...
            {
                FarpointHeight = FS.FrameWork.Function.NConvert.ToInt32((int)this.neuFpEnter1.Sheets[0].Rows.Default.Height * (dt.Rows.Count % printBill.RowCount + 1 + maxPageNO) + this.neuFpEnter1_Sheet1.ColumnHeader.Rows[0].Height);
            }
        ...
        private void SetPrintData(DataTable dt, int inow, int icount, string title,DateTime beginTime,DateTime endTime)
        {
            ...
            #region Farpoint赋值
            decimal sumPurCost = 0;
            decimal sumRetCost = 0;
            decimal sumDif = 0;
            decimal sumCurPurCost = 0;
            decimal sumCurRetCost = 0;
            decimal sumCurDif = 0;
            for (int i = 0; i < this.dt.Rows.Count; i++)
            {
                if (i != 0 && i % (this.printBill.RowCount - 1) == 0)
                {
                      this.neuFpEnter1_Sheet1.Rows.Add(this.neuFpEnter1_Sheet1.Rows.Count, 1);
                      this.neuFpEnter1_Sheet1.Cells[this.neuFpEnter1_Sheet1.Rows.Count - 1, 0].Text = "本页合计：";
                      this.neuFpEnter1_Sheet1.Cells[this.neuFpEnter1_Sheet1.Rows.Count - 1, 1].Text = sumCurPurCost.ToString("F2");
                      this.neuFpEnter1_Sheet1.Cells[this.neuFpEnter1_Sheet1.Rows.Count - 1, 2].Text = sumCurRetCost.ToString("F2");
                      this.neuFpEnter1_Sheet1.Cells[this.neuFpEnter1_Sheet1.Rows.Count - 1, 3].Text = sumCurDif.ToString("F2");
                      sumCurPurCost = 0;
                      sumCurRetCost = 0;
                      sumCurDif = 0;
                }
                ...
                this.neuFpEnter1_Sheet1.Cells[this.neuFpEnter1_Sheet1.Rows.Count - 1, 0].Text = dr["单位名称"].ToString();
                this.neuFpEnter1_Sheet1.Cells[this.neuFpEnter1_Sheet1.Rows.Count - 1, 1].Text = dr["买入金额"].ToString();
                this.neuFpEnter1_Sheet1.Cells[this.neuFpEnter1_Sheet1.Rows.Count - 1, 2].Text = dr["零售金额"].ToString();
                this.neuFpEnter1_Sheet1.Cells[this.neuFpEnter1_Sheet1.Rows.Count - 1, 3].Text = dr["购零差"].ToString();
                sumPurCost += FS.FrameWork.Function.NConvert.ToDecimal(dr["买入金额"].ToString());
                sumRetCost += FS.FrameWork.Function.NConvert.ToDecimal(dr["零售金额"].ToString());
                sumDif += FS.FrameWork.Function.NConvert.ToDecimal(dr["购零差"].ToString());
                sumCurPurCost += FS.FrameWork.Function.NConvert.ToDecimal(dr["买入金额"].ToString());
                sumCurRetCost += FS.FrameWork.Function.NConvert.ToDecimal(dr["零售金额"].ToString());
                sumCurDif += FS.FrameWork.Function.NConvert.ToDecimal(dr["购零差"].ToString());
            }
            ...
            this.neuFpEnter1_Sheet1.Cells[this.neuFpEnter1_Sheet1.Rows.Count - 1, 1].Text = sumPurCost.ToString("F2");
            this.neuFpEnter1_Sheet1.Cells[this.neuFpEnter1_Sheet1.Rows.Count - 1, 2].Text = sumRetCost.ToString("F2");
            this.neuFpEnter1_Sheet1.Cells[this.neuFpEnter1_Sheet1.Rows.Count - 1, 3].Text = sumDif.ToString("F2"); 
            #endregion
            this.nlbOper.Text = "制单人：" + SOC.HISFC.BizProcess.Cache.Common.GetEmployeeName(ajustMgr.Operator.ID);
