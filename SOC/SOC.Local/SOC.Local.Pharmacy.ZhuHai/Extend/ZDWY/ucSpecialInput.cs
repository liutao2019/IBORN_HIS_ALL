...
namespace FS.SOC.Local.Pharmacy.ZhuHai.Extend.ZDWY
{
    ...
    public partial class ucSpecialInput : UserControl, SOC.HISFC.BizProcess.PharmacyInterface.Pharmacy.IInputInfoControl
    {
        ...
        public int Init(string unitShowState)
        {
            ...
            this.costDecimals = FS.SOC.HISFC.Components.Pharmacy.Function.GetCostDecimals("0310", "11");
        ...
        public int SetInputObject(FS.HISFC.Models.Pharmacy.Item item, FS.HISFC.Models.Pharmacy.Storage storage, bool isUseBatchNO, DateTime defaultValidDate)
        {
            ...
            this.ntxtWholeSalePrice.Text = curItem.PriceCollection.WholeSalePrice.ToString();

            if (storage != null && storage.Item != null)
            {
                this.ntxtPurchasePrice.Text = storage.Item.PriceCollection.PurchasePrice.ToString();
                this.ntxtWholeSalePrice.Text = storage.Item.PriceCollection.WholeSalePrice.ToString();
                this.ntxtRetailPrice.Text = storage.Item.PriceCollection.RetailPrice.ToString();
            }

            //{72708811-6675-467c-8B60-CBB42631CC0D}
            if (curItem.Product.User01 == "爱博恩特殊入库")
            {
                this.ntxtBatchNO.Text = curItem.Product.User02;
                this.ntxtValidDate.Text = curItem.Product.User03;
            }
            ...
            this.ncmbProducer.Tag = item.Product.Company.ID;
        ...
        public int SetInputObject(FS.HISFC.Models.Pharmacy.Input input, FS.HISFC.Models.Pharmacy.Item item)
        {
            ...
            this.ntxtWholeSalePrice.Text = wholeSalePrice.ToString("F4").TrimEnd('0').TrimEnd('.');
        ...
        public FS.HISFC.Models.Pharmacy.Input GetInputObject()
        {
            ...
            //{878CA713-5EC6-4d90-802E-B0037013B3AD}
            if (SOC.Public.String.Length(this.ntxtBatchNO.Text) > 50)
            {
                Function.ShowMessage("批号太长，允许范围50个字母（1个汉字按2个字母计算），请修改！", MessageBoxIcon.Information);
                this.ntxtBatchNO.Select();
                this.ntxtBatchNO.Focus();
                return null;
            }
            ...
            {
                Function.ShowMessage("温馨提示：自增加入库类型请录入备注！", MessageBoxIcon.Information);
                this.ntxtMemo.Select();
                this.ntxtMemo.Focus();
                return null;
            }
