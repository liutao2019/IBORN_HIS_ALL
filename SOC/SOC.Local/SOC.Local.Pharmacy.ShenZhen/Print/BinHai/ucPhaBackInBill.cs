...
namespace FS.SOC.Local.Pharmacy.ShenZhen.Print.BinHai
{
    ...
    public partial class ucPhaBackInBill : ucBaseControl, Base.IPharmacyBillPrint
    {
        ...
        private void SetPrintData(ArrayList al, int inow, int icount, string title)
        {
            ...
            this.lblBillID.Text = "单号: " + info.InListNO + " 原入库单号：" + (string.IsNullOrEmpty(info.OutListNO) ? "无" : info.OutListNO);
            ...
            for (int i = 0; i < al.Count; i++)
            {
                ...
                this.neuFpEnter1_Sheet1.Cells[i, 0].Text = SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(input.Item.ID).GBCode; //药品自定义码
                ...
                else
                {
                    ...
                    this.neuFpEnter1_Sheet1.Cells[i, 6].Text = (count / input.Item.PackQty).ToString("F2");//数量
                ...
                this.neuFpEnter1_Sheet1.Cells[i, 10].Text = input.InvoiceNO;//input.PurchaseCost.ToString("F4");
                ...
                //当前页数据
                //this.lblCurRet.Text += sumRetail.ToString();
                //this.lblCurPur.Text += sumPurchase.ToString();
                //this.lblCurDif.Text += sumDif.ToString();
            }
