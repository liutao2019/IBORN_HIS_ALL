...
namespace FS.SOC.Local.Pharmacy.ShenZhen.Report
{
    public partial class ucGeneralQuery : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        {
            toolBarService.AddToolButton("日  期", "设置检索日期", FS.FrameWork.WinForms.Classes.EnumImageList.C查询历史, true, false, null);
            toolBarService.AddToolButton("按部门", "按部门汇总查询", FS.FrameWork.WinForms.Classes.EnumImageList.Z转科, true, false, null);
            toolBarService.AddToolButton("按药品", "按药品汇总查询", FS.FrameWork.WinForms.Classes.EnumImageList.Y药品, true, false, null);
            toolBarService.AddToolButton("按单据", "按单据汇总查询", FS.FrameWork.WinForms.Classes.EnumImageList.B摆药单, true, false, null);
            toolBarService.AddToolButton("配置设置", "设置配置文件", FS.FrameWork.WinForms.Classes.EnumImageList.S设置, true, false, null);
        ...
        private void InitControlParam()
        {
            ...
            this.PParam = ctrlParamIntegrate.GetControlParam<string>(FS.HISFC.BizProcess.Integrate.PharmacyConstant.Query_Commo_Type, false, "入库|In,出库|Out,盘点|Check,调价|Adjust");

            this.PIParam = ctrlParamIntegrate.GetControlParam<string>(FS.HISFC.BizProcess.Integrate.PharmacyConstant.Query_PI_Type, false, "入库计划|InPlan,采购|Stock,台帐|Record");
        }
        ...
        protected void InitPrivOper()
        {
            ...
            FS.HISFC.Models.Admin.PowerLevelClass3 privClass3;             //查询权限
        ...
        private void Filter()
        {
            ...
            switch (this.assortType)
            {
                ...
                case AssortType.Company:    //按供货公式
                    filter = this.GetFilterStr(this.dt.DefaultView, queryCode);
                    break;
        ...
        private int GetDataByConfig(bool isDetail)
        {
            ...
            if (isDetail)
            {
                ...
                if (!this.hsSqlConfig.ContainsKey(index))
                {
                    ...
                    {
                        this.ShowMsg(Language.Msg("未找到" + index + "Sql语句"));
                        return -1;
                    }
                }
                ...
                if (!this.hsSqlConfig.ContainsKey(index))
                {
                    this.ShowMsg("未找到" + index + "对应Sql语句");
                    return -1;
                }
                ...
                {
                    this.ShowMsg(this.reportManager.Err);
                    return -1;
                }
        ...
        protected void ShowDetail()
        {
            try
            {
                FS.FrameWork.WinForms.Classes.Function.ShowWaitForm("正在检索明细信息...请稍候");
        ...
        private int GetSetting()
        {
            ...
            //this.configPath = Application.StartupPath + "\\Report_Setting.xml";
            #endregion

            try
            {
                //doc.Load(serverPath + configPath);
                doc.Load(Application.StartupPath + "\\Report_Setting.xml");
            }
            catch (Exception ex)
            {
        ...
        private int GetFpSetting(bool isDetail)
        {
            ...
            try
            {
                doc.Load(Application.StartupPath + "\\" + configPath);
        ...
        public string GetFilterStr(DataView dv, string queryCode)
        {
            ...
            if (dv.Table.Columns.Contains("拼音码"))
                filterStr = this.ConnectFilterStr(filterStr, string.Format("拼音码 like '{0}'", queryCode), "or");
            if (dv.Table.Columns.Contains("五笔码"))
                filterStr = this.ConnectFilterStr(filterStr, string.Format("五笔码 like '{0}'", queryCode), "or");
            if (dv.Table.Columns.Contains("自定义码"))
                filterStr = this.ConnectFilterStr(filterStr, string.Format("自定义码 like '{0}'", queryCode), "or");
            if (dv.Table.Columns.Contains("商品名称"))
                filterStr = this.ConnectFilterStr(filterStr, string.Format("商品名称 like '{0}'", queryCode), "or");
            if (dv.Table.Columns.Contains("通用名"))
                filterStr = this.ConnectFilterStr(filterStr, string.Format("通用名 like '{0}'", queryCode), "or");
            if (dv.Table.Columns.Contains("通用名拼音码"))
                filterStr = this.ConnectFilterStr(filterStr, string.Format("通用名拼音码 like '{0}'", queryCode), "or");
            if (dv.Table.Columns.Contains("通用名五笔码"))
                filterStr = this.ConnectFilterStr(filterStr, string.Format("通用名五笔码 like '{0}'", queryCode), "or");
            if (dv.Table.Columns.Contains("通用名自定义码"))
                filterStr = this.ConnectFilterStr(filterStr, string.Format("通用名自定义码 like '{0}'", queryCode), "or");
            if (dv.Table.Columns.Contains("英文商品名"))
                filterStr = this.ConnectFilterStr(filterStr, string.Format("英文商品名 like '{0}'", queryCode), "or");
            if (dv.Table.Columns.Contains("英文通用名"))
                filterStr = this.ConnectFilterStr(filterStr, string.Format("英文通用名 like '{0}'", queryCode), "or");
            if (dv.Table.Columns.Contains("英文别名"))
                filterStr = this.ConnectFilterStr(filterStr, string.Format("英文别名 like '{0}'", queryCode), "or");
            if (dv.Table.Columns.Contains("部门名称"))
                filterStr = this.ConnectFilterStr(filterStr, string.Format("部门名称 like '{0}'", queryCode), "or");
            if (dv.Table.Columns.Contains("学名"))
                filterStr = this.ConnectFilterStr(filterStr, string.Format("学名 like '{0}'", queryCode), "or");
            if (dv.Table.Columns.Contains("学名拼音码"))
                filterStr = this.ConnectFilterStr(filterStr, string.Format("学名拼音码 like '{0}'", queryCode), "or");
            if (dv.Table.Columns.Contains("别名"))
                filterStr = this.ConnectFilterStr(filterStr, string.Format("别名 like '{0}'", queryCode), "or");
            if (dv.Table.Columns.Contains("别名拼音码"))
                filterStr = this.ConnectFilterStr(filterStr, string.Format("别名拼音码 like '{0}'", queryCode), "or");
            return filterStr;
        }
