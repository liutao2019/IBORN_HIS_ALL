...
namespace FS.SOC.Local.Pharmacy.ShenZhen.Extend.BinHai
{
    ...
    public partial class ucCommonInput : UserControl,SOC.HISFC.BizProcess.PharmacyInterface.Pharmacy.IInputInfoControl
    {
        ...
        public int Init()
        {
            ...
            #endregion
            //>>{4950DEF3-A062-4981-AC8A-B3578AE6B1F3}送单日期默认当前时间
        ...
        public FS.HISFC.Models.Pharmacy.Input GetInputObject()
        {
            ...
            if (purchasePrice != input.Item.PriceCollection.RetailPrice)
            {
                if (MessageBox.Show("购入价与零售价不一致, 是否继续?", "确认", MessageBoxButtons.YesNo,
            ...
            if (!string.IsNullOrEmpty(this.ntxtValidDate.Text))
            {
                ...
                {
                    DateTime dtNow = (new FS.HISFC.BizLogic.Pharmacy.Item()).GetDateTimeFromSysDateTime();
                    System.TimeSpan ts = validDate.Subtract(dtNow);
                    if (ts.TotalDays / 365 > 5 || ts.TotalDays / 365 < -5)
                    {
                        if (MessageBox.Show("有效期范围超过 5 年, 是否继续?", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.No)
                        {
                            this.ntxtValidDate.Select();
                            this.ntxtValidDate.Focus();
                            return null;
        ...
        private void ntxtPurchasePrice_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (FS.FrameWork.Function.NConvert.ToDecimal(this.ntxtPurchasePrice.Text) != FS.FrameWork.Function.NConvert.ToDecimal(this.ntxtRetailPrice.Text))
                {
                    MessageBox.Show("购入价与零售价不一致，请确认");
                    ntxtPurchasePrice.Focus();
                }
            }
        }
