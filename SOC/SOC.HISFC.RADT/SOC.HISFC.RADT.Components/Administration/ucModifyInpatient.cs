...
namespace FS.SOC.HISFC.RADT.Components.Administration
{
    ...
    public partial class ucModifyInpatient : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        {
            //by han-zf 2014-10-21 住院号合并打断医保数据流程bug
            if (this.patientInfoOld.Pact.ID != "1")
            {
                string tip = "非自费患者不能直接做住院号合并！";
                tip = tip + "\n当前患者合同单位：" + "[" + this.patientInfoOld.Pact.ID + "]" + this.patientInfoOld.Pact.Name;
                tip = tip + "\n医保患者住院号合并步骤：";
                tip = tip + "\n   1、身份变更为自费";
                tip = tip + "\n   2、住院号合并";
                tip = tip + "\n   3、身份变更为相应医保";
                MessageBox.Show(tip, "错误");
                return -1;
            }
