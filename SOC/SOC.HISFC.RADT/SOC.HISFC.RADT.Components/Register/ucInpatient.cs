...
namespace FS.SOC.HISFC.RADT.Components.Register
{
    /// <summary>
    /// 入院登记主界面
    /// 用接口方式实现 列表显示和登记界面
    ...
    public partial class ucInpatient : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        private string currentInpatientNo = string.Empty;//当前住院流水号
        ...
        [Category("A.参数设置"), Description("登记时是否按合同单位自动生成默认警戒线,True:默认的警戒线取常数维护中合同单位的备注，请将备注设置为数字！")]
        ...
        void PrepayPatient()
        {
            ...
            if (null == control.PatientInfo) return;

            string inpatientNO = radtIntegrate.GetMaxPatientNOByCardNO(control.PatientInfo.PID.CardNO);
            if (inpatientNO == null) return;//数据库出错!
            ...
            if (inpatientNO.Trim() == string.Empty)
            {
                ...
                pInfo = control.PatientInfo;
            }
            else
            {//找到了上次住院信息 
                pInfo = this.radtIntegrate.GetPatientInfomation(inpatientNO);
            ...
            this.IRegister.SetPatientInfo(pInfo, false);
            MessageBox.Show("注意核对患者信息是否正确");
        }
        ...
        protected virtual int insertPatientInfo()
        {
            ...
            //获取新的住院流水号：
            this.patientInfo.ID = string.IsNullOrEmpty(this.currentInpatientNo) ? this.radtIntegrate.GetNewInpatientNO() : this.currentInpatientNo;
            ...
            if (returnValue != 1)
            {
                this.currentInpatientNo = this.patientInfo.ID;
                FS.FrameWork.Management.PublicTrans.RollBack();
                medcareInterfaceProxy.Rollback();
                this.myMessageBox("待遇接口住院登记失败" + medcareInterfaceProxy.ErrMsg, MessageBoxIcon.Error);
                return -1;
            }
            this.currentInpatientNo = string.Empty;
        ...
        public int ReadCard()
        {
            ...
            Base.Inpatient.frmSiRegisterInfo frmSiRegisterInfo = new FS.SOC.HISFC.RADT.Components.Base.Inpatient.frmSiRegisterInfo();
