...
namespace FS.SOC.HISFC.Components.Pharmacy.Extend
{
    ...
    public partial class ucBillRePrint : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        FS.SOC.HISFC.BizLogic.Pharmacy.Constant cntMgr = new FS.SOC.HISFC.BizLogic.Pharmacy.Constant();
        #endregion 
        ...
        private void InitCompany()
        {
            try
            {
                FS.HISFC.Models.Pharmacy.Company ob;
                ArrayList alCnInfo = new ArrayList();
                alCnInfo = this.cntMgr.QueryCompany("1");
                ArrayList alDrugDept = this.cntMgr.QueryDeptConstantList();
                if (alDrugDept != null)
                {
                    if (alDrugDept.Count > 0)
                    {
                        foreach (FS.HISFC.Models.Pharmacy.DeptConstant deptCnt in alDrugDept)
                        {
                            ob = new FS.HISFC.Models.Pharmacy.Company();
                            ob.ID = deptCnt.ID;
                            ob.Name = deptCnt.Name;
                            ob.SpellCode = FS.FrameWork.Public.String.GetSpell(deptCnt.Name);
                            ob.Memo = deptCnt.Memo;
                            alCnInfo.Add(ob);
                        }
                    }
                }
                if (alCnInfo != null)
                {
                    if (alCnInfo.Count > 0)
                    {
                        this.cmbCompany.AddItems(alCnInfo);
                    }
                }
            }
            catch
        ...
        private void PrintInputBill(int activeRowIndex)
        {
            ...
            if (this.IsPrint)
            {
                ((FS.HISFC.Models.Pharmacy.Input)alInputDetail[0]).SpecialFlag = "补打";
        ...
        private void PrintOutputBill(int activerow)
        {
            ...
            {
                #region 增加补打标记
                ((FS.HISFC.Models.Pharmacy.Output)al[0]).SpecialFlag = "补打";
        ...
        private void SetFpForOutput()
        {
            ...
            this.neuSpread1_Sheet1.ColumnHeader.Cells.Get(0, 3).Text = "领用单位名称";
            ...
            this.neuSpread1_Sheet1.Columns.Get(2).Width = 65F;
            this.neuSpread1_Sheet1.Columns.Get(3).Label = "领用单位名称";
            this.neuSpread1_Sheet1.Columns.Get(3).Width = 211F;
            this.neuSpread1_Sheet1.Columns.Get(3).AllowAutoSort = true;
            ...
            this.neuSpread1_Sheet2.ColumnHeader.Cells.Get(0, 3).Text = "领用单位";
            this.neuSpread1_Sheet2.ColumnHeader.Cells.Get(0, 4).Text = "出库数量";
            this.neuSpread1_Sheet2.ColumnHeader.Cells.Get(0, 5).Text = "包装购入价";
            ...
            this.neuSpread1_Sheet2.Columns.Get(3).Label = "领用单位";
            this.neuSpread1_Sheet2.Columns.Get(3).Width = 87F;
            this.neuSpread1_Sheet2.Columns.Get(4).Label = "出库数量";
            this.neuSpread1_Sheet2.Columns.Get(4).Width = 87F;
            numberCellType1.DecimalPlaces = 4;
            this.neuSpread1_Sheet2.Columns.Get(5).CellType = numberCellType1;
            this.neuSpread1_Sheet2.Columns.Get(5).Label = "包装购入价";
        ...
        private void cmbCompany_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                string tempfilter = this.cmbCompany.Text.Trim();
                if (this.BillType == "I")
                {
                    if (this.dvInput != null)
                    {
                        this.dvInput.RowFilter = "(供货公司 LIKE '%" + tempfilter + "%')";
                ...
                else if (this.BillType == "O")
                {
                    if (this.dvOutput != null)
                    {
                        this.dvOutput.RowFilter = "(领用单位名称 LIKE '%" + tempfilter + "%')";
                        this.neuSpread1_Sheet1.DataSource = this.dvOutput;
