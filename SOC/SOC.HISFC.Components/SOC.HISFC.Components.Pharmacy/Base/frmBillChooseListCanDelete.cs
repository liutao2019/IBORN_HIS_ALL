...
{
    public partial class frmBillChooseListCanDelete : Form
    {
        public frmBillChooseListCanDelete()
        {
            InitializeComponent();
        }
        ...
        void nbtDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                ArrayList alApplyout = null;
                ArrayList allValidApplyOut = new ArrayList();
                string[] values = this.GetChooseData(new int[]{0,1});
                ...
                else
                {
                    ...
                    alApplyout = this.itemMgr.QueryApplyOutInfoByListCode(values[1], values[0], "1");
                    ...
                    foreach (FS.HISFC.Models.Pharmacy.ApplyOut applyOut in alApplyout)
                    {
                        ...
                        allValidApplyOut.Add(applyOut);
                    }
                }
                if (allValidApplyOut != null && allValidApplyOut.Count > 0)
                {
                    if ((DialogResult)MessageBox.Show("是否确认删除单据：" + values[0] + "?", "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        FS.FrameWork.Management.PublicTrans.BeginTransaction();
                        foreach (FS.HISFC.Models.Pharmacy.ApplyOut applyOutInfo in allValidApplyOut)
                        {
                            if (this.itemMgr.InValidApplyOut(applyOutInfo.ID, this.itemMgr.Operator.ID) == -1)
                            {
                                FS.FrameWork.Management.PublicTrans.RollBack();
                                Function.ShowMessage("删除流水号为" + applyOutInfo.ID + "的出库申请信息失败,请联系信息科！", MessageBoxIcon.Error);
                                return;
                            }
                        }
        ...
        private int SetFormat()
        {
            if (System.IO.File.Exists(curSettingFileName))
            {
                ...
                for (int index = 0; index < this.neuDataListSpread_Sheet1.Rows.Count; index++)
                {
                    try
                    {
                        if (this.neuDataListSpread.GetCellText(0, index, "颜色") == "无")
                        {
                            this.neuDataListSpread.Sheets[0].Rows[index].BackColor = System.Drawing.Color.Transparent;
                        }
                        else
                        {
                            this.neuDataListSpread.Sheets[0].Rows[index].BackColor = System.Drawing.Color.Yellow;
                        }
                    }
                    catch { }
                }
