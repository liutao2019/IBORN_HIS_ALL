...
{
    public partial class ucRadiotherapyUsageManager : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Forms.IMaintenanceControlable
    {
        public ucRadiotherapyUsageManager()
        {
            InitializeComponent();
        }
        ...
        /// 配置中心用法常数类别
        /// </summary>
        private string radiotherapyUsageTypeCode = "RadiotherapyUsage";
        ...
        {
            //增加院注用法处理
            ArrayList alCompoundUsage = constantManager.GetAllList(this.radiotherapyUsageTypeCode);
            if (alCompoundUsage == null)
            {
                MessageBox.Show(Language.Msg("查询院注用法常数发生错误!\r\n") + this.constantManager.Err);
                return;
            }

            Hashtable hsCompoundUsage = new Hashtable();
            foreach (FS.FrameWork.Models.NeuObject con in alCompoundUsage)
            {
        ...
        public int Save()
        {
            ...
            try
            {
                //增加配置中心用法处理
                //先全部删除，然后单个插入需要配置的常数
                if (this.constantManager.DelConstant(this.radiotherapyUsageTypeCode) == -1)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                    MessageBox.Show(Language.Msg("删除院注用法常数失败！\r\n" + this.constantManager.Err));
                    return -1;
                }
                ...
                for (int i = 0; i < this.neuSpread1_Sheet1.Rows.Count; i++)
                {
                    ...
                    //增加院注用法处理
                    if (FS.FrameWork.Function.NConvert.ToBoolean(neuSpread1_Sheet1.Cells[i, (int)ColumnSet.IsCompound].Value))
                    {
                        if (this.constantManager.InsertItem(this.radiotherapyUsageTypeCode, cons) == -1)
                        {
                            FS.FrameWork.Management.PublicTrans.RollBack();
                            MessageBox.Show(Language.Msg("保存发生错误" + this.constantManager.Err));
                            return -1;
