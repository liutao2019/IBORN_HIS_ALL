...
namespace FS.SOC.HISFC.Components.Pharmacy.Maintenance
{
    ...
    public partial class ucItemManager : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Classes.IPreArrange
    {
        ...
        /// 已使用的情况下是否可以修改购入价
        /// </summary>
        [Description("已使用的情况下是否可以修改购入价"), Category("设置"), Browsable(true)]
        public bool IsCanEditPurchasePrice
        {
            get { return this.isCanEditPurchasePrice; }
            set
            {
                this.isCanEditPurchasePrice = value;
                this.frmItem.IsCanEditPurchasePrice = value;
            }
        }
        ...
        private frmItemNew frmItem = new frmItemNew();
        ...
        private void GenarateCharOrderString()
        {
            ...
            foreach (FS.HISFC.Models.Pharmacy.Item itemInfo in listItem)
            {
                string charOrderString = FS.FrameWork.Function.NConvert.ToCharSortCode(itemInfo.Name);
               
                itemInfo.NameCollection.WBCode = itemInfo.NameCollection.WBCode + charOrderString;

                if (itemInfo.NameCollection.WBCode.Length > 40)
                {
                    itemInfo.NameCollection.WBCode = itemInfo.NameCollection.WBCode.Substring(0, 40);
                }
                if (itemMgr.UpdateCharOrderString(itemInfo) <= 0)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                    Function.ShowMessage(itemInfo.Name + "更新笔画码出错，请联系系统管理员！", MessageBoxIcon.Warning);
                    return;
                }
            }
        ...
        private int Init()
        {
            try
            {
                ...
                this.frmItem.GetNextItem -= new frmItemNew.GetNextItemHandler(this.frmItem_GetNextItem);
                this.frmItem.GetNextItem += new frmItemNew.GetNextItemHandler(this.frmItem_GetNextItem);
                this.frmItem.EndSave -= new frmItemNew.SaveItemHandler(this.frmItem_EndSave);
                this.frmItem.EndSave += new frmItemNew.SaveItemHandler(this.frmItem_EndSave);
        ...
        private int SetDataRowValue(FS.HISFC.Models.Pharmacy.Item item, DataRow row)
        {
            ...
            {
                row["门诊拆分"] = ((EnumSplitType)(FS.FrameWork.Function.NConvert.ToInt32(item.SplitType))).ToString();
            }
            if (!string.IsNullOrEmpty(item.LZSplitType))
            {
                row["临嘱拆分"] = ((EnumSplitType)(FS.FrameWork.Function.NConvert.ToInt32(item.LZSplitType))).ToString();
            }
            if (!string.IsNullOrEmpty(item.CDSplitType))
            {
                row["长嘱拆分"] = ((EnumSplitType)(FS.FrameWork.Function.NConvert.ToInt32(item.CDSplitType))).ToString();
            }
            ...
            row["特殊标记1"] = this.baseCache.GetExtendFieldName("特殊标记1", item.SpecialFlag, item.SpecialFlag);
            row["特殊标记2"] = this.baseCache.GetExtendFieldName("特殊标记2", item.SpecialFlag1, item.SpecialFlag1);
            row["特殊标记3"] = this.baseCache.GetExtendFieldName("特殊标记3", item.SpecialFlag2, item.SpecialFlag2);
            row["特殊标记4"] = this.baseCache.GetExtendFieldName("特殊标记4", item.SpecialFlag3, item.SpecialFlag3);
            row["特殊标记5"] = this.baseCache.GetExtendFieldName("特殊标记5", item.SpecialFlag4, item.SpecialFlag4);

            row["字符扩展1"] = this.baseCache.GetExtendFieldName("字符扩展1", item.ExtendData1, item.ExtendData1);
            row["字符扩展2"] = this.baseCache.GetExtendFieldName("字符扩展2", item.ExtendData2, item.ExtendData2);
            row["字符扩展3"] = this.baseCache.GetExtendFieldName("字符扩展3", item.ExtendData3, item.ExtendData3);
            row["字符扩展4"] = this.baseCache.GetExtendFieldName("字符扩展4", item.ExtendData4, item.ExtendData4);
        ...
        private void Delete()
        {
            ...
            if (hsItem.Contains(drugNO))
            {
                ...
                alDrug.Add(item);
                int param = Function.SendBizMessage(alDrug, FS.SOC.HISFC.BizProcess.MessagePatternInterface.EnumOperType.Delete, FS.SOC.HISFC.BizProcess.MessagePatternInterface.EnumInfoType.Drug, ref errInfo);
                if (param == -1)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
        ...
        private void Filter()
        {
            string filter = Function.GetFilterStr(this.dtItems.DefaultView, "%" + this.ntxtFilter.Text + "%");
        ...
        protected override int OnPrint(object sender, object neuObject)
        {
            ...
            if (this.hsItem.Contains(drugNO))
            {
                ...
                Function.PrintBill("0301", "01", alItem);
            }
