...
namespace FS.SOC.HISFC.Components.Pharmacy.Maintenance
{
    ...
    public partial class frmItemNew : Form
    {
        ...
        /// 是否自动生成自定义码
        /// </summary>
        private bool IsAutoUserCode = false;
        ...
        private bool isCanEditPurchasePrice = false;
        /// <summary>
        /// 已使用的情况下是否可以修改购入价
        /// </summary>       
        public bool IsCanEditPurchasePrice
        {
            get { return this.isCanEditPurchasePrice; }
            set { this.isCanEditPurchasePrice = value; }
        }
        #endregion
        ...
        /// </summary>
        /// <returns></returns>
        public int Init(BaseCache baseCache)
        {
            if (baseCache == null)
            {
                return 1;
            }
            IsAutoUserCode = FS.FrameWork.Function.NConvert.ToBoolean(this.ctlMgr.QueryControlerInfo("MZ0203", false));
            ...
            {
                IItemExtendControl = Function.GetItemExtendControl();
            ...
            {
                this.ncmbLZSplitType.Items.Add(((EnumSplitType)index).ToString());
                this.ncmbSplitType.Items.Add(((EnumSplitType)index).ToString());
                this.ncmbCDSplitType.Items.Add(((EnumSplitType)index).ToString());
            }
            ...
            this.cmbBigPackUnit.AddItems(baseCache.bigPackUnitHelper.ArrayObject);
        ...
        public int Clear()
        {
            ...
            this.ntxtSecondDose.Text = string.Empty;
            this.ncmbSecondDoseUnit.Tag = "";
            this.ncmbSecondDoseUnit.Text = string.Empty;
            this.ntxtSecondDose.Enabled = this.IsShowSecondDosage;
            this.ncmbSecondDoseUnit.Enabled = this.IsShowSecondDosage;
        ...
        public void SetItem(FS.HISFC.Models.Pharmacy.Item item, bool isUsed)
        {
            ...
            this.chbIsStop.Enabled = Function.JugePrive("0300", "01");
            ...
            if (IsAutoUserCode && string.IsNullOrEmpty(item.UserCode))// {0E2619A7-5196-4475-A672-3906D86A4AD5}
            {
                string sql = "select seq_baseinfo_custom_code.nextval from dual";
                string userCode = consMgr.ExecSqlReturnOne(sql);
            ...
            this.txtProductID.Text = item.ProductID;
            ...
            #region 新增大包装单位和大包装数量
            this.cmbBigPackUnit.Text = item.BigPackUnit;
            this.txtBigPackQty.Text = item.BigPackQty;
            #endregion
        ...
        private bool CheckValid()
        {
            ...
            //{25122023-EC6F-40f9-B032-811E19F24323}
            ...
            else
            {
                ...
                FS.HISFC.Models.Pharmacy.Item tmp = itemMgr.GetItemByUserCode(userCode);

                if (tmp != null && (this.item == null || ( this.item != null && tmp.ID != this.item.ID)))
                {
                    Function.ShowMessage("自定义码重复，请重新输入自定义码！", MessageBoxIcon.Information);
                    return false;
                }
            }
        ...
        private FS.HISFC.Models.Pharmacy.Item GetItem()
        {
            ...
            item.ProductID = this.txtProductID.Text;
            ...
            //添加新药赋值{6EF28428-19E4-4c64-ABDA-1EEDFAE53E7B}
            item.IsNew = this.chbIsNew.Checked;
            ...
            #region 增加大包装单位和大包装数量
            decimal bigPackQty = 0;
            if(decimal.TryParse(this.txtBigPackQty.Text,out bigPackQty))
            {
                 item.BigPackUnit = this.cmbBigPackUnit.Text;
                 item.BigPackQty = bigPackQty.ToString();
            }
            #endregion
            return item;
            ...
            if (this.CheckValid())
            {
                ...
                if (item != null)
                {
                    ...
                    if (string.IsNullOrEmpty(item.ID))
                    {
                        ...
                        if (Function.SendBizMessage(alItem, FS.SOC.HISFC.BizProcess.MessagePatternInterface.EnumOperType.Add, FS.SOC.HISFC.BizProcess.MessagePatternInterface.EnumInfoType.Drug, ref errInfo) == -1)
                        {
                            FS.FrameWork.Management.PublicTrans.RollBack();
                    ...
                    else
                    {
                        ...
                        alItem.Add(item.Clone());
                        if (Function.SendBizMessage(alItem, FS.SOC.HISFC.BizProcess.MessagePatternInterface.EnumOperType.Modify, FS.SOC.HISFC.BizProcess.MessagePatternInterface.EnumInfoType.Drug, ref errInfo) == -1)
                        {
                            FS.FrameWork.Management.PublicTrans.RollBack();
                            Function.ShowMessage("保存失败，请向系统管理员联系并报告错误：" + errInfo, MessageBoxIcon.Error);
                            return;
        ...
        {
            FS.HISFC.Models.Base.Spell spell = Function.GetSpellCode(this.txtFormalName.Text);
        ...
        {
            FS.HISFC.Models.Base.Spell spell = Function.GetSpellCode(this.txtRegularName.Text);
        ...
        private string GetItemBaseDrugInfo(FS.HISFC.Models.Pharmacy.Item item)
        {
            ...
            string extend2 = string.Empty;
            if (!string.IsNullOrEmpty(item.ID))
            {
                extend2 = SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(item.ID).ExtendData2;
            }
            else
            {
        ...
        protected string GetRetailPriceComputeFormula(FS.HISFC.Models.Pharmacy.Item item)
        {
            ...
            if (hsFormula.Contains(item.Type.ID))
            {
                ArrayList alFormula = hsFormula[item.Type.ID] as ArrayList;
                ...
                foreach (FS.SOC.HISFC.Models.Pharmacy.Adjust.RetailPriceFormula retailPriceFormula in alFormula)
                {
                    ...
                    if (retailPriceFormula.DrugType.ID == item.Type.ID)
                    {
                        if (retailPriceFormula.PriceType == "0" && itemTmp.PriceCollection.PurchasePrice >= retailPriceFormula.PriceLower && item.PriceCollection.PurchasePrice <= retailPriceFormula.PriceUpper)
                        {
                            formula = retailPriceFormula.Name;
                        }
                        else if (retailPriceFormula.PriceType == "1" && itemTmp.PriceCollection.WholeSalePrice >= retailPriceFormula.PriceLower && item.PriceCollection.WholeSalePrice <= retailPriceFormula.PriceUpper)
                        {
                            formula = retailPriceFormula.Name;
                        }
                        else if (retailPriceFormula.PriceType == "2" && itemTmp.PriceCollection.RetailPrice >= retailPriceFormula.PriceLower && item.PriceCollection.RetailPrice <= retailPriceFormula.PriceUpper)
                        {
                            formula = retailPriceFormula.Name;
                        }
                    }
            ...
            if (!string.IsNullOrEmpty(itemTmp.BigPackQty) && itemTmp.BigPackQty != "0")
            {
                formula = formula.Replace("购入价", (itemTmp.PriceCollection.PurchasePrice).ToString("F4").TrimEnd('0').TrimEnd('.'));
                formula = formula.Replace("批发价", (itemTmp.PriceCollection.WholeSalePrice).ToString("F4").TrimEnd('0').TrimEnd('.'));
                formula = formula.Replace("零售价", (itemTmp.PriceCollection.RetailPrice).ToString("F4").TrimEnd('0').TrimEnd('.'));
            }
            else
            {
                formula = formula.Replace("购入价", item.PriceCollection.PurchasePrice.ToString("F4").TrimEnd('0').TrimEnd('.'));
        ...
        private void txtPurchasePrice_TextChanged(object sender, EventArgs e)
        {
            if (item != null && string.IsNullOrEmpty(item.ID))
            {
                ...
                itemTemp.BigPackQty = this.txtBigPackQty.Text.ToString();
                ...
                else
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(itemTemp.BigPackQty) && itemTemp.BigPackQty != "0")
                        {
                            this.txtRetailPrice.Text = (FS.FrameWork.Function.NConvert.ToDecimal(FS.FrameWork.Public.String.ExpressionVal(formula))/FS.FrameWork.Function.NConvert.ToDecimal(itemTemp.BigPackQty)).ToString("F2");
                        }
                        else
                        {
