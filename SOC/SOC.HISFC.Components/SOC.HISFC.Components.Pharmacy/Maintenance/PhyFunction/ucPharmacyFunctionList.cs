...
namespace FS.SOC.HISFC.Components.Pharmacy.Maintenance
{
    ...
    /// [创建时间: 2009-6-2]<br></br>
    /// </summary>
    public partial class ucPharmacyFunctionList : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        //药理常数管理
        private FS.SOC.HISFC.BizLogic.Pharmacy.Constant pharmacyConstant = new FS.SOC.HISFC.BizLogic.Pharmacy.Constant();
        ...
        //帮助类
        FS.FrameWork.Public.ObjectHelper ehSysClass = new FS.FrameWork.Public.ObjectHelper();
        FS.FrameWork.Public.ObjectHelper ehType = new FS.FrameWork.Public.ObjectHelper();
        FS.FrameWork.Public.ObjectHelper ehQuality = new FS.FrameWork.Public.ObjectHelper();
        FS.FrameWork.Public.ObjectHelper ehMinFee = new FS.FrameWork.Public.ObjectHelper();
        FS.FrameWork.Public.ObjectHelper ehFunction1 = new FS.FrameWork.Public.ObjectHelper();
        FS.FrameWork.Public.ObjectHelper ehFunction2 = new FS.FrameWork.Public.ObjectHelper();
        FS.FrameWork.Public.ObjectHelper ehFunction3 = new FS.FrameWork.Public.ObjectHelper();
        ...
        #region 属性
        private bool isShowViewList = false;

        [Description("是否显示药理作用ViewList"), Category("设置")]
        public bool IsShowViewList
        {
            get { return isShowViewList; }
            set
            {
                isShowViewList = value;
                
            }
        }

        private bool isShowDrugInfo = false;

        [Description("是否显示药品详细信息"), Category("设置")]
        public bool IsShowDrugInfo
        {
            get { return isShowDrugInfo; }
            set 
            { 
                isShowDrugInfo = value;
                this.neuTab.Visible = value;
            }
        }
        ...
        private void ShowDrugList(string fCode, int fLevl)
        {
            ...
            FS.SOC.HISFC.BizLogic.Pharmacy.Constant cItem = new FS.SOC.HISFC.BizLogic.Pharmacy.Constant();
            List<FS.HISFC.Models.Pharmacy.Item> al = cItem.QueryItemListByFunctionID(fCode, fLevl);
            if (al != null)
            {
                this.PutList( al );
            }
        ...
        /// </summary>
        /// <param name="al"></param>
        private void PutList(List<FS.HISFC.Models.Pharmacy.Item> al)
        {
            fpDrugList.RowCount = 0;
            fpDrugList.RowCount = al.Count;
            for (int i = 0; i < al.Count; i++)
            {
                FS.HISFC.Models.Pharmacy.Item pItem = new FS.HISFC.Models.Pharmacy.Item();
                pItem = al[i];
                ...
                //有效性
                if (pItem.ValidState == FS.HISFC.Models.Base.EnumValidState.Valid)
                {
                    fpDrugList.Cells[i, 12].Text = "有效";
                }
                else if (pItem.ValidState == FS.HISFC.Models.Base.EnumValidState.Invalid)
                {
                    fpDrugList.Cells[i, 12].Text = "无效";
                }
                else if (pItem.ValidState == FS.HISFC.Models.Base.EnumValidState.Ignore)
                {
                    fpDrugList.Cells[i, 12].Text = "废弃";
                }
                else if (pItem.ValidState == FS.HISFC.Models.Base.EnumValidState.Extend)
                {
        ...
        {
            this.ehSysClass.ArrayObject = FS.HISFC.Models.Base.SysClassEnumService.List();//系统类别
            this.ehType.ArrayObject = consManager.GetList( FS.HISFC.Models.Base.EnumConstant.ITEMTYPE );//药品类别
            this.ehQuality.ArrayObject = consManager.GetList( FS.HISFC.Models.Base.EnumConstant.DRUGQUALITY );//药品性质
            this.ehMinFee.ArrayObject = consManager.GetList( FS.HISFC.Models.Base.EnumConstant.MINFEE );//项目分类
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            else
            {
                if (!this.neuTabControl1.TabPages.Contains(this.tabPage1))
                {
                    this.neuTabControl1.TabPages.Insert(0, this.tabPage1);
                }
            }
            base.OnLoad( e );
        }
        ...
        private void neuSpread1_CellClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
        {
            ...
            FS.SOC.HISFC.BizLogic.Pharmacy.Item Pitem = new FS.SOC.HISFC.BizLogic.Pharmacy.Item();
