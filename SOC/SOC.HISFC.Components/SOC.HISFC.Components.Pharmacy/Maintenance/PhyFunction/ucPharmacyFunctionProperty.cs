    ...
    public partial class ucPharmacyFunctionProperty : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        private int Save()
        {
            ...
            try
            {
                ...
                if(this.operKind=="UPDATE")
                {
                    operType = FS.SOC.HISFC.BizProcess.MessagePatternInterface.EnumOperType.Modify;
                }
                else if (this.operKind == "DELETE")
                {
                    operType = FS.SOC.HISFC.BizProcess.MessagePatternInterface.EnumOperType.Delete;
                }
                string errInfo ="";
                ArrayList alFunction =new ArrayList();
                alFunction.Add(functionObject);
                if (Function.SendBizMessage(alFunction, operType, FS.SOC.HISFC.BizProcess.MessagePatternInterface.EnumInfoType.DrugFunction, ref errInfo) == -1)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                    Function.ShowMessage("操作失败，请向系统管理员联系并报告错误：" + errInfo, MessageBoxIcon.Error);
                    return -1;
                }
