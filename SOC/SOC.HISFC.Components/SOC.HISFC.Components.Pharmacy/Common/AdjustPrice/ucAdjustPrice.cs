...
namespace FS.SOC.HISFC.Components.Pharmacy.Common.AdjustPrice
{
    ...
    /// 协定处方的调价没有考虑
    /// </summary>
    public partial class AdjustPrice : ucBaseAdjustPrice, FS.FrameWork.WinForms.Classes.IPreArrange
    {
        public AdjustPrice()
        {
            InitializeComponent();
        ...
        /// 当前操作的调价申请单
        /// </summary>
        private string curAdjustApplyBillNO = string.Empty;
        ...
        protected int Init()
        {
            ...
            if(curAdjustPriceType == EnumAdjustPriceType.PriceAdjustMentBatch)
            {
                strAdjustType = "批次调价";
            }
            else if (curAdjustPriceType == EnumAdjustPriceType.PriceAdjustMentPolicy)
            {
                strAdjustType = "政策性调价";
            }
            this.nlbAdjustType.Text = strAdjustType;
            ...
            this.settingFileName = FS.FrameWork.WinForms.Classes.Function.CurrentPath + "\\Profile\\PhaAdjustPrice" + this.Class2Code + this.curPriveType.ID + "Setting.xml";
        ...
        protected int InitDataTable()
        {
            ...
            {
                if (dc.ColumnName == "新零差价"||dc.ColumnName == "新零售价" || dc.ColumnName == "调价文号" || dc.ColumnName == "备注" || dc.ColumnName == "差价")
                {
                    continue;
                }
                dc.ReadOnly = true;
        ...
        protected int InitFarPoint()
        {
            ...
            if (!System.IO.File.Exists(this.settingFileName))
            {
                ...
                this.curIDataDetail.FpSpread.SetColumnWith(0, "调价数量", 60f);
                this.curIDataDetail.FpSpread.SetColumnWith(0, "原购入价", 60f);
                this.curIDataDetail.FpSpread.SetColumnWith(0, "原零售价", 60f);
                this.curIDataDetail.FpSpread.SetColumnWith(0, "原零差价", 60f);
                this.curIDataDetail.FpSpread.SetColumnWith(0, "新购入价", 60f);
                this.curIDataDetail.FpSpread.SetColumnWith(0, "加成价", 60f);
                this.curIDataDetail.FpSpread.SetColumnWith(0, "新零售价", 60f);
                this.curIDataDetail.FpSpread.SetColumnWith(0, "新零差价", 60f);
                this.curIDataDetail.FpSpread.SetColumnWith(0, "最高限价", 60f);
                ...
                this.curIDataDetail.FpSpread.SetColumnCellType(0, "原零差价", nPrice);
                ...
                this.curIDataDetail.FpSpread.SetColumnCellType(0, "新零差价", tWrite);
                columnIndex = this.curIDataDetail.FpSpread.GetColumnIndex(0, "新零差价");
                this.curIDataDetail.FpSpread.Sheets[0].Columns[columnIndex].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Right;
        ...
        protected bool CheckValid()
        {
            ...
            foreach (DataRow dr in this.dtDetail.Rows)
            {
                ...
                if (!decimal.TryParse(dr["新零差价"].ToString(), out retailPrice))
                {
                    Function.ShowMessage(dr["药品名称"].ToString() + "新零差价不正确，请录入数字", MessageBoxIcon.Information);
                    return false;
                }
                ...
                if (FS.FrameWork.Function.NConvert.ToDecimal(dr["新零差价"]) < 0)
                {
                    Function.ShowMessage(dr["药品名称"].ToString() + "新零差价不能小于0", MessageBoxIcon.Information);
                    return false;
                }
                //if (!isIgoreZero && (FS.FrameWork.Function.NConvert.ToDecimal(dr["新零售价"]) == 0||FS.FrameWork.Function.NConvert.ToDecimal(dr["新零差价"]) == 0))
                //{
                ...
                bool existRetail2Adj = true;
                bool existRetailAdj = true;
                if (FS.FrameWork.Function.NConvert.ToDecimal(dr["新零差价"]) == FS.FrameWork.Function.NConvert.ToDecimal(dr["新零售价"]))
                {
                    existRetail2Adj = false;
                }
                if (FS.FrameWork.Function.NConvert.ToDecimal(dr["新零售价"]) == FS.FrameWork.Function.NConvert.ToDecimal(dr["原零售价"]))
                {
                    existRetailAdj = false;
                }
                if (!existRetailAdj && !existRetail2Adj)
                {
        ...
        protected int Save()
        {
            ...
            foreach (DataRow dr in this.dtDetail.Rows)
            {
                ...
                info.AfterWholesalePrice = FS.FrameWork.Function.NConvert.ToDecimal(dr["新购入价"]); //调价后购入价
                info.AfterRetailPrice2 = FS.FrameWork.Function.NConvert.ToDecimal(dr["新零差价"]);
            ...
            #region 更新调价申请表状态
            if (!string.IsNullOrEmpty(curAdjustApplyBillNO))
            {
                this.adjustMgr.UpdateAdjustPriceApplyInfo(curAdjustApplyBillNO, "1");
            }
            #endregion
        ...
        #region 显示单据{00FFBF33-AA43-45d0-8644-A5E49ECB54D9}
        ...
        protected int ShowList()
        {
            ...
            if (txtfilter.Text != "")
            {
                if (this.IsSingleDept)
                {
                    alAdjustPriceList = this.adjustMgr.QueryBillListAndItem(this.priveDept.ID, this.ndtpBegin.Value, this.ndtpEnd.Value, this.txtfilter.Text);
                }
                else
                {
                    alAdjustPriceList = this.adjustMgr.QueryGlobalBillListAndItem(this.ndtpBegin.Value, this.ndtpEnd.Value, this.txtfilter.Text);
                }
            }
            else
            {
            ...
            foreach (FS.HISFC.Models.Pharmacy.AdjustPrice adjustPrice in alAdjustPriceList)
            {
                ...
                {
                    if (this.CurAdjustPriceType == EnumAdjustPriceType.PriceAdjustMentBatch && adjustPrice.AdjustPriceType != "2")
                    {
                        continue;
                    }
                    else if (this.curAdjustPriceType == EnumAdjustPriceType.PriceAdjustMentPolicy && adjustPrice.AdjustPriceType != "1")
                    {
                        continue;
                    }
                    else
        ...
        protected int ShowDetail(FS.HISFC.Models.Pharmacy.AdjustPrice adjustPriceInfo)
        {
            ...
            if (adjustPriceInfo.State == "1")
            {
                 alAdjustPriceInfo = this.adjustMgr.QueryAdjustPriceInfoDetailListByPurhancePrice(adjustPriceInfo.ID);
            }
            else
            {
                alAdjustPriceInfo = this.adjustMgr.QueryAdjustPriceInfoList(adjustPriceInfo.ID);
            }
        ...
        public override void ToolStrip_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            else if (e.ClickedItem.Text == "申请单")
            {
                this.ChooseApplyBill();
            }
        ...
        /// </summary>
        /// <returns></returns>
        private int ChooseApplyBill()
        {
            curAdjustApplyBillNO = string.Empty;
            ...
            FS.SOC.HISFC.BizProcess.PharmacyInterface.Pharmacy.ChooseDataSetting chooseDataSetting = SOC.HISFC.Components.Pharmacy.Function.GetBizChooseDataSetting("0304", curPriveType.Memo, curPriveType.ID, "1", ref errInfo);
            ...
            if (chooseDataSetting.IsDefault)
            {
                ...
                string SQL = "";
                if (this.adjustMgr.Sql.GetSql("SOC.Pharmacy.AjustPrice.ChooseBill", ref SQL) == -1)
                {
                    Function.ShowMessage("没有找到SQL：SOC.Pharmacy.AjustPrice.ChooseBill,请与系统管理员联系！", MessageBoxIcon.Error);
                    return -1;
                }
                ...
                chooseDataSetting.SettingFileName = FS.FrameWork.WinForms.Classes.Function.CurrentPath + "\\Profile\\AjustPriceApplySettings.xml";
            }
        ...
        void frmBillChooseList_ChooseCompletedEvent()
        {
            ...
            {
                ArrayList allAdjustApply = this.adjustMgr.QueryAdjustPriceApplyInfoList(values[0], "0");
                if (allAdjustApply == null)
                {
                    Function.ShowMessage("查询调价申请数据发生错误：" + this.adjustMgr.Err + "\n请与系统管理员联系!", MessageBoxIcon.Error);
                    return;
                }

                foreach (FS.HISFC.Models.Pharmacy.AdjustPrice adjustPriceApply in allAdjustApply)
                {

                    adjustPriceApply.Item = SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(adjustPriceApply.Item.ID);
                    if (this.AddObjectToDataTable(adjustPriceApply) == -1)
                    {
                        break;
                    }
                }
            }
            curAdjustApplyBillNO = values[0];
        }
        ...
        protected override FS.FrameWork.WinForms.Forms.ToolBarService OnInit(object sender, object neuObject, object param)
        {
           ...
           toolBarService.AddToolButton("申请单", "调价申请单", FS.FrameWork.WinForms.Classes.EnumImageList.C采购单, true, false, null);
    ...
    public enum  EnumAdjustPriceType
    {
        Default,//默认调价
        PriceAdjustMentPolicy,//零售价调价
        PriceAdjustMentBatch  //批次调价
    }
