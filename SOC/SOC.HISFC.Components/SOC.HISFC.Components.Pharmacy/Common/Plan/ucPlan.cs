...
namespace FS.SOC.HISFC.Components.Pharmacy.Common.Plan
{
    /// <summary>
    /// [功能描述: 入库计划]<br></br>
    /// [创 建 者: cube]<br></br>
    /// [创建时间: 2011-04]<br></br>
    /// </summary>
    public partial class ucPlan : Base.ucBaseControl, FS.FrameWork.WinForms.Classes.IPreArrange
    {
        public ucPlan()
        {
            InitializeComponent();
       ...
       // private FS.SOC.HISFC.BizProcess.PharmacyInterface.Pharmacy.IDataChooseList curIDataChooseList = null;//ucDataChooseList
        FS.SOC.HISFC.Components.Pharmacy.Base.ucDataChooseList curIDataChooseList = null;
        ...
        private decimal totPurhanceCost = 0;
        ...
        string class2Code = "0312";


        /// <summary>
        /// 二级权限
        /// </summary>
        [Description("二级权限,0312制单,其它编码审核"), Category("设置"), Browsable(true)]
        ...
        /// 是否需要走审批流程
        /// </summary>
        private bool isNeedExamine = true;

        /// <summary>
        /// 是否需要走审批流程
        /// </summary>
        [Description("是否需要走审批流程，否不显示已审、未审节点"), Category("设置"), Browsable(true)]
        public bool IsNeedExamine
        {
            get { return isNeedExamine; }
            set
            {
                isNeedExamine = value;
            }
        ...
        protected int InitChooseData()
        {
            ...
            if (chooseDataSetting.IsDefault)
            {
                ...
                string SQL = "";//SOC.Pharmacy.Common.InPlanPrive.ChooseList
                //{5DDC1B83-0693-4949-93A0-98FAC0630510}//左侧物资菜单栏添加科室过滤
                if (this.itemMgr.Sql.GetSql("SOC.Pharmacy.Common.InPlanPrive.ChooseList1", ref SQL) == -1)
                {
            ...
            if (this.curIDataChooseList != null)
            {
                ...
                this.curIDataChooseList.DeptID = this.priveDept.ID;
                ...
                //{5DDC1B83-0693-4949-93A0-98FAC0630510}更改为科室过滤取左侧药品信息
                this.curIDataChooseList.ShowChooseList1(curChooseDataSetting.ListTile, curChooseDataSetting.SQL, curChooseDataSetting.IsNeedDrugType, curChooseDataSetting.Filter);
        ...
        protected int AddObjectToDataTable(FS.HISFC.Models.Pharmacy.InPlan plan)
        {
            ...
            if (plan.PlanQty == 0)
            {
                //row["计划量"] = (FS.FrameWork.Function.NConvert.ToDecimal(plan.Extend) / plan.Item.PackQty).ToString("F0");
                row["计划量"] = Math.Ceiling(FS.FrameWork.Function.NConvert.ToDecimal(plan.Extend) / plan.Item.PackQty);
                qty =  FS.FrameWork.Function.NConvert.ToDecimal(plan.Extend);
            }
            else
            {
                //row["计划量"] = plan.PlanQty / plan.Item.PackQty;
                row["计划量"] = Math.Ceiling(plan.PlanQty / plan.Item.PackQty);
            }
            
            //row["审批量"] = plan.StockQty / plan.Item.PackQty;
            if (plan.StockQty != 0)
            {
                row["审批量"] = Math.Ceiling(plan.StockQty / plan.Item.PackQty);
            }
            else
            {
                row["审批量"] = Math.Ceiling(FS.FrameWork.Function.NConvert.ToDecimal(plan.Extend) / plan.Item.PackQty);
            }
            //row["入库量"] = plan.InQty / plan.Item.PackQty;
            row["入库量"] = Math.Ceiling(plan.InQty / plan.Item.PackQty);
            ...
            this.totRowQty++;
            this.totPurhanceCost += FS.FrameWork.Function.NConvert.ToDecimal(row["金额"]);
            if (this.curIDataDetail != null)
            {
                this.curIDataDetail.Info = "当前录入品种数：" + this.totRowQty.ToString("F0") + ", 购入总额：" + this.totPurhanceCost.ToString("F2").TrimEnd('0').TrimEnd('.'); ;
            }
        ...
        protected bool GiveUpChanges()
        {
            ...
            if (this.dtDetail.GetChanges(DataRowState.Added | DataRowState.Modified) != null)
            {
                ...
                if (this.class2Code == "0312" && (this.curPlan.State == "0" || string.IsNullOrEmpty(this.curPlan.State)))
                {
                    isChanged = true;
                }
                else if (this.class2Code != "0312" && (this.curPlan.State == "0" ||this.curPlan.State == "1"))
                {
                    isChanged = true;
                }
            }
        ...
        protected int SetPlan()
        {
            ...
            //通过本地化算法获取内部入库申请数据表
            //各项目可以用医院自定义规则生成计划
            this.alFormulaPlan = Function.SetInputPlan(this.priveDept.ID, alData);
        ...
        protected int Save()
        {
            ...
            if (this.curPlan.State == "1" && this.class2Code=="0312")
            {
            ...
            Hashtable hsComplanForBillNo = new Hashtable();
            ...
            else
            {
                ...
                foreach (DataRow dr in this.dtDetail.Rows)
                {
                    if (!hsComplanForBillNo.ContainsKey(dr["供货公司"].ToString()))
                    {
                        ...
                        //“入库计划制定”内选择删除0的项目实际未删除，会变成理论数量。“入库计划”会被分单，多个单“入库计划”排序不对
                        if (string.IsNullOrEmpty(strBillNO))
                        {
                            strBillNO = this.curPlan.BillNO;
                        }
                        else
                        {
                            strBillNO = (FS.FrameWork.Function.NConvert.ToDecimal(strBillNO) + 1).ToString();
                        }
                        hsComplanForBillNo.Add(dr["供货公司"].ToString(), strBillNO);
                    }
                }
            }
            ...
            else
            {
                ...
                foreach (DataRow dr in this.dtDetail.Rows)
                {
                    ...
                    plan.BillNO = hsComplanForBillNo[dr["供货公司"].ToString()].ToString();//this.curPlan.BillNO;                     //计划单号
                    plan.PlanType = this.curPlan.PlanType;                    //采购类型
                    //单据状态
                    if (plan.State != "2")
                    {
            ...
            if (Function.DealExtendBiz(this.class2Code, curPriveType.ID, alPlan, ref errInfo) == -1)
            {
            ...
            #region 打印
            CompareByComplan ic = new CompareByComplan();
            alPlan.Sort(ic);
            ArrayList alTempALL = new ArrayList();
            string strTemp="千万不要有相同的，有相同的我不玩了";
            ArrayList alTemp = new ArrayList();
            foreach (FS.HISFC.Models.Pharmacy.InPlan plan in alPlan)
            {
                if (strTemp == plan.Company.Name)
                {
                    alTemp.Add(plan);
                }
                else
                {
                    if (alTemp.Count > 0)
                    {
                        alTempALL.Add(alTemp);
                    }
                    alTemp = new ArrayList();
                    alTemp.Add(plan);
                    strTemp = plan.Company.Name;
                }
            }
            alTempALL.Add(alTemp);
            foreach (ArrayList alPrint in alTempALL)
            {
                //Function.PrintBill(this.class2Code, curPriveType.ID, alPrint);
            }
            #endregion
        ...
        void IDataChooseList_ChooseCompletedEvent()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    //    return;
                    //}
                    //{5DDC1B83-0693-4949-93A0-98FAC0630510}获取药房药库映射科室库存
                    if (this.itemMgr.GetStorageNum1(this.priveDept.ID, plan.Item.ID, out storageNum) == -1)
                    {
                        Function.ShowMessage("获取库存数量时出错，请与系统管理员联系！\n报告错误：" + this.itemMgr.Err, MessageBoxIcon.Error);
                        return;
                    }
                    ...
                    //{5DDC1B83-0693-4949-93A0-98FAC0630510}获取全院科室库存
                    decimal hospitalNum = 0;
                    if (this.itemMgr.GetHospitalNum1(plan.Item.ID, out hospitalNum) == -1)
                    {
                        Function.ShowMessage("获取全院库存数量时出错，请与系统管理员联系！\n报告错误：" + this.itemMgr.Err, MessageBoxIcon.Error);
                        return;
                    }
                    plan.StoreTotQty = hospitalNum;
        ...
        protected virtual int Print()
        {
            if (this.dtDetail.Rows.Count == 0)
            {
                return Function.PrintBill("0312", "01", null);
            }
            ...
            return Function.PrintBill("0312", "01", alDetail);
        }
        #endregion
        ...
        public int SetTotInfo()
        {
            this.totPurhanceCost = 0;
            this.totRowQty = 0;
            for (int rowIndex = 0; rowIndex < this.curIDataDetail.FpSpread.Sheets[0].Rows.Count; rowIndex++)
            {
                string keys = this.curIDataDetail.FpSpread.GetCellText(0, rowIndex, "药品编码");

                if (hsPlan.Contains(keys))
                {
                    FS.HISFC.Models.Pharmacy.InPlan inPlan = hsPlan[keys] as FS.HISFC.Models.Pharmacy.InPlan;
                    string strQty = string.Empty;
                    if (this.class2Code == "0312")
                    {
                        strQty = this.curIDataDetail.FpSpread.GetCellText(0, rowIndex, "计划量");
                    }
                    else if(this.class2Code == "0313")
                    {
                        strQty = this.curIDataDetail.FpSpread.GetCellText(0, rowIndex, "审批量");
                    }
                    decimal qty = FS.FrameWork.Function.NConvert.ToDecimal(strQty);

                    decimal purchaseCost = inPlan.Item.PriceCollection.PurchasePrice * qty;
                    purchaseCost = FS.FrameWork.Function.NConvert.ToDecimal(purchaseCost.ToString("F2"));

                    this.dtDetail.Columns["金额"].ReadOnly = false;
                    this.curIDataDetail.FpSpread.SetCellValue(0, rowIndex, "金额", purchaseCost);
                    this.dtDetail.Columns["金额"].ReadOnly = true;

                    this.totRowQty++;
                    this.totPurhanceCost += purchaseCost;


                    this.curIDataDetail.Info = "当前品种数：" + this.totRowQty.ToString("F0")
                       + ", 购入总额：" + this.totPurhanceCost.ToString("F2").TrimEnd('0').TrimEnd('.');
        ...
        internal class CompareByComplan : System.Collections.IComparer
        {
            public int Compare(object x, object y)
            {
                FS.HISFC.Models.Pharmacy.InPlan oX = new FS.HISFC.Models.Pharmacy.InPlan();
                FS.HISFC.Models.Pharmacy.InPlan oY = new FS.HISFC.Models.Pharmacy.InPlan();
                if (x is FS.HISFC.Models.Pharmacy.InPlan && y is FS.HISFC.Models.Pharmacy.InPlan)
                {
                    oX = x as FS.HISFC.Models.Pharmacy.InPlan;
                    oY = y as FS.HISFC.Models.Pharmacy.InPlan;
                }
                int nComp;

                if (string.IsNullOrEmpty(oX.Company.Name.ToString()))
                {
                    nComp = (string.IsNullOrEmpty(oY.Company.Name.ToString())) ? 1 : 0;
                }
                else if (string.IsNullOrEmpty(oY.Company.Name.ToString()))
                {
                    nComp = 1;
                }
                else
                {
                    nComp = string.Compare(oX.Company.Name.ToString(), oY.Company.Name.ToString());
                }
