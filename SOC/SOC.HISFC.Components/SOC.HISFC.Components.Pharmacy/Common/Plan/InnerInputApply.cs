...
namespace FS.SOC.HISFC.Components.Pharmacy.Common.Plan
{
    ...
    /// [创建时间: 2011-3]<br></br>
    /// </summary>
    public class InnerInputApply : Base.IBaseBiz
    {
        ...
        public ArrayList GetFromDeptArray(ref string fromDeptTypeName)
        {
            ...
            return Function.QueryManagerStockDept();
        }
        ...
        protected int Save()
        {
            ...
            foreach (DataRow dr in this.dtDetail.Rows)
            {
                ...
                if (applyOut.ID == "")
                {
                    ...
                    if (billNO == "")
                    {
                        billNO = Function.GetBillNO(this.curStockDept.ID, "0310", this.curPriveType.ID, ref errInfo);
            ...
            if (Function.DealExtendBiz("0310", this.PriveType.ID, alInnerInputApply, ref errInfo) == -1)
            {
            ...
            Function.PrintBill("0310", this.PriveType.ID, alInnerInputApply);
        ...
        private int PrintApplyBill()
        {
            ...
            {
                Function.PrintBill("0310", this.PriveType.ID, allApplyOutData);
            }
            return 1;
        }
        #endregion
        ...
        protected int SetApply()
        {
            ...
            //通过本地化算法获取内部入库申请数据表
            //各项目可以用医院自定义规则生成计划
            this.alFormulaPlan = Function.SetInnerInputApply(this.curStockDept.ID, this.curFromDept.ID, alData);
            ...
            for (int rowIndex = 0; rowIndex < this.curIDataDetail.FpSpread.Sheets[0].Rows.Count; rowIndex++)
            {
                ...
                string strQty2 = this.curIDataDetail.FpSpread.GetCellText(0, rowIndex, "申请科室库存");
                if (FS.FrameWork.Function.NConvert.ToDecimal(strQty1) > FS.FrameWork.Function.NConvert.ToDecimal(strQty1))
                {
        ...
        protected virtual int SetTotInfo()
        {
            ...
            foreach (DataColumn dc in this.dtDetail.Columns)
            {
                if (dc.ColumnName == "购入金额" || dc.ColumnName == "零售金额")
                {
                    dc.ReadOnly = false;
                }
            }
