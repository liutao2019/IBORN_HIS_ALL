...
{
    public partial class ucChangeCompanyDilverTime : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Classes.IPreArrange
    {
        public ucChangeCompanyDilverTime()
        {
            InitializeComponent();
        ...
        private void Query()
        {
            ...
            foreach (FS.HISFC.Models.Pharmacy.Input info in al)
            {
                ...
                this.neuSpread1_Sheet1.Cells[0, 3].Text = info.User01.ToString();
        ...
        private void Save()
        {
            ...
            if (string.IsNullOrEmpty(this.lbDileverTime.Text))
            {
                Function.ShowMessage("新送单日期没有输入，请输入送单日期", MessageBoxIcon.Information);
                return;
            }
            ...
            for (int i = 0; i < this.neuSpread1_Sheet1.Rows.Count; i++)
            {
                ...
                //if (FS.FrameWork.Function.NConvert.ToBoolean(this.neuSpread1_Sheet1.Cells[i, 0].Value))
                {
                    ...
                    if (itemManager.UpdateInputCompanyDiliverTime(input.ID.ToString(), input.SerialNO.ToString(), this.lbDileverTime.Text) != 1)
                    {
                        FS.FrameWork.Management.PublicTrans.RollBack();
                        Function.ShowMessage("更新送单日期信息失败，数据可能已经核准",MessageBoxIcon.Information);
                        return;
                    }
                }
            }
        ...
        private void lbDileverTime_Leave(object sender, EventArgs e)
        {
            this.lbDileverTime.Text = this.TryConvertToDateTimeFormat(this.lbDileverTime.Text);
        }

        private void lbDileverTime_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)13)
            {
                this.lbDileverTime_Leave(sender, e);
            }
        }
