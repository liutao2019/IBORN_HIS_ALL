...
namespace FS.SOC.HISFC.Components.Pharmacy.Common.Output
{
    ...
    /// 2、暂存按钮插入出库申请表
    /// </summary>
    public class ApplyOutOutput : Base.IBaseBiz
    {
        ...
        public int Init(FS.SOC.HISFC.BizProcess.PharmacyInterface.Pharmacy.IDataChooseList IDataChooseList, SOC.HISFC.BizProcess.PharmacyInterface.Pharmacy.IDataDetail IDataDetail, ToolStrip toolStrip)
        {
            ...
            if (this.curPriveType == null)
            {
                ...
                curPriveType.ID = "33";
                curPriveType.Name = "出库申请";
                curPriveType.Memo = "24";
            }
        ...
        {
            ArrayList alOutDept = Function.QueryManagerStockDept(this.curStockDept.ID, "0320");
            if (alOutDept == null)
            {
        ...
        protected int InitToolStrip(ToolStrip toolStrip)
        {
            ...
            tb1.Image = FS.FrameWork.WinForms.Classes.Function.GetImage(FS.FrameWork.WinForms.Classes.EnumImageList.S申请单);
            tb1.ToolTipText = "显示申请单列表";
            ...
            tb.Image = FS.FrameWork.WinForms.Classes.Function.GetImage(FS.FrameWork.WinForms.Classes.EnumImageList.Z暂存);
            tb.ToolTipText = "暂存出库记录";
            tb.ImageScaling = ToolStripItemImageScaling.SizeToFit;
            tb.TextImageRelation = TextImageRelation.ImageAboveText;
            toolStrip.Items.Insert(1, tb);   
        ...
        {
            string errInfo = "";
            FS.SOC.HISFC.BizProcess.PharmacyInterface.Pharmacy.ChooseDataSetting chooseDataSetting = Function.GetBizChooseDataSetting("0320", curPriveType.Memo, curPriveType.ID, "0", ref errInfo);
        ...
        #region 暂存按钮调用
        public int SaveTemporary()
        {
            if (this.dtDetail.Rows.Count == 0)
            {
                Function.ShowMessage("当前没有暂存数据！", MessageBoxIcon.Information);
                return 0;
            }
            ...
            DialogResult rs = MessageBox.Show("确认进行暂存操作吗?", "提示>>", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1);
            ...
            FS.FrameWork.WinForms.Classes.Function.ShowWaitForm("正在进行暂存操作..请稍候");
            ...
            foreach (DataRow dr in dtAddMofity.Rows)
            {
                ...
                if (temporaryBillNo == "")
                {
                    temporaryBillNo = Function.GetApplyListNO(this.curStockDept.ID);
                    if (string.IsNullOrEmpty(temporaryBillNo) || temporaryBillNo == "-1")
                    {
                ...
                applyOut.Days = 1;
                applyOut.SystemType = "24";
                applyOut.StockDept.ID = this.curStockDept.ID;
                ...
                applyOut.BillNO = temporaryBillNo;
                applyOut.State = "0";
                FS.HISFC.Models.Pharmacy.Item item = this.itemMgr.GetItem(output.Item.ID);
                applyOut.Item = item.Clone();
                applyOut.Operation.ApplyQty = output.Quantity;
                if (!string.IsNullOrEmpty(output.ShowState))
                { 
                    applyOut.ShowState = output.ShowState;
                }
                if (!string.IsNullOrEmpty(output.ShowUnit))
                {
                    applyOut.ShowUnit = output.ShowUnit;
                }

                int returnValue = this.itemMgr.InsertApplyOut(applyOut);

                if (returnValue == -1)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                    Function.ShowMessage("插入出库申请表失败:" + errInfo, MessageBoxIcon.Error);
                    return -1;
                }
        ...
        public int ChooseApplyBill()
        {
            ...
            FS.SOC.HISFC.BizProcess.PharmacyInterface.Pharmacy.ChooseDataSetting chooseDataSetting = Function.GetBizChooseDataSetting("0320", curPriveType.Memo, curPriveType.ID, "3", ref errInfo);
            ...
            if (chooseDataSetting.IsDefault)
            {
                ...
                string SQL = "";
                if (this.itemMgr.Sql.GetSql("SOC.Pharmacy.Output.ApplyOutPrive.ChooseBill", ref SQL) == -1)
                {
                    Function.ShowMessage("没有找到SQL：SOC.Pharmacy.Output.ApplyOutPrive.ChooseBill,请与系统管理员联系！", MessageBoxIcon.Error);
                    return -1;
                }

                string curDeptNO = "All";
                if (this.curStockDept != null && this.curStockDept.ID != "")
                {
                    curDeptNO = this.curStockDept.ID;
                }
                //替换库存科室和供货公司，时间和状态不替换
                SQL = string.Format(SQL,  this.curStockDept.ID, "{0}");

                chooseDataSetting.ListTile = "暂存单列表";
                chooseDataSetting.SQL = SQL;
                chooseDataSetting.ColumnIndexs = new int[] { 0, 1, 2 };
                ...
                chooseDataSetting.SettingFileName = FS.FrameWork.WinForms.Classes.Function.CurrentPath + "\\Profile\\ApplyOutputL1Setting.xml";
            }
        ...
        void frmBillChooseList_ChooseCompletedEvent()
        {
            ...
            else
            {
                ...
                FS.HISFC.Models.Pharmacy.ApplyOut applyOutData = alApplyout[0] as FS.HISFC.Models.Pharmacy.ApplyOut;
                FS.FrameWork.Management.PublicTrans.BeginTransaction();
                foreach (FS.HISFC.Models.Pharmacy.ApplyOut applyOut in alApplyout)
                {
                    ...
                    {
                        FS.FrameWork.Management.PublicTrans.RollBack();
                        Function.ShowMessage("请与系统管理员联系：删除申请信息出错：" + this.itemMgr.Err, MessageBoxIcon.Error);
                        return;
                    }
