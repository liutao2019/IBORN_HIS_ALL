...
{
    /// <summary>
    /// [功能描述: 药库按批特殊出库：出库时指定某个批次]<br></br>
    /// [创 建 者: zhaozf]<br></br>
    /// [创建时间: 2011-6-21]<br></br>
    /// 说明：
    /// 1、按批特殊出库不增加对方科室库存
    /// 2、按批特殊出库不允许向管理库存的科室操作    
    /// </summary>
    public class BatchSpecialOutput : Base.IBaseBiz
    {
        ...
        protected int InitDataTable()
        {
            ...
            {
                if (dc.ColumnName == "出库量" || dc.ColumnName == "零售金额"||dc.ColumnName=="购入金额")
                {
                    continue;
                }
                dc.ReadOnly = true;
