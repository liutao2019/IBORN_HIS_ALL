...
namespace FS.SOC.HISFC.Components.PharmacyCommon
{
    ...
    /// [创建时间: 2006-11]<br></br>
    /// </summary>
    public partial class tvDeptTree : FS.SOC.HISFC.Components.Common.Base.baseTreeView
    {
        public tvDeptTree()
        {
            InitializeComponent();
        ...
        /// 其他科室
        /// </summary>
        List<FS.HISFC.Models.Base.Department> cList = null;
        ...
        /// 是否显示其他科室
        /// </summary>
        public bool IsShowCommonDept
        {
            get { return isShowCommonDept; }
            set
            {
                isShowCommonDept = value;
                this.InitTree();
            }
        }
        ...
        protected virtual int InitDept()
        {
            ...
            cList = new List<FS.HISFC.Models.Base.Department>();

            #region 按照科室类型加载

            ArrayList alPharmacyStatDept = new ArrayList();

            if (this.IsShowCommonDept)
            {
                FS.SOC.HISFC.BizLogic.Pharmacy.Constant phaConsManager = new FS.SOC.HISFC.BizLogic.Pharmacy.Constant();
                alPharmacyStatDept = phaConsManager.QueryPharmacyStatList();
            }
            ...
            foreach (FS.HISFC.Models.Base.Department dept in deptList)
            {
                ...
                if (this.IsShowCommonDept)
                {
                    foreach (FS.HISFC.Models.Pharmacy.DeptConstant d in alPharmacyStatDept)
                    {
                        if (d.ID == dept.ID)
                        {
                            cList.Add(dept);
        ...
        protected virtual int InitTree()
        {
            ...
            TreeNode commonDeptRootNode = new TreeNode("基药", 0, 0);
            ...
            if (this.IsShowCommonDept)
            {
                foreach (FS.HISFC.Models.Base.Department dept in cList)
                {
                    TreeNode node = new TreeNode(dept.Name);
                    node.ImageIndex = 4;
                    node.SelectedImageIndex = 5;
                    ...
                    node.Tag = dept;
                    commonDeptRootNode.Nodes.Add(node);
                }
                this.Nodes.Add(commonDeptRootNode);
            }
