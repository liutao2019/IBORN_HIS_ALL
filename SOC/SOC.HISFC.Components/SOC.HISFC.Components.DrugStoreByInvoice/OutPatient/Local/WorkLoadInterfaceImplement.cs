...
namespace SOC.HISFC.Components.DrugStoreByInvoice.OutPatient.Local
{
    ...
    /// </summary>
    public class WorkLoadInterfaceImplement : SOC.HISFC.Components.DrugStoreByInvoice.OutPatient.Local.Interface.IOutpatientWorkLoadBatch
    {
        ...
        public string Set(FS.FrameWork.Models.NeuObject dept, FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe, string type, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal)
        {
            ...
            {
                //直接发药，计入发药工作量
                frmWorkLoad frmWorkLoad = new frmWorkLoad("1", drugRecipe.StockDept.ID, dept.ID, drugRecipe.RecipeQty, drugRecipe.RecipeNO, drugRecipe.PatientName);
                frmWorkLoad.ShowDialog();
            }
            else if (type == "1")
            {
                //配药发药，计入配药工作量
                frmWorkLoad frmWorkLoad = new frmWorkLoad("0", drugRecipe.StockDept.ID, dept.ID, drugRecipe.RecipeQty, drugRecipe.RecipeNO, drugRecipe.PatientName);
                frmWorkLoad.ShowDialog();
            }
            else if (type == "2")
            {
        ...
        #region IOutpatientWorkLoadBatch 成员


        public string Set(FS.FrameWork.Models.NeuObject dept, List<FS.HISFC.Models.Pharmacy.DrugRecipe> drugRecipe, string type, FS.HISFC.Models.Pharmacy.DrugTerminal drugTerminal)
        {
            if (type == "0")
            {
                //直接发药，计入发药工作量
                frmWorkLoad frmWorkLoad = new frmWorkLoad("1", dept.ID, drugRecipe);
                frmWorkLoad.ShowDialog();
            }
            else if (type == "1")
            {
                //配药发药，计入配药工作量
                frmWorkLoad frmWorkLoad = new frmWorkLoad("0", dept.ID,drugRecipe);
                frmWorkLoad.ShowDialog();
            }
            else if (type == "2")
            {
                //发药，计入发药工作量
                Common.WorkLoadManager workLoadManager = new Common.WorkLoadManager();
                foreach (FS.HISFC.Models.Pharmacy.DrugRecipe dr in drugRecipe)
                {
                    workLoadManager.SetOutpatientWorkLoad(dr.RecipeNO, dr.StockDept.ID, dept.ID, "1", dr.SendOper.ID, dr.RecipeQty);
                }
            }
            return "";
        }
