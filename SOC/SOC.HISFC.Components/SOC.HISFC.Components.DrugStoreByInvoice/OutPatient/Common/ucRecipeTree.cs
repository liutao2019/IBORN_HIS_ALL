...
namespace SOC.HISFC.Components.DrugStoreByInvoice.OutPatient.Common
{
    ...
    public partial class ucRecipeTree : UserControl
    {
        ...
        /// 是否允许按工号查询
        /// </summary>
        public bool isQueryByEmpCode = true;
        public bool IsQueryByEmpCode
        {
            get{ return isQueryByEmpCode;}
            set{ this.isQueryByEmpCode = value; }
        }
        ...
        /// 用于标记是否手工保存
        /// </summary>
        public bool isMarkHandSave = true;
        public Boolean IsMarkHandSave
        {
            get { return isMarkHandSave; }
            set { this.isMarkHandSave = value; }
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!DesignMode)
            {
                try
                {
                    ...
                    queryType = FS.SOC.Public.XML.SettingFile.ReadSetting(FS.FrameWork.WinForms.Classes.Function.CurrentPath + "\\Profile\\OutpatientDrugStore.xml", "ReipeQueryType", "Dept" + this.PriveDept.ID + "Type", queryType);
        ...
        void neuTabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else if (this.neuTabControl1.SelectedTab == tabPage2)
            {
                if (this.tvBaseTree2.Nodes.Count > 0)
                {
                    ...
                    {
                        this.ShowRecipeInfo(this.tvBaseTree2.SelectedNode,this.isShowInvoice);
                    }
                }
            }
        }
        ...
        public int ShowRecipeList(ArrayList alDrugRecipe, bool isSupplemental, string state,bool isShowInvoice)
        {
            ...
            {
                this.tvBaseTree2.ShowList(alDrugRecipe, isSupplemental, isShowInvoice);
            }
            else
            {
                return -1;
        ...
        public int MoveNode(int state1, int state2)
        {
            try
            {
                SOC.HISFC.Components.DrugStoreByInvoice.OutPatient.Base.tvRecipeBaseTree tv1 = new SOC.HISFC.Components.DrugStoreByInvoice.OutPatient.Base.tvRecipeBaseTree();
                if (state1 == 0)
                {
                ...
                SOC.HISFC.Components.DrugStoreByInvoice.OutPatient.Base.tvRecipeBaseTree tv2 = new SOC.HISFC.Components.DrugStoreByInvoice.OutPatient.Base.tvRecipeBaseTree();
                if (state2 == 0)
                {
                ...
                {  
                    if (node.Tag != null && node.Tag is FS.HISFC.Models.Pharmacy.DrugRecipe)
                    {
                        ...
                        al.Add(dr);
                        tv2.ShowList(al, true, isShowInvoice);
                        return 1;
                    }
                }
                else
                {
                    if (node.Tag != null && node.Tag is List<FS.HISFC.Models.Pharmacy.DrugRecipe>)
                    {
                        ArrayList al = new ArrayList();
                        List<FS.HISFC.Models.Pharmacy.DrugRecipe> allDrugRecipe = node.Tag as List<FS.HISFC.Models.Pharmacy.DrugRecipe>;
                        foreach (FS.HISFC.Models.Pharmacy.DrugRecipe dr in allDrugRecipe)
                        {
                            dr.RecipeState = this.tvBaseTree2.State;
                            tv1.Nodes.Remove(node);
                            if (isQueryByEmpCode)
                            {
                                this.ntxtBillNO.Text = "";
                                this.ntxtBillNO.Focus();
                                this.ntxtBillNO.Select();
                                this.ntxtBillNO.SelectAll();
                            }
        ...
        private int QueryRecipe()
        {
            ...
            if (isQueryByEmpCode)
            {
                ...
                employee = personMgr.GetPersonByID(this.ntxtBillNO.Text.Trim());


                if (employee != null && !string.IsNullOrEmpty(employee.ID))
                {
                    queryNO = employee.ID.ToString();
                }
                else
                {
            ...
            if (isQueryByEmpCode)
            {
                ...
                findedType1 = this.FindRecipeAtTreeByEmpCode(this.tvBaseTree1, queryNO);
            }
            else
            {
        ...
        /// <returns></returns>
        private int FindRecipeAtTreeByEmpCode(SOC.HISFC.Components.DrugStoreByInvoice.OutPatient.Base.tvRecipeBaseTree tv, string queryNO)
        {
            //增加工号长度为6的判断
            if (string.IsNullOrEmpty(queryNO) || queryNO.Length != 6)
            {
                return -1;
            }
            if (tv.Nodes.Count == 0)
            ...
            foreach (TreeNode node in tv.Nodes)
            {
                if (!node.Text.Contains("(手工方)"))
                {
                    tv.SelectedNode = node;
                    this.isMarkHandSave = false;
                    this.operCode = queryNO;
                    return 1;
                }
            }
            return -1;
        ...
        private int FindRecipeAtDataBase()
        {
            ...
            if ((this.tvBaseTree1.State + this.tvBaseTree2.State != "01") && (this.tvBaseTree1.State + this.tvBaseTree2.State != "10"))
            {
                ...
                if (al.Count > 0)
                {
                    ...
                    {
                        this.ShowMessage("您查找的处方处方还没有配药！");
                        return 2;
                    }
                }
            ...
            //配发药流程：配药台补充3状态的查询
            if ((this.tvBaseTree1.State + this.tvBaseTree2.State == "01") || (this.tvBaseTree1.State + this.tvBaseTree2.State == "10") || (this.tvBaseTree1.State + this.tvBaseTree2.State == "02") || (this.tvBaseTree1.State + this.tvBaseTree2.State == "20"))
            {
