...
namespace SOC.HISFC.Components.DrugStoreByInvoice.OutPatient.Common
{
    ...
    public partial class ucDrug : ucDrugBase, FS.FrameWork.WinForms.Classes.IPreArrange
    {
        ...
        ///是否按工号查询
        /// </summary>
        public bool isQueryByEmpCode = false;
        [Description("是否工号查询列表"), Category("设置"), Browsable(true)]
        public bool IsQueryByEmpCode
        {
            get { return isQueryByEmpCode; }
            set { isQueryByEmpCode = value; }
        }

        /// <summary>
        ///手工保存是否记录工作量
        /// </summary>
        public bool isSavWorkLoadByHand = false;
        [Description("手工保存是否记录工作量"), Category("设置"), Browsable(true)]
        public bool IsSaveWorkLoadByHand
        {
            get { return isSavWorkLoadByHand; }
            set { isSavWorkLoadByHand = value; }
        }
        ...
        private void Init()
        {
            ...
            if (this.isAutoPrint)
            {
                this.ucDrugTree1.TabPage1Name = "已打印处方列表F11";
                this.ucDrugTree1.TabPage2Name = "未打印处方列表F12";
                this.ucDrugTree1.SetTreeState("1", "0");
            }
            else
            {
                this.ucDrugTree1.isQueryByEmpCode = this.isQueryByEmpCode;
                this.ucDrugTree1.TabPage1Name = "待配药处方列表F11";
                this.ucDrugTree1.TabPage2Name = "已配药处方列表F12";
                this.ucDrugTree1.SetTreeState("1", "2");
            }
        }
        ...
        private int QueryOnLoad()
        {
            ...
            if (this.isAutoPrint)
            {
                ...
                if (al1 != null)
                {
                    this.ucDrugTree1.ShowRecipeList(al1, true, "0", isShowByInvoice);
                }
            }
            else
            {
                ArrayList al1 = drugStoreMgr.QueryList(this.PriveDept.ID, terminalNO, type, "2", manualRefreshBeginTime);
                if (al1 != null)
                {
                    this.ucDrugTree1.ShowRecipeList(al1, true, "1", isShowByInvoice);
                }
                ArrayList al2 = drugStoreMgr.QueryList(this.PriveDept.ID, terminalNO, type, "1", manualRefreshBeginTime);
                {
        ...
        private int ManulRefresh()
        {
            ...
            else
            {
                ...
                if (al2 != null)
                {
                    this.ucDrugTree1.ShowRecipeList(al2, true, "1", isShowByInvoice);
                }
            }
            ...
            if (al0 != null)
            {
                ...
                if (this.isAutoPrint)
                {
                    this.ucDrugTree1.ShowRecipeList(al0, true, "0", isShowByInvoice);
                }
                else
                {
                    this.ucDrugTree1.ShowRecipeList(al0, true, "0", isShowByInvoice);
        ...
        private void ShowInfo()
        {
            ...
            else
            {
                ...
                //原有根据处方号查询出库申请信息屏蔽
                //ArrayList alRecipeDetail = this.drugStoreMgr.QueryApplyOutListForClinic(this.PriveDept.ID, "M1", state, this.ucDrugTree1.DrugRecipe.RecipeNO);
                ArrayList alRecipeDetail = this.drugStoreMgr.QueryApplyOutListForInvoiceNO(this.priveDept.ID, "M1", state, this.ucDrugTree1.DrugRecipe.InvoiceNO);
                if (alRecipeDetail == null)
                {
                    this.ShowMessage("查询处方明细出错：" + drugStoreMgr.Err, MessageBoxIcon.Error);
                    return;
        ...
        private int Save()
        {
            ...
            else
            {
                ...
                foreach (FS.HISFC.Models.Pharmacy.ApplyOut applyInfo in alData)
                {
                    ...
                    {
                        List<FS.HISFC.Models.Pharmacy.ApplyOut> al = hsRecipe[applyInfo.RecipeNO] as List<FS.HISFC.Models.Pharmacy.ApplyOut>;
                        al.Add(applyInfo);
                    }
                    else
                    {
                        List<FS.HISFC.Models.Pharmacy.ApplyOut> allDataTemp = new List<FS.HISFC.Models.Pharmacy.ApplyOut>();
                ...
                foreach (String recipeNo in allRecipe)
                {
                    ...
                    //配药保存：更新调剂数
                    parm = Function.OutpatientDrug(allRecipeData, DrugTerminal, this.StockDept, this.drugStoreMgr.Operator, true);
            ...
            if (this.IsSetWorkLoad)
            {
                if (isQueryByEmpCode)
                {
                    if (!this.ucDrugTree1.isMarkHandSave)
                    {
                        List<FS.HISFC.Models.Pharmacy.DrugRecipe> alldr = this.ucDrugTree1.AllDrugRecipe;
                        foreach (FS.HISFC.Models.Pharmacy.DrugRecipe drugRcipe in alldr)
                        {
                            drugRcipe.RecipeState = "2";
                            drugRcipe.DrugedOper.ID = this.drugStoreMgr.Operator.ID;
                            drugRcipe.DrugedOper.Name = this.drugStoreMgr.Operator.Name;
                            Local.Common.WorkLoadManager workLoadManager = new Local.Common.WorkLoadManager();
                            if (!string.IsNullOrEmpty(this.ucDrugTree1.operCode))
                            {
                                workLoadManager.SetOutpatientWorkLoad(drugRcipe.RecipeNO, dr.StockDept.ID, this.StockDept.ID, "0", this.ucDrugTree1.operCode.ToString(), drugRcipe.RecipeQty);
                            }
                    ...
                    else
                    {
                        if (this.IsSaveWorkLoadByHand)
                        {
                            ...
                            this.IOutpatientWorkLoad.Set(this.StockDept, alldr, "1", this.DrugTerminal);
                        }
                    }
