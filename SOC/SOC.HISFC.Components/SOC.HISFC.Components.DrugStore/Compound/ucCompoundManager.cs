...
namespace FS.SOC.HISFC.Components.DrugStore.Compound
{
    ...
    public partial class ucCompoundManager : FS.FrameWork.WinForms.Controls.ucBaseControl,FS.HISFC.BizProcess.Interface.Pharmacy.ICompoundGroup
    {
        ...
        /// <summary>
        /// 是否是护士站操作，为护士站修改批次准备的参数
        /// {7C687F9C-A2E7-48be-A0D6-BEFD912B864F}
        /// </summary>
        private bool isOperNurse = false;

        /// <summary>
        /// 配液确认时打印标签
        /// {7C687F9C-A2E7-48be-A0D6-BEFD912B864F}
        /// </summary>
        private bool isConfimPrint = false;
        ...
        /// 单据打印接口
        /// </summary>
        private FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.ICompoundPrint iCompoundPrintObj = null;

        /// <summary>
        /// 辅材收费接口
        /// </summary>
        private FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.ICompoundFee iCompoundFeeObj = null;
        ...
        /// {7C687F9C-A2E7-48be-A0D6-BEFD912B864F}
        /// </summary>
        [Description("是否是护士站登录操作 True 提供给护士站操作 False 提供给配液中心操作 "), Category("设置")]
        public bool IsOperNurse
        {
            get 
            {
                return this.isOperNurse;
            }
            set
            {
                this.isOperNurse = value;
            }
        }

        /// <summary>
        /// 是否需要配液确认，原有变量，但无界面参数，增加
        /// {7C687F9C-A2E7-48be-A0D6-BEFD912B864F}
        /// </summary>
        [Description("是否需要确认 false 不需要 true 需要确认"), Category("设置")]
        ...
        /// {7C687F9C-A2E7-48be-A0D6-BEFD912B864F}
        /// </summary>
        [Description("是否配液确认时打印标签 false 不是 true 是"), Category("设置")]
        public bool IsConfimPrint
        {
            get
            {
                return this.isConfimPrint;
            }
            set
            {
                this.isConfimPrint = value;
            }
        }
        ...
        private int ModifyCompandGroup()
        {
            ...
            SOC.HISFC.BizLogic.Pharmacy.Compound itemManager = new SOC.HISFC.BizLogic.Pharmacy.Compound();
        ...
        private int PrintCancelDate()
        {
            ...
            foreach (FS.HISFC.Models.Pharmacy.ApplyOut unvalidData in al)
            {
                ...
                if (this.compoundMgr.UpdateApplyOut(unvalidData) == -1)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
        ...
        protected override int OnPrint(object sender, object neuObject)
        {
            ...
            if (this.currentWinFun == WinFun.Print)         //仅当打印状态下，才进行打印标记更新
            {
                ...
                {
                    if (this.compoundMgr.UpdateApplyOutPrintState(info.ID, true) == -1)
                    {
                        FS.FrameWork.Management.PublicTrans.RollBack();
                    ...
                    if (string.IsNullOrEmpty( info.DrugNO ) == true)
                    {
                        ...
                        if (this.compoundMgr.ExamApplyOut( info ) == -1)
                        {
                            FS.FrameWork.Management.PublicTrans.RollBack();
        ...
        protected virtual int Init()
        {
            ...
            ///若是护士站登录，则不允许看到卡片样式
            ///{7C687F9C-A2E7-48be-A0D6-BEFD912B864F} by yangxl
            if (this.isOperNurse)
            {
                this.IsShowCardStyle = false;
            }
            ...
            this.cmbOrderGroup.SelectionStart  = 0;
        ...
        private void InitInterface()
        {
            ...
            iCompoundFeeObj = FS.FrameWork.WinForms.Classes.UtilInterface.CreateObject(this.GetType(), typeof(FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.ICompoundFee)) as FS.SOC.HISFC.BizProcess.PharmacyInterface.DrugStore.ICompoundFee;
        }
        ...
        protected virtual int ShowList()
        {
            ...
            if (!isOperNurse)
            {
                ...
                {
                    //根据库存药房/批次获取列表
                    SOC.HISFC.BizLogic.Pharmacy.Item itemManager = new SOC.HISFC.BizLogic.Pharmacy.Item();
                    ...
                    {
                        alList =this.compoundMgr.QueryCompoundList(this.ApproveDept.ID, this.GroupCode, "0", "ALL", true);
                    }
                    else
                    {
                        if (this.isConfimPrint)
                        {
                            alList =this.compoundMgr.QueryCompoundList(this.ApproveDept.ID, this.GroupCode, "0", "ALL", true, true);
                        }
                        else
                        {
                            //此时可包含无效数据 以打印负单据
                            alList = this.compoundMgr.QueryCompoundList(this.ApproveDept.ID, this.GroupCode, "0", "ALL", false);
                        }
                    }
                    if (alList == null)
                    {
            ...
            else
            {
                ...
                if (this.DrugCode != "ALL")
                {
                    this.tvCompound.ShowListByParmacy(this.ApproveDept.ID, this.isOperNurse, ref this.alThisParList);
                }
                else
                {
                    //根据库存药房/批次获取列表
                    ...
                    List<FS.HISFC.Models.Pharmacy.ApplyOut> alListNurse = new List<FS.HISFC.Models.Pharmacy.ApplyOut>();

                    alListNurse = this.compoundMgr.QueryCompoundListByNurse("ALL", this.GroupCode, "0", "ALL", this.ApproveDept.ID);
                    ...
                    this.tvCompound.ShowList(alListNurse);
                }
        ...
        public virtual int SaveApply()
        {
            ...
            if (Inpatient.Common.Function.DrugConfirm(alCheck, null, null, this.ApproveDept.Clone(),ref  errInfo) == -1)
            {
                FS.FrameWork.Management.PublicTrans.RollBack();
                FS.FrameWork.WinForms.Classes.Function.HideWaitForm();
                return -1;
            ...
            if (!this.isNeedConfirm)
            {
                ...
                foreach (FS.HISFC.Models.Pharmacy.ApplyOut info in alCheck)
                {
                    if (this.compoundMgr.UpdateCompoundApplyOut(info, compoundOper, true) == -1)
                    {
                        FS.FrameWork.Management.PublicTrans.RollBack();
                        FS.FrameWork.WinForms.Classes.Function.HideWaitForm();
                        return -1;
            ...
            #region 辅材计费
            if (iCompoundFeeObj != null)
            {
                if (iCompoundFeeObj.SaveFee(alCheck, this.approveDept, FS.FrameWork.Management.PublicTrans.Trans) == -1)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                    FS.FrameWork.WinForms.Classes.Function.HideWaitForm();
                    MessageBox.Show(Language.Msg("配置收费发生错误,药品库存已扣除,") + this.iCompoundFeeObj.Err);
                    return -1;
                }
            }
            #endregion
            ...
            if (this.currentWinFun == WinFun.Save && this.isConfimPrint)         //仅当打印状态下，才进行打印标记更新
            {
                string drugBillNO = this.itemManager.GetNewDrugBillNO();
        ...
        public void Print(ArrayList alPrintData)
        {
            ...
            else
            {
                ...
                {
                    iCompoundPrintObj.PrintCompound(alData);       
                }
            }
        ...
        private void tvCompound_SelectDataEvent(ArrayList alData)
        {
            ...
            this.neuTabControl1.SelectedTab = this.tpListStyle;
        }
        ...
        private void chkPhamrcy_CheckedChanged(object sender, EventArgs e)
        {
            if (this.chkPhamrcy.Checked)
            {
                ...
                this.cmbOrderGroup.SelectedIndex = -1;
