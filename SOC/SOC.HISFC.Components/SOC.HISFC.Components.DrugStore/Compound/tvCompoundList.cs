...
namespace FS.SOC.HISFC.Components.DrugStore.Compound
{
    ...
    /// </说明>
    /// </summary>
    public partial class tvCompoundList : Inpatient.Base.tvMessageBaseTree
    {
        ...
        public delegate void GetSelectApplyDataHandler(FS.HISFC.Models.Pharmacy.ApplyOut info,string patientNO, string groupCode, string state, bool isExec, ref ArrayList al);

        public event GetSelectApplyDataHandler GetSelectApplyDataEvent;
        ...
        /// <param name="drugCode">药品编码</param>
        /// <param name="list">返回申请数组</param>
        public virtual void ShowListByParmacy(string stockDeptId, bool isOpNurse, ref ArrayList list)
        {
            if (isOpNurse)
            {
                this.Nodes.Clear();
                ...
                this.Nodes.Add(nodeDept);
                FS.SOC.HISFC.BizLogic.Pharmacy.Compound itemManager = new FS.SOC.HISFC.BizLogic.Pharmacy.Compound();
                list = itemManager.QueryCompoundApplyOutByNurse("ALL", stockDeptId, "U", null, "0", false);

            }
            else
            {
                this.ShowListByParmacy(stockDeptId, ref list);
            }
        }
        ...
        /// <param name="drugCode">药品编码</param>
        /// <param name="list">返回申请数组</param>
        public virtual void ShowListByParmacy(string stockDeptId, string sate,ref ArrayList list)
        {
            this.Nodes.Clear();
            TreeNode nodeDept = new TreeNode();
            nodeDept.Tag = "allDept";
            ...
            this.Nodes.Add(nodeDept);
            FS.SOC.HISFC.BizLogic.Pharmacy.Compound itemManager = new FS.SOC.HISFC.BizLogic.Pharmacy.Compound();
            list = itemManager.QueryCompoundApplyOut(stockDeptId, "ALL", "U", null, sate, false);
        }
        ...
        protected virtual void GetSelectData(TreeNode selectNode, ref ArrayList al)
        {
            ...
            {
                if (this.GetSelectApplyDataEvent != null)
                {
                    this.GetSelectApplyDataEvent(info,null, this.GroupCode, this.state, this.isExec, ref al);
                }
                else
                {
            ...
            else
            {
                if (this.GetSelectApplyDataEvent != null)
                {
                    this.GetSelectApplyDataEvent(info,info.PatientNO, this.GroupCode, this.state, this.isExec,ref al);
                }
                else
                {
