...
namespace FS.SOC.HISFC.Components.DrugStore.Maintenence
{
    ...
    public partial class ucGetDrugDept : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        //定义默认取药科室
        private FS.SOC.HISFC.BizLogic.Pharmacy.Constant phaConstant = new FS.SOC.HISFC.BizLogic.Pharmacy.Constant();
        //定义帮助
        ...
        private void SetFp()
        {
            ...
            string[] items = new string[] { "门诊处方","住院医嘱","全部"};
            ...
            this.neuSpread1_Sheet1.Columns.Get(5).Label = "发药类型";
            this.neuSpread1_Sheet1.Columns.Get(5).Width = 119F;
            this.neuSpread1_Sheet1.Columns.Get(5).CellType = comboBoxCellType;
        ...
        private void AddToDataSet(ArrayList al)
        {
            ...
            FS.FrameWork.WinForms.Classes.Function.ShowWaitForm(FS.FrameWork.Management.Language.Msg("正在检索数据.请稍候..."));
            ...
            for (int i = 0; i < al.Count; i++)
            {
                ...
                if (obj is FS.HISFC.Models.Base.Spell)
                {
                    spellobj = al[i] as FS.HISFC.Models.Base.Spell;
                }
                else
                {
                ...
                if (spellobj == null)
                {
                    ...
                    MessageBox.Show(FS.FrameWork.Management.Language.Msg(obj.Name + " 自动生成该科拼音码时发生错误." + obj.ID + "科室"));
                    continue;
                }

                //{BB505126-A265-4c62-9392-30D99503E36E}
                //替换obj.User01.tostring() 为 FS.FrameWork.Function.NConvert.ToDateTime(obj.User01).ToLongTimeString()
                ...
                {
                    FS.FrameWork.WinForms.Classes.Function.HideWaitForm();
                    MessageBox.Show(FS.FrameWork.Management.Language.Msg(obj.Name + "该科已存在 不能重复添加" + obj.ID + "该科室"));
                    continue;
                }

                string getDrugType = "全部";
                if (obj.Memo == "I")
                {
                    getDrugType = "住院医嘱";
                }
                ...
                this.ds.Tables[0].Rows.Add(new object[]{
                    ...
                    FS.FrameWork.Function.NConvert.ToDateTime(obj.User01).ToLongTimeString(),//开始时间
                    FS.FrameWork.Function.NConvert.ToDateTime(obj.User02).ToLongTimeString(),//结束时间
                    getDrugType,//发药类别
        ...
        private void AddDept()
        {
            ...
            List<FS.HISFC.Models.Base.Department> selectedDeptList = uc.SelectedDeptList;
        ...
        private void SaveDept()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < this.neuSpread1_Sheet1.RowCount; i++)
                {
                    ...
                    obj.Memo = "A";
                    if (this.neuSpread1_Sheet1.Cells[i, 5].Text == "住院医嘱")
                    {
                        obj.Memo = "I";
                    }
                    else if (this.neuSpread1_Sheet1.Cells[i, 5].Text == "门诊处方")
                    {
                        obj.Memo = "O";
                    }
        ...
        protected override void OnLoad(EventArgs e)
        {
            if (System.Diagnostics.Process.GetCurrentProcess().ProcessName.ToUpper() != "DEVENV")
            {
                ...
                FS.SOC.HISFC.Components.PharmacyCommon.tvDeptTree tvDeptTree = this.tv as FS.SOC.HISFC.Components.PharmacyCommon.tvDeptTree;

                if (tvDeptTree != null)
                {
                    tvDeptTree.IsUseDeptStruct = this.isUseDeptStruct;
                    tvDeptTree.IsShowCommonDept = true;
                    tvDeptTree.Reset();
                }
            }
        ...
        {
            //增加工具栏
            this.toolBarService.AddToolButton("增加", "增加取药科室", FS.FrameWork.WinForms.Classes.EnumImageList.T添加, true, false, null);
            this.toolBarService.AddToolButton("删除", "删除取药科室", FS.FrameWork.WinForms.Classes.EnumImageList.S删除, true, false, null);
