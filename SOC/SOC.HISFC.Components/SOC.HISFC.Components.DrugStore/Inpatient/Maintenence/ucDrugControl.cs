...
namespace FS.SOC.HISFC.Components.DrugStore.Inpatient.Maintenence
{
    ...
    public partial class ucDrugControl : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        //定义药房管理类
        private FS.SOC.HISFC.BizLogic.Pharmacy.DrugStore drugStore = new FS.SOC.HISFC.BizLogic.Pharmacy.DrugStore( );
        //定义当前选择科室
        ...
        public void SetDrugControlInfo( DrugControl drugControl )
        {
            ...
            //显示等级
            //by cube这个地方因为删除了0显示科室汇总项目，所以SelectedIndex+1和原来的值对应
            this.cmbShowGrade.SelectedIndex = this.currentDrugControlInfo.ShowLevel - 1;
        ...
        public DrugControl GetNewDrugControlInfo( )
        {
            ...
            this.currentDrugControlInfo.ShowLevel = this.cmbShowGrade.SelectedIndex + 1;
        ...
        private void AddDrugControl( )
        {
            if( currentDept.ID == "" )
            {
                MessageBox.Show( FS.FrameWork.Management.Language.Msg( "请选择具体的科室" ) );
                return;
            }
        ...
        private void DeleteDrugControl( )
        {
            ...
            {
                MessageBox.Show( FS.FrameWork.Management.Language.Msg( "请选择您要删除的摆药台！") );
                return;
            }

            if( this.currentDrugControlInfo.ID != "" )
            {
                DialogResult result = MessageBox.Show( FS.FrameWork.Management.Language.Msg( "您确定要删除【" + this.currentDrugControlInfo.Name + "】摆药台吗？" ), FS.FrameWork.Management.Language.Msg( "删除提示") , System.Windows.Forms.MessageBoxButtons.OKCancel );
                ...
                if( parm == -1 )
                {
                    ...
                    MessageBox.Show( this.drugStore.Err ,FS.FrameWork.Management.Language.Msg( "提示"));
                    return;
                }
                else
                {
        ...
        private bool IsSaveDrugControlValid(DrugControl drugControl)
        {
            foreach (ListViewItem lv in this.lvDrugControlList.Items)
            {
                ...
                {
                    MessageBox.Show(FS.FrameWork.Management.Language.Msg(drugControl.Name + " 摆药台名称重复"));
                    return false;
                }
            }
        ...
        private void SaveDrugControl( )
        {
            ...
            {
                MessageBox.Show( FS.FrameWork.Management.Language.Msg( "请为摆药台添加摆药单!" ));
                return;
            }
            ...
            {
                MessageBox.Show( FS.FrameWork.Management.Language.Msg( "请选择一个摆药台。" ));
                return;
            }
            ...
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("摆药台名称超长,请适当简略"));
                return;
            }
            if (this.currentDrugControlInfo.Memo.Length > 30)
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("备注超长,请适当简略"));
                return;
            }
            ...
            if( currentDrugControlInfo.ID == "" )
            {
                ...
                {
                    MessageBox.Show( FS.FrameWork.Management.Language.Msg( "取摆药台流水号时失败:") + this.drugStore.Err );
                    return;
                }
            }
            ...
            try
            {
                ...
                FS.FrameWork.Management.PublicTrans.Commit();
                this.neuTabControl1.SelectedIndex = 0;
                MessageBox.Show( FS.FrameWork.Management.Language.Msg( "摆药台设置成功！" ));
            }
            catch( Exception ex )
            {
                FS.FrameWork.Management.PublicTrans.RollBack();
                MessageBox.Show( FS.FrameWork.Management.Language.Msg( "保存摆药台数据时出错：" + ex.Message ));
                return;
            }
        ...
        protected override int OnSetValue( object neuObject , TreeNode e )
        {
            try
            {
                if( e != null )
                {
                    ...
                    else
                    {
                        ...
                        {
                            MessageBox.Show(FS.FrameWork.Management.Language.Msg("请选择具体的科室"), FS.FrameWork.Management.Language.Msg("提示"));
                        }
                    }
                }
                else
            ...
            {
                MessageBox.Show( ex.Message ,FS.FrameWork.Management.Language.Msg( "提示"));
            }
        ...
        protected override FS.FrameWork.WinForms.Forms.ToolBarService OnInit( object sender , object neuObject , object param )
        {
            ...
            //增加工具栏
            this.toolBarService.AddToolButton( "增加" , "增加摆药台" , FS.FrameWork.WinForms.Classes.EnumImageList.T添加 , true , false ,null );
            this.toolBarService.AddToolButton( "删除" , "删除摆药台" , FS.FrameWork.WinForms.Classes.EnumImageList.S删除 , true , false , null );
