...
namespace FS.SOC.HISFC.Components.DrugStore.Inpatient.Maintenence
{
    ...
    public partial class lvPutDrugBill : FS.FrameWork.WinForms.Controls.NeuListView
    {
        ...
        protected virtual void InitListView( )
        {
            ...
            try
            {
                FS.SOC.HISFC.BizLogic.Pharmacy.DrugStore drugStore = new FS.SOC.HISFC.BizLogic.Pharmacy.DrugStore();
        ...
        protected virtual void SaveDefaultBill(bool isP,bool isR)
        {
            if (isR)            //设置退药摆药单
            {
                ...
                {
                    MessageBox.Show(FS.FrameWork.Management.Language.Msg("保存特殊摆药单失败"));
                    return;
                }
            }
