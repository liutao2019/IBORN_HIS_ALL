...
namespace FS.SOC.HISFC.Components.DrugStore.Outpatient.Base
{
    /// <summary>
    ...
    ///  />
    /// </summary>
    public partial class tvRecipeBaseTree : FS.SOC.HISFC.Components.Common.Base.baseTreeView
    {
        ...
        public void ShowList(ArrayList alDrugRecipe, bool isSupplemental,bool isAutoShow)
        {
            ...
            {
                //查找是否已经添加，避免重复
                System.Windows.Forms.TreeNode node = this.FindNode(drugRecipe.RecipeNO);
                if (node == null)
                {
                ...
                //给树节点添加键值
                node.Name = drugRecipe.RecipeNO;
                if (drugRecipe.CancelOper.OperTime > drugRecipe.FeeOper.OperTime)
                {
                    node.Text = drugRecipe.PatientName + "[退]";
                }
                else
                {
                    node.Text = drugRecipe.PatientName;
        ...
        /// <returns></returns>
        public System.Windows.Forms.TreeNode FindNode(string recipeNO)
        {
            foreach (System.Windows.Forms.TreeNode node in this.Nodes)
            {
                if (node.Tag != null && node.Tag is FS.HISFC.Models.Pharmacy.DrugRecipe)
                {
                    if (((FS.HISFC.Models.Pharmacy.DrugRecipe)node.Tag).RecipeNO == recipeNO)
                    {
                        return node;
                    }
                }
