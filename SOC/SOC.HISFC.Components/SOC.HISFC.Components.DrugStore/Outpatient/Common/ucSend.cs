...
namespace FS.SOC.HISFC.Components.DrugStore.Outpatient.Common
{
    ...
    public partial class ucSend : ucDrugBase, FS.FrameWork.WinForms.Classes.IPreArrange
    {
        ...
        /// 当前发药的工号信息
        /// </summary>
        private FS.HISFC.Models.Base.Employee curSendEmployee = new FS.HISFC.Models.Base.Employee();
        public FS.HISFC.Models.Base.Employee CurSendEmployee
        {
            get { return curSendEmployee; }
            set { curSendEmployee = value; }
        }
        ...
        private int QueryOnLoad()
        {
            ...
            if (al1 != null)
            {
                this.ucDrugTree1.ShowRecipeList(al1, true, "1");
            }
        ...
        private void CallPatient()
        {
            if (this.IOutpatientLED != null)
            {
                this.IOutpatientLED.ShowDataAfterSelect(this.ucDrugTree1.GetRecipeList(0), this.ucDrugTree1.DrugRecipe);
            }
        }
        /// <summary>
        /// 显示选中的处方信息
        ...
        private int OverNO()
        {
            ...
            this.EnumOutpatintDrugOperType = Function.EnumOutpatintDrugOperType.大屏显示处理;

            if (this.IOutpatientLED != null)
            {
                this.IOutpatientLED.OverNO(this.ucDrugTree1.GetRecipeList(0), this.ucDrugTree1.DrugRecipe);
            }
        ...
        private int Save()
        {
            ...
            {
                if (dr.ValidState == FS.HISFC.Models.Base.EnumValidState.Invalid)
                {
                    dr.RecipeState = "3";
                    FS.FrameWork.Management.PublicTrans.BeginTransaction();
                    if (this.drugStoreMgr.UpdateDrugRecipeInfo(dr) <= 0)
                    {
            ...
            if (this.IsSetWorkLoad)
            {
                ...
                {
                    dr.RecipeState = "3";
                    if (string.IsNullOrEmpty(this.curSendEmployee.ID))
                    {
                        dr.SendOper.ID = this.drugStoreMgr.Operator.ID;
                        dr.SendOper.Name = this.drugStoreMgr.Operator.Name;
                    }
                    ...
                    {
                        dr.SendOper.ID = this.curSendEmployee.ID;
                        dr.SendOper.Name = this.curSendEmployee.Name;
                    }
        ...
        {
            this.ToolBarService.AddToolButton("过号", "过号挂起", FS.FrameWork.WinForms.Classes.EnumImageList.A安排, true, false, null);
            this.ToolBarService.AddToolButton("叫号", "启用LED及语音叫号", FS.FrameWork.WinForms.Classes.EnumImageList.A安排, true, false, null);
        ...
        public override void ToolStrip_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            else if (e.ClickedItem.Text == "叫号")
            {
                this.CallPatient();
            }
            else if (e.ClickedItem.Text == "过号")
            {
                this.OverNO();
            }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (IsSetWorkLoad)
            {
                ...
                ArrayList allPerson =  personMgr.GetEmployee(FS.HISFC.Models.Base.EnumEmployeeType.P);
                allPerson.Add(this.drugStoreMgr.Operator);
                this.cmbSendEmployee.AddItems(allPerson);
                for (int index = 0; index < this.cmbSendEmployee.alItems.Count; index++)
                {
                    if ((this.cmbSendEmployee.alItems[index] as FS.HISFC.Models.Base.Employee).ID != this.drugStoreMgr.Operator.ID)
                    {
                        continue;
                    }
                    this.cmbSendEmployee.SelectedIndex = index;
                    break;
                }
                curSendEmployee = cmbSendEmployee.SelectedItem as FS.HISFC.Models.Base.Employee;
                string workLoadInfo = "发药人：" + this.drugStoreMgr.Operator.Name + "工号：" + this.drugStoreMgr.Operator.ID;
                this.nlbWorkLoadInfo.Text = workLoadInfo;
                this.cmbSendEmployee.SelectedIndexChanged += new EventHandler(cmbSendEmployee_SelectedIndexChanged);
            }
        ...
        void cmbSendEmployee_SelectedIndexChanged(object sender, EventArgs e)
        {
            this.EnumOutpatintDrugOperType = Function.EnumOutpatintDrugOperType.保存;
            if (this.cmbSendEmployee.SelectedItem.ID != this.curSendEmployee.ID)
            {
                SOC.HISFC.Components.Common.Base.frmCheckPassWord frmCheckPassWord = new FS.SOC.HISFC.Components.Common.Base.frmCheckPassWord((this.cmbSendEmployee.SelectedItem as FS.HISFC.Models.Base.Employee).ID);
                frmCheckPassWord.StartPosition = FormStartPosition.CenterScreen;
                if (frmCheckPassWord.ShowDialog() != DialogResult.OK)
                {
                    for (int index = 0; index < this.cmbSendEmployee.alItems.Count; index++)
                    {
                        if ((this.cmbSendEmployee.alItems[index] as FS.HISFC.Models.Base.Employee).ID != this.curSendEmployee.ID)
                        {
                            continue;
                        }
                        this.cmbSendEmployee.SelectedIndex = index;
                ...
                curSendEmployee = this.cmbSendEmployee.SelectedItem as FS.HISFC.Models.Base.Employee;
                string workLoadInfo = "发药人：" + curSendEmployee.Name + "工号：" + curSendEmployee.ID;
                this.nlbWorkLoadInfo.Text = workLoadInfo;
                this.EnumOutpatintDrugOperType = Function.EnumOutpatintDrugOperType.空闲;
                this.ManulRefresh();
        ...
        private void SetScreen()
        {
            if (this.IOutpatientLED != null)
            {
                try
                {
                    this.IOutpatientLED.SetLED(); //在SetLED中写show()应该就行
                    List<FS.HISFC.Models.Pharmacy.DrugRecipe> alDrugRecipe = this.ucDrugTree1.GetRecipeList(0);
                    if (alDrugRecipe == null || alDrugRecipe.Count == 0)
                    {
                        FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe = new FS.HISFC.Models.Pharmacy.DrugRecipe();
                        drugRecipe.StockDept.ID = this.StockDept.ID;
                        drugRecipe.SendTerminal.ID = this.SendTerminal.ID;
                        alDrugRecipe.Add(drugRecipe);
                    }
                    this.IOutpatientLED.AutoShowData(this.ucDrugTree1.GetRecipeList(0), this.EnumOutpatintDrugOperType != Function.EnumOutpatintDrugOperType.空闲);
                }
                catch
