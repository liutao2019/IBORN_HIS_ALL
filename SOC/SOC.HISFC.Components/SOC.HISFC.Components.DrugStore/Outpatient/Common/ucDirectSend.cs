...
namespace FS.SOC.HISFC.Components.DrugStore.Outpatient.Common
{
    ...
    /// </summary>
    public partial class ucDirectSend : ucDrugBase, FS.FrameWork.WinForms.Classes.IPreArrange
    {
        public ucDirectSend()
        {
            InitializeComponent();
        ...
        /// 是否使用终端自动打印注册功能
        /// </summary>
        private bool isPrintByInvoiceNO = false;

        /// <summary>
        /// 是否使用终端自动打印注册功能
        /// </summary>
        [Description("是否使用终端自动打印注册功能"), Category("设置"), Browsable(true)]
        public bool IsPrintByInvoiceNO
        {
            get { return isPrintByInvoiceNO; }
            set { isPrintByInvoiceNO = value; }
        }
        ...
        public int PreArrange()
        {
            ...
            {
                //记录是否自动打印
                this.isAutoPrint = this.SendTerminal.IsAutoPrint;
                ...
                if (this.isRegisterAutoPrint)
                {
                    this.ncbAutoPrintRegister.Checked = Function.CheckAutoPrintPrive(this.SendTerminal);
                }
        ...
        private void Init()
        {
            ...
            {
                //从零点开始刷新
                this.manualRefreshBeginTime = this.drugStoreMgr.GetDateTimeFromSysDateTime().Date.AddDays(-this.DaySpan);
            }
            catch(Exception ex)
            {
        ...
        private int QueryOnLoad()
        {
            ...
            //1、未打印的不加载，避免打开界面处理打印任务变得很慢，在以后的自动刷新或者手工刷新中处理

            //2、已经打印但是没有配药的
            ArrayList al1 = drugStoreMgr.QueryList(this.PriveDept.ID, terminalNO, type, "1", this.manualRefreshBeginTime);
            if (al1 != null)
            {
                this.ucDrugTree1.ShowRecipeList(al1, true, "0");
            }
            //3、已经配药但是没有发药的，直接发药应该不存在这样的数据，不允许配发药盒直接发药同步使用，产生的问题程序不处理

            //4、已经发药的，避免数据量过多，只加载当天的
            ArrayList al3 = drugStoreMgr.QueryList(this.PriveDept.ID, terminalNO, type, "3", DateTime.Now.Date);
            if (al3 != null)
            {
                this.ucDrugTree1.ShowRecipeList(al3, true, "1");
            }
        ...
        private int ManulRefresh()
        {
            ...
            if (al2 != null)
            {
                this.ucDrugTree1.ShowRecipeList(al2, true, "0");
            }
            ...
            if (al0 != null)
            {
                ...
                foreach (FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe in al0)
                {
                    ...
                    //更新申请明细的打印状态
                    {
                        ArrayList alRecipeDetail = this.drugStoreMgr.QueryApplyOutListForClinic(this.PriveDept.ID, "M1", "0", drugRecipe.RecipeNO);
                        ...
                        foreach (FS.HISFC.Models.Pharmacy.ApplyOut applyOut in alRecipeDetail)//{4B8A931E-FD8D-4786-BBA3-459D2A9B4534} lfhm
                        {
                            if (string.IsNullOrEmpty(applyOut.PrintState) || applyOut.PrintState == "0")
                            {
                                if (applyOut.ValidState == FS.HISFC.Models.Base.EnumValidState.Valid)
                                {
                                    ...
                                    if (drugStoreMgr.UpdateAppleOutPrintState("1", "1", operID, applyNum) == -1)
                                    {
                                        return -1;
                                    }
                                }
        ...
        {
            if (this.EnumOutpatintDrugOperType == Function.EnumOutpatintDrugOperType.打印)
            {
                return;
            }
            if (this.EnumOutpatintDrugOperType != Function.EnumOutpatintDrugOperType.空闲)
            ...
            //未打印的
            ArrayList al0 = drugStoreMgr.QueryList(this.PriveDept.ID, terminalNO, type, "0", this.autoRefreshBeginTime);
            if (al0 != null)
            {
                ...
                if (this.isAutoPrint)
                {
                    //佛山四院按相同发票号一起打印，天天说，烦死了！
                    if (isPrintByInvoiceNO)
                    {
                        this.Print1(al0);
                    }
                    else
        ...
        /// <summary>
        /// 显示选中的处方信息，这个是处方树操作引起的
        /// </summary>
        private void ShowRecipeInfo()
        {
            //封装在原子操作中：应该在处方树操作时候封装
            //if (this.EnumOutpatintDrugOperType != Function.EnumOutpatintDrugOperType.空闲)
            //{
            //    this.ShowBalloonTip("正在" + this.EnumOutpatintDrugOperType.ToString() + ",请稍候再试...");
            //    return 0;
            ...
            //{DA6F0853-BA2E-4678-B0B2-39733FCB04F3}设置年龄
            if (this.ucDrugTree1.DrugRecipe.Age == null || this.ucDrugTree1.DrugRecipe.Age.ToString("yyyy-MM-dd") == "0001-01-01")
            {
                ...
                Register r = registerManager.GetByClinic(this.ucDrugTree1.DrugRecipe.ClinicNO);
                this.ucDrugTree1.DrugRecipe.Age = r.Birthday;
            }
            ...
            if (alRecipeDetail == null)
            {
                this.ShowMessage("查询处方明细出错：" + this.drugStoreMgr.Err, MessageBoxIcon.Error);
                return;
            }
            if (alRecipeDetail.Count == 0)
            {
                ...
                if (alRecipeDetail == null)
                {
                    this.ShowMessage("查询处方" + this.ucDrugTree1.DrugRecipe.RecipeNO + "调剂信息出错：" + this.drugStoreMgr.Err, MessageBoxIcon.Error);
                    return;
                }
        ...
        private int Save()
        {
            ...
            //[2011-9-7]zhao.zf unDruged表示该处方状态是否还没配药，还没配药的话调用直接发药流程，否则调用正常发药流程
            bool unDruged = true;
            ...
            if (MessageBox.Show("是否确认发药？", "询问", MessageBoxButtons.YesNo) == DialogResult.No)// {47F1129F-CC17-41fa-9F1A-6D4E9085FD2E}
            {
                return 0;
            }
            ...
            if (dr.RecipeState == "0")
            {
                if ((this.IsCheckPrivePower && this.IsOtherDeptDrug) || !this.IsRegisterAutoPrint)
                {
            ...
            else if (dr.RecipeState == "1")// {4E0F4013-8A02-4ed1-B4AF-2188ABA79483}
            {
                FS.FrameWork.Management.PublicTrans.BeginTransaction();
                int param = this.drugStoreMgr.UpdateDrugRecipeState(this.PriveDept.ID, dr.RecipeNO, "M1", "1", "3");
                if (param == -1)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                }
            ...
            {
                recipeState = "1";
                parm = Function.OutpatientPartSend(alData, terminal, this.StockDept.Clone(), this.drugStoreMgr.Operator, unDruged, false);
            }
            else if (Function.JudgePartSend() == 1 && this.ucRecipeDetail1.CheckAll)
            {
                parm = Function.OutpatientPartSend(alData, terminal, this.StockDept.Clone(), this.drugStoreMgr.Operator, unDruged, true);
            }
            else
            {
                //[2011-9-7]zhao.zf unDruged表示该处方状态是否还没配药，还没配药的话调用直接发药流程，否则调用正常发药流程
                parm = Function.OutpatientSend(alData, terminal, this.StockDept.Clone(), this.drugStoreMgr.Operator, unDruged, true);
            }
            if (parm == 1)
            {
                //移动到函数结尾
            ...
            if (this.IsSetWorkLoad)
            {
                ...
                else
                {
                    ...
                    if (this.DrugTerminal != null && !string.IsNullOrEmpty(this.DrugTerminal.ID))
                    {
                        this.IOutpatientWorkLoad.Set(this.StockDept, dr, "0", this.DrugTerminal);
                    }
                    else 
                    {
        ...
        /// <summary>
        /// 打印（佛山四院，多个处方的发票号相同时，按发票号打印）
        /// 上层调用函数必须保证这个打印过程不可以被刷新、查询等中断或多线程同步执行
        /// </summary>
        /// <param name="alDrugRecipe">必须是状态为0的处方调剂信息数组</param>
        /// <returns>-1失败</returns>
        private int Print1(ArrayList alDrugRecipe)
        {
            ArrayList newDrugrecipe = new ArrayList();
            string invoiceNO = null;
            //一张发票有多张处方，只取之中一张,按发票号排序
            foreach (FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe in alDrugRecipe)
            {
                if (invoiceNO == null || invoiceNO != drugRecipe.InvoiceNO)
                {
                    newDrugrecipe.Add(drugRecipe);
                    invoiceNO = drugRecipe.InvoiceNO;
                }
            }
            foreach (FS.HISFC.Models.Pharmacy.DrugRecipe drugRecipe in newDrugrecipe)
            {
                //显示处方明细
                string state = "0";
                if (drugRecipe.RecipeState == "2")
                ...
                //找出改发票号所有的处方明细
                ArrayList alRecipeDetail = this.drugStoreMgr.QueryApplyOutListForInvoiceNO(this.PriveDept.ID, "M1", state, drugRecipe.InvoiceNO);
                if (alRecipeDetail == null)
                {
                    this.ShowMessage("查询处方明细出错：" + drugStoreMgr.Err, MessageBoxIcon.Error);
                    return -1;
        ...
        {
            this.ucRecipeDetail1.SelectNone();
        }
        #endregion
        ...
        protected override FS.FrameWork.WinForms.Forms.ToolBarService OnInit(object sender, object neuObject, object param)
        {
            ...
            this.ToolBarService.AddToolButton("全不选", "全不选药品", FS.FrameWork.WinForms.Classes.EnumImageList.Q全不选, true, false, null);
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            this.ucDrugTree1.RecipeAfterSelectEven += new ucRecipeTree.RecipeAfterSelectHandler(this.ShowRecipeInfo);
        ...
        void ncbAutoPrintRegister_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (!this.ncbAutoPrintRegister.Checked)
            {
                ...
                {
                    successful = Function.RegisterDrugTermial(this.SendTerminal);
                }
                if (successful)
                {
                    this.ManulRefresh(); 
        ...
        void nlbWorkLoadInfo_DoubleClick(object sender, EventArgs e)
        {
            if (this.IsSetWorkLoad && this.IOutpatientWorkLoad != null)
            {
                if (this.DrugTerminal != null && !string.IsNullOrEmpty(this.DrugTerminal.ID))
                {
                    this.nlbWorkLoadInfo.Text = this.IOutpatientWorkLoad.Reassigned(this.PriveDept, "0", this.DrugTerminal);
                }
                else
                {
                    this.nlbWorkLoadInfo.Text = this.IOutpatientWorkLoad.Reassigned(this.PriveDept, "0", this.SendTerminal);
                }
            }
        }
