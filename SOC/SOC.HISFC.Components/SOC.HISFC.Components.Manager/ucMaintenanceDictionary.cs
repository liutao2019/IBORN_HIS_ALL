...
{
    public partial class ucMaintenanceDictionary : FS.FrameWork.WinForms.Controls.ucMaintenance
    {
        public ucMaintenanceDictionary()
            : base("const")
        {
            InitializeComponent();
        ...
        protected override string SQL
        {
            get
            {
                textStr = ((Control)this).Text.Split('#');
                Init();
        ...
        private new int Init()
        {
            ...
            //非药品项目
            alConst_UndrugItem = this.managerIntegrate.GetConstantList(textStr[1]);
            if (alConst_UndrugItem == null)
            {
                MessageBox.Show("查询字典项目出错：" + this.managerIntegrate.Err);
                return -1;
            }
