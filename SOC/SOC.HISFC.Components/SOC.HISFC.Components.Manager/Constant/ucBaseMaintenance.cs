...
{
    public partial class ucBaseMaintenance : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Forms.IMaintenanceControlable
    {
        public ucBaseMaintenance()
        {
            InitializeComponent();
        }
        ...
        private bool CheckChanged(int iIndex)
        {
            ...
            curCons.UserCode = this.neuSpread1_Sheet1.Cells[iIndex, (int)ColumnSet.ColUserCode].Text;
        ...
        private int Save(bool isCheckChange)
        {
            ...
            for (int i = 0; i < this.neuSpread1_Sheet1.Rows.Count; i++)
            {
                ...
                if (this.constantManager.SetConstant(this.Tag.ToString(), cons) == -1)
                {
                    FS.FrameWork.Management.PublicTrans.RollBack();
                ...
                cons.OperEnvironment.Memo = this.Tag.ToString();
                alInfo.Add(cons);
            ...
            this.QueryConsData(this.Tag.ToString());
        ...
        public int Add()
        {
            ...
            this.neuSpread1_Sheet1.Cells[index, (int)ColumnSet.ColID].Locked = false;
            this.neuSpread1_Sheet1.Cells[index, (int)ColumnSet.ColFlag].Text = "New";
            this.neuSpread1_Sheet1.Cells[index, (int)ColumnSet.ColSort].Value = (index + 1).ToString();
            this.neuSpread1_Sheet1.Cells[index, (int)ColumnSet.ColOperCode].Text = this.constantManager.Operator.ID.ToString();
            this.neuSpread1_Sheet1.Cells[index, (int)ColumnSet.ColValid].Value = true;

            this.neuSpread1_Sheet1.SetActiveCell(index, 0);
        ...
        public int Del()
        {
            ...
            if (MessageBox.Show(Language.Msg("删除该条数据"), "删除", System.Windows.Forms.MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                int flag = this.constantManager.CanDeleteCons(this.Tag.ToString(), code);
                ...
                if (ifNew != "New")
                {
                    if (this.constantManager.DelConstant(this.Tag.ToString(), code) == -1)
                    {
                    ...
                    c.OperEnvironment.Memo = this.Tag.ToString();
                    c.ID = code;
                    ArrayList alInfo = new ArrayList();
                    alInfo.Add(c);
                    int param = Function.SendBizMessage(alInfo, FS.SOC.HISFC.BizProcess.MessagePatternInterface.EnumOperType.Delete, FS.SOC.HISFC.BizProcess.MessagePatternInterface.EnumInfoType.Constant, ref errInfo);

                    if (param == -1)
                    {
                        FS.FrameWork.Management.PublicTrans.RollBack(); ;
                        Function.ShowMessage("删除失败，请向系统管理员报告错误信息：" + errInfo, MessageBoxIcon.Error);
                        return -1;
                    }
                }
