...
namespace FS.SOC.HISFC.Components.Nurse.Controls.SDFY
{
    ...
    public partial class ucArray : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        ...
        private void RefreshQueue()
        {
            ...
            string noonID = Function.GetNoon(current);//午别
            if (noonID == "")
            {
        ...
        private int RefreshQueue(string nurseID, DateTime queueDate, string noonID)
        {
            ...
            if (this.alCollections != null)
            {
                foreach (FS.HISFC.Models.Nurse.Queue obj in this.alCollections)
                {
                    ...
                    item.Tag = obj;
                    //如果获取的队列午别与当前队列不一致，则背景色置红 ygch {EEA49694-3632-427f-BF2F-6870576B9AA6}
        ...
        private int QueryReg(string cardNo)
        {
            ...
            {
                ucPopPatient uc = new ucPopPatient();
            ...
            if (queueList != null || queueList.Count > 0)
            {
                ...
                for (int i = 0; i < lvQueue.Items.Count; i++)
                {
                    ...
                    if (queItem.ID == queueID)
                    {
                        ...
                        lvQueue.Items[i].Text = lvQueue.Items[i].Text + "(" + que.WaitingCount.ToString() + "人)";
                    }
                }
            }
        }
        ...
        private void CancelTriageUnsee()
        {
            ...
            string noonID = Function.GetNoon(endDate);
            ArrayList al = new ArrayList();
        ...
        private void Screen()
        {
            ...
            #region {E1BE1043-5BDA-4c1d-94F0-8C2D5FF84F9F}
            if (tsb == null)
            {
            ...
            if (!IsShowScreen)
            {
                ...
                string RefreshTime = this.controlMgr.QueryControlerInfo("900011"); //取控制参数"900011" 控制分诊屏幕分页刷新速度 ygch {E7AD911A-5EFD-4999-8849-52BA9D61FAD7}
                try
                {
                    f.PageRefreshTime = FS.FrameWork.Function.NConvert.ToInt32(RefreshTime) * 1000;
                }
                catch (Exception e)
                {
                    MessageBox.Show("获取分诊屏幕分页刷新速度参数失败！");
                }
                f.Show();
                IsShowScreen = true;
            }
