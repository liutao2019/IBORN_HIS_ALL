...
namespace FS.SOC.HISFC.Components.Nurse.Controls.FoSan
{
    public partial class ucRegister : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        ...
        /// </summary>
        [Description("用法是否打印在巡视卡上"), Category("设置")]
        public string Usage
        {
            get
            {
        ...
        [Description("用法维护（输液用法）"), Category("设置")]
        public string InfusionUsage
        {
            get
            {
                return this.infusionUsage;
            }
            set
            {
                this.infusionUsage = value;
            }
        }
        ...
        { 
            ...
            foreach (ArrayList al in hsInfos.Values)
            {
                ...
                ArrayList alZL = new ArrayList();
                ArrayList alSY = new ArrayList();
                for (int i = 0; i < al.Count; i++)
                {
                    FS.HISFC.Models.Nurse.Inject info = al[i] as FS.HISFC.Models.Nurse.Inject;
                    if (this.infusionUsage.Contains(info.Item.Order.Usage.Name.ToString() + ";")&& info.Item.Item.ItemType == FS.HISFC.Models.Base.EnumItemType.Drug)
                    {
                        alSY.Add(info);
                    }
                    else
                    ...
                    if (alZL.Count > 0)
                    {
                        SOC.HISFC.Components.Nurse.Controls.FoSan.ucPrintTreatment ucPrintTreatment = new ucPrintTreatment();
                        ucPrintTreatment.IsReprint = false;
                        ucPrintTreatment.Init(alZL);
                    }
                }
        ...
        private void AddDetail(ArrayList details)
        {
            ...
            if (details != null)
            {
                foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList detail in al)
                {
                    ...
                    //if (detail.Item.ItemType != FS.HISFC.Models.Base.EnumItemType.Drug) continue;
                    //#endregion
        ...
        private void AddDetail(FS.HISFC.Models.Fee.Outpatient.FeeItemList info)
        {
            ...
            if (!string.IsNullOrEmpty(info.ConfirmOper.ID))
            {
                ...
                //}
                //{
                //    empl = this.PsMgr.GetEmployeeInfo(applyOutObj.Operation.ApproveOper.ID);
                //    if (empl == null)
                //    {
                //        MessageBox.Show("获取摆药人信息失败:" + PsMgr.Err, "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                //    }
                //    else
                //    {
