    ...
    public class Function
    {
        ...
        public static int CheckArray(FS.HISFC.Models.Registration.Register regObj, FS.HISFC.Models.Nurse.Queue queueObj)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(queueObj.Doctor.ID))
                {
                    ...
                    if (!string.IsNullOrEmpty(reglevlCode))
                    {
        ...
        /// <returns></returns>
        public static string GetNoonID(DateTime current)
        {
            if (alNoon == null)
            {
        ...
        /// <param name="current"></param>
        /// <returns></returns>
        public static FS.HISFC.Models.Base.Noon GetNoon(DateTime current)
        {
            if (alNoon == null)
            {
                FS.HISFC.BizLogic.Registration.Noon noonMgr = new FS.HISFC.BizLogic.Registration.Noon();
            ...
            string TrigeWhereFlag, ref string error)
        {
            ...
            try
            {
                ...
                //是否重新更新看诊序号
                bool isUpdateSeeNo = false;

                if (!string.IsNullOrEmpty(assign.Queue.Doctor.ID))
                {
                    Type = "1";//医生
                    Subject = assign.Queue.Doctor.ID;
                }
                else if (!string.IsNullOrEmpty(assign.Queue.SRoom.ID))
                {
                    Type = "5";//诊室
                    Subject = assign.Queue.SRoom.ID;
                }
                else
                {
                    Type = "3";//诊区
                ...
                string noonID = GetNoonID(assignMgr.GetDateTimeFromSysDateTime());
