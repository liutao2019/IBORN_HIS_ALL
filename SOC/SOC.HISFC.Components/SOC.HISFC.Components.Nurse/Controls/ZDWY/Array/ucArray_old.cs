...
namespace FS.SOC.HISFC.Components.Nurse.Controls.ZDWY.Array
{
    ...
    ///  />
    /// </summary>
    public partial class ucArray_old : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        /// <summary>
        /// 构造
        /// </summary>
        ...
        public FS.HISFC.Models.Registration.Register CurrentReg
        {
            ...
            set
            {
                ...
                if (currentReg != null)
                {
                    ...
                    if (regObj != null)
                    {
                        lblSelectPatient.Text = regObj.DoctorInfo.Templet.Dept.Name + "  " + regObj.Name + "  " + regObj.Sex.Name + "  " + assginMgr.GetAge(regObj.Birthday);
                    }
                }
                else
                {
        ...
        /// 显示屏
        /// </summary>
        FS.SOC.HISFC.Components.Nurse.Controls.ZDWY.Array.frmDisplay f = null;
        ...
        private int RefreshQueue(string nurseID, DateTime queueDate, string noonID)
        {
            ...
            if (!isShowAllQueue)
            {
                ...
                foreach (FS.HISFC.Models.Nurse.Queue queue in this.alCollections)
                {
            ...
            if (this.alCollections != null)
            {
                ...
                foreach (FS.HISFC.Models.Nurse.Queue queue in this.alCollections)
                {
                    ...
                    item.SubItems.Add(this.doctHelper.GetName(queue.Doctor.ID));
                    item.SubItems.Add(queue.Doctor.ID);
                    item.SubItems.Add(queue.SRoom.Name);
                    item.SubItems.Add(queue.WaitingCount.ToString());
                    item.SubItems.Add(noonHelper.GetObjectFromID(queue.Noon.ID).Name);
                    #endregion

                    ArrayList al = assginMgr.Query(nurseID, queueDate.Date, queue.ID, FS.HISFC.Models.Nurse.EnuTriageStatus.Triage);
                    if (al == null)
                    {
                        MessageBox.Show("查询队列信息出错！\r\n" + assginMgr.Err, "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return -1;
                    }
                    count += al.Count;

                    al = assginMgr.Query(nurseID, queueDate.Date, queue.ID, FS.HISFC.Models.Nurse.EnuTriageStatus.Delay);
                    if (al == null)
                    {
                        MessageBox.Show("查询队列信息出错！\r\n" + assginMgr.Err, "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return -1;
                    ...
                    if (!queue.IsValid)
                    {
                        item.Text = item.Text + "(" + count + "人)\r\n" + "[停]" + FS.SOC.HISFC.BizProcess.Cache.Common.GetNoonInfo(queue.Noon.ID).Name;
                    }
                    else
                    {
                        item.Text = item.Text + "(" + count + "人)\r\n" + FS.SOC.HISFC.BizProcess.Cache.Common.GetNoonInfo(queue.Noon.ID).Name;
                    }
        ...
        private void lvQueue_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                this.RetrieveWait(((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept.ID, this.assginMgr.GetDateTimeFromSysDateTime().Date, (selected.Tag as FS.HISFC.Models.Nurse.Queue).ID);
            }
            else
            {
                this.RetrieveWait(((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept.ID, this.assginMgr.GetDateTimeFromSysDateTime().Date, (selected.Tag as FS.HISFC.Models.Registration.DoctSchema).ID);
            }
        }
