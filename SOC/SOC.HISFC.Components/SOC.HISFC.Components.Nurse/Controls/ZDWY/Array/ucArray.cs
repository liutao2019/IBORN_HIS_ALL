...
namespace FS.SOC.HISFC.Components.Nurse.Controls.ZDWY.Array
{
    ...
    public partial class ucArray : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        ...
        private FS.HISFC.Models.Registration.Register currentReg = null;
        ...
        public FS.HISFC.Models.Registration.Register CurrentReg
        {
            get
            {
                return currentReg;
            ...
            set
            {
                ...
                if (currentReg != null)
                {
                    FS.HISFC.Models.Registration.Register regObj = regMgr.GetByClinic(currentReg.ID);
                    if (regObj != null)
                    {
                        lblSelectPatient.Text = regObj.DoctorInfo.Templet.Dept.Name + "  " + regObj.Name + "  " + regObj.Sex.Name + "  " + localMgr.GetAge(regObj.Birthday);
                    }
                }
                else
                {
                    lblSelectPatient.Text = "请选择看诊患者";
                }
            }
        }
        ...
        /// <summary>
        /// 是否显示全部队列，否则就只显示当前午别的队列
        /// </summary>
        bool isShowAllQueue = false;

        /// <summary>
        /// 是否显示全部队列，否则就只显示当前午别的队列
        /// </summary>
        [Category("显示设置"), Description("是否显示全部队列，否则就只显示当前午别的队列")]
        public bool IsShowAllQueue
        {
            get
            {
                return isShowAllQueue;
            }
            set
            {
                isShowAllQueue = value;
            }
        }
        ...
        /// 队列名称里面是否显示科室
        /// </summary>
        bool isShowDeptName = true;

        /// <summary>
        /// 队列名称里面是否显示科室
        /// </summary>
        [Category("显示设置"), Description("队列名称里面是否显示科室")]
        ...
        protected override FS.FrameWork.WinForms.Forms.ToolBarService OnInit(object sender, object neuObject, object param)
        {
            ...
            this.toolBarService.AddToolButton("叫号", "选择患者叫号进诊", (int)FS.FrameWork.WinForms.Classes.EnumImageList.X下一个, true, false, null);
            //this.toolBarService.AddToolButton("取消叫号", "取消患者叫号", (int)FS.FrameWork.WinForms.Classes.EnumImageList.Q清空, true, false, null);
        ...
        {
            DateTime current = this.localMgr.GetDateTimeFromSysDateTime().Date;
            string myNurseDept = "";
            ...
            ///待修改，现在按照诊室查询的，待分诊患者就无法查到了
            ///暂时还保留原有模式吧

            ArrayList alCollections = this.regMgr.QueryNoTriagebyDept(current, myNurseDept);
            if (alCollections != null)
            {
                foreach (FS.HISFC.Models.Registration.Register obj in alCollections)
                {
        ...
        private string GetDoct(string seatID)
        {
            ...
            for (int i = 0; i < this.lvQueue.Items.Count; i++)
            {
                ...
                DateTime current = this.localMgr.GetDateTimeFromSysDateTime();
        ...
        private int RefreshRoom()
        {
            ...
            //查询分诊台包含的诊室列表
            ArrayList alCollections = roomMgr.GetRoomInfoByNurseNo(((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept.ID);

            if (alCollections != null)
            {
                foreach (FS.HISFC.Models.Nurse.Room room in alCollections)
                {
                    if (room.IsValid == "0")
                    {
                        continue;
                    ...
                    ArrayList alConsole = this.seatMgr.Query(room.ID);
                    if (alConsole != null)
                    {
        ...
        private void AddPatientByConsole()
        {
            ...
            ArrayList alCollections = assginMgr.QueryUnionRegister(((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept.ID, localMgr.GetDateTimeFromSysDateTime().Date, FS.HISFC.Models.Nurse.EnuTriageStatus.In);

            if (alCollections != null)
            {
                foreach (FS.HISFC.Models.Nurse.Assign assign in alCollections)
                {
        ...
        {
            DateTime current = localMgr.GetDateTimeFromSysDateTime();
        ...
        private int RefreshQueue(string nurseID, DateTime queueDate, string noonID)
        {
            ...
            ArrayList alCollections = queueMgr.Query(nurseID, queueDate.Date.ToShortDateString());
            
            if (!isShowAllQueue)
            {
                ArrayList alTemp = new ArrayList();
                foreach (FS.HISFC.Models.Nurse.Queue queue in alCollections)
                {
                    if (queue.Noon.ID != noonID
                        || !queue.IsValid)
                    {
                ...
                alCollections = alTemp;
            }

            //人数不一样时才刷新
            if (this.lvQueue.Items.Count != alCollections.Count)
            {
                this.lvQueue.Items.Clear();
            }
            //午别变化时也要刷新
            bool isNoonChange = false;
            ...
            if (alCollections != null)
            {
                ...
                if (this.lvQueue.SelectedItems != null
                    && this.lvQueue.SelectedItems.Count > 0)
                {
                    ListViewItem selected = this.lvQueue.SelectedItems[0];

                    if (selected.Tag != null)
                    {
                        selectQueue = (selected.Tag as FS.HISFC.Models.Nurse.Queue).ID;
                    }
                }
                ...
                foreach (FS.HISFC.Models.Nurse.Queue queue in alCollections)
                {
                    ...
                    item.SubItems.Add(queue.AssignDept.Name);
                    item.SubItems.Add(SOC.HISFC.BizProcess.Cache.Common.GetEmployeeName(queue.Doctor.ID));
                    item.SubItems.Add(queue.Doctor.ID);
                    item.SubItems.Add(queue.SRoom.Name);
                    item.SubItems.Add(queue.WaitingCount.ToString());
                    item.SubItems.Add(SOC.HISFC.BizProcess.Cache.Common.GetNoonInfo(queue.Noon.ID).Name);

                    ArrayList al = assignMgr.Query(nurseID, queueDate.Date, queue.ID, FS.HISFC.Models.Nurse.EnuTriageStatus.Triage);
                    if (al == null)
                    {
                        MessageBox.Show("查询队列信息出错！\r\n" + assignMgr.Err, "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return -1;
                    }
                    count += al.Count;

                    al = assignMgr.Query(nurseID, queueDate.Date, queue.ID, FS.HISFC.Models.Nurse.EnuTriageStatus.Delay);
                    if (al == null)
                    {
                        MessageBox.Show("查询队列信息出错！\r\n" + assignMgr.Err, "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return -1;
                    ...
                    item.Text = queue.Name;
                    if (queue.Memo != null && queue.Memo != "")
                        item.Text = item.Text + "\n" + "[" + queue.AssignDept.Name + "]";

                    //item.Text = item.Text + "(" + obj.WaitingCount.ToString() + "人)";
                    if (!queue.IsValid)
                    {
                        item.Text = item.Text + "(" + count + "人)\r\n" + "[停]" + FS.SOC.HISFC.BizProcess.Cache.Common.GetNoonInfo(queue.Noon.ID).Name + (isShowDeptName ? SOC.HISFC.BizProcess.Cache.Common.GetDeptName(queue.AssignDept.ID) : "");
                    }
                    else
                    {
                        item.Text = item.Text + "(" + count + "人)\r\n" + FS.SOC.HISFC.BizProcess.Cache.Common.GetNoonInfo(queue.Noon.ID).Name + (isShowDeptName ? SOC.HISFC.BizProcess.Cache.Common.GetDeptName(queue.AssignDept.ID) : "");
                    }
                    ...
                    if (queue.Noon.ID != noonID
                        || !queue.IsValid)
                    {
                        item.BackColor = System.Drawing.Color.FromArgb(255, 128, 128);
                    ...
                    {
                        ListViewItem viewItem = lvQueue.Items[i] as ListViewItem;
                        if ((viewItem.Tag as FS.HISFC.Models.Nurse.Queue).ID == queue.ID)
                        {
                            lvQueue.Items[i] = item;
                            if (queue.ID == selectQueue)
                            {
                                lvQueue.Items[i].Selected = true;
                            }
                            isContain = true;
                            break;
                        }
        ...
        private void LoadMenuSet()
        {
            try
            {
                ...
                doc3.Load(Application.StartupPath + "/Setting/arrangeSetting.xml");
                refreshTime = doc3.SelectSingleNode("//RefreshTime").InnerText;
        ...
        private void lvQueue_DragDrop(object sender, DragEventArgs e)
        {
            ...
            if (e.Data.GetDataPresent(typeof(FS.HISFC.Models.Registration.Register)))
            {
                ...
                //FS.FrameWork.WinForms.Classes.Function.ShowBalloonTip(2, "提示", p.X.ToString() + " Y:" + p.Y.ToString() + "\r\n\r\n" + item.ToString(), ToolTipIcon.Info);
                ...
                //FS.FrameWork.WinForms.Classes.Function.ShowBalloonTip(2, "提示", "检查前" + item.ToString(), ToolTipIcon.Info);
                if (Function.CheckArray(assign.Register, info) == -1)
                {
                    return;
                }
                //FS.FrameWork.WinForms.Classes.Function.ShowBalloonTip(2, "提示", "检查后" + item.ToString(), ToolTipIcon.Info);
                ...
                try
                {
                    ...
                    DateTime current = localMgr.GetDateTimeFromSysDateTime();// this.regMgr.GetDateTimeFromSysDateTime();
                    string noonID = FS.SOC.HISFC.Components.Nurse.Controls.ZDLY.Array.Function.GetNoon(current);
                    ...
                    //该患者已经退号
                    if (this.Judgeback(assign.Register))
                    {
                        MessageBox.Show("该患者已经退号!请刷新屏幕!");
                        return;
                    }
                    //该患者已经分诊
                    if (this.JudgeTrige(assign.Register))
                    {
                    ...
                    */
                    ...
                    assign.TirageTime = localMgr.GetDateTimeFromSysDateTime(); //localMgr.GetDateTimeFromSysDateTime().Date;// this.regMgr.GetDateTimeFromSysDateTime();
                    ...
                    this.RetrieveWait(((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept.ID, localMgr.GetDateTimeFromSysDateTime().Date, (item.Tag as FS.HISFC.Models.Nurse.Queue).ID);

                    lblRoomInfo.Text = SOC.HISFC.BizProcess.Cache.Common.GetDeptName((item.Tag as FS.HISFC.Models.Nurse.Queue).AssignDept.ID) + "  " + (item.Tag as FS.HISFC.Models.Nurse.Queue).SRoom.Name;
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message + ex.StackTrace, "异常错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
            }
        }
        ...
        /// <returns></returns>
        private bool JudgeTrige(FS.HISFC.Models.Registration.Register regObj)
        {
            //从FIN_OPB_REGISTER中,根据ClinicCode取出数据,如果assign_flag是Triage,则返回true,否则返回false
            bool bl = false;
            bl = this.regMgr.QueryIsTriage(regObj.ID);
        ...
        /// <param name="ClinicCode"></param>
        /// <returns></returns>
        private bool Judgeback(FS.HISFC.Models.Registration.Register regObj)
        {
            //从FIN_OPB_REGISTER中,根据ClinicCode取出数据,如果valid_flag = 1 返回false,否则返回true
            bool bl = false;
            bl = this.regMgr.QueryIsCancel(regObj.ID);
        ...
        private FS.HISFC.Models.Nurse.Queue GetMinQueue(ArrayList al, string regDept)
        {
            ...
            DateTime currenttime = localMgr.GetDateTimeFromSysDateTime().Date; //this.regMgr.GetDateTimeFromSysDateTime();
        ...
        private void RetrieveWait(string nurseID, DateTime today, string queueID)
        {
            ...
            foreach (ListViewItem item in lvPatient.Items)
            {
                if (item.Tag != null
                    &&item.Selected)
                {
                    selectedItem = (item.Tag as FS.HISFC.Models.Nurse.Assign).ID;
                }
            }

            this.lvPatient.Items.Clear();

            ArrayList al = this.assginMgr.Query(nurseID, today.Date, queueID, FS.HISFC.Models.Nurse.EnuTriageStatus.Triage);

            //显示延迟叫号患者
            al.AddRange(this.assginMgr.Query(nurseID, today.Date, queueID, FS.HISFC.Models.Nurse.EnuTriageStatus.Delay));

            if (al != null)
            {
                foreach (FS.HISFC.Models.Nurse.Assign assgin in al)
                {
                    this.AddAssign(assgin, selectedItem);
                }
            }
        }
        ...
        /// </summary>
        /// <param name="assign"></param>
        private void AddAssign(FS.HISFC.Models.Nurse.Assign assign, string selectedItem)
        {
            ListViewItem item = new ListViewItem();
            ...
            //这里队列的排序号，要从分诊表获取，因为可能存在二次分诊的情况
            item.SubItems.Add(retTemp.OrderNO.ToString() + "(" + assign.SeeNO.ToString() + ")");
        ...
        private int ValidRegInfo(FS.HISFC.Models.Registration.Register reginfo)
        {
            ...
            //			//该患者今天没有挂号信息
            //			if(reginfo.RegDate.ToString("yyyy-MM-dd") != localMgr.GetDateTimeFromSysDateTime().ToString("yyyy-MM-dd"))
            //			{
        ...
        {
            DateTime dt = localMgr.GetDateTimeFromSysDateTime().Date;
            ...
            if (assign != null && !this.Judgeback(assign.Register) && assign.TriageDept == ((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept.ID)
            {
                //对已经分诊的信息,入诊室
        ...
        private int QueryReg(string cardNo)
        {
            ...
            DateTime dtvalid = localMgr.GetDateTimeFromSysDateTime().Date.AddDays(1 - validDays);// this.regMgr.GetDateTimeFromSysDateTime().Date.AddDays(1 - validDays);
            ...
            {
                FS.HISFC.Models.Registration.Register obj = (FS.HISFC.Models.Registration.Register)nodeCompare.Tag;
                if (obj.ID == reginfo.ID 
                    && obj.PID.CardNO == reginfo.PID.CardNO 
                    && obj != null && obj.PID.CardNO != null)
                {
                    IsFound = true;
        ...
        private void AutoExpert()
        {
            ...
            ArrayList alCollections = this.regMgr.QueryNoTriagebyDept(current, ((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept.ID);
            if (alCollections == null) return;
            ...
            #region 自动分诊
            foreach (FS.HISFC.Models.Registration.Register reg in alCollections)
            {
                if (reg.DoctorInfo.Templet.Doct.ID != null || reg.DoctorInfo.Templet.Doct.ID != "")
                {
                    ...
                    for (int i = 0; i < this.lvQueue.Items.Count; i++)
                    {
                            ...
                            && assign.Queue.Console.ID != null && assign.Queue.Console.ID != "")
                        {
                            ...
                            assign.TriageDept = ((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept.ID;
                            assign.TirageTime = localMgr.GetDateTimeFromSysDateTime().Date;// this.regMgr.GetDateTimeFromSysDateTime();
                            assign.Oper.OperTime = assign.TirageTime;
                            assign.Oper.ID = FS.FrameWork.Management.Connection.Operator.ID;
                            assign.Queue.Dept = assign.Register.DoctorInfo.Templet.Dept;
                            assign.InTime = localMgr.GetDateTimeFromSysDateTime().Date;// this.regMgr.GetDateTimeFromSysDateTime();
            ...
            FS.HISFC.Models.Nurse.Queue queue)
        {
            ...
            //assign.TriageDept = ((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Nurse.ID;
            assign.TriageDept = ((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept.ID;
            assign.TirageTime = localMgr.GetDateTimeFromSysDateTime();// this.regMgr.GetDateTimeFromSysDateTime();
            assign.Oper.OperTime = assign.TirageTime;
            assign.Oper.ID = FS.FrameWork.Management.Connection.Operator.ID;
            assign.Queue.Dept = assign.Register.DoctorInfo.Templet.Doct;
            assign.InTime = localMgr.GetDateTimeFromSysDateTime();
        ...
        private void Auto()
        {
            ...
            //获取时间
            DateTime currenttime = localMgr.GetDateTimeFromSysDateTime();// this.regMgr.GetDateTimeFromSysDateTime();
            DateTime current = currenttime.Date;
            string noonID = FS.SOC.HISFC.Components.Nurse.Controls.ZDLY.Array.Function.GetNoon(currenttime);
            ...
            ArrayList alCollections = this.regMgr.QueryNoTriagebyDept(current, ((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept.ID);
            if (alCollections == null || alCollections.Count <= 0) return;
            ...
            #region 专家
            foreach (FS.HISFC.Models.Registration.Register reginfo in alCollections)
            {
        ...
        private void CancelTriage()
        {
            ...
            INurseAssign.CancelCall((selected.Tag as FS.HISFC.Models.Nurse.Assign).Register.ID);
        ...
        {
            DateTime endDate = localMgr.GetDateTimeFromSysDateTime();
        ...
        private void In()
        {
            ...
            //防止已经退号
            if (this.Judgeback(assigninfo.Register))
            {
                MessageBox.Show("该患者已经退号!请取消其分诊信息");
                return;
            }
        ...
        FS.SOC.HISFC.CallQueue.Interface.INurseAssign INurseAssign = null;

        private void InitINurseAssign()
        {
            if (INurseAssign == null)
            {
                INurseAssign = FS.FrameWork.WinForms.Classes.UtilInterface.CreateObject(this.GetType(), typeof(FS.SOC.HISFC.CallQueue.Interface.INurseAssign)) as FS.SOC.HISFC.CallQueue.Interface.INurseAssign;
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        private int CancleCalled()
        {
            FS.HISFC.Models.Nurse.Assign assign = this.assginMgr.QueryByClinicID(currentReg.ID);
            if (assign == null
                || string.IsNullOrEmpty(assign.Register.ID))
            {
                MessageBox.Show("患者还未分诊，无法取消叫号！\r\n\r\n", "操作错误>>", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                return -1;
            }

            string sql = @"update fin_opr_register r
                            set r.TRIAGE_FLAG='0'
            ...
            int rtn = assginMgr.ExecNoQuery(sql);

            if (rtn == -1)
            {
                FS.FrameWork.Management.PublicTrans.RollBack();
                MessageBox.Show("取消叫号失败！\r\n" + assginMgr.Err, "提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return -1;
            }
            else if (rtn == 0)
            {
                FS.FrameWork.Management.PublicTrans.RollBack();
                MessageBox.Show("取消叫号失败！\r\n患者可能已经退号或已看诊！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return -1;
            }

            sql = @"delete FROM met_nuo_assignrecord f  --护士分诊记录表
                        where f.clinic_code='{0}'";

            sql = string.Format(sql, assign.Register.ID);

            rtn = assginMgr.ExecNoQuery(sql);
        ...
        private int Call()
        {
            ////MessageBox.Show("叫号成功！", "提示>>", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            ////return 1;
            ...
            {
                string errInfo = "";
                FS.HISFC.Models.Base.Noon noon = Function.GetNoon(this.regMgr.GetDateTimeFromSysDateTime());

                FS.HISFC.Models.Base.Noon baseNoon = new FS.HISFC.Models.Base.Noon();
                baseNoon.ID = noon.ID;
                baseNoon.Name = noon.Name;
                baseNoon.StartTime = noon.StartTime;
                baseNoon.EndTime = noon.EndTime;

                FS.HISFC.Models.Nurse.Assign assign = this.assginMgr.QueryByClinicID(currentReg.ID);
                if (assign == null
                    || string.IsNullOrEmpty(assign.Register.ID))
                {
                    MessageBox.Show("患者还未分诊，无法叫号！\r\n\r\n因无法确认看诊的诊室！", "操作错误>>", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    return -1;
                }

                if (INurseAssign.Insert(this.currentReg, currentReg.DoctorInfo.Templet.Dept, ((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept, assign.Queue.SRoom, assign.Queue.Console, baseNoon, ref errInfo) < 0)
                {
                    MessageBox.Show("叫号出错，请联系管理员！\r\n" + errInfo, "警告>>", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return -1;
                }

                MessageBox.Show("患者【" + currentReg.Name + "】叫号成功！", "提醒>>", MessageBoxButtons.OK, MessageBoxIcon.Information);

                return 1;
            }
            else
            {
                MessageBox.Show("叫号功能未配置，请联系管理员！\r\n未配置叫号接口...", "警告>>", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return -1;
            }
        }
        ...
        private void txtCard2_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(txtCard2.Text.Trim()))
            {
                return;
            }
            ...
            string whereSQL = @"
                                where (name ='{0}'
                                or order_no='{0}'
                                or card_no = lpad('{0}',10,'0')
                                or card_no = (select f.card_no from fin_opb_accountcard f
                                    where f.markno='{0}'))
                                and trunc(reg_date)=trunc(sysdate)";

            //录入的是数字
            //if (Regex.IsMatch(txtCard2.Text.Trim(), @"^/d+$"))
            //{
            if (Regex.IsMatch(txtCard2.Text.Trim(), @"^\d*$"))
            {
                whereSQL = @"
            ...
            whereSQL = string.Format(whereSQL, txtCard2.Text.Trim());
            ArrayList alReg = regMgr.QueryRegListBase(whereSQL);
            if (alReg == null)
            {
                MessageBox.Show("查询挂号信息失败!\r\n\r\n" + regMgr.Err, "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            if (alReg.Count == 1)
            {
                cardNo = ((FS.HISFC.Models.Registration.Register)alReg[0]).PID.CardNO;
            }
            else
            {
                ArrayList alTemp = new ArrayList();
                foreach (FS.HISFC.Models.Registration.Register reg in alReg)
                {
                    FS.HISFC.Models.Base.Spell spe = new FS.HISFC.Models.Base.Spell();
                    spe.ID = reg.ID;
                    spe.Name = reg.Name;
                    spe.Memo = reg.Pact.Name;
                    spe.SpellCode = reg.DoctorInfo.Templet.Dept.Name;
                    spe.WBCode = reg.DoctorInfo.Templet.Doct.Name;
                    spe.UserCode = reg.DoctorInfo.SeeDate.ToString();
                ...
                if (alTemp.Count > 0)
                {
                    ...
                    if (FrameWork.WinForms.Classes.Function.ChooseItem(alTemp, new string[] { "门诊流水号", "姓名", "合同单位", "挂号科室", "挂号医生", "挂号时间" }, new bool[] { false, true, true, true, true, true }, new int[] { 50, 50, 70, 100, 70, 160 }, ref selectPatient) != 1)
                    {
                        return;
                    }
                    foreach (FS.HISFC.Models.Registration.Register reg in alReg)
                    {
                        if (reg.ID == selectPatient.ID)
                        {
                            cardNo = reg.PID.CardNO;
                            break;
                        }
                    }
                }
                ...
                {
                    MessageBox.Show("没有找到患者信息！", "警告", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
            ...
            if (assign != null && !this.Judgeback(assign.Register) 
                //&& assign.TriageDept == ((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept.ID /*var.User.Nurse.ID*/
                )
            {
                //查询已经进诊的信息
                ...
                //{
                    ...
                    string temp = "患者名称:" + assign.Register.Name + "\n\r\n"
                        + "门诊号：" + assign.Register.PID.CardNO + "\n\r\n"
                        + "-----------------------------" + "\n\r\n"
                        + "诊区:" + SOC.HISFC.BizProcess.Cache.Common.GetDeptName(assign.TriageDept) + "\n\r\n"
                        + "诊室:" + assign.Queue.SRoom.Name + "\n\r\n"
                        + "诊台:" + assign.Queue.Console.Name + "\n\r\n"
                        + "诊室序号:" + assign.SeeNO.ToString() + "\n\r\n"
                        + "全院序号" + assign.Register.OrderNO.ToString() + "\n\r\n"
                        + "看诊医生:" + this.GetDoct(assign.Queue.Console.ID) + "\n\r\n"
                        + "挂号时间:" + assign.Register.DoctorInfo.SeeDate.ToString();
                    MessageBox.Show(temp, "患者查询", MessageBoxButtons.OK, MessageBoxIcon.Information);
                //}
            }
            if (!bl)
            {
                MessageBox.Show("没有分诊信息，可能患者不是通过门诊挂号途径进行看诊的！", "警告", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        {
            if (this.neuTreeView2.GetNodeAt(e.X, e.Y).Tag is FS.HISFC.Models.Nurse.Assign)
            {
                //获取分诊信息
                FS.HISFC.Models.Nurse.Assign info = new FS.HISFC.Models.Nurse.Assign();
        ...
        private void UpMove()
        {
            ...
            this.RetrieveWait(((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept.ID,/*var.User.Nurse.ID, */
                            localMgr.GetDateTimeFromSysDateTime().Date, /*this.regMgr.GetDateTimeFromSysDateTime().Date*/
                            info.Queue.ID);
        ...
        private void CalledPatient()
        {
            ...
            //        //查询最近分诊的患者
            //        FS.HISFC.Models.Nurse.Assign assignObj = this.assginMgr.QueryLastAssignPatient(((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept.ID, localMgr.GetDateTimeFromSysDateTime().Date, queueID, "2");
            //        if (assignObj != null)
            //        {
            //            try
            //            {
            ...
            //                this.controlMgr.Speak("请" + assignObj.Register.Name + "到" + assignObj.Queue.Console.Name + "就诊");
            //                FS.FrameWork.Management.PublicTrans.BeginTransaction();
            //                if (this.assginMgr.UpdateByCalled(queueID, assignObj.Register.ID, localMgr.GetDateTimeFromSysDateTime(), "") == -1)
            //                {
            //                    FS.FrameWork.Management.PublicTrans.RollBack();
            ...
            {
                DateTime current = localMgr.GetDateTimeFromSysDateTime();
                string noonID = Function.GetNoonID(current);
        ...
        private void neuTreeView1_MouseUp(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                ...
                else if (this.neuTreeView1.SelectedNode != null && this.neuTreeView1.Nodes.Count > 0)
                {
                    //this.neuTreeView1.ContextMenuStrip = this.contextMenuStrip3;
                    this.neuTreeView1.SelectedNode.ContextMenuStrip = this.contextMenuStrip3;
                    this.MenuSetControl(true);
                }
                //else
                //{
                //    this.neuTreeView1.ContextMenuStrip = null;
                //}

            }


            Point p = this.neuTreeView1.PointToClient(Cursor.Position);

            TreeNode node = this.neuTreeView1.GetNodeAt(p.X, p.Y);
            ...
            if (node.Tag is FS.HISFC.Models.Registration.Register)
            {
                this.CurrentReg = node.Tag as FS.HISFC.Models.Registration.Register;
            }
        }
        ...
        private void lvPatient_MouseUp(object sender, MouseEventArgs e)
        {
            if (lvPatient.SelectedItems.Count > 0)
            {
                ListViewItem selected = this.lvPatient.SelectedItems[0];
                FS.HISFC.Models.Nurse.Assign assign = selected.Tag as FS.HISFC.Models.Nurse.Assign;
                if (assign != null)
                {
                    CurrentReg = assign.Register;

                    if (e.Button == MouseButtons.Right)
                    {
                        this.contextMenu1.Items.Clear();

                        ToolStripMenuItem mnuChange = new ToolStripMenuItem();//停止
                        mnuChange.Click += new EventHandler(mnuChange_Click);
                        mnuChange.Text = "重新排队叫号！";
                        this.contextMenu1.Items.Add(mnuChange);//删除、作废
                        this.contextMenu1.Show(lvPatient, new Point(e.X, e.Y));
                    }
                }
            }
        ...
        {
            FS.FrameWork.Management.PublicTrans.BeginTransaction();
            int rev = assginMgr.UpdateAssignFlag(CurrentReg.ID, assginMgr.Operator.ID, FS.HISFC.Models.Nurse.EnuTriageStatus.Triage);
            ...
            if (rev <= 0)
            {
                ...
                if (rev < 0)
                {
                    MessageBox.Show("重新排队叫号失败！\r\n" + assginMgr.Err, "错误", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                }
                else
                {
            ...
            MessageBox.Show("重新排队叫号成功！\r\n", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);

            lvQueue_Click(sender, e);
        }

        private void neuTreeView1_MouseDown(object sender, MouseEventArgs e)
        {
            if (neuTreeView1.SelectedNode != null)
            {
            }
        }
        ...
        private void lvQueue_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                this.RetrieveWait(((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept.ID, this.localMgr.GetDateTimeFromSysDateTime().Date, (selected.Tag as FS.HISFC.Models.Nurse.Queue).ID);

                lblRoomInfo.Text = SOC.HISFC.BizProcess.Cache.Common.GetDeptName((selected.Tag as FS.HISFC.Models.Nurse.Queue).AssignDept.ID) + "  " + (selected.Tag as FS.HISFC.Models.Nurse.Queue).SRoom.Name;
            }
            else
            {

                lblRoomInfo.Text = selected.Text;
                this.RetrieveWait(((FS.HISFC.Models.Base.Employee)FS.FrameWork.Management.Connection.Operator).Dept.ID, localMgr.GetDateTimeFromSysDateTime().Date, (selected.Tag as FS.HISFC.Models.Registration.DoctSchema).ID);
            }
        }
    }
