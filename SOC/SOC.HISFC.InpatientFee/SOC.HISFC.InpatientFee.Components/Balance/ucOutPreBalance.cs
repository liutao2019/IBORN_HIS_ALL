...
{
    //{C4231074-D350-4df9-AF7C-C37124B44B80}
    public partial class ucOutPreBalance : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        #region 管理类
        /// <summary>
        /// 合同管理类
        ...
        /// <summary>
        /// 是否进行匹配  //{a93690aa-75f9-4e8d-95de-8ec3d34b98ba}
        /// </summary>
        private bool IsMatchingBypack = false;

        /// <summary>
        /// 是否预结算匹配
        /// </summary>
        private bool IsYjsMatching = false;
        /// <summary>
        /// 支付方式输入框
        /// </summary>
        ...
        /// <summary>
        /// 费用全清  {34a15202-a3f9-4d3e-9bad-c7e6783b540c}
        /// </summary> 
        /// <param name="sender"></param>
        /// <param name="e"></param>
        ...
        private void btnPackageMatch_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                this.IsMatching = true;
                //{a93690aa-75f9-4e8d-95de-8ec3d34b98ba}
                this.IsMatchingBypack = true;
        ...
        private void FpTotFee_Sheet1_CellChanged(object sender, SheetViewEventArgs e)
        {
            ...
            try
            {
                ...
                if (this.hsSelectedFeeItem.ContainsKey(curFee.ID))
                {
                    ...
                    //{d5007c0f-0c23-40b9-9341-4d7c6735eab1}
                    if ((!isfee && !IsMatching) || IsYjsMatching)
                    {
                        int insers = this.FpSelectedFee_Sheet1.Rows.Count;
                        this.FpSelectedFee_Sheet1.Rows.Add(insers, 1);
                        this.FpSelectedFee_Sheet1.Rows[insers].Tag = SelectedItem;
                        this.FpSelectedFee_Sheet1.Cells[insers, (int)SelectedFeeCols.ItemName].Text = SelectedItem.Item.Name;
                        this.FpSelectedFee_Sheet1.Cells[insers, (int)SelectedFeeCols.Spec].Text = SheetObj.Cells[e.Row, (int)TotFeeCols.Spec].Text;
                        this.FpSelectedFee_Sheet1.Cells[insers, (int)SelectedFeeCols.Price].Value = SelectedItem.Item.Price;
                        this.FpSelectedFee_Sheet1.Cells[insers, (int)SelectedFeeCols.Unit].Text = SheetObj.Cells[e.Row, (int)TotFeeCols.Unit].Text;
                        this.FpSelectedFee_Sheet1.Cells[insers, (int)SelectedFeeCols.Qty].Value = SelectedItem.Item.Qty;
                        this.FpSelectedFee_Sheet1.Cells[insers, (int)SelectedFeeCols.PriceUnit].Text = SelectedItem.Item.PriceUnit;
                        string unit = string.Empty;
                        if (SelectedItem.Item.ItemType == EnumItemType.Drug)
                        {
                            FS.HISFC.Models.Pharmacy.Item pharmacy = SelectedItem.Item as FS.HISFC.Models.Pharmacy.Item;
                        ...
                        this.FpSelectedFee_Sheet1.Cells[insers, (int)SelectedFeeCols.Cost].Value = costValue; //SelectedItem.Item.Price * SelectedItem.Item.Qty;
                        this.FpSelectedFee_Sheet1.Cells[insers, (int)SelectedFeeCols.Cost].CellType = nCost;
                    }
                }
                else
                {
                    ...
                    SelectedItem.Order.ID = curFee.Order.ID;
        ...
        /// <summary>
        /// 套餐预结算 {983972e8-1c2e-4502-8fe9-2acd5beb5a0a}
        /// </summary>
        /// <returns></returns>
        public int PreBalance()
        {
            ...
            if (FpSelectedFee_Sheet1.Rows.Count <= 0)
            {
            ...
            else
            {
                ...
                for (int i = 0; i < FpSelectedFee_Sheet1.Rows.Count; i++)
                {
                    ...
                    detail.Order_ID = updateFee.Order.ID;
        ...
        {
            toolBarService.AddToolButton("住院预结算", "住院预结算", (int)FS.FrameWork.WinForms.Classes.EnumImageList.B保存, true, false, null);
        ...
        public override void ToolStrip_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            {
                case "住院预结算":
                    string tab = this.tabPackage.SelectedTab.Text;
                    this.PreBalance();
                    this.GetPreBalance();
                    break;
        ...
        private bool ValidPreBalancePackage()
        {
        ...
        private void PreBalanceMatching()
        {

            //{a93690aa-75f9-4e8d-95de-8ec3d34b98ba}
            if (this.FpPreBalance_Sheet1.ActiveRow == null)
            {
                MessageBox.Show("预结算记录不存在");
                return;
            }
            ...
            this.IsYjsMatching = true;//{d5007c0f-0c23-40b9-9341-4d7c6735eab1}
            FS.HISFC.Models.Fee.Inpatient.PreBalance head = this.FpPreBalance_Sheet1.ActiveRow.Tag as FS.HISFC.Models.Fee.Inpatient.PreBalance;
            ...
            this.IsMatching = false;
            this.IsYjsMatching = false;//{d5007c0f-0c23-40b9-9341-4d7c6735eab1}
            FS.FrameWork.WinForms.Classes.Function.HideWaitForm();
            this.SetCostInfo(this.BalanceEco);

        }

        private void DeletePreBalance()
        {
            if (MessageBox.Show("是否作废这条预结算记录？？", "Confirm Message", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                FS.HISFC.Models.Fee.Inpatient.PreBalance head = this.FpPreBalance_Sheet1.ActiveRow.Tag as FS.HISFC.Models.Fee.Inpatient.PreBalance;
                FS.HISFC.Models.Base.Employee oper = FS.FrameWork.Management.Connection.Operator as FS.HISFC.Models.Base.Employee;
                head.CANCELCODE = oper.ID;
                head.CANCELDATE = DateTime.Now;
                head.CANCELMAN = oper.Name;
                head.ISVALID = "0";
                this.prebalancelogic.UpdatePreBalance(head);
