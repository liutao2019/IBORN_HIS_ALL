...
namespace FS.SOC.HISFC.InpatientFee.Components.Balance
{
    ...
    /// </修改记录>
    /// </summary>
    public partial class ucDerateFee : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        public ucDerateFee()
        {
            ...
            this.cmbMinFee.TextChanged += new EventHandler(cmbMinFee_TextChanged);
            this.fpFeeInfo.ActiveSheetChanged += new EventHandler(fpFeeInfo_ActiveSheetChanged);
        }
        ...
        protected virtual void SetPatientInfo(FS.HISFC.Models.RADT.PatientInfo patientInfo)
        {
            ...
            else
            {
...

                {
                    ...
                    FS.HISFC.Models.Base.PactInfo pact = PactManagment.GetPactUnitInfoByPactCode(patientInfo.Pact.ID);
                    string pactName = patientInfo.Pact.Name + " 自付比例：" + pact.Rate.PayRate * 100 + "%";//合同单位
                    this.lblPatientInfo.Text = "姓名:" + patientInfo.Name + " 入院日期:" + patientInfo.PVisit.InTime.ToString("yyyy-MM-dd") + " 出院日期:" + outDate + " 天数:" + days + "结算类别:" + pactName + " 日限额：" + patientInfo.FT.DayLimitCost + " 出院诊断:" + diagnoseName + strBabyInfo;
                }
                else
                {
        ...
        protected virtual int GetTotCost()
        {
            ...
            ds = this.derateMgr.GetTotFeeAndDerateByInpatientNO(this.patientInfo.ID);
            if (ds == null)
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("查询患者总费用出错") + this.derateMgr.Err);
                return -1;
            }

            this.fpFeeInfo_TotFee.Cells[0, (int)EnumTotFeeCol.TotCost].Text = ds.Tables[0].Rows[0][1].ToString();
            this.fpFeeInfo_TotFee.Cells[0, (int)EnumTotFeeCol.OwnCost].Text = ds.Tables[0].Rows[0][2].ToString();
            this.fpFeeInfo_TotFee.Cells[0, (int)EnumTotFeeCol.PayCost].Text = ds.Tables[0].Rows[0][3].ToString();
            this.fpFeeInfo_TotFee.Cells[0, (int)EnumTotFeeCol.PubCost].Text = ds.Tables[0].Rows[0][4].ToString();
            this.fpFeeInfo_TotFee.Cells[0, (int)EnumTotFeeCol.DerateCost].Text = ds.Tables[0].Rows[0][5].ToString();
            this.fpFeeInfo_TotFee.Cells[0, (int)EnumTotFeeCol.UnDerateCost].Text = ds.Tables[0].Rows[0][6].ToString();
        ...
        protected virtual int GetMinfee()
        {
            ...
            if (ds == null)
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("查询患者最小费用出错") + this.derateMgr.Err);
                return -1;
            }
            this.fpFeeInfo_FeeCode.DataAutoSizeColumns = false;
            this.fpDerateInfo_Sheet1.DataAutoHeadings = false;
            this.fpFeeInfo_FeeCode.DataSource = ds.Tables[0].DefaultView;
            return 1;
        }
        ...
        protected virtual int GetFeeDetail()
        {
            ...
            if (ds == null)
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("查询患者费用明细出错") + this.derateMgr.Err);
                return -1;
            }
            this.fpFeeInfo_Items.DataAutoHeadings = false;
            this.fpFeeInfo_Items.DataAutoSizeColumns = false;

            this.fpFeeInfo_Items.DataSource = ds.Tables[0].DefaultView;
            return 1;
        }
        ...
        protected int DerateTotoalFee(bool isRate, decimal derateFee, decimal totOwnFee, decimal rate)
        {
            if (!isRate) //按金额
            {
                ...
                if (totOwnFee <= 0)
                {
                    MessageBox.Show(Language.Msg("可减免金额不能小于等于0"));
                    return -1;
                }

                if (derateFee > totOwnFee)
                {
                    MessageBox.Show(Language.Msg("减免金额应该小于可减免金额总额"));
                    this.ntxtFee.Focus();
                    return -1;
                }
            ...
            //已经减免的金额
            decimal deratedFee = FS.FrameWork.Function.NConvert.ToDecimal(this.fpFeeInfo_TotFee.Cells[0, (int)EnumTotFeeCol.DerateCost].Text);
            ...
            {
                //取可减免金额
                decimal ownCost = FS.FrameWork.Function.NConvert.ToDecimal(this.fpFeeInfo_FeeCode.Cells[i, 8].Text);
                //如果可减免金额为零，则返回
                if (ownCost <= 0)
                {
                    continue;
                ...
                derateFeeObj.FeeCode = this.fpFeeInfo_FeeCode.Cells[i, (int)EnumItemCol.MinFeeCode].Text;
                derateFeeObj.FeeName = this.fpFeeInfo_FeeCode.Cells[i, (int)EnumItemCol.MinFeeName].Text;
                derateFeeObj.ItemCode = this.fpFeeInfo_FeeCode.Cells[i, (int)EnumItemCol.ItemCode].Text;
                derateFeeObj.ItemName = this.fpFeeInfo_FeeCode.Cells[i, (int)EnumItemCol.ItemName].Text;
            ...
            this.fpFeeInfo_TotFee.Cells[0, (int)EnumTotFeeCol.DerateCost].Text = (deratedFee + derateFee).ToString();
            this.fpFeeInfo_TotFee.Cells[0, (int)EnumTotFeeCol.UnDerateCost].Text = (FS.FrameWork.Function.NConvert.ToDecimal(this.fpFeeInfo_TotFee.Cells[0, (int)EnumTotFeeCol.UnDerateCost].Text) - totOwnFee).ToString();
        ...
        protected int DerateMinFee(bool isRate, decimal derateFee, decimal rate, bool isAll)
        {
            ...
            if (isAll) //全选
            {
                ...
                for (int i = 0; i < this.fpFeeInfo_FeeCode.RowCount; i++)
                {
                    if (this.fpFeeInfo_FeeCode.IsSelected(i, 0))
                    {
            ...
            {
                int row = this.fpFeeInfo_FeeCode.ActiveRowIndex;
                decimal ownCost = FS.FrameWork.Function.NConvert.ToDecimal(this.fpFeeInfo_FeeCode.Cells[row, 4].Text);
                leftCost = FS.FrameWork.Function.NConvert.ToDecimal(this.fpFeeInfo_FeeCode.Cells[row, 8].Text);
                if (isRate)
                {
                    derateFee = ownCost * rate;
                }
                ...
                if (!isAllowDerate)
                {
                    MessageBox.Show(Language.Msg(this.fpFeeInfo_FeeCode.Cells[row, 2].Text + "的可减免金额不足"));
                    return -1;
                }
                ...
                derateFeeObj.DerateKind.ID = "1";
                derateFeeObj.DerateKind.Name = this.GetDerateKindName(derateFeeObj.DerateKind.ID);
                derateFeeObj.FeeCode = this.fpFeeInfo_FeeCode.Cells[row, 1].Text;
                derateFeeObj.FeeName = this.fpFeeInfo_FeeCode.Cells[row, 2].Text;
                derateFeeObj.ItemCode = this.fpFeeInfo_FeeCode.Cells[row, 1].Text;
                derateFeeObj.ItemName = this.fpFeeInfo_FeeCode.Cells[row, 2].Text;
                derateFeeObj.DerateCost = derateFee;
                derateFeeObj.IsValid = true;
                derateFeeObj.DerateOper.Dept.ID = this.patientInfo.PVisit.PatientLocation.Dept.ID;
                derateFeeObj.DerateType = this.GetDerateType();
                derateFeeObj.DerateOper.Dept.Name = this.patientInfo.PVisit.PatientLocation.Dept.Name;
                derateFeeObj.DerateOper.ID = this.derateMgr.Operator.ID;
                //设置减免后金额
        ...
        protected int DerateFeeDetail(bool isRate, decimal derateFee, decimal rate, bool isAll)
        {
            ...
            if (isAll) //全选
            {
                ...
                {
                    if (this.fpFeeInfo_Items.IsSelected(i, 0))
                    {
            ...
            {
                int row = this.fpFeeInfo_Items.ActiveRowIndex;
                decimal ownCost = FS.FrameWork.Function.NConvert.ToDecimal(this.fpFeeInfo_Items.Cells[row, 5].Text);
                leftCost = FS.FrameWork.Function.NConvert.ToDecimal(this.fpFeeInfo_Items.Cells[row, 7].Text);
                if (isRate)
                {
                    derateFee = ownCost * rate;
                }
                ...
                derateFeeObj.DerateKind.ID = "2";
                derateFeeObj.DerateKind.Name = this.GetDerateKindName(derateFeeObj.DerateKind.ID);
                derateFeeObj.FeeCode = this.fpFeeInfo_Items.Cells[row, 0].Text;
                derateFeeObj.FeeName = this.fpFeeInfo_Items.Cells[row, 1].Text;
                derateFeeObj.ItemCode = this.fpFeeInfo_Items.Cells[row, 2].Text;
                derateFeeObj.ItemName = this.fpFeeInfo_Items.Cells[row, 3].Text;
                derateFeeObj.DerateCost = derateFee;
                derateFeeObj.IsValid = true;
                derateFeeObj.DerateOper.Dept.ID = this.patientInfo.PVisit.PatientLocation.Dept.ID;//((FS.FrameWork.Management.Connection.Operator) as FS.HISFC.Models.Base.Employee).Dept.ID;
                derateFeeObj.DerateType = this.GetDerateType();
                ...
                al.Add(derateFeeObj);
                this.InSertDataSet(al);
                int rowMinIndex = this.GetRowIndex(derateFeeObj.FeeCode);
                ...
                this.SetAfterDerateMinFee(rowMinIndex, derateFee);
                this.SetAfterDerateFeeDetail(row, derateFee);
                this.SetAtferDerateTotFee(derateFee);
            }
        ...
        protected int CancelDerateAll()
        {
            ...
            {
                if (this.fpDerateInfo_Sheet1.IsSelected(i, 0))
                {
                    rows.Add(i);
                }
            }
        ...
        {
            if (this.rbRate.Checked && FS.FrameWork.Function.NConvert.ToDecimal(ntxRate.Text) <= 0)
            {
                MessageBox.Show("请输入减免比例");
                this.ntxRate.Focus();
                return -1;
            }

            if (this.rbCost.Checked && FS.FrameWork.Function.NConvert.ToDecimal(ntxtFee.Text) <= 0)
            {
                MessageBox.Show("请输入减免金额");
                this.ntxtFee.Focus();
                return -1;
            }
        ...
        protected override int OnSave(object sender, object neuObject)
        {
            ...
            returnValue = this.feeInpatient.OpenAccount(this.patientInfo.ID);
            {
                if (returnValue == -1)
                {
                    FS.FrameWork.WinForms.Classes.Function.Msg("减免失败封帐失败" + this.feeInpatient.Err, 211);
                    FS.FrameWork.Management.PublicTrans.RollBack();
                    return -1;
                }
            }
        ...
        protected virtual int Init()
        {
            ...
            ArrayList alTemp = SOC.HISFC.BizProcess.CommonInterface.CommonController.Instance.QueryConstant(FS.HISFC.Models.Base.EnumConstant.MINFEE);
        ...
        private void ucQueryInpatientNo1_myEvent()
        {
            ...
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("该患者处于无费退院状态,不能进行费用减免"));
                this.ucQueryInpatientNo1.Focus();
                return;
            }
            ...
            {
                MessageBox.Show(FS.FrameWork.Management.Language.Msg("该患者已经结算,不能进行费用减免"));
                this.ucQueryInpatientNo1.Focus();
                return;
            }
            ...
            int returnValue = this.feeInpatient.CloseAccount(this.patientInfo.ID);
            {
                if (returnValue == -1)
                {
                    FS.FrameWork.WinForms.Classes.Function.Msg("封帐失败" + this.feeInpatient.Err, 211);
                    return;
                }
            }
            ...
            //{
            //    ArrayList al = this.feeInpatient.QueryFeeInfosAndChangeCostGroupByMinFeeByInpatientNO(patientInfo.ID, "0");
            //    if (al == null)
            //    {
            //        MessageBox.Show(feeInpatient.Err, "提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
            //        return;
            //    }
            //    string errText = string.Empty;
            //     if (Function.SplitFeeItem(patientInfo, al, ref errText)==false)
            //     {
            //         MessageBox.Show(errText, "提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
            //         return;
            //    }
            //}
        ...
        private void btDerateOne_Click(object sender, EventArgs e)
        {
            ...
            {
                //取可减免金额
                decimal totOwncost = FS.FrameWork.Function.NConvert.ToDecimal(this.fpFeeInfo_TotFee.Cells[0, 5].Text);
        ...
        {
            tooBarService.AddToolButton("清屏", "清屏", FS.FrameWork.WinForms.Classes.EnumImageList.Q清空, true, false, null);
        ...
        {
            if (this.fpFeeInfo.ActiveSheet == this.fpFeeInfo_FeeCode&&this.fpFeeInfo_FeeCode.DataSource is DataView)
            {
                ((DataView)this.fpFeeInfo_FeeCode.DataSource).RowFilter = "1=1";
                if (this.cmbMinFee.Text == "全部")
                {
                    ((DataView)this.fpFeeInfo_FeeCode.DataSource).RowFilter = "费用名称 like '%%'";
                }
                else
                {
                    ((DataView)this.fpFeeInfo_FeeCode.DataSource).RowFilter = "费用名称 like '%" + this.cmbMinFee.Text + "%'";
                }

                ((DataView)this.fpFeeInfo_FeeCode.DataSource).RowFilter += " and ( 费用名称 like '%" + this.txtFilter.Text + "%'" +
                                                               " or  拼音码 like '%" + this.txtFilter.Text + "%'" +
                                                               " or  五笔码 like '%" + this.txtFilter.Text + "%')";
            }
            else if (this.fpFeeInfo.ActiveSheet == this.fpFeeInfo_Items && this.fpFeeInfo_Items.DataSource is DataView)
            {
                ((DataView)this.fpFeeInfo_Items.DataSource).RowFilter = "1=1";
                if (this.cmbMinFee.Text == "全部")
                {
                    ((DataView)this.fpFeeInfo_Items.DataSource).RowFilter = "费用名称 like '%%'";
                }
                else
                {
                    ((DataView)this.fpFeeInfo_Items.DataSource).RowFilter = "费用名称 like '%" + this.cmbMinFee.Text + "%'";
                }

                ((DataView)this.fpFeeInfo_Items.DataSource).RowFilter += " and (  项目名称 like '%" + this.txtFilter.Text + "%'" +
                                                               " or  拼音码 like '%" + this.txtFilter.Text + "%'" +
                                                               " or  五笔码 like '%" + this.txtFilter.Text + "%'" +
                                                               " or  自定义码 like '%" + this.txtFilter.Text + "%')";
            }

        }

        void cmbMinFee_TextChanged(object sender, EventArgs e)
        {
            this.txtFilter_TextChanged(null, null);
        }

        void fpFeeInfo_ActiveSheetChanged(object sender, EventArgs e)
        {
