...
namespace FS.SOC.HISFC.InpatientFee.Components.Report.DayBalanceCheck
{
    ...
    /// 门诊收费日结
    /// </summary>
    public partial class ucInpatientBalanceCheck : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Classes.IPreArrange
    {
        public ucInpatientBalanceCheck()
        {
            InitializeComponent();
        }
        ...
        private void loadReportFile()
        {
            if (this.report != null)
            {
                ...
                {
                    report.QueryFilePath = "\\Report\\住院收费报表\\住院收费日结审核查询设置.xml";
                }
            ...
            if (this.showType == EnumShowType.FarPoint)
            {
                ...
                {
                    this.mainDWDataObject = "\\Report\\住院收费报表\\住院收费日结审核.xml";
                }
            }
        }
        ...
        private int check()
        {
            ...
            {
                if (dayReportProcess.InpatientFeeCheck(FS.FrameWork.Management.Connection.Operator, allMap["balanceNO"].ToString()) < 0)
                {
                    CommonController.CreateInstance().MessageBox("住院收费日结审核失败，原因：" + dayReportProcess.Err, MessageBoxIcon.Error);
                    return 1;
                }
            }
        ...
        private int cancelCheck()
        {
            ...
            {
                if (dayReportProcess.InpatientFeeCancelCheck(FS.FrameWork.Management.Connection.Operator, allMap["balanceNO"].ToString()) < 0)
                {
                    CommonController.CreateInstance().MessageBox("取消住院收费日结审核失败，原因：" + dayReportProcess.Err, MessageBoxIcon.Error);
                    return 1;
                }
            }
        ...
        {
            this.toolBarService.AddToolButton("审核", "审核本次数据", (int)FS.FrameWork.WinForms.Classes.EnumImageList.B保存, true, false, null);
            this.toolBarService.AddToolButton("取消审核", "取消审核本次数据", (int)FS.FrameWork.WinForms.Classes.EnumImageList.Q取消, true, false, null);
        ...
        public int PreArrange()
        {
            ...
            {
                CommonController.CreateInstance().MessageBox("您没有住院收费日结审核权限!", MessageBoxIcon.Warning);
                return -1;
            }
            return 1;
        }
            ...
            if (context != null && context.Instance != null && provider != null)
            {
                ...
                if (editorService != null)
                {
                    ...
                    ICommonReportController.ISettingReportForm ISettingReportForm =
                         ICommonReportController.CreateFactory().CreateInferface<ICommonReportController.ISettingReportForm>(typeof(ucInpatientBalanceCheck), null);
