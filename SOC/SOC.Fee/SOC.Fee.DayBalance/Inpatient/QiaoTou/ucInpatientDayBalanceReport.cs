...
namespace SOC.Fee.DayBalance.Inpatient.QiaoTou
{
    ...
    public partial class ucInpatientDayBalanceReport : UserControl
    {
        ...
        protected virtual void SetFpStyle(FarPoint.Win.Spread.SheetView sheet, List<string> lstTitle)
        {
            ...
            try
            {
                ...
                sheet.RowCount += 1;
                //现金结算
                sheet.Cells[sheet.RowCount - 1, 0].Text = "现金金额：";
                sheet.Cells[sheet.RowCount - 1, 1].Tag = EnumCellName.A013;
                //账户结算
                sheet.Cells[sheet.RowCount - 1, 2].Text = "冲销预交金额：";
                sheet.Cells[sheet.RowCount - 1, 3].Tag = EnumCellName.A010;
                ...
                //使用票据号
                sheet.Rows.Count += 1;
                sheet.Cells[sheet.Rows.Count - 1, 0].Text = "使用票据号:";
                sheet.Cells[sheet.Rows.Count - 1, 1].Text = "";
                sheet.AddSpanCell(sheet.Rows.Count - 1, 1, 1, 7);
                sheet.Cells[sheet.Rows.Count - 1, 1].Tag = EnumCellName.A014;
                sheet.Cells[sheet.Rows.Count - 1, 1].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.General;
                sheet.Cells[sheet.Rows.Count - 1, 1].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.General;
                sheet.Rows[sheet.Rows.Count - 1].Height = 40;

                //使用票据号
                sheet.Rows.Count += 1;
                sheet.Cells[sheet.Rows.Count - 1, 0].Text = "作废票据号:";
                sheet.Cells[sheet.Rows.Count - 1, 1].Text = "";
                sheet.AddSpanCell(sheet.Rows.Count - 1, 1, 1, 7);
                sheet.Cells[sheet.Rows.Count - 1, 1].Tag = EnumCellName.A015;
                sheet.Cells[sheet.Rows.Count - 1, 1].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.General;
                sheet.Cells[sheet.Rows.Count - 1, 1].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.General;
                sheet.Rows[sheet.Rows.Count - 1].Height = 40;
                ...
                //统计支付方式
                int iColCount = 4;
                int iRowCount = FS.FrameWork.Function.NConvert.ToInt32(Math.Ceiling(lstTitle.Count * 1.0 / iColCount));
                int iCurrentRow = sheet.Rows.Count;
                sheet.Rows.Count += iRowCount;
        ...
        public virtual void SetFarPoint(List<string> lstTitle)
        {
        ...
        {
            if (this.lstFeecodeStat != null && this.lstFeecodeStat.Count > 0)
            {
                int sortID = 0;
                FS.HISFC.Models.Fee.FeeCodeStat codeStatTemp = null;

                for (int i = 0; i < lstFeecodeStat.Count; i++)
                {
                    codeStatTemp = lstFeecodeStat[i];
                    sortID = codeStatTemp.SortID - 1;

                    int rowIndex = System.Convert.ToInt32(sortID % 3);
                    int colIndex = System.Convert.ToInt32(sortID / 3) * 2;

                    this.neuSpread1_Sheet1.Cells[rowIndex, colIndex].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Center;
                    this.neuSpread1_Sheet1.Cells[rowIndex, colIndex].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Center;
                    this.neuSpread1_Sheet1.Cells[rowIndex, colIndex].Text = codeStatTemp.FeeStat.Name;

                    this.neuSpread1_Sheet1.Cells[rowIndex, colIndex + 1].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Center;
                    this.neuSpread1_Sheet1.Cells[rowIndex, colIndex + 1].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Right;
                    this.neuSpread1_Sheet1.Cells[rowIndex, colIndex + 1].Text = "";
        ...
        /// <param name="lstPactTitle"></param>
        public void ClearALL(string beginDate, string endDate, List<string> lstTitle)
        {
            this.Clear(beginDate,endDate);

            this.SetDetailName();
            this.SetFarPoint(lstTitle);
        }
