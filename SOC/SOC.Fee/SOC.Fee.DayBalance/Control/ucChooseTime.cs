...
{
    public partial class ucChooseTime : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        public ucChooseTime()
        {
            InitializeComponent();
        ...
        {
            DateTime dtnow = (new SOC.Fee.DayBalance.Manager.InpatientDayReport()).GetDateTimeFromSysDateTime();
            if (this.EndDate < this.BeginDate)
            {
                MessageBox.Show("结束时间小于起始时间请重新选择");
                return;
            }
            if (this.EndDate >dtnow)
            {
