...
namespace SOC.Fee.DayBalance.InpatientDayBalance.GYSY
{
    public partial class ucDayBalance : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        /// 报表标题
        /// </summary>
        [Description("发票区间中每行的发票段个数"), Category("报表设置")]
        ...
        public void InitUC()
        {
            ...
            this.SetOneCellText(this.neuSpread1.Sheets[0], EnumOtherInfo.lblPrintTime,this.inpatientDayBalanceManage.GetDateTimeFromSysDateTime().ToString());
        ...
        protected virtual void SetDetial(FarPoint.Win.Spread.SheetView sheet,DataTable table)
        {
            ...
            for (int i = 0; i < table.Rows.Count; i++)
            {
                ...
                countMoney += decTemp;
                //获取发票分类打印序号
                this.cell = sheet.GetCellFromTag(null, table.Rows[i]["SORT_ID"].ToString());
                if (this.cell != null)
                {
        ...
        protected virtual void SetInvoice(FarPoint.Win.Spread.SheetView sheet, DateTime begin, DateTime end)
        {
            ...
            this.SetOneCellText(sheet, EnumOtherInfo.ValidNumber, effectiveBill);
            ...
            //this.SetOneCellText(sheet, EnumOtherInfo.BadNumber, uneffectiveBill);

            string tmp = this.inpatientDayBalanceManage.GetOutReceiptBack(strEmpID, begin.ToString("yyyy-MM-dd HH:mm:ss"), end.ToString("yyyy-MM-dd HH:mm:ss"));
            string[] badInvoice=tmp.Split('|');
            List<string> lstBadInvoice = new List<string>();
            foreach (string invoice in badInvoice)
            {
                if (!string.IsNullOrEmpty(invoice)&&!lstBadInvoice.Contains(invoice))
                {
                    lstBadInvoice.Add(invoice);
                }
            }
            if (lstBadInvoice.Count==0)
            {
                return;
            }
            tmp = GetZone(lstBadInvoice);

            ////显示作废数、作废区间
            this.SetOneCellText(sheet, EnumOtherInfo.BadNumber, lstBadInvoice.Count.ToString());
            this.SetOneCellText(sheet, EnumOtherInfo.InvoiceBadBill, this.SpiltString(tmp));
        ...
        protected virtual void SetInvoiceBeginAndEnd(DataTable dtInvoice, FarPoint.Win.Spread.SheetView sheet)
        {
            ...
            List<string> lstValidInvoice = new List<string>();

            for (idx = 0; idx < iRowCount; idx++)
            {
                drTemp = dtInvoice.Rows[idx];

                strTemp = drTemp["print_invoiceno"].ToString();
                if (!lstInvoice.Contains(strTemp))
                {
                    lstInvoice.Add(strTemp);
                }
               
                if (!lstValidInvoice.Contains(strTemp) && drTemp["waste_flag"].ToString() == "1")
                {
                    lstValidInvoice.Add(strTemp);
                }

            }

            //设置发票数
            this.SetOneCellText(sheet, EnumOtherInfo.lblInvoice, lstInvoice.Count.ToString());
            lblInvoiceInfo.Text = "发票数：     " + lstInvoice.Count.ToString() + "     " + lstInvoice[0] + "  -  " + lstInvoice[lstInvoice.Count - 1];

            //有效发票区间
            string result = GetZone(lstValidInvoice);
            this.SetOneCellText(sheet, EnumOtherInfo.InvoiceValidBill, SpiltString(result));
        ...
        /// <returns></returns>
        private string GetZone(List<string> invoice)
        {
        ...
        protected virtual void SetMoneyValue(FarPoint.Win.Spread.SheetView sheet,DateTime begin,DateTime end)
        {
            ...
            resultValue = this.inpatientDayBalanceManage.QueryDayBalanceBillMoneyForDllname(employeeID, begin.ToString("yyyy-MM-dd HH:mm:ss"), end.ToString("yyyy-MM-dd HH:mm:ss"), out dtBillMoney);
        ...
        protected virtual void SetPayTypeValue(FarPoint.Win.Spread.SheetView sheet, DataTable dtBillMoney)
        {
            for (int i = 0; i < sheet.RowCount; i++)
            {
                for (int j = 0; j < sheet.ColumnCount; j++)
                {
                    ...
                    for(int m=0;m<dtBillMoney.Rows.Count;m++)
                    {
                        ...
                        {
                            if (this.cell.Tag.ToString().Contains(dtBillMoney.Rows[m][0].ToString() + "|" + dtBillMoney.Columns[n].ColumnName.ToString()))
                            {
            ...
            ///记账总金额
            decimal decPubCost = FS.FrameWork.Function.NConvert.ToDecimal(dtBillMoney.Compute("Sum(pubcost)", ""));
            this.SetOneCellText(sheet, EnumOtherInfo.PubCostTot, decPubCost.ToString("0.00"));
            this.SetOneCellText(sheet, EnumOtherInfo.PubCostUpper, FS.FrameWork.Public.String.LowerMoneyToUpper(decPubCost));
            ...
            ///发票总金额
            decimal decInvoiceTot = FS.FrameWork.Function.NConvert.ToDecimal(dtBillMoney.Compute("Sum(totalcost)", ""));
            this.SetOneCellText(sheet, EnumOtherInfo.InvoiceCost, decInvoiceTot.ToString("0.00"));

            ////应收总金额
            decimal decIncome = decInvoiceTot - decPubCost;
            this.SetOneCellText(sheet, EnumOtherInfo.IncomeCost, decIncome.ToString("0.00"));

            ////退费金额
            decimal decReturn = decYJJJS - decIncome;
            this.SetOneCellText(sheet, EnumOtherInfo.BackCost, decReturn.ToString("0.00"));
        ...
        protected virtual void PrintInfo(System.Windows.Forms.Control panelPrint)
        {
            ...
            if (this.neuTabControl1.SelectedIndex == 0 || this.neuTabControl1.SelectedIndex == 1)
            {
                FS.HISFC.Models.Base.PageSize ps = psManager.GetPageSize("ZYRJ");
                if (ps == null)
                {
                    ps = new FS.HISFC.Models.Base.PageSize("ZYRJ", 1040, 850);
                    ps.Top = 0;
                    ps.Left = 0;
                }
                ...
                {
                    print.PrintPreview(ps.Top, ps.Left, panelPrint);
                }
                else
                {
                    print.PrintPage(ps.Top, ps.Left, panelPrint);
                }
            }
            else
            {
                #region 先屏蔽，纸张高度太小的话，核心打印分页出现死循环
                //FS.HISFC.Models.Base.PageSize ps = psManager.GetPageSize("ZYRJMX");
                //if (ps == null)
                //{
                //    ps = new FS.HISFC.Models.Base.PageSize("ZYRJMX", 1040, 850);
                //    ps.Top = 0;
                //    ps.Left = 0;
                //}
                //print.SetPageSize(ps);
