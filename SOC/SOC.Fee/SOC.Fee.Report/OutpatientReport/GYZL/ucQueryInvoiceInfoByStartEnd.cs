...
namespace SOC.Fee.Report.OutpatientReport.GYZL
{
    public partial class ucQueryInvoiceInfoByStartEnd : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        public int QueryReport()
        {
            ...
            bool isDateTime = false;//按时间条件查询

            string invoiceStart = this.tbInvoiceStart.Text.Trim();
            string invoiceEnd = this.tbInvoiceEnd.Text.Trim();

            if (string.IsNullOrEmpty(invoiceStart) && string.IsNullOrEmpty(invoiceEnd))
            {
                isDateTime = true;
            }
            ...
            if (isDateTime)
            {
                DateTime dtBegin = this.beginDate.Value.Date;
                DateTime dtEnd = this.endDate.Value.Date.AddHours(23).AddMinutes(59).AddSeconds(59);
            ...
            if (this.feeMgr.GYZLQueryItemIncomeByDept("GYZL.Report.Clinic.GetCheckInvoice.Select", invoiceStart, invoiceEnd, "", "", ref dsResult) == -1)
            {
            ...
            //if (invoiceStart == "ALL" || invoiceEnd == "ALL")
            //{
            //    this.lblInfo.Text = "打印人：" + this.feeMgr.Operator.Name + "     发票号："  +
            //           (dsResult.Tables[0].Select("print_invoiceno = min(print_invoiceno)"))[0]["print_invoiceno"].ToString()+ " 至 " +
            //           (dsResult.Tables[0].Select("print_invoiceno = max(print_invoiceno)"))[0]["print_invoiceno"].ToString();
            //}
            //else
            //{
            //    this.lblInfo.Text = "打印人：" + this.feeMgr.Operator.Name + "     发票号：" + invoiceStart + " 至 " + invoiceEnd;
            //}
            this.lblInfo.Text = "发票起止号：" + invoiceStart + " — " + invoiceEnd;
