...
{
    public partial class ucQueryBalanceIncomeByOper : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        /// <summary>
        /// 临时处理的报表，有多余代码！chengym
        /// </summary>
        public ucQueryBalanceIncomeByOper()
        {
            InitializeComponent();
        }
        ...
        public int QueryReport()
        {
            ...
            //    strOperID = "ALL";
            //}
            this.SetInvoice(this.neuSpread1_Sheet1);
            //chengym 屏蔽
            //DataSet dsResult = new DataSet();
            //if (this.feeMgr.GYZLQueryItemIncomeByOper(this.sqlID, strOperID,begin, end, ref dsResult) == -1)
            //{
            ...
            this.lblInfo.Text = "开始时间：" + this.beginDate.Value.ToShortDateString() + " 截止时间：" + this.endDate.Value.ToShortDateString();
            ...
            this.lblInfo.Location = new Point(titleX -100, this.lblTitle.Location.Y+20);
        ...
        protected virtual void SetInvoice(FarPoint.Win.Spread.SheetView sheet)
        {
            ...
            if (pnlEmployee.Visible && this.cmbFeeOper.SelectedItem != null)
            {
                strEmpID = this.cmbFeeOper.SelectedItem.ID;
            }
            else
            {
                strEmpID = "ALL";
            ...
            OutPatientDayBalance clinicDayBalance = new OutPatientDayBalance();

            //{014680EC-6381-408b-98FB-A549DAA49B82}
            //发票起止号段
            //int resultValue = clinicDayBalance.GetDayInvoiceBeginEndNum(strEmpID, begin, end, ref ds);
            ...
            foreach (DataRow rows in table.Rows)
            {
                ...
                sheet.Cells[row, 0].Text = rows["oper_name"].ToString();
                sheet.Cells[row, 0].Tag = rows["oper_code"].ToString();
                sheet.Cells[row, 1].Text = rows["start_no"].ToString() + " ~ " + rows["end_no"].ToString();
                sheet.Cells[row, 2].Text = rows["invoiceNumTOT"].ToString();
                sheet.Cells[row, 2].Tag = rows["trans_type"].ToString();
            }
            //根据起止号获取对应的费用
            DataSet ds1 = new DataSet();
            DataTable table1 = new DataTable();
            DataView dv1 = new DataView();
            for(int rown=0;rown<sheet.RowCount;rown++)
            {
                if(sheet.Cells[rown,2].Tag==null)
                {
                    return;
                }
                string tran_type=sheet.Cells[rown,2].Tag.ToString();
                string No = sheet.Cells[rown, 1].Text;
                string[] str=No.Split('~');
                string beginNo = string.Empty;
                ...
                //{014680EC-6381-408b-98FB-A549DAA49B82}
                //int resultValue1 = clinicDayBalance.GetDayInvoiceDataGYZLBeginEndNum(strEmpID, beginNo, endNo, tran_type, ref ds1);
                //if (resultValue1 == -1)
                //{
                //    MessageBox.Show(clinicDayBalance.Err);
                //    return;
                ...
                dv1 = table1.DefaultView;

                sheet.Cells[rown, 3].Text = dv1.ToTable().Compute("sum(记账)", "").ToString();//记账
                sheet.Cells[rown, 4].Text = dv1.ToTable().Compute("sum(中行磁卡)", "").ToString();//中行磁卡
                sheet.Cells[rown, 5].Text = dv1.ToTable().Compute("sum(商行磁卡)", "").ToString();//商行磁卡
                sheet.Cells[rown, 6].Text = dv1.ToTable().Compute("sum(现金)", "").ToString();//现金
                sheet.Cells[rown, 7].Text = dv1.ToTable().Compute("sum(汇款)", "").ToString();//汇款
                sheet.Cells[rown, 8].Text = dv1.ToTable().Compute("sum(支票)", "").ToString();//支票
                sheet.Cells[rown, 9].Text = dv1.ToTable().Compute("sum(退款)", "").ToString();//退款
                sheet.Cells[rown, 10].Text = dv1.ToTable().Compute("sum(合计)", "").ToString();//合计
            }
            ...
            for (int rown1 = 0; rown1 < sheet.RowCount; rown1++)
            {
                if (sheet.Cells[rown1, 2].Tag != null && sheet.Cells[rown1, 2].Tag.ToString() == "1")
                {
                    sheet.Cells[sheet.RowCount - 1, 2].Formula = "SUM(C1:C" + (rown1+1).ToString()+")";
                }
                sheet.Cells[sheet.RowCount - 1, 3].Formula = "SUM(D1:D" + (rown1 + 1).ToString() + ")";//记账
                sheet.Cells[sheet.RowCount - 1, 4].Formula = "SUM(E1:E" + (rown1 + 1).ToString() + ")";//中行磁卡
                sheet.Cells[sheet.RowCount - 1, 5].Formula = "SUM(F1:F" + (rown1 + 1).ToString() + ")";//商行磁卡
                sheet.Cells[sheet.RowCount - 1, 6].Formula = "SUM(G1:G" + (rown1 + 1).ToString() + ")";//现金
                sheet.Cells[sheet.RowCount - 1, 7].Formula = "SUM(H1:H" + (rown1 + 1).ToString() + ")";//汇款
                sheet.Cells[sheet.RowCount - 1, 8].Formula = "SUM(I1:I" + (rown1 + 1).ToString() + ")";//支票
                sheet.Cells[sheet.RowCount - 1, 9].Formula = "SUM(J1:J" + (rown1 + 1).ToString() + ")";//退款
                sheet.Cells[sheet.RowCount - 1, 10].Formula = "SUM(K1:K" + (rown1 + 1).ToString() + ")";//合计
            }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            else if(this.ReportType == "2")
            {
                ...
                */
                ...
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 3].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Center;
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 3].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Center;
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 3].Text = "记账";

                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 4].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Center;
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 4].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Center;
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 4].Text = "中行磁卡";
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 5].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Center;
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 5].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Center;
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 5].Text = "商行磁卡";
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 6].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Center;
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 6].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Center;
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 6].Text = "现金";

                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 7].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Center;
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 7].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Center;
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 7].Text = "汇款";
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 8].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Center;
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 8].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Center;
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 8].Text = "支票";
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 9].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Center;
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 9].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Center;
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 9].Text = "优惠";

                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 10].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Center;
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 10].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Center;
                this.neuSpread1_Sheet1.ColumnHeader.Cells[0, 10].Text = "金额合计";
