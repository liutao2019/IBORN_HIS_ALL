...
{
    class OutPatientDrugStoreConfirm
    {
        /// <summary>
        /// 发送门诊发药确认消息（RGV^O15）
        /// </summary>
        ...
        /// <returns></returns>
        public int ProcessMessage(FS.HISFC.Models.Registration.Register patientInfo, ArrayList alDrugInfo, bool isPositive, ref NHapi.Model.V24.Message.RGV_O15 RGV_O15, ref string errInfo)
        {
            ...
            RGV_O15.PATIENT.PATIENT_VISIT.PV1.PatientClass.Value = "O";//门诊类型
            ...
            foreach (FS.HISFC.Models.Pharmacy.ApplyOut applyOut in alDrugInfo)
            {
                ...
                order.ORDER_DETAIL.RXO.RequestedGiveAmountMinimum.Value = (applyOut.Operation.ApplyQty /applyOut.Item.PackQty).ToString();//数量
                order.ORDER_DETAIL.RXO.RequestedGiveUnits.Identifier.Value = applyOut.Item.PackUnit.ToString();//单位
                order.ORDER_DETAIL.RXO.DeliverToLocation.Facility.UniversalID.Value = applyOut.StockDept.ID;//药房
                order.ORDER_DETAIL.RXO.DeliverToLocation.Facility.NamespaceID.Value = applyOut.ExtFlag1;//发药机出药口
                order.ORDER_DETAIL.RXO.GetOrderingProviderSDEANumber(0).IDNumber.Value = FS.SOC.HISFC.BizProcess.Cache.Pharmacy.GetItem(applyOut.Item.ID).Product.BarCode;//条码
