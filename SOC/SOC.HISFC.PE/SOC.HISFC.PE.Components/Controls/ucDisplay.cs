...
using FS.SOC.Local.OutpatientFee.GuangZhou.Zdly.IOutpatientItemInputAndDisplay;
using FS.SOC.Local.OutpatientFee.GuangZhou.Zdly;
...
namespace FS.SOC.HISFC.PE.Components.Controls
{
    public partial class ucDisplay : UserControl, FS.HISFC.BizProcess.Integrate.FeeInterface.IOutpatientItemInputAndDisplay, FS.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        public ucDisplay()
        {
            ...
            this.tsMenuRebateRate.Click += new EventHandler(tsMenuRebateRate_Click);
        }
        ...
        /// <summary>
        /// 门诊收费项目过滤 第一位过滤检验 第二位过滤检查 后两位预留
        /// </summary>
        protected string conditions = string.Empty;
        ...
        FS.HISFC.BizLogic.Fee.UndrugPackAge undrugMgr = new FS.HISFC.BizLogic.Fee.UndrugPackAge();
        ...
        {
            //门诊收费项目过滤 第一位过滤检验 第二位过滤检查 后两位预留
            this.conditions = this.controlParamIntegrate.GetControlParam<string>("MZSF01", true, "0000");
        ...
        private void SetItem(string itemCode, string drugFlag, string exeDeptCode, int row, decimal amount, decimal saleprice, string unitFlag)
        {
            ...
            if (findRow != null)
            {
                ...
                if (string.IsNullOrEmpty(this.fpSpread1_Sheet1.Cells[row, (int)Columns.ExeDept].Text))
                {
        ...
        private ArrayList ConvertGroupToDetail(FeeItemList f)
        {
            ...
            decimal rate = NConvert.ToDecimal(rowFind["EMERG_SCALE"]);
            int sequence = 0;
            ...
            foreach (FS.HISFC.Models.Fee.Item.UndrugComb undrugCombo in undrugCombList)
            {
                ...
                feeDetail.Item.UserCode = rowFindZT["USER_CODE"].ToString();
                ...
                feeDetail.UndrugComb.Extend1 = f.RecipeNO;
                feeDetail.UndrugComb.Extend2 = sequence.ToString();
                ...
                //公费高级仪器
                feeDetail.FT.FTRate.User03 = f.FT.FTRate.User03;

                if (!string.IsNullOrEmpty(f.User02)&&feeDetail.UndrugComb.ID==f.Item.ID)
                {
                    int i = FS.FrameWork.Function.NConvert.ToInt32(f.User02);
                    if (hsFeeItem.Contains(f.Item.ID + i + feeDetail.Item.ID))
                    {
                        FeeItemList temp = hsFeeItem[f.Item.ID + i + feeDetail.Item.ID] as FeeItemList;
                        feeDetail.ItemRateFlag = temp.ItemRateFlag;
                        feeDetail.NewItemRate = temp.NewItemRate;
                        feeDetail.FT.FTRate.User03 = temp.FT.FTRate.User03;
            ...
            for (int i = 0; i < this.fpSpread1_Sheet1.RowCount; i++)
            {
                if (this.fpSpread1_Sheet1.Rows[i].Tag != null && this.fpSpread1_Sheet1.Rows[i].Tag is FeeItemList)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        {
                            if (((FS.HISFC.Models.Fee.Outpatient.FeeItemList)this.fpSpread1_Sheet1.Rows[i].Tag).Item.SysClass.ID.ToString().Equals(FS.HISFC.Models.Base.EnumSysClass.PCC.ToString()))
                            {
                                this.fpSpread1_Sheet1.SetActiveCell(i, (int)Columns.DoseOnce, false);
                            }
                            else
                            {
        ...
        public void RefreshItemForPact()
        {
            ...
            for (int currRow = 0; currRow < this.fpSpread1_Sheet1.RowCount; currRow++)
            {
                if (this.fpSpread1_Sheet1.Rows[currRow].Tag != null && this.fpSpread1_Sheet1.Rows[currRow].Tag.GetType() == typeof(FeeItemList))
                {
                    ...
                    if (((FeeItemList)this.fpSpread1_Sheet1.Rows[currRow].Tag).OrgPrice <= 0)
                    {
                        ((FeeItemList)this.fpSpread1_Sheet1.Rows[currRow].Tag).OrgPrice =
                            ((FeeItemList)this.fpSpread1_Sheet1.Rows[currRow].Tag).Item.Price;
                    }
        ...
        public int Init()
        {
            ...
            this.tsMenuRebateRate.Visible = Function.IsContainYKDept(myOperator.Dept.ID);
        ...
        protected void chooseItemControl_SelectedItem(string itemCode, string drugFlag, string exeDeptCode, decimal price)
        {
            ...
            #region 根据相关政策,不允许收费员开某些项目 add by yerl
            if (!string.IsNullOrEmpty(conditions))
            {
                if (!string.IsNullOrEmpty(itemCode))
                {
                    FS.HISFC.Models.Fee.Item.Undrug item = unDrugMgr.GetUndrug(itemCode);
                    if (item != null)
                    {
                        if (conditions.Substring(0, 1) == "1" && item.SysClass.ID.ToString() == "UL" && item.UnitFlag == "0")
                        {

                            MessageBox.Show("根据相关规定,不允许在收费界面录入检验项目单项");
                            return;
                        }
                        if (conditions.Substring(1, 1) == "1" && item.SysClass.ID.ToString() == "UC" && item.UnitFlag == "0")
                        {
                            MessageBox.Show("根据相关规定,不允许在收费界面录入检查项目单项");
                            return;
                        }
                    }
                }
            ...
            if (this.fpSpread1_Sheet1.Rows[currRow].Tag != null)
            {
                if (this.fpSpread1_Sheet1.Rows[currRow].Tag is FeeItemList)
                {
                    ...
                    if (feeTemp.RecipeNO != null && feeTemp.RecipeNO != string.Empty && feeTemp.Order.ID != string.Empty)
                    {
                       ...
                       FS.HISFC.Models.Fee.Outpatient.FeeItemList  feeRealTemp = this.outpatientManager.GetFeeItemList(feeTemp.RecipeNO, feeTemp.SequenceNO);
        ...
        public ArrayList GetFeeItemListForCharge()
        {
            ...
            for (int i = 0; i < this.fpSpread1_Sheet1.RowCount; i++)
            {
                ...
                if (f.IsGroup)
                {
                    ...
                    alFeeItemList.AddRange(alDetail);
                ...
                if (this.fpSpread1_Sheet1.Rows[i].Tag != null)
                {
                    if (this.fpSpread1_Sheet1.Rows[i].Tag is FeeItemList)
                    {
                        ...
                        if (fTemp.Item.ID != "999")
                        {
                            ...
                            price *= 1 - pRate.Rate.RebateRate;
                            orgPrice *= 1 - pRate.Rate.RebateRate;//原始价格也要打折，否则拆分金额不正确
        ...
        Hashtable hsFeeItem = new Hashtable();
        /// <summary>
        /// 更新修改比例后的费用.
        /// </summary>
        ...
        public void RefreshNewRate(ArrayList feeDetails)
        {
            ...
            for (int i = 0; i < this.fpSpread1_Sheet1.RowCount; i++)
            {
                if (this.fpSpread1_Sheet1.Rows[i].Tag != null && this.fpSpread1_Sheet1.Rows[i].Tag is FeeItemList)
                {
                        ...
                        for(int j=0;j<=feeDetails.Count-1;++j)
                        {
                            FeeItemList f = feeDetails[j] as FeeItemList;
                            if (f.ItemRateFlag=="3"&&string.IsNullOrEmpty(f.UndrugComb.ID) &&
                                ((FeeItemList)this.fpSpread1_Sheet1.Rows[i].Tag).Item.ID == f.Item.ID)
                            {
                                if (!hsFeeItem.Contains(f.Item.ID + j + f.Item.ID))
                                {
                                    ((FeeItemList)this.fpSpread1_Sheet1.Rows[i].Tag).NewItemRate = f.NewItemRate;
                                    ((FeeItemList)this.fpSpread1_Sheet1.Rows[i].Tag).ItemRateFlag = f.ItemRateFlag;
                                    this.SetItemRateInfo(i, (FeeItemList)this.fpSpread1_Sheet1.Rows[i].Tag);

                                    hsFeeItem.Add(f.Item.ID + j + f.Item.ID, f);
                                    break;
                                }
                                else 
                                {
                            ...
                            {
                                //组套或者复合项
                                if (((FeeItemList)this.fpSpread1_Sheet1.Rows[i].Tag).Item.Name == f.UndrugComb.Name && f.ItemRateFlag == "3")
                                {
                                    //保存 key为组套ID+farpoint的行号 +f.item.id
                                    string key = ((FeeItemList)this.fpSpread1_Sheet1.Rows[i].Tag).Item.ID;
                                    if (!hsFeeItem.Contains(key + i + f.Item.ID))
                                    {
                                        hsFeeItem.Add(key + i + f.Item.ID, f);
                                    }
                                }
                            }
                        }
        ...
        public ArrayList GetFeeItemList()
        {
            ...
            for (int i = 0; i < this.fpSpread1_Sheet1.RowCount; i++)
            {
                ...
                if (this.fpSpread1_Sheet1.Rows[i].Tag is FeeItemList)
                {
                    ...
                    //if (f.IsGroup)
                    //{
                    //    if(string.IsNullOrEmpty(f.User02))
                    //    {
                    //       f.User02 = i.ToString();//第一次修改公费比例
                    //    }
                    //    ArrayList alDetail = ConvertGroupToDetail(f);
                    //    if (alDetail == null)
                    //    {
        ...
        /// </summary>
        /// <returns></returns>
        public ArrayList GetFeeItemList(bool isGroupDetail)
        {
            ArrayList feeItemLists = new ArrayList();
        ...
        public void RefreshSeeDoc(string recipeSeq, string deptCode, FS.FrameWork.Models.NeuObject obj)
        {
            ...
            for (int i = 0; i < this.fpSpread1_Sheet1.RowCount; i++)
            {
                ...
                if (this.fpSpread1_Sheet1.Rows[i].Tag is FeeItemList)
                {
                    ...
                    {
                        if (string.IsNullOrEmpty(deptCode) == false)
                        {
        ...
        //获取医保记账类型
        private string GetGFItemTypeName(string itemGradeCode, decimal newRate)
        {
            switch (itemGradeCode)
            {
                case "1":
        ...
        private void SetItemRateInfo(int row, FeeItemList feeItemList)
        {
            ...
            if (this.rInfo.Pact.PayKind.ID == "01")//自费
            {
                ...
                if (this.fpSpread1_Sheet1.Cells[row, (int)Columns.Self].Text == "未对照")
                {
            ...
            if (this.rInfo.Pact.PayKind.ID == "02")
            {
                if (feeItemList.Compare == null)
                {
                    feeItemList.ItemRateFlag = "1";
                    feeItemList.NewItemRate = 1;
                    this.fpSpread1_Sheet1.Cells[row, (int)Columns.Self].Text = "未对照";
                    //  this.fpSpread1_Sheet1.Cells[row, (int)Columns.Self].ForeColor = Color.Red;
                }
                else
                {
                    ...
                    {
                        feeItemList.ItemRateFlag = "2";
                        ArrayList alFees = undrugMgr.QueryUndrugPackagesBypackageCode(feeItemList.Item.ID);
                        if (alFees.Count == 0)
                        {
                ...
                if (this.fpSpread1_Sheet1.Cells[row, (int)Columns.Self].Text == "未对照")
                {
                    this.fpSpread1_Sheet1.Rows[row].ForeColor = Color.Red;
                }
                if (this.fpSpread1_Sheet1.Cells[row, (int)Columns.SIPactType].Text == "丙类" && this.fpSpread1_Sheet1.Cells[row, (int)Columns.Self].Text == "")
                {
                    this.fpSpread1_Sheet1.Cells[row, (int)Columns.Self].Text = "自费";
                }
            ...
            //公费类型
            this.fpSpread1_Sheet1.Cells[row, (int)Columns.GFPactType].Value = this.GetGFItemTypeName(feeItemList.Compare.CenterItem.ItemGrade, feeItemList.NewItemRate);
            //医保类型
                        ...
                        if (currColumn == (int)Columns.Days)
                        {
                            ...
                            if (feeItem.Item.ItemType == EnumItemType.Drug)
                            {
                                ...
                                if (feeItem.Item.SysClass.ID.ToString() == "PCC" && !(feeItem.Item as FS.HISFC.Models.Pharmacy.Item).IsNostrum)
                                {
                                    ...
                                    for (int i = 0; i < this.fpSpread1_Sheet1.RowCount; i++)
                                    {
                                        FS.HISFC.Models.Fee.Outpatient.FeeItemList feeItemRow = this.fpSpread1_Sheet1.Rows[i].Tag as FS.HISFC.Models.Fee.Outpatient.FeeItemList;
                                        if (feeItemRow != null && feeItemRow.Order.Combo.ID.Equals(feeItem.Order.Combo.ID))
                                        {
                                            isDealCellChange = false;
                                            this.fpSpread1_Sheet1.Cells[i, (int)Columns.Days].Value = days;
                                            isDealCellChange = true;
                                            qty = NConvert.ToDecimal(FS.FrameWork.Public.String.ExpressionVal(this.fpSpread1_Sheet1.Cells[i, (int)Columns.DoseOnce].Text.ToString()));
                                            qty = FS.FrameWork.Public.String.FormatNumber(qty, 2);
                                            // {1FAD3FA2-C7D8-4cac-845F-B9EBECDE2312}
                                            totQty = qty * days / ((feeItemRow.Item as FS.HISFC.Models.Pharmacy.Item).BaseDose == 0 ? 1 : (feeItemRow.Item as FS.HISFC.Models.Pharmacy.Item).BaseDose);
                                            //totQty = qty * days;
                                            feeItemRow.Item.Qty = totQty;
                                            feeItem.Days = days;
                                            FS.HISFC.Models.Base.FT ft = this.ComputCost(feeItemRow.Item.Price, totQty, feeItemRow);
                                            if (ft == null)
                                            {
                                                this.fpSpread1.Select();
                                                this.fpSpread1.Focus();
                                            ...
                                            feeItemRow.FT.TotCost = ft.TotCost;
                                            feeItemRow.FT.OwnCost = ft.OwnCost;
                                            feeItemRow.FT.PubCost = ft.PubCost;
                                            feeItemRow.FT.PayCost = ft.PayCost;
                                            //add by niuxy处理优惠
                                            feeItemRow.FT.RebateCost = ft.RebateCost;
                                            this.fpSpread1_Sheet1.Cells[i, (int)Columns.Cost].Value = ft.TotCost;
                                            this.fpSpread1_Sheet1.Cells[i, (int)Columns.Amount].Value = totQty;
                                        }
                                    }
                        ...
                        if (currColumn == (int)Columns.DoseOnce)
                        {
                            ...
                            if (feeItem.Item.ItemType == EnumItemType.Drug)
                            {
                                ...
                                if (feeItem.Item.SysClass.ID.ToString() == "PCC" && !(feeItem.Item as FS.HISFC.Models.Pharmacy.Item).IsNostrum)
                                {
                                    ...
                                    if (FS.FrameWork.Function.NConvert.ToDecimal(this.fpSpread1_Sheet1.Cells[currRow, (int)Columns.Days].Value) > 0&&currRow>0)
                                    {
                        ...
                        else
                        {
                            ...
                            if (!this.isCanInputNegativeQty)
                            {
                                if (qty <= 0)
                                {
                                    ...
                                    this.fpSpread1.Select();
                                    this.fpSpread1.Focus();
                                    if (this.fpSpread1_Sheet1.Rows[e.Row].Tag is FS.HISFC.Models.Fee.Outpatient.FeeItemList)
                                    {
                                        if (((FS.HISFC.Models.Fee.Outpatient.FeeItemList)this.fpSpread1_Sheet1.Rows[e.Row].Tag).Item.SysClass.ID.ToString().Equals(FS.HISFC.Models.Base.EnumSysClass.PCC.ToString()))
                                        {
                                            this.fpSpread1_Sheet1.SetActiveCell(e.Row, (int)Columns.DoseOnce, false);
                                        }
                                        else
                                        {
        ...
        void tsMenuRebateRate_Click(object sender, EventArgs e)
        {
            ArrayList alFeeItem = this.GetFeeItemList(false);
            ucModifyRate ucModifyRate = new ucModifyRate();
            ucModifyRate.SetFeeDetail(alFeeItem);
            FS.FrameWork.WinForms.Classes.Function.ShowControl(ucModifyRate, FormBorderStyle.FixedToolWindow, FormWindowState.Normal);
            this.ModifyPrice();
        }

        #endregion

        private void tsMenuFeeItems_Click(object sender, EventArgs e)
        {
            FS.HISFC.Models.Fee.Outpatient.FeeItemList f = this.fpSpread1_Sheet1.Rows[this.fpSpread1_Sheet1.ActiveRow.Index].Tag as FS.HISFC.Models.Fee.Outpatient.FeeItemList;
            if (f != null && !string.IsNullOrEmpty(f.Item.ID))
            {

                ArrayList alFees = undrugMgr.QueryUndrugPackagesBypackageCode(f.Item.ID);
                if (alFees.Count > 0)
                {
                    //弹出费用明细显示框
                    frmFeeItems feeitems = new frmFeeItems();
                    feeitems.Register = this.rInfo;
                    feeitems.FItem = f;
                    feeitems.UndrugCombs = alFees;
        ...
        public void PreCountInvos()
        {
            ...
            {
                alInvoices = this.outpatientManager.QueryBalancesByInvoiceNO(frmChoosePatientinfo.CardNo);
                if (alInvoices == null)
                {
            ...
            this.ChargeInfoList = preFeeItemList;
            #endregion
