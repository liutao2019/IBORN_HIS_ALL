           ...
           //}
            ...
            if (feeDetails.Count > 0)
            {
                ...
                else
                {
                    ...
                    if (FS.HISFC.Components.OutpatientFee.InterfaceManager.GetIOrder().SendFeeInfo(this.patient, oldFeeItemLists, false) < 0)
                    {
                        FS.FrameWork.Management.PublicTrans.RollBack();
                        this.medcareInterfaceProxy.Rollback();
                        MessageBox.Show(this, "退费失败，请向系统管理员报告错误信息：" + FS.HISFC.Components.OutpatientFee.InterfaceManager.GetIOrder().Err, "提示>>", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return -1;
                    }
                    //{014680EC-6381-408b-98FB-A549DAA49B82}
                    ...
                    returnCostString = "应退金额: " + FS.HISFC.Components.OutpatientFee.Class.Function.DealCent(returnCost).ToString();
                ...
                foreach (FS.HISFC.Models.Fee.Outpatient.FeeItemList item in feeDetails)
                {
                    ...
                    {
                        MessageBox.Show("调用惠侨体检接口退费出错，请联系对方负责人！退费项目："+ item.Item.Name);
                    }
                }
                #endregion
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    returnCostString = "应退金额: " + (caCost + chCost + cdCost + dbCost + xxCost).ToString("F2") + System.Environment.NewLine +
                        "其中：" + System.Environment.NewLine +
                        "现金{0}" + System.Environment.NewLine +
                        "中行磁卡{1}" + System.Environment.NewLine +
                        "商行磁卡{2}" + System.Environment.NewLine +
                        "支票{3}" + System.Environment.NewLine +
                        "记账{4}" + System.Environment.NewLine
                        ;
                ...
                if (!"0".Equals(peResult))
                {
                    MessageBox.Show("调用惠侨体检接口退费出错，请联系对方负责人！");
                }
                #endregion
            }
