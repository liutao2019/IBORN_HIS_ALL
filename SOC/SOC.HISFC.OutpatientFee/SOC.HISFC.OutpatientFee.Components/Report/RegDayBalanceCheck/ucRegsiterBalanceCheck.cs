...
{
    public partial class ucRegsiterBalanceCheck : FS.FrameWork.WinForms.Controls.ucBaseControl, FS.FrameWork.WinForms.Classes.IPreArrange
    {
        public ucRegsiterBalanceCheck()
        {
            InitializeComponent();
        }
        ...
        private void loadReportFile()
        {
            if (this.report != null)
            {
                ...
                {
                    report.QueryFilePath = "\\Report\\门诊收费报表\\门诊挂号日结审核查询设置.xml";
                }
            ...
            if (this.showType == EnumShowType.FarPoint)
            {
                ...
                {
                    this.mainDWDataObject = "\\Report\\门诊收费报表\\门诊挂号日结审核.xml";
                }
            }
        }
        ...
        private int check()
        {
            ...
            {
                //if (Function.GetDayReportBizProcess().RegisterCheck(FS.FrameWork.Management.Connection.Operator, allMap["balanceNO"].ToString()) < 0)
                //{
                CommonController.CreateInstance().MessageBox("挂号日结审核失败，原因：" + Function.GetDayReportBizProcess().Err, MessageBoxIcon.Error);
                //    return 1;
                //}
        ...
        private int cancelCheck()
        {
            ...
            {
                //if (Function.GetDayReportBizProcess().RegisterCancelCheck(FS.FrameWork.Management.Connection.Operator, allMap["balanceNO"].ToString()) < 0)
                //{
                CommonController.CreateInstance().MessageBox("取消挂号日结审核失败，原因：" + Function.GetDayReportBizProcess().Err, MessageBoxIcon.Error);
                //    return 1;
                //}
        ...
        protected override FS.FrameWork.WinForms.Forms.ToolBarService OnInit(object sender, object neuObject, object param)
        {
            ...
            this.toolBarService.AddToolButton("取消审核", "取消本次审核数据", (int)FS.FrameWork.WinForms.Classes.EnumImageList.Q取消, true, false, null);
        ...
        public int PreArrange()
        {
            ...
            {
                CommonController.CreateInstance().MessageBox("您没有挂号日结审核权限!", MessageBoxIcon.Warning);
                return -1;
            }
            return 1;
        }
