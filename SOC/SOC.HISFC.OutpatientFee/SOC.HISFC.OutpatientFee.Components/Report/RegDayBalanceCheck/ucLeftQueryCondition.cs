...
namespace FS.SOC.HISFC.OutpatientFee.Components.Report.RegDayBalanceCheck
{
    public partial class ucLeftQueryCondition : FS.FrameWork.WinForms.Controls.ucBaseControl, ILeftQueryCondition
    {
        ...
        private int query()
        {
            ...
            parentNode.Text = this.dtBeginTime.Value.ToString("yyyy年MM月dd日");

            string error = string.Empty;
            List<FS.FrameWork.Models.NeuObject> list = null;// Function.GetDayReportBizProcess().GetRegisterBalanceListByDate(this.dtBeginTime.Value);
            if (list == null)
            {
