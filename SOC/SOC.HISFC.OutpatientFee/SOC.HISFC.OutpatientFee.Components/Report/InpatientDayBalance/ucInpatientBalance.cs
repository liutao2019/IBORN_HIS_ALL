    ...
    public partial class ucInpatientBalance : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        private int balance()
        {
            if (CommonController.CreateInstance().MessageBox("确认日结？", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                int i = Function.GetDayReportBizProcess().GetLastInpatientBalanceDateAndNO(FS.FrameWork.Management.Connection.Operator.ID, ref dtBeginTime, ref balanceNO);
                if (i < 0)
                {
                ...
                //开始日结
                if (Function.GetDayReportBizProcess().InpatientInvoice(FS.FrameWork.Management.Connection.Operator, dtBeginTime, dtEndTime) < 0)
                {
        ...
        private int cancelBalance()
        {
            if (CommonController.CreateInstance().MessageBox("确认取消上一次日结？", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    //开始取消日结
                    if (Function.GetDayReportBizProcess().CancelInpatientInvoice(FS.FrameWork.Management.Connection.Operator, balanceNO) < 0)
                    {
        ...
        private void ILeftQueryCondition_OnFilterHandler(string filterStr)
        {
            ...
            string error = string.Empty;
            if (Function.GetDayReportBizProcess().GetInpatientBalanceDate(filterStr, ref dtBeginTime, ref dtEndTime, ref operCode) < 0)
            {
