...
namespace FS.SOC.HISFC.OutpatientFee.Components.Report.DayBalance
{
    ...
    /// 门诊收费日结
    /// </summary>
    public partial class ucClinicFeeBalance : FS.FrameWork.WinForms.Controls.ucBaseControl
    {
        public ucClinicFeeBalance()
        {
            InitializeComponent();
        }
            ...
            if (this.report != null)
            {
                ...
                if (string.IsNullOrEmpty(this.report.QueryFilePath) == false)
                {
                    ...
                    else
                    {
                        ...
                        this.report.PrintInfo = r.PrintInfo;
                        sr.Close();
                    }
                }
            }
        ...
        private int balance()
        {
            if (CommonController.CreateInstance().MessageBox("确认日结？", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                int i = Function.GetDayReportBizProcess().GetClinicFeeLastBalanceDateAndNO(FS.FrameWork.Management.Connection.Operator.ID,hospitalid, ref dtBeginTime, ref balanceNO);
                if (i < 0)
                {
                ...
                //开始日结
                if (Function.GetDayReportBizProcess().ClinicFee(FS.FrameWork.Management.Connection.Operator, dtBeginTime, dtEndTime) < 0)
                {
        ...
        private int cancelBalance()
        {
            if (CommonController.CreateInstance().MessageBox("确认取消上一次日结？", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    //开始取消日结
                    if (Function.GetDayReportBizProcess().CancelClinicFee(FS.FrameWork.Management.Connection.Operator, balanceNO) < 0)
                    {
