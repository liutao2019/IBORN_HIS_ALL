...
namespace FS.SOC.HISFC.OutpatientFee.Components.Report.Common.Helper
{
    public class DataSetHelper
    {
        ...
        //Private MethodsPrivate MethodsPrivate Methods
        ...
        //SelectDistinct / DistinctSelectDistinct / DistinctSelectDistinct / Distinct
        ...
        {
            DataTable dt = new DataTable(tableName);
            object[] values = new object[fieldIndexs.Length];
            string fields = "";
            for (int i = 0; i < fieldIndexs.Length; i++)
            {
                dt.Columns.Add(sourceTable.Columns[fieldIndexs[i]].Caption, sourceTable.Columns[fieldIndexs[i]].DataType);
                fields += sourceTable.Columns[fieldIndexs[i]].Caption + ",";
            }
            fields = fields.Remove(fields.Length - 1, 1);
            DataRow lastRow = null;
            foreach (DataRow dr in sourceTable.Select("", fields))
        ...
        //Select Table IntoSelect Table IntoSelect Table Into
        ...
        /// <returns>新DataTable</returns> 
        public DataTable SelectInto(string tableName, DataTable sourceTable,
                                    string[] fieldLists, string rowFilter, string sort)
        {
            object[] values = new object[fieldLists.Length];
            string fields = string.Empty;
            for (int i = 0; i < fieldLists.Length; i++)
            {
                //dt.Columns.Add(fieldList[i], sourceTable.Columns[fieldList[i]].DataType);
                fields += fieldLists[i] + ",";
            }
            string fieldList = string.Empty;
            fieldList = fields.Remove(fields.Length - 1, 1);
            DataTable dt = CreateTable(tableName, sourceTable, fieldList);
            InsertInto(dt, sourceTable, fieldList, rowFilter, sort);
            return dt;
        }
        ...
        /// <param name="sort">排序</param> 
        /// <returns>新DataTable</returns> 
        public DataTable SelectIntoByIndex(string tableName, DataTable sourceTable,
                                    string[] fieldIndexs, string rowFilter, string sort)
        {
            object[] values = new object[fieldIndexs.Length];
            string fields = "";
            for (int i = 0; i < fieldIndexs.Length; i++)
            {
                fields += sourceTable.Columns[int.Parse(fieldIndexs[i])].Caption + ",";
            }
            string fieldList = string.Empty;
            fieldList = fields.Remove(fields.Length - 1, 1);
            DataTable dt = CreateTable(tableName, sourceTable, fieldList);
        ...
        //Group By TableGroup By TableGroup By Table
        ...
        //Join TablesJoin TablesJoin Tables
        #region Join Tables
        ...
        //Create TableCreate TableCreate Table
        #region Create Table
