...
namespace FS.SOC.HISFC.Fee.Components.Maintenance.ItemGroup
{
    ...
    /// 说明：
    /// </summary>
     partial class ucItemGroupDetailNew : FS.FrameWork.WinForms.Controls.ucBaseControl,FS.SOC.HISFC.Fee.Interface.Components.IItemGroupDetail
    {
         public ucItemGroupDetailNew()
         {
             InitializeComponent();
         }
        ...
        private void initDataTable()
        {
            ...
            this.dtItems.Columns.AddRange(new DataColumn[] { 
                                                   ...
                                                   new DataColumn("价格一", typeof(string)),
                                                   new DataColumn("价格二", typeof(string)),
                                                   new DataColumn("价格三", typeof(string)),
        ...
        private int setDataRowValue(FS.HISFC.Models.Fee.Item.UndrugComb item, DataRow row)
        {
            ...
            row["顺序号"] = item.SortID;
            row["备注"] = item.Memo;
            row["价格一"] = item.Price.ToString("F4");
            row["价格二"] = item.ChildPrice.ToString("F4");
            row["价格三"] = item.SpecialPrice.ToString("F4");
