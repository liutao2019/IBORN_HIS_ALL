...
{    
    /// <summary>
    /// [功能描述:医保费用对照显示界面]<br></br>
    /// [创 建 者: ma_d]<br></br>
    /// [创建时间: 2012-7]<br></br>
    /// 说明：
    /// </summary>
    public partial class ucFeeItem : FS.FrameWork.WinForms.Controls.ucBaseControl, Fee.Interface.Components.IPactCompareItem
    {
        public ucFeeItem(int itemtype)
        {
        ...
        /// Fp设置文件
        /// </summary>
        private string settingFile = FS.FrameWork.WinForms.Classes.Function.SettingPath+ "SOC.Fee.CompareItem.FeeItem.xml";
        ...
        private void initDataTable()
        {
            ...
            this.dtItems.Columns.AddRange(new DataColumn[] { 
                                                   ...
                                                   new DataColumn("医保等级", typeof(string)),
                                                   new DataColumn("自付比例", typeof(string)),
                                                   new DataColumn("对照标识", typeof(string)),
        ...
        private void initBaseData()
        {
            ...
            FS.SOC.HISFC.Fee.BizLogic.PYCompareItem compareItemMgr = new FS.SOC.HISFC.Fee.BizLogic.PYCompareItem();
            //项目
            ...
            else 
            {
                ...
                {
                    itemList = compareItemMgr.QueryCenterItems(strPacts);
                }
            }
            if (itemList == null)
            {
        ...
        private void loadBaseData(List<FS.SOC.HISFC.Fee.Models.CompareItemModel> itemList)
        {
            ...
            foreach (FS.SOC.HISFC.Fee.Models.CompareItemModel item in itemList)
            {
                //FS.FrameWork.WinForms.Classes.Function.ShowWaitForm(progress++, count);
                if (this.addItemObjectToDtItem(item) != 1)
                {
                    continue;
                }
            }
        ...
        private int addItemObjectToDtItem(FS.SOC.HISFC.Fee.Models.CompareItemModel item)
        {
            return this.addItemObjectToDtItem(item, false);
        }

        private int addItemObjectToDtItem(FS.SOC.HISFC.Fee.Models.CompareItemModel item, bool isModify)
        {
            if (item == null)
            {
                CommonController.CreateInstance().MessageBox("向DataTable中添加项目基本信息失败：项目为null", System.Windows.Forms.MessageBoxIcon.Error);
                return -1;
            }
            if (this.dtItems == null)
            {
                CommonController.CreateInstance().MessageBox("向DataTable中添加项目基本信息失败：DataTable为null", System.Windows.Forms.MessageBoxIcon.Error);
                return -1;
            }
            if (!this.hsItem.Contains(item.HisCode))
            {             
                this.hsItem.Add(item.HisCode, item);
            }
        ...
        private int setItemDataRowValue(FS.SOC.HISFC.Fee.Models.CompareItemModel pactItemRate)
        {
            ...
            row["国家编码"] = pactItemRate.WBCode;
            string ybgrade = "";
            if (!string.IsNullOrEmpty(pactItemRate.User01))
            {
                if (pactItemRate.User01=="1")
                {
                    ybgrade = "甲类";
                }
                else if (pactItemRate.User01=="2")
                {
                    ybgrade = "乙类";
                }
                else if (pactItemRate.User01 == "3")
                {
                    ybgrade = "自费";
                }
                else 
            ...
            row["医保等级"] = ybgrade;
            string zfpay = "";
            if (!string.IsNullOrEmpty(pactItemRate.User02))
            {
                zfpay = (FS.FrameWork.Function.NConvert.ToDecimal(pactItemRate.User02) * 100).ToString() + "%";
            }
            row["自付比例"] = zfpay;
            row["对照标识"] = pactItemRate.User03;
            row["规格"] = pactItemRate.Memo;
            return 1;
        }
        ...
        public void filterItem()
        {
            ...
            if (this.isUnCompare && this.item_type == 1)
            {
                this.lblMemo.Text = "未对照本地项目";
                filter = Function.ConnectFilterStr(filter, "对照标识 like '0'", "and");
            }
            else if(this.item_type==1)
            {
                this.lblMemo.Text = "全部医院项目";
            }
        ...
        void neuSpread_SelectionChanged(object sender, FarPoint.Win.Spread.SelectionChangedEventArgs e)
        {
            if (this.SelectedItemChingedEvent != null)
            {
                ...
                FS.SOC.HISFC.Fee.Models.CompareItemModel item=hsItem[key] as FS.SOC.HISFC.Fee.Models.CompareItemModel;
        ...
        bool isUnCompare = false;
        public bool IsUnCompare
        {
            set { isUnCompare = value; }
        }
        ...
        {
                this.neuSpread.ExportExcel(FarPoint.Win.Spread.Model.IncludeHeaders.ColumnHeadersCustomOnly);
