...
{
    /// <summary>
    /// [功能描述:番禺医保对照项目比例SOC业务层，从核心版本独立出来，只对SOC层有效]<br></br>
    /// [创 建 者: zhaoj]<br></br>
    /// [创建时间: 2011-10]<br></br>
    /// </summary>
    public class PYCompareItem : FS.FrameWork.Management.Database
    {
        ...
        private object[] getParams(FS.SOC.HISFC.Fee.Models.CompareItemModel info)
        {
            return new Object[] { 
                   info.PactCode, 
                    ...
                    info.CenterItemType, 
                    this.Operator.ID,
                    info.User01,//医保等级 1：甲类 2：乙类 3：自费
                    info.User02 //自付比例
            }
                ;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public List<FS.SOC.HISFC.Fee.Models.CompareItemModel> QueryLocalItems()
        {
            string strSql = FS.SOC.HISFC.Fee.Data.AbstractPYCompareItem.Current.QueryLocalItems;
            ...
            try
            {
                ...
                while (this.Reader.Read())
                {
                    ...
                    info.TypeCode = Reader[0].ToString();
                    info.HisCode = Reader[1].ToString();
                    info.HisName = Reader[2].ToString();
                    info.SpellCode = this.Reader[3].ToString();
                    info.WBCode = this.Reader[4].ToString();//目前保存国家编码
                    info.UserCode = this.Reader[5].ToString();
                    info.User03 = this.Reader[6].ToString();
                    info.Memo = this.Reader[7].ToString();
        ...
        /// </summary>
        /// <returns></returns>
        public List<FS.SOC.HISFC.Fee.Models.CompareItemModel> QueryCenterItems(string strPacts)
        {
            string strSql = FS.SOC.HISFC.Fee.Data.AbstractPYCompareItem.Current.QueryCenterItems;

            try
            {
                if (this.ExecQuery(string.Format(strSql,strPacts)) < 0)
                {
                    this.WriteDebug(this.Err);
                    return null;
                }
                ...
                while (this.Reader.Read())
                {
                    ...
                    info.UserCode = this.Reader[5].ToString();
                    info.CenterItemType = this.Reader[6].ToString();
                    info.User01 = this.Reader[7].ToString();//医保登记
                    info.User02 = this.Reader[8].ToString();//自付比例
        ...
        {
            string strSql = FS.SOC.HISFC.Fee.Data.AbstractPYCompareItem.Current.QueryComparedItems;
        ...
        {
            string strSql = FS.SOC.HISFC.Fee.Data.AbstractPYCompareItem.Current.QueryItemsByType;
            try
            {
        ...
        {
            string strSql = FS.SOC.HISFC.Fee.Data.AbstractPYCompareItem.Current.QueryPacts;
        ...
        {
            string strSql = FS.SOC.HISFC.Fee.Data.AbstractPYCompareItem.Current.QueryCenterTypes;
        ...
        {
            string strSql = FS.SOC.HISFC.Fee.Data.AbstractPYCompareItem.Current.Delete;
        ...
        {
            string strSql = FS.SOC.HISFC.Fee.Data.AbstractPYCompareItem.Current.Insert;
            try
            {
        ...
        {
            string strSql = FS.SOC.HISFC.Fee.Data.AbstractPYCompareItem.Current.Update;
            try
            {
                strSql = string.Format(strSql, this.getParams(info));
            }
