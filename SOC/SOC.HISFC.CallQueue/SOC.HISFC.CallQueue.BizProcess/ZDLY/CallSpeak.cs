...
namespace FS.SOC.HISFC.CallQueue.BizProcess.ZDLY
{
    /// <summary>
    /// 语音叫号
    /// </summary>
    public class CallSpeak:ICallSpeak
    {
        ...
        {
            string speakText = "请 ";
            for(int i=0;i<nurseAssign.PatientName.Length;i++)
            {
                if (System.Text.RegularExpressions.Regex.IsMatch(nurseAssign.PatientName.Substring(i, 1).ToString(), "[^\x00-\xff]"))
                {
                    speakText += nurseAssign.PatientName.Substring(i, 1) + " ";
                }
                else 
                {
                    speakText += nurseAssign.PatientName.Substring(i, 1);
            ...
            speakText += "到" + nurseAssign.Room.Name + "就诊";
